...
// Type of blending
enum EBlendingType { EBlend_FadeOut, EBlend_ToEnd, EBlend_Stop };
...
public:
    ...
    void EnableEventProcessing(bool bEnable) {
        m_bEnableEventProcessing = bEnable;
    }
...
public:
    ...
    bool IsEnd() {
        return (m_itVec == m_Vec.end());
    };
    const char* Next() {
        return IsEnd() ? "" : (*m_itVec++).c_str();
    };
    void MoveFirst() {
        m_itVec = m_Vec.begin();
    };
...
public:
    CSmartCriticalSection(CRITICAL_SECTION& CS) {
        m_pCS = &CS;
