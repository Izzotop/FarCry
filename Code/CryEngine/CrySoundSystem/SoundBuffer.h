...
enum BufferType { btNONE, btSAMPLE, btSTREAM };
...
public:
    ...
    SSoundBufferProps GetProps() {
        return (m_Props);
    }
    CS_SAMPLE* GetSample() {
        return (m_Type == btSAMPLE) ? m_Data.m_pSample : nullptr;
    }
    CS_STREAM* GetStream() {
        return (m_Type == btSTREAM) ? m_Data.m_pStream : nullptr;
    }
    BufferType GetType() {
        return m_Type;
    }
    ...
    bool NotLoaded() {
        return (m_Data.m_pData == nullptr) && (m_pReadStream == 0);
    }
    bool Loaded() {
        return (m_Data.m_pData != nullptr) && (m_pReadStream == 0);
    }
