...
#define CHECK_LOADED(_func, _retval)                                                                                                                                               \
    if (!m_pSound->Loaded()) {                                                                                                                                                     \
        TRACE("%s(%d) Warning: %s called without a sound (%s) being fully loaded !", __FILE__, __LINE__, #_func, GetName());                                                       \
        return _retval;                                                                                                                                                            \
    }
...
void CSound::Play(float fRatio, bool bForceActiveState, bool bSetRatio) {
    ...
    if (m_pSound->Loading()) {
        ...
        if (!m_pSound->WaitForLoad())
                return;
        if (m_pSSys->m_pCVarDebugSound->GetIVal())
                m_pSSys->m_pILog->Log("Warning: Synchroneous reading of %s due to Play() call before loading was finished stalled for %1.3f milliseconds !", GetName(),
        (m_pTimer->GetAsyncCurTime()-fTime)*1000.0f);
        */
    }
    // CHECK_LOADED(Play);
    // UpdatePosition();
    ...
    if (bStartSound) {
        ...
        case btSTREAM:
            if ((!m_bLoop) || (!IsPlayingOnChannel())) {
                if (bCanPlay) {
                    m_nChannel = CS_Stream_PlayEx(CS_FREE, m_pSound->GetStream(), nullptr, true);
                    m_PlayingChannels++;
        ...
        case btSAMPLE: {
            if (m_nFlags & FLAG_SOUND_3D) {
                ...
                if ((!m_bLoop) || (!IsPlayingOnChannel())) {
                    if (bCanPlay) {
                        m_nChannel = CS_PlaySoundEx(CS_FREE, m_pSound->GetSample(), nullptr, true); //,m_pSSys->GetDSPUnitFilter(),FALSE);
                        m_PlayingChannels++;
                        m_fChannelPlayTime = currTime;
                        m_fSoundLengthSec = GetLengthMs() / 1000.0f;
                        bAllocatedChannel = true;
    ...
    if (bAllocatedChannel && m_pSSys->m_pCVarDebugSound->GetIVal() == 1) {
        m_pSSys->m_pILog->Log("<Sound> Playing sound: %s,channel %d,volume=%d,pan=%d, priority=%d Snd:%X, TimeDiff:%.2f", GetName(), m_nChannel, m_nPlayingVolume, m_nPan,
                              m_nSoundPriority, this, timeSinceLastPlay);
    }
...
void CSound::SetLoopMode(bool bLoop) {
    ...
    if (m_pSound->Loaded()) // && m_pSound->m_btType==SoundBuffer::btSAMPLE && m_pSound->m_data.m_sfx)
    {
        ...
        case btSAMPLE:
            CS_Sample_SetMode(m_pSound->GetSample(), m_pSound->GetFModFlags(bLoop));
            break;
            // case btSTREAM:  if (m_nChannel>=0) CS_SetLoopMode(m_nChannel, bLoop ? CS_LOOP_NORMAL : CS_LOOP_OFF); break;  // [Lennert] this call screws up ! Always set loopmode
            // before playing the stream-sound !
        }
    }
    if (bLoop)
        ...
        m_nFlags &= ~FLAG_SOUND_LOOP;
    if ((m_nFlags & FLAG_SOUND_STREAM) &&
        (m_bLoop != bLoop)) { // this is kinda unfortunate, but we have to reload the stream when we the change loop-mode because changing it runtime doesnt seem to work...
    ...
    */
    ...
    if ((m_pSSys->m_fDirAttCone > 0.0f) && (m_pSSys->m_fDirAttCone < 1.0f)) {
        ...
        if ((fScale > 0.0f) && (fScale <= 1.0f)) // inside cone
        {
            ...
            fPos[1] = m_RealPos.z;
            fPos[2] = m_RealPos.y;
            // TRACE("Sound-Proj: %s, %1.3f, %1.3f, %1.3f, (%5.3f, %5.3f, %5.3f)", m_strName.c_str(), fScale, DirToPlayer*m_pSSys->m_DirAttDir, m_pSSys->m_fDirAttCone, m_RealPos.x,
            // m_RealPos.y, m_RealPos.z);
        }
    }
    ...
    if (m_pSSys->m_pCVarDopplerEnable->GetIVal() && (m_nFlags & FLAG_SOUND_DOPPLER)) {
        ...
        CS_3D_SetAttributes(m_nChannel, fPos, nullptr);
    }
...
int CSound::GetLengthMs() {
    ...
    switch (m_pSound->GetType()) {
    case btSTREAM:
        return CS_Stream_GetLengthMs(m_pSound->GetStream());
    case btSAMPLE:
        return (int)((float)CS_Sample_GetLength(m_pSound->GetSample()) / (float)m_pSound->GetBaseFreq() * 1000.0f);
    }
    return 0;
}
...
unsigned int CSound::GetCurrentSamplePos(bool bMilliSeconds) {
    ...
    case btSAMPLE:
        if (bMilliSeconds) {
            int nFreq;
            CS_Sample_GetDefaults(m_pSound->GetSample(), &nFreq, nullptr, nullptr, nullptr);
...
void CSound::OnEvent(ESoundCallbackEvent event) {
    ...
    if (!m_listeners.empty()) {
        ...
        else if (numListeners < THRESHOLD) {
