...
CSoundSystemCommon::CSoundSystemCommon(ISystem* pSystem) {
    m_pCVARDummySound = pSystem->GetIConsole()->CreateVariable("s_DummySound", "0", VF_DUMPTODISK, "Toggles dummy(nullptr) sound system.\n");
    ...
    m_pCVarInactiveSoundIterationTimeout = pSystem->GetIConsole()->CreateVariable("s_InactiveSoundIterationTimeout", "1", VF_DUMPTODISK, "This variable is for internal use only.");
    ...
    m_pCVarMaxSoundSpots = pSystem->GetIConsole()->CreateVariable("s_MaxActiveSoundSpots", "100", VF_DUMPTODISK, "Sets the maximum number of active soundspots.\n");
    m_pCVarMinRepeatSoundTimeout =
        pSystem->GetIConsole()->CreateVariable("s_MinRepeatSoundTimeout", "200", VF_DUMPTODISK, "Timeout in ms,play of same sounds cannot be repeated more often then that.\n");
    m_pCVarCompatibleMode =
        pSystem->GetIConsole()->CreateVariable("s_CompatibleMode", "0", VF_DUMPTODISK, "Toggles sound system compatible mode ON/OFF. Use only in case of playback issues.\n");
    m_pCVarCapsCheck = pSystem->GetIConsole()->CreateVariable("s_CapsCheck", "0", VF_DUMPTODISK, "Checks for sound CAPS on startup and write them into the log.\n");
}
