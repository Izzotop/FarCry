...
bool COGGDecoder::Open(const char* pszFilename) {
    ...
    if (pInfo->channels != 2) {
        MTRACE("Warning: [COGGDecoder::Open()] Cannot load file %s, due to unsupported channel-number (%d channels found; 2 channels needed) !", m_FileInfo.sFilename.c_str(),
               pInfo->channels);
        return false;
    }
...
COGGDecoderInstance::COGGDecoderInstance(COGGDecoder* pDecoder) {
    ...
    //  getTicks(&nEndTime);
    //  MTRACE("Info: [COGGDecoderInstance::c_tor()] It took %d cycles to initialize ogg-decoder-instance of %s.", (long)(nEndTime-nStartTime),
    //m_pDecoder->m_FileInfo.sFilename.c_str());
}
