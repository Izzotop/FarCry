...
CMusicSystem::CMusicSystem(ISystem* pSystem) {
    ...
    m_pCVarMusicMaxSimultaniousPatterns = pSystem->GetIConsole()->CreateVariable("s_MusicMaxPatterns", "12", VF_DUMPTODISK, "Max simultaniously playing music patterns.\n");
...
bool CMusicSystem::SetData(SMusicData* pMusicData, bool bNoRelease) {
    ...
    for (TPatternDefVecIt It = m_pDataPtr->vecPatternDef.begin(); It != m_pDataPtr->vecPatternDef.end(); ++It) {
        ...
        if (!pPattern) {
            m_pSystem->Warning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, VALIDATOR_FLAG_SOUND, pPatternDef->sFilename.c_str(), "Music file %s failed to load",
                               pPatternDef->sFilename.c_str());
            continue;
...
bool CMusicSystem::SetTheme(SMusicTheme* pNewTheme, bool bOverride) {
    ...
    if (!((m_pCurrTheme == m_pNextTheme) || (pNewTheme))) // only switch to the new theme if there was no switch in this frame yet OR the new theme is NOT nullptr
        return true;
    if (pNewTheme)
...
bool CMusicSystem::LoadFromXML(const char* sFilename, bool bAddData) {
    ...
    if (!root) {
        m_pSystem->Warning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, VALIDATOR_FLAG_SOUND, sFilename, "Music description XML file %s failed to load", sFilename);
        return false;
    }
