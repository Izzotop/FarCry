...
void CSoundBuffer::SoundLoaded() {
    ...
    case btSAMPLE: {
        ...
        CS_Sample_GetDefaults(m_Data.m_pSample, &m_nBaseFreq, nullptr, nullptr, nullptr);
...
bool CSoundBuffer::Load(bool bLooping, CSound* pSound) {
    ...
    if (m_Props.nFlags & FLAG_SOUND_STREAM) {
        ...
        // file system
        GUARD_HEAP;
        CS_STREAM* pSoundStream = nullptr;
...
void CSoundBuffer::DestroyData() {
    ...
    if (m_Data.m_pData) {
        ...
        switch (m_Type) {
        case btSAMPLE:
            CS_Sample_Free(m_Data.m_pSample);
            SetSample(nullptr);
            break;
        case btSTREAM:
            CS_Stream_Close(m_Data.m_pStream);
            SetStream(nullptr);
            break;
        }
    }
