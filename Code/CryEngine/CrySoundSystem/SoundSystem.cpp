...
CSoundSystem::CSoundSystem(ISystem* pSystem, HWND hWnd) : CSoundSystemCommon(pSystem) {
    ...
    CS_SetMemorySystem(nullptr, 0, CrySound_Alloc, CrySound_Realloc, CrySound_Free);
#endif
        ...
        if (pRenderer) {
            ...
            if (m_pCVarSoundInfo->GetIVal() == 1) {
                for (SoundListItor It = m_lstSoundSpotsActive.begin(); It != m_lstSoundSpotsActive.end(); It++) {
                    ...
                    if (cs->m_nFlags & FLAG_SOUND_3D)
                        pRenderer->Draw2dLabel(x, ypos, 1, pCurColor, false, "%s,3d,pos=%d,%d,%d,minR=%d,maxR=%d,vol=%d", cs->GetName(), (int)(cs->m_position.x),
                                               (int)(cs->m_position.y), (int)(cs->m_position.z), (int)(cs->m_fMinDist), (int)(cs->m_fMaxDist), cs->m_nPlayingVolume);
                    else
            ...
            // All playing sounds...
            {
                ...
                for (SoundListItor It = m_vecSounds.begin(); It != m_vecSounds.end(); It++) {
                    ...
                    if (cs->m_nFlags & FLAG_SOUND_3D)
                        pRenderer->Draw2dLabel(x, ypos, 1, pCurColor, false, "%s,3d,pos=%d,%d,%d,minR=%d,maxR=%d,vol=%d,channel=%d", cs->GetName(), (int)(cs->m_position.x),
                                               (int)(cs->m_position.y), (int)(cs->m_position.z), (int)(cs->m_fMinDist), (int)(cs->m_fMaxDist), cs->m_nPlayingVolume,
                                               cs->m_nChannel);
                    else
        ...
        for (int i = 0; i < nInactiveSoundItCount; i++) {
            ...
            if (cs->m_bPlaying) {
                ...
                if (fDist2 <= cs->m_fPreloadRadius2) {
                    ...
                    nSoundSpotAdded++;
                    It = m_lstSoundSpotsInactive.erase(It);
    ...
    for (It = m_lstSoundSpotsActive.begin(); It != m_lstSoundSpotsActive.end();) {
        ...
        if (cs->m_nFlags & FLAG_SOUND_RADIUS) {
            ...
            if (bZeroFadeCheckDist) {
                ++It;
                continue; // the sound is faded out to 0, we just do this check here because it might also be too far away in which case it will be moved to the inactive list
                          // again...
            }
            if (fDist2 > cs->m_fMinRadius2) {
                // calc attenuation
    ...
    if (m_nLastEax != EAX_PRESET_UNDERWATER) {
        ...
        if (bWasInside != m_bInside) {
            if (!m_bInside) {
                ...
                SetEaxListenerEnvironment(m_EAXOutdoor.nPreset, (m_EAXOutdoor.nPreset == -1) ? &m_EAXOutdoor.EAX : nullptr);
            }
        }
    }
...
void CSoundSystem::SetSoundActiveState(CSound* pSound, ESoundActiveState State) // take care from where you call this function, because it may cause crashes if you change the state
                                                                                // while the system is iterating through the lists (Update())
{
    GUARD_HEAP;
...
void CSoundSystem::SetListener(const CCamera& cCam, const Vec3d& vVel) {
    ...
    if (m_pCVarDopplerEnable->GetIVal()) {
        ...
        // CS_3D_Listener_SetAttributes(pos, nullptr, 0, 0, 1.0f, 0, 1.0f, 0);
        CS_3D_Listener_SetAttributes(pos, fVel, FVec1.x, FVec1.z, FVec1.y, TVec1.x, TVec1.z, TVec1.y);
    } else {
        CS_3D_Listener_SetAttributes(pos, nullptr, FVec1.x, FVec1.z, FVec1.y, TVec1.x, TVec1.z, TVec1.y);
    };
...
bool CSoundSystem::SetEaxListenerEnvironment(int nPreset, CS_REVERB_PROPERTIES* tpProps, int nFlags) {
    ...
    */
    if (tpProps) {
        ...
        m_nLastEax = nPreset;
        m_pLastEAXProps = nullptr; // using one of pre-defined presets
    }
    return true;
}
...
void CSoundSystem::CalcDirectionalAttenuation(Vec3d& Pos, Vec3d& Dir, float fConeInRadians) {
    ...
    if (IsEquivalent(Pos, m_DirAttPos, VEC_EPSILON) && IsEquivalent(Dir, m_DirAttDir, VEC_EPSILON) && (fCosCone == m_fDirAttCone))
        return;
    m_DirAttPos = Pos;
    m_DirAttDir = Dir;
