...
struct ImageObject {
    ...
    virtual IDirect3DTexture9* CopyToXRGB(LPDIRECT3DDEVICE9 pd3ddev) {
        return nullptr;
    };

    //! gets the internal DX texture, if the object has one
    virtual LPDIRECT3DBASETEXTURE9 GetDXTex() {
        return nullptr;
    };
    virtual void GetDXTexI(IDirect3DTexture9*& pTexture) {
        pTexture = nullptr;
    };
...
struct DXImage : ImageObject {
    ...
    bool Save(const char* filename) {
        return D3DXSaveTextureToFile(filename, D3DXIFF_DDS, m_pTex, nullptr) == S_OK;
    };
    ...
    HRESULT Convert(LPDIRECT3DSURFACE9 psurfSrc, int mip, int filter, D3DCUBEMAP_FACES facetype, LPDIRECT3DDEVICE9 pd3ddev) {
        ...
        if (hr == S_OK)
            hr = D3DXLoadSurfaceFromSurface(psurfDest, nullptr, nullptr, psurfSrc, nullptr, nullptr, filter, 0);

        ReleasePpo(&psurfDest);
        return hr;
    };

    LPDIRECT3DBASETEXTURE9 GetDXTex() {
        return m_pTex;
    };
    virtual void GetDXTexI(IDirect3DTexture9*& pTexture) {
        m_pTex->QueryInterface(IID_IDirect3DTexture9, (void**)&pTexture);
    };
};
...
struct DXImageCube : DXImage {
    ...
    HRESULT Convert(LPDIRECT3DSURFACE9 psurfSrc, int mip, int filter, D3DCUBEMAP_FACES facetype, LPDIRECT3DDEVICE9 pd3ddev) {
        LPDIRECT3DSURFACE9 pcubeDest = nullptr;
        HRESULT hr = ((LPDIRECT3DCUBETEXTURE9)m_pTex)->GetCubeMapSurface(facetype, mip, &pcubeDest);
        if (hr == S_OK)
            hr = D3DXLoadSurfaceFromSurface(pcubeDest, nullptr, nullptr, psurfSrc, nullptr, nullptr, filter, 0);
...
struct DXImageVol : DXImage {
    ...
    HRESULT Convert(LPDIRECT3DSURFACE9 psurfSrc, int mip, int filter, D3DCUBEMAP_FACES facetype, LPDIRECT3DDEVICE9 pd3ddev) {
        ...
        if (hr == S_OK)
            hr = D3DXLoadVolumeFromVolume(pvolDest, nullptr, nullptr, (LPDIRECT3DVOLUME9)psurfSrc, nullptr, nullptr, filter, 0);
        ReleasePpo(&pvolDest);
        return hr;
    };
};
...
struct P8Image : ImageObject {
    ...
    P8Image(int w, int h, int m) : m_pPalette(nullptr), m_nWidth(w), m_nHeight(h), m_nMips(m) {
        m_ppIndices = new unsigned char*[m];
        for (int i = 0; i < m_nMips; i++)
            m_ppIndices[i] = nullptr;
    };
