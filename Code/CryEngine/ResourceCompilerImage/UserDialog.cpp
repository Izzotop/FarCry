...
// constructor
CUserDialog::CUserDialog() : m_iPreviewWidth(256), m_iPreviewHeight(256) {
    m_pImageCompiler = 0;
    m_fShiftX = 0.5f;
    m_fShiftY = 0.5f;
    m_iScale16 = 16;
...
void CenterWindow(HWND hwndChild, HWND hwndParent = 0) {
    ...
    // Set it, and return
    //    SetWindowPos(hwndChild, nullptr, xNew, yNew, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
...
bool CUserDialog::DoModal(CImageCompiler* inpImageCompiler) {
    ...
    // message loop
    {
        ...
        while (GetMessage(&Msg, nullptr, 0, WM_USER)) {
...
void CUserDialog::CreateDialogItems() {
    // tab control
    {
        ...
        // sub windows
        m_hTab_Normalmapgen = CreateDialog(g_hInst, MAKEINTRESOURCE(IDD_TAB_NORMALMAPGEN), m_hWindow, CUserDialog::WndProcRedirect);
        assert(m_hTab_Normalmapgen);
        SetWindowPos(m_hTab_Normalmapgen, HWND_TOP, wnp.left, wnp.top, 0, 0, SWP_NOSIZE);

        m_hTab_Saveformat = CreateDialog(g_hInst, MAKEINTRESOURCE(IDD_TAB_SAVEFORMAT), m_hWindow, CUserDialog::WndProcRedirect);
    ...
    // templates
    {
        HWND hwnd = GetDlgItem(m_hWindow, IDC_TEMPLATECOMBO);
        assert(hwnd);
    ...
    // pixelformats
    {
        HWND hwnd = GetDlgItem(m_hTab_Saveformat, IDC_PIXELFORMAT);
        assert(hwnd);
    ...
    // mipmaps
    {
        HWND hwnd = GetDlgItem(m_hTab_Saveformat, IDC_MIPMAPS);
        assert(hwnd);
    ...
    // reduce resolution
    {
        HWND hwnd = GetDlgItem(m_hTab_Saveformat, IDC_REDUCERES);
        assert(hwnd);
    ...
    // dither mode
    {
        HWND hwnd = GetDlgItem(m_hTab_Saveformat, IDC_DITHERMODE);
        assert(hwnd);
...
void CUserDialog::UpdateWindowTitle() {
    assert(m_pImageCompiler);

    CString Zoom;
    Zoom.Format("  Zoom:%d%%", (100 * m_iScale16) / 16);
...
void CUserDialog::UpdatePixelFormatDesc() {
    HWND hwnds = GetDlgItem(m_hTab_Saveformat, IDC_PIXELFORMATDESC);
    assert(hwnds);

    SetWindowText(hwnds, m_pImageCompiler->GetPixelFormatDesc(m_pImageCompiler->m_Props.m_iDestPixelFormat));
}

void CUserDialog::UpdatePreview() {
    // preview images
    {
        RECT rect;
    ...
    // info text
    {
        HWND hwndc = GetDlgItem(m_hTab_Saveformat, IDC_PIXELFORMAT);
        assert(hwndc);
        ...
        HWND hwnd1 = GetDlgItem(m_hWindow, IDC_LEFTPREVIEWINFO);
        assert(hwnd1);
        HWND hwnd2 = GetDlgItem(m_hWindow, IDC_RIGHTPREVIEWINFO);
        assert(hwnd2);
