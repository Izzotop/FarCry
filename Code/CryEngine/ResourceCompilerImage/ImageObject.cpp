...
HRESULT P8Image::Convert(LPDIRECT3DSURFACE9 psurfSrc, int mip, int filter, D3DCUBEMAP_FACES facetype, LPDIRECT3DDEVICE9 pd3ddev) {
    ...
    HRESULT hr = pd3ddev->CreateOffscreenPlainSurface(sd.Width, sd.Height, D3DFMT_A8R8G8B8, D3DPOOL_SCRATCH, &psurfTarget, nullptr);
    if (FAILED(hr))
        return hr;
    hr = D3DXLoadSurfaceFromSurface(psurfTarget, nullptr, nullptr, psurfSrc, nullptr, nullptr, D3DX_FILTER_TRIANGLE, 0);
    if (FAILED(hr))
        return hr;
...

    {
        ...
        hr = psurfTarget->LockRect(&lrTarg, nullptr, 0);
        if (FAILED(hr))
            return hr;
...
bool P8Image::Save(const char* name) {
    ...
    for (unsigned int mip = 0; mip < tex->GetLevelCount(); mip++) {
        ...
        D3DLOCKED_RECT lr;
        DXERRORCHECK(psurf->LockRect(&lr, nullptr, 0));
...
void P8Image::SavePaletizedTextureHeader(FILE* fh) {
    ...
    */
    ...
    unsigned char tgaheader[] = {
        0, 1, 1, 0, 0, 0, 1, 24, 0, 0, 0, 0, (m_nWidth & 0x00FF), (m_nWidth & 0xFF00) / 256, (m_nHeight & 0x00FF), (m_nHeight & 0xFF00) / 256, 8, 0,
    };
...
IDirect3DTexture9* P8Image::CopyToXRGB(LPDIRECT3DDEVICE9 pd3ddev) {
    ...
    if (FAILED(pd3ddev->CreateTexture(m_nWidth, m_nHeight, 1, 0, D3DFMT_X8R8G8B8, D3DPOOL_MANAGED, &ptexNew, nullptr)))
        return nullptr;

    LPDIRECT3DSURFACE9 psurfd = nullptr;
    if (FAILED(ptexNew->GetSurfaceLevel(0, &psurfd)))
        return nullptr;

    D3DLOCKED_RECT lrd;
    if (FAILED(psurfd->LockRect(&lrd, nullptr, 0)))
        return nullptr;
