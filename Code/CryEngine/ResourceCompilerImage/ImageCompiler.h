...
public:
     ...
     * @param indwReduceResolution
     * @param inbRemoveMips
     * @return
     */
    LPDIRECT3DBASETEXTURE9 CreateUncompressedMipMaps(const DWORD indwFilter, const DWORD indwReduceResolution, const bool inbRemoveMips);
...
private:
    ...
    //!
    //! /param indwFilter D3DX_FILTER bitmask (see DirectX documentation): e.g. D3DX_FILTER_POINT|D3DX_FILTER_MIRROR, D3DX_FILTER_TRIANGLE
    HRESULT ChangeFormat(LPDIRECT3DBASETEXTURE9 ptexCur, D3DFORMAT fmtTo, ImageObject*& pNewImage, DWORD indwFilter, DWORD indwReduceResolution);

    //!
    //! /param indwFilter D3DX_FILTER bitmask (see DirectX documentation): e.g. D3DX_FILTER_POINT|D3DX_FILTER_MIRROR, D3DX_FILTER_TRIANGLE
    HRESULT BltAllLevels(D3DCUBEMAP_FACES FaceType, LPDIRECT3DBASETEXTURE9 ptexSrc, ImageObject* pNewImage, DWORD indwFilter, DWORD indwReduceResolution,
                         D3DFORMAT fmtTo);
    ...
    //! helper function
    bool GetBoolParam(CString insName, bool inbDefault) const;

    //! helper function
    int GetIntParam(CString insName, int iniDefault) const;

    //!
    DWORD CalcTextureMemory(bool inbOrig) const;

    //! \return pixelformat no which has same properties but is uncompressed
    static int GetPixelFormatUncompressed(int iniNo);
...
#define DXERRORCHECK(cmt, exp)                                                                                                                                                     \
    {                                                                                                                                                                              \
        HRESULT _hr = (exp); /*assert(!_hr);*/                                                                                                                                     \
        if (_hr)                                                                                                                                                                   \
            CCLOG->LogError("'%s' DX ERROR: %s", cmt, DXGetErrorString(_hr));                                                                                                      \
    }
