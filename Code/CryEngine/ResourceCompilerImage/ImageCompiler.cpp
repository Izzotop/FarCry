...
static SPixelFormats g_pixelformats[] = {
    ...
    //    { 16,"8",            D3DFMT_A8R3G3B2,        "A8R3G3B2",        "A8R8G8B8",        "16-bit ARGB texture format using 8 bits for alpha, 3 bits each for red and green, and
    //    2 bits for blue",false },
    ...
    //    { 16,"0",            D3DFMT_L6V5U5,            "L6V5U5",            "X8L8V8U8",        "16-bit signed/unsigned bump-map format with luminance using 6 bits for luminance,
    //    and 5 bits each for u and v",false },
    //    { 32,"0",            D3DFMT_X8L8V8U8,        "X8L8V8U8",        "X8L8V8U8",        "32-bit signed/unsigned bump-map format with luminance using 8 bits for each
    //    channel",false },
    //    { 32,"2",            D3DFMT_A2W10V10U10,    "A2W10V10U10","A2W10V10U10","32-bit signed/unsigned bump-map format using 2 bits for alpha and 10 bits each for w, v, and
    //    u",false },
...
bool CImageCompiler::Save(const char* lpszPathName) {
    ...
    switch(sd.Format)
    {
        ...
        default:
            if( FAILED( D3DXSaveTextureToFile( lpszPathName, D3DXIFF_DDS, m_ptexNew, nullptr ) ) )
            {
...
bool CImageCompiler::Init(HWND inhWnd) {
    ...
    m_hWnd = CreateWindow("skeleton", "the title", WS_OVERLAPPEDWINDOW, 0, 0, 400, 400, nullptr, nullptr, g_hInst, nullptr);
    ...
    hr = m_pd3d->CreateDevice(D3DADAPTER_DEFAULT, devType, m_hWnd, D3DCREATE_SOFTWARE_VERTEXPROCESSING, &m_presentParams, &m_pd3ddev);
    if (FAILED(hr))
        return false;
...
bool CImageCompiler::Load(const char* lpszPathName) {
    ...
    case D3DRTYPE_TEXTURE:
        hRes = D3DXCreateTextureFromFileEx(m_pd3ddev, lpszPathName, imageinfo.Width, imageinfo.Height, imageinfo.MipLevels, 0, imageinfo.Format, D3DPOOL_DEFAULT, D3DX_FILTER_NONE,
                                           D3DX_FILTER_NONE, 0, &imageinfo2, nullptr, (LPDIRECT3DTEXTURE9*)&m_ptexOrig);

        if (FAILED(hRes)) {
            CCLOG->LogError("D3DXCreateTextureFromFileEx failed (power of two?) '%s'", DXGetErrorString(hRes));
            return false;
        }
    ...
    case D3DRTYPE_VOLUMETEXTURE:
        if (FAILED(D3DXCreateVolumeTextureFromFileEx(m_pd3ddev, lpszPathName, imageinfo.Width, imageinfo.Height, imageinfo.Depth, imageinfo.MipLevels, 0, imageinfo.Format,
                                                     D3DPOOL_MANAGED, D3DX_FILTER_NONE, D3DX_FILTER_NONE, 0, &imageinfo2, nullptr, (LPDIRECT3DVOLUMETEXTURE9*)&m_ptexOrig))) {
    ...
    case D3DRTYPE_CUBETEXTURE:
        if (FAILED(D3DXCreateCubeTextureFromFileEx(m_pd3ddev, lpszPathName, imageinfo.Width, imageinfo.MipLevels, 0, imageinfo.Format, D3DPOOL_MANAGED, D3DX_FILTER_NONE,
                                                   D3DX_FILTER_NONE, 0, &imageinfo2, nullptr, (LPDIRECT3DCUBETEXTURE9*)&m_ptexOrig))) {
...
LPDIRECT3DBASETEXTURE9 CImageCompiler::CreateUncompressedMipMaps(const DWORD indwFilter, const DWORD indwReduceResolution, const bool inbRemoveMips) {
    D3DFORMAT fmt;
    HRESULT hr;
    ...
    if (pvoltex != nullptr) {
        if (FAILED(hr = m_pd3ddev->CreateVolumeTexture(dwW, dwH, m_dwDepth, dwNumNewMips, 0, fmt, D3DPOOL_SYSTEMMEM, &pvoltexNew, nullptr))) {
            assert(0);
            return (0);
        }
        ...
        hr = D3DXLoadVolumeFromVolume(pvolDest, nullptr, nullptr, pvolSrc, nullptr, nullptr, indwFilter, 0);
        ReleasePpo(&pvolSrc);
        ReleasePpo(&pvolDest);
        hr = D3DXFilterVolumeTexture(pvoltexNew, nullptr, 0, indwFilter);
        assert(pvoltexNew);
        return ((LPDIRECT3DBASETEXTURE9)pvoltexNew);
    } else if (pmiptex != nullptr) {
        HRESULT hr = m_pd3ddev->CreateTexture(dwW, dwH, dwNumNewMips, 0, fmt, D3DPOOL_MANAGED, &pmiptexNew, nullptr);
        ...
        hr = D3DXLoadSurfaceFromSurface(psurfDest, nullptr, nullptr, psurfSrc, nullptr, nullptr, indwFilter, 0);
        ReleasePpo(&psurfSrc);
        ReleasePpo(&psurfDest);
        hr = D3DXFilterTexture(pmiptexNew, nullptr, 0, indwFilter);
        assert(pmiptexNew);
        return ((LPDIRECT3DBASETEXTURE9)pmiptexNew);
    } else {
        if (FAILED(hr = m_pd3ddev->CreateCubeTexture(dwW, dwNumNewMips, 0, fmt, D3DPOOL_MANAGED, &pcubetexNew, nullptr))) {
            assert(0);
            return (0);
        }
        ...
        ReleasePpo(&psurfSrc);
        ReleasePpo(&psurfDest);
        hr = D3DXFilterCubeTexture(pcubetexNew, nullptr, 0, indwFilter);
        assert(pcubetexNew);
        return ((LPDIRECT3DBASETEXTURE9)pcubetexNew);
    }
}

CString CImageCompiler::GetSourceFilename() {
    assert(m_pCC);
...
HRESULT CImageCompiler::ChangeFormat(LPDIRECT3DBASETEXTURE9 ptexCur, D3DFORMAT fmtTo, ImageObject*& pNewImage, const DWORD indwFilter, const DWORD indwReduceResolution) {
    assert(ptexCur);
    ...
    if (IsVolumeMap()) {
        hr = m_pd3ddev->CreateVolumeTexture(dwWidth, dwHeight, m_dwDepth, dwMips, 0, fmtTo, D3DPOOL_SYSTEMMEM, &pvoltexNew, nullptr);
        if (FAILED(hr))
            return hr;
        ...
        hr = m_pd3ddev->CreateCubeTexture(dwWidth, dwMips, 0, fmtTo, D3DPOOL_MANAGED, &pcubetexNew, nullptr);
        if (FAILED(hr))
            return hr;
        ...
        if (fmtTo == D3DFMT_P8) {
            ...
            if ((fmtTo == D3DFMT_DXT1 || fmtTo == D3DFMT_DXT2 || fmtTo == D3DFMT_DXT3 || fmtTo == D3DFMT_DXT4 || fmtTo == D3DFMT_DXT5) &&
                (m_dwOrigWidth % 4 != 0 || m_dwOrigHeight % 4 != 0)) {
                CCLOG->LogError("ERROR_NEEDMULTOF4 = for DXT compression we need width and height to be multiple of 4");
                return E_FAIL;
            }

            hr = m_pd3ddev->CreateTexture(dwWidth, dwHeight, dwMips, 0, fmtTo, D3DPOOL_MANAGED, &pmiptexNew, nullptr);
            if (FAILED(hr)) {
...
HRESULT CImageCompiler::BltAllLevels(D3DCUBEMAP_FACES FaceType, LPDIRECT3DBASETEXTURE9 ptexSrc, ImageObject* pNewImage, const DWORD indwFilter, const DWORD indwReduceResolution,
                                     D3DFORMAT fmtTo) {
    ...
    for (DWORD iLevel = 0; iLevel < dwMips; iLevel++) {
        if (IsVolumeMap()) {
...

            {
                ...
                if (FAILED(_hr)) {
                    CCLOG->LogError("'%s' DX ERROR: %s", "Convert3", DXGetErrorString(_hr));
                }
            }
...
bool CImageCompiler::Process(ConvertContext& cc) {
    ...
    if (Load(sourceFile)) {
        ...
        if (m_Props.m_bUserDialog) {
            ...
            RunWithProperties(true); // run and save

    } else
        CCLOG->LogError("CImageCompiler::Process load '%s' failed", (const char*)sourceFile);
...
int CImageCompiler::GetPixelFormatCount() {
...
DWORD CImageCompiler::CalcTextureMemory(const bool inbOrig) const {
    ...
    if (!inbOrig && m_Props.m_bMipmaps)
        iMipmaps = _CalcMipCount(m_dwOrigWidth, m_dwOrigHeight);

    int iIgnoreTopMips = 0;

    if (!inbOrig)
        iIgnoreTopMips = (int)m_Props.m_dwReduceResolution;
...
static void PrintInPreview(HWND inHWND, RECT& inRect, const char* inszTxt1, const char* inszTxt2 = nullptr) {
    HDC hdc = GetDC(inHWND);
    FillRect(hdc, &inRect, GetSysColorBrush(COLOR_3DFACE));
    SetBkMode(hdc, TRANSPARENT);
    SetTextAlign(hdc, TA_CENTER);
    ExtTextOut(hdc, (inRect.left + inRect.right) / 2, (inRect.top + inRect.bottom) / 2 - 8, ETO_CLIPPED, &inRect, inszTxt1, (int)strlen(inszTxt1), nullptr);
    if (inszTxt2)
        ExtTextOut(hdc, (inRect.left + inRect.right) / 2, (inRect.top + inRect.bottom) / 2 + 8, ETO_CLIPPED, &inRect, inszTxt2, (int)strlen(inszTxt2), nullptr);
...
bool CImageCompiler::BlitTo(HWND inHWND, RECT& inRect, const float infOffsetX, const float infOffsetY, const int iniScale16, const bool inbOrig) {
    ...
    if (g_pixelformats[m_Props.m_iDestPixelFormat].DxtNo == D3DFMT_P8 && !inbOrig) {
        ...
        if (inbOrig) {
            ...
            if (m_pNewImage == nullptr || m_pNewImage->GetDXTex() == nullptr) {
                PrintInPreview(inHWND, inRect, "Conversion failed", "(format not supported by driver?)");
                return true;
            }
        }
    ...
    m_pd3ddev->SetVertexShader(nullptr);
    ...
    my_vertex g_triangle_vertices[] = {
        //  x      y       z     rhw  u1  v1
        {
            ...
            0.5f,
            1,
            (float)(-iScaleX + iX) / GetWidth(),
            (float)(-iScaleY + iY) / GetHeight(),
        },
        {
            255.5f,
            -0.5f,
    ...
    hr = m_pd3ddev->Present(nullptr, &inRect, inHWND, nullptr);
    ...
    for (int y = 0; y < iHeight; y++) {
        ...
        // BOOL ok = StretchBlt(inHdc, inRect.left, inRect.top, iWidth, iHeight, dc, iX - iW, iY - iH, iW * 2, iH * 2, SRCCOPY);
        // assert(ok);
...
bool CImageCompiler::ClampBlitOffset(const int iniWidth, const int iniHeight, float& inoutfX, float& inoutfY, const int iniScale16) const {
    ...
    if (iniWidth >= (int)(m_dwOrigWidth * iniScale16) / 16)
        inoutfX = 0.5f;
    else {
    ...
    if (iniHeight >= (int)(m_dwOrigHeight * iniScale16) / 16)
        inoutfY = 0.5f;
    else {
...
DWORD CImageCompiler::GetWidth() const {
    return (m_dwOrigWidth);
}

DWORD CImageCompiler::GetHeight() const {
...
bool CImageCompiler::CopyAlphaToRGB(LPDIRECT3DSURFACE9 psurf) const {
    ...
    hr = m_pd3ddev->CreateOffscreenPlainSurface(sd.Width, sd.Height, D3DFMT_A8R8G8B8, D3DPOOL_SCRATCH, &psurfTarget, nullptr);
    assert(hr == D3D_OK);
    hr = D3DXLoadSurfaceFromSurface(psurfTarget, nullptr, nullptr, psurf, nullptr, nullptr, D3DX_FILTER_TRIANGLE, 0);
    assert(hr == D3D_OK);
    ...
    hr = D3DXLoadSurfaceFromSurface(psurf, nullptr, nullptr, psurfTarget, nullptr, nullptr, D3DX_FILTER_TRIANGLE, 0);
    assert(hr == D3D_OK);
...
bool CImageCompiler::LoadConfigFile() {
    assert(m_pCC);
    ...
    if (!m_pCC->config->Get("preset", m_Props.m_sPreset))
        m_Props.m_sPreset = "";
...
// set the stored properties to the current file and save it
bool CImageCompiler::UpdateAndSaveConfigFile() {
