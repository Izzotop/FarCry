...
bool COPApproach::Execute(CPipeUser* pOperand) {
    ...
    if (((!pOperand->m_bNeedsPathIndoor) && bIndoor) || ((!pOperand->m_bNeedsPathOutdoor) && (!bIndoor)) || pOperand->m_State.pathUsage == SOBJECTSTATE::PU_NoPathfind)
...
bool COPStrafe::Execute(CPipeUser* pOperand) {
    ...
    if (m_bPositionSnapshotTaken) {
        ...
        if (((m_fLastRemainingDistance - m_fRemainingDistance) < 0.01f) || (m_fRemainingDistance < 0.5)) {
            m_bPositionSnapshotTaken = false;
            pOperand->m_State.right = false;
            pOperand->m_State.left = false;
...
int COPLookAround::XMLExport(const char* pBuffer) {
    strcat((char*)pBuffer,
           "<lookaround type=\"Goal\" Description=\"Instructs the agent to look away from its target in the specified bounds, or 360 deg if bounds not specified.\">\n");
...
bool COPTrace::Execute(CPipeUser* pOperand) {
    ...
    if (!m_pNavTarget) {
        ...
        ray_hit hit;
        int rayresult =
            pWorld->RayWorldIntersection(vectorf(vPos), vectorf(0, 0, -40), ent_terrain | ent_static, rwi_stop_at_pierceable | geom_colltype_player << rwi_colltype_bit, &hit, 1);
        if (rayresult) {
    ...
    if (!m_pJumpTarget) {
        ...
        if (m_fSearchDistance < 1.f) {
            if (pOperand->m_pLastOpResult) {
                ...
                if ((pOperand->m_pLastOpResult->GetType() == AIOBJECT_PLAYER) || (pOperand->m_pLastOpResult->GetType() == AIOBJECT_PUPPET)) {
                    bAnticipate = true;
