...
// Steers the vehicle outdoors and makes it avoid the immediate obstacles
void CAIVehicle::Reset() {
        ...
        if (fdot > startTurnCos && poslength < 27) {
            ...
            for (VectorOfLinks::iterator theLink = pNode->link.begin(); theLink != pNode->link.end(); ++theLink) {
                if ((*theLink).nStartIndex != (*vi) && (*theLink).nEndIndex != (*vi))
                    continue;
                float size = ((*theLink).vEdgeCenter - m_pAISystem->m_VertexList.GetVertex((*vi)).vPos).len2() - (*theLink).fMaxRadius * (*theLink).fMaxRadius;
                if (size > obsSize)
                    obsSize = size;
            }
