        ...
        if (pBegin == m_pSafeFirst) {
            ...
            if (pBegin != m_pSafeFirst || !pBegin->mark) {
                for (vi = pBegin->link.begin(); vi != pBegin->link.end(); vi++) {
                    ...
                    Vec3d midpoint =
                        (m_pAISystem->m_VertexList.GetVertex(pBegin->vertex[(*vi).nStartIndex]).vPos + m_pAISystem->m_VertexList.GetVertex(pBegin->vertex[(*vi).nEndIndex]).vPos) /
                        2.f;
                    midpoint.z = pos.z;
...
bool CGraph::ReadNodes(CCryFile& file) {
    ...
    for (ni = m_vBuffer.begin(); ni != m_vBuffer.end(); ni++, index++) {
        ...
        //    AIWarning("![AIERROR] A node was found that had buildingid:%d during export, but now it has buildingid:-1 [pos:(x:%.3f,y:%.3f,z:%.3f)]. Please make sure that all your
        //    navigation modifiers are correctly exported.",
...
void CGraph::DisconnectUnreachable() {
...
void CGraph::ResolveLinkData(GraphNode* pOne, GraphNode* pTwo) {
    ...
    Vec3d OneNormal = (m_pAISystem->m_VertexList.GetVertex(pOne->vertex[0]).vPos - m_pAISystem->m_VertexList.GetVertex(pOne->vertex[1]).vPos)
                          .Cross(m_pAISystem->m_VertexList.GetVertex(pOne->vertex[2]).vPos - m_pAISystem->m_VertexList.GetVertex(pOne->vertex[1]).vPos);
    Vec3d TwoNormal = (m_pAISystem->m_VertexList.GetVertex(pTwo->vertex[0]).vPos - m_pAISystem->m_VertexList.GetVertex(pTwo->vertex[1]).vPos)
                          .Cross(m_pAISystem->m_VertexList.GetVertex(pTwo->vertex[2]).vPos - m_pAISystem->m_VertexList.GetVertex(pTwo->vertex[1]).vPos);
    ...
    for (vi = pOne->link.begin(); vi != pOne->link.end(); vi++) {
        if ((*vi).pLink == pTwo) {
            ...
            float fdot =
                normal.Dot(m_pAISystem->m_VertexList.GetVertex(pTwo->vertex[iNoTwo]).vPos - (m_pAISystem->m_VertexList.GetVertex(pTwo->vertex[iTwoEdge1]).vPos + edge / 2.f));
...
void CGraph::ResolveTotalLinkData() {
    m_lstNodeStack.clear();
...
void CGraph::FixDegenerateTriangles() {
    m_lstNodeStack.clear();
    m_lstNodeStack.push_back(m_pSafeFirst);
