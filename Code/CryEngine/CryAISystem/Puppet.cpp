...
void CPuppet::UpdatePuppetInternalState() {
    ...
    if (m_bUpdateInternal) {
        ...
        if (m_State.bMemory) {
            ...
            } else if ((pInterest != m_pAttentionTarget) && (fMaxInterest > 0.f)) {
                // go into investigate
...
void CPuppet::FireCommand() {
    ...
    if (m_pAttentionTarget) {
        ...
        if (m_bSmartFire) {
            ...
            if ((nType == AIOBJECT_DUMMY) || (nType == AIOBJECT_HIDEPOINT) || (nType == AIOBJECT_WAYPOINT)) {
...
void CPuppet::CheckPlayerTargeting() {
