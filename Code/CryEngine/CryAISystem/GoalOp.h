...
public:
    ...
    virtual bool Execute(CPipeUser* pOperand) {
        return true;
    }
    virtual int XMLExport(const char* pBuffer) {
        return 0;
    }
...
public:
    void Reset(CPipeUser* pOperand);
    COPLookAround(float maximum) {
        m_pDummyAttTarget = 0;
        m_fMaximumAngle = maximum;
    }
    ~COPLookAround();
...
public:
    COPPathFind(const char* szName, IAIObject* pTarget = 0) {
        m_pTarget = pTarget;
...
public:
    COPLocate(const char* szName, unsigned int ot = 0) {
        if (szName)
            m_sName = szName;
        m_nObjectType = ot;
    }
...
public:
    ...
    void Reset(CPipeUser* pOperand);
    COPTrace(bool bExact, float fDistance = 0, bool bSingleStep = false) {
        m_pNavTarget = 0;
        bExactFollow = bExact;
        m_fDistance = fDistance;
        m_bSingleStep = bSingleStep;
    }
    ~COPTrace() {}
...
public:
    COPSignal(int param, const char* szSignal, unsigned char cFilter) {
        m_bSent = false;
        m_nSignalID = param;
        m_pTarget = 0;
        m_cFilter = cFilter;
...
public:
    COPDeValue(int nPuppetsAlso = 0) {
        bDevaluePuppets = (nPuppetsAlso != 0);
    }
    ~COPDeValue() {}
...
public:
    COPStick(float dist, bool bGroup) {
        m_fDistance = dist;
        m_bGroup = bGroup;
    }
    ~COPStick() {}
...
public:
    COPLookAt(float startangle, float endangle) {
