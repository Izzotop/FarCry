...
int CGraph::ProcessMegaMerge(ListNodes& nodesList, const Vec3d& vCutStart, const Vec3d& vCutEnd) {
    ...
    for (it = nodesList.begin(); it != nodesList.end(); it++) {
        ...
        if (std::find(rightSideNodes.begin(), rightSideNodes.end(), curNode) != rightSideNodes.end()) // already in list - skip
            continue;
        if ((curNode->vertex[0] == cutStart) || (curNode->vertex[1] == cutStart) || (curNode->vertex[2] == cutStart) || (curNode->vertex[0] == cutEnd) ||
            (curNode->vertex[1] == cutEnd) || (curNode->vertex[2] == cutEnd))
        ...
        for (; vIdx < 3 && !bHasPointOnEdge; vIdx++) {
            for (ListNodes::iterator li = rightSideNodes.begin(); li != rightSideNodes.end() && !bHasPointOnEdge; li++) {
                ...
                if (!pNewLink)
                    continue;
                if (refNode->vertex[pNewLink->nStartIndex] == curNode->vertex[vIdx] || refNode->vertex[pNewLink->nEndIndex] == curNode->vertex[vIdx])
        ...
        Vec3d theOtherVertex = m_pAISystem->m_VertexList.GetVertex(curNode->vertex[otherVertexIdx1]).vPos - vCutStart;
        theOtherVertex.normalize();
        float curCross = cutN.x * theOtherVertex.y - cutN.y * theOtherVertex.x;
        theOtherVertex = m_pAISystem->m_VertexList.GetVertex(curNode->vertex[otherVertexIdx2]).vPos - vCutStart;
        theOtherVertex.normalize();
    ...
    for (it = nodesList.begin(); it != nodesList.end(); it++) {
        ...
        for (vi = curNode->link.begin(); vi != curNode->link.end(); vi++)
            if ((*vi).fMaxRadius < 0.0f ||
                ...
                //                )
                (curNode->vertex[(*vi).nStartIndex] == cutStart && curNode->vertex[(*vi).nEndIndex] == cutEnd) ||
                (curNode->vertex[(*vi).nEndIndex] == cutStart && curNode->vertex[(*vi).nStartIndex] == cutEnd))
    ...
    // for(it=insideNodesCurrent.begin(); it!=insideNodesCurrent.end(); it++)
    {
        ...
        for (vi = curNode->link.begin(); vi != curNode->link.end(); vi++) {
            if ((*vi).IsNewLink()) // it's on the edge - it's a new node
                if ((curNode->vertex[(*vi).nStartIndex] == (*outline.begin())) || (curNode->vertex[(*vi).nEndIndex] == (*outline.begin()))) {
...
float GraphNode::GetCross(const Vec3d& vCutStart, const Vec3d& vDir, const GraphLink& theLink) {
    Vec3d theOtherVertex = GetAISystem()->m_VertexList.GetVertex(vertex[3 - (theLink.nStartIndex + theLink.nEndIndex)]).vPos - vCutStart;
    theOtherVertex.normalize();
