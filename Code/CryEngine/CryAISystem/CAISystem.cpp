...
CAISystem::CAISystem(ISystem* pSystem) : m_nNumPuppets(0), m_pTheSkip(nullptr) {
    ...
    m_cvRunAccuracyMultiplier = pSystem->GetIConsole()->CreateVariable("ai_autobalance", "0", 0, "Set to 1 to enable autobalancing.");
    m_cvTargetMovingAccuracyMultiplier =
        pSystem->GetIConsole()->CreateVariable("ai_allow_accuracy_increase", "0", VF_SAVEGAME, "Set to 1 to enable AI accuracy increase when target is standing still.");
    m_cvLateralMovementAccuracyDecay =
        pSystem->GetIConsole()->CreateVariable("ai_allow_accuracy_decrease", "1", VF_SAVEGAME, "Set to 1 to enable AI accuracy decrease when target is moving lateraly.");
...
bool CAISystem::Init(ISystem* pSystem, const char* szLevel, const char* szMission) {
    ...
    m_cvPercentSound = pConsole->CreateVariable("ai_percent_sound","0", VF_CHEAT, "This variable is not used.\n");
...
void CAISystem::DebugDraw(IRenderer* pRenderer) {
    ...
    if (m_cvDebugDraw->GetIVal() == 100) {
        ...
        for (li = m_pGraph->m_lstTrapNodes.begin(); li != liend; ++li) {
            ...
            for (vi = pThisNode->link.begin(); vi != pThisNode->link.end(); vi++) {
                ...
                pRenderer->TextToScreenColor(100, 100, 1, 0, 0, 1, "BAD TRIANGLE FOUND (%.3f,%.3f,%.3f)", pThisNode->data.m_pos.x, pThisNode->data.m_pos.y,
                                             pThisNode->data.m_pos.z);
            } // vi
        }
    }
    ...
    if (m_pCurrentRequest) {
        pRenderer->TextToScreen(1, 22, "PATHFINDER HOG:> %s, (%.3f,%.3f,%.3f)", m_pCurrentRequest->pRequester->GetName(), m_pCurrentRequest->pRequester->GetPos().x,
                                m_pCurrentRequest->pRequester->GetPos().y, m_pCurrentRequest->pRequester->GetPos().z);
        pRenderer->SetMaterialColor(0, 1, 0, 1);
    ...
    if (stricmp(m_cvStatsTarget->GetString(), "none")) {
        ...
        if (pTargetObject) {
            ...
            if (pTargetObject->CanBeConvertedTo(AIOBJECT_CPUPPET, (void**)&pTargetPuppet)) {
                ...
                if (pTargetPuppet->m_State.aimLook)
                    pRenderer->TextToScreen(0, 68, "RESPONSIVENESS (NEW/OLD): (%.2f/%.2f)", pTargetPuppet->GetParameters().m_fResponsiveness,
                                            pTargetPuppet->GetParameters().m_fResponsiveness * 7.5f / 400.f);
...
IGraph* CAISystem::GetNodeGraph() {
    return (IGraph*)m_pGraph;
}

void CAISystem::FlushSystem() {
    m_lstWaitingToBeUpdated.clear();
    m_lstAlreadyUpdated.clear();
            ...
            if (pPuppet->m_mapDevaluedPoints.find(ai->second) == pPuppet->m_mapDevaluedPoints.end()) {
                ...
                if (nOption & AIFAF_VISIBLE_TARGET) {
                    if (pPuppet->m_pAttentionTarget) {
                        ...
                        ray_hit rh;
                        int colliders =
                            m_pWorld->RayWorldIntersection(vectorf(ob_pos), vectorf(pRealTarget->GetPos() - ob_pos), ent_static | ent_terrain, rwi_stop_at_pierceable, &rh, 1);
                        if (colliders)
                            continue;
                    }
                }
            ...
            if ((pCurrent->vertex.size() == 3)) {
                ...
                for (li = pCurrent->link.begin(); li != pCurrent->link.end(); li++) {
                    ...
                    if (IsForbidden(m_VertexList.GetVertex(pCurrent->vertex[(*li).nStartIndex]).vPos, m_VertexList.GetVertex(pCurrent->vertex[(*li).nEndIndex]).vPos)) {
                        ...
                        /*
                                                                ray_hit rh;
                                                                int colliders = m_pWorld->RayWorldIntersection(vectorf(vStart),vectorf(vEnd-vStart),ent_static,
                rwi_stop_at_pierceable  | geom_colltype_player<<rwi_colltype_bit,&rh,1); while (colliders)
                                                                {
                ...
                //                colliders =
                m_pWorld->RayWorldIntersection(vectorf(vStartTrace),vectorf(vEndTrace-vStartTrace),ent_static, 0,&rh,1);
                //                continue;
                //              }
                                                                        ...
                                                                        if ( normal.Dot(vEnd-vStart) > 0.00001f )
                                                                        {
                                                                                ...
                                                                                if (!IsEquivalent(vStart,vEnd,0.001f))
                                                                                {
                                                                                        ...
                                                                                        colliders = m_pWorld->RayWorldIntersection(vectorf(vStart),vectorf(vEnd-vStart),ent_static,
                rwi_stop_at_pierceable  | geom_colltype_player<<rwi_colltype_bit,&rh,1);
                                                                ...
                                                                if (!IsEquivalent(vStart,vEnd,0.1f))
                                                                        colliders = m_pWorld->RayWorldIntersection(vectorf(vEnd),vectorf(vStart-vEnd),ent_static,
                rwi_stop_at_pierceable  | geom_colltype_player<<rwi_colltype_bit,&rh,1); else colliders = 0;
                    ...
                    while ((ai != m_Objects.end()) && (ai->first == (*li))) {
                        ...
                        if (pPuppet->PointVisible(pTarget->GetPos())) {
                            ...
                            if (pPuppet->m_bHaveLiveTarget) {
                                ray_hit hit_aux[2];
                                rwi = m_pWorld->RayWorldIntersection(vectorf(ppos), vectorf(pTarget->GetPos() - ppos), ent_terrain | ent_static | ent_sleeping_rigid, 0,
                                                                     &hit_aux[0], 2, skip, m_pTheSkip);
                                if (rwi) {
                                    if (hit_aux[0].dist < 0) {
                                ...
                                rwi = m_pWorld->RayWorldIntersection(vectorf(ppos), vectorf(pTarget->GetPos() - ppos), ent_terrain | ent_static | ent_sleeping_rigid, 13, &hit, 1,
                                                                     skip, m_pTheSkip);
                            }
                            ...
                            if (rwi && !pPuppet->m_bHaveLiveTarget) {
                                if (hit.n.Dot(pTarget->GetPos() - ppos) > 0) {
                                    rwi = m_pWorld->RayWorldIntersection(vectorf(hit.pt), vectorf(pTarget->GetPos() - hit.pt), ent_terrain | ent_static | ent_sleeping_rigid, 13,
                                                                         &hit, 1, skip, m_pTheSkip);
                                    m_nNumRaysShot++;
                                    m_nRaysThisUpdateFrame++;
                                }
...
IAutoBalance* CAISystem::GetAutoBalanceInterface() {
