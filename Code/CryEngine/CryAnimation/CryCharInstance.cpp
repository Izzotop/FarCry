...
CryCharInstance::CryCharInstance(CryCharBody* pBody) : m_pCryCharBody(pBody), m_fLastAnimUpdateTime(0), m_bEnableStartAnimation(true) {
...
bool CryCharInstance::StartAnimation(const char* szAnimName, const struct CryCharAnimationParams& Params) {
    ...
    if (!m_bEnableStartAnimation) {
        ...
        g_GetLog()->LogError("\002%d. %p->StartAnimation(file=%s): %s, Layer=%u, blending:in=%.2f,out=%.2f  -  called while StartAnimation is disabled", g_nFrameID, this,
                             m_pCryCharBody->GetNameCStr(), szAnimName, Params.nLayerID, Params.fBlendInTime, Params.fBlendOutTime);
        // GetSystem()->LogCallStack();
    }

    if (g_GetCVars()->ca_Debug())
        g_GetLog()->Log("\002%p->StartAnimation(file=%s): %s, Layer=%u, blending:in=%.2f,out=%.2f", this, m_pCryCharBody->GetNameCStr(), szAnimName, Params.nLayerID,
                        Params.fBlendInTime, Params.fBlendOutTime);

    if (Params.nLayerID & ~0xF) {
        g_GetLog()->LogError("\002%p->StartAnimation(file=%s): %s, Layer=%u, blending:in=%.2f,out=%.2f: Layer is out of range", this, m_pCryCharBody->GetNameCStr(), szAnimName,
                             Params.nLayerID, Params.fBlendInTime, Params.fBlendOutTime);
        return false;
    }
...
const float CryCharInstance::GetRadius() {
    ...
    if (vSize.z >= 32)
        g_GetLog()->LogWarning("CryCharInstance::GetRadius: bbox is very big: %s (%.2f)", m_pCryCharBody->GetNameCStr(), vSize.z);
...
// If pWeaponModel is nullptr, detaches any object from the given bone
...
ICryCharInstance::ObjectBindingHandle CryCharInstance::AttachObjectToBone(IBindable* pWeaponModel, const char* szBoneName, bool bUseRelativeToDefPoseMatrix, unsigned nFlags) {
    ...
    if (nBone < 0) {
        if (!pWeaponModel) {
            ...
            g_GetLog()->LogError("\002AttachObjectToBone is called for bone \"%s\", which is not in the model \"%s\". Ignoring, but this may cause a crash because the "
                                 "corresponding object won't be detached after it's destroyed",
                                 szBoneName, m_pCryCharBody->GetFilePathCStr());
#ifdef _DEBUG
...
//////////////////////////////////////////////////////////////////////////
//! Returns the light by the light handle; returns nullptr if no such light found
...
void CryCharInstance::RenderShadowVolumes(const SRendParams* rParams, int nLimitLOD) {
    ...
    if (!m_arrBinds.empty()) {
        ...
        for (BindArray::iterator it = m_arrBinds.begin(); it != m_arrBinds.end(); ++it) {
            ...
            if (pBoundObject) { // get weapon position
...
int CryCharInstance::GetUpdateFrequencyMask(Vec3 vPos, float fRadius) {
    ...
    else if (fScaledDist < 64.f)
        iMask = 3; // close to camera but outside of frustum
    ...
    //  g_pIRenderer->Draw2dLabel( 1,g_YLine, 1.3f, col, false,"vPos:(%15.10f,%15.10f,%15.10f)  t:(%15.10f,%15.10f,%15.10f)",vPos.x,vPos.y,vPos.z, t.x,t.y,t.z);
    // g_YLine+=16.0f;
...
void CryCharInstance::Update(Vec3d vPos, float fRadius, unsigned uFlags) {
    ...
    // float fColor[4] = {0,1,0,1};
    // g_pIRenderer->Draw2dLabel( 1,g_YLine, 1.3f, fColor, false,"InstanceNum:%d  nFrameID:%d (%01d %01d) fRadius:%15.10f ",m_pModelState->getInstanceNumber(), nFrameID,
    // update,nUFM, fRadius );  g_YLine+=16.0f;
    ...
    */
    ...
    if (m_pModelState) {
        ...
        /*    Vec3 trans=RendParams.vPos+m_pModelState->m_vOffset;
                        float fColor[4] = {0,1,0,1};
                        extern float g_YLine;
                        g_pIRenderer->Draw2dLabel( 1,g_YLine, 1.3f, fColor, false,"render trans: %15.10f %15.10f %15.10f %08x",trans.x,trans.y,trans.z,RendParams.pMatrix );
           g_YLine+=16.0f; g_YLine+=16.0f;*/
...
void CryCharInstance::AddAnimationEventSink(const char* szAnimName, ICharInstanceSink* pCharInstanceSink) {
    ...
    if (nAnimId < 0) {
        if (g_GetCVars()->ca_AnimWarningLevel() >= 2)
            g_GetLog()->LogWarning("CryCharInstance(0x%p)::AddAnimationEventSink(\"%s\",0x%p) - animation not found (model \"%s\")", this, szAnimName, pCharInstanceSink,
                                   m_pCryCharBody->GetFilePathCStr());
    } else {
        // g_GetLog()->LogToFile("\005CryCharInstance(0x%p)::AddAnimationEventSink(\"%s\",0x%p) - animation not found (model \"%s\")", this, szAnimName, pCharInstanceSink,
        // m_pCryCharBody->GetFilePathCStr());
...
void CryCharInstance::RemoveAnimationEventSink(const char* szAnimName, ICharInstanceSink* pCharInstanceSink) {
    int nAnimId = m_pCryCharBody->GetModel()->findAnimation(szAnimName);
    if (nAnimId < 0)
        g_GetLog()->LogError("CryCharInstance(0x%p)::RemoveAnimationEventSink(\"%s\",0x%p) - animation not found (model \"%s\")", this, szAnimName, pCharInstanceSink,
                             m_pCryCharBody->GetFilePathCStr());
    else {
        // g_GetLog()->LogToFile("\005CryCharInstance(0x%p)::RemoveAnimationEventSink(\"%s\",0x%p) - animation not found (model \"%s\")", this, szAnimName, pCharInstanceSink,
        // m_pCryCharBody->GetFilePathCStr());
...
#if defined(LINUX)
        g_GetLog()->LogToFile("\001  %d binds, speed %f, matrix:{%f,%f,%f,%f}{%f,%f,%f,%f}{%f,%f,%f,%f}{%f,%f,%f,%f}", m_arrBinds.size(), m_fAnimSpeedScale,
                              m_matTranRotMatrix.GetData());
#else
