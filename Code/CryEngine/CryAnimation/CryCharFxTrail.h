...
protected:
    ...
    unsigned getTailEntry() const {
        return (m_nHeadEntry + m_numEntries - 1) % numMaxEntries();
    }

    // max number of history entries
    unsigned numMaxEntries() const {
        return m_Params.numMaxQuads + 1;
    }

    unsigned numVertices() const {
        return m_Params.numVerts * numMaxEntries();
    }
