...
CryModelAnimationContainer::CryModelAnimationContainer(CControllerManager* pControllerManager)
    : m_pControllerManager(pControllerManager), m_arrBones("CryModelAnimationContainer.BoneInfo"), m_arrMorphTargets("CryModelAnimationContainer.MorphTargets") {
...
void CryModelAnimationContainer::RegisterAnimation(const char* szFileName, int nGlobalAnimId, const char* szAnimName) {
    ...
    m_arrAnimByGlobalId.insert(std::lower_bound(m_arrAnimByGlobalId.begin(), m_arrAnimByGlobalId.end(), nGlobalAnimId, AnimationGlobIdPred(m_arrAnimations)),
                               LocalAnimId(nLocalAnimId));
    ...
    if (nLocalAnimId == -1) {
        ...
        if (!g_GetCVars()->ca_AnimationDeferredLoad() || (nGlobalAnimFlags & GlobalAnimation::FLAGS_DISABLE_DELAY_LOAD)) {
            if (g_GetCVars()->ca_Debug() && (g_GetCVars()->ca_AnimationUnloadDelay() > 0 || !(nGlobalAnimFlags & GlobalAnimation::FLAGS_DISABLE_AUTO_UNLOAD)))
                g_GetLog()->LogWarning("\003Performance penalty: animation deferred load is disabled, but automatic animation unload is enabled. This will most surely lead to "
                                       "poor performance upon loading the level (because everything is loaded, more memory is used and massive precalculations are needed) and "
                                       "even some time after (because of animation controllers trashing) (animation %s)",
                                       szAnimName);
        ...
        g_GetLog()->LogError("\003Trying to load animation with alias \"%s\" from file \"%s\" into the animation container. Such animation alias already exists and uses file "
                             "\"%s\". Please use another animation alias.",
                             szAnimName, szFileName, m_pControllerManager->GetAnimation(nGlobalAnimID).strFileName.c_str());
    }
