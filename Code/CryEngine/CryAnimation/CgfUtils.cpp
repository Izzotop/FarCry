...
// if the given string is nullptr, assigns the "" to the pointer
void chknullptrString(const char*& pszString) {
    if (!pszString)
        pszString = "";
}
...
CMatEntityNameTokenizer::CMatEntityNameTokenizer() : m_szMtlName(nullptr), szName(""), szTemplate(""), szPhysMtl(""), nSortValue(0), bInvert(false) {}
...
void CMatEntityNameTokenizer::tokenize(const char* szMtlFullName) {
    ...
    enum StateEnum { kUnknown, kName, kTemplate, kPhysMtl, kIndex };
    ...
    if ((!this->szTemplate || !this->szTemplate[0]) && this->szName[0] == '$' && tolower(this->szName[1]) == 's' && this->szName[2] == '_') {
    ...
    chknullptrString(this->szName);
    chknullptrString(this->szTemplate);
    chknullptrString(this->szPhysMtl);
