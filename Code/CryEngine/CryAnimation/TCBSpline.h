...
template <int N> class BasisFunction {
public:
...
template <class T> inline void Zero(T& val) {
...
**                            Key classes                                   **
****************************************************************************/
template <class T> struct SplineKey {
...
template <class T> bool operator==(const SplineKey<T>& k1, const SplineKey<T>& k2) {
...
/*!
        TCB spline key.
*/
template <class T> struct TCBSplineKey : public SplineKey<T> {
    // Key controls.
...
**                           Spline class                                   **
****************************************************************************/
template <class KeyType, class BasisType> class TSpline {
public:
    ...
    // Keys access methods.
    void reserve_keys(int num) {
        m_keys.reserve(num);
    }; // Reserve memory for more keys.
    void resize(int num) {
        m_keys.resize(num);
        m_flags |= MODIFIED;
    }; // Set new key count.
    bool empty() const {
        return m_keys.empty();
    }; // Check if curve empty (no keys).
    int num_keys() const {
        return (int)m_keys.size();
    }; // Return number of keys in curve.
    key_type& key(int n) {
        return m_keys[n];
    }; // Return n key.
    float time(int n) const {
        return m_keys[n].time;
    }; // Shortcut to key n time.
    value_type& value(int n) {
        return m_keys[n].value;
    }; // Shortcut to key n value.
    value_type& ds(int n) {
        return m_keys[n].ds;
    }; // Shortcut to key n incoming tangent.
    value_type& dd(int n) {
        return m_keys[n].dd;
    }; // Shortcut to key n outgoing tangent.
...
template <class T, class Basis> inline TSpline<T, Basis>::TSpline() {
    m_flags = MODIFIED;
    m_ORT = 0;
    m_curr = 0;
    m_rangeStart = 0;
...
template <class T, class Basis> inline int TSpline<T, Basis>::seek_key(float t) {
    ...
    if (m_curr < num_keys()) {
        int last = num_keys() - 1;
        while ((m_curr != last) && (time(m_curr + 1) <= t))
            ++m_curr;
    }
    return m_curr;
}

template <class T, class Basis> inline void TSpline<T, Basis>::interpolate(float tm, value_type& val) {
    if (empty())
        return;
...
****************************************************************************/
template <class T, class Key = TCBSplineKey<T>> class TCBSpline : public TSpline<Key, HermitBasis> {
public:
...
template <class T, class Key> inline void TCBSpline<T, Key>::compMiddleDeriv(int curr) {
...
template <class T, class Key> inline void TCBSpline<T, Key>::compFirstDeriv() {
    typename TSpline<Key, HermitBasis>::key_type& k = this->key(0);
    Zero(k.ds);
    k.dd = 0.5f * (1.0f - k.tens) * (3.0f * Subtract(Subtract(this->value(1), k.value), this->ds(1)));
}

template <class T, class Key> inline void TCBSpline<T, Key>::compLastDeriv() {
    int last = this->num_keys() - 1;
    typename TSpline<Key, HermitBasis>::key_type& k = this->key(last);
    k.ds = -0.5f * (1.0f - k.tens) * (3.0f * Concatenate(Subtract(this->value(last - 1), k.value), this->dd(last - 1)));
    Zero(k.dd);
}

template <class T, class Key> inline void TCBSpline<T, Key>::comp2KeyDeriv() {
...
template <class T, class Key> inline void TCBSpline<T, Key>::comp_deriv() {
    if (this->num_keys() > 1) {
...
template <class T, class Key> inline float TCBSpline<T, Key>::calc_ease(float t, float a, float b) {
    float k;
    float s = a + b;
...
template <class T, class Key> inline void TCBSpline<T, Key>::interp_keys(int from, int to, float u, T& val) {
