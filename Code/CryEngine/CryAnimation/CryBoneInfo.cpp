...
// RETURNS:
//  The binded controller, if the bind was successful, or nullptr if there's
...
void CryBoneInfo::PostInitialize() {
    if (getParent()) {
        ...
        m_pqDefRelTransform.vPos = getParent()->getInvDefGlobal().TransformPointOLD((matrix3x3in4x4f&)getInvDefGlobal() * -getInvDefGlobal().GetTranslationOLD());
