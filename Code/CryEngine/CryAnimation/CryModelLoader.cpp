...
CryModelLoader::CryModelLoader(CControllerManager* pControllerManager)
    : m_pControllerManager(pControllerManager), m_fCalFile(nullptr), m_nCafFindFileHandle(-1), m_pModel(nullptr), m_bLoadFromCCG(false), m_bExtCCG(false) {}
...
CryModel* CryModelLoader::loadNew(CryCharBody* pBody, const string& strGeomFileName, float fScale) {
  ...
  // on this stage, if m_pCryModel == nullptr, it means the loader couldn't load the model
  if (m_pModel)
...
bool CryModelLoader::loadAnimations() {
...

    {
        ...
        else if (m_nCafFindFileHandle != -1)
...

    {
        ...
        if (!numAnimations && !m_pModel->numMorphTargets()) {
            g_GetLog()->LogWarning("\004CryModelLoader::loadAnimations(%s): couldn't find any animations for the model. Standalone character will be useless.",
                                   m_strGeomFileNameNoExt.c_str());
            // return false;
        }
    ...
    return m_pModel->m_pDefaultModelState && ((m_pModel->numBoneInfos() && m_pModel->m_pDefaultModelState->getRootBone()) || m_pModel->numMorphTargets());
}
...
unsigned CryModelLoader::loadAnimationArray(const AnimFileArray& arrAnimFiles) {
    ...
    for (unsigned i = 1; i < arrAnimFiles.size(); ++i) {
        if (m_pModel->LoadCAF(arrAnimFiles[i].strFileName.c_str(), m_fScale, nAnimID, arrAnimFiles[i].strAnimName.c_str(), arrAnimFiles[i].nAnimFlags) >= 0) {
            ...
            g_GetLog()->LogWarning("\002Animation (Caf) file \"%s\" could not be read (it's an animation of \"%s.cgf\")", arrAnimFiles[i].strFileName.c_str(),
                                   m_strGeomFileNameNoExt.c_str());
    }
    ...
    AnimFileArray arrAnimFiles;
    {
        ...
        for (int i = 0; m_fCalFile && !g_GetPak()->FEof(m_fCalFile); ++i) {
            ...
            if (szAnimName[0] == '$') {
                ...
                if (!stricmp(szDirective, "AnimationDir") || !stricmp(szDirective, "AnimDir") || !stricmp(szDirective, "AnimationDirectory") ||
                    !stricmp(szDirective, "AnimDirectory")) {
                    strAnimDirName = strDirName + "\\" + szFileName;
                    // delete the trailing slashes
                    while (!strAnimDirName.empty() && strAnimDirName[strAnimDirName.length() - 1] == '\\')
                        strAnimDirName[strAnimDirName.length() - 1] = '\0';
                } else if (!stricmp(szDirective, "ModelOffsetX")) {
                    float fValue;
                ...
                } else if (!stricmp(szDirective, "ModelOffsetY")) {
                    float fValue;
                    if (sscanf(szFileName, "%f", &fValue) != 1)
                ...
                } else if (!stricmp(szDirective, "ModelOffsetZ")) {
                    float fValue;
                    if (sscanf(szFileName, "%f", &fValue) != 1)
                ...
                } else if (!stricmp(szDirective, "AutoUnload")) {
                ...
                } else if (!stricmp(szDirective, "DelayLoad")) {
                    switch (CryStringUtils::toYesNoType(szFileName)) {
                    case CryStringUtils::nYNT_Yes:
