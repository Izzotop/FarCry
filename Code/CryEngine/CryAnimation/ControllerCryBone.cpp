...
CControllerCryBone::CControllerCryBone() : m_nControllerId(0), m_arrTimes("CControllerCryBone.Keys"), m_arrKeys("CControllerCryBone.Keys") {}
...
bool CControllerCryBone::Load(const CONTROLLER_CHUNK_DESC_0827* pChunk, unsigned nSize, float fScale) {
    ...
    if ((const char*)(pCryKey + numKeys) > ((const char*)pChunk) + nSize) {
        GetLog()->LogError("\002CControllerCryBone::Load: cannot load controller 0x%08X, chunk is truncated", m_nControllerId);
        return false;
    }
