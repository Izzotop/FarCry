 ...
 *******************************************************************************/
#ifndef WIN32
extern "C" void portable_splitpath(const char* path, char* drive, char* dir, char* fname, char* ext) {
    char* p;
    ...
    if (last_slash) {
        ...
        if (dir) {
            len = min((unsigned)(((char*)last_slash - (char*)path) / sizeof(char)), (_MAX_DIR - 1));
    ...
    if (dot && (dot >= path)) {
         ...
         */
        if (fname) {
            len = min((unsigned)(((char*)dot - (char*)path) / sizeof(char)), (_MAX_FNAME - 1));
         ...
         */
        if (ext) {
            len = min((unsigned)(((char*)p - (char*)dot) / sizeof(char)), (_MAX_EXT - 1));
            strncpy(ext, dot, len);
            *(ext + len) = ('\0');
        }
    } else {
         ...
         */
        if (fname) {
            len = min((unsigned)(((char*)p - (char*)path) / sizeof(char)), (_MAX_FNAME - 1));
            strncpy(fname, path, len);
            *(fname + len) = ('\0');
        }
        if (ext) {
