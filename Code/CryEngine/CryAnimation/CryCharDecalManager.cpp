...
CryCharDecalManager::CryCharDecalManager(class CryGeometryInfo* pGeomInfo) : m_pGeometry(pGeomInfo), m_bNeedUpdateIndices(false) {
...
CryCharDecalManager::~CryCharDecalManager() {
    ...
    if (!m_arrOldRE.empty()) {
        if (!g_GetISystem()->IsQuitting())
            g_LogToFile("Warning: ~CryCharDecalManager: There are still %d render elements present that may be in rendering queue. But since destruction was requested, attempting "
                        "to destruct those elements",
                        m_arrOldRE.size());
...
bool CryCharDecalManager::NeedRealize() const {
    return !m_arrDecals.empty() ||
           !m_arrDecalRequests.empty()
...
void CryCharDecalManager::RefreshVertexBufferIndices() {
    ...
    for (unsigned nMaterial = 0; nMaterial < numMaterials; ++nMaterial) {
        assignMaterial(
            nMaterial,
            g_SubmeshInfo[nMaterial].nTextureId,
            g_SubmeshInfo[nMaterial].nFirstIndex,
            g_SubmeshInfo[nMaterial].numIndices,
            g_SubmeshInfo[nMaterial].nFirstVertex,
            g_SubmeshInfo[nMaterial].numVertices
        );
    }
}
...
unsigned CryCharDecalManager::groupMaterials() {
    ...
    for (unsigned i = 0; i < numDecals; ++i) {
        ...
        if ((pNext == g_SubmeshInfo || pNext[-1].nTextureId != rDecal.getTextureId()) && pNext < pEnd) // we don't support more than the given amount of decal types
        {
...
void CryCharDecalManager::LogStatistics() {
    if (!g_Statistics.empty())
        g_GetLog()->LogToFile("%d decals created, %d total vertices, %d faces, %.1f vers/decal, %.1f faces/decal", g_Statistics.numDecals, g_Statistics.numDecalVertices,
                              g_Statistics.numDecalFaces, g_Statistics.getAveVertsPerDecal(), g_Statistics.getAveFacesPerDecal());
}
...
void CryCharDecalManager::debugDump() {
    ...
    for (i = 0; i < m_arrDecals.size(); ++i) {
        CryCharDecal& rDecal = m_arrDecals[i];
        g_GetLog()->Log("\001      decal %3d: %d verts, %d faces, \"%s\" (texId=%d)", i, rDecal.numVertices(), rDecal.numFaces(),
                        g_GetIRenderer()->EF_GetTextureByID(rDecal.getTextureId())->GetName(), rDecal.getTextureId());
    }
    for (i = 0; i < numMaterials; ++i) {
        SubmeshInfo& rSubmesh = g_SubmeshInfo[i];
        g_GetLog()->Log("\001      chunk %d: %d verts @%d, %d indices @%d, texture %d \"%s\"", i, rSubmesh.numVertices, rSubmesh.nFirstVertex, rSubmesh.numIndices,
                        rSubmesh.nFirstIndex, rSubmesh.nTextureId, g_GetIRenderer()->EF_GetTextureByID(rSubmesh.nTextureId)->GetName());
    }
}
