...
public:
    ...
    int mapIndexToId(int nIndex) const {
        return m_arrIndexToId[nIndex];
    }
    const int* getIndexToIdMap() const {
        return &m_arrIndexToId[0];
    }
    int mapIdToIndex(int nId) const {
        return m_arrIdToIndex[nId];
    }
    const int* getIdToIndexMap() const {
        return &m_arrIdToIndex[0];
    }

    // initializes the map id->index (inverse of mapIndexToId)
    void getIdToIndexMap(unsigned* pMap) const;

    const CryBoneDesc& getBoneByIndex(unsigned nIndex) const {
        return m_arrBones[nIndex];
    }
    ...
    unsigned numBones() const {
        return m_pChunkBoneAnim ? m_pChunkBoneAnim->nBones : 0;
    }

    bool hasInitPos() const {
        return !m_arrInitPose.empty();
    }
    const Matrix44& getInitPosMatrixByIndex(int nBoneIndex) {
        return m_arrInitPose[mapIndexToId(nBoneIndex)];
    }
