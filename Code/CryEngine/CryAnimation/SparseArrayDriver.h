...
template <class T> class TSparseArrayDriver {
protected:
...
public:
    ...
    TSparseArrayDriver(void* pData, int nStride) : m_pData((byte*)pData), m_nStride(nStride) {}
    ...
    TSparseArrayDriver<T>& operator+=(int nShift) {
        m_pData += nShift * m_nStride;
        return *this;
    }
    TSparseArrayDriver<T>& operator++() {
        m_pData += m_nStride;
        return *this;
    }
