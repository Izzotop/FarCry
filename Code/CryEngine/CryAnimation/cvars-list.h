...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_MorphNormals, 0,
                               "this variable enables changing of normals during morphing. Required for the reflection effect when only vertex animation is used");
...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_CollapseRangeBegin, 0,
                               "Debug variable: If you set the DeformRange to non-empty range, then the vertices in that interval won't be skinned for all animatable characters");
DECLARE_INT_VARIABLE_IMMEDIATE(ca_CollapseRangeEnd, 0,
                               "Debug variable: If you set the DeformRange to non-empty range, then the vertices in that interval won't be skinned for all animatable characters");
...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_DisablePlusRotationMask, 0,
                               "flag set:\n0 - effectively does nothing (enables all coordinate transforms)\n1 - disables Z rotation\n2 - disables Y rotation\n4 - disables X "
                               "rotation\ncan be used ORed: 1+4 disables X and Z rotations");

// if set to 0, effectively disables creation of decals on characters
// 2 - alternative method of calculating/building the decals
DECLARE_INT_VARIABLE_IMMEDIATE_DUMP(ca_EnableDecals, 1,
                                    "if set to 0, effectively disables creation of decals on characters\n2 - alternative method of calculating/building the decals");

DECLARE_INT_VARIABLE_IMMEDIATE(ca_PerforatingDecals, 1,
                               "if set to 1, all decals will be penetrating, i.e. the bullets will pierce through the character. 0 - decal will be placed only around the bullet "
                               "income position; this can yield incorrect results with gutter [tangential] wounds");
...
DECLARE_FLOAT_VARIABLE(ca_MinVertexWeightLOD0, 0.07f,
                       "this is the threshold of LOD minimal vertex weight that will be truncated\nthe vertex bindings with the given weight and less will be "
                       "truncated\nEffectively, the value >0.5 sets rigid linking, because there is never more than 1 weight > 0.5");
DECLARE_FLOAT_VARIABLE(ca_MinVertexWeightLOD1, 0.3f,
                       "this is the threshold of LOD minimal vertex weight that will be truncated\nthe vertex bindings with the given weight and less will be "
                       "truncated\nEffectively, the value >0.5 sets rigid linking, because there is never more than 1 weight > 0.5");
DECLARE_FLOAT_VARIABLE(ca_MinVertexWeightLOD2, 0.5f,
                       "this is the threshold of LOD minimal vertex weight that will be truncated\nthe vertex bindings with the given weight and less will be "
                       "truncated\nEffectively, the value >0.5 sets rigid linking, because there is never more than 1 weight > 0.5");
...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_VertsPerTangSubskin, 3000,
                               "this parameter determines how many vertices can be used in one tangent basis skin\nThis is used for strip-mining optimization recommended by the "
                               "Pentium4 Optimization Manual:\nonly part of the skin is processed at a moment.");
...
// purposes.
DECLARE_INT_VARIABLE_IMMEDIATE(
    ca_ZDeleteConstructionData, 1,
...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_TangentBasisCalcPeriod, 1,
                               "the period, in frames, when the tangents will be recalculated for the character. 1 means ever frame, 2 means every 2nd frame and so on");

// 0 - the same shadow vertex buffer is used for every frame shadow volume rendering
// 1 - the shadow vertex buffers are switched
DECLARE_INT_VARIABLE_IMMEDIATE(ca_ShadowDoubleBuffer, 1,
                               "0 - the same shadow vertex buffer is used for every frame shadow volume rendering\n1 - the shadow vertex buffers are switched");
...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_ShadowBufferLimit, 6,
                               "This is the maximum number of shadow buffers per instance.\nWith double buffering, there must be 2 * max number of light sources");
...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_RainPower, 5,
                               "this is the \"power\" with which the rain drops the character:\nwith infinity, only the vertex that's the most parallel to the rain will react to "
                               "the rain;\nwith 0, random point (including those not facing the rain flow) will be chosen.");
...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_LimitShadowLOD, 0,
                               "this is the maximum LOD to use for shadow volumes; if it's 0, it effectively does nothing\n(all the range of LODs is available for shadow "
                               "builder), if it's 1, then objects will use\nLOD 1 shadows for LOD 0 characters if available .  Etc.");
...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_EnableCubicBlending, 1,
                               "if this is true, then the blending between layers is performed by \ncubic (spline) function, not linear. This should look more natural");
...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_EnableVertexColors, 0,
                               "if set to 1, during loading of the model, the vertex colors will be loaded and they will be updated in the vertex buffer");

// if set to 1, the lights (heat and light sources, dynamic and bound during CGF load) will be updated
// upon each Draw() call
DECLARE_INT_VARIABLE_IMMEDIATE(ca_EnableLightUpdate, 1,
                               "if set to 1, the lights (heat and light sources, dynamic and bound during CGF load) will be updated upon each Draw() call");

// If not 0, then the unused animations get unloaded after the specified number of frames being not used
DECLARE_INT_VARIABLE_IMMEDIATE(ca_AnimationUnloadDelay, nDefaultAnimUnloadDelay,
                               "If not 0, then the unused animations get unloaded after the specified number of frames being not used");
...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_AnimationDeferredLoad, nDefaultAnimDeferredLoad,
                               "if this is non-0, the animation are not loaded immediately upon request (when loading CGF), but\nrather on first request. This feature should not "
                               "be used with ca_AnimationUnloadDelay, because loading\nall animations first and then automatically unloading them doesn't have much sense.");
...
DECLARE_INT_VARIABLE_IMMEDIATE(ca_SkinBasedBBoxMask, 0xFFFFFFFF,
                               "This (binary) mask is applied to the frame id to determine if the calculation of character BBox based on the skin vertex coordinates is enabled");
...
/*! Controls the intensity of the additional directional light. Right now, we only allow for different gray levels.
        Initial value = 0.2f */
DECLARE_FLOAT_VARIABLE_DUMP(ca_ambient_light_intensity, 0.2f,
                            "Controls the intensity of the additional directional light. Right now, we only allow for different gray levels.\nInitial value = 0.2f");
...
DECLARE_FLOAT_VARIABLE(ca_BoundZOffset, 0.0015f,
                       "This is the relative offset of the bound objects with the corresponding flag set. It's a hack to avoid hemlets from penetrating the head when looking at a "
