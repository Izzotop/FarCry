...
CryCharDecalBuilder::CryCharDecalBuilder(CryEngineDecalInfo& rDecal, CryGeometryInfo* pGeometry, const Vec3d* pVertices)
    : m_rDecal(rDecal), m_pGeometry(pGeometry), m_pSkinVertices(pVertices) {
...
float distanceToThinTriangle(const Vec3d v[3]) {
    float d[3] = {(float)distanceToPoint2Dxy(v[0]), (float)distanceToPoint2Dxy(v[1]), (float)distanceToPoint2Dxy(v[2])};
