...
CryGeometryInfo::CryGeometryInfo()
    : m_pStencilShadowConnectivity(nullptr), m_numUsedVertices(0), m_arrVertBuf("CryGeometryInfo.VertBuf"), m_nVertBufFormat(-1), m_arrExtTangents("CryGeometryInfo.ExtTangents"),
      m_arrExtToIntMap("CryGeometryInfo.ExtToIntMap"), m_arrExtUVs("CryGeometryInfo.ExtUVs"), m_arrVertices("CryGeometryInfo.Vertices"), m_arrNormals("CryGeometryInfo.Normals"),
      m_arrTexFaces("CryGeometryInfo.TexFaces"), m_arrUVs("CryGeometryInfo.UVs"), m_arrLinks("CryGeometryInfo.Links"), m_arrIntToExtMap("CryGeometryInfo.IntToExtMap") {}
...
// returns the number of texture faces, which is either nullptr,
...
void CryGeometryInfo::PrepareLinks(int numVertices) {
    assert(numVertices == this->numVertices());
    m_arrLinks.reinit(numVertices);
}

// returns the number of links, link array is either nullptr or an array with numVertices() elements
...
CIndexedMesh* CryGeometryInfo::new3DEngineIndexedMesh() const {
    ...
    CIndexedMesh* pMesh = new CIndexedMesh((unsigned)numFaces(), pCryFaces, getTexFaces(), (unsigned)numVertices(), getVertices(), getNormals(), (unsigned)numUVs(), getUVs());
    delete[] pCryFaces;
    return pMesh;
}
    ...
    if (bHasBoneInfo) {
        ...
        for (int i = 0; i < pChunk->nVerts; i++) {
            ...
            if (arrLinks.empty()) {
                ...
                continue;
            } else if (!EatRawData(&arrLinks[0], (unsigned)arrLinks.size(), pRawData, nChunkSize))
                return 0;
...
bool CryGeometryInfo::ValidateTexFaceIndices() {
    ...
    TexFaceArray::iterator it = m_arrTexFaces.begin(), itEnd = it + numTexFaces();

    // check indices of each face
    for (; it != itEnd; ++it) {
        if (it->t0 >= numUVs || it->t0 < 0 || it->t1 >= numUVs || it->t1 < 0 || it->t2 >= numUVs || it->t2 < 0)
            return false;
    }
    return true; // validation passed successfully
}
...
public:
    CSkinVertexSorter(CryGeometryInfo* pGeom) : m_pGeom(pGeom) {}
...
#endif
            if (m_pStencilShadowConnectivity) // if there's no shadow connectivity yet, we'll need vertices for construction (HACK: we could get rid of it if we didn't split the
                                              // vertices because of different normals per one vertex)
                m_arrVertices.clear();
    }
    m_arrIntToExtMap.clear();
}
...
// already be set up properly and extra fields like color will be nullptred
// Will clean the buffer and return nullptr when the vertex format is < 0
char* CryGeometryInfo::getVertBuf(int nVertFormat) {
    if (nVertFormat == m_nVertBufFormat)
        return &m_arrVertBuf[0];

    // clean the buffer and return nullptr if invalid vertex format
