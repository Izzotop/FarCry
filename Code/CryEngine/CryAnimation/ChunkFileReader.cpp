...
bool CChunkFileReader::open(CFileMapping* pFile) {
    ...
    if ((m_pFile != (CFileMapping*)nullptr) && (m_pFile->getData() != nullptr)) {
        if (m_pFile->getSize() >= sizeof(FileHeader)) { // the file must contain the header
            const FileHeader& fileHeader = getFileHeader();
            if (m_pFile->getSize() >= fileHeader.ChunkTableOffset + sizeof(int) &&
                (int)fileHeader.ChunkTableOffset > (int)sizeof(fileHeader)) { // there must be room for the chunk table header
