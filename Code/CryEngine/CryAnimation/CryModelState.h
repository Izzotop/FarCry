...
public:
    ...
    void SetCharacterPhysParams(float mass, int surface_idx, float scale) {
        m_fMass = mass;
    ...
    unsigned getInstanceNumber() const {
        return m_nInstanceNumber;
    }
    // returns the root bone
    inline CryBone* getRootBone() {
        return m_arrBones.empty() ? nullptr : &m_arrBones[0];
    }
    inline const CryBone* getRootBone() const {
        return m_arrBones.empty() ? nullptr : &m_arrBones[0];
    }
    ...
    int getBoneParentIndex(const CryBone* pBone) {
        return getBoneParentIndex(pBone - getBones());
    }
    ...
    unsigned numBoneChildren(int nBoneIndex) {
        return getBoneInfo(nBoneIndex)->numChildren();
    }
    ...
    CryBoneInfo* getBoneInfo(int nBone) {
        return &GetModel()->getBoneInfo(nBone);
    }
    const CryBoneInfo* getBoneInfo(int nBone) const {
        return &GetModel()->getBoneInfo(nBone);
    }
    CryBoneInfo* getBoneInfo(const CryBone* pBone) {
        return getBoneInfo(pBone - &getBone(0));
    }
    const CryBoneInfo* getBoneInfo(const CryBone* pBone) const {
        return getBoneInfo(pBone - getBones());
    }
    ...
    const Matrix44* getBoneGlobalMatrices() const {
        return &m_arrBoneGlobalMatrices[0];
    }
    const Matrix44& getBoneMatrixGlobal(int nBone) const {
        return m_arrBoneGlobalMatrices[nBone];
    }
    Matrix44& getBoneMatrixGlobal(int nBone) {
        return m_arrBoneGlobalMatrices[nBone];
    }
    Matrix44& getBoneMatrixGlobal(const CryBone* pBone) {
        return getBoneMatrixGlobal(pBone - getBones());
    }
...
private:
    ...
    // returns nullptr if can't deform
    ...
    struct AnimationRecord : public CryCharAnimationParams {
        AnimationRecord() {}
        AnimationRecord(int _nAnimId, const CryCharAnimationParams& Params, float _fSpeed) : nAnimId(_nAnimId), fSpeed(_fSpeed), CryCharAnimationParams(Params) {}
    ...
    // effector layer. On some layers, the effectors may not be present (nullptr)
    ...
    // pDstNormlas can be non-nullptr, in which case the normals are (fakely) modified
    ...
    CryCharParticleManager* getParticleManager() {
...
protected:
    ...
    // this is nullptr for objects with even parity of scale matrix (that is, non-flipped)
