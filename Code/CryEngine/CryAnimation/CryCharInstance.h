...
public:
    ...
    struct StatObjBind {
        ...
        unsigned nBone;
        unsigned nFlags;
        StatObjBind(IBindable* obj, unsigned bone, unsigned flags) : pObj(obj), nBone(bone), nFlags(flags) {
            assert(pObj);
        }
    };
...
private:
    ...
    const Vec3d& GetSpitFirePos() {
        return m_v3pvSpitFirePosTranslated;
    }
    ...
    virtual void SetAnimationSpeed(float speed) {
        m_fAnimSpeedScale = speed;
    }
    virtual float GetAnimationSpeed() {
        return m_fAnimSpeedScale;
    }
