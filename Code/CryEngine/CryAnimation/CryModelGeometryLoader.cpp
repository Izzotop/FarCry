...
CryModelGeometryLoader::CryModelGeometryLoader()
    :
      ...
      m_arrGeomBoneNameTable("CryModelGeometryLoader.m_arrGeomBoneNameTable"), m_mapLights("CryModelGeometryLoader.mapLights"),
      m_mapObjectNodes("CryModelGeometryLoader.mapObjectNodes"), m_arrTempBoneIdToIndex("TempBoneIdToIndex") m_mapLimbPhysGeoms("CryModelGeometryLoader.mapLimbPhysGeoms"),
#endif
      m_numBoneLightBinds(0), m_pBoneLightBind(nullptr), m_pModel(nullptr), m_nLOD(-1), m_numMorphTargets(0) {
}
...
bool CryModelGeometryLoader::loadBoneNameList(const CHUNK_HEADER& chunkHeader, const void* pChunk, unsigned nChunkSize) {
    ...
    if (m_nLOD > 0) {
        if (m_arrGeomBoneNameTable.size() != m_pModel->numBoneInfos()) {
            g_GetLog()->LogError("lod %d file %s has inconsistent number of bones (%d, expected %d). Please re-export the lod", m_nLOD, getFilePathCStr(),
                                 m_arrGeomBoneNameTable.size(), m_pModel->numBoneInfos());
...
bool CryModelGeometryLoader::finalize() {
    ...
    if (m_pModel->m_arrMaterials.empty()) { // make some default material
    ...
    for (unsigned i = 0; i < m_numBoneLightBinds; ++i) {
        ...
        if (binding.nBoneId >= m_arrTempBoneIdToIndex.size()) {
            g_GetLog()->LogError("\002CryModel::LoadGeomChunks: invalid light-bone binding in the file for bone id %d, light 0x%08X. Bone id is out of range.", binding.nBoneId,
                                 binding.nLightChunkId);
            continue;
        }
        ...
        if (!pNodeChunk) {
            g_GetLog()->LogWarning("\002there is bone light in the file, but there is no Node for it. Unknown light name. Assuming a heat source. Bone id %d, light 0x%08X",
                                   binding.nBoneId, binding.nLightChunkId);
        }
...
bool CryModelGeometryLoader::loadMesh(const CHUNK_HEADER& chunkHeader, const void* pChunk, unsigned nChunkSize) {
    ...
    if (pMeshChunk->chdr.ChunkVersion == MESH_CHUNK_DESC_0744::VERSION) {
        ...
        g_GetLog()->LogError("\003CryModel::LoadGeomChunks: old file format, mesh chunk version 0x%08X (expected 0x%08X)", pMeshChunk->chdr.ChunkVersion,
                             MESH_CHUNK_DESC_0744::VERSION);
        return false;
    }
...
bool CryModelGeometryLoader::loadBoneMesh(const CHUNK_HEADER& chunkHeader, const void* pChunk, unsigned nChunkSize) { // load bones geometry

    const MESH_CHUNK_DESC_0744* pMeshChunk = (const MESH_CHUNK_DESC_0744*)pChunk;
    if (pMeshChunk->chdr.ChunkVersion != MESH_CHUNK_DESC_0744::VERSION) {
        g_GetLog()->LogError("\002CryModel::LoadGeomChunks: old file format, mesh chunk version 0x%08X (expected 0x%08X)", pMeshChunk->chdr.ChunkVersion,
                             MESH_CHUNK_DESC_0744::VERSION);
        return false;
    }
    ...
    if (nReadData != 0) // to account for a lot of broken mesh chunks; <<TODO>> change this line to "else" when all cgf are not broken
    {
        ...
        for (j = 0; j < GeomInfo.numFaces(); ++j) {
            ...
            if (nMtlID >= 0 && (unsigned)nMtlID + m_nFirstMat < m_pModel->numMaterials())
        ...
        if (m_pPhysicalGeometryManager) {
            ...
            IGeometry* pPhysicalGeometry = m_pPhysicalGeometryManager->CreateMesh(
                ...
                &points[0], &indices[0], (short*)&pmats[0], (unsigned)GeomInfo.numFaces(),
                (GeomInfo.numFaces() <= 20 ? mesh_SingleBB : mesh_OBB | mesh_AABB) | mesh_multicontact0 | mesh_uchar_ids | mesh_approx_box | mesh_approx_sphere |
                    mesh_approx_cylinder);
            assert(pPhysicalGeometry);
            m_mapLimbPhysGeoms[chunkHeader.ChunkID] = m_pPhysicalGeometryManager->RegisterGeometry(pPhysicalGeometry, pmats.empty() ? 0 : pmats[0]);
        }
    }
