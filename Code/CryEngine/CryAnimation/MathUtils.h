...
public:
    ...
    Vec3dA16() {}
    Vec3dA16(float x, float y, float z) : v(x, y, z) {}
};
...
inline Matrix44 OrthoUniformGetInverted(const Matrix44& b) {
    ...
    m(2, 2) = b(2, 2);
    m(2, 3) = 0;
    m.SetTranslationOLD(b.TransformVectorOLD(b.GetTranslationOLD()));
    m(3, 3) = 1;
...
template <typename T> T max3(T val1, T val2, T val3) {
    return max2(max2(val1, val2), val3);
}

template <typename T> T min3(T val1, T val2, T val3) {
...
template <typename TVec> class CryBBox_tpl {
public:
    CryBBox_tpl() {}
    ...
    template <typename T2> CryBBox_tpl(const CryBBox_tpl<T2>& right) {
    ...
    static Vec3& toVec3d(Vec3& v) {
        return v;
    }
    static const Vec3& toVec3d(const Vec3& v) {
        return v;
    }
    static Vec3& toVec3d(Vec3dA16& v) {
        return v.v;
    }
    static const Vec3& toVec3d(const Vec3dA16& v) {
        return v.v;
    }

    Vec3 getSize() const {
        return toVec3d(vMax) - toVec3d(vMin);
    }
    Vec3 getCenter() const {
        return (toVec3d(vMin) + toVec3d(vMax)) * 0.5f;
    }
    CryBBox_tpl<TVec>& operator=(const Vec3& v) {
        toVec3d(vMin) = toVec3d(vMax) = v;
        return *this;
    }
    void include(const Vec3& v) {
        AddToBounds(v, toVec3d(vMin), toVec3d(vMax));
    }
    template <typename T3> void include(const CryBBox_tpl<T3>& right) {
