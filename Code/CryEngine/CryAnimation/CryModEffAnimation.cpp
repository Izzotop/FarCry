...
CCryModEffAnimation::CCryModEffAnimation(CryModelState* pParent)
    : //  m_bMatrixPlusInUse(false),
      m_fAnimTime(0.0f), m_nAnimId(-1), m_fBlendInTime(0.0f), m_fBlendOutTime(0.0f), m_fOrigBlendOutTime(0.0f), m_pParent(pParent), m_fStopTime(0) {
    m_uFlags.bLoop = 0;
...
void CCryModEffAnimation::OnTimeChanged(int nAnimId, float fPrevTime, float fAnimTime, float fSpeed) {
    ...
    if (pSink && !arrEvents.empty() && fPrevTime != fAnimTime) {
        ...
        if (m_uFlags.bLoop && rAnim.fStart < rAnim.fStop && (fSpeed >= 0 ? fAnimTime > rAnim.fStop : fAnimTime < rAnim.fStart)) {
            bSecondPass = true;
            if (fSpeed >= 0) {
        ...
        for (it = arrEvents.begin(); it != arrEvents.end(); ++it) {
            ...
            if ((fPrevTime <= fTriggerTime && fTriggerTime < fAnimTime) || (fPrevTime >= fTriggerTime && fTriggerTime > fAnimTime)) {
                DEFINE_PROFILER_SECTION("AllCallbacks");
                pSink->OnAnimationEvent(rAnim.strName.c_str(), it->UserData);
                if (g_GetCVars()->ca_Debug())
                    g_GetLog()->Log("\004 %p->OnAnimationEvent (\"%s\",frame:%d,time:%g, trigger time: %g, time change: %g to %g)", m_pParent, rAnim.strName.c_str(),
                                    g_GetIRenderer()->GetFrameID(), fAnimTime, fTriggerTime, fPrevTime, fAnimTime);
            }
        }
        ...
        if (bSecondPass) {
            ...
            for (it = arrEvents.begin(); it != arrEvents.end(); ++it) {
                ...
                if ((fPrevTime <= fTriggerTime && fTriggerTime < fAnimTime) || (fPrevTime >= fTriggerTime && fTriggerTime > fAnimTime)) {
                    ...
                    pSink->OnAnimationEvent(rAnim.strName.c_str(), it->UserData);
                    if (g_GetCVars()->ca_Debug())
                        g_GetLog()->Log("\004 %p->OnAnimationEvent(2nd) (\"%s\",frame:%d,time:%g, trigger time: %g, time change: %g to %g)", m_pParent, rAnim.strName.c_str(),
                                        g_GetIRenderer()->GetFrameID(), fAnimTime, fTriggerTime, fPrevTime, fAnimTime);
                }
            }
        }
...
unsigned CCryModEffAnimation::Tick(float fDeltaTime, const std::vector<ICharInstanceSink*>& arrSinks, CAnimationLayerInfoArray& arrOutLayers) {
...
float CCryModEffAnimation::getBlending() const {
    ...
    else if (m_fBlendOutTime > 0 && m_fAnimTime > m_fStopTime)
...
void CCryModEffAnimation::StartAnimation(unsigned nAnimID, float fBlendInTime, float fBlendOutTime, CCryModEffAnimation* pSynchronizeWith, float fSpeed, unsigned nStartAnimFlags) {
    ...
    if (m_nAnimId >= 0) {
        if (g_GetCVars()->ca_Debug()) {
            ...
            g_GetLog()->LogToFile("\005Overriding existing animation #%u \"%s\" [%f..%f] at time %f", m_nAnimId, animOld.strName.c_str(), animOld.fStart, animOld.fStop,
                                  m_fAnimTime);
        }
