...
class IBSpline3Packed : public _reference_target<unsigned short> // we don't need the alignment of 4 bytes here
{
public:
...
////////////////////////////////////////////////////////////////////////
template <bool isOpen, typename FixedPointType> struct TBSplineVec3dPackedBase {
public:
    // fixed-point values.
...
protected:
    TBSplineVec3dPackedBase() : m_nDegree(1), m_numKnots(0) {}
    ...
    struct Scale {
        ...
        Scale() : fBase(0), fScale(1.0f / (float)nMaxFixedValue) {}
...
struct TBSplineVec3dPackedDesc : public IBSpline3Packed, public TBSplineVec3dPackedBase<isOpen, FixedPointType> {
...
template <bool isOpen = true, typename FixedPointType = unsigned char> class TBSplineVec3dPacked : public TBSplineVec3dPackedDesc<isOpen, FixedPointType> {
public:
...
template <bool isOpen, typename FixedPointType> TBSplineVec3dPacked<isOpen, FixedPointType>::TBSplineVec3dPacked() : m_pData(nullptr) {}

template <bool isOpen, typename FixedPointType> TBSplineVec3dPacked<isOpen, FixedPointType>::~TBSplineVec3dPacked() {
    clear();
}

// copies the packed version of the BSpline
template <bool isOpen, typename FixedPointType>
TBSplineVec3dPacked<isOpen, FixedPointType>::TBSplineVec3dPacked(const TBSplineVec3dPacked<isOpen, FixedPointType>& that) : Desc(that) {
...
// unpacks this object out of the given storage;
// returns the size of storage used for unpacking; 0 means unpack failed
template <bool isOpen, typename FixedPointType> int TBSplineVec3dPacked<isOpen, FixedPointType>::unpack(void* pBuffer, int nSize) {
    clear();
...
//////////////////////////////////////////////////////////////////////////
// clears the contents of the spline
template <bool isOpen, typename FixedPointType> void TBSplineVec3dPacked<isOpen, FixedPointType>::clear() {
    if (m_pData) {
        delete[] m_pData;
        m_pData = nullptr;
    }
...
//////////////////////////////////////////////////////////////////////////
// constructs a packed version of the given unpacked bspline
template <bool isOpen, typename FixedPointType> void TBSplineVec3dPacked<isOpen, FixedPointType>::copy(BSplineVec3d* pSpline) {
    int i, nCoord;
    ...
    for (i = 1; i < pSpline->numCPs(); ++i) {
        ...
        for (nCoord = 0; nCoord < 3; ++nCoord) {
            ...
            else if (vMaxCP[nCoord] < vCP[nCoord])
...
//////////////////////////////////////////////////////////////////////////
// packs the spline into the given buffer
template <bool isOpen, typename FixedPointType> void TBSplineVec3dPacked<isOpen, FixedPointType>::pack(void* pBuffer) {
    memcpy(pBuffer, static_cast<const typename TBSplineVec3dPackedDesc<isOpen, FixedPointType>::Base*>(this),
           sizeof(typename TBSplineVec3dPackedDesc<isOpen, FixedPointType>::Base));
    memcpy(static_cast<typename TBSplineVec3dPackedDesc<isOpen, FixedPointType>::Base*>(pBuffer) + 1, m_pData, this->getRawDataSize());
}

//////////////////////////////////////////////////////////////////////////
// returns the CP from the array of value triplets
template <bool isOpen, typename FixedPointType> Vec3 TBSplineVec3dPacked<isOpen, FixedPointType>::getCP(int nCP) {

    if (isOpen) {
        if (nCP < 0)
            nCP = 0;
        else if (nCP >= this->numCPs())
            nCP = this->numCPs() - 1;
    } else {
        nCP = nCP % this->numCPs();
        if (nCP < 0)
...
//////////////////////////////////////////////////////////////////////////
// returns the time
template <bool isOpen, typename FixedPointType> float TBSplineVec3dPacked<isOpen, FixedPointType>::getKnotTime(int nKnot) const {
    if (!isOpen) {
...
//////////////////////////////////////////////////////////////////////////
// returns the max corner of the bounding box for CPs
template <bool isOpen, typename FixedPointType> Vec3 TBSplineVec3dPackedDesc<isOpen, FixedPointType>::getCPMax() const {
    return Vec3(this->m_PosScale[0].getMax(), this->m_PosScale[1].getMax(), this->m_PosScale[2].getMax());
}

//////////////////////////////////////////////////////////////////////////
// returns the min corner of the bounding box for CPs
template <bool isOpen, typename FixedPointType> Vec3 TBSplineVec3dPackedDesc<isOpen, FixedPointType>::getCPMin() const {
    return Vec3(this->m_PosScale[0].getMin(), this->m_PosScale[1].getMin(), this->m_PosScale[2].getMin());
}

//////////////////////////////////////////////////////////////////////////
// returns the min time
template <bool isOpen, typename FixedPointType> float TBSplineVec3dPackedDesc<isOpen, FixedPointType>::getTimeMin() const {
    return this->m_TimeScale.getMin();
}

//////////////////////////////////////////////////////////////////////////
// returns the max time
template <bool isOpen, typename FixedPointType> float TBSplineVec3dPackedDesc<isOpen, FixedPointType>::getTimeMax() const {
    return this->m_TimeScale.getMax();
}

//////////////////////////////////////////////////////////////////////////
// returns the max-min time
template <bool isOpen, typename FixedPointType> float TBSplineVec3dPackedDesc<isOpen, FixedPointType>::getTimeRange() const {
    return this->m_TimeScale.getRange();
}

//////////////////////////////////////////////////////////////////////////
// calculates the value of the spline at the specified point
template <bool isOpen, typename FixedPointType> Vec3 TBSplineVec3dPacked<isOpen, FixedPointType>::getValue(float fTime) {
...
// each pair of knots defines interval [k1,k2)
// -1 is before the 0-th knot, numKnots()-1 is after the last knot
template <bool isOpen, typename FixedPointType> int TBSplineVec3dPacked<isOpen, FixedPointType>::findInterval(float fTime) const {
...
// returns the i-th basis function of degree d, given the time t
template <bool isOpen, typename FixedPointType> float TBSplineVec3dPacked<isOpen, FixedPointType>::getBasis(int i, int d, float t) const {
...
// returns the i-th basis function of degree d, given the time t and a hint, in which interval to search for it
template <bool isOpen, typename FixedPointType> float TBSplineVec3dPacked<isOpen, FixedPointType>::getBasis(int i, int d, float t, int nIntervalT) const {
...
// returns the value of the basis function of degree d, given the time t
// the knots of the basis function start at pKnotBegin, and the first knot before t is pKnotBeforeT
template <bool isOpen, typename FixedPointType> float TBSplineVec3dPacked<isOpen, FixedPointType>::getBasisUnsafe(int nKnotBegin, int d, float t, int nKnotBeforeT) const {
...
// apply the given scale to the spline (multiply the spline by scale
template <bool isOpen, typename FixedPointType> void TBSplineVec3dPackedDesc<isOpen, FixedPointType>::scale(float fScale) {
