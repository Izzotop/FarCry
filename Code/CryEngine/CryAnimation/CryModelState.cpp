...
CryModelState::CryModelState(CryModel* pMesh)
    :
      ...
      m_arrBones("CryModelState.Bones"), m_arrBoneGlobalMatrices("CryModelState.arrBoneGlobalMatrices"), m_arrAnimationLayers("CryModelState.AnimationLayers"),
      m_arrMorphEffectors("CryModelState.MorphTargets"), m_arrHeatSources("CryModelState.HeatSources"), m_arrSinks("CryModelState.Sinks"),
#endif
      m_vRuntimeScale(1, 1, 1), m_nLastSkinBBoxUpdateFrameId(0), m_pShaderStateCull(nullptr), m_pShaderStateShadowCull(nullptr)
#ifdef _DEBUG
      ,
...
void CryModelState::InitBBox() {
    ...
    if (m_BBox.empty())
        g_GetLog()->LogError("\003CryModel::LoadPostInitialize:Character %s bbox initialized to empty {%g,%g,%g}{%g,%g,%g}", m_pMesh->getFilePathCStr(), m_BBox.vMin.x,
                             m_BBox.vMin.y, m_BBox.vMin.z, m_BBox.vMax.x, m_BBox.vMax.y, m_BBox.vMax.z);
}
...
void CryModelState::ValidateBBox() {
    if (m_BBox.vMax.x < m_BBox.vMin.x ||
        ...
        g_GetLog()->LogWarning(
            "\001[Animation] invalid BBox{{%.3f,%.3f,%.3f}-{%.3f,%.3f,%.3f}}. Model state 0x%X, model 0x%X, \"%s\" instance %d, frame %d",
            m_BBox.vMin.x, m_BBox.vMin.y, m_BBox.vMin.z,
            m_BBox.vMax.x, m_BBox.vMax.y, m_BBox.vMax.z,
...
void CryModelState::UpdateAnimatedEffectors(float fDeltaTimeSec, ActiveLayerArray& arrActiveLayers) {
    ...
    for (SubmeshArray::iterator it = m_arrSubmeshes.begin(); it != m_arrSubmeshes.end(); ++it)
        if (*it)
            (*it)->UpdateMorphEffectors(fDeltaTimeSec);
            ...
            if (layer.queDelayed.empty()) {
                if (layer.nDefaultIdleAnimID >= 0 && layer.bEnableDefaultIdleAnimRestart) {
                    ...
                    if (g_GetCVars()->ca_Debug())
                        g_GetLog()->Log("\003Restarting default idle animation %s in layer %d, blend in=%.2f, out=%.2f",
                                        m_pMesh->getAnimation(layer.nDefaultIdleAnimID).strName.c_str(), nLayer, layer.fDefaultAnimBlendTime, layer.fDefaultAnimBlendTime);
                ...
                if (g_GetCVars()->ca_Debug())
                    g_GetLog()->Log("\003Autostarting queued animation %s in layer %d, blend in=%.2f, out=%.2f", m_pMesh->getAnimation(record.nAnimId).strName.c_str(), nLayer,
                                    record.fBlendInTime, record.fBlendOutTime);
...
void CryModelState::UpdateBones(const ActiveLayerArray& arrActiveLayers) {
    ...
    */
    ...
    for (CryCharFxTrailArray::iterator it = m_arrFxTrails.begin(); it != m_arrFxTrails.end(); ++it) {
        if (*it) {
            (*it)->Deform(getBoneGlobalMatrices());
        }
    }
}
...
void CryModelState::UpdateBoneMatricesGlobal() {
    ...
    *pmatGlobal = pBone->m_matRelativeToParent;
    ...
    for (++pBone, ++pBoneInfo, ++pmatGlobal; pBone != pBoneEnd; ++pBone, ++pBoneInfo, ++pmatGlobal) {
...
void CryModelState::Render(const struct SRendParams& RendParams, Matrix44& mtxObjMatrix, CryCharInstanceRenderParams& rCharParams, const Vec3& translation) {
    ...
    for (SubmeshArray::iterator it = m_arrSubmeshes.begin(); it != m_arrSubmeshes.end(); ++it)
        if (*it)
            (*it)->Render(RendParams, mtxObjMatrix, rCharParams, translation);

    for (CryCharFxTrailArray::iterator it = m_arrFxTrails.begin(); it != m_arrFxTrails.end(); ++it)
        if (*it)
            (*it)->Render(RendParams, mtxObjMatrix, rCharParams);
...
bool CryModelState::RunAnimation(int nAnimID, const CryCharAnimationParams& Params, float fSpeed, bool bInternal) {
    ...
    assert(rLayer.pEffector != (CCryModEffAnimation*)nullptr);
    ...
    if (!bInternal && rLayer.pEffector->isActive() && 0 != ((Params.nFlags | rLayer.pEffector->getStartAnimFlags()) & CryCharAnimationParams::FLAGS_ALIGNED)) {
        ...
        rLayer.pEffector->StartAnimation(nAnimID, Params.fBlendInTime, Params.fBlendOutTime,
                                         (Params.nFlags & Params.FLAGS_SYNCHRONIZE_WITH_LAYER_0) != 0 ? m_arrAnimationLayers[0].pEffector : rLayer.pEffector,
                                         fSpeed * g_GetCVars()->ca_UpdateSpeed(Params.nLayerID) * getAnimationSpeed(Params.nLayerID), Params.nFlags);
...
void CryModelState::ResetAllAnimations() {
    ...
    for (unsigned nLayer = 0; nLayer < m_arrAnimationLayers.size(); ++nLayer) { // TODO: Allow more detail layers
        AnimationLayer& layer = m_arrAnimationLayers[nLayer];
        if (layer.pEffector)
...
bool CryModelState::SetAnimationFrame(const char* szString, int nFrame) {
    ...
    // so we pass 0
    CryCharAnimationParams nullptrAnim(0, 0);
    nullptrAnim.nFlags = nullptrAnim.FLAGS_NO_DEFAULT_IDLE;
    bool bRes = RunAnimation(szString, nullptrAnim, 0);
...
CryModelState::AnimationLayer::AnimationLayer() : nDefaultIdleAnimID(-1), fDefaultAnimBlendTime(0), bEnableDefaultIdleAnimRestart(false), bKeepLayer0Phase(false) {}
...
void CryModelState::addAnimEvent(int nAnimId, int nFrame, AnimSinkEventData UserData) {
    if (g_GetCVars()->ca_Debug())
        g_GetLog()->LogToFile("\003AnimDebug: 0x%08X->addAnimEvent(anim:%d, frame:%d, userdata:%p,%d) model %s", this, nAnimId, nFrame, UserData.p, UserData.n,
                              m_pMesh->getFilePathCStr());
...
void CryModelState::DumpState() {
    ...
    */
    ...
    for (unsigned i = 0; i < m_arrBones.size(); ++i) {
        g_GetLog()->LogToFile("\003  \"%s\" relative %s, global %s pq %s%s%s", m_pMesh->getBoneInfo(i).getNameCStr(), toString(getBone(i).m_matRelativeToParent).c_str(),
                              toString(getBoneMatrixGlobal(i)).c_str(), getBone(i).m_pqTransform.toString().c_str(), getBone(i).m_bUseMatPlus ? " (Mat+)" : "",
...
void CryModelState::PreloadResources(float fDistance, float fTime, int nFlags) {
    for (SubmeshArray::iterator it = m_arrSubmeshes.begin(); it != m_arrSubmeshes.end(); ++it)
        if (*it)
            (*it)->PreloadResources(fDistance, fTime, nFlags);
}
...
CryModelSubmesh* CryModelState::AddSubmesh(CryModel* pCryModel, bool bVisible) {
    ...
    if (!IsEmpty() && pCryModel->numBoneInfos() != GetCryModelSubmesh(0)->GetCryModel()->numBoneInfos()) {
        g_GetLog()->LogError("Trying to add submesh %s to an existing instance with main model %s: incompatible skeletons", pCryModel->getFilePathCStr(),
                             GetCryModelSubmesh(0)->GetCryModel()->getFilePathCStr());
        return nullptr;
    }
...
void CryModelState::SetShaderFloat(const char* Name, float fVal, const char* ShaderName) {
    for (SubmeshArray::iterator it = m_arrSubmeshes.begin(); it != m_arrSubmeshes.end(); ++it)
        if (*it)
            (*it)->SetShaderFloat(Name, fVal, ShaderName);
}
...
void CryModelState::RenderShadowVolumes(const SRendParams* rParams, int nLimitLOD) {
    ...
    for (SubmeshArray::iterator it = m_arrSubmeshes.begin(); it != m_arrSubmeshes.end(); ++it)
        if (*it)
            (*it)->RenderShadowVolumes(rParams, nLimitLOD);
}
