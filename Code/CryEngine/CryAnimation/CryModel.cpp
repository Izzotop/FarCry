...
CryModel::CryModel(CryCharBody* pBody, CControllerManager* pControllerManager)
    : CryModelAnimationContainer(pControllerManager), m_pBody(pBody), m_arrDamageTable("CryModel.DamageTable"), m_arrGeomInfo("CryModel.GeomInfo"), m_vModelOffset(0, 0, 0),
      m_bDeleteLBMats(true), m_bFromCCG(false), m_nDefaultGameID(GetDefaultPhysMaterial())
...
void CryModel::InitDamageTable() {
    ...
    for (unsigned idx = 0; idx < numBoneInfos(); ++idx) {
        ...
        else if (findString(szBoneName, g_szDamageBonesTorso) >= 0)
            m_arrDamageTable[idx] = g_nDamageAreaTorso;
        else if (findString(szBoneName, g_szDamageBonesArmL) >= 0)
            m_arrDamageTable[idx] = g_nDamageAreaArmL;
        else if (findString(szBoneName, g_szDamageBonesArmR) >= 0)
            m_arrDamageTable[idx] = g_nDamageAreaArmR;
        else if (findString(szBoneName, g_szDamageBonesLegL) >= 0)
            m_arrDamageTable[idx] = g_nDamageAreaLegL;
        else if (findString(szBoneName, g_szDamageBonesLegR) >= 0)
...
bool CryModel::initFromCCG(const string& strTextureDir, const string& strAnimationDir, class CCFMemReader& Reader, float fScale) {
    ...
    if (numStateBones != numBoneInfos()) {
        g_GetLog()->LogError("FATAL: could not create bones for the default model state: %d instead of %d were created. %d reported.", numStateBones, numBoneInfos(),
                             m_pDefaultModelState->numBones());
        return false;
    }
...
bool CryModel::loadCCGBoneGeom(IGeomManager* pGeomManager, unsigned nLOD, float fScale, const CCFBGBone* pHeader, unsigned nSize) {
    ...
    IGeometry* pPhysicalGeometry = pGeomManager->CreateMesh(&arrVertices[0], &arrIndices[0], &arrMtls[0], pHeader->numFaces,
                                                            (pHeader->numFaces <= 20 ? mesh_SingleBB : mesh_OBB | mesh_AABB) | mesh_multicontact0 | mesh_approx_box |
                                                                mesh_approx_sphere | mesh_approx_cylinder);
...
// as the scene user properties. if there's no such data, returns nullptr
...
void CryModel::ExportModelsASC() {
    ...
    for (unsigned n = 0; n < numLODs(); ++n) {
        ...
        for (unsigned nMat = 0; nMat < pMats->size(); ++nMat) {
            ...
            fprintf(f, "%s: indices[0x%04x..0x%04x], vertices[0x%04x..0x%04x]\n", m.sMaterialName, m.nFirstIndexId, m.nFirstIndexId + m.nNumIndices, m.nFirstVertId,
                    m.nFirstVertId + m.nNumVerts);
        }
    }
    fprintf(f, "\n");
    fclose(f);
