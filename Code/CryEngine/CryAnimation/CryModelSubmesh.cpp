...
#endif
      m_pParent(pParent), m_pDecalManager(nullptr), m_nLastSkinBBoxUpdateFrameId(0), m_nLastTangentsUpdatedFrameId(0), m_nLastTangentsUpdatedLOD(-1), m_pMesh(pMesh),
...
// returns the model of the submesh, or nullptr in case of failure
...
void CryModelSubmesh::GenerateRenderArrays(const char* szFileName) {
    ...
    for (unsigned nLod = 0; nLod < m_pMesh->numLODs(); nLod++) {
        ...
        {
            CLeafBuffer* pVertBuffer = m_pLeafBuffers[nLod] = g_GetIRenderer()->CreateLeafBuffer(eBT_Dynamic, "CryModelArray");
                ...
                for (nMaterial = 0; nMaterial < numUsedMtls; ++nMaterial) {
                    ...
                    *(unsigned int*)&rMaterial = *(unsigned int*)&defaultMaterial;
                    ...
                    if (arrMtlUsage[nMaterial]) {
                        ...
                        CIndexedMesh__LoadMaterial(nullptr, CryStringUtils::GetParentDirectory(string(szFileName)).c_str(), rMaterial, g_GetIRenderer(),
                                                   &m_pMesh->getMaterial(nMaterial));
...

        {
            ...
            for (unsigned i = 0; i < pGeomInfo->numFaces(); i++) {
                if (pInds[0] >= m_pLeafBuffers[nLod]->m_SecVertCount || pInds[1] >= m_pLeafBuffers[nLod]->m_SecVertCount || pInds[2] >= m_pLeafBuffers[nLod]->m_SecVertCount) {
    ...
    if (pGeomInfo->m_arrPrimGroups.empty()) {
        ...
        for (unsigned i = 0; i < pLeafBuffer->m_pMats->size(); i++) {
            ...
            if (si.m_pShader && i < (unsigned)pLeafBuffer->m_pMats->Count() && pREOcLeaf) {
                ...
                } else if (rParams.nShaderTemplate > 0) {
                    int nTempl = rParams.nShaderTemplate;
                    si.m_pShader->AddTemplate(si.m_pShaderResources, nTempl, nullptr);
                }
                if (nTempl > 0)
                    obj->m_nTemplId = nTempl;
                g_GetIRenderer()->EF_AddEf(rParams.nFogVolumeID, pREOcLeaf, si.m_pShader, si.m_pShaderResources, obj, nTempl,
                                           m_pParent->m_pShaderStateCull ? m_pParent->m_pShaderStateCull : rParams.pStateShader, nSort);
                if (nTempl1 > 0)
                    g_GetIRenderer()->EF_AddEf(rParams.nFogVolumeID, pREOcLeaf, si.m_pShader, si.m_pShaderResources, obj1, nTempl1,
                                               m_pParent->m_pShaderStateCull ? m_pParent->m_pShaderStateCull : rParams.pStateShader, nSort);
            }
        }
    } else {
        ...
        for (unsigned nPrimGroup = 0; nPrimGroup < pGeomInfo->m_arrPrimGroups.size(); nPrimGroup++) {
            ...
            if (si.m_pShader) {
                ...
                if (rParams.nShaderTemplate && si.m_pShader->GetTemplate(nTempl)->GetSort() == eS_HeatVision) {
                    g_GetIRenderer()->EF_AddEf(rParams.nFogVolumeID, pREOcLeaf, si.m_pShader, si.m_pShaderResources, obj, nTempl,
                                               m_pParent->m_pShaderStateCull ? m_pParent->m_pShaderStateCull : rParams.pStateShader);
                    continue;
                }
...
void CryModelSubmesh::Deform(int nLodToDeform, unsigned nDeformFlags) {
    ...
    if ((unsigned)nLodToDeform >= SIZEOF_ARRAY(m_pLeafBuffers)) {
        g_GetLog()->LogError("\001CryModelState::Deform(lod=%d,%s,%s,%s):invalid lod", nLodToDeform, (nDeformFlags & FLAG_DEFORM_UPDATE_TANGENTS) ? "Update Tangents" : "",
                             (nDeformFlags & FLAG_DEFORM_UPDATE_NORMALS) ? "Update Normals" : "", (nDeformFlags & FLAG_DEFORM_FORCE_UPDATE) ? "Force Update" : "");
        return;
    }
    ...
    if (!lb) {
        g_GetLog()->LogError("\001CryModelState::Deform(lod=%d,%s,%s,%s):no lod leaf buffer", nLodToDeform, (nDeformFlags & FLAG_DEFORM_UPDATE_TANGENTS) ? "Update Tangents" : "",
                             (nDeformFlags & FLAG_DEFORM_UPDATE_NORMALS) ? "Update Normals" : "", (nDeformFlags & FLAG_DEFORM_FORCE_UPDATE) ? "Force Update" : "");
        return;
    }
    ...
    if (!pRenderVertexBuffer) {
        g_GetLog()->LogError("\001CryModelState::Deform(lod=%d,%s,%s,%s):no vertex buffer in leaf buffer %x", nLodToDeform,
                             (nDeformFlags & FLAG_DEFORM_UPDATE_TANGENTS) ? "Update Tangents" : "", (nDeformFlags & FLAG_DEFORM_UPDATE_NORMALS) ? "Update Normals" : "",
                             (nDeformFlags & FLAG_DEFORM_FORCE_UPDATE) ? "Force Update" : "", lb);
        return;
    }
    ...
    // the tangents will be skinned here; if it remains nullptr, they won't be skinned at all
    SPipTangentsA* pTangentBases = nullptr;

    // these are the sizes of buffers needed in the temporary buffer, in bytes
    unsigned sizeVertices = pGeomInfo->numUsedVertices() * sizeof(Vec3dA16), sizeNormals = (nDeformFlags & FLAG_DEFORM_UPDATE_NORMALS) ? sizeVertices : 0, sizeTangents = 0;

    if ((nDeformFlags & FLAG_DEFORM_UPDATE_TANGENTS) && m_pMesh->numBoneInfos()) {
        if ((nDeformFlags & FLAG_DEFORM_FORCE_UPDATE) || // we calculate tangents if the videobuffer has changed and we're forced to recalculate all buffers
            !m_nLastTangentsUpdatedFrameId ||            // we calculate tangents if they haven't been calculated yet
            m_nLastTangentsUpdatedLOD != nLodToDeform || (nFrameId >= m_nLastTangentsUpdatedFrameId + g_GetCVars()->ca_TangentBasisCalcPeriod())) {
...
// The normals, if not nullptr, are modified according to the vertex movements
...
void CryModelSubmesh::RenderShadowVolumes(const SRendParams* rParams, int nLimitLOD) {
    ...
    Vec3d vLSourcePos(vLightTrans.x * fCos - vLightTrans.y * fSin, vLightTrans.x * fSin + vLightTrans.y * fCos, vLightTrans.z);
...
void CryModelSubmesh::ProcessSkinning(const Vec3& t, const Matrix44& mtxModel, int nTemplate, int nLod, bool bForceUpdate) {
    ...
    if (!bNeedVertices && !bForceUpdate && !bNeedTangents && (m_pParent->m_uFlags & (CryModelState::nFlagNeedReskinLOD << nLod)) == 0 && m_pLeafBuffers[nLod] &&
    ...
    //  g_pIRenderer->Draw2dLabel( 1,g_YLine, 1.3f, fColor, false,"trans: %15.10f %15.10f %15.10f  bForceUpdate:%d",t.x,t.y,t.z, bForceUpdate );  g_YLine+=16.0f;
            g_pIRenderer->Draw2dLabel( 1,g_YLine, 1.3f, fColor, false,"aabbsize: %15.10f %15.10f %15.10f",(aabb.max.x-aabb.min.x),(aabb.max.y-aabb.min.y),(aabb.max.z-aabb.min.z) );
    g_YLine+=16.0f;
            //g_pIRenderer->Draw2dLabel( 1,g_YLine, 1.3f, fColor, false,"trans: %15.10f %15.10f %15.10f  bForceUpdate:%d",trans.x,trans.y,trans.z,bForceUpdate );
    g_YLine+=16.0f;
...
void CryModelSubmesh::Render(const struct SRendParams& RendParams, Matrix44& mtxObjMatrix, CryCharInstanceRenderParams& rCharParams, const Vec3& translation) {
    ...
    */
    ...
    if ((RendParams.dwFObjFlags & FOB_NEAREST) || (rCharParams.m_nFlags & CS_FLAG_DRAW_NEAR)) {
    ...
    if (g_GetIRenderer()->EF_GetHeatVision())
    ...
    if (g_GetCVars()->ca_ambient_light_range() > 0.05f && !(RendParams.dwFObjFlags & FOB_FOGPASS) && !(RendParams.dwFObjFlags & FOB_LIGHTPASS)) {
        ...
        if (col >= 0.0f) {
            ...
            if (ambientColor.GetLengthSquared() > 0.001) {
                ...
                //        pObj->m_AmbColor -= Vec3d(rCharParams.m_ambientLight.m_Color.r*2.0f, rCharParams.m_ambientLight.m_Color.g*2.0f,
                // rCharParams.m_ambientLight.m_Color.b*2.0f);
...
// returns the morph target effector, or nullptr if no such effector found
