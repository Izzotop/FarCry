...
template <class T> struct LessString {
...
template <class Key, class Traits = LessString<Key>> class BoneNameHashCompare {
    Traits comp;
...
public:
    ...
    inline bool operator()(const Key& _Key1, const Key& _Key2) const {
...
public:
    ...
    CryBoneInfo& getRootBoneInfo() {
        return getBoneInfo(0);
    }
    ...
    float getTicksPerFrame() const {
        return m_fTicksPerFrame;
    }
    float getSecondsPerTick() const {
        return m_fSecsPerTick;
    }
    float getSecondsPerFrame() const {
        return m_fTicksPerFrame * m_fSecsPerTick;
    }
    float getTicksPerSecond() const {
        return float(1.0 / m_fSecsPerTick);
    }
    ...
    bool isDummyAnimation(const char* szAnimName) {
        return m_setDummyAnimations.find(szAnimName) != m_setDummyAnimations.end();
    }
...
protected:
    ...
    void RegisterDummyAnimation(const char* szAnimName) {
        m_setDummyAnimations.insert(szAnimName);
    }
...
protected:
    ...
    struct AnimationGlobIdPred {
        ...
        AnimationGlobIdPred(const std::vector<AnimData>& arrAnims) : m_arrAnims(arrAnims) {}
    ...
    struct AnimationNamePred {
        const std::vector<AnimData>& m_arrAnims;
        AnimationNamePred(const std::vector<AnimData>& arrAnims) : m_arrAnims(arrAnims) {}
