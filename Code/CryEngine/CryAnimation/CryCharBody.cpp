...
CryCharBody::CryCharBody(CryCharManager* pManager, const string& strFileName)
    : m_pCryModel(nullptr), m_fAnimationFrameRate(30), // the default animation rate
...
void CryCharBody::CleanupInstances() {
    ...
    if (!m_setInstances.empty())
        g_GetISystem()->Warning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, 0, "CryCharBody.CleanupInstances",
                                "Forcing deletion of %d instances for body %s. CRASH POSSIBLE because other subsystems may have stored dangling pointer(s).", NumRefs(),
                                m_strFilePath.c_str());
...
void CryCharBody::DumpModel() {
    g_GetLog()->LogToFile("\001%60s  %3d instance%s, %3d animations (%3d unique)", m_strFilePath.c_str(), m_setInstances.size(), m_setInstances.size() == 1 ? "" : "s",
                          m_pCryModel->numAnimations(), m_pCryModel->numUniqueAnimations());
    g_GetLog()->LogToConsole("\001%60s  %3d instance%s, %3d animations (%3d unique)", m_strFilePath.c_str(), m_setInstances.size(), m_setInstances.size() == 1 ? "" : "s",
                             m_pCryModel->numAnimations(), m_pCryModel->numUniqueAnimations());
}
