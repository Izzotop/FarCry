...
CCryModEffIKSolver::CCryModEffIKSolver(CryModelState* pParent) : m_pParent(pParent), m_nPrimaryBone(-1) {
    m_flags = 0;
    m_ptGoal.zero();
    m_GoalNormal.zero();
    m_bActive = true;
...
void CCryModEffIKSolver::UpdateBoneChildren(int iBone) {
    ...
    for (i = m_pParent->numBoneChildren(iBone) - 1; i >= 0; i--) {
        ...
        m_pParent->getBoneMatrixGlobal(iChild) = m_pParent->getBoneChild(iBone, i)->m_matRelativeToParent * m_pParent->getBoneMatrixGlobal(iBone);
...
void CCryModEffIKSolver::ApplyToBone(int nLayer) {
    ...
    int i, nbones;
    int arrBones[4] = {m_nPrimaryBone, m_pParent->getBoneChildIndex(m_nPrimaryBone, 0), m_pParent->getBoneGrandChildIndex(m_nPrimaryBone, 0, 0), -1};
    ...
    if (m_GoalNormal.len2() > 0) {
        ...
        if (nlen > 0.001) {
            n /= nlen;
            beta = cry_atan2f(nlen, c * m_GoalNormal);
            if (beta < 0.95f) {
                if (fabsf(beta) > 0.7f)
...
bool CCryModEffIKSolver::AffectsBone(int nBone) {
    return m_bActive && (nBone == m_nPrimaryBone || nBone == m_pParent->getBoneChildIndex(m_nPrimaryBone, 0) || nBone == m_pParent->getBoneGrandChildIndex(m_nPrimaryBone, 0, 0));
}
