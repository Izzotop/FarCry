...
public:
    ...
    GeomMtlID getFaceMtl(unsigned i) const {
        return m_arrFaceMtl[i];
    }

    size_t numFaces() const {
        return m_arrFaces.size();
    }
    GeomFace getFace(unsigned i) const {
        return m_arrFaces[i];
    }
    const GeomFace* getFaces() const {
        return &m_arrFaces[0];
    }
    ...
    void setNumUsedVertices(unsigned num) {
        m_numUsedVertices = num;
    }
...
public:
    ...
    */
    ...
    unsigned numNormals() const {
        return m_arrNormals.empty() ? 0 : m_numUsedVertices;
    }
    ...
    unsigned numExtUVs() const {
        return m_arrExtUVs.empty() ? 0 : numExtToIntMapEntries();
    }
    unsigned numExtVertices() const {
        return numExtToIntMapEntries();
    }
...
public:
    CrySkinVertexSource(CryGeometryInfo* pGeometry)
        : ICrySkinSource(pGeometry->getLinks(), pGeometry->numLinks(), pGeometry->getVertices(), pGeometry->numVertices(), pGeometry->getExtTangents(), pGeometry->numExtTangents(),
                         pGeometry->getExtToIntMapEntries()) {}

    ~CrySkinVertexSource() { /* no need to deallocate any stuff*/
    }
};
...
public:
    CrySkinNormalSource(CryGeometryInfo* pGeometry, const CryBoneInfo* pBoneInfo)
        : ICrySkinSource(pGeometry->newNormalLinks(pBoneInfo), pGeometry->numLinks(), pGeometry->getVertices(), pGeometry->numVertices(), pGeometry->getExtTangents(),
                         pGeometry->numExtTangents(), pGeometry->getExtToIntMapEntries()) {}
