...
class CryCharBody : public ICryCharModel {
public:
    CryCharBody(CryCharManager* pManager, const string& strFileName);
    ~CryCharBody();

    // Returns the pointer to the loaded model for this body.
    // also works as an indicator of load operation success: if the file was not successfully loaded, then returns nullptr
    CryModel* GetModel();
    ...
    const char* GetProperty(const char* szName) {
        return m_pCryModel->GetProperty(szName);
    }

    virtual ClassEnum GetClass() {
        return CLASS_CRYCHARBODY;
    }
