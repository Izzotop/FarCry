        ...
        for (i = 0; i < (int)numBones(); i++)
            if (getBoneInfo(i)->m_PhysInfo[nLod].pPhysGeom) {
                ...
                if ((ptr = strstr(pBoneInfo->getNameCStr(), "Forearm"))) {
                    for (j = 0; j < (int)numBones(); j++)
                        if (getBoneInfo(j)->m_PhysInfo[nLod].pPhysGeom &&
                            (strstr(getBoneInfo(j)->getNameCStr(), "Pelvis") || strstr(getBoneInfo(j)->getNameCStr(), "Head") || strstr(getBoneInfo(j)->getNameCStr(), "Spine") ||
                             (ptr1 = strstr(getBoneInfo(j)->getNameCStr(), "Thigh")) && ptr[-2] == ptr1[-2] || strstr(getBoneInfo(j)->getNameCStr(), "Forearm") && i > j))
...
IPhysicalEntity* CryModelState::CreateCharacterPhysics(IPhysicalEntity* pHost, float mass, int surface_idx, float stiffness_scale, float scale, Vec3d offset, int nLod) {
    if (m_pCharPhysics) {
        ...
        for (i = k = 0; i < (int)numBones(); i++)
            if (!strcmp(m_auxPhys[j].strName, getBoneInfo(i)->getNameCStr())) {
                ...
                if (m_auxPhys[j].pauxBoneInfo[k].dir0.len2() > 0)
                    m_auxPhys[j].pauxBoneInfo[k].dir0 *= (m_auxPhys[j].pauxBoneInfo[k].rlen0 = 1.0f / m_auxPhys[j].pauxBoneInfo[k].dir0.len());
                else {
...
void CryModelState::ConvertBoneGlobalToRelativeMatrices() {
    ...
    for (i = numBones - 1; i > 0; --i) {
        ...
        matrix3x3in4x4Tf &mtxrel((matrix3x3in4x4Tf&)Mtxrel), &mtxparent((matrix3x3in4x4Tf&)mtxParentGlobal), &mtxchild((matrix3x3in4x4Tf&)mtxBoneGlobal);
...
void CryModelState::SynchronizeWithPhysicalEntity(IPhysicalEntity* pent, const Vec3& posMaster, const Quat& qMaster, Vec3 offset, int iDir) {
    if (pent && (iDir == 0 || iDir < 0 && pent->GetType() != PE_ARTICULATED && pent->GetType() != PE_RIGID)) { // copy state _to_ physical entity
        // m_vOffset = offset;
        // m_fScale = scale;
    ...
    } else { // copy state _from_ physical entity
        if (!pent && !m_nAuxPhys)
            return;
        ...
        if (pent) {
            ...
            for (; i < (int)numBones(); i++)
                if (getBoneInfo(i)->m_PhysInfo[nLod].pPhysGeom) {
                    ...
                    (matrix3x3in4x4Tf&)getBone(i).m_matRelativeToParent = Matrix33(getBoneInfo(i)->getqRelPhysParent(nLod) * sj.quat0 * GetRotationXYZ<float>(sj.q + sj.qext));
        ...
        for (j = 0; j < m_nAuxPhys; j++) {
            ...
            for (i = 0; i < m_auxPhys[j].nBones; i++) {
                vectorf &pt0 = *(vectorf*)((char*)pr.pPoints + i * pr.iStride), &pt1 = *(vectorf*)((char*)pr.pPoints + (i + 1) * pr.iStride), dir = (pt1 - pt0) * sp.q;
                float len = dir.len();
                if (len > 1E-4f) {
                    ...
                    *(matrix3x3in4x4Tf*)&matBoneGlobal =
                        (matrix3x3f(GetRotationV0V1(m_auxPhys[j].pauxBoneInfo[i].dir0, dir / len) * m_auxPhys[j].pauxBoneInfo[i].quat0) * len * m_auxPhys[j].pauxBoneInfo[i].rlen0);
...
IPhysicalEntity* CryModelState::RelinquishCharacterPhysics() {
    if (m_bHasPhysics) {
        ...
        for (i = 0; i < (int)numBones(); i++) { // '-' since pqTransform.buildMatrix assumes flipped quaternion
...
void CryModelState::ProcessPhysics(float fDeltaTimePhys, int nNeff) {
    ...
    if (m_pCharPhysics && (m_bPhysicsAwake = m_pCharPhysics->GetStatus(&pe_status_awake()))) {
        if (nNeff == 0) { // if there's no animation atm, just read the state from physics verbatim
    ...
    if (m_pCharPhysics) {
        ...
        if (m_bPhysicsAwake) {
            ...
            for (i = 0; i < (int)numBones(); i++)
                if (getBoneInfo(i)->m_PhysInfo[0].pPhysGeom && (iParent = getBonePhysParentIndex(i)) >= 0) { // now force positions of IK-controlled bones to physics
                    for (j = 0; j < 4 && !(m_pIKEffectors[j] && m_pIKEffectors[j]->AffectsBone(i)); j++)
                        ;
                    if (j == 4)
                        continue;
