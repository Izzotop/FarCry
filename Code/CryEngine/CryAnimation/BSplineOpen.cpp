...
BSplineVec3d::BSplineVec3d(int numKnots, int nDegree, bool isOpen) : m_nDegree(nDegree), m_Knots(numKnots + 2 * nDegree), m_isOpen(isOpen), m_arrCPs("BSplineVec3d.CPs") {
...
BSplineVec3d::Value BSplineVec3d::getValue(Time fTime) const {
    ...
    for (int i = -1; i >= -m_nDegree; --i) {
        ...
        if (!m_isOpen) { // the CP is cyclic in closed splines; in case the number of CPs is less than the degree,
