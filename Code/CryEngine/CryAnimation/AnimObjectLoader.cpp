...
void CAnimObjectLoader::LoadChunks(CChunkFileReader* pReader, bool bMakeNodes) {
    ...
    for (int nChunk = 0; nChunk < pReader->numChunks(); ++nChunk) {
        ...
        case ChunkType_Controller:
            ...
            /*
            default:
            GetLog()->LogError ("\003Unsupported controller chunk 0x%08X version 0x%08X in file %s. Please re-export the file.", chunkHeader.ChunkID, chunkHeader.ChunkVersion,
            strFileName.c_str()); return -1; break;
            }
            */
