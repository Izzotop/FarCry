...
#endif
{
    ...
    if (!cpu::hasRDTSC())
        pSystem->GetILog()->LogToFile(
            "\002Error: (Critical) this is development version of animation system, with the profiling enabled. Built-in profiler uses RDTSC function for precise time profiling. "
            "This machine doesn't seem to have RDTSC instruction implemented. Please recompile with ENABLE_FRAME_PROFILER 0. Don't use ca_Profile 1");
#endif
...
CryCharManager::~CryCharManager() {
    ...
    if (!m_arrBodyCache.empty()) {
        g_GetLog()->LogToFile("*ERROR* CryCharManager: %u body instances still not deleted. Forcing deletion (though some instances may still refer to them)",
                              m_arrBodyCache.size());
        CleanupBodyCache();
    }
    ...
    if (g_nAsyncAnimCounter)
        g_GetLog()->LogToFile("\003%d async anim loads; %d (ave %.2f) frame-skips", g_nAsyncAnimCounter, g_nAsyncAnimFrameDelays,
                              double(g_nAsyncAnimFrameDelays) / g_nAsyncAnimCounter);
...
void CryCharManager::DecideModelLockStatus(CryCharBody* pCryCharBody, unsigned nHints) {
    ...
    else if (nHints & nHintModelPersistent)
        m_setLockedBodies.insert(pCryCharBody);
    else if (g_GetCVars()->ca_KeepModels())
        m_setLockedBodies.insert(pCryCharBody);
    else
        m_setLockedBodies.erase(pCryCharBody);
}
...
void CryCharManager::RemoveCharacter(ICryCharInstance* pCryCharInstance, unsigned nFlags) {
    if (pCryCharInstance && !m_pAnimObjectManager->RemoveCharacter(pCryCharInstance)) {
        ...
        if (!bFound) {
            g_GetLog()->LogError(
                "\001attempt to delete invalid character pointer 0x%08X. Perhaps it was auto-deleted during Forced Resource Cleanup upon level reloading, see the log for details.",
                pCryCharInstance);
            return;
        }
...
void CryCharManager::ClearResources() {
    CleanupBodyCache();
}
