...
void exportTestModel(CryGeometryInfo* pGeometry, CLeafBuffer* pLeafBuffer) {
    ...
    for (unsigned long x = 0; x < numVertices; x++) {
        ...
        fprintf(
            stream, "%20.12ff,%20.12ff,%20.12ff,   %15.12ff,%15.12ff,%15.12ff,    %15.12ff,%15.12ff,%15.12ff,    %15.12ff,%15.12ff,%15.12ff,        %15.12ff,%15.12ff, //%04x \r\n",
            pVertex->x, pVertex->y, pVertex->z, pExtTangents[x].tangent.x, pExtTangents[x].tangent.y, pExtTangents[x].tangent.z, pExtTangents[x].binormal.x,
            pExtTangents[x].binormal.y, pExtTangents[x].binormal.z, pExtTangents[x].tnormal.x, pExtTangents[x].tnormal.y, pExtTangents[x].tnormal.z, pExtUVs[x].u, pExtUVs[x].v, x);
    }
    ...
    for (int i = 0; i < mcount; i++) {
        ...
        for (int i = 0; i < nNumIndices; i = i + 3) {
            fprintf(stream, "0x%04x,0x%04x,0x%04x,  //0x%04x \r\n", pIndices[FirstIndex + i + 0], pIndices[FirstIndex + i + 1], pIndices[FirstIndex + i + 2], i / 3);
        }
    }
