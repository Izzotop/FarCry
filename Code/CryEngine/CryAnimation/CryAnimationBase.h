...
// Upon destruction, it's nullptred. In any case, there must be no object needing it
...
// deinitializes the Class - actually just nullptrs the system pointer and deletes the variables
...
__forceinline CCamera& GetViewCamera() {
    return g_pISystem->GetViewCamera();
}
__forceinline ISystem* GetISystem() {
    return g_pISystem;
} // we need this one just for the profiler

__forceinline ISystem* g_GetISystem() {
    return g_pISystem;
}
__forceinline ITimer* g_GetTimer() {
    return g_pITimer;
}
__forceinline ILog* g_GetLog() {
    return g_pILog;
}
__forceinline IConsole* g_GetConsole() {
    return g_pIConsole;
}
__forceinline ICryPak* g_GetPak() {
    return g_pIPak;
}
__forceinline IStreamEngine* g_GetStreamEngine() {
    return g_pIStreamEngine;
}

__forceinline IPhysicalWorld* GetPhysicalWorld() {
    return g_pIPhysicalWorld;
}
__forceinline I3DEngine* Get3DEngine() {
    return g_pI3DEngine;
}
__forceinline IRenderer* g_GetIRenderer() {
    return g_pIRenderer;
}

__forceinline bool IsProfilerOn() {
    return g_bProfilerOn;
}
__forceinline CryAnimVars* g_GetCVars() {
    return g_pCVariables;
}
