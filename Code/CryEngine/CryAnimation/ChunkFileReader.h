...
class CChunkFileReader : public _reference_target_t {
public:
...
template <typename T> void EatRawData(T* pArray, unsigned nSize, const void*& pRawData) {
...
template <typename T> bool EatRawData(T* pArray, unsigned nSize, const void*& pRawData, unsigned& nDataSize) {
...
template <typename T> bool EatRawData(T* pArray, unsigned nSize, const void*& pRawData, const void* pRawDataEnd) {
...
// RETURNS:
//   false if failed to read the data
template <typename T> bool EatRawDataPtr(const T*& pArray, unsigned nSize, const void*& pRawData, unsigned& nDataSize) {
    if (sizeof(T) * nSize <= nDataSize) {
...
template <typename T> bool EatRawDataPtr(const T*& pArray, unsigned nSize, const void*& pRawData, const void* pRawDataEnd) {
    if ((const char*)pRawData + sizeof(T) * nSize <= (const char*)pRawDataEnd) {
        pArray = (const T*)pRawData;
        pRawData = static_cast<const T*>(pRawData) + nSize;
        return true;
