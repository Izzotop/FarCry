...
CControllerManager::CControllerManager()
    :
      ...
      m_arrAnims("CControllerManager.Anims"), m_arrAnimByFile("CControllerManager.AnimByFile"),
#endif
      m_nCachedSizeofThis(0), m_nLastCheckedUnloadCandidate(0) {
...
CControllerManager::~CControllerManager() {
    ...
    if (!m_setPendingAnimLoads.empty())
        g_GetLog()->LogWarning("\002%d asynchronous operations are left; destructing the controller manager anyway, because they seem to be dangling.If a crash happens after "
                               "this, perhaps this is because some async operation took _very_ long to complete.",
                               m_setPendingAnimLoads.size());
}
...
bool CControllerManager::LoadAnimation(int nAnimId) {
    ...
    if (Anim.IsInfoLoaded()) { // we can start loading asynchronously
...
#else
        g_GetLog()->LogError("\003Asynchronous load of file %s has failed (%d of %d bytes read), error code 0x%X", Anim.strFileName.c_str(), pStream->GetBytesRead(),
                             pAnimLoad->pFile->getSize(), nError);
#endif
    } else {
        CChunkFileReader Reader;
        if (!Reader.open(pAnimLoad->pFile))
#if defined(LINUX)
            g_GetLog()->LogError("\003Error: Asynchronous load of file %s has completed successfully, but the data can't be parsed. The last \"%s\".", Anim.strFileName.c_str(),
                                 Reader.getLastError());
#else
            g_GetLog()->LogError(
                "\003Error: Asynchronous load of file %s has completed successfully (%d of expected %d bytes read), but the data can't be parsed. The last \"%s\".",
                Anim.strFileName.c_str(), pStream->GetBytesRead(), pAnimLoad->pFile->getSize(), Reader.getLastError());
#endif
        else {
...
bool CControllerManager::LoadAnimation(int nAnimId, CChunkFileReader* pReader) {
    ...
    for (int nChunk = 0; nChunk < pReader->numChunks(); ++nChunk) {
        ...
        case ChunkType_Controller:
            ...
            default:
                g_GetLog()->LogError("\003Unsupported controller chunk 0x%08X version 0x%08X in file %s. Please re-export the file.", chunkHeader.ChunkID, chunkHeader.ChunkVersion,
                                     Anim.strFileName.c_str());
                return false;
                break;
            }
            break;
    ...
    if (nController != Anim.arrCtrls.size()) {
        g_GetLog()->LogError("\003%d controllers (%d expected) loaded from file %s. Please re-export the file. The animations will be discarded.", nController,
                             Anim.arrCtrls.size(), Anim.strFileName.c_str());
    }

    std::sort(Anim.arrCtrls.begin(), Anim.arrCtrls.end(), AnimCtrlSortPred());
...
void CControllerManager::LogUsageStats() {
    ...
    g_GetLog()->LogToFile("%u controllers, %u (%.1f percent) unused, %.2f average refs per used controller", nCtrlTotal, nCtrlUnused, (nCtrlUnused * 100.0f / nCtrlTotal),
...
void CControllerManager::DumpAnims() {
    ...
    for (SizeMap::reverse_iterator itSize = mapSizes.rbegin(); itSize != mapSizes.rend(); ++itSize) {
        ...
        g_GetLog()->LogToFile("\001%*s : %6.1f%10u%10u%s%s %d refs", nMaxNameLength, anim.strFileName.c_str(), itSize->first / 1024.0f, anim.nTickCount, anim.nStartCount,
                              anim.IsLoaded()       ? ""
                              : anim.IsInfoLoaded() ? ",Unloaded"
                                                    : ",Not Loaded",
                              anim.nRefCount ? "" : ",Not Used:", anim.nRefCount);
        g_GetLog()->LogToConsole("\001%*s : %6.1f%10u%10u%s%s %d refs", nMaxNameLength, anim.strFileName.c_str(), itSize->first / 1024.0f, anim.nTickCount, anim.nStartCount,
                                 anim.IsLoaded()       ? ""
                                 : anim.IsInfoLoaded() ? ",Unloaded"
                                                       : ",Not Loaded",
                                 anim.nRefCount ? "" : ",Not Used:", anim.nRefCount);
...
void CControllerManager::Update() {
    ...
    if (m_nLastCheckedUnloadCandidate < m_arrAnims.size()) {
        ...
        if (GlobalAnim.IsAutoUnload() && GlobalAnim.nLastAccessFrameId + g_GetCVars()->ca_AnimationUnloadDelay() < g_nFrameID) {
