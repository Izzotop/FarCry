...
CryBone::CryBone() : m_bUseMatPlus(false), m_bUseReadyRelativeToParentMatrix(false), m_pParent(nullptr) {
    // m_MatPlus.Identity();
    m_matRelativeToParent.SetIdentity();
    ; // current result of animation
...
void CryBone::BuildRelToParentFromQP(const IController::PQLog& pqNew) {
    ...
    if (_isnan(m_matRelativeToParent[0][0]))
        g_GetLog()->LogWarning("\001CryBone::BuildRelToParentFromQP: Invalid QP(q=%g,%g,%g;p=%g,%g,%g)", pqNew.vRotLog.x, pqNew.vRotLog.y, pqNew.vRotLog.z, pqNew.vPos.x,
                               pqNew.vPos.y, pqNew.vPos.z);
#ifdef _DEBUG
...
void CryBone::SetPlusRotation(float dX, float dY, float dZ) {
    ...
    m_bUseMatPlus = true;
    m_qRotPlus = quatAxis(0, -dX) * quatAxis(1, -dY) * quatAxis(2, -dZ);

    if (g_GetCVars()->ca_MatPlusDebug()) {
        if (fabs(dX) > 5 || fabs(dY) > 5 || fabs(dZ) > 5)
            g_GetLog()->LogToFile("\005 MatPlus(%s, %s)(%.1f,%.1f,%.1f)", getBoneInfo()->getNameCStr(), m_bUseReadyRelativeToParentMatrix ? "TM<-outside" : "normal TM", dX, dY,
                                  dZ);
    }
}
...
void CryBone::FixBoneOriginInWorld(const Vec3d& vCharPos, const Vec3d& vCharAngles, const Vec3d& vTargetOrigin) {
    ...
    if (!isSane(vCharPos) || !isSane(vCharAngles) || !isSane(vTargetOrigin)) {
        g_GetLog()->LogError("\001CryBone::FixBoneOriginInWorld: insane input vCharPos = %s, vCharAngles = %s, vTargetOrigin = %s", toString(vCharPos).c_str(),
                             toString(vCharAngles).c_str(), toString(vTargetOrigin).c_str());
        return;
    }
...
void CryBone::SetBoneOriginInWorld(const Vec3d& vCharPos, const Vec3d& vCharAngles, const Vec3d& vTargetOrigin) {
    using namespace CryStringUtils;
    if (!isSane(vCharPos) || !isSane(vCharAngles) || !isSane(vTargetOrigin)) {
        g_GetLog()->LogError("\001CryBone::SetBoneOriginInWorld: insane input vCharPos = %s, vCharAngles = %s, vTargetOrigin = %s", toString(vCharPos).c_str(),
                             toString(vCharAngles).c_str(), toString(vTargetOrigin).c_str());
        return;
    }
    // make tran&rot matrix
