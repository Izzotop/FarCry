...
protected:
    ...
    struct CStatistics {
        ...
        float getAveVertsPerDecal() {
            return float(numDecalVertices) / numDecals;
        }
        float getAveFacesPerDecal() {
            return float(numDecalFaces) / numDecals;
        }
        bool empty() const {
            return numDecals == 0;
        }
        void onDecalAdd(unsigned numVertices, unsigned numFaces);
        CStatistics() : numDecalVertices(0), numDecals(0), numDecalFaces(0) {}
    };
