...
public:
    ...
    CryBoneInfo* getChild(unsigned i) {
        assert(i < numChildren());
        return this + m_nOffsetChildren + i;
    }
    const CryBoneInfo* getChild(unsigned i) const {
        assert(i < numChildren());
        return this + m_nOffsetChildren + i;
    }
    CryBoneInfo* getParent() {
        return m_nOffsetParent ? this + m_nOffsetParent : nullptr;
    }
    const CryBoneInfo* getParent() const {
        return m_nOffsetParent ? this + m_nOffsetParent : nullptr;
    }
    ...
    void PostInitialize();
    IController::PQLog& getDefRelTransform() {
        return m_pqDefRelTransform;
    }
    CryQuat& getqRelPhysParent(int nLod) {
        return m_qRelPhysParent[nLod];
    }
