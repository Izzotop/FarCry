...
void CStatCFGCompiler::FindDependencies(CIndexedMesh* pIndexedMesh, ConvertContext& cc) {
    ...
    for (int m = 0; m < pIndexedMesh->m_lstMatTable.Count(); m++) {
        ...
        if (!ref.pMatEnt)
            ...
            // texture path names
#define ADD_MAP(MAP)                                                                                                                                                               \
    if (ref.pMatEnt->map_##MAP.name[0])                                                                                                                                            \
        cc.pRC->AddDependencyFile(sourceFile.GetString(), ref.pMatEnt->map_##MAP.name);
        ADD_MAP(a);
        ADD_MAP(d);
        ADD_MAP(o);
        ADD_MAP(b);
...
void CStatCFGCompiler::GetFileParams(ConvertContext& cc, CString& sGeomName, bool& bStripify, bool& bLoadAdditinalInfo, bool& bKeepInLocalSpace) {
...
bool CStatCFGCompiler::Process(ConvertContext& cc) {
    try {
        CString sGeomName;
        bool bStripify = false, bLoadAdditinalInfo = false, bKeepInLocalSpace = false;
...
void CStatCFGCompiler::ProcessCGA(ConvertContext& cc, CString& sourceFile, CString& outputFile, FILETIME fileTime, bool bStripify) {
    ...
    CIndexedMesh* pIndexedMesh = new CIndexedMesh(cc.pLog, sourceFile.GetString(), nullptr, &nLoadedTrisCount, true, true);
    if (pIndexedMesh) {
...
//////////////////////////////////////////////////////////////////////////
void CStatCFGCompiler::ProcessCGF(ConvertContext& cc, CString& sourceFile, CString& outputFile, FILETIME fileTime, CString sGeomName, bool bStripify, bool bLoadAdditinalInfo,
                                  bool bKeepInLocalSpace) {
    // load source cgf
    int nLoadedTrisCount = 0;
    CIndexedMesh* pIndexedMesh =
        new CIndexedMesh(cc.pLog, sourceFile.GetString(), sGeomName[0] ? sGeomName.GetString() : nullptr, &nLoadedTrisCount, bLoadAdditinalInfo, bKeepInLocalSpace);
    ...
    CSimpleLeafBuffer LeafBuffer(cc.pLog, pIndexedMesh, bStripify, pIndexedMesh->m_lstGeomNames.Count() > 0 && strstr(pIndexedMesh->m_lstGeomNames[0], "cloth") != 0);
    ...
    // get data size
    if (nLoadedTrisCount) {
        StatObj.Serialize(nPos, nullptr, true);
        LeafBuffer.Serialize(nPos, nullptr, true, outputFile.GetString());
        StatCGFShadVol.Serialize(nPos, nullptr, true);
    }
...
bool CStatCFGCompiler::GetOutputFile(ConvertContext& cc) {
    ...
    sprintf(szOutputFileName, "%s_%s_%d_%d_%d.ccgf", outputFileNoExt.GetString(), sGeomName.GetString(), (int)bStripify, (int)bLoadAdditinalInfo, (int)bKeepInLocalSpace);
...
int CStatCFGCompiler::SetTexType(TextureMap3* tm) {
    if (tm->type == TEXMAP_CUBIC)
        return eTT_Cubemap;
    else if (tm->type == TEXMAP_AUTOCUBIC)
