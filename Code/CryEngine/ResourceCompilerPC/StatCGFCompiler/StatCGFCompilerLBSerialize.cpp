...
bool CSimpleLeafBuffer::Serialize(int& nPos, uchar* pSerBuf, bool bSave, const char* szFolderName) {
    ...
    SaveBuffer((char*)"LeafBuffer", 11, pSerBuf, nPos);
    ...
    for (int i = 0; i < m_pMats->Count(); i++) {
        ...
        if (m_pMats->GetAt(i).m_dwNumSections) {
            SaveBuffer((void*)m_pMats->GetAt(i).m_pPrimitiveGroups, sizeof(SPrimitiveGroup) * m_pMats->GetAt(i).m_dwNumSections, pSerBuf, nPos);
        }
    }
