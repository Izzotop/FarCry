...
// from nvidia kitchen
bool CSimpleLeafBuffer::compute_tangent(const float* v0, const float* v1, const float* v2, const float* t0, const float* t1, const float* t2, Vec3d& tangent, Vec3d& binormal,
                                        Vec3d& tnormal, Vec3d& face_normal) {
    Vec3d cp;
    Vec3d e0;
    Vec3d e1;
            ...
            for (int nn = 0; nn < 2; nn++) {
                ...
                for (i = 0; i < m_SecVertCount; i++) {
                    ...
                    for (j = 0; j < hash_table[nn][hash].Num(); j++) {
                        ...
                        if (fabs(v1[0] - v[0]) < TV_EPS && fabs(v1[1] - v[1]) < TV_EPS && fabs(v1[2] - v[2]) < TV_EPS && fabs(n1[0] - n[0]) < TN_EPS &&
                            fabs(n1[1] - n[1]) < TN_EPS && fabs(n1[2] - n[2]) < TN_EPS) {
