...
void CSimpleStatObj::Physicalize() {
    ...
    for (int nMesh = 0; nMesh <= 2; nMesh++) { // fill physics indices
        ...
        if (nMesh == MESH_PHYSIC) { // find all physicalized faces
            ...
            if (nObstrMatID >= 0 || nLeavesMatID >= 0) { // find all obstruct faces
                for (int i = 0; i < m_pTriData->m_nFaceCount; i++) {
                    if (m_pTriData->m_pFaces[i].shader_id == nObstrMatID || m_pTriData->m_pFaces[i].shader_id == nLeavesMatID) {
                        for (int v = 0; v < 3; v++)
                            lstPhysIndices.Add(m_pTriData->m_pFaces[i].v[v]);

                        lstFaceMaterials.Add((unsigned char)m_pTriData->m_pFaces[i].shader_id);

                        if (m_pTriData->m_pFaces[i].shader_id == nObstrMatID) { // remove face from list (it's not needed for rendering)
                            if (m_pTriData->m_nFaceCount > 1)
                                m_pTriData->m_pFaces[i] = m_pTriData->m_pFaces[m_pTriData->m_nFaceCount - 1];
                            m_pTriData->m_nFaceCount--;
                            i--;
...
void CSimpleStatObj::Serialize(int& nPos, uchar* pSerBuf, bool bSave) {
    ...
    SaveBuffer((void*)"StatObj", 8, pSerBuf, nPos);
