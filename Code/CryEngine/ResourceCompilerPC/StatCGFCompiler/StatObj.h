...
public:
    CLeafBuffer* GetLeafBuffer() override { return m_pLeafBuffer; };
    void SetLeafBuffer(CLeafBuffer* buf) override { m_pLeafBuffer = buf; };

    Vec3d m_vBoxMin, m_vBoxMax, m_vBoxCenter; //, m_vGeometryAngles;
    phys_geometry* m_arrPhysGeomInfo[2];
    phys_geometry* GetPhysGeom(int n = 0) override { return m_arrPhysGeomInfo[n];  }

    const char* GetFolderName() override { return m_szFolderName; }
    const char* GetFileName() override { return m_szFileName; }
    const char* GetGeoName() override { return m_szGeomName; }
    bool IsSameObject(const char* szFileName, const char* szGeomName) override;

    // set object's min/max bbox
    void SetBBoxMin(const Vec3d& vBBoxMin) override { m_vBoxMin = vBBoxMin; }
    void SetBBoxMax(const Vec3d& vBBoxMax) override { m_vBoxMax = vBBoxMax; }
    Vec3d GetBoxMin() override { return m_vBoxMin; } // Izzotop: Originally Vec3d&
    Vec3d GetBoxMax() override { return m_vBoxMax; } // Izzotop: Originally Vec3d&

    int m_nUsers; // reference counter

    ShadowMapLightSource* m_pSMLSource;

    void MakeShadowMaps(Vec3d vSunPos);
...
public:
    //  void DrawShadowMapOnTerrain(const Vec3d & pos, const float fScale, float fAlpha, struct IndexedVertexBuffer ** ppShadowGridBuffer, CTerrain * pTerrain, bool
    //  bLMapsGeneration);
    ...
    virtual const char* GetHelperById(int nId, Vec3d& vPos, struct Matrix* pMat, int* pnType);
    virtual const Matrix44* GetHelperMatrixByName(const char* szHelperName); // Izzotop: Originally const Matrix
    ...
    virtual bool IsDefaultObject() {
        return (m_bDefaultObject);
    }
