...
bool CIndexedMesh::LoadCGF(const char* szFileName, const char* szGeomName, bool bLoadAdditinalInfo, bool bKeepInLocalSpace, bool bIgnoreFakeMats) {
    ...
    if (!pStaticCGF->OnLoadgeom((char*)szFileName, nullptr, bReset, bKeepInLocalSpace)) {
        delete pStaticCGF;
        pStaticCGF = nullptr;
    ...
    for (int g = 0; g < pStaticCGF->m_lstGeoms.Count(); g++) {
        ...
        if (!geom) {
            m_pSystem->Log("CIndexedMesh::LoadCGF error");
            break;
        }
                ...
                if (m_pFaces[c + m_nFaceCount].shader_id >= pStaticCGF->m_lstMaterials.Count() && pStaticCGF->m_lstMaterials.Count()) {
                    if (!bFacesWithInvalidMatIdFoundInThisFile) {
                        if (m_pFaces[c + m_nFaceCount].shader_id == (unsigned short)-1) {
                            m_pSystem->Log("Warning: CIndexedMesh::LoadCGF: Some faces has undefined material."
                                           " File name is %s%s%s.",
                                           szFileName, szGeomName ? ", object name is " : "", szGeomName ? szGeomName : "");
                        } else {
                            m_pSystem->Log("Warning: CIndexedMesh::LoadCGF: Some faces are referencing to material which is not present in the file."
                                           " File name is %s%s%s."
...
CIndexedMesh::CIndexedMesh(IRCLog* pLog, const char* szFileName, const char* szGeomName, int* _count, bool bLoadAdditinalInfo, bool bKeepInLocalSpace, bool bIgnoreFakeMats) {
    ...
    for (int m = 0; m < m_lstMatTable.Count(); m++) {
        ...
        if (pMatInfo->pMatEnt) {
            char szMatName[128];
            strncpy(szMatName, pMatInfo->pMatEnt->name, sizeof(szMatName));
            szMatName[sizeof(szMatName) - 1] = 0;
            strlwr(szMatName);
