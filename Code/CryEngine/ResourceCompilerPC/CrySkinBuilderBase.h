...
public:
    ...
    ICrySkinSource(const CryVertexBinding* pLinks, unsigned numLinks, const Vec3* pVertices, unsigned numVertices, const TangData* pExtTangents, unsigned numExtTangents,
                   const unsigned* pExtToIntMapping)
        : m_pLinks(pLinks), m_numLinks(numLinks), m_pVertices(pVertices), m_numVertices(numVertices), m_pExtTangents(pExtTangents), m_numExtTangents(numExtTangents),
    ...
    const CryVertexBinding& getLink(unsigned i) const {
        return m_pLinks[i];
    }
    unsigned numExtTangents() const {
        return m_numExtTangents;
    }
    const unsigned* getExtToIntMapEntries() const {
        return m_pExtToIntMapping;
    }
    const TangData& getExtTangent(unsigned i) const {
        return m_pExtTangents[i];
    }
...
public:
    CrySkinBuilderBase0(const class ICrySkinSource* pGeometry) : m_pGeometry(pGeometry) {}
...
protected:
    ...
    struct BoneVertexGroup {
        ...
        bool empty() const {
            return arrRigid.empty() && arrSmooth.empty();
        }
