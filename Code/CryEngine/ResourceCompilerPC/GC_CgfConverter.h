...
public:
    GC_CCFFileWriter(FILE* f) : m_pFile(f), m_nLastChunk(-1) {}

    GC_CCFFileWriter() : m_pFile(nullptr), m_nLastChunk(-1) {}
...
public:
    ...
    GC_CGFConvertor() : m_fTarget(nullptr), m_pStatCGFCompiler(nullptr) {}
...
protected:
    ...
    template <class T> void write(const T& rData) {
        writeRaw(&rData, sizeof(T));
    }

    template <class T> void write(const T* pData, unsigned numElements) {
        writeRaw(pData, numElements * sizeof(T));
    }

    template <class T> void writeArray(const std::vector<T>& arrData) {
        if (!arrData.empty()) {
