...
class CStencilShadowConnectivitynullptr : public IStencilShadowConnectivity {
public:
    ...
    const virtual CStencilShadowConnectivity* GetInternalRepresentation() const {
        return nullptr;
    }
    ...
    virtual void GetStats(DWORD& outVertexCount, DWORD& outTriangleCount) {
        outVertexCount = outTriangleCount = 0;
    }

    //! Memorize the vertex buffer in this connectivity object. This is needed if a static object doesn't need this info
    virtual void SetVertices(const Vec3d* pVertices, unsigned numVertices) {
        assert(0);
    }

    //! Serializes this object to the given memory block; if it's nullptr, only returns
    //! the number of bytes required. If it's not nullptr, returns the number of bytes written
...
public:
    ...
    struct BasicEdge {
        ...
        inline bool operator<(const BasicEdge& rEdge) const {
            if (m_nVertex[0] < rEdge.m_nVertex[0])
                return true;
            else if (m_nVertex[0] == rEdge.m_nVertex[0])
    ...
    struct EdgeFace {
        ...
        //! /return face index
        inline vindex getFaceIndex() const {
            return m_nFace;
        }

        //! /return vertex index
        inline vindex getVertexIndex() const {
            return m_nVertex;
        }
    ...
    public:
        ...
        Edge(const BasicEdge& be, const EdgeFace& ef0, const EdgeFace& ef1) : BasicEdge(be) {
            m_Face[0] = ef0;
            m_Face[1] = ef1;
        }
    ...
    public:
        ...
        // constructor
        OrphanEdge(const BasicEdge& be, const EdgeFace& ef) : BasicEdge(be), m_Face(ef) {}

        const EdgeFace& getFace() const {
            return m_Face;
        }
        void setFace(const EdgeFace& ef) {
            m_Face = ef;
        }
    };
    ...
    virtual bool DebugConnectivityInfo(const char* szFilename) {
        ...
        for (unsigned i = 0; i < m_numEdges; i++) {
            fprintf(out, "   face={%d,%d}, vertex={%d,%d}\n", (m_pEdges[i].getFace(0)).getFaceIndex(), (m_pEdges[i].getFace(1)).getFaceIndex(), m_pEdges[i].m_nVertex[0],
...
protected:
    ...
    virtual ~CStencilShadowConnectivity() {
        if (m_pEdges)
            delete[] m_pEdges;
        if (m_pOrphanEdges)
            delete[] m_pOrphanEdges;
        if (m_pFaces)
            delete[] m_pFaces;
        if (m_pPlanes)
            delete[] m_pPlanes;
...
public:
    ...
    unsigned Deserialize(void* pStream, unsigned nSize, IMiniLog* pWarningLog = nullptr) {
        ...
        unsigned nRequiredSize = sizeof(unsigned) + sizeof(m_numEdges) + m_numEdges * sizeof(Edge) + sizeof(m_numOrphanEdges) + m_numOrphanEdges * sizeof(OrphanEdge) +
                                 sizeof(m_numVertices) + sizeof(m_numFaces) + m_numFaces * sizeof(Face) + sizeof(numPlanes) + numPlanes * sizeof(Plane) + sizeof(numVertices) +
                                 numVertices * sizeof(Vec3d);
        ...
        if (m_numOrphanEdges && pWarningLog)
            pWarningLog->LogWarning("Connectivity warning: there are %d open edges (%d closed, %d faces, %d planes, %d vertices)", m_numOrphanEdges, m_numEdges, m_numFaces,
                                    numPlanes, numVertices);
#endif
    ...
    unsigned Serialize(bool bSave, void* pStream, unsigned nSize, IMiniLog* pWarningLog = nullptr) {
        ...
        unsigned numPlanes = m_pPlanes ? m_numFaces : 0, numVertices = m_pVertices ? m_numVertices : 0,
                 nRequiredSize = sizeof(unsigned) + sizeof(m_numEdges) + m_numEdges * sizeof(Edge) + sizeof(m_numOrphanEdges) + m_numOrphanEdges * sizeof(OrphanEdge) +
                                 sizeof(m_numVertices) + sizeof(m_numFaces) + m_numFaces * sizeof(Face) + sizeof(numPlanes) + numPlanes * sizeof(Plane) + sizeof(numVertices) +
                                 numVertices * sizeof(Vec3d);
    ...
    bool hasPlanes() const {
        return m_pPlanes != nullptr;
    }
    const Plane& getPlane(unsigned i) const {
        assert(i < m_numFaces && m_pPlanes);
        return m_pPlanes[i];
    }

    const Vec3d* getVertices() const {
        return m_pVertices;
    }

    bool IsStandalone() const {
        return m_pPlanes != nullptr && m_pVertices != nullptr;
    }
...
private:
    ...
    Face* m_pFaces;      //!<
    Plane* m_pPlanes;    //!< if not nullptr, then use these normals instead of computing them on the fly; 1 face - 1 normal
    Vec3d* m_pVertices;
};
