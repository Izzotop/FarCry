...
inline bool isEqual(const AnimEvent& left, const AnimEvent& right, float fTolerance = 1e-3f) {
    return left.UserData == right.UserData && fabs(left.fTime - right.fTime) < fTolerance;
}
...
struct AnimData {
    ...
    AnimData() : fStart(0), fStop(0), bLoop(false), nGlobalAnimId(-1) {}
    bool isStatic() const {
        return fStart == fStop;
    }
...
struct GlobalAnimation {
    ...
    bool IsLoaded() const {
        return !arrCtrls.empty();
    }

    bool IsInfoLoaded() const {
        return (nFlags & FLAGS_INFO_LOADED) != 0;
    }
    void OnInfoLoaded() {
        nFlags |= FLAGS_INFO_LOADED;
    }
    bool IsAutoUnload() const {
        return (nFlags & FLAGS_DISABLE_AUTO_UNLOAD) == 0;
    }
