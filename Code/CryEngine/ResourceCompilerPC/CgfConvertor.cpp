...
CGFConvertor::CGFConvertor() : m_fTarget(nullptr), m_pStatCGFCompiler(nullptr) {
...
bool CGFConvertor::Process(ConvertContext& cc) {
    ...
    if (CryStringUtils::MatchWildcard(CryStringUtils::toLower(string(cc.sourceFile.GetString())).c_str(), "*_lod?.cgf") && isAnimatedFastCheck(strSourceFilePath.GetString())) {
        ...
        if (!m_arrLODs.empty()) {
            ...
            if (m_fTarget) {
                ...
                for (unsigned nLOD = 0; nLOD < m_arrLODs.size(); ++nLOD) {
                    ...
                    if (nLOD < 2) { // write the LOD 0 and possibly 1 physical geometry for bones
...
void CGFConvertor::WriteBoneGeometry(unsigned nLOD) {
    ...
    for (unsigned nBone = 0; nBone < pMaster->Bones.numBones(); ++nBone) {
        ...
        if (!pMesh) {
            if (nChunkIdMesh != -1)
                LogError("Cannot find bone mesh for bone %d (lod bone %d) LOD %d (mesh chunk id expected: 0x%08X). No physics will be initialized for this bone.", nBone, nLODBone,
                         nLOD, nChunkIdMesh);
            continue;
        }
...
void CGFConvertor::writeMorphTargets(unsigned nLOD) {
    ...
    for (unsigned nMorphTarget = 0; nMorphTarget < numMorphTargets; ++nMorphTarget) {
        ...
        // write the name
        unsigned nLen = pMorphTarget->strName.length() + 1; // write the ternimating nullptr also
...
// Log ("  %s", me.name);
#define ADD_MAP(MAP)                                                                                                                                                               \
    if (me.map_##MAP.name[0])                                                                                                                                                      \
        m_pContext->pRC->AddDependencyFile(inszSrcFileName, me.map_##MAP.name);
            ADD_MAP(a);
            ADD_MAP(d);
            ADD_MAP(o);
            ADD_MAP(b);
