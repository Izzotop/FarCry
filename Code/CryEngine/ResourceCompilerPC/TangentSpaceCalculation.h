...
template <class InputProxy> class CTangentSpaceCalculation {
public:
    // IN ------------------------------------------------
...
private: // -----------------------------------------------------------------
    ...
    public:
        ...
        CVec2() {}
        CVec2(float fXval, float fYval) {
            x = fXval;
            y = fYval;
        }
        friend CVec2 operator-(const CVec2& vec1, const CVec2& vec2) {
            return CVec2(vec1.x - vec2.x, vec1.y - vec2.y);
        }
        operator float*() {
            return ((float*)this);
        }
    ...
    public:
        ...
        CVec3() {}
        CVec3(float fXval, float fYval, float fZval) {
            x = fXval;
            y = fYval;
            z = fZval;
        }
        friend CVec3 operator-(const CVec3& vec1, const CVec3& vec2) {
            return CVec3(vec1.x - vec2.x, vec1.y - vec2.y, vec1.z - vec2.z);
        }
        friend CVec3 operator-(const CVec3& vec1) {
            return CVec3(-vec1.x, -vec1.y, -vec1.z);
        }
        friend CVec3 operator+(const CVec3& vec1, const CVec3& vec2) {
            return CVec3(vec1.x + vec2.x, vec1.y + vec2.y, vec1.z + vec2.z);
        }
        friend CVec3 operator*(const CVec3& vec1, const float fVal) {
            return CVec3(vec1.x * fVal, vec1.y * fVal, vec1.z * fVal);
        }
        friend float operator*(const CVec3& vec1, const CVec3& vec2) {
        ...
        friend CVec3 cross(const CVec3& vec1, const CVec3& vec2) {
            return CVec3(vec1.y * vec2.z - vec1.z * vec2.y, vec1.z * vec2.x - vec1.x * vec2.z, vec1.x * vec2.y - vec1.y * vec2.x);
        }
        friend float length(const CVec3& invA) {
            return (float)sqrt(invA.x * invA.x + invA.y * invA.y + invA.z * invA.z);
        }
        ...
        friend bool IsZero(const CVec3& invA) {
            return (invA.x == 0.0f && invA.y == 0.0f && invA.z == 0.0f);
        }
        friend bool IsNormalized(const CVec3& invA) {
            float f = length(invA);
    ...
    public:
        CBase33() {}
        CBase33(CVec3 Uval, CVec3 Vval, CVec3 Nval) {
            u = Uval;
            v = Vval;
            n = Nval;
        }
...
private:
    ...
    DWORD AddUV2Base(std::multimap<CBaseIndex, DWORD, CBaseIndexOrder>& inMap, const DWORD indwPosNo, const DWORD indwNormNo, const CVec3& inU, const CVec3& inV,
                     const CVec3& inNormN);
...
template <class InputProxy> void CTangentSpaceCalculation<InputProxy>::DebugMesh(const InputProxy& inInput) const {
...
template <class InputProxy> void CTangentSpaceCalculation<InputProxy>::CalculateTangentSpace(const InputProxy& inInput) {
    DWORD dwTriCount = inInput.GetTriangleCount();
        ...
        for (DWORD i = 0; i < dwTriCount; i++) {
            ...
            */
            ...
            if (div != 0.0) {
                //    area(u1*v2-u2*v1)/2
                float fAreaMul2 =
                    fabsf(fDeltaU1 * fDeltaV2 - fDeltaU2 * fDeltaV1); // weight the tangent vectors by the UV triangles area size (fix problems with base UV assignment)
    ...
    // distribute the normals to the vertices
    {
        ...
        for (DWORD i = 0; i < dwTriCount; i++) {
            ...
            for (e = 0; e < 3; e++) {
                ...
                //                float fWeight=length(cross( vPos[(e+2)%3]-vPos[e],vPos[(e+1)%3]-vPos[e] ));            // weight by area, that does not fix the L-Shape problem
                //                100%
...
DWORD CTangentSpaceCalculation<InputProxy>::AddUV2Base(std::multimap<CBaseIndex, DWORD, CBaseIndexOrder>& inMap, const DWORD indwPosNo, const DWORD indwNormNo, const CVec3& inU,
                                                       const CVec3& inV, const CVec3& inNormN) {
    // no mesh is perfect
...
template <class InputProxy>
void CTangentSpaceCalculation<InputProxy>::AddNormal2Base(std::multimap<CBaseIndex, DWORD, CBaseIndexOrder>& inMap, const DWORD indwPosNo, const DWORD indwNormNo,
                                                          const CVec3& inNormal) {
    CBaseIndex Indx;
...
template <class InputProxy> void CTangentSpaceCalculation<InputProxy>::GetBase(const DWORD indwPos, float outU[3], float outV[3], float outN[3]) {
...
template <class InputProxy> void CTangentSpaceCalculation<InputProxy>::GetTriangleBaseIndices(const DWORD indwTriNo, DWORD outdwBase[3]) {
...
template <class InputProxy> size_t CTangentSpaceCalculation<InputProxy>::GetBaseCount() {
