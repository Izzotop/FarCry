...
struct CrySkinRigidVertex {
    ...
    CrySkinRigidVertex(const Vec3& _ptOffset, unsigned _nDest) : pt(_ptOffset), nDest(_nDest) {}
...
struct CrySkinRigidBaseInfo {
    ...
    CrySkinRigidBaseInfo(const Matrix44& matInvDef, const TangData& rBasis, unsigned _nDest)
        : ptTangent(matInvDef.TransformVectorOLD(rBasis.tangent)), ptBinormal(matInvDef.TransformVectorOLD(rBasis.binormal)), nDest(_nDest) {}
...
struct CrySkinSmoothVertex : public CrySkinRigidVertex {
    ...
    CrySkinSmoothVertex(const CryLink& rLink, unsigned _nDest) : CrySkinRigidVertex(rLink.offset, _nDest), fWeight(rLink.Blending) {}
