...
void PrintTrace(int signum) {
    ...
    case SIGHUP:
        ...
        if (p)
            fprintf(p, "Received signal SIGHUP: user's terminal is disconnected\n");
        break;
    case SIGTERM:
        printf("Received signal SIGTERM: cause program termination\n");
        if (p)
            fprintf(p, "Received signal SIGTERM: cause program termination\n");
        break;
    case SIGQUIT:
        printf("Received signal SIGQUIT: received QUIT character (normally C-\) character\n");
        if (p)
            fprintf(p, "Received signal SIGQUIT: received QUIT character (normally C-\) character\n");
        break;
    case SIGKILL:
        printf("Received signal SIGKILL: cause immediate program termination\n");
        if (p)
            fprintf(p, "Received signal SIGKILL: cause immediate program termination\n");
        break;
    case SIGILL:
        printf("Received signal SIGILL: 'illegal instruction', program is trying to execute garbage or a privileged instruction\n");
        if (p)
            fprintf(p, "Received signal SIGILL: 'illegal instruction', program is trying to execute garbage or a privileged instruction\n");
        break;
    case SIGBUS:
        printf("Received signal SIGBUS: invalid pointer is dereferenced\n");
        if (p)
            fprintf(p, "Received signal SIGBUS: invalid pointer is dereferenced\n");
        break;
    case SIGSEGV:
        printf("Received signal SIGSEGV: program tries to read or write outside the memory that is allocated for it\n");
        if (p)
            fprintf(p, "Received signal SIGSEGV: program tries to read or write outside the memory that is allocated for it\n");
        break;
    case SIGABRT:
        printf("Received signal SIGABRT: error detected by the program itself and reported by calling 'abort'\n");
        if (p)
            fprintf(p, "Received signal SIGABRT: error detected by the program itself and reported by calling 'abort'\n");
        break;
    }
    ...
    if (p)
        fprintf(p, "Stack-Trace: obtained %d stack frames.\n", size);
...
static void SetMasterCDFolder(const char* pExecutableName) {
    ...
    string currentDir(szExeFileName);
    currentDir += '/'; // temp storage
...
void CopyPunkBusterFiles() {
    ...
    char szExeFileName[_MAX_PATH];
    GetCurrentDirectory(sizeof(szExeFileName), szExeFileName);
    string destDir(szExeFileName);
    if (destDir.c_str()[destDir.size() - 1] != '/')
        destDir += "/";
    string linuxDestDir(destDir);
    linuxDestDir += "pb";
    ...
    if (dirp == nullptr) {
        ...
        if (dirp == nullptr) {
            printf("Cannot read symlink to Punkbuster directory: %s , Punkbuster won't work\n. Please create a symlink 'pb' pointing to 'PB' in the mastercd folder manually to "
                   "make it work. Check access rights too.\n",
                   linuxDestDir.c_str());
            return;
        }
    } else {
...
bool OnWinInChar(unsigned int uiChar, unsigned int uiRepCnt, unsigned int uiFlags) {
    switch (uiChar) {
    case VK_TAB:
    case VK_RETURN: {
        return true;
...
LRESULT CALLBACK WndInWindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
    case WM_KEYDOWN: {
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) {
#ifdef WIN32
    g_WndMain = CreateWindow(g_szWindowClass, "Far Cry dedicated server", WS_OVERLAPPED | WS_CAPTION | WS_MINIMIZEBOX | WS_SYSMENU, CW_USEDEFAULT, 0,
                             g_dwInnerWidth + 2 * GetSystemMetrics(SM_CXFIXEDFRAME), g_dwInnerHeight + 2 * GetSystemMetrics(SM_CYFIXEDFRAME) + GetSystemMetrics(SM_CYCAPTION), nullptr,
    ...
    g_WndOut =
        CreateWindow(g_szWindowClass, "", WS_CHILD | WS_VISIBLE | WS_DISABLED, 0, 0, g_dwInnerWidth, g_dwInnerHeight - g_dwEditLineHeight, g_WndMain, HMENU(), hInstance, nullptr);

    g_WndIn = CreateWindow("EDIT", "", WS_CHILD | WS_VISIBLE, 0, g_dwInnerHeight - g_dwEditLineHeight, g_dwInnerWidth, g_dwEditLineHeight, g_WndMain, HMENU(), hInstance, nullptr);
