...
class CEntityCamera : public IEntityCamera {
public:
    ...
    Matrix44 GetMatrix() const {
        return m_camera.GetVCMatrixD3D9();
    };
    ...
    void SetThirdPersonMode(const Vec3d& pos, const Vec3d& angles, int mode, float frameTime, float range, int dangleAmmount, IPhysicalEntity* physic, IPhysicalEntity* physicMore,
                            I3DEngine* p3DEngine, float safe_range = 0.0f);

    void SetViewOffset(float f) {
        m_sParam.m_viewoffset = f;
    };
    float GetViewOffset() {
        return m_sParam.m_viewoffset;
    };
    ...
    void SetParameters(const EntityCameraParam* pParam) {
        memcpy(&m_sParam, pParam, sizeof(EntityCameraParam));
    };
    void GetParameters(EntityCameraParam* pParam) {
        memcpy(pParam, &m_sParam, sizeof(EntityCameraParam));
    };

    void SetCameraOffset(const Vec3d& offset) {
        m_vCameraOffset = m_camera.m_vOffset = offset;
    }
    void GetCameraOffset(Vec3d& offset) {
        offset = m_vCameraOffset;
    }
