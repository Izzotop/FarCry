...
bool CEntity::DrawEntity(const SRendParams& _EntDrawParams) {
    ...
    if (nRecursionLevel == 0) { // movement detection (if no recursion)
    ...
    if (!(GetRndFlags() & ERF_DONOTCHECKVIS))
        if (!rParms.pShadowVolumeLightSource && !(rParms.dwFObjFlags & FOB_RENDER_INTO_SHADOWMAP)) {
            ...
            if (m_bForceBBox) { // if bbox was forced - make it bigger for frustum culling
...
void CEntity::ProcessEntityLightSources() {
    ...
    for (int i = 0; i < MAX_ANIMATED_MODELS; i++) {
        ...
        if (cmodel && (cmodel->GetFlags() & CS_FLAG_DRAW_MODEL) && cmodel->GetBoundLight(0)) {
            ...
            if (m_pContainer) { // use character angles and entity pos
...
int CEntity::CreateEntityParticleEmitter(int nSlotId, const ParticleParams& PartParams, float fSpawnPeriod, Vec3d vOffSet, Vec3d vDir, IParticleEffect* pEffect, float fScale) {
    if (nSlotId > 16) {
...
float CEntity::GetMaxViewDist() {
    ...
    return max(p3DEngine->GetObjectsMinViewDist(), GetRenderRadius() * p3DEngine->GetObjectsViewDistRatio() * GetViewDistRatioNormilized());
}
