...
void CEntityCamera::Init(IPhysicalWorld* pIPhysWorld, UINT iWidth, UINT iHeight, IConsole* pConsole) {
    ...
    m_sParam.m_1pcam_butt_pos(0, 0, 0);
    m_sParam.m_1pcam_eye_pos(0, 0, 0);
...
void CEntityCamera::SetThirdPersonMode(const Vec3d& center, const Vec3d& angles0, int mode, float frameTime, float range, int dangleAmmount, IPhysicalEntity* physic,
                                       IPhysicalEntity* physicMore, I3DEngine* p3DEngine, float safeRange) {
    ...
    if (mode == CAMERA_3DPERSON1) {
        ...
        if (range < 3 && GetLengthSquared((m_sParam.m_1pcam_eye_pos - m_sParam.m_1pcam_butt_pos)) > 0) {
            ...
            cam_kstiffness = m_sParam.m_cam_kstiffness;
            cam_kdamping = m_sParam.m_cam_kdamping;
            cam_angle_kstiffness = m_sParam.m_cam_angle_kstiffness;
            cam_angle_kdamping = m_sParam.m_cam_angle_kdamping;
        }
        dir = vectorf(GetPos()) - tview;

        Quat q(1, 0, 0, 0);
        Vec3 angles = angles0;
        if (m_sParam.m_cam_angle_flags & 4)
            q = GetRotationAA(angles.x * (gf_PI / 180.0f), vectorf(1, 0, 0));
        if (m_sParam.m_cam_angle_flags & 2)
            q = GetRotationAA(angles.y * (gf_PI / 180.0f), vectorf(0, 1, 0)) * q;
        ...
        if (m_sParam.m_cam_angle_flags & 1)
            q = GetRotationAA(angles.z * (gf_PI / 180.0f), vectorf(0, 0, 1)) * q;
        ...
        if (!b1p) {
            ...
            if (!bPiercing && m_pIPhysWorld->RayWorldIntersection(tview, camposTmp - tview, ent_static | ent_sleeping_rigid | ent_rigid | ent_terrain,
                                                                  //      rwi_stop_at_pierceable,
                                                                  rwi_ignore_noncolliding, &hit, 1, physic, physicMore)) {
        ...
        if (!b1p && m_fDeltaDist == 0.0f) {
            dir = (campos - tview).normalize() * 0.5;
            campos += dir;
            if (!bPiercing && m_pIPhysWorld->RayWorldIntersection(tview, campos - tview, ent_static | ent_sleeping_rigid | ent_rigid | ent_terrain,
                                                                  //      rwi_stop_at_pierceable,
                                                                  rwi_ignore_noncolliding, &hit, 1, physic, physicMore)) {
                m_fTimeIdle = 0.0f;
                campos = hit.pt;
