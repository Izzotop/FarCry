...
CEntity::CEntity(CEntitySystem* pEntitySystem, ISystem* pISystem, IScriptSystem* pSS)
    : m_pAnimationEventParams(pSS), m_pObjectCollide(pSS), m_vObjPosCollide(pSS), m_vObjVelCollide(pSS), m_pSplashList(pSS) //,m_vNormDirCollide(pSS)
{
    ...
    m_pDynLight = nullptr;
    m_pEntityRenderState = nullptr;  // m_pISystem->GetI3DEngine()->MakeEntityRenderState();
...
void CEntity::ShutDown() {
    ...
    for (itaux = m_auxObjects.begin(); itaux < m_auxObjects.end(); itaux++) {
        if (*itaux) {
...
void CEntity::SetAngles(const Vec3d& sAngle, bool bNotifyContainer, bool bUpdatePhysics, bool forceInWorld) {
    ...
    */
    ...
    if (forceInWorld) {
        m_angles = sAngle;
        return;
    } else if (m_bIsBound || m_bForceBindCalculation) {
        m_realangles = sAngle;
        CalculateInWorld();
    } else {
        Vec3d diff;
...
// update a bunch of various stuff
...
void CEntity::OnCollide(float fDeltaTime) {
    ...
    // todo (Izzotop): Taking the address of a temporary object of type 'pe_status_awake'
    ...
    if ((bAwake + m_bWasAwake) && (nColls = m_physic->GetStatus(&sc))) {
        ...
        m_pObjectCollide->SetTonullptrChain("impact");
        m_pObjectCollide->SetTonullptrChain("roll");
        m_pObjectCollide->SetTonullptrChain("slide");
        ...
        if (pColliderEntity)
            ...
            m_pObjectCollide->SetTonullptrChain("collider");
        ...
        sd.waterResistance > m_pEntitySystem->m_pSplashThreshold->GetFVal()) // now test if the object hit the water hard enough
    {
        ...
        // todo (Izzotop): Taking the address of a temporary object of type 'pe_status_nparts'
        for (sp.ipart = m_physic->GetStatus(&pe_status_nparts()) - 1; sp.ipart >= 0; sp.ipart--) {
            m_physic->GetStatus(&sp);
            gwd.offset = sp.pos;
            gwd.R = matrix3x3f(sp.q);
            gwd.scale = sp.scale;
            for (iCont = sp.pGeomProxy->Intersect(pWaterSurface, &gwd, nullptr, nullptr, pContacts) - 1; iCont >= 0; iCont--) {
                for (iCircle = pWorld->GetPhysUtils()->CoverPolygonWithCircles(strided_pointer<Vec2>((Vec2 *) pContacts[iCont].ptborder, sizeof(Vec3)), pContacts[iCont].nborderpt, pContacts[iCont].bBorderConsecutive, (const Vec2 &) pContacts[iCont].center, pCenters, pRadii, 0.5f) - 1; iCircle >= 0; iCircle--) {
    ...
    if (nCircles) {
        ...
        m_pObjectCollide->SetTonullptrChain("num_splashes");
        m_pObjectCollide->SetTonullptrChain("splashes");
    }
    ...
    if ((sd.waterResistance > 10.0f) && (sd.submergedFraction > 0.0f) && (m_fLastSubMergeFracion <= 0.0f)) {
        ...
        if (bCallOnCollide)
            m_pObjectCollide->SetTonullptrChain("waterresistance");
...
void CEntity::UpdatePhysics(SEntityUpdateContext& ctx) {
    ...
    if (m_physic && (m_flags & ETY_FLAG_CALC_PHYSICS) && !(m_flags & ETY_FLAG_IGNORE_PHYSICS_UPDATE)) {
        ...
        else if (m_pClientState && m_pClientState->pFunction[ScriptState_OnCollide])
            bOnCollideImplemented = true;
    ...
    for (int k = 0; k < m_nMaxCharNum; k++)
        if (m_pCryCharInstance[k] && (m_pCryCharInstance[k]->GetFlags() & CS_FLAG_UPDATE)) {
            ...
            if (m_physic && m_pEntitySystem->m_pUpdatePhysics->GetIVal()) {
                ...
                if (!m_bIsBound && m_pEntitySystem->m_pCharacterIK->GetIVal() && m_physic->GetType() == PE_LIVING && !(livstat.bFlying && livstat.timeFlying > 0.2f) &&
                    ...
                    for (i = 0; i < 2; i++)
                        if (sensors.pPoints[i].z > livdim.heightCollider * 0.7f)
                    ...
                    if (m_nFlyingFrames < 4 && livstat.vel.len2() < 16 && (livstat.vel.len2() < 0.01f || !livstat.bOnStairs))
                        for (i = 0; i < 2; i++)
                            if (sensors.pPoints[i].z - fCharZOffsetBase /*m_pCryCharInstance[k]->GetOffset().z*/ < -0.02f && sensors.flags & 1 << i)
...
void CEntity::UpdateAIObject(bool bEntityVisible) {
    ...
    if (bEntityVisible)
        m_pAIObject->Event(AIEVENT_WAKEUP, nullptr);
}
...
int CEntity::CreatePhysicalEntityCallback(int iForeignFlags) {
    if (iForeignFlags & 1 << 15) {
        ...
        switch (m_iPhysType) {
        case PHYS_RIGID: {
            CreateRigidBody(m_PhysData.RigidBody.type, m_PhysData.RigidBody.density, m_PhysData.RigidBody.mass, m_PhysData.RigidBody.surface_idx, nullptr, m_PhysData.RigidBody.slot, true);
            SendScriptEvent(ScriptEvent_PhysicalizeOnDemand, 0);
...
void CEntity::EnablePhysics(bool enable) {
    if (!enable && m_physPlaceholder) {
        ...
        switch (m_iPhysType) {
        case PHYS_RIGID:
            CreateRigidBody(m_PhysData.RigidBody.type, m_PhysData.RigidBody.density, m_PhysData.RigidBody.mass, m_PhysData.RigidBody.surface_idx, 0, m_PhysData.RigidBody.slot);
            break;
        case PHYS_STATIC:
...
void CEntity::AddImpulse(int ipart, Vec3d pos, Vec3d impulse, bool bPos, float fAuxScale) {
    ...
    if (physic && (!m_bIsADeadBody
        ...
        // todo (Izzotop): Taking the address of a temporary object of type 'pe_status_nparts'
        if (!(physic->GetStatus(&pe_status_nparts()) > 5 && physic->GetType() == PE_ARTICULATED)) { // don't scale impulse for complex articulated entities
            pe_status_dynamics sd;
...
bool CEntity::IsObjectLoaded(unsigned int slot) {
    if (slot < m_objects.size()) {
        if (m_objects[slot].object != nullptr)
            return true;
    }
    return false;
}
...
void CEntity::OnAnimationEvent(const char* sAnimation, AnimSinkEventData UserData) {
    ...
    //  if(udUserData>0)
    m_pAnimationEventParams->SetTonullptr("number");
    m_pAnimationEventParams->SetTonullptr("userdata");
...
int
CEntity::GetBoneHitZone(int boneIdx) const {
...
void CEntity::GetMemoryStatistics(ICrySizer* pSizer) {
    size_t nSize = sizeof(CEntity) + sizeof_std_string(m_sClassName) + sizeof_std_string(m_name) + sizeof_std_string(m_sStateName);
