...
bool CEntity::LoadObject(unsigned int slot, const char* fileName, float scale, const char* geomName) {
    ...
    if ((slot > ENTITY_MAX_OBJECTS) || !fileName || (strlen(fileName) == 0)) {
        m_pISystem->Warning(VALIDATOR_MODULE_ENTITYSYSTEM, VALIDATOR_WARNING, 0, 0, "CEntity::LoadObject called with Empty filename, Entity: %s", m_name.c_str());
        return false;
    }
...
void CEntity::CalcWholeBBox() {
    ...
    if (m_physic && m_physic->GetType() == PE_SOFT) {
        ...
        Vec3d mins = m_vForceBBoxMin;
        mins *= m_fScale;
        Vec3d maxs = m_vForceBBoxMax;
        maxs *= m_fScale;
...
bool CEntity::CreateRigidBody(pe_type type, float density, float mass, int surface_id, Vec3d* pInitialVelocity, int slot, bool bPermanent) {
    ...
    } else if (m_objects[slot].object) {
        pe_geomparams partpos;
        partpos.pos = m_objects[slot].pos /*- m_center*/;
        partpos.q = GetRotationAA(m_objects[slot].angles.z * (gf_PI / 180.0f), vectorf(0, 0, 1)) * GetRotationAA(m_objects[slot].angles.y * (gf_PI / 180.0f), vectorf(0, 1, 0)) *
                    GetRotationAA(m_objects[slot].angles.x * (gf_PI / 180.0f), vectorf(1, 0, 0));
        if (density != 0)
            partpos.density = density;
        else
            partpos.mass = mass;
...
bool CEntity::PhysicalizeCharacter(int pos, float mass, int surface_idx, float stiffness_scale, bool bInstant) {
    ...
    if (!m_physic || m_physic->GetType() != PE_LIVING) {
        if (m_physPlaceholder) { // if the entity is physicalized for on-demand usage (via placeholder), rephysicalize it as permanent
    ...
    if (!physic || physic->GetType() == PE_LIVING) {
        int nAux = m_pCryCharInstance[pos]->CreateAuxilaryPhysics(m_pCryCharInstance[pos]->CreateCharacterPhysics(physic, mass, surface_idx, stiffness_scale));
        if (!physic) {
            pe_params_pos pp;
            pp.pos = m_center;
...
void CEntity::KillCharacter(int pos) {
    ...
    if (!m_physic || m_physic->GetType() == PE_LIVING) {
        ...
        if (m_physic) {
            ...
            do {
                ++ppart.ipart;
            } while (m_physic->SetParams(&ppart));
...
bool CEntity::IsAnimationPresent(int pos, const char* szAnimation) {
    ...
    return m_pCryCharInstance[pos] && m_pCryCharInstance[pos]->GetModel()->GetAnimationSet()->Find(szAnimation) >= 0;
}
...
bool CEntity::LoadVehicle(const char* objfile, pe_cargeomparams* pparts, pe_params_car* params, bool bDestroy) {
    ...
    if (!bDestroy) {
        if (HaveCamera()) {
            ...
            sCamParam.m_cam_dir(0, -3, 2);
            sCamParam.m_cam_dir.Normalize();
...
//////////////////////////////////////////////////////////////////////////////////
bool CEntity::CreateParticleEntity(float size, float mass, Vec3d velocity, float acc_thrust, float k_air_resistance, float acc_lift, float gravity, int surface_idx,
                                   bool bSingleContact) {
    // UnregisterInSector();
    DestroyPhysics();
    m_physicEnabled = true;
