...
CEntitySystem::CEntitySystem(ISystem* pSystem) {
    ...
    m_pCharacterIK = pSystem->GetIConsole()->CreateVariable(
            "p_characterik", "1", VF_CHEAT,
            "Toggles character IK.\n"
            "Usage: p_characterik [0/1]\n"
            "Default is 1 (on). Set to 0 to disable inverse kinematics."
    );
    m_pShowEntityBBox = pSystem->GetIConsole()->CreateVariable(
            "es_bboxes", "0", VF_CHEAT, // this console variable is repeated
            "Toggles entity bounding boxes.\n"
            "Usage: es_bboxes [0/1]\n"
            "Default is 0 (off). Set to 1 to display bounding boxes."
    );
    m_pShowHelpers = pSystem->GetIConsole()->CreateVariable(
            "es_helpers", "0", VF_CHEAT,
            "Toggles helpers.\n"
            "Usage: es_helpers [0/1]\n"
            "Default is 0 (off). Set to 1 to display entity helpers."
    );
    m_pProfileEntities = pSystem->GetIConsole()->CreateVariable(
            "es_profileentities", "0", VF_CHEAT,
            "\n"
            "Usage: \n"
            "Default is 0 (off)."
    );
    m_pUpdateInvisibleCharacter = pSystem->GetIConsole()->CreateVariable(
            "es_UpdateInvisibleCharacter", "0", VF_CHEAT,
            "\n"
            "Usage: \n"
            "Default is 0 (off)."
    );
    m_pUpdateBonePositions = pSystem->GetIConsole()->CreateVariable(
            "es_UpdateBonePositions", "1", VF_CHEAT,
            "\n"
            "Usage: \n"
            "Default is 1 (on)."
    );
    m_pUpdateScript = pSystem->GetIConsole()->CreateVariable(
            "es_UpdateScript", "1", VF_CHEAT,
            "\n"
            "Usage: \n"
            "Default is 1 (on)."
    );
    m_pUpdatePhysics = pSystem->GetIConsole()->CreateVariable(
            "es_UpdatePhysics", "1", VF_CHEAT,
    ...
    m_pUpdateAI = pSystem->GetIConsole()->CreateVariable(
            "es_UpdateAI", "1", VF_CHEAT,
            "Toggles updating of AI entities.\n"
            "Usage: es_UpdateAI [0/1]\n"
            "Default is 1 (on). Set to 0 to prevent AI entities from updating."
    );
    m_pUpdateEntities = pSystem->GetIConsole()->CreateVariable(
            "es_UpdateEntities", "1", VF_CHEAT,
            "Toggles entity updating.\n"
            "Usage: es_UpdateEntities [0/1]\n"
            "Default is 1 (on). Set to 0 to prevent all entities from updating."
    );
    m_pUpdateCollision = pSystem->GetIConsole()->CreateVariable(
            "es_UpdateCollision", "1", VF_CHEAT,
    ...
    m_pUpdateContainer = pSystem->GetIConsole()->CreateVariable(
            "es_UpdateContainer", "1", VF_CHEAT,
            "\n"
            "Usage: es_UpdateContainer [0/1]\n"
            "Default is 1 (on)."
    );
    m_pUpdateTimer = pSystem->GetIConsole()->CreateVariable(
            "es_UpdateTimer", "1", VF_CHEAT,
            "\n"
            "Usage: es_UpdateTimer [0/1]\n"
            "Default is 1 (on)."
    );
    m_pDebugTimer = pSystem->GetIConsole()->CreateVariable(
            "es_DebugTimer", "0", VF_CHEAT,
            "\n"
            "Usage: es_DebugTimer [0/1]\n"
            "Default is 0 (on)."
    );
    m_pUpdateCamera = pSystem->GetIConsole()->CreateVariable(
            "es_UpdateCamera", "1", VF_CHEAT,
            "Toggles camera updating.\n"
            "Usage: es_UpdateCamera [0/1]\n"
            "Default is 1 (on)."
    );
    m_pUpdateCollisionScript = pSystem->GetIConsole()->CreateVariable(
            "es_UpdateCollisionScript", "1", VF_CHEAT,
            "\n"
            "Usage: es_UpdateCollisionScript [0/1]\n"
            "Default is 1 (on)."
    );
    m_pUpdateCoocooEgg = pSystem->GetIConsole()->CreateVariable(
            "es_UpdateCoocooEgg", "1", VF_CHEAT,
            "\n"
            "Usage: es_UpdateCoocooEgg [0/1]\n"
            "Default is 1 (on)."
    );
    m_pPiercingCamera = pSystem->GetIConsole()->CreateVariable(
            "es_PiercingCamera", "0", VF_CHEAT,
            "\n"
            "Usage: es_PiercingCamera [0/1]\n"
            "Default is 0 (off)."
    );
    m_pVisCheckForUpdate = pSystem->GetIConsole()->CreateVariable(
            "es_VisCheckForUpdate", "1", VF_CHEAT,
            "\n"
            "Usage: es_VisCheckForUpdate [0/1]\n"
            "Default is 1 (on)."
    );
    m_pEntityBBoxes = pSystem->GetIConsole()->CreateVariable(
            "es_bboxes", "0", VF_CHEAT, // this console variable is repeated, which is the right one
            "\n"
            "Usage: es_bboxes [0/1]\n"
            "Default is 0 (off)."
    );
    m_pEntityHelpers = pSystem->GetIConsole()->CreateVariable(
            "es_helpers", "0", VF_CHEAT,
            "\n"
            "Usage: es_helpers [0/1]\n"
            "Default is 0 (off)."
    );
    m_pOnDemandPhysics = pSystem->GetIConsole()->CreateVariable(
            "es_OnDemandPhysics", "0", VF_CHEAT,
            "\n"
            "Usage: es_OnDemandPhysics [0/1]\n"
            "Default is 1 (on)."
    );
    m_pMinImpulseVel = pSystem->GetIConsole()->CreateVariable(
            "es_MinImpulseVel", "0.0", VF_CHEAT,
            "\n"
            "Usage: es_MinImpulseVel 0.0\n"
            ""
    );
    m_pImpulseScale = pSystem->GetIConsole()->CreateVariable(
            "es_ImpulseScale", "0.0", VF_CHEAT,
            "\n"
            "Usage: es_ImpulseScale 0.0\n"
            ""
    );
    m_pMaxImpulseAdjMass = pSystem->GetIConsole()->CreateVariable(
            "es_MaxImpulseAdjMass", "2000.0", VF_CHEAT,
            "\n"
            "Usage: es_MaxImpulseAdjMass 2000.0\n"
            ""
    );
    m_pSplashThreshold = pSystem->GetIConsole()->CreateVariable(
            "es_SplashThreshold", "1.0", VF_CHEAT,
            "minimum instantaneous water resistance that is detected as a splash"
            "Usage: es_SplashThreshold 200.0\n"
            ""
    );
    m_pSplashTimeout = pSystem->GetIConsole()->CreateVariable(
            "es_SplashTimeout", "3.0", VF_CHEAT,
            "minimum time interval between consecutive splashes"
            "Usage: es_SplashTimeout 3.0\n"
            ""
    );
    m_pHitCharacters = pSystem->GetIConsole()->CreateVariable("es_HitCharacters", "1", 0, "specifies whether alive characters are affected by bullet hits (0 or 1)");
    m_pHitDeadBodies = pSystem->GetIConsole()->CreateVariable("es_HitDeadBodies", "1", 0, "specifies whether dead bodies are affected by bullet hits (0 or 1)");
    m_pEnableCloth = pSystem->GetIConsole()->CreateVariable("es_EnableCloth", "1", VF_DUMPTODISK, "specifies whether cloth will be physicalized (0 or 1)");
    m_pCharZOffsetSpeed = pSystem->GetIConsole()->CreateVariable("es_CharZOffsetSpeed", "2.0", VF_DUMPTODISK, "sets the character Z-offset change speed (in m/s), used for IK");
    m_bClient = false;
    m_bServer = false;
...
IEntity* CEntitySystem::SpawnEntity(CEntityDesc& ed, bool bAutoInit) {
...

    {
        ...
        if (ed.id == 0) {
            ...
            } else if (m_EntityIDGenerator.IsUsed(ed.id)) {
    ...
    void* pDummy = 0;
    if (pEntity->GetContainer() &&
        (pEntity->GetContainer()->QueryContainerInterface(CIT_IPLAYER, (void**)&pDummy) || pEntity->GetContainer()->QueryContainerInterface(CIT_IVEHICLE, (void**)&pDummy)))
...
void CEntitySystem::UpdateEntity(CEntity* ce, SEntityUpdateContext& ctx) {
    if (ce->m_bGarbage) {
        if (ce->IsDestroyable()) {
            DeleteEntity(ce);
        } else { // [kirill] we must be in editor gamemode (Timur assures it wil newer happen in game)
...
IEntityCamera* CEntitySystem::CreateEntityCamera() {
    ...
    pNew->Init(m_pISystem->GetIPhysicalWorld(), m_pISystem->GetIRenderer()->GetWidth(), m_pISystem->GetIRenderer()->GetHeight(), m_pISystem->GetIConsole());
