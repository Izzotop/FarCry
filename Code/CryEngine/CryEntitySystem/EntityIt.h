...
public:
    ...
    bool IsEnd() {
        return (m_itEntityMap == m_pEntityMap->end());
    };
    IEntity* Next() {
        return IsEnd() ? nullptr : (IEntity*)(*m_itEntityMap++).second;
    };
    void MoveFirst() {
        m_itEntityMap = m_pEntityMap->begin();
    };
    void AddRef() {
        m_nRefCount++;
    }
...
public:
    ...
    bool IsEnd() {
        return (m_itEntityVec == m_pEntityVec->end());
    };
    IEntity* Next() {
        return IsEnd() ? nullptr : (*m_itEntityVec++);
    };
    void MoveFirst() {
        m_itEntityVec = m_pEntityVec->begin();
    };
    void AddRef() {
        m_nRefCount++;
    }
