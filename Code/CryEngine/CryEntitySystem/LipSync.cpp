...
bool CLipSync::LoadRandomExpressions(const char* pszExprScript, bool bRaiseError) {
    ...
    if (m_pScriptSystem->ExecuteFile(pszExprScript, true, false)) {
        ...
        _splitpath(pszExprScript, nullptr, nullptr, filename, nullptr);
        ...
        while ((nBytesRead < nBufferSize) && pBuffer && (pBuffer[0] != 0)) //! m_pPak->FEof(pFile))
        {
            ...
            if (pNewBuffer) {
                ...
                if (bReadHeader) {
                    ...
                    while (((pCheckBuffer = strnstr(pLastCheckBuffer, ",", pEndBuffer - pLastCheckBuffer)) != nullptr) &&
                           (pCheckBuffer < strnstr(pLastCheckBuffer, "\n", pEndBuffer - pLastCheckBuffer))) {
                        m_nLipSyncTracks++;
            ...
            while (((pScanBufferComma = strnstr(pLastScanBuffer, ",", pEndBuffer - pLastScanBuffer)) != nullptr) ||
                   ((pScanBufferBreak = strnstr(pLastScanBuffer, "\n", pEndBuffer - pLastScanBuffer)) != nullptr)) {
...
bool CLipSync::PlayDialog(bool bUnloadWhenDone) {
    ...
    if (m_pAITable != nullptr) {
        m_pSystem->GetILog()->Log("\002 Now adding a conversation timer at time %d for the duration of %d seconds", (unsigned long)(m_pTimer->GetCurrTime() * 1000),
                                  (unsigned long)(m_pSound->GetLengthMs()));
...
bool CLipSync::UpdateRandomExpressions(float fFrameTime, bool bAnimate) {
    ...
    for (int i = 0; i < (int)m_vecExprPatterns.size(); i++) {
        ...
        if (Expr.nMorphTargetId != -1) {
            ...
            //      TRACE("Start morph %s (rnd-expr) - Amp: %1.2f; In: %3.2f; Out: %3.2f; Ofs: %1.2f", Expr.sName.c_str(), MorphParams.fAmplitude, MorphParams.fBlendIn,
            // MorphParams.fBlendOut, MorphParams.fStartTime);
        }
...
bool CLipSync::UpdateLipSync(float fFrameTime, bool bAnimate) {
    ...
    for (int i = 0; i < m_nLipSyncTracks; i++) {
        ...
        if ((ThisData.nPat >= 0) && (ThisData.nPat < (int)m_vecPatterns.size())) {
            ...
            // TRACE("Start morph %s (lip-sync) - Amp: %1.2f; In: %3.2f; Out: %3.2f; Ofs: %1.2f", ThisPattern.sName.c_str(), MorphParams.fAmplitude, MorphParams.fBlendIn,
            // MorphParams.fBlendOut, MorphParams.fStartTime);
        }
