...
#define BEGIN_CHECK_STACK int __nStack = lua_stackspace(m_pLS);
...
void CScriptObject::SetTonullptrChain(const char* sKey) {
    lua_pushstring(m_pLS, sKey);
    lua_pushnil(m_pLS);
    SET_FUNCTION(m_pLS, -3);
}

void CScriptObject::SetTonullptr(const char* sKey) {
    _GUARD_STACK(m_pLS);
    if (!_GET_THIS())
        return;
    SetTonullptrChain(sKey);
}
...
void CScriptObject::SetnullptrAt(int nIdx) {
    _GUARD_STACK(m_pLS);
    if (!_GET_THIS())
        return;
...
#define FIXME_ASSERT(cond)                                                                                                                                                         \
    {                                                                                                                                                                              \
        if (!(cond)) {                                                                                                                                                             \
            CryError("<ScriptSystem> ASSERT %s", #cond);                                                                                                                           \
        }                                                                                                                                                                          \
    }
#endif
...
void CScriptObject::Dump(IScriptObjectDumpSink* p) {
    ...
    while (lua_next(m_pLS, trgTable) != 0) {
        ...
        if (lua_isnumber(m_pLS, -2)) {
            ...
            if (lua_isnil(m_pLS, -1)) {
                p->OnElementFound(nIdx, svtnullptr);
            ...
            if (lua_isnil(m_pLS, -1)) {
                p->OnElementFound(sName, svtnullptr);
            } else if (lua_isfunction(m_pLS, -1)) {
...
ScriptVarType CScriptObject::GetValueType(const char* sKey) {
    _GUARD_STACK(m_pLS);
    ScriptVarType svtRetVal = svtnullptr;
    if (!_GET_THIS())
        return svtnullptr;
