...
bool CStatObj::CheckValidVegetation() {
    if (m_pLeafBuffer && m_pLeafBuffer->m_pMats)
        for (int i = 0; i < m_pLeafBuffer->m_pMats->Count(); i++) {
            ...
            strlwr(buff);
            if (!strstr(buff, "templplants") && !strstr(buff, "nodraw") && !strstr(buff, "templdecal_vcolors") && !strstr(buff, "templdecalalphatest_vcolors")) {
#if !defined(LINUX) // don't worry, we won't render anything under linux
                Warning(0, m_szFileName, "CStatObj::CheckValidVegetation: Shader template is undefined or can not be used for vegetations: %s [%s]", pTemplateName, m_szFileName);
//      return false;
#endif
            }
        }
...
bool CStatObj::IsSphereOverlap(const Sphere& sSphere) {
    if (m_pLeafBuffer && Overlap::Sphere_AABB(sSphere, AABB(m_vBoxMin, m_vBoxMax))) { // if inside bbox
