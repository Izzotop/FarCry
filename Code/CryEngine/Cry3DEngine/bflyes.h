...
public:
    Cry_Butterfly();
    void Render(ITimer* pITimer, IRenderer* pIRenderer, const Vec3d& vCamPos, const bool& bEven, const Vec3d& vColor, struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F* pVertBufChunk,
                CTerrain* pTerrain);
...
public:
    ...
    void SetCount(int nCount) {
        m_nCurrentObjectsCount = min(nCount, MAX_BF_COUNT);
    }
    int GetCount() {
        return m_nCurrentObjectsCount;
    }
    int GetMemoryUsage() {
        return sizeof(Cry_Butterfly) * MAX_BF_COUNT + sizeof(ushort) * MAX_BF_COUNT;
    }
};
