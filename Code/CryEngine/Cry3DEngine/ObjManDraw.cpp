...
bool CObjManager::IsBoxOccluded(const Vec3d& vBoxMin, const Vec3d& vBoxMax, float fDistance, OcclusionTestClient* pOcclTestVars) {
    ...
    if (pOcclTestVars->nLastVisibleFrameID < GetFrameID() - 1) { // if was invisible last frames (because of frustum culling)
    ...
    { // test occlusion by objects
        if (!pOcclTestVars->ucOcclusionByObjectsFrames && fDistance > COVERAGEBUFFER_OCCLUSION_TESTERS_MIN_DISTANCE &&
            GetCVars()->e_cbuffer) { // less distance do not work because objects are sorted wrong
    ...
    */
    { // test occlusion by terrain
        ...
        { // randomize test time, do this only here otherwice ucOcclusionByTerrainFrames may never become 0 and next test will not happend
            static int arrRnd[16] = {rand() % 4 + 1, rand() % 4 + 1, rand() % 4 + 1, rand() % 4 + 1, rand() % 4 + 1, rand() % 4 + 1, rand() % 4 + 1, rand() % 4 + 1,
