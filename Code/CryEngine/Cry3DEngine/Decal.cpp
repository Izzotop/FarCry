...
void CDecal::Process(bool& active, IRenderer* pIRenderer, const float fCurTime, C3DEngine* p3DEngine, IShader* pShader, CCamera* pCamera, float fSortOffset) {
    ...
    if (m_pDecalOwner) {
        ...
        if (m_pBigDecalLeafBuffer && m_pBigDecalLeafBuffer->m_Indices.m_nItems) {
            ...
            float D0 = m_arrBigDecalCustomData[0] * m_vPos.x + m_arrBigDecalCustomData[1] * m_vPos.y + m_arrBigDecalCustomData[2] * m_vPos.z;
            ...
            float D1 = m_arrBigDecalCustomData[4] * m_vPos.x + m_arrBigDecalCustomData[5] * m_vPos.y + m_arrBigDecalCustomData[6] * m_vPos.z;
            ...
            p3DEngine->GetObjManager()->AddPolygonToRenderer(
                m_nTexId, pShader, nDynLightMask, vRight * fSizeK, vUp * fSizeK, uCol, ParticleBlendType_AlphaBased, m_vAmbient, vPos,
                0, 0, 0, 0, fSortOffset, 0, 0,
                m_pDecalOwner->GetEntityRenderType() == eERType_Vegetation ? (CStatObjInst*)m_pDecalOwner : nullptr
            );
        }
    } else if (!m_bOnTheGround) { // draw small world space decal untransformed
        CDLight* pStrongestLightForTranspGeom = nullptr;
        ...
        if (pStrongestLightForTranspGeom && pStrongestLightForTranspGeom->m_fRadius) {
            ...
            uCol.dcolor = 0;
        uCol.bcolor[3] = fastftol_positive(fAlpha * 255);
        p3DEngine->GetObjManager()->AddPolygonToRenderer(m_nTexId, pShader, nDynLightMask, m_vRight * m_fSize * fSizeK, m_vUp * m_fSize * fSizeK, uCol,
                                                         ParticleBlendType_AlphaBased, m_vAmbient, m_vPos, nullptr, 0, nullptr, 0, fSortOffset);
    }
