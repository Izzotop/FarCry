...
class CDetailGrass : public Cry3DEngineBase {
    ...
    void CreateSectorGrassInUnit(const int x, const int y, const int nStep, struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F* pGrassVertices);
    void AddIndexedArray(GrassType* o, float X, float Y, float Z, float fbr, float fSizeRatio, float fXSign, float fYSign, int nSwapXY, struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F* pGrassVertices);
...
public:
    ...
    void UpdateGrass() {
        m_GrassFocusX = m_GrassFocusY = -CTerrain::GetTerrainSize();
    }
