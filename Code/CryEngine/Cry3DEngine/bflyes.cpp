...
void Cry_Butterfly::Render(ITimer* pITimer, IRenderer* pIRenderer, const Vec3d& vCamPos, const bool& bEven, const Vec3d& vColor,
                           struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F* pVertBufChunk, CTerrain* pTerrain) {
        ...
        m_vPos.z < (pTerrain->GetWaterLevel() + 82)) // !IsPointInFuncZone(m_vPos.x,m_vPos.y))
    {
        ...
        float x1 = 0 + m_vCurWingPos.x, x2 = 0.125f + m_vCurWingPos.x, x3 = 0.25f + m_vCurWingPos.x;
        float y1 = 1.f - (0 + m_vCurWingPos.y), y2 = 1.f - (0.125f + m_vCurWingPos.y), y3 = 1.f - (0.25f + m_vCurWingPos.y);

        struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F data[6] = {
            { -c * R, -R, R * s, r, g, b, 255, x1, y 3}, { -c * R, R, R * s, r, g, b, 255, x1, y1 },
            { 0, -R, -R * s / 3, r, g, b, 255, x2, y 3}, { 0, R, -R * s / 3, r, g, b, 255, x2, y1 },
            { c * R, -R, R * s, r, g, b, 255, x3, y3 },  { c * R, R, R * s, r, g, b, 255, x3, y1 }
        };
...
//    }
    ...
    Vec3d vColor = GetSystem()->GetI3DEngine()->GetWorldColor(); // GetSystem()->GetI3DEngine()->GetWorldBrightnes();
