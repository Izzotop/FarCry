...
bool CBrush::DrawEntity(const struct SRendParams& _EntDrawParams) {
    ...
    if (m_dwRndFlags & ERF_USELIGHTMAPS && HasLightmap(nLod) && GetCVars()->e_light_maps && _EntDrawParams.nShaderTemplate != EFT_INVLIGHT &&
        ...
        if (GetCVars()->e_light_maps_occlusion)
            if (m_arrOcclusionLightOwners[0] || m_arrOcclusionLightOwners[1] || m_arrOcclusionLightOwners[2] || m_arrOcclusionLightOwners[3]) {
    ...
    if (rParms.dwFObjFlags & FOB_RENDER_INTO_SHADOWMAP) { // modify position
...
void CBrush::Physicalize(bool bInstant) {
    ...
    if (fabs(fScaleX - fScaleY) > 0.01f || fabs(fScaleX - fScaleZ) > 0.01f || !pBody->IsPhysicsExist()) { // scip non uniform scaled object and object without physics
        Dephysicalize();
        return;
    }
...
void CObjManager::MergeBrushes() {
    ...
    for (i = 0; i < m_lstBrushContainer.Count(); i++) {
        ...
        for (int m = 0; m < pLB->m_pMats->Count(); m++) {
            ...
            for (j = 0; j < groupBrushes[nMergeId].Num(); j++) {
                ...
                if (shm == mg->sh.m_pShader && newMatInfo.shaderItem.m_pShaderResources == mg->sh.m_pShaderResources && nLMTexId == mg->nLMId)
                    break;
            }
    ...
    while (nEntityId < m_lstBrushContainer.Count() || nNextStartId >= 0) {
        ...
        for (; nEntityId < m_lstBrushContainer.Count(); nEntityId++) {
            ...
            if (pBrush->GetMergeId() <= 0)
                continue;
            if (pBrush->m_dwRndFlags & ERF_MERGED || pBrush->m_dwRndFlags & ERF_CASTSHADOWVOLUME || pBrush->m_dwRndFlags & ERF_CASTSHADOWMAPS)
                continue;
        ...
        // sort
        if (lstChunks.Count())
            qsort(lstChunks.GetElements(), lstChunks.Count(), sizeof(lstChunks[0]), CBrush__Cmp_MatChunks);

        // merge chunks
        for (int nChunk = 0; nChunk < lstChunks.Count(); nChunk++) {
            if (!nChunk || CBrush__Cmp_MatChunks(&lstChunks[nChunk], &lstChunks[nChunk - 1])) { // not equal materials - add new chunk
        ...
        // make leaf buffer
        CLeafBuffer* pNewLB = GetRenderer()->CreateLeafBufferInitialized(lstVerts.GetElements(), lstVerts.Count(), VERTEX_FORMAT_P3F_N_COL4UB_TEX2F, lstIndices.GetElements(),
                                                                         lstIndices.Count(), R_PRIMV_TRIANGLES, "MergedLB", eBT_Static, lstChunks.Count());

        pNewLB->UpdateTangBuffer(lstTangBasises.GetElements());

        for (int i = 0; i < lstChunks.Count(); i++) {
            pNewLB->SetChunk(lstChunks[i].GetShaderItem().m_pShader, lstChunks[i].nFirstVertId, lstChunks[i].nNumVerts, lstChunks[i].nFirstIndexId, lstChunks[i].nNumIndices, i,
                             true);
    ...
    GetLog()->UpdateLoadingScreen("\003---%d additional brushes, %d new brushes: %d old brushes (%.3f perc)", nNewBrushes, nMergedBrushes, nOldBrushes,
                                  (float)nMergedBrushes / (float)nOldBrushes * 100.0f);
}
...
void CObjManager::LoadBrushes() {
    ...
    if ((strncmp(header.signature, BRUSH_FILE_SIGNATURE, 3) != 0) || (header.filetype != BRUSH_FILE_TYPE) || (header.version != BRUSH_FILE_VERSION)) {
        // Not Cry file.
    ...
    if (nCount < 0 || nCount > 100000) {
        GetSystem()->Warning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR, 0, szName, "CObjManager::LoadBrushes: invalid brush.lst file, Reexport the level from Editor again");
        return;
    }
    ...
    for (int i = 0; i < lstSExportedBrush.Count(); i++) {
        ...
        if (!CBrush::IsMatrixValid(mat44)) {
            Warning(0, szName, "CObjManager::LoadBrushes: Invalid transformation matrix specified for object: %s", lstSExportedBrushGeom[lstSExportedBrush[i].geometry].filename);
            continue;
        }
        ...
        IStatObj* pStatObj =
            MakeObject(szFileName, nullptr, (lstSExportedBrush[i].flags & ERF_USELIGHTMAPS) ? evs_NoSharing : evs_ShareAndSortForCache, true, false, GetCVars()->e_stream_cgf == 1);

        // Find or allocate slot containing pointer to this StatObj
        int nBrushGeomId = CBrush::m_lstBrushTypes.Find(pStatObj);
        if (nBrushGeomId < 0) { // make new slot
    ...
    GetLog()->UpdateLoadingScreen(
        "\003Brushes loaded: %d models, %d instances, %d custom materials",
        lstSExportedBrushGeom.Count(),
        lstSExportedBrush.Count(),
        CBrush::m_lstSExportedBrushMaterials.Count()
    );
...
bool CBrush::IsMatrixValid(const Matrix44& mat) {
    ...
    if (vScaleTest.Length() > 100.f || vScaleTest.Length() < 0.01f || fDist > 64000 || !_finite(vScaleTest.x) || !_finite(vScaleTest.y) || !_finite(vScaleTest.z)) {
        return false;
    }
...
float CBrush::GetMaxViewDist() {
    return max(GetCVars()->e_obj_min_view_dist, m_fWSRadius * GetCVars()->e_obj_view_dist_ratio * GetViewDistRatioNormilized());
}
...
void CBrush::Serialize(bool bSave, ICryPak* pPak, FILE* f) {
    ...
    if (bSave) {
        ...
        if (m_nMaterialId >= 0) { // restore material
        ...
        if (nSize) {
            ...
            // Make leafbuffer and fill it with texture coordinates
            m_pLMTCBuffer = GetRenderer()->CreateLeafBufferInitialized(pData, nVertCount, VERTEX_FORMAT_P3F, 0, 0, R_PRIMV_TRIANGLES, "LMapTexCoordsStreamed", eBT_Static);
