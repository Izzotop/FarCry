...
void C3DEngine::Draw() {
    ...
    if (GetSystem()->IsDedicated())
        ...
        if (*((bool*)m_pREScreenProcess->mfGetParameter(SCREENPROCESS_NIGHTVISION, SCREENPROCESS_ACTIVE))) {
            ...
            int e_shadow_maps = GetCVars()->e_shadow_maps;
            GetCVars()->e_shadow_maps = 0;
            int e_stencil_shadows = GetCVars()->e_stencil_shadows;
            GetCVars()->e_stencil_shadows = 0;
            int r_VolumetricFog = pCVVolFog->GetIVal();
            pCVVolFog->Set(0);
...
#pragma warning(pop) // AMD Port
    ...
    if (IsOutdoorVisible()) {
        if (m_pVisAreaManager->m_lstOutdoorPortalCameras.Count() && (m_pVisAreaManager->m_pCurArea || m_pVisAreaManager->m_pCurPortal))
        ...
        if (m_pVisAreaManager->IsOutdoorAreasVisible() && (dwDrawFlags & DLD_TERRAIN_WATER) && m_pTerrain->m_fDistanceToSectorWithWater >= 0 && m_pTerrain->GetWaterLevel() &&
            m_pTerrain->m_bOceanIsVisibe && m_bOcean) {
        ...
        if ((dwDrawFlags & DLD_TERRAIN_LIGHT) && m_pTerrain && m_pVisAreaManager->IsOutdoorAreasVisible()) { // render terrain light passes
    ...
    IRenderer* ppp = GetRenderer();
    {
        ...
        if (!m_nRenderStackLevel) {
            if (GetCVars()->e_widescreen) { // set view port to 16/9 mode
...
void C3DEngine::DisplayInfo(float& fTextPosX, float& fTextPosY, float& fTextStepY) {
    ...
    { // current fps
        ...
        if (fFrameRateMin > fFrameRate)
            fFrameRateMin = fFrameRate;
        if (fFrameRateMax < fFrameRate)
    ...
    *szLevelName = 0;
    ...
    DrawText(fTextPosX, fTextPosY += fTextStepY, "CamPos=%3d %3d %3d Angl=%3d %2d %3d", (int)vPos.x, (int)vPos.y, (int)vPos.z, (int)GetViewCamera().GetAngles().x,
             (int)GetViewCamera().GetAngles().y, (int)GetViewCamera().GetAngles().z);
    ...
    if (GetCVars()->e_timedemo_frames && nFrames >= 0) { // print current and average fps
        if (fStartTime != GetTimer()->gfGetTime())
            DrawText(fTextPosX, fTextPosY += fTextStepY, "FPS %3d (%3d..%3d) / %3.2f / %3.2f", (int)fCurrentFPS, (int)fFrameRateSecMin, (int)fFrameRateSecMax, fFrameRate,
                     float(nFrames) / (GetTimer()->gfGetTime() - fStartTime));
    } else { // print current fps
        DrawText(fTextPosX, fTextPosY += fTextStepY, "FPS %3d (%3d..%3d) / %5.1f", (int)fCurrentFPS, (int)fFrameRateSecMin, (int)fFrameRateSecMax, fFrameRate);
    }
    ...
    if (GetCVars()->e_timedemo_milliseconds) {
        if (int((GetTimer()->gfGetTime() - fStartTime) * 1000) > GetCVars()->e_timedemo_milliseconds) {
            GetLog()->Log("\001Timedemo quit in %dms (started at %g sec, stopped at %g sec) Average %3.2f FPS", GetCVars()->e_timedemo_milliseconds, fStartTime,
                          GetTimer()->gfGetTime(), float(nFrames - nStartFrame) / (GetTimer()->gfGetTime() - fStartTime));
            GetSystem()->Quit();
        }
    }
    ...
    if (GetCVars()->e_particles_debug) {
        ...
        DrawText(fTextPosX, fTextPosY += fTextStepY, "Parts=%4d/%4d  Emit=%d  Sprites=%4d", nParts, nFree, nEmitters, nSprites);
    }
...
#pragma warning(pop) // AMD Port
    ...
    else if (p_r_SM2XPATH->GetIVal())
        pRenderPath = "SM2.X path";
    else if (p_r_Quality_BumpMapping->GetIVal() == 3)
        pRenderPath = "SM2.0 Path";
    else if (p_r_Quality_BumpMapping->GetIVal() > 0)
...
#define DLD_RESTORE_CVAR(VarName) GetCVars()->VarName = VarName;
...
void C3DEngine::SetupDistanceFog() {
    ...
    if (GetCVars()->e_use_global_fog_in_fog_volumes && !m_nRenderStackLevel) { // find camera fog volume
        CVisArea* pCurVisArea = m_pVisAreaManager->m_pCurArea ? m_pVisAreaManager->m_pCurArea : m_pVisAreaManager->m_pCurPortal;
        if (pCurVisArea) { // find indoor fog volume with this id
    ...
    GetRenderer()->SetFog(0, pFogVolume ? pFogVolume->fMaxViewDist * 0.01f : m_fFogNearDist, pFogVolume ? pFogVolume->fMaxViewDist : m_fFogFarDist,
                          GetRenderer()->EF_GetHeatVision() ? Vec3d(0, 0, 0)
                          : pFogVolume                      ? pFogVolume->vColor
                                                            : m_vFogColor,
                          R_FOGMODE_LINEAR);
...
void C3DEngine::RenderVolumeFogTopPlane() {
    ...
    if (!m_pFogTopPlane) {
        ...
        m_pFogTopPlane = GetRenderer()->CreateLeafBufferInitialized(lstVertices.GetElements(), lstVertices.Count(), VERTEX_FORMAT_P3F, lstIndices.GetElements(), lstIndices.Count(),
                                                                    R_PRIMV_MULTI_STRIPS, "VolFogTopCircle", eBT_Dynamic, nChunksNum, 0x1000);

        for (int i = 0; i < nChunksNum; i++) {
            m_pFogTopPlane->SetChunk(pFogVolume->pShader, lstIndices[lstFirstIdxId[i]], lstVertices.Count() / (lstFirstIdxId.Count() - 1), lstFirstIdxId[i],
                                     lstFirstIdxId[i + 1] - lstFirstIdxId[i], i);
        }
    }
...
void C3DEngine::MakeHiResScreenShot() {
    ...
    if (nHiResShootCounter < 0) { // start loop
        lstSubImages.Clear();
        ...
        while (1) {
            char sFileName[256];
            snprintf(sFileName, sizeof(sFileName), "HiResScreenShoots\\%dx%d_%d.tga", nFileId, GetRenderer()->GetWidth() * GetCVars()->e_hires_screenshoot,
...
// IVO: I have a special solution for HiResScreenShoots on GameCube!
        ...
        GetRenderer()->ReadFrameBuffer(lstSubImages.Last(), GetRenderer()->GetWidth(), GetRenderer()->GetHeight(), false, true);
#endif
    }

    nHiResShootCounter--;

    if (nHiResShootCounter < 0) { // end loop
        ...
        int nFinSize = lstSubImages.Count() * GetRenderer()->GetWidth() * GetRenderer()->GetHeight() * 4;
        ...
        if (pFinalImage) {
            ...
            GetRenderer()->WriteTGA(pFinalImage, GetRenderer()->GetWidth() * GetCVars()->e_hires_screenshoot, GetRenderer()->GetHeight() * GetCVars()->e_hires_screenshoot,
                                    sFileName, 32);
        ...
        GetRenderer()->SetViewport(-nHiResShootCounter % GetCVars()->e_hires_screenshoot * GetRenderer()->GetWidth(),
                                   -nHiResShootCounter / GetCVars()->e_hires_screenshoot * GetRenderer()->GetHeight(), GetRenderer()->GetWidth() * GetCVars()->e_hires_screenshoot,
                                   GetRenderer()->GetHeight() * GetCVars()->e_hires_screenshoot);
    }
#endif
}
...
void C3DEngine::DrawShadowSpotOnTerrain(Vec3d vPos, float fRadius) {
    ...
    if (y1 < 0)
        y1 = 0;
    if (x2 >= CTerrain::GetTerrainSize())
        x2 = CTerrain::GetTerrainSize();
    if (y2 >= CTerrain::GetTerrainSize())
        y2 = CTerrain::GetTerrainSize();
    ...
    float fAlpha = (1.f - (m_pObjManager->m_vOutdoorAmbientColor.x + m_pObjManager->m_vOutdoorAmbientColor.y + m_pObjManager->m_vOutdoorAmbientColor.z) * 0.3333f * 0.5f);
    ...
    if (bCameraInOutdoors)
        if (GetViewCamera().GetPos().z < GetWaterLevel() && m_pTerrain) {
            ...
            if (!pSectorInfo || !pSectorInfo->m_pFogVolume || GetViewCamera().GetPos().z > pSectorInfo->m_pFogVolume->vBoxMax.z) {
                if (GetViewCamera().GetPos().z < GetWaterLevel() && m_pTerrain && m_pTerrain->m_lstFogVolumes.Count() && m_pTerrain->m_lstFogVolumes[0].bOcean)
