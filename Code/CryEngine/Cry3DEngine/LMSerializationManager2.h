...
public:
    ...
    template <typename T> void Write(const T* pT, unsigned numElements) {
...
public:
    ...
    virtual void AddRawLMData(const DWORD indwWidth, const DWORD indwHeight, const std::vector<int>& _cGLM_IDs_UsingPatch, BYTE* _pColorLerp4, BYTE* _pHDRColorLerp4,
                              BYTE* _pDomDirection3, BYTE* _pOccl2 = 0);
    virtual void AddTexCoordData(const std::vector<TexCoord2Comp>& vTexCoords, int iGLM_ID_UsingTexCoord, const DWORD indwHashValue,
    ...
    virtual RenderLMData* CreateLightmap(const char* pszFileName, int nItem, UINT iWidth, UINT iHeight, BYTE* pColorLerp4, BYTE* pHDRColorLerp4, BYTE* pDomDirection3,
                                         BYTE* pOccl2 = 0);
    // ----------------------------------------------
...
protected:
    ...
    struct RawLMData {
        ...
        enum BitmapEnum { TEX_COLOR, TEX_DOMDIR, TEX_OCCL, TEX_HDR };
    ...
    private:
        ...
        RawLMData& operator=(const RawLMData& a) {
            return (*this);
        }
    };
