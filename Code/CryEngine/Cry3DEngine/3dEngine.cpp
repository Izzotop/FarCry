...
bool C3DEngine::IsCameraAnd3DEngineInvalid(const CCamera cam, const char* szCaller) {
    ...
    if (m_fMaxViewDist <= 0 || m_fFogFarDist <= 0 || !_finite(vCamPos.x) || !_finite(vCamPos.y) || !_finite(vCamPos.z) || !_finite(vAngles.x) || !_finite(vAngles.y) ||
        !_finite(vAngles.z) || IsEquivalent(vCamPos, Vec3d(0, 0, 0), VEC_EPSILON) || vCamPos.z < -GetTerrainSize() * 4 || vCamPos.z > GetTerrainSize() * 4 || fFov < 0.025f ||
        fFov > gf_PI) {
        Warning(0, 0, "%s: Camera undefined : Pos=(%.1f, %.1f, %.1f), Rot=(%.1f, %.1f, %.1f), Fov=%.1f, MaxViewDist=%.1f, FogFarDist=%.1f", szCaller, vCamPos.x, vCamPos.y,
                vCamPos.z, vAngles.x, vAngles.y, vAngles.z, fFov, m_fMaxViewDist, m_fFogFarDist);
        return true;
    }
...
void C3DEngine::SetCamera(const CCamera& cam, bool bToTheScreen) {
    ...
    */
    ...
    GetViewCamera().Init(GetRenderer()->GetWidth(), GetRenderer()->GetHeight(), GetViewCamera().GetFov(), m_fMaxViewDist, 0, GetViewCamera().GetZMin());
...
IStatObj* C3DEngine::MakeObject(const char* szFileName, const char* szGeomName, EVertsSharing eVertsSharing, bool bLoadAdditinalInfo, bool bKeepInLocalSpace) {
    if (!szFileName || !szFileName[0]) {
        Warning(0, nullptr, "I3DEngine::MakeObject: filename is not specified");
        return nullptr;
    }
...
void C3DEngine::RegisterEntity(IEntityRender* pEntityRS) {
    ...
    if (!szName[0] && !szClass[0])
        Warning(0, nullptr, "C3DEngine::RegisterEntity: Entity undefined"); // do not register undefined objects

    if (!_finite(pEntityRS->GetPos().x) || !_finite(pEntityRS->GetPos().y) || !_finite(pEntityRS->GetPos().z)) {
        Warning(0, nullptr, "Warning: C3DEngine::RegisterEntity: Entity position undefined: %s", szName);
        return;
    }
...
bool C3DEngine::UnRegisterEntity(IEntityRender* pEntityRS) {
    ...
    if ((m_pTerrain && m_pTerrain->UnRegisterInAllSectors(pEntityRS)) || (m_pVisAreaManager && m_pVisAreaManager->UnRegisterInAllSectors(pEntityRS))) {
        const char* szName = pEntityRS->GetName();
...
void C3DEngine::CreateDecal(const CryEngineDecalInfo& DecalInfo) {
    ...
    if (GetCVars()->e_decals == 2)
        GetLog()->Log("Debug: C3DEngine::CreateDecal: Pos=(%.1f,%.1f,%.1f) Size=%.2f ObjName=%s nPartID=%d", DecalInfo.vPos.x, DecalInfo.vPos.y, DecalInfo.vPos.z, DecalInfo.fSize,
                      DecalInfo.pDecalOwner ? DecalInfo.pDecalOwner->GetName() : "nullptr", DecalInfo.nPartID);
    ...
    if (m_pDecalManager) {
        ...
        if (DecalInfo.pDecalOwner) { // try to find at least one marked to be visible static component
            ...
            while (!pEntObject && nPartID < 16) { // find first visible entity component
...

    {
        ...
        if (pSecInfo) {
            ...
            if (pSecInfo) {
                ...
                Vec3d vSecBoxMin((float)pSecInfo->m_nOriginX, (float)pSecInfo->m_nOriginY, 0);
                Vec3d vSecBoxMax((float)pSecInfo->m_nOriginX + CTerrain::GetSectorSize(), (float)pSecInfo->m_nOriginY + CTerrain::GetSectorSize(), 512);
                ...
                if (min_x < 0)
                    min_x = 0;
                else if (min_x >= CTerrain::GetSectorsTableSize())
                    min_x = CTerrain::GetSectorsTableSize() - 1;
                if (min_y < 0)
                    min_y = 0;
                else if (min_y >= CTerrain::GetSectorsTableSize())
                    min_y = CTerrain::GetSectorsTableSize() - 1;
                if (max_x < 0)
                    max_x = 0;
                else if (max_x >= CTerrain::GetSectorsTableSize())
                    max_x = CTerrain::GetSectorsTableSize() - 1;
                if (max_y < 0)
                    max_y = 0;
                else if (max_y >= CTerrain::GetSectorsTableSize())
                        ...
                        if (!(pEnt->GetRndFlags() & ERF_HIDDEN))
                            if (pEnt->GetRndFlags() & ERF_CASTSHADOWINTOLIGHTMAP)
                                if (!pEnt->GetEntityVisArea()) {
                                    ...
                                    if (Overlap::AABB_AABB(AABB(vEntBoxMin, vEntBoxMax), AABB(vSecBoxMin - vRadius, vSecBoxMax + vRadius)) && lstAllEntities.Find(pEnt) < 0)
...
float C3DEngine::GetDistanceToSectorWithWater() {
    ...
    return (pSectorInfo && (m_pTerrain->m_fDistanceToSectorWithWater > 0.1f)) ? m_pTerrain->m_fDistanceToSectorWithWater : max(GetViewCamera().GetPos().z - GetWaterLevel(), 0.1f);
}
...
bool C3DEngine::IsBoxVisibleOnTheScreen(const Vec3d& vBoxMin, const Vec3d& vBoxMax, OcclusionTestClient* pOcclusionTestClient) {
    ...
    if (min_x >= 0 && max_x < CTerrain::GetSectorsTableSize() ||
        min_y >= 0 && max_y < CTerrain::GetSectorsTableSize()) { // if some sector was visible in last 5 sec - box is visible
        for (int x = min_x; x <= max_x; x++)
            for (int y = min_y; y <= max_y; y++) { // if some sector was visible in last 5 sec - box is visible
                if (!GetCVars()->e_terrain || (GetTimer()->GetCurrTime() - m_pTerrain->m_arrSecInfoTable[x][y]->GetLastTimeUsed()) < 3.f) {
                    if (pOcclusionTestClient) { // test terrain and objects (cbuffer) occlusion
...
bool C3DEngine::IsSphereVisibleOnTheScreen(const Vec3d& vPos, const float fRadius, OcclusionTestClient* pOcclusionTestClient) {
    return IsBoxVisibleOnTheScreen(vPos - Vec3d(fRadius, fRadius, fRadius), vPos + Vec3d(fRadius, fRadius, fRadius), pOcclusionTestClient);
}
...
IVisArea* C3DEngine::CreateVisArea() {
    return m_pObjManager ? m_pVisAreaManager->CreateVisArea() : nullptr;
}

void C3DEngine::DeleteVisArea(IVisArea* pVisArea) {
    if (!m_pVisAreaManager->IsValidVisAreaPointer((CVisArea*)pVisArea)) {
        Warning(0, nullptr, "I3DEngine::DeleteVisArea: Invalid VisArea pointer");
        return;
    }
    if (m_pObjManager) {
...
void C3DEngine::UpdateVisArea(IVisArea* pVisArea, const Vec3d* pPoints, int nCount, const char* szName, float fHeight, const Vec3d& vAmbientColor, bool bAfectedByOutLights,
                              bool bSkyOnly, const Vec3& vDynAmbientColor, float fViewDistRatio, bool bDoubleSide, bool bUseDeepness, bool bUseInIndoors) {
    if (!m_pObjManager) {
        return;
    }
    ...
    m_pVisAreaManager->UpdateVisArea((CVisArea*)pVisArea, pPoints, nCount, szName, fHeight, vAmbientColor, bAfectedByOutLights, bSkyOnly, m_pTerrain, vDynAmbientColor,
                                     fViewDistRatio, bDoubleSide, bUseDeepness, bUseInIndoors);
...
float C3DEngine::GetLightAmountForEntity(IEntityRender* pEntity, bool bOnlyVisibleLights) {
    ...
    CheckDistancesToLightSources(dwDLightMask, pEntity->GetPos(), 1, pEntity, 16, nullptr, 0, &vLightAmount);
...
bool C3DEngine::IsVisAreasConnected(IVisArea* pArea1, IVisArea* pArea2, int nMaxRecursion, bool bSkipDisabledPortals) {
    if (pArea1 == pArea2)
        return true; // includes the case when both pArea1 & pArea2 are nullptr (totally outside)
...
typedef void (*voidfunctionptr)(); // ptr to function returning void
...
CRY_GCRENDER_API void _unresolved() {
    u32 i;
    u32* p;
    ...
    for (i = 0, p = (u32*)OSGetStackPointer();                // get current sp
         p && (u32)p != 0xffffffff && i++ < 16; p = (u32*)*p) // get caller sp
    {
...
IMatInfo* C3DEngine::CreateMatInfo() {
    return m_pMatMan ? m_pMatMan->CreateMatInfo() : nullptr;
}
...
bool C3DEngine::IsOutdoorsVisible() {
    return (m_pObjManager && m_pVisAreaManager) ? m_pVisAreaManager->IsOutdoorAreasVisible() : false;
}

bool C3DEngine::IsTerainHightMapModifiedByGame() {
    return m_pTerrain ? m_pTerrain->m_bHightMapModified : false;
}
...
bool C3DEngine::IsPotentiallyVisible(IEntityRender* pEntityRender, float fAdditionalRadius) {
    ...
    if (pVisArea) { // object is in indoors - test vis of current visarea
        if ((GetFrameID() - pVisArea->GetVisFrameId()) < 2)
            return true; // visible

        if (fAdditionalRadius > 0) { // test neighbours
            IVisArea* Areas[64];
    ...
    } else if (m_pTerrain) { // object is in outdoors - test vis of terrain sectors
...
void C3DEngine::CheckPhysicalized(const Vec3d& vBoxMin, const Vec3d& vBoxMax) {
    ...
    if (pVisArea) { // indoor
        ...
        CSectorInfo* arrSecInfo[] = {m_pTerrain->GetSecInfo(int(vBoxMin.x), int(vBoxMin.y)), m_pTerrain->GetSecInfo(int(vBoxMax.x), int(vBoxMin.y)),
                                     m_pTerrain->GetSecInfo(int(vBoxMin.x), int(vBoxMax.y)), m_pTerrain->GetSecInfo(int(vBoxMax.x), int(vBoxMax.y))};
...
bool C3DEngine::SetMaterialFloat(char* szMatName, int nSubMatId, int nTexSlot, char* szParamName, float fValue) {
    ...
    if (!pMatInfo) {
        Warning(0, nullptr, "I3DEngine::SetMaterialFloat: Material not found: %s", szMatName);
        return false;
    }
    ...
    if (nSubMatId > 0) {
        if (nSubMatId <= pMatInfo->GetSubMtlCount())
            ...
            Warning(0, nullptr, "I3DEngine::SetMaterialFloat: SubMaterial not found: %s, SubMatId: %d", szMatName, nSubMatId);
            return false;
        }
    }
    ...
    if (!pTex) {
        Warning(0, nullptr, "I3DEngine::SetMaterialFloat: Texture slot not found: %s, TexSlot: %d", szMatName, nTexSlot);
        return false;
    }
...
void IEntityRenderState::ShadowMapInfo::Release(enum EERType eEntType, struct IRenderer* pRenderer) {
    ...
    if (pShadowMapFrustumContainer && eEntType != eERType_Vegetation) // vegetations share same frustum allocated in CStatObj
    {
        ...
        pShadowMapFrustumContainer->m_LightFrustums.Get(0)->pEntityList = nullptr;

        delete pShadowMapFrustumContainer->m_LightFrustums.Get(0)->pModelsList;
        pShadowMapFrustumContainer->m_LightFrustums.Get(0)->pModelsList = nullptr;

        delete pShadowMapFrustumContainer;
        pShadowMapFrustumContainer = nullptr;
    }
    ...
    if (pShadowMapFrustumContainerPassiveCasters && eEntType != eERType_Vegetation) // vegetations share same frustum allocated in CStatObj
    {
        ...
        pShadowMapFrustumContainerPassiveCasters->m_LightFrustums.Get(0)->pEntityList = nullptr;

        delete pShadowMapFrustumContainerPassiveCasters->m_LightFrustums.Get(0)->pModelsList;
        pShadowMapFrustumContainerPassiveCasters->m_LightFrustums.Get(0)->pModelsList = nullptr;

        delete pShadowMapFrustumContainerPassiveCasters;
        pShadowMapFrustumContainerPassiveCasters = nullptr;
    }
    ...
    if (pShadowMapLeafBuffersList) {
        ...
        pShadowMapLeafBuffersList = nullptr;
    }
