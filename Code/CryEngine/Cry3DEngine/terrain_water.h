...
public:
    void SetLastFov(float fLastFov) {
        m_fLastFov = fLastFov;
    }
    CWaterOcean(IShader* pTerrainWaterShader, int nBottomTexId, IShader* pSunRoadShader, float fWaterTranspRatio, float fWaterReflectRatio, float fWaterBumpAmount,
                float fWaterBumpAmountY, float fWaterBorderTranspRatio);
    ~CWaterOcean();
    void Render(const int nRecursionLevel);
    int GetMemoryUsage();
    void SetFogVolumrId(int nFogVolumeId) {
        m_nFogVolumeId = nFogVolumeId;
    }
    int GetFogVolumrId() {
        return m_nFogVolumeId;
    }
    bool IsWaterVisible();
