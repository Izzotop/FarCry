...
void CSectorInfo::DrawArray(CArrayInfo* pArrayInfo, CCObject* pTerrainCCObject) {
        ...
        if (!GetCVars()->e_terrain_single_pass || !GetCVars()->e_detail_texture_quality) {
            ...
            for (int t = 0; t < MAX_DETAIL_LAYERS_IN_SECTOR; t++)
                if (m_arrDetailTexInfo[t]) {
                    ...
                    if (m_arrDetailTexInfo[t]->pMatInfo && !bAllowSingePassZ) { // if material is used - do not render single pass detail texture near camera
    ...
    m_pLeafBuffer->AddRenderElements(pTerrainCCObject, 0, -1, (m_pCurShader == m_pTerrain->m_pTerrainWithFog && m_pFogVolume) ? m_pFogVolume->nRendererVolumeID : 0,
                                     EFSLIST_STENCIL);
    ...
    if (m_pCurShader != m_pTerrain->m_pTerrainWithFog && m_pFogVolume && m_pFogVolume->pShader) {
        if (!m_pFogVolume->bOcean || (m_pTerrain->m_pWater && m_pTerrain->m_pWater->IsWaterVisible() &&
...
void CSectorInfo::UpdateVarBuffer() {
    ...
    if (!m_pLeafBuffer || m_cPrevGeomMML != m_cGeometryMML) {
        ...
        m_pLeafBuffer =
            GetRenderer()->CreateLeafBufferInitialized(m_pTerrain->m_lstSectorVertArray.GetElements(), m_pTerrain->m_lstSectorVertArray.Count(), VERTEX_FORMAT_P3F_N_COL4UB_COL4UB,
                                                       pArrayInfo->idx_array.GetElements(), pArrayInfo->idx_array.Count(), R_PRIMV_MULTI_STRIPS, "TerrainSector", eBT_Static,
                                                       CTerrain::GetSectorSize() + 16, m_nTextureID ? m_nTextureID : 0x1000, nullptr, nullptr, false, false);
        ...
        m_pLeafBuffer->m_vBoxMax = Vec3d((float)m_nOriginX + CTerrain::GetSectorSize(), (float)m_nOriginY + CTerrain::GetSectorSize(), m_fMaxZ);
    }
    ...
    for (i = 0; i < pArrayInfo->strip_info.Count(); i++) {
        ...
        m_pLeafBuffer->SetChunk(m_pCurShader, 0, m_nSecVertsCount, pArrayInfo->strip_info[i].begin, pArrayInfo->strip_info[i].end - pArrayInfo->strip_info[i].begin, i - j);
...
void CSectorInfo::RenderSector(CCObject* pTerrainCCObject) {
    ...
    if (m_bHasHoles && nStep == CTerrain::GetHeightMapUnitSize()) { // draw with holes
        for (int x = X1; x < X2; x += nStep) {
            bool in_strip = false;
    ...
    } else {                                           // base index (degenerative strip)
...
void CSectorInfo::MergeSectorIntoLowResTerrain(bool bCalcFarTerrain) {
    ...
    if (bCalcFarTerrain) { // low res optimized rendering can be used only on low res sectors
...
void CSectorInfo::GenerateIndicesForQuad(int x1, int y1, int x2, int y2, CArrayInfo* pArrayInfo, ShadowMapFrustum* pFrustum, Vec3d* pvFrustPos, float fFrustScale) {
    ...
    if (x1 < 0)
        x1 = 0;
    else if (x1 >= CTerrain::GetSectorSize())
        return;
    if (y1 < 0)
        y1 = 0;
    else if (y1 >= CTerrain::GetSectorSize())
        return;
    if (x2 <= 0)
        return;
    else if (x2 > CTerrain::GetSectorSize())
        x2 = CTerrain::GetSectorSize();
    if (y2 <= 0)
        return;
    else if (y2 > CTerrain::GetSectorSize())
        y2 = CTerrain::GetSectorSize();
...
CLeafBuffer* CSectorInfo::MakeSubAreaLeafBuffer(const Vec3d& vPos, float fRadius, CLeafBuffer* pPrevLeafBuffer, IShader* pShader, bool bRecalcLeafBuffer, const char* szLSourceName,
                                                ShadowMapFrustum* pFrustum, Vec3d* pvFrustPos, float fFrustScale) {
    ...
    if (!pPrevLeafBuffer) {
        ...
        pPrevLeafBuffer = GetRenderer()->CreateLeafBufferInitialized(EmptyVertBuffer.GetElements(), EmptyVertBuffer.Count(), VERTEX_FORMAT_P3F_N_COL4UB_COL4UB,
                                                                     m_ArrayInfo_MSALB.idx_array.GetElements(), m_ArrayInfo_MSALB.idx_array.Count(), R_PRIMV_TRIANGLES,
                                                                     szLSourceName, eBT_Static, 1, m_nTextureID, nullptr, nullptr, false, false);
    }

    pPrevLeafBuffer->SetVertexContainer(m_pLeafBuffer);
    // pPrevLeafBuffer->m_SecVertCount = m_pLeafBuffer->m_SecVertCount;

    GenerateIndicesForQuad(int(vPos.x - fRadius), int(vPos.y - fRadius), int(vPos.x + fRadius), int(vPos.y + fRadius), &m_ArrayInfo_MSALB, pFrustum, pvFrustPos, fFrustScale);
