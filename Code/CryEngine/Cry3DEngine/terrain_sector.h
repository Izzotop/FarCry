...
public:
    ...
    int GetSecIndex() {
        return (m_nOriginX / CTerrain::GetSectorSize()) * CTerrain::GetSectorsTableSize() + (m_nOriginY / CTerrain::GetSectorSize());
    }
...
protected:
    ...
    struct CArrayInfo {
        list2<CStripInfo> strip_info;
        list2<unsigned short> idx_array;
        inline void Clear() {
            strip_info.Clear();
            idx_array.Clear();
        }
...
#define ARR_TEX_OFFSETS_SIZE 64
    ...
    CLeafBuffer* MakeSubAreaLeafBuffer(const Vec3d& vPos, float fRadius, CLeafBuffer* pPrevLeafBuffer, IShader* pShader, bool bRecalcLeafBufferconst, const char* szLSourceName,
    ...
    uint GetLastTimeUsed() {
        return m_cLastTimeUsed;
    }
    uint GetLastTimeRendered() {
        return m_cLastTimeRendered;
    }
    void GetMemoryUsage(ICrySizer* pSizer);
