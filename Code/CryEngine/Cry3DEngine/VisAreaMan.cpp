                ...
                if (v1Max.y > v2Min.y && v2Max.y > v1Min.y)
                    if (v1Max.z > v2Min.z && v2Max.z > v1Min.z)
                        if (!pTerrain->m_lstFogVolumes[f].bOcean) {
                            Vec3d arrVerts3d[8] = {Vec3d(v1Min.x, v1Min.y, v1Min.z), Vec3d(v1Min.x, v1Max.y, v1Min.z), Vec3d(v1Max.x, v1Min.y, v1Min.z),
                                                   Vec3d(v1Max.x, v1Max.y, v1Min.z), Vec3d(v1Min.x, v1Min.y, v1Max.z), Vec3d(v1Min.x, v1Max.y, v1Max.z),
                                                   Vec3d(v1Max.x, v1Min.y, v1Max.z), Vec3d(v1Max.x, v1Max.y, v1Max.z)};
...
void CVisAreaManager::PortalsDrawDebug() {
        ...
        else*/
    {
        ...
        for (int v = 0; v < m_lstVisAreas.Count(); v++) {
            ...
            GetRenderer()->DrawLabelEx((m_lstVisAreas[v]->m_vBoxMin + m_lstVisAreas[v]->m_vBoxMax) * 0.5f, 1, (float*)&Vec3d(1, 1, 1), 0, 1, m_lstVisAreas[v]->m_sName);
        ...
        for (int v = 0; v < m_lstPortals.Count(); v++) {
            ...
            GetRenderer()->DrawLabelEx((m_lstPortals[v]->m_vBoxMin + m_lstPortals[v]->m_vBoxMax) * 0.5f, 1, (float*)&Vec3d(1, 1, 1), 0, 1, m_lstPortals[v]->m_sName);
...
bool CVisAreaManager::SetEntityArea(IEntityRender* pEntityRS) {
    ...
    if (pEntityRS->m_dwRndFlags & ERF_DONOTCHECKVIS) { // if CS_FLAG_DRAW_NEAR is set (fps weapon) - use camera position because fps weapon bbox is not correct
    ...
    if (*pVisArea) { // detect if we are still in the same area
...
void CVisAreaManager::Render(CObjManager* pObjManager) {
    ...
    if (GetCVars()->e_portals == 3) { // draw everything for debug
        for (int i = 0; i < m_lstVisAreas.Count(); i++)
        ...
        if (pObjManager->m_nRenderStackLevel) { // use another starting point for reflections
        ...
        } else if (m_pCurArea) { // camera inside some sector
            ...
            {                                                           // for each portal build list of potentially visible entrances into other areas
                ...
                for (int i = 0; i < m_lstActiveEntransePortals.Count(); i++) // entrance into another building is visible
                    m_lstActiveEntransePortals[i]->DrawVolume(pObjManager, i == 0 ? 5 : 1, m_lstOutdoorPortalCameras.Count() ? m_lstOutdoorPortalCameras[0] : camRoot, 0,
                                                              m_pCurPortal, 0, 0, 0);
            }
            ...
            if (m_bSkyVisible && m_lstOutdoorPortalCameras.Count() == 1)
                m_lstOutdoorPortalCameras[0].m_ScissorInfo.x1 = m_lstOutdoorPortalCameras[0].m_ScissorInfo.x2 = m_lstOutdoorPortalCameras[0].m_ScissorInfo.y1 =
                    m_lstOutdoorPortalCameras[0].m_ScissorInfo.y2 = 0;
        } else if (m_pCurPortal) { // camera inside some portal
            m_pCurPortal->DrawVolume(pObjManager, 5, camRoot, 0, m_pCurPortal, &m_bOutdoorVisible, &m_lstOutdoorPortalCameras, &m_bSkyVisible);

            if (m_pCurPortal->m_lstConnections.Count() == 1 || m_lstOutdoorPortalCameras.Count()) { // if camera is in exit portal or exit is visible
                MakeActiveEntransePortalsList(m_lstOutdoorPortalCameras.Count() ? m_lstOutdoorPortalCameras[0] : camRoot, m_lstActiveEntransePortals,
        ...
        } else if (m_lstActiveEntransePortals.Count()) { // camera in outdoors - process visible entrance portals
            for (int i = 0; i < m_lstActiveEntransePortals.Count(); i++)
                m_lstActiveEntransePortals[i]->DrawVolume(pObjManager, 5, camRoot, 0, m_lstActiveEntransePortals[i], &m_bOutdoorVisible, &m_lstOutdoorPortalCameras,
                                                          &m_bSkyVisible);
            m_lstActiveEntransePortals.Clear();
...
bool CVisAreaManager::IsValidVisAreaPointer(CVisArea* pVisArea) {
    if (m_lstVisAreas.Find(pVisArea) < 0 && m_lstPortals.Find(pVisArea) < 0 && m_lstOcclAreas.Find(pVisArea) < 0)
        return false;
...
void CVisAreaManager::UpdateVisArea(CVisArea* pArea, const Vec3d* pPoints, int nCount, const char* szName, float fHeight, const Vec3d& vAmbientColor, bool bAfectedByOutLights,
                                    bool bSkyOnly, CTerrain* pTerrain, const Vec3& vDynAmbientColor, float fViewDistRatio, bool bDoubleSide, bool bUseDeepness,
                                    bool bUseInIndoors) { // on first update there will be nothing to delete, area will be added into list only in this function
...
void CVisAreaManager::UpdateConnections() {
    ...
    for (int p = 0; p < m_lstPortals.Count(); p++) {
        ...
        for (int v = 0; v < m_lstVisAreas.Count(); v++) {
            if (m_lstVisAreas[v]->IsPortalIntersectAreaInValidWay(m_lstPortals[p])) { // if bboxes intersect
...
void CVisAreaManager::MoveAllEntitiesIntoList(list2<IEntityRender*>* plstVisAreasEntities, const Vec3d& vBoxMin, const Vec3d& vBoxMax) {
    for (int p = 0; p < m_lstPortals.Count(); p++) {
        if (m_lstPortals[p]->m_vBoxMin.x < vBoxMax.x && m_lstPortals[p]->m_vBoxMax.x > vBoxMin.x && m_lstPortals[p]->m_vBoxMin.y < vBoxMax.y &&
            m_lstPortals[p]->m_vBoxMax.y > vBoxMin.y) {
    ...
    for (int v = 0; v < m_lstVisAreas.Count(); v++) {
        if (m_lstVisAreas[v]->m_vBoxMin.x < vBoxMax.x && m_lstVisAreas[v]->m_vBoxMax.x > vBoxMin.x && m_lstVisAreas[v]->m_vBoxMin.y < vBoxMax.y &&
            m_lstVisAreas[v]->m_vBoxMax.y > vBoxMin.y) {
        ...
        {                     // check is lsource area was rendered in prev frame
            ...
            if (abs(pVisArea->m_nRndFrameId - nRndFrameId) > 2) {
                ...
                if (pEnt->m_pVisArea->IsPortal()) {
                    ...
                    for (int n = 0; n < pPort->m_lstConnections.Count(); n++) { // loop other sectors
                    ...
                    for (int t = 0; !bFound && t < pVisArea->m_lstConnections.Count(); t++) { // loop portals
...
void CVisAreaManager::MakeActiveEntransePortalsList(const CCamera& curCamera, list2<CVisArea*>& lstActiveEntransePortals, CVisArea* pThisPortal, CObjManager* pObjManager) {
    ...
    if (lstActiveEntransePortals.Count()) {
        qsort(&lstActiveEntransePortals[0], lstActiveEntransePortals.Count(), sizeof(lstActiveEntransePortals[0]), CVisAreaManager__CmpDistToPortal);
...
void CVisAreaManager::DrawOcclusionAreasIntoCBuffer(CCoverageBuffer* pCBuffer) {
            ...
            if (GetViewCamera().IsAABBVisibleFast(AABB(pArea->m_vBoxMin, pArea->m_vBoxMax))) {
                ...
                if (fDist < fRadius * pArea->m_fViewDistRatio / fZoomFactor * 0.125f && pArea->m_lstShapePoints.Count() >= 2) {
                    ...
                    *pArea->m_pOcclCamera = GetViewCamera();
                    ...
                    int nShift = pArea->m_lstShapePoints.Count() > 2 && (GetSquaredDistance(pArea->m_lstShapePoints[0], pArea->m_lstShapePoints[1]) <
    ...
    if (m_lstActiveOcclVolumes.Count()) { // sort occluders by distance to the camera
        qsort(&m_lstActiveOcclVolumes[0], m_lstActiveOcclVolumes.Count(), sizeof(m_lstActiveOcclVolumes[0]), CVisAreaManager__CmpDistToPortal);
        ...
        if (GetCVars()->e_portals == 4) { // show really active occluders
