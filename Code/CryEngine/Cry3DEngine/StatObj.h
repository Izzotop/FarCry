...
struct CStatObj : public Cry3DEngineBase, public IStreamCallback, public IStatObj {
    ...
    CIndexedMesh* GetTriData() {
        return m_pTriData;
    }

    int m_nLoadedTrisCount;
    const Vec3d GetCenter() {
        return m_vBoxCenter;
    }
...
public:
    // Loader
    bool Load(const char* szFileName, const char* szGeomName, enum EVertsSharing eVertsSharing, bool bLoadAdditinalInfo, bool bKeepInLocalSpace, bool bUseStreaming = false,
    ...
    //! set shadow volume
    ItShadowVolume* GetShadowVolume() {
        return (m_pSvObj);
    }

    //! get shadow volume
    void SetShadowVolume(ItShadowVolume* pSvObj) {
        m_pSvObj = pSvObj;
    }
...
#define MAX_STATOBJ_LODS_NUM 3
    ...
    bool IsSpritesCreated() {
        return m_arrSpriteTexID[0] > 0;
    }
    float GetDistFromPoint(const Vec3d& vPoint);
    int GetLoadedTrisCount() {
        return m_nLoadedTrisCount;
    }
    ...
    bool LoadUncompiled(const char* szFileName, const char* szGeomName, EVertsSharing eVertsSharing, bool bLoadAdditinalInfo, bool bKeepInLocalSpace, bool bLoadLater,
                        bool bMakePhysics);
