...
void CTerrain::MakeSplatVertex(const int& x, const int& y, const uchar& alpha, struct_VERTEX_FORMAT_P3F_N_COL4UB& vert, const int& nTexID, const Vec3d& vDetTexOffset) {
...
void CTerrain::SetDetailVertNormal(struct_VERTEX_FORMAT_P3F_N_COL4UB& v0) {
    ...
    if (pSec0 && pSec0->m_pLeafBuffer && pSec0->m_cPrevGeomMML == pSec0->m_cGeometryMML) {
        ...
        int nVertId = ((int)(v0.xyz.x + 0.2f) - pSec0->m_nOriginX) / nStep * (CTerrain::GetSectorSize() / nStep + 1) + ((int)(v0.xyz.y + 0.2f) - pSec0->m_nOriginY) / nStep;
            ...
            if (!GetRenderer()->EF_GetHeatVision())                                  // if no heat vision
                if (m_pViewCamera->GetFov() >= GetCVars()->e_detail_texture_min_fov) // if no big zoom
                    if (m_pRETerrainDetailTextureLayers && m_pTerrainDetailTextureLayersEff) {
                        ...
                        //            GetRenderer()->EF_AddEf(0, m_pRETerrainDetailTextureLayers, m_pTerrainDetailTextureLayersEff, nullptr, pObj, 0, nullptr, EFSLIST_STENCIL |
                        //            eS_TerrainDetailTextures/*, eS_Opaque*/);
    ...
    if (m_nRenderStackLevel == 0) {
        ...
        if (fDiffSqr > fGeometryUpdateStepSqr) {
            ...
            for (nTexID = 0; nTexID < MAX_SURFACE_TYPES_COUNT; nTexID++) {
                ...
                bool bUpdated = false;
                if (!m_DetailTexInfo[nTexID].pVertBuff || nVertCount > m_DetailTexInfo[nTexID].pVertBuff->m_SecVertCount ||
                    nVertCount < m_DetailTexInfo[nTexID].pVertBuff->m_SecVertCount / 2 || m_DetailTexInfo[nTexID].pVertBuff->m_nVertexFormat != VERTEX_FORMAT_P3F_N_COL4UB) {
                    GetRenderer()->DeleteLeafBuffer(m_DetailTexInfo[nTexID].pVertBuff);
                    if (nVertCount) {
                        bUpdated = true;
                        m_DetailTexInfo[nTexID].pVertBuff =
                            GetRenderer()->CreateLeafBufferInitialized(lstVerts.GetElements(), lstVerts.Count(), VERTEX_FORMAT_P3F_N_COL4UB, lstInds.GetElements(), lstInds.Count(),
                                                                       R_PRIMV_TRIANGLES, "TerrainDetailTex", eBT_Static, 1, m_DetailTexInfo[nTexID].nTexID);
