...
struct CLightEntity : public IEntityRender, public Cry3DEngineBase {
    const char* GetEntityClassName() const override { return "LightEntityClass"; }
    float GetScale() const override { return 1.f; }
    const char* GetName() const override { return "LightEntityName"; }
    const Vec3d& GetPos(bool) const override;
    const Vec3d& GetAngles(int) const override;
    void GetRenderBBox(Vec3d&, Vec3d&) override;
    float GetRenderRadius() const override;
    bool DrawEntity(const SRendParams&) override { return true; }
    bool IsStatic() const override { return false; }
    bool IsEntityHasSomethingToRender() override { return false; }
    bool IsEntityAreasVisible() override { return true; }
    IPhysicalEntity* GetPhysics() const override { return nullptr; }
    void SetPhysics(IPhysicalEntity*) override {}
    void SetMaterial(IMatInfo*) override {}
    IMatInfo* GetMaterial() const override { return nullptr; }
    class CDLight* GetLight() override { return m_pLight; }
    float GetMaxViewDist() override;
    void PreloadInstanceResources(Vec3d vPrevPortalPos, float fPrevPortalDistance, float fTime) override {}

    CLightEntity();
    ~CLightEntity() override;
...
struct SLevelInfo {
    float m_fSkyBoxAngle, m_fSkyBoxStretching, m_fSunHeightScale, m_fWaterTranspRatio, m_fWaterReflectRatio, m_fWaterBumpAmountX, m_fWaterBumpAmountY, m_fWaterBorderTranspRatio,
        m_fUnderWaterFogDistance;
    float m_fFogNearDist, m_fFogFarDist, m_fMaxViewDist;
    float m_fDefFogNearDist, m_fDefFogFarDist, m_fDefMaxViewDist;
...
struct SLevelShaders {
    IShader *m_pTerrainWaterShader, *m_pSunRoadShader, *m_pSHTerrainParticles;
};
...
class C3DEngine : public I3DEngine, public SLevelInfo, public SLevelShaders, public SRenderElements, public Cry3DEngineBase {
...
public:
    ...
    virtual IStatObj* MakeObject(const char* szFileName, const char* szGeomName = 0, EVertsSharing eVertsSharing = evs_ShareAndSortForCache, bool bLoadAdditinalInfo = true,
    ...
    virtual void SetRenderCallback(void (*pFunc)(void* pParams), void* pParams) {
        m_pRenderCallbackFunc = pFunc;
        m_pRenderCallbackParams = pParams;
    }
    ...
    virtual class IEdgeConnectivityBuilder* GetNewConnectivityBuilder();
    ...
    virtual class IEdgeConnectivityBuilder* GetNewStaticConnectivityBuilder();
    virtual class IEdgeDetector* GetEdgeDetector();
    ...
    virtual void UpdateVisArea(IVisArea* pArea, const Vec3d* pPoints, int nCount, const char* szName, float fHeight, const Vec3d& vAmbientColor, bool bAfectedByOutLights,
...
public:
    ...
    CDLight* CheckDistancesToLightSources(uint& nDLightMask, const Vec3d vObjPos, const float fObjRadius, IEntityRender* pEntityRender = 0, int nMaxLightBitsNum = 8,
                                          CDLight** pSelectedLights = nullptr, int nMaxSelectedLights = 0, Vec3d* pvSummLightAmmount = nullptr);
    ...
    CObjManager* GetObjManager() {
        return m_pObjManager;
    }
    CVars* GetCVars() {
        return m_pCVars;
    }
    CVisAreaManager* GetVisAreaManager() {
        return m_pVisAreaManager;
    }
    CMatMan* GetMatMan() {
        return m_pMatMan;
    }
...
private:
    // misc
    CCObject *m_SunObject[8], *m_pBlurObj, *m_pScreenObj;
...
#define MAX_LIGHTS_NUM 32
    ...
    Vec3d GetTerrainSurfaceNormal(Vec3d vPos) {
        return m_pTerrain ? m_pTerrain->GetTerrainSurfaceNormal(vPos) : Vec3d(0, 0, 0);
    }
...
public:
    ...
    list2<CDLight>* GetDynamicLightSources() {
        return &m_lstDynLights;
    }
    void ProcessScreenEffects();
    int GetRealLightsNum() {
        return m_nRealLightsNum;
    }
