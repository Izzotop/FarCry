...
void CVisArea::Update(const Vec3d* pPoints, int nCount, const char* szName, float fHeight, const Vec3d& vAmbientColor, bool bAfectedByOutLights, bool bSkyOnly,
                      const Vec3d& vDynAmbientColor, float fViewDistRatio, bool bDoubleSide, bool bUseDeepness, bool bUseInIndoors) {
...
void CVisArea::UpdateGeometryBBox() {
    ...
    if (IsPortal()) { // fix for big objects passing portal
...
bool CVisArea::FindVisArea(IVisArea* pAnotherArea, int nMaxReqursion, bool bSkipDisabledPortals) { // todo: avoid going into parent
...
bool CVisArea::UpdatePortalCameraScissor(CCamera& cam, list2<Vec3d>* lstVerts, bool bMergeFrustums) {
    ...
    for (int i = 0; i < lstVerts->Count() && i < 8; i++) { // result is in range from 0 to 100
        const Vec3d& v3d = lstVerts->GetAt(i);

        if (pCVars->e_portals == 4)
            pRend->Draw3dBBox(v3d - Vec3d(0.1f, 0.1f, 0.1f), v3d + Vec3d(0.1f, 0.1f, 0.1f));

        pRend->ProjectToScreen(v3d.x, v3d.y, v3d.z, &arrScreenSpacePos[i].x, &arrScreenSpacePos[i].y, &arrScreenSpacePos[i].z);
...
void CVisArea::UpdatePortalCameraPlanes(CCamera& cam, Vec3d* pVerts, bool bMergeFrustums) { // todo: do also take into account GetViewCamera()
    ...
    if (!bMergeFrustums || plane.n.Dot(cam.GetFrustumPlane(FR_PLANE_LEFT)->n) < cam.GetFrustumPlane(FR_PLANE_RIGHT)->n.Dot(cam.GetFrustumPlane(FR_PLANE_LEFT)->n))
        cam.SetFrustumPlane(FR_PLANE_RIGHT, plane);

    plane.CalcPlane(vCamPos, pVerts[0], pVerts[1]); // update plane only if it reduces fov
    if (!bMergeFrustums || plane.n.Dot(cam.GetFrustumPlane(FR_PLANE_RIGHT)->n) < cam.GetFrustumPlane(FR_PLANE_LEFT)->n.Dot(cam.GetFrustumPlane(FR_PLANE_RIGHT)->n))
        cam.SetFrustumPlane(FR_PLANE_LEFT, plane);

    plane.CalcPlane(vCamPos, pVerts[3], pVerts[0]); // update plane only if it reduces fov
    if (!bMergeFrustums || plane.n.Dot(cam.GetFrustumPlane(FR_PLANE_TOP)->n) < cam.GetFrustumPlane(FR_PLANE_BOTTOM)->n.Dot(cam.GetFrustumPlane(FR_PLANE_TOP)->n))
        cam.SetFrustumPlane(FR_PLANE_BOTTOM, plane);

    plane.CalcPlane(vCamPos, pVerts[1], pVerts[2]); // update plane only if it reduces fov
    if (!bMergeFrustums || plane.n.Dot(cam.GetFrustumPlane(FR_PLANE_BOTTOM)->n) < cam.GetFrustumPlane(FR_PLANE_TOP)->n.Dot(cam.GetFrustumPlane(FR_PLANE_BOTTOM)->n))
...
void CVisArea::DrawVolume(CObjManager* pObjManager, int nReqursionLevel, CCamera CurCamera, CVisArea* pParent, CVisArea* pCurPortal, bool* pbOutdoorVisible,
                          list2<CCamera>* plstOutPortCameras, bool* pbSkyVisible) {
                ...
                if (!pObjManager->m_nRenderStackLevel) // light scissor can not be shared between reqursion levels because same CDlight objects are used
                    if (pDLight && pDLight->m_Flags & DLF_THIS_AREA_ONLY && pDLight->m_pOwner)
                        if (pDLight->m_pOwner->GetEntityVisArea() == this) {
                            if (!pDLight->m_sWidth || pDLight->m_sWidth == (CurCamera.m_ScissorInfo.x2 - CurCamera.m_ScissorInfo.x1)) { // first time - set
    ...
    // render area statics
    DrawEntities(m_nFogVolumeId, nDLMask, 0, CurCamera, m_lstShapePoints.Count() ? &m_vAmbColor : 0, m_lstShapePoints.Count() ? &m_vDynAmbColor : 0, nullptr, true, 0, pObjManager,
    ...
    if (bThisIsPortal && m_lstConnections.Count() && (this != pCurPortal || !pCurPortal->IsPointInsideVisArea(CurCamera.GetPos()))) {
        Vec3d vPortNorm = (!pParent || pParent == m_lstConnections[0] || m_lstConnections.Count() == 1) ? m_vConnNormals[0] : m_vConnNormals[1];
        ...
        if (pParent && !IsEquivalent(vPortNorm, Vec3d(0, 0, 0), VEC_EPSILON) && vPortNorm.z) { // up/down portal
        ...
        } else if (!IsEquivalent(vPortNorm, Vec3d(0, 0, 0), VEC_EPSILON) && vPortNorm.z == 0) { // basic portal
            ...
            for (int i = 0; i < m_lstShapePoints.Count() && nInAreaPointCounter < 2; i++) {
                ...
                if ((pParent && (pParent->IsPointInsideVisArea(vTestPoint))) || (!pParent && (!pAnotherArea->IsPointInsideVisArea(vTestPoint)))) {
                    arrInAreaPointId[nInAreaPointCounter] = i;
                    arrInAreaPoint[nInAreaPointCounter++] = m_lstShapePoints[i];
                }
            }

            if (nInAreaPointCounter == 2) { // success, take into account volume and portal shape versts order
                int nEven = IsShapeClockwise();
        ...
        } else if (!pParent && vPortNorm.z == 0 && m_lstConnections.Count() == 1) { // basic entrance portal
        ...
        } else if (!pParent && vPortNorm.z != 0 && m_lstConnections.Count() == 1) { // up/down entrance portal
        ...
        } else { // something wrong or areabox portal - use simple solution
            if (
        ...
        if (bScisorValid && pParent && m_lstConnections.Count() == 1) { // set this camera for outdoor
    ...
    if (!bThisIsPortal && m_lstConnections.Count()) {
        ...
        qsort(&m_lstConnections[0], m_lstConnections.Count(), sizeof(m_lstConnections[0]), CVisAreaManager__CmpDistToPortal);
    }
    ...
    for (int p = 0; p < m_lstConnections.Count(); p++) {
        ...
        if (pNeibVolume != pParent) {
            if (!bThisIsPortal) { // skip far portals
...
bool CVisArea::IsShapeClockwise() {
    float fClockWise = (m_lstShapePoints[0].x - m_lstShapePoints[1].x) * (m_lstShapePoints[2].y - m_lstShapePoints[1].y) -
...
void CVisArea::DrawVolume_NotThisAreaOnlyLights(CObjManager* pObjManager, int nReqursionLevel, CCamera CurCamera, CVisArea* pParent, CVisArea* pCurPortal, bool* pbOutdoorVisible,
                                                list2<CCamera>* plstOutPortCameras, bool* pbSkyVisible) {
    if (!pParent)
        return;
    ...
    // render lsources
    {
        for (int i = 0; i < m_lstEntities[DYNAMIC_ENTITIES].Count(); i++) {
            ...
            if (pLight && !(pLight->m_Flags & DLF_THIS_AREA_ONLY)) { // process light sources
                ...
                if (AABB(pParent->m_vBoxMin, pParent->m_vBoxMax).IsIntersectBox(AABB(vBoxMin, vBoxMax)))
                    pObjManager->RenderObject(pEntityRender, 0, 0, false, CurCamera, 0, 0, 0, true, pEntityRender->GetMaxViewDist());
            } else if (pEntityRender->GetRndFlags() & ERF_DONOTCHECKVIS) { // process 1p weapon
                pEntityRender->m_fWSMaxViewDist = pEntityRender->GetMaxViewDist();
                uint nDLightMask = ((C3DEngine*)Get3DEngine())->GetFullLightMask();
                pObjManager->RenderObject(pEntityRender, m_nFogVolumeId, nDLightMask, false, CurCamera, &m_vAmbColor, &m_vDynAmbColor, 0, true, pEntityRender->m_fWSMaxViewDist);
            }
        }
    }
    ...
    if (bThisIsPortal && nReqursionLevel > 0) { // recurse to not rendered connetions
        for (int p = 0; p < m_lstConnections.Count(); p++) {
            CVisArea* pNeibVolume = m_lstConnections[p];
            if (pNeibVolume != pParent) {
                pNeibVolume->DrawVolume_NotThisAreaOnlyLights(pObjManager, nReqursionLevel - 1, CurCamera, this, pCurPortal, pbOutdoorVisible, plstOutPortCameras, pbSkyVisible);
