...
float CStatObjInst::GetBendingRandomFactor() { // calculate random
...
bool CStatObjInst::DrawEntity(const struct SRendParams& _EntDrawParams) {
    ...
    float near_far_dist = (18.f * pBody->GetRadiusVert() * m_fScale) * max(0.5f, m_pObjManager->m_lstStaticTypes[m_nObjectTypeID].fSpriteDistRatio) +
                          0.2f * (float)fabs((m_vPos.z + pBody->GetCenter().z * m_fScale) - vCamPos.z);
    /*
    static float fAverageSpriteDistRatio = 0.5f;
    fAverageSpriteDistRatio += m_pObjManager->m_lstStaticTypes[m_nObjectTypeID].fSpriteDistRatio;//(fAverageSpriteDistRatio +
    m_pObjManager->m_lstStaticTypes[m_nObjectTypeID].fSpriteDistRatio)*0.5f; if(m_nStatObjNumPerFrame%10000==0) GetLog()->Log("m_nStatObjNumPerFrame =
    %.4f",fAverageSpriteDistRatio/m_nStatObjNumPerFrame);
    */
    ...
    const float fLodSwitchCountDown = GetCVars()->e_vegetation_sprites_slow_switch ? min(1.f, fabsf(fDistance - near_far_dist) / (near_far_dist * 0.33f)) : 1.f;

    if (!m_pObjManager->m_nRenderStackLevel) { // calculate bending amount
  ...
  m_pObjManager->m_lstStaticTypes[m_nObjectTypeID].bCalcLighting );
    ...
    if (bUse3DOnly || fDistance < near_far_dist) {
        ...
        if (m_pObjManager->m_lstStaticTypes[m_nObjectTypeID].bUseAlphaBlending) {
            ...
            if (m_pObjManager->m_lstStaticTypes[m_nObjectTypeID].bUseSprites) { // set fixed alpha
                rParms.fAlpha = 0.99f;
        ...
        if (!GetCVars()->e_vegetation_debug) { // ignore editor colors per instance
        ...
        if (fDistance < COVERAGEBUFFER_OCCLUDERS_MAX_DISTANCE && GetCVars()->e_cbuffer && pBody->GetLoadedTrisCount() == 12) {
        ...
        if (!bUse3DOnly && fDistance > near_far_dist) { // add to the list of far objects
...
float CStatObjInst::GetRenderRadius() const {
    CStatObj* pBody = m_pObjManager->m_lstStaticTypes[m_nObjectTypeID].GetStatObj();
    if (pBody)
...
const char* CStatObjInst::GetName() const {
    return (m_nObjectTypeID >= 0 && m_pObjManager->m_lstStaticTypes[m_nObjectTypeID].GetStatObj()) ? m_pObjManager->m_lstStaticTypes[m_nObjectTypeID].GetStatObj()->GetFileName()
                                                                                                   : "StatObjNotSet";
}
