...
// Shader params ids.
enum { SHP_REFLECT_AMMOUNT, SHP_BUMP_AMMOUNT_X, SHP_BUMP_AMMOUNT_Y };

CWaterOcean::CWaterOcean(IShader* pTerrainWaterShader, int nBottomTexId, IShader* pSunRoadShader, float fWaterTranspRatio, float fWaterReflectRatio, float fWaterBumpAmountX,
                         float fWaterBumpAmountY, float fWaterBorderTranspRatio) {
...
void CWaterOcean::Render(const int nRecursionLevel) {
    ...
    if (vCamPos.z < fWaterLevel) { // if camera is in indoors and lower than ocean level
    ...
    // test visibility
    if (fabs(m_fLastFov - GetViewCamera().GetFov()) < 0.01f && GetCVars()->e_hw_occlusion_culling_water && (GetRenderer()->GetFeatures() & RFT_OCCLUSIONTEST) &&
        (nRecursionLevel == 0)) {
        // make element if not ready
        if (!m_pREOcclusionQueries[nRecursionLevel][nBufID])
            m_pREOcclusionQueries[nRecursionLevel][nBufID] = (CREOcclusionQuery*)GetRenderer()->EF_CreateRE(eDATA_OcclusionQuery);
    ...
    if (bWaterVisible) {
        ...
        if (m_pLeafBufferWaters[nRecursionLevel][nBufID] && m_pLeafBufferWaters[nRecursionLevel][nBufID]->m_pSecVertBuffer->m_vertexformat == VERTEX_FORMAT_P3F_COL4UB &&
            ...
            m_pLeafBufferWaters[nRecursionLevel][nBufID] =
                GetRenderer()->CreateLeafBufferInitialized(Verts_DWQ.GetElements(), Verts_DWQ.Count(), VERTEX_FORMAT_P3F_COL4UB, Indices_DWQ.GetElements(), Indices_DWQ.Count(),
                                                           R_PRIMV_MULTI_STRIPS, "OutdoorWaterCircle", eBT_Dynamic, nChunksNum, 0x1000);

            for (int i = 0; i < nChunksNum && i < (lstFirstIdxId.Count() - 1); i++) {
                m_pLeafBufferWaters[nRecursionLevel][nBufID]->SetChunk(m_pTerrainWaterShader, Indices_DWQ[lstFirstIdxId[i]], Verts_DWQ.Count() / (lstFirstIdxId.Count() - 1),
...
void CWaterOcean::RenderBottom(int nRecursionLevel) {
    ...
    if (!m_pLeafBufferBottom) {
        ...
        m_pLeafBufferBottom = GetRenderer()->CreateLeafBufferInitialized(lstVertices.GetElements(), lstVertices.Count(), VERTEX_FORMAT_P3F, lstIndices.GetElements(),
                                                                         lstIndices.Count(), R_PRIMV_MULTI_STRIPS, "OceanBottomCircle", eBT_Dynamic, nChunksNum, m_nBottomTexId);

        for (int i = 0; i < nChunksNum; i++) {
            m_pLeafBufferBottom->SetChunk(m_pTerrainWaterBottomShader, lstIndices[lstFirstIdxId[i]], lstVertices.Count() / (lstFirstIdxId.Count() - 1), lstFirstIdxId[i],
    ...
    float fScale = GetViewCamera().GetZMax() / nChunksNum;
    CCObject* pObj = GetRenderer()->EF_GetObject(true);
    pObj->m_Matrix.SetTranslationMat(Vec3d(vCamPos.x, vCamPos.y, vCamPos.z > fWaterLevel ? -fBottomZOffset : 0.f));
