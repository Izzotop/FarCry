...
public:
    ...
    virtual RenderLMData* GetLightmap(int nLod) {
        return m_arrLMData[nLod].m_pLMData;
    };
    virtual struct CLeafBuffer* GetLightmapTexCoord(int nLod) {
        return m_arrLMData[nLod].m_pLMTCBuffer;
    };
    ...
    void SetStatObjGroupId(int nStatObjInstanceGroupId) {
        m_nObjectTypeID = nStatObjInstanceGroupId;
    }
    void SetMergeId(int nId) {
        m_nMergeID = nId;
    }
    int GetMergeId() {
        return m_nMergeID;
