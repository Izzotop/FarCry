...
void CTerrain::SetDetailTextures(int nId, const char* szFileName, float fScaleX, float fScaleY, uchar ucProjAxis, const char* szSurfName) {
...
int __cdecl CTerrain__Cmp_CStatObjInstForLoading_Size(const void* v1, const void* v2) {
    ...
    { // get pos
        ...
        if (x < 0 || x >= CTerrain::GetSectorsTableSize() || y < 0 || y >= CTerrain::GetSectorsTableSize() || (vBMax.x - vBMin.x) > TERRAIN_SECTORS_MAX_OVERLAPPING * 2 ||
            (vBMax.y - vBMin.y) > TERRAIN_SECTORS_MAX_OVERLAPPING * 2)
            x = y = 0;
...
void CTerrain::LoadStatObjInstances() {
    ...
    qsort(static_objects.GetElements(), static_objects.Count(), sizeof(static_objects[0]), CTerrain__Cmp_CStatObjInstForLoading_Size);
    ...
    for (int i = 0; i < static_objects.Count(); i++) {
        ...
        if (nId >= 0 && nId < m_pObjManager->m_lstStaticTypes.Count() && fScale > 0 && x >= 0 && x < CTerrain::GetTerrainSize() && y >= 0 && y < CTerrain::GetTerrainSize() &&
...
void CTerrain::InitFogVolumes() {
    ...
    for (int v = 0; v < m_lstFogVolumes.Count(); v++) {
        ...
        int nVolumeID = m_lstFogVolumes[v].nRendererVolumeID =
            GetRenderer()->EF_RegisterFogVolume(m_lstFogVolumes[v].fMaxViewDist, m_lstFogVolumes[v].vBoxMax.z, m_lstFogVolumes[v].vColor, -1, m_lstFogVolumes[v].m_bCaustics);
...
void CTerrain::SetSectorFogVolume(CSectorInfo* pSecInfo) {
    pSecInfo->m_pFogVolume = 0;
    for (int v = 0; v < m_lstFogVolumes.Count(); v++) {
        if (pSecInfo && pSecInfo->m_fMinZ < m_lstFogVolumes[v].vBoxMax.z && m_lstFogVolumes[v].IntersectBBox(pSecInfo->m_vBoxMin, pSecInfo->m_vBoxMax) &&
...
bool CTerrain::LoadTerrainSettingsFromXML() {
    ...
    { // HeightmapUnitSize
        ...
        if (m_nUnitSize != 1 && m_nUnitSize != 2 && m_nUnitSize != 4 && m_nUnitSize != 8 && m_nUnitSize != 16 && m_nUnitSize != 32)
            return 0;
    }
    ...
    { // HeightmapSize
        ...
        if (m_nTerrainSize != 64 && m_nTerrainSize != 128 && m_nTerrainSize != 256 && m_nTerrainSize != 512 && m_nTerrainSize != 1024 && m_nTerrainSize != 2048 &&
            m_nTerrainSize != 4096)
            return 0;
