...
void CRainItem::Process(Vec3d& right, Vec3d& up, Vec3d& front, const int& nTexID, const Vec3d& delta, IRenderer* pIRenderer, ITimer* pITimer, const Vec3d& vFocusPos,
                        CPartManager* pPartManager, CTerrain* pTerrain, class CObjManager* pObjManager, const Vec3d& _vCamPos) {
    { // space loop
    ...
    if (0 && fabs((m_vPos.z + m_fSize) - pTerrain->GetZSafe(m_vPos.x, m_vPos.y)) < 0.1f && delta.z < 0) {
        ...
        float fDist = (float)max(fabs(vCollPos.x - _vCamPos.x), max(fabs(vCollPos.y - _vCamPos.y), fabs(vCollPos.z - _vCamPos.z)));
...
void CRainManager::Render(CTerrain* pTerrain, const Vec3d& vColor, CObjManager* pObjManager, CPartManager* pPartManager, const Vec3d& vWindDir) {
    ...
    for (int i = 0; i < RAIN_COUNT; i++)
        m_arrItems[i].Process(right, vUp, front, m_nRainTexID, vWindDir, GetRenderer(), GetTimer(), vFocusPos, pPartManager, pTerrain, pObjManager, vCamPos);
    ...
    while (m_fDropTime > fDropPeriod) {
        ...
        float fDist = (float)max(fabs(vCollPos.x - vCamPos.x), max(fabs(vCollPos.y - vCamPos.y), fabs(vCollPos.z - vCamPos.z)));
