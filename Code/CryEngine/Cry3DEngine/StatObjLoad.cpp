...
void CStatObj::Refresh(int nFlags) {
    if (nFlags & FRO_GEOMETRY) {
        ...
        ShutDown();
        Init();
        bool bRes = Load(m_szFileName, m_szGeomName[0] ? m_szGeomName : 0, m_eVertsSharing, m_bLoadAdditinalInfo, m_bKeepInLocalSpace, false);
...
bool CStatObj::Load(const char* szFileName, const char* szGeomName, EVertsSharing eVertsSharing, bool bLoadAdditinalInfo, bool bKeepInLocalSpace, bool bUseStreaming,
                    bool bMakePhysics) {
...
bool CStatObj::LoadUncompiled(const char* szFileName, const char* szGeomName, EVertsSharing eVertsSharing, bool bLoadAdditinalInfo, bool bKeepInLocalSpace, bool bLoadLater,
                              bool bMakePhysics) {
    if (!szFileName[0])
    ...
    // load uncompiled
    m_nLoadedTrisCount = 0;
    m_pTriData = new CIndexedMesh(GetSystem(), szFileName, (szGeomName && szGeomName[0]) ? szGeomName : 0, &m_nLoadedTrisCount, bLoadAdditinalInfo, bKeepInLocalSpace,
                                  m_bIgnoreFakeMaterialsInCGF);
...
#if !defined(LINUX)
            Warning(0, sLodFileName, "CStatObj::LoadLowLODs: Low lod model contains too many polygons comparing to full LOD model [%d agains %d], there is no sense to use it",
