...
bool CTerrain::RenderAreaLeafBuffers(Vec3d vPos, float fRadius, int nDynLMask, CLeafBuffer** arrLightLeafBuffers, int nMaxLeafBuffersNum, CCObject* pObj, IShader* pShader,
                                     bool bRecalcLeafBuffers, const char* szComment, IShader* pEffStencilTest, int nShaderSortOrder, ShadowMapFrustum* pFrustum, Vec3d* pvFrustPos,
                                     float fFrustScale) {
    ...
    for (int x = min_x; x <= max_x; x++)
        for (int y = min_y; y <= max_y; y++) {
            ...
            arrLightLeafBuffers[i] =
                pSectorInfo->MakeSubAreaLeafBuffer(vPos, fRadius, arrLightLeafBuffers[i], pShader, bRecalcLeafBuffers, szComment, pFrustum, pvFrustPos, fFrustScale);
