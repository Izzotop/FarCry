...
bool CTerrain::IsSectorNonMergable(CSectorInfo* info) {
    if (info->m_cGeometryMML < MAX_MML_LEVEL || info->m_fDistance < MIN_ALLOWED_MERGED_SECTORS_DISTANCE ||
        !(GetCVars()->e_terrain_merge_far_sectors && CTerrain::GetHeightMapUnitSize() >= 2) || info->m_pFogVolume ||
...
void CTerrain::DrawVisibleSectors() {
    ...
    for (int i = 0; i < m_lstVisSectors.Count(); i++) {
        ...
        if (GetCVars()->e_detail_texture_quality) {
            if (!info->m_nDynLightMaskNoSun && info->m_pFogVolume && GetCVars()->e_terrain_single_pass_vol_fog && info->m_fDistance > 32)
        ...
        if (pCVars->e_terrain_debug == 1) {
            ...
            GetRenderer()->Draw3dBBox(Vec3d((float)info->m_nOriginX - TERRAIN_SECTORS_MAX_OVERLAPPING, (float)info->m_nOriginY - TERRAIN_SECTORS_MAX_OVERLAPPING, 0),
...
void CTerrain::RenderLowResTerrain() {
    ...
    if (!m_pLowResTerrainLeafBuffer) {
        ...
        // make leaf buffer
        m_pLowResTerrainLeafBuffer = GetRenderer()->CreateLeafBufferInitialized(lstLowResTerrainVertArray.GetElements(), lstLowResTerrainVertArray.Count(), VERTEX_FORMAT_P3F,
                                                                                m_lstLowResTerrainIdxArray.GetElements(), m_lstLowResTerrainIdxArray.Count(), R_PRIMV_TRIANGLES,
                                                                                "LowResTerrain", eBT_Dynamic, 1, m_pLowLodCoverMapTex->GetTextureID());
    ...
    if (!m_nRenderStackLevel) {
        ...
        m_pLowResTerrainLeafBuffer->SetChunk(m_pLowResTerrainShader, 0, m_nLowResTerrainVertCount, 0, m_lstLowResTerrainIdxArray.Count());
...
void CTerrain::RenderReflectedTerrain() {
    ...
    if (!m_pLowResTerrainLeafBuffer ||
        ...
        !m_pLowResTerrainLeafBuffer->m_SecVertCount || !m_lstReflectedTerrainIdxArray.Count())
        return;
    ...
    if (!m_pReflectedTerrainLeafBuffer) {
        ...
        m_pReflectedTerrainLeafBuffer = GetRenderer()->CreateLeafBufferInitialized(
            /*&lstLowResTerrainVertArray[0], lstLowResTerrainVertArray.Count(), */ 0, 0, VERTEX_FORMAT_P3F, m_lstReflectedTerrainIdxArray.GetElements(),
            m_lstReflectedTerrainIdxArray.Count(), R_PRIMV_TRIANGLES, "ReflectedTerrain", eBT_Dynamic, 1, m_pLowLodCoverMapTex->GetTextureID());
    ...
    //  if(!m_nRenderStackLevel)
    {
        ...
        m_pReflectedTerrainLeafBuffer->SetChunk(m_pLowResTerrainShader, 0, m_nLowResTerrainVertCount, 0, m_lstReflectedTerrainIdxArray.Count());
