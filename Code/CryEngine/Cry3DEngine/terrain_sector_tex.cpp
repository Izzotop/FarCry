...
void CSectorInfo::SetTextures(bool bMakeUncompressedForEditing) {
    ...
    // desired actions
    if (m_cTextureMML > m_cNewTextMML) { // increase tex resolution
        if (!m_bLockTexture) {
    ...
    } else if (m_cTextureMML < m_cNewTextMML) { // reduce tex resolution
        if (m_nTextureID == m_nLowLodTextureID)
            GetLog()->Log("CSectorInfo::SetTextureAndLOD: m_nTextureID == m_nLowLodTextureID");
...
int CSectorInfo::MakeSectorTextureDDS(int sec_id, int nMipMapLevelToLoad, bool bMakeUncompressedForEditing) {
    ...
    if (!m_pTerrain->m_fpTerrainTextureFile) {
        Warning(0, 0, "MakeSectorTextureDDS: !m_pTerrain->m_fpTerrainTextureFile");
        return 0;
    }
    ...
    INT_PTR readed = GetSystem()->GetIPak()->FRead(m_pTerrain->m_ucpTmpTexBuffer, 1, // AMD Port
                                                   GetCVars()->e_terrain_texture_mipmaps ? nDataSize : nTexSize * nTexSize / 2, m_pTerrain->m_fpTerrainTextureFile);
...
void CSectorInfo::UnloadHeighFieldTexture(float fDistanse, float fMaxViewDist) {
    if (m_nTextureID && m_cTextureMML == 0 && m_nTextureID != m_nLowLodTextureID) { // unload if to far or not in use int time
        if (m_nTextureID == m_nLowLodTextureID)
    ...
    } else if (m_nTextureID && m_nTextureID == m_nLowLodTextureID) { // only low lod
    ...
    } else if (!m_nTextureID && !m_nLowLodTextureID) { // no textures
        m_nTextureID = m_nLowLodTextureID;
    } else {
