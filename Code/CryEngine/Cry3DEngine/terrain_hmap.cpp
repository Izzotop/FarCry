...
float CHighMap::GetZApr(float x1, float y1) {
    ...
    if (x1 < 1 || y1 < 1 || x1 >= CTerrain::GetTerrainSize() || y1 >= CTerrain::GetTerrainSize())
        ...
        float dDownLandZ0 = (1.f - dx1) * (m_arrusHightMapData[nX][nY] & (~INFO_BITS_MASK)) + (dx1) * (m_arrusHightMapData[nX + 1][nY] & (~INFO_BITS_MASK));

        float dDownLandZ1 = (1.f - dx1) * (m_arrusHightMapData[nX][nY + 1] & (~INFO_BITS_MASK)) + (dx1) * (m_arrusHightMapData[nX + 1][nY + 1] & (~INFO_BITS_MASK));
...
inline int fastround_positive(float f) {
    ...
    i = (int)(f + 0.5f);
    return i;
} // note: only positive numbers works correct
#endif
...
bool CHighMap::LoadHighMap(const char* file_name, ICryPak* pCryPak) { // Terrain Land dimensions = file dimensions + 1
    ...
    int nBytesReaded = pCryPak->FRead(pTmpBuff, sizeof(pTmpBuff[0]),
                                      (CTerrain::GetTerrainSize() / CTerrain::GetHeightMapUnitSize()) * (CTerrain::GetTerrainSize() / CTerrain::GetHeightMapUnitSize()), f);
