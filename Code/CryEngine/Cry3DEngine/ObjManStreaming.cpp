...
void CObjManager::PreloadNearObjects() {
    ...
    {                                                                          // mark this and neighbor sectors
        ...
        for (ObjectsMap::iterator it = m_lstLoadedObjects.begin(); it != m_lstLoadedObjects.end(); ++it) {
            if ((*it)->m_bUseStreaming && !(*it)->GetLeafBuffer() && (*it)->m_nMarkedForStreamingFrameId > GetFrameID() + 10) { // streaming
                CStatObj::m_fStreamingTimePerFrame -= GetTimer()->GetAsyncCurTime();
                bool bRes = (*it)->Load((*it)->m_szFileName, (*it)->m_szGeomName[0] ? (*it)->m_szGeomName : 0, (*it)->m_eVertsSharing, (*it)->m_bLoadAdditinalInfo,
                                        (*it)->m_bKeepInLocalSpace, false);
...
void CObjManager::CheckUnload() {
    if (GetCVars()->e_stream_cgf)
        for (ObjectsMap::iterator it = m_lstLoadedObjects.begin(); it != m_lstLoadedObjects.end(); ++it) {
            ...
            if (pStatObj->m_bUseStreaming && pStatObj->GetLeafBuffer() && max(pStatObj->m_nLastRendFrameId, pStatObj->m_nMarkedForStreamingFrameId) < GetFrameID() - 100) {
                int p;
                for (p = 0; p < 2; p++) { // check is phys geometry still in use
