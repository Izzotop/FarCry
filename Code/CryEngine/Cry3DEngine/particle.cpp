...
bool CParticle::Update(const PartProcessParams& PPP) {
    ...
    if (m_pParams->nParticleFlags & PART_FLAG_SPACELOOP && m_pEmitter) { // process space loop
    ...
    if ((m_pParams->nParticleFlags & PART_FLAG_SPACELIMIT) && m_pEmitter) {
        ...
        if (vNextPos.x < vBoxMin.x || vNextPos.y < vBoxMin.y || vNextPos.z < vBoxMin.z || vNextPos.x > vBoxMax.x || vNextPos.y > vBoxMax.y || vNextPos.z > vBoxMax.z)
            return false;
    }

    if (m_pPhysEnt) { // use phys engine to control particle
        ...
        m_vAngles *= 180 / gf_PI;
    } else { // process movement, size and rotation
        if (!(m_pParams->nParticleFlags & PART_FLAG_LINEPARTICLE)) {
            ...
            if (m_pParams->fTurbulenceSize) {
                m_vPos += Vec3d(cry_cosf(fAge * m_pParams->fTurbulenceSpeed), cry_sinf(fAge * m_pParams->fTurbulenceSpeed), 0) * PPP.fFrameTime * (max(1.f, fAge)) *
    ...
    bool bContact = false;
    if (m_pPhysEnt) { // use real physics
    ...
    { // if inside the building
      if(pCVars->e_particles_indoor_collisions)
      {
        ...
        if (PPP.pTerrain && _finite(m_vPos.x) && _finite(m_vPos.y) && m_vPos.z < PPP.pTerrain->GetZApr(m_vPos.x, m_vPos.y) &&
            m_vPos.z > PPP.pTerrain->GetZApr(m_vPos.x, m_vPos.y) - 0.1f && !(m_pParams->nParticleFlags & PART_FLAG_SPACELOOP)) { // collide with terrain
            int x = (int)m_vPos.x;
            int y = (int)m_vPos.y;
    ...
    if (bContact) { // Run child process on collision
    ...
    if (m_pParams->pChild && m_pParams->pChild->nCount > 0 && m_pParams->fChildSpawnPeriod >= 0 && PPP.fCurTime > m_fChildSpawnLastTime + m_pParams->fChildSpawnPeriod) {
...
//                //last = last*(1.0f-a) + (prev+Vec3(0,0,0))*a;
    ...
    if ((m_pParams->nParticleFlags & PART_FLAG_SPACELIMIT) && m_pEmitter) {
        ...
        if (m_pParams->pStatObj) { // get world space radius of the object
        ...
        if (vHitPos.x < vBoxMin.x || vHitPos.y < vBoxMin.y || vHitPos.z < vBoxMin.z || vHitPos.x > vBoxMax.x || vHitPos.y > vBoxMax.y || vHitPos.z > vBoxMax.z)
            return false;
    }
