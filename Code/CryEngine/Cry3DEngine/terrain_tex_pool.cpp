...
int CTexturePool::MakeTexture(uchar* pData, int nTexSize, CSectorInfo* pNewSectorInfo, bool bMakeUncompressedForEditing) {
    if (!GetCVars()->e_terrain_texture_pool) { // if pool not used
        return GetRenderer()->DownLoadToVideoMemory(pData, nTexSize, nTexSize, eTF_DXT1, bMakeUncompressedForEditing ? eTF_8888 : eTF_DXT1, 0, false, FILTER_LINEAR);
    }
...
const char* CTexturePool::GetStatusText(CTerrain* pTerrain) {
    ...
    int arrTexMem[2] = {nInUseTexNum[0] * 128 * 128 * 3 / 6, nInUseTexNum[1] * 64 * 64 * 3 / 6};
    ...
    if (!GetCVars()->e_terrain_texture_pool)
        ...
        snprintf(szText, sizeof(szText), "TerrTexPoolStats %d/%d (%d/%d) / %d/%d (%d/%d)", nInUseTexNum[0], m_TexturePool[0].Count(), arrTexMem[0] / 1024,
                 m_TexturePool[0].Count() * 128 * 128 * 3 / 6 / 1024, nInUseTexNum[1], m_TexturePool[1].Count(), arrTexMem[1] / 1024,
                 m_TexturePool[1].Count() * 64 * 64 * 3 / 6 / 1024);
