...
CLightManager::~CLightManager() {
    for (LightsMap::iterator it = m_arrLights->begin(); it != m_arrLights->end(); ++it) { // free lights lists
...
void CLightManager::GetLightBounds(CDLight* pLight, int& min_x, int& min_y, int& max_x, int& max_y) {
    ...
    if (min_x < 0)
        min_x = 0;
    else if (min_x >= nTableSize)
        min_x = nTableSize - 1;
    if (min_y < 0)
        min_y = 0;
    else if (min_y >= nTableSize)
        min_y = nTableSize - 1;
    if (max_x < 0)
        max_x = 0;
    else if (max_x >= nTableSize)
        max_x = nTableSize - 1;
    if (max_y < 0)
        max_y = 0;
    else if (max_y >= nTableSize)
        max_y = nTableSize - 1;
}
...
void CLightManager::AddLight(CDLight* pLight) {
    ...
    for (int x = min_x; x <= max_x; x++)
        for (int y = min_y; y <= max_y; y++) {
            ...
            else { // allocate new set if needed
...
int CLightManager::MakeLMaskFromPositionAndActivateLights(const Vec3d vObjPos, const float fObjRadius, IEntityRender* pEntityRender, int nMaxLightBitsNum,
    ...
    for (LightsSet::iterator itLightsSet = m_setTmpDL_MMFP.begin(); itLightsSet != m_setTmpDL_MMFP.end(); ++itLightsSet) {
        ...
        if (pEntityRender && pDLight->m_Flags & DLF_LM && pEntityRender->GetRndFlags() & ERF_USELIGHTMAPS && pEntityRender->HasLightmap(0)) { // in case of lightmaps
        ...
        if (pDLight->m_Flags & DLF_PROJECT && pEntityRender) { // check projector frustum
            ...
            if (GetCVars()->e_projector_exact_test) { // test
        ...
        fLightAmount *=
            (pDLight->m_Color.r + pDLight->m_Color.g + pDLight->m_Color.b) * 0.333f + (pDLight->m_SpecColor.r + pDLight->m_SpecColor.g + pDLight->m_SpecColor.b) * 0.333f;
                ...
                if ((pDLight->m_Flags & DLF_THIS_AREA_ONLY) /*|| (pDLight->m_Flags & DLF_SUN)*/) {
                    if (pEntityRender->GetEntityVisArea() && pDLight->m_pOwner != (IEntityRender*)-1) {
                        if (pDLight->m_pOwner && pDLight->m_pOwner->GetEntityRS() && pDLight->m_pOwner->m_pVisArea) {
                            ...
                            if (pEntityRender->GetEntityVisArea() != pLightArea) { // try also portal volumes
...
float CLightEntity::GetRenderRadius() const {
