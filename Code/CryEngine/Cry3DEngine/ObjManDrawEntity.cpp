...
void CObjManager::ProcessActiveShadowReceiving(IEntityRender* pEnt, float fEntDistance, CDLight* pLight, bool bLMapGeneration) {
    ...
    if (pLsource->GetShadowMapFrustum()) {
        if (nTexSize != pLsource->GetShadowMapFrustum()->nTexSize || !IsEquivalent(pLsource->vObjSpaceSrcPos, vObjSpaceLightPos, 0.001f) || pEnt->HasChanged() ||
            pLsource->GetShadowMapFrustum()->depth_tex_id == 0) {
            ...
            ShadowMapFrustum* lof =
                MakeEntityShadowFrustum(pLsource->GetShadowMapFrustum(), pLsource, pEnt, EST_DEPTH_BUFFER, SMC_DYNAMICS | SMC_STATICS | SMC_ALLOW_PASSIVE_SHADOWMAP_CASTERS);
    ...
    assert(pLsource);
    {
        ...
        if (LightSourceInfo.m_pLS->m_LightFrustums.Count() && LightSourceInfo.m_pLS->m_LightFrustums[0].pEntityList &&
...
void CObjManager::RequestEntityShadowMapGeneration(IEntityRender* pEntityRnd) {
    ...
    GetRenderer()->EF_AddEf(0, m_p3DEngine->m_pREShadowMapGenerator, m_p3DEngine->m_pSHShadowMapGen, nullptr, pObj, 0);

    if (GetCVars()->e_shadow_maps_frustums && pEntityRnd->GetShadowMapFrustumContainer())
        pEntityRnd->GetShadowMapFrustumContainer()->m_LightFrustums.Get(0)->DrawFrustum(GetRenderer(), pEntityRnd->GetPos(), 1.f);

    if (GetCVars()->e_shadow_maps_frustums && pEntityRnd->GetShadowMapFrustumContainerPassiveCasters())
        pEntityRnd->GetShadowMapFrustumContainerPassiveCasters()->m_LightFrustums.Get(0)->DrawFrustum(GetRenderer(), pEntityRnd->GetPos(), 1.f);
}
...
void CObjManager::RenderObjectVegetationNonCastersNoFogVolume(IEntityRender* pEntityRS, uint nDLightMask, const CCamera& EntViewCamera, bool bNotAllInFrustum, float fMaxViewDist,
                                                              IEntityRenderInfo* pEntInfo) {
    assert(pEntInfo);
...
void CObjManager::RenderObject(IEntityRender* pEntityRS, int nFogVolumeID, uint nDLightMask, bool bLMapGeneration, const CCamera& EntViewCamera, Vec3d* pvAmbColor,
                               Vec3d* pvDynAmbColor, VolumeInfo* pFogVolume, bool bNotAllInFrustum, float fMaxViewDist, IEntityRenderInfo* pEntInfo) {
    ...
    CDLight* pStrongestLightForTranspGeom = nullptr;
    if (pCVars->e_stencil_shadows && (nRenderFlags & ERF_CASTSHADOWVOLUME || nRenderFlags & ERF_CASTSHADOWMAPS) && fEntRadius) { // adjust bbox by shadow before frustum check
        list2<CDLight>* pSources = m_p3DEngine->GetDynamicLightSources();
        if (nDLightMask == 1 && pSources->Count() && pSources->GetAt(0).m_Flags & DLF_SUN)
        ...
        bool bAdjustBBoxByShadowSize = (nRenderFlags & ERF_CASTSHADOWVOLUME) && pStrongestShadowLight && (pStrongestShadowLight->m_Flags & DLF_CASTSHADOW_VOLUME);
        bAdjustBBoxByShadowSize |= (nRenderFlags & ERF_CASTSHADOWMAPS) && pStrongestShadowLight && (pStrongestShadowLight->m_Flags & DLF_CASTSHADOW_MAPS);

        bool bShadowIsVisible = false;
        if (pStrongestShadowLight) { // shadow bbox frustum test
    ...
    if (pCVars->e_shadow_maps && m_nRenderStackLevel == 0 && (nRenderFlags & ERF_RECVSHADOWMAPS || nRenderFlags & ERF_CASTSHADOWMAPS) && pStrongestShadowLight &&
        ...
        fEntDistance < (pEntityRS->GetRenderRadius() * GetCVars()->e_shadow_maps_view_dist_ratio) && (!pEntityRS->GetLight() || pEntityRS->GetContainer()) &&
        (pEntityRS->GetLight() != pStrongestShadowLight) && (pStrongestShadowLight->m_pOwner != pEntityRS) && // do not cast from it own light
        (!(pEntStatObj = (CStatObj*)pEntityRS->GetEntityStatObj(0, nullptr, true)) || pEntStatObj->GetRenderTrisCount()) && pEntityRS->IsEntityHasSomethingToRender()) {
    ...
    if (pEntityRS->GetShadowFrame(m_nRenderStackLevel) != (unsigned short)GetFrameID())
        if (pCVars->e_stencil_shadows && pStrongestShadowLight && fEntRadius && !GetCVars()->e_debug_lights) { // make list of entities for each light casting shadow volume
            uint nNewMask = DrawParams.nDLightMask;
            for (int i = 0; DrawParams.nDLightMask; i++) {
                if (DrawParams.nDLightMask & 1) { // todo: remove EF_Query
    ...
    if (bEntityBodyVisible) { // draw entity components (ambient(z-pass) with lights without shadowvolumes and fog)
        ...
        if (bFogNeeded && nDLightMaskBeforeLightPassesSeparation == DrawParams.nDLightMask && !pEntityRS->GetEntityVisArea()) // can make overlays double fogged
        {
        ...
        if (bFogNeeded && nFogVolumeID && !DrawParams.nFogVolumeID) { // render separate fog pass if still was not rendered
    ...
    if (!m_nRenderStackLevel && fEntDistance < COVERAGEBUFFER_OCCLUDERS_MAX_DISTANCE && pCVars->e_cbuffer && !bLMapGeneration &&
        bEntityBodyVisible) { // only use occlusion volume of first entity object
        Matrix44 objMatrix;
        ...
        if (pStatObj && pStatObj->GetOcclusionVolume(plstOcclVolVerts, plstOcclVolInds) && pCVars->e_cbuffer) {
            m_pCoverageBuffer->AddMesh(&(*plstOcclVolVerts)[0], (*plstOcclVolVerts).Count(), &(*plstOcclVolInds)[0], (*plstOcclVolInds).Count(), &objMatrix);
        }
    }
}
...
void CObjManager::DrawEntitiesLightPass() {
    ...
    for (int nLightId = 0; nLightId < MAX_LIGHTS_NUM; nLightId++) {
        ...
        for (int nEntId = 0; nEntId < m_lstLightEntities[nLightId].Count(); nEntId++) {
            ...
            if (!(pDLight->m_Flags & DLF_LM && pEntityRS->GetRndFlags() & ERF_USELIGHTMAPS && pEntityRS->HasLightmap(0)) || pDLight->m_SpecColor != Col_Black) { // light pass
                SRendParams DrawParams;
            ...
            if (pEntityRS == (IEntityRender*)-1) {
                if (GetVisAreaManager()->IsOutdoorAreasVisible())
                    m_pTerrain->RenderDLightOnHeightMap(pDLight); // light on terrain
                       ...
                       pDLight->m_SpecColor != Col_Black) { // entity light pass
                if (!(pEntityRS->GetRndFlags() & ERF_SELFSHADOW))
                    continue;
        ...
        if (bUseStencilStateTest) // todo: add shadow-object bbox test
            for (int nEntId = 0; nEntId < m_lstLightEntities[nLightId].Count(); nEntId++) {
                IEntityRender* pEntityRS = m_lstLightEntities[nLightId][nEntId];
                if (pEntityRS != (IEntityRender*)-1)
                    if (pEntityRS->GetRndFlags() & ERF_USELIGHTMAPS && pEntityRS->HasLightmap(0) && pDLight->m_Flags & DLF_LM && pDLight->m_Flags & DLF_CASTSHADOW_VOLUME) {
                        if (pDLight->m_Flags & DLF_DIRECTIONAL) { // aditionall ambient pass with stencil test for outdoor brushes
                            if (pEntityRS->GetEntityVisArea())
                            ...
                            for (int nCasterId = 0; nCasterId < m_lstLightEntities[nLightId].Count(); nCasterId++) {
                                if (m_lstLightEntities[nLightId][nCasterId]->GetRndFlags() & ERF_CASTSHADOWVOLUME && pEntityRS != m_lstLightEntities[nLightId][nCasterId] &&
...
void CObjManager::SetupEntityShadowMapping(IEntityRender* pEnt, SRendParams* pDrawParams, bool bLMapGeneration, float fEntDistance, CDLight* pDLight) {
    ...
    if (pEnt->GetRndFlags() & ERF_CASTSHADOWMAPS) { // make shadow frustum to project shadow to the world (and this entity)
        ProcessShadowMapCasting(pEnt, pDLight);
        if (GetCVars()->e_entities_debug)
            m_lstDebugEntityList.Add(pEnt);
    } else if (pEnt->GetEntityRS()->pShadowMapInfo->pShadowMapFrustumContainer && !bLMapGeneration) { // unmake if not needed
    ...
    if (pEnt->GetRndFlags() & ERF_RECVSHADOWMAPS_ACTIVE &&
        GetCVars()->e_active_shadow_maps_receving) { // make frustum containing all potential shadow casters (even not marked for shadow casting)
        ProcessActiveShadowReceiving(pEnt, fEntDistance, pDLight, bLMapGeneration);
    } else if (pEnt->GetRndFlags() & ERF_RECVSHADOWMAPS && GetCVars()->e_shadow_maps_receiving) { // make list of potential active shadow map casters to cast to this entity
        MakeShadowMapInstancesList(pEnt, fEntDistance, pEnt->GetEntityRS()->pShadowMapInfo->pShadowMapCasters, SMC_STATICS | SMC_DYNAMICS, pDLight);
    ...
    if (pLsList && pLsList->Count() && pLsList->Get(0)->m_pLS && pLsList->Get(0)->m_pLS->GetShadowMapFrustum()) {
        ...
        if (pLsList->Count() == 1 && pLsList->Get(0)->m_pLS->GetShadowMapFrustum()->pOwner == pLsList->Get(0)->m_pReceiver &&
            !(pLsList->Get(0)->m_pLS->GetShadowMapFrustum()->dwFlags & SMFF_ACTIVE_SHADOW_MAP))
            assert(pDrawParams->pShadowMapCasters == nullptr); // skip single self shadowing pass
        else
    ...
    assert(pEnt->GetRndFlags() & ERF_RECVSHADOWMAPS || ((pDrawParams->pShadowMapCasters == 0) || (pDrawParams->pShadowMapCasters->Count() == 1)));
...
CFColor CObjManager::CalcShadowOnTerrainColor(float fAlpha, bool bLMapGeneration) { // calc shadow color
    ...
    return CFColor(min(255.f, vAverColor.x), min(255.f, vAverColor.y), min(255.f, vAverColor.z));
}
...
void CObjManager::RenderEntityShadowOnTerrain(IEntityRender* pEntityRnd, bool bLMapGeneration, CREShadowMapGen* pREShadowMapGenerator) {
    ...
    if (pEntityRnd->GetShadowMapFrustumContainer() && pEntityRnd->GetShadowMapFrustumContainer()->m_LightFrustums.Count() && pRendState->pShadowMapInfo->pShadowMapCasters &&
        fEntRenderRadius) {
        Vec3d vEntBoxMin, vEntBoxMax;
        ...
        bool bREAdded = false;
        if (!pEntityRnd->GetEntityVisArea() && GetVisAreaManager()->IsOutdoorAreasVisible() && (!(pEntityRnd->GetRndFlags() & ERF_CASTSHADOWINTOLIGHTMAP) || bLMapGeneration) &&
            fAlpha > 0) { // generate shadow map and render terrain shadow pass
            ...
            bool bRecalcLeafBuffers =
                (bLMapGeneration || (!IsEquivalent(pRendState->pShadowMapInfo->vPrevTerShadowPos, vPos)) || pRendState->pShadowMapInfo->fPrevTerShadowRadius != fQuadRadius);
            ...
            bREAdded =
                m_pTerrain->RenderAreaLeafBuffers(vPos, fQuadRadius, 0, pRendState->pShadowMapInfo->pShadowMapLeafBuffersList->GetElements(),
                                                  pRendState->pShadowMapInfo->pShadowMapLeafBuffersList->Count(), pObj, m_pTerrain->m_pTerrainShadowPassEf, bRecalcLeafBuffers,
                                                  "EntityShadowOnTerrain", 0, 0, pRendState->pShadowMapInfo->pShadowMapFrustumContainer->GetShadowMapFrustum(),
                                                  &Vec3d(pEntityRnd->GetPos()), (pEntityRnd->GetEntityRenderType() == eERType_Vegetation) ? pEntityRnd->GetScale() : 1.f);
        }

        if (pREShadowMapGenerator && !bREAdded) { // just generate shadow map to use in indoors
...
float CObjManager::CalculateEntityShadowVolumeExtent(IEntityRender* pEntity, CDLight* pDLight) {
    ...
    if (pArea && pDLight->m_Flags & DLF_THIS_AREA_ONLY) { // limit light radius by area box
...
void CObjManager::ProcessEntityParticles(IEntityRender* pEnt, float fEntDistance) {
    ...
    for (int i = 0; i < 2; i++) { // anim objects
...
void CObjManager::MakeShadowBBox(Vec3d& vBoxMin, Vec3d& vBoxMax, const Vec3d& vLightPos, float fLightRadius, float fShadowVolumeExtent) {
    ...
    Vec3d arrVerts3d[8] = {Vec3d(vBoxMin.x, vBoxMin.y, vBoxMin.z), Vec3d(vBoxMin.x, vBoxMax.y, vBoxMin.z), Vec3d(vBoxMax.x, vBoxMin.y, vBoxMin.z),
                           Vec3d(vBoxMax.x, vBoxMax.y, vBoxMin.z), Vec3d(vBoxMin.x, vBoxMin.y, vBoxMax.z), Vec3d(vBoxMin.x, vBoxMax.y, vBoxMax.z),
                           Vec3d(vBoxMax.x, vBoxMin.y, vBoxMax.z), Vec3d(vBoxMax.x, vBoxMax.y, vBoxMax.z)};
...
bool CObjManager::ProcessShadowMapCasting(IEntityRender* pEnt, CDLight* pDLight) {
    if (pEnt->GetEntityRenderType() == eERType_Vegetation) { // shadow casting already prepared at loading time, todo: move it in real-time
        ...
        if (nTexSize != pLsource->GetShadowMapFrustum()->nTexSize || !IsEquivalent(pLsource->vObjSpaceSrcPos, vObjSpaceLightPos, 0.01f) || pEnt->HasChanged() ||
            pLsource->GetShadowMapFrustum()->depth_tex_id == 0) {
            pLsource->GetShadowMapFrustum()->bUpdateRequested = true;
