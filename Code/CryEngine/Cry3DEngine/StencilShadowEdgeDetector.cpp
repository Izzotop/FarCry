...
CStencilShadowEdgeDetector::CStencilShadowEdgeDetector() : m_pConnectivity(nullptr), m_pModelVertices(nullptr) {
...
void CStencilShadowEdgeDetector::reinit(const IStencilShadowConnectivity* pConnectivity, const Vec3d* pDeformedVertices) {
...
bool CStencilShadowEdgeDetector::IsFaceTurnedToLight(unsigned nFace, const Vec3d& vLight) {
    ...
    DWORD dwIndex[3] = {face.getVertex(0), face.getVertex(1), face.getVertex(2)};
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CStencilShadowEdgeDetector::detectShadowEdges() {
    ...
    for (nEdge = 0; nEdge < nEdgeCount; ++nEdge) {
        ...
        // check the edge
        switch (CheckEdgeType(rEdge.getFace(0).getFaceIndex(), rEdge.getFace(1).getFaceIndex())) {
        case nET_Boundary:
    ...
    for (nEdge = 0; nEdge < nEdgeCount; ++nEdge) {
        ...
        // check the edge
        switch (CheckOrphanEdgeType(rEdge.getFace().getFaceIndex())) {
        case nET_Boundary:
            AddEdge(rEdge[0], rEdge[1]);
            break;
        case nET_ReverseBoundary:
...
CStencilShadowEdgeDetector::EdgeTypeEnum CStencilShadowEdgeDetector::CheckEdgeType(unsigned nFace0, unsigned nFace1) {
...
CStencilShadowEdgeDetector::EdgeTypeEnum CStencilShadowEdgeDetector::CheckOrphanEdgeType(unsigned nFace0) {
    assert(m_arrFaceOrientations.size());
...
void CStencilShadowEdgeDetector::BuildSilhuetteFromPos(const IStencilShadowConnectivity* pConnectivity, const Vec3d& invLightPos, const Vec3d* inpDeformedVertices) {
    assert(pConnectivity);
