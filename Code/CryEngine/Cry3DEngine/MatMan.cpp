...
CMatMan::~CMatMan() {
    ...
    if (nNotUsed)
        GetLog()->Log("Warning: CMatMan::~CMatMan: %d(%d) of %d materials was not used in level", nNotUsedParents, nNotUsed, m_mtlSet.size());
}
...
inline CFColor ToCFColor(const Vec3& col) {
    return CFColor(col);
}
...
//////////////////////////////////////////////////////////////////////////
bool CMatMan::LoadMaterialShader(IMatInfo* pMtl, const char* sShader, int mtlFlags, unsigned int nShaderGenMask, SInputShaderResources& sr, SLightMaterial& lm,
                                 XmlNodeRef& publicsNode) {
