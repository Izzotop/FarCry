...
bool C3DEngine::WriteNodes(TArray<CHUNK_HEADER>& Chunks, TArray<NODE_CHUNK_DESC>& Nodes, FILE* out, TArray<SNodeInfo>& NI, int& MatChunk, int& ExpFrame,
                           std::vector<IStatObj*>& pObjs) {
    CHUNK_HEADER ch;
    int numMeshs = 0;
    NODE_CHUNK_DESC nd;
    SNodeInfo ni;
        ...
        for (int j = 0; j < co->m_lstLSources.Count(); j++) {
            ...
            */
            ...
            if (dl->m_pLightImage) {
                ...
                lcd.type = LT_OMNI;
            } else if (dl->m_Flags & DLF_DIRECTIONAL)
                lcd.type = LT_DIRECT;
