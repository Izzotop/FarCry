...
bool CDecalManager::AdjustDecalPosition(CryEngineDecalInfo& DecalInfo, bool bMakeFatTest) {
    ...
    if (fabs(vObjSpaceHitDirection.Dot(Vec3d(0, 0, 1))) > 0.999f) { // horiz surface
        vRi = Vec3d(0, 1, 0);
        vUp = Vec3d(1, 0, 0);
    } else {
    ...
    if (pLB && pLB->m_pSecVertBuffer && pLB->m_SecVertCount &&
        RayLeafBufferIntersection(pLB, vObjSpacePos, vObjSpaceHitDirection * 0.1f, vObjSpaceOutPos, vObjSpaceOutNormal)) { // now check that none of decal sides will not fly in the air
        Vec3d vDecalNormal = vObjSpaceOutNormal * DecalInfo.fSize;
        if (!bMakeFatTest ||
            (RayLeafBufferIntersection(pLB, vObjSpaceOutPos + vUp, -vDecalNormal, vTmp, vTmp) && RayLeafBufferIntersection(pLB, vObjSpaceOutPos - vUp, -vDecalNormal, vTmp, vTmp) &&
...
struct HitPosInfo {
    HitPosInfo() {
        memset(this, 0, sizeof(HitPosInfo));
    }
    Vec3d vPos, vNormal;
    float fDistance;
};
...
bool CDecalManager::RayLeafBufferIntersection(CLeafBuffer* pLeafBuffer, const Vec3d& vInPos, const Vec3d& vInDir, Vec3d& vOutPos, Vec3d& vOutNormal) {
    ...
    if (nResultCount) { // return closest to the shooter
...
void CDecalManager::Spawn(CryEngineDecalInfo DecalInfo, float fMaxViewDistance, CTerrain* pTerrain) {
    ...
    */
    ...
    if (DecalInfo.fSize > 1 && GetCVars()->e_decals_neighbor_max_life_time) { // force near decals to fade faster
        float fCurrTime = GetTimer()->GetCurrTime();
        for (int i = 0; i < DECAL_COUNT; i++) {
            if (m_arrbActiveDecals[i]) {
    ...
    m_arrDecals[m_nCurDecal].m_pBigDecalLeafBuffer = nullptr;
    ...
    float fObjScale = 1.f;
    if (DecalInfo.pDecalOwner && DecalInfo.nPartID >= 0 && DecalInfo.fSize > COMPLEX_DECAL_MIN_SIZE) { // get object pointer
        Matrix44 objMat;
        ...
        if (pEntObject) {
            ...
            // make decal geometry
            m_arrDecals[m_nCurDecal].m_pBigDecalLeafBuffer = MakeBigDecalLeafBuffer(pSourceLeafBuffer, DecalInfo.vPos, DecalInfo.fSize, DecalInfo.vNormal, DecalInfo.nTid);
    ...
    } else if (DecalInfo.pDecalOwner && (!DecalInfo.pDecalOwner->IsStatic() || DecalInfo.pDecalOwner->GetEntityRenderType() == eERType_Vegetation) &&
               DecalInfo.nPartID >= 0) { // transform decal from world space into entity space
        Matrix44 objMat;
        IStatObj* pEntObject = DecalInfo.pDecalOwner->GetEntityStatObj(DecalInfo.nPartID, &objMat);
        assert(pEntObject);
        objMat.Invert44();
        ...
        if (DecalInfo.fSize > COMPLEX_DECAL_MIN_SIZE && !DecalInfo.pDecalOwner && (DecalInfo.vPos.z - pTerrain->GetZApr(DecalInfo.vPos.x, DecalInfo.vPos.y)) < DecalInfo.fSize) {
    ...
    if (fabs(DecalInfo.vNormal.Dot(Vec3d(0, 0, 1))) > 0.999f) { // horiz surface
    ...
    if (!DecalInfo.pDecalOwner && DecalInfo.fSize <= COMPLEX_DECAL_MIN_SIZE) {
        ...
        if (GetPhysicalWorld()->RayWorldIntersection(DecalInfo.vPos - DecalInfo.vNormal * fSize + m_arrDecals[m_nCurDecal].m_vUp * fSize, DecalInfo.vNormal * fSize * 2, ent_all, rwi_stop_at_pierceable, &hit, 1) &&
            GetPhysicalWorld()->RayWorldIntersection(DecalInfo.vPos - DecalInfo.vNormal * fSize - m_arrDecals[m_nCurDecal].m_vUp * fSize, DecalInfo.vNormal * fSize * 2, ent_all, rwi_stop_at_pierceable, &hit, 1) &&
            GetPhysicalWorld()->RayWorldIntersection(DecalInfo.vPos - DecalInfo.vNormal * fSize + m_arrDecals[m_nCurDecal].m_vRight * fSize, DecalInfo.vNormal * fSize * 2, ent_all, rwi_stop_at_pierceable, &hit, 1) &&
            GetPhysicalWorld()->RayWorldIntersection(DecalInfo.vPos - DecalInfo.vNormal * fSize - m_arrDecals[m_nCurDecal].m_vRight * fSize, DecalInfo.vNormal * fSize * 2, ent_all, rwi_stop_at_pierceable, &hit, 1)
        ) {
            int t = 0; // ok
        } else {
...
void CDecalManager::SubmitLightHolesToRenderer() {
    ...
    for (int i = 0; i < m_LightHoles.Count(); i++) {
        GetRenderer()->EF_SetLightHole(m_LightHoles[i].m_vPos, m_LightHoles[i].m_vFront, m_LightHoles[i].m_nTexId, m_LightHoles[i].m_fSize);
    }
    m_LightHoles.Clear();
}
        ...
        if (m_arrbActiveDecals[i]) {
            if (m_arrDecals[i].m_pDecalOwner == pEntityRender) {
                ...
                if (m_arrDecals[i].m_pBigDecalLeafBuffer) {
                    ...
                    m_arrDecals[i].m_pBigDecalLeafBuffer = nullptr;
                }
                m_arrDecals[i].m_pDecalOwner = nullptr;
            }
        }
}
...
CLeafBuffer* CDecalManager::MakeBigDecalLeafBuffer(CLeafBuffer* pSourceLeafBuffer, Vec3d vPos, float fRadius, Vec3d vProjDir, int nTexID) {
    ...
    CLeafBuffer* pLeafBuffer = GetRenderer()->CreateLeafBufferInitialized(EmptyVertBuffer.GetElements(), EmptyVertBuffer.Count(), VERTEX_FORMAT_P3F_COL4UB,
                                                                          lstIndices.GetElements(), lstIndices.Count(), R_PRIMV_TRIANGLES, "BigDecal", eBT_Static, 1, nTexID);
...
CLeafBuffer* CDecalManager::MakeBigDecalLeafBuffer(CLeafBuffer* pSourceLeafBuffer, Vec3d vPos, float fRadius, Vec3d vProjDir, int nTexID) {
    ...
    CLeafBuffer* pLeafBuffer = GetRenderer()->CreateLeafBufferInitialized(VertBuffer.GetElements(), VertBuffer.Count(), VERTEX_FORMAT_P3F_N, lstIndices.GetElements(),
                                                                          lstIndices.Count(), R_PRIMV_TRIANGLES, "BigDecal", eBT_Static, 1, nTexID);
...
void CDecalManager::DeleteDecalsInRange(Vec3d vBoxMin, Vec3d vBoxMax, bool bDeleteBigTerrainDecals) {
    for (int i = 0; i < DECAL_COUNT; i++)
        if (m_arrbActiveDecals[i] && !m_arrDecals[i].m_bOnTheGround) {
            if (!m_arrDecals[i].m_pDecalOwner && m_arrDecals[i].m_vPos.x < vBoxMax.x && m_arrDecals[i].m_vPos.y < vBoxMax.y && m_arrDecals[i].m_vPos.z < vBoxMax.z &&
                m_arrDecals[i].m_vPos.x > vBoxMin.x && m_arrDecals[i].m_vPos.y > vBoxMin.y && m_arrDecals[i].m_vPos.z > vBoxMin.z) {
                m_arrbActiveDecals[i] = false;
                GetRenderer()->DeleteLeafBuffer(m_arrDecals[i].m_pBigDecalLeafBuffer);
                m_arrDecals[i].m_pBigDecalLeafBuffer = nullptr;
            }
