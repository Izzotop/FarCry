...
bool CTerrain::AddStaticObject(int nObjectID, const Vec3d& vPos, const float fScale, CObjManager* pObjManager, uchar ucBright) {
    ...
    if (nObjectID < 0 || nObjectID >= pObjManager->m_lstStaticTypes.Count()) { // object id is out of range
        /*assert(0); */ return 0;
    }
...
void CTerrain::SetTerainHightMapBlock(int x1, int y1, int nSizeX, int nSizeY, ushort* TerrainBlock, ushort nUpdateMask) {
    ...
    for (int x = x1; x < x1 + nSizeX; x++)
        for (int y = y1; y < y1 + nSizeY; y++) {
            ...
            // update vert buffers
            CSectorInfo* info =
                m_arrSecInfoTable[x * CTerrain::GetHeightMapUnitSize() / CTerrain::GetSectorSize()][y * CTerrain::GetHeightMapUnitSize() / CTerrain::GetSectorSize()];
...
int CTerrain::LockSectorTexture(int nSectorOriginX, int nSectorOriginY, int& nTexDim) {
    if (nSectorOriginX / CTerrain::GetSectorSize() < 0 || nSectorOriginY / CTerrain::GetSectorSize() < 0 ||
        nSectorOriginX / CTerrain::GetSectorSize() >= CTerrain::GetSectorsTableSize() || nSectorOriginY / CTerrain::GetSectorSize() >= CTerrain::GetSectorsTableSize()) {
