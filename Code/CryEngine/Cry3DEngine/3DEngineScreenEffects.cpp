...
// some helper macros
#define GET_ACTIVEPARAMETER(pProcess) *((bool*)m_pREScreenProcess->mfGetParameter((pProcess), SCREENPROCESS_ACTIVE))

#define GET_CVAR(pVar) (GetConsole()->GetCVar((pVar)))
...
void C3DEngine::SetScreenFxParam(const char* pEffectName, const char* pEffectParam, void* pValue) {
    ...
    if (!stricmp(pEffectName, "ScreenFade")) {
        ...
        } else if (!stricmp(pEffectParam, "ScreenPreFadeTime")) // set screen pre fade flag (count number of frames, before start fading)
        {
            m_pREScreenProcess->mfSetParameter(SCREENPROCESS_FADE, SCREENPROCESS_PRETRANSITIONTIME, pValue);
        }
    } else if (!stricmp(pEffectName, "ScreenBlur")) // set parameters for screen blur
    {
        ...
        } else if (!stricmp(pEffectParam, "ScreenBlurColorRed")) // screen blur color red..
        {
            m_pREScreenProcess->mfSetParameter(SCREENPROCESS_BLUR, SCREENPROCESS_BLURCOLORRED, pValue);
        } else if (!stricmp(pEffectParam, "ScreenBlurColorGreen")) // screen blur color green..
        {
            m_pREScreenProcess->mfSetParameter(SCREENPROCESS_BLUR, SCREENPROCESS_BLURCOLORGREEN, pValue);
        } else if (!stricmp(pEffectParam, "ScreenBlurColorBlue")) // screen blur color blue..
        {
...
int C3DEngine::GetScreenFxParam(const char* pEffectName, const char* pEffectParam, void*& pValue) {
    ...
    } else if (!stricmp(pEffectName, "ScreenBlur")) // get parameters for screen blur
    {
        if (!stricmp(pEffectParam, "ScreenBlurAmount")) // screen blur amount
        {
    ...
    } else if (!stricmp(pEffectName, "FlashBang")) // get parameters for flashbang
    {
        if (!stricmp(pEffectParam, "FlashBangTimeScale")) // flashbang timeout
        {
        ...
        } else if (!stricmp(pEffectParam, "FlashBangTimeOut")) // get current time out time
        {
...
void C3DEngine::ProcessScreenEffects() {
    ...
    if (!pDisableSfx->GetIVal()) {
        ...
        if (GET_ACTIVEPARAMETER(SCREENPROCESS_FLASHBANG) || GET_ACTIVEPARAMETER(SCREENPROCESS_COLORTRANSFER) || GET_ACTIVEPARAMETER(SCREENPROCESS_BLUR) ||
            GET_ACTIVEPARAMETER(SCREENPROCESS_NIGHTVISION) || GET_ACTIVEPARAMETER(SCREENPROCESS_HEATVISION) || GET_ACTIVEPARAMETER(SCREENPROCESS_GLARE) ||
