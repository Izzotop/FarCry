...
void CObjManager::MakeShadowCastersListInArea(CBasicArea* pArea, IEntityRender* pReceiver, list2<IEntityRender*>* pEntList, int dwAllowedTypes, Vec3d vLightPos,
                                              float fLightRadius) {
...
void CObjManager::MakeShadowCastersList(IEntityRender* pReceiver, list2<IEntityRender*>* pEntList, int dwAllowedTypes, Vec3d vLightPos, float fLightRadius) {
    ...
    else { // make list of sectors around
...
void CObjManager::MakeShadowMapInstancesList(IEntityRender* pReceiver, float obj_distance, list2<ShadowMapLightSourceInstance>* pLSourceInstances, int dwAllowedTypes,
                                             CDLight* pLight) {
    ...
    if (GetCVars()->e_shadow_maps_self_shadowing && pReceiver->GetRndFlags() & ERF_SELFSHADOW && GetRenderer()->GetFeatures() & (RFT_DEPTHMAPS | RFT_SHADOWMAP_SELFSHADOW))
    ...
    for (int e = 0; e < lstEntList_MLSMCIA.Count(); e++) {
        ...
        if (!lstEntList_MLSMCIA[e]->GetShadowMapFrustumContainer() || !lstEntList_MLSMCIA[e]->GetShadowMapFrustumContainer()->m_LightFrustums.Count() ||
...
ShadowMapFrustum* CObjManager::MakeEntityShadowFrustum(ShadowMapFrustum* pFrustum, ShadowMapLightSource* pLs, IEntityRender* pEnt, EShadowType nShadowType, int dwAllowedTypes) {
    Vec3d vMin, vMax;
