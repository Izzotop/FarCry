...
void CSprite::Render(const PartProcessParams& PPP, IShader* pShader) {
    ...
    if (m_pParams->nTexId) {
        ...
        */
        ...
        float fAge = fCurTime - m_fSpawnTime;
        if ((m_nParticleFlags & PARTICLE_ANIMATED_TEXTURE) && m_pParams->pAnimTex) { // animated
            t = fAge / m_fLifeTime;
            if (t < 0)
                t = 0;
            else if (t > 1)
        ...
        } else if (m_pParams->nTexId >= 0) { // one frame
        ...
        if (m_nParticleFlags & CParticle::PARTICLE_COLOR_BASED) {
            ...
            if (PPP.p3DEngine->GetFogEnd() > PPP.p3DEngine->GetFogStart()) {
                float fDist = (L1Distance2D(m_vPos, PPP.vCamPos) - PPP.p3DEngine->GetFogStart()) / (PPP.p3DEngine->GetFogEnd() - PPP.p3DEngine->GetFogStart());
                if (fDist > 1.f)
                    fDist = 1.f;
                else if (fDist < 0)
                    fDist = 0;
        ...
        if (m_pParams->nParticleFlags & PART_FLAG_LINEPARTICLE) { // line bullet trail
            ...
            PPP.pObjManager->AddPolygonToRenderer(nTexBindId, pShader, nDynLightMask, vSideStep, m_vDelta * 0.5f, ucResCol, m_pParams->eBlendType, vAmbientColor,
                                                  vParticlePos + m_vDelta * 0.5f, 0, 0, 0, 0, (float)nSortId, dwCCObjFlags, m_pMaterial);
        } else if (m_pParams->fTailLenght) { // render with tail
            ...
            if (nTailVertsNum > 2) { // fill tail indices
            ...
            //            nTailIndsNum=0;
            PPP.pObjManager->AddPolygonToRenderer(nTexBindId, pShader, nDynLightMask, vRight, vUp, ucResCol, m_pParams->eBlendType, vAmbientColor, vParticlePos, arrTailVerts,
                                                  nTailVertsNum, arrTailIndices, nTailIndsNum, (float)nSortId, dwCCObjFlags, m_pMaterial);
            ...
            if (PPP.pObjManager->GetCVars()->e_particles_receive_shadows && m_pSpawnerEntity && m_pSpawnerEntity->GetEntityRS() && m_pSpawnerEntity->GetEntityRS()->pShadowMapInfo)
                pShadowsList = m_pSpawnerEntity->GetEntityRS()->pShadowMapInfo->pShadowMapCasters;

            // no tail
            PPP.pObjManager->AddPolygonToRenderer(nTexBindId, pShader, nDynLightMask, vRight, vUp, ucResCol, m_pParams->eBlendType, vAmbientColor, vParticlePos, 0, 0, 0, 0,
                                                  (float)nSortId, dwCCObjFlags, m_pMaterial, nullptr, pShadowsList);
        }
    }
}
int CSprite::FillTailVertBuffer(SColorVert* pTailVerts, const Vec3d& vCamVec, const UCol& ucColor) {
    int nVertCount = 0;
