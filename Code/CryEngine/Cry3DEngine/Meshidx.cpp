...
bool CIndexedMesh::LoadMaterial(const char* szFileName, const char* szFolderName, CMatInfo& newMat, IRenderer* pRenderer, MAT_ENTITY* me) {
    ...
    if (bLoadAdditinalInfo) {
        ...
        for (int h = 0; h < pStaticCGF->m_lstHelpers.Count(); h++) {
            ...
            StatHelperInfo hi = StatHelperInfo( // pStaticCGF->m_lstHelpers[h].vPos,
                ...
                pStaticCGF->m_lstHelpers[h].szName, pStaticCGF->m_lstHelpers[h].Chunk.type, pShader, pStaticCGF->m_lstHelpers[h].tMat);
...
#pragma warning(pop) // AMD Port
    ...
    for (int g = 0; g < pStaticCGF->m_lstGeoms.Count(); g++) {
        ...
        if (!geom) {
            m_pSystem->GetILog()->Log("CIndexedMesh::LoadCGF error");
            break;
        }
        ...
        if (chunk->nTVerts) { // realloc texcoords
        ...
        { // realloc faces
                    ...
                    if (pFace->m_lInfo->nTextureIdLM!=65535)
          {
            int n = pFace->m_lInfo->nTextureIdLM;
                ...
                if (geom->m_pTexFaces) {
                    assert(geom->m_pTexFaces[c].t0 < geom->m_Chunk.nTVerts && geom->m_pTexFaces[c].t1 < geom->m_Chunk.nTVerts && geom->m_pTexFaces[c].t2 < geom->m_Chunk.nTVerts);
                }
...
#if !defined(LINUX)
                            m_pSystem->GetILog()->Log("Warning: CIndexedMesh::LoadCGF: Some faces have undefined material."
                                                      " File name is %s%s%s.",
                                                      szFileName, szGeomName ? ", object name is " : "", szGeomName ? szGeomName : "");
#endif
                        } else {
#if !defined(LINUX)
                            m_pSystem->GetILog()->Log("Warning: CIndexedMesh::LoadCGF: Some faces are referencing to material which is not present in the file."
                                                      " File name is %s%s%s."
                                                      " Face material id is %d, number of materials in file is %d.",

                                                      szFileName, szGeomName ? ", object name is " : "", szGeomName ? szGeomName : "",

                                                      m_pFaces[c + m_nFaceCount].shader_id, pStaticCGF->m_lstMaterials.Count());
#endif
                        }
    ...
    if (!szGeomName) {
        CryLogComment("  %d faces, %d verts, %d texcoord, %d geoms, %d of %d mats", m_nFaceCount, m_nVertCount, m_nCoorCount, pStaticCGF->m_lstGeoms.Count(), m_lstMatTable.Count(),
                      m_lstMatTable.Count() ? (m_lstMatTable.Last().m_nCGFMaterialID + 1) : 0);
...
void CIndexedMesh::MakeLightSources(CryStaticModel* pStaticCGF) { // get light sources
    m_lstLSources.Clear();
    m_tgtLSources.Clear();
    for (int l = 0; l < pStaticCGF->m_lstLights.Count(); l++) {
        ...
        } else if (pStaticCGF->m_lstLights[l].Chunk.type == LT_OMNI) {
            if (pStaticCGF->m_lstLights[l].Chunk.useAtten)
                lsource->m_Flags |= DLF_POINT;
            else
                lsource->m_Flags |= DLF_DIRECTIONAL;
        } else if (pStaticCGF->m_lstLights[l].Chunk.type == LT_DIRECT)
...
CIndexedMesh::CIndexedMesh(ISystem* pSystem, const char* szFileName, const char* szGeomName, int* _count, bool bLoadAdditinalInfo, bool bKeepInLocalSpace, bool bIgnoreFakeMats) {
...
int CIndexedMesh::GetAllocatedBytes() {
    return sizeof(*this) + sizeof(CObjFace) * m_nFaceCount + sizeof(Vec3d) * m_nVertCount + sizeof(TexCoord) * m_nCoorCount + sizeof(Vec3d) * m_nNormCount +
           sizeof(CDLight) * m_lstLSources.Count() + sizeof(CDLight) * m_tgtLSources.Count();
}
