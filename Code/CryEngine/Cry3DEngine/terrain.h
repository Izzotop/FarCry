...
public:
    ...
    float GetZ(const int x, const int y) {
    ...
    bool GetHole(const int& x, const int& y) {
        return (m_arrusHightMapData[(x + HMAP_BIT_SHIFT) >> HMAP_BIT_SHIFT][(y + HMAP_BIT_SHIFT) >> HMAP_BIT_SHIFT] & STYPE_BIT_MASK) == STYPE_HOLE;
    }
    bool GetHoleSafe(const int& x, const int& y);
    inline unsigned short& GetHMValue(const int& x, const int& y) {
        return m_arrusHightMapData[x >> HMAP_BIT_SHIFT][y >> HMAP_BIT_SHIFT];
    }
    ...
    int HMAP_BIT_SHIFT;
    void GetHighMapMemoryUsage(ICrySizer* pSizer) {
        m_arrusHightMapData.GetMemoryUsage(pSizer);
    }
...
struct DetailTexInfo {
    DetailTexInfo() {
        memset(this, 0, sizeof(DetailTexInfo));
...
struct VolumeInfo {
    ...
    bool IntersectBBox(const Vec3& vObjBoxMin, const Vec3& vObjBoxMax) {
        if (vBoxMin.x >= vObjBoxMax.x)
            return 0;
        if (vBoxMin.y >= vObjBoxMax.y)
            return 0;
        if (vBoxMin.z >= vObjBoxMax.z)
            return 0;
        if (vBoxMax.x <= vObjBoxMin.x)
            return 0;
        if (vBoxMax.y <= vObjBoxMin.y)
            return 0;
        if (vBoxMax.z <= vObjBoxMin.z)
            return 0;
        return 1;
    }

    bool InsideBBox(const Vec3& vObjPos) {
        if (vBoxMin.x >= vObjPos.x)
            return 0;
        if (vBoxMin.y >= vObjPos.y)
            return 0;
        if (vBoxMin.z >= vObjPos.z)
            return 0;
        if (vBoxMax.x <= vObjPos.x)
            return 0;
        if (vBoxMax.y <= vObjPos.y)
            return 0;
        if (vBoxMax.z <= vObjPos.z)
            return 0;
        return 1;
    }
};
...
public:
    ...
    float GetWaterLevel() {
        return m_fGlobalWaterLevel; /* ? m_fGlobalWaterLevel : WATER_LEVEL_UNKNOWN;*/
    }
    ...
    void SetCBuffer(class CCoverageBuffer* pBuffer) {
        m_pCoverageBuffer = pBuffer;
    };
...
public:
    void SetObjManager(CObjManager* pObjManager) {
    ...
    static const int GetTerrainSize() {
        return m_nTerrainSize;
    }
    static const int GetSectorSize() {
        return m_nSectorSize;
    }
    static const int GetHeightMapUnitSize() {
        return m_nUnitSize;
    }
    static const int GetSectorsTableSize() {
        return m_nSectorsTableSize;
    }
    static const float GetInvUnitSize() {
    ...
    void InitTerrainWater(bool bEditorMode, IShader* pTerrainWaterShader, int nWaterBottomTexId, IShader* pSunRoadShader, float fWaterTranspRatio, float fWaterReflectRatio,
                          float fWaterBumpAmountX, float fWaterBumpAmountY, float fWaterBorderTranspRatio);
    ...
    bool RenderAreaLeafBuffers(Vec3d vPos, float fRadius, int nDynLMask, CLeafBuffer** arrLightLeafBuffers, int nMaxLeafBuffersNum, CCObject* pObj, IShader* pShder,
                               bool bRecalcLeafBuffers, const char* szComment, IShader* pEffStencilTest = nullptr, int nShaderSortOrder = 0, ShadowMapFrustum* pFrustum = nullptr,
                               Vec3d* pvFrustPos = nullptr, float fFrustScale = 1.f);
    ...
    int GetTerrainTextureDim() {
        return m_nSectorTextureReadedSize * GetSectorsTableSize();
    }
...
public:
    ...
    float GetX() {
        return float(m_nX) * CTerrain::GetTerrainSize() / 65536.f;
    }
    float GetY() {
        return float(m_nY) * CTerrain::GetTerrainSize() / 65536.f;
    }
    float GetZ() {
        return float(m_nZ) * CTerrain::GetTerrainSize() / 65536.f;
    }
    float GetScale() {
        return m_fScale;
    }
    int GetID() {
        return (int)m_ucObjectTypeID;
    }
    uchar GetBrightness() {
        return m_ucBrightness;
    }
