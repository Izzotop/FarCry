...
int CStatObj::FindInPosBuffer(const Vec3d& opt, Vec3d* _vbuff, int _vcount, list2<int>* pHash) {
    for (int i = 0; i < pHash->Count(); i++) {
        if (IsEquivalent(*((Vec3d*)(&_vbuff[(*pHash)[i]].x)), *((Vec3d*)(&opt.x)), VEC_EPSILON))
            return (*pHash)[i];
    }
...
void CStatObj::Physicalize() {
    ...
    for (int m = 0; m < m_pTriData->m_lstMatTable.Count(); m++) {
        ...
        // find obstruct material id
        else if (pPhysMaterialEnumerator && !pPhysMaterialEnumerator->IsCollidable(m_pTriData->m_lstMatTable[m].nGamePhysMatId) &&
...
#define MESH_OCCLUSION 3
    ...
    for (int nMesh = 0; nMesh <= 3; nMesh++) { // fill physics indices
        list2<int> lstPhysIndices;
        list2<unsigned char> lstFaceMaterials;

        if (nMesh == MESH_PHYSIC) { // find all physicalized faces
            for (int i = 0; i < m_pTriData->m_nFaceCount; i++) {
                if (((m_pTriData->m_lstMatTable[m_pTriData->m_pFaces[i].shader_id].m_Flags & MIF_PHYSIC) && nPhysMatID < 0) || m_pTriData->m_pFaces[i].shader_id == nPhysMatID) {
                    for (int v = 0; v < 3; v++)
                    ...
                    if (m_pTriData->m_pFaces[i].shader_id == nPhysMatID) { // remove face from list (it's not needed for rendering)
        ...
        } else if (nMesh == MESH_OBSTRUCT) { // find all obstruct faces
            if (arrObstrMatIDs.Count()) {    // find all obstruct faces
                for (int i = 0; i < m_pTriData->m_nFaceCount; i++) {
        ...
        } else if (nMesh == MESH_LEAVES) { // find all obstruct faces
            if (arrLeavesMatIDs.Count()) { // find all obstruct faces
                for (int i = 0; i < m_pTriData->m_nFaceCount; i++) {
            ...
            if (nOcclMatID >= 0) { // find all occlusion faces
                for (int i = 0; i < m_pTriData->m_nFaceCount; i++) {
        ...
        if (lstPhysIndices.Count()) {
            ...
            if (GetPhysicalWorld() && (nMesh == MESH_PHYSIC || nMesh == MESH_OBSTRUCT || nMesh == MESH_LEAVES) && nInitVertCount > 2) {
                ...
                if (GetCVars()->e_check_number_of_physicalized_polygons && nPhysTris > 100 + m_pTriData->m_nFaceCount / 2) {
#if !defined(LINUX) // does not matter in dedicated server mode
                    GetLog()->Log("Physicalized geometry contains too many polygons(%d of %d), for CGF: %s", nPhysTris, m_pTriData->m_nFaceCount, GetFileName());
                ...
                m_arrPhysGeomInfo[nMesh] =
                    pGeoman->RegisterGeometry(pGeoman->CreateMesh((vectorf*)&pExVerts[0], &lstPhysIndices[0], (short*)&lstFaceMaterials[0], lstPhysIndices.Count() / 3, flags, true,
                                                                  true, tol, nMinTrisPerNode, nMaxTrisPerNode, 2.5f));
        ...
        */
        ...
        if (lstPhysIndices.Count()) {
            ...
            if (GetPhysicalWorld() && (nMesh == MESH_PHYSIC || nMesh == MESH_OBSTRUCT)) {
                ...
                nFaceCount /= 3;
                if (GetCVars()->e_check_number_of_physicalized_polygons && nPhysTris > 100 + nFaceCount / 2) {
#if !defined(LINUX) // does not matter in dedicated server mode
                    GetLog()->Log("Physicalized geometry contains too many polygons(%d of %d), for CGF: %s", nPhysTris, nFaceCount, GetFileName());
                    GetLog()->Log("  Number of physicalized tris is more than 100 + number of all tris divided by 2");
#endif
                }
