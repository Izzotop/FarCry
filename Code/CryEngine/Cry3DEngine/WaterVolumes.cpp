...
bool CWaterVolume::TesselateFace(list2<struct_VERTEX_FORMAT_P3F_N_COL4UB_TEX2F>& lstVerts, list2<ushort>& lstIndices, int nFacePos, list2<Vec3d>& lstDirections) {
    ...
    // tesselate longest
    if (fDist01 > fDist12 && fDist01 > fDist20) { // 01
        ...
        lstIndices.Add(nNewIndex);
        lstIndices.Add(n1);
    } else if (fDist12 > fDist01 && fDist12 > fDist20) { // 12
        ...
        lstIndices.Add(nNewIndex);
        lstIndices.Add(n2);
    } else if (fDist20 > fDist12 && fDist20 > fDist01) { // 20
...
void CWaterVolume::CheckForUpdate(bool bMakeLowestLod) {
    ...
    if (m_fTriMinSize != m_fPrevTriMinSize || m_fTriMaxSize != m_fPrevTriMaxSize ||
        ((m_vCurrentCamPos.GetDistance(GetViewCamera().GetPos()) > 2 && (m_fTriMinSize < m_fTriMaxSize)))) { // force to rebuild water volume geometry
        ...
        if (m_pShader)
            if (m_lstPoints.Count() > 3)
                if (GetDistance(m_lstPoints.Last(), m_lstPoints[0]) < 1.f) {
                    ...
                    m_pLeafBuffer = GetRenderer()->CreateLeafBufferInitialized(lstVertices.GetElements(), lstVertices.Count(), VERTEX_FORMAT_P3F_N_COL4UB_TEX2F,
                                                                               lstIndices.GetElements(), lstIndices.Count(), R_PRIMV_TRIANGLES, "WaterVolume", eBT_Static);

                    m_pLeafBuffer->SetChunk(m_pShader, 0, lstVertices.Count(), 0, lstIndices.Count());
...
void CWaterVolumeManager::RenderWaterVolumes(bool bOutdoorVisible) {
    ...
    for (int i = 0; i < m_lstWaterVolumes.Count(); i++) {
        ...
        if (m_lstWaterVolumes[i]->m_lstVisAreas.Count()) { // water in indoors
        ...
        m_lstWaterVolumes[i]->m_pLeafBuffer->AddRenderElements(pObject, nDynMask, -1, 0, (GetViewCamera().GetPos().z > Get3DEngine()->GetWaterLevel()) ? eS_SeeThrough : eS_Water,
...
float CWaterVolumeManager::GetWaterVolumeLevelFor2DPoint(const Vec3d& vPos, Vec3d* pvFlowDir) {
    ...
    for (int i = 0; i < m_lstWaterVolumes.Count(); i++) {
        ...
        if (pWaterVolumes->m_pLeafBuffer && Overlap::Point_AABB(vPos, vBoxMin2d, vBoxMax2d)) { // if inside bbox
            int nInds = 0;
            ...
            for (int i = 0; (i + 2) < nInds; i += 3) { // test all triangles of water surface strip
                ...
                if (Overlap::PointInTriangle(vPos2d, v0, v1, v2, Vec3d(0, 0, 1.f))) { // triangle found
                    Plane plane;
                    plane.CalcPlane( // calc plane using real vertices
                        *(Vec3d*)&pPos[nPosStride * pInds[i + 0]], *(Vec3d*)&pPos[nPosStride * pInds[i + 1]], *(Vec3d*)&pPos[nPosStride * pInds[i + 2]]);
                    ...
                    if (GetCVars()->e_water_volumes == 2) {
                        GetLog()->Log("CameraLevel=%.2f, WaterVolumeLevel=%.2f %d", GetViewCamera().GetPos().z, fDist, GetFrameID());
                    ...
                    if (pvFlowDir) {
                        ...
                        *pvFlowDir = pWaterVolumes->m_lstDirections[pInds[i + 0]] * (1.f - fDist0) + pWaterVolumes->m_lstDirections[pInds[i + 1]] * (1.f - fDist1) +
