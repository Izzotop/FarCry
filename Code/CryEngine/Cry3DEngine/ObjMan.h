...
public:
    ...
    CStatObj* CObjManager::MakeObject(const char* __szFileName, const char* _szGeomName = 0, EVertsSharing eVertsSharing = evs_NoSharing, bool bLoadAdditinalInfo = true,
                                      bool bKeepInLocalSpace = false, bool bLoadLater = false);
    ...
    void MakeShadowMapInstancesList(IEntityRender* pEntityRS, float obj_distance, list2<ShadowMapLightSourceInstance>* pLSourceInstances, int dwAllowedTypes, CDLight* pLight);
...
protected:
    ...
    ShadowMapFrustum* MakeEntityShadowFrustum(ShadowMapFrustum* pFrustum, ShadowMapLightSource* pLs, IEntityRender* pEntityRS, EShadowType nShadowType, int dwAllowedTypes);

    list2<IEntityRender*> m_lstDebugEntityList;

    void MakeShadowCastersList(IEntityRender* pEntityRS, list2<IEntityRender*>* pEntList, int dwAllowedTypes, Vec3d vLightPos, float fLightRadius);
...
public:
    int GetLoadedObjectCount() {
        return m_lstLoadedObjects.size();
    }
...
#pragma warning(pop) // AMD Port
    ...
    void RenderObject(IEntityRender* o, int nFogVolumeID, uint nDLightMask, bool bLMapGeneration, const CCamera& EntViewCamera, Vec3d* pvAmbColor, Vec3d* pvDynAmbColor,
                      VolumeInfo* pFogVolume, bool bNotAllInFrustum, float fMaxViewDist, IEntityRenderInfo* pEntInfo = nullptr);
    ...
    void AddPolygonToRenderer(const int nTexBindId, IShader* pShader, const int nDynLMask, Vec3d right, Vec3d up, const UCol& ucResCol, const ParticleBlendType eBlendType,
                              const Vec3d& vAmbientColor, Vec3d vPos, const SColorVert* pTailVerts = nullptr, const int nTailVertsNum = 0, const byte* pTailIndices = nullptr,
                              const int nTailIndicesNum = 0, const float fSortId = 0, const int dwCCObjFlags = 0, IMatInfo* pCustomMaterial = nullptr,
                              CStatObjInst* pStatObjInst = nullptr, list2<struct ShadowMapLightSourceInstance>* pShadowMapCasters = nullptr);
...
#define MAX_LIGHTS_NUM 32
    ...
    void MakeShadowCastersListInArea(CBasicArea* pArea, IEntityRender* pReceiver, list2<IEntityRender*>* pEntList, int dwAllowedTypes, Vec3d vLightPos, float fLightRadius);
    void PrefechObjects();
    void RequestEntityShadowMapGeneration(IEntityRender* pEntityRnd);
    bool m_bLockCGFResources;
    void FreeNotUsedCGFs();
    void RenderObjectVegetationNonCastersNoFogVolume(IEntityRender* pEntityRS, uint nDLightMask, const CCamera& EntViewCamera, bool bNotAllInFrustum, float fMaxViewDist,
