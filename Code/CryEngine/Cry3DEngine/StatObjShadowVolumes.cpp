...
ItShadowVolume* CStatObj::MakeConnectivityInfo(CIndexedMesh* pMesh, const Vec3d& vOrigin, CStatObj* pStatObj) {
    ...
    // precalc edges
    if (!ptSvObj->pSvObj->CreateConnectivityInfo()) { // delete empty objects
        ptSvObj->Release();
        ptSvObj = nullptr;
    }
...
#if !defined(LINUX)
        Warning(0, m_szFileName, "%d open edges found during shadow volume calculations for %s %s", pConnectivity->numOrphanEdges(), m_szFileName, m_szGeomName);
#endif
...
bool CShadowVolObject::CreateConnectivityInfo() {
    ...
    if (m_pEdgeConnectivity) {
        ...
        #ifdef _DEBUG
                char str[256];
                snprintf(str, sizeof(str), "StencilEdgeConnectivity Indoor Stats %p: %d/%d Vertices %d/%d
        Faces\n",m_pEdgeConnectivity,dwVertCount,pMesh->m_nVertCount,dwTriCount,nNumFaces); OutputDebugString(str); #endif*/
    }
...
//        assert(fShadowVolumeExtent>=1.0f);
    ...
    */
    ...
    if (nNumVertices > 1 && nNumIndices > 1) {
        ...
        if (pSVI->pVB && pSVI->pVB->m_SecVertCount == (int)m_nNumVertices && pSVI->pVB->m_Indices.m_nItems == (int)nNumIndices) {
            ...
            pSVI->pVB = GetRenderer()->CreateLeafBufferInitialized(m_pSystemVertexBuffer, m_nNumVertices, VERTEX_FORMAT_P3F, &m_arrIndices[0], nNumIndices, R_PRIMV_TRIANGLES,
                                                                   "ShadowVolume", eBT_Dynamic, 1, 0, 0, this);
