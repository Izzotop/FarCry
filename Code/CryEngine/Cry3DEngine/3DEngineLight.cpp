...
void C3DEngine::RegisterLightSourceInSectors(CDLight* pDynLight) {
    ...
    */
    ...
    if (pDynLight->m_Id == -1) // ignored by renderer
        return;                // fake
    else if (pDynLight->m_pShader != 0 && (pDynLight->m_pShader->GetLFlags() & LMF_DISABLE))
        return; // fake
...
void CTerrain::RenderDLightOnHeightMap(CDLight* pDLight) {
    ...
    */
    ...
    if (!(pDLight->m_Flags & DLF_DIRECTIONAL)) { // draw light on the ground
        ...
        RenderAreaLeafBuffers(vPos, fRadius, 1 << pDLight->m_Id, pDLight->m_arrLightLeafBuffers, sizeof(pDLight->m_arrLightLeafBuffers) / sizeof(pDLight->m_arrLightLeafBuffers[0]),
...
void C3DEngine::UpdateStaticLightSources() {
    ...
    for (int i = 0; i < m_lstStaticLights.Count(); i++) {
        ...
        Matrix34 rt34                        =    Matrix34::GetRotationXYZ34(gf_DEGTORAD*pLightEntity->GetAngles(0),pLightEntity->GetPos(true) );    //set scaling and translation
        in one function call Matrix44 EntityMatrix        =    rt34*diag;    //optimised concatenation: m34*diag EntityMatrix.Transpose();                            //TODO: remove
        this after E3 and use Matrix34 instead of Matrix44
...
void C3DEngine::PrepareLightSourcesForRendering() {
    ...
    if (m_pObjManager->m_nRenderStackLevel) { // do not delete lsources during reqursion, becasue hmap lpasses are shared between levels
        ...
        for (int i = 0; i < m_lstDynLights.Count(); i++) {
            ...
            */
            if (m_lstDynLights[i].m_pOwner && m_lstDynLights[i].m_pOwner->GetEntityVisArea()) { // vis area lsource
            ...
            } else { // outdoor lsource
                if ((!(m_lstDynLights[i].m_Flags & DLF_DIRECTIONAL)) && !IsSphereVisibleOnTheScreen(m_lstDynLights[i].m_Origin, m_lstDynLights[i].m_fRadius, 0)) {
                    FreeLightSourceComponents(&m_lstDynLights[i]);
                    m_lstDynLights.Delete(i);
                    i--;
            ...
            if (GetRenderer()->EF_IsFakeDLight(&m_lstDynLights[i])) { // ignored by renderer
            ...
            if (m_lstDynLights[i].m_Id == -1) { // ignored by renderer

                assert(i >= MAX_LIGHTS_NUM);

                if (i >= MAX_LIGHTS_NUM) { // no more sources can be accepted by renderer
            ...
            if (m_lstDynLights[i].m_pShader != 0 && (m_lstDynLights[i].m_pShader->GetLFlags() & LMF_DISABLE)) { // fake
            ...
            if (m_lstDynLights[i].m_Flags & DLF_PROJECT && m_lstDynLights[i].m_fLightFrustumAngle < 90 // actual fov is twice bigger
                && m_lstDynLights[i].m_pLightImage && !(m_lstDynLights[i].m_pLightImage->GetFlags2() & FT2_REPLICATETOALLSIDES) &&
                (m_lstDynLights[i].m_pLightImage->GetFlags2() & FT2_CUBEASSINGLETEXTURE)) { // prepare projector camera for frustum test
        ...
        for (int i = m_nRealLightsNum; i < m_lstDynLights.Count(); i++) { // ignored by renderer
            m_lstDynLights[i].m_Id = -1;
            GetRenderer()->EF_ADDDlight(&m_lstDynLights[i]);
...
void C3DEngine::UpdateLightSources() {
    ...
    for (int i = 0; i < m_lstDynLights.Count(); i++) {
        ...
        if (pLight->m_fLifeTime && !pLight->m_pShader) { // evaluate radius
...
CDLight* C3DEngine::CheckDistancesToLightSources(uint& nDLightMask, const Vec3d vObjPos, const float fObjRadius, IEntityRender* pEntityRender, int nMaxLightBitsNum,
                                                 CDLight** pSelectedLights, int nMaxSelectedLights, Vec3d* pvSummLightAmmount) {
    FUNCTION_PROFILER(GetSystem(), PROFILE_3DENGINE);
    ...
    for (int n = 0; n < 32 && n < m_nRealLightsNum /*m_lstDynLights.Count()*/; n++) {
        ...
        if (nId >= 0 && nDLightMask & (1 << nId) && nId == n) {
            ...
            if (pEntityRender && pDLight->m_Flags & DLF_LM && !(pDLight->m_Flags & DLF_CASTSHADOW_VOLUME) && pEntityRender->GetRndFlags() & ERF_USELIGHTMAPS &&
                pEntityRender->HasLightmap(0)) { // in case of lightmaps
                ...
                if (!(pDLight->m_Flags & DLF_CASTSHADOW_MAPS)) {
                    if ((pDLight->m_SpecColor == Col_Black) || ((pDLight->m_Flags & DLF_SPECULAR_ONLY_FOR_HIGHSPEC) && (m_LightConfigSpec < CONFIG_HIGH_SPEC)))
            ...
            if (pDLight->m_Flags & DLF_PROJECT && pEntityRender && pDLight->m_fLightFrustumAngle < 90                 // actual fov is twice bigger
                && !(pDLight->m_pLightImage != 0 && pDLight->m_pLightImage->GetFlags2() & FT2_REPLICATETOALLSIDES)) { // check projector frustum
                ...
                nProjectorsNum++;
            } else if (pEntityRender) { // check sphere/bbox intersection
                Vec3d vBoxMin, vBoxMax;
            ...
            float fLightAmount =
                (pDLight->m_Color.r + pDLight->m_Color.g + pDLight->m_Color.b) * 0.233f + (pDLight->m_SpecColor.r + pDLight->m_SpecColor.g + pDLight->m_SpecColor.b) * 0.1f;
                ...
                if (pEntityRender) {
                    if (pEntityVisArea) {
                        if (pDLight->m_pOwner && pDLight->m_pOwner->GetEntityRS() && pDLight->m_pOwner->m_pVisArea) {
                            ...
                            if (pEntityVisArea != pLightArea && !(pEntityRender->GetRndFlags() & ERF_DONOTCHECKVIS)) { // try also neighbor volumes
                                int nSearchDepth;
    ...
    return pStrongestShadowCaster ? pStrongestShadowCaster : ((nMaxLightBitsNum > 0) ? m_lstDL_CDTLS[0].pDLight : 0); // return strongest
}
...
bool C3DEngine::LoadStaticLightSources(const char* pszFileName) {
    ...
    if (sHeader.iNumDLights == 0 || sHeader.iVersion != 0 || sHeader.iSizeOfDLight != sizeof(CDLightDummy32)) {
        GetConsole()->Exit("C3DEngine::LoadStaticLightSources: StatLights.dat version error (%d,%d). Please reexport using latest version of 32-bit editor.", sHeader.iSizeOfDLight,
                           sizeof(dummy));
        return false;
    }
...
bool C3DEngine::LoadStaticLightSources(const char* pszFileName) {
    ...
    if (sHeader.iNumDLights == 0 || sHeader.iVersion != sHdrCmp.iVersion || sHeader.iSizeOfDLight != sHdrCmp.iSizeOfDLight) {
