...
void CCoverageBuffer::AddBBox(const Vec3d & mins, const Vec3d & maxs, const Vec3d & vDirToCamera) {
    ...
    Point2d arrVerts[8] =  { // transform into screen space
    ...
    for (int i = 0; i < 8; i++) {
        if (arrVerts[i].x < min2d.x) min2d.x = arrVerts[i].x;
        if (arrVerts[i].x > max2d.x) max2d.x = arrVerts[i].x;
        if (arrVerts[i].y < min2d.y) min2d.y = arrVerts[i].y;
        if (arrVerts[i].y > max2d.y) max2d.y = arrVerts[i].y;
    }
    ...
    // adjust 2d vertices
    for (int i = 0; i < 8; i++) {
        if (arrVerts[i].x < min2d.x) arrVerts[i].x = min2d.x;
        if (arrVerts[i].x > max2d.x) arrVerts[i].x = max2d.x;
        if (arrVerts[i].y < min2d.y) arrVerts[i].y = min2d.y;
        if (arrVerts[i].y > max2d.y) arrVerts[i].y = max2d.y;
    }
    ...
    for (int i = 0; i < nIndCount; i += 3) {
        ...
        */
        ScanTriangleWithCliping(
            m_arrVerts_AddMesh[pIndices[i + 0]], m_arrVerts_AddMesh[pIndices[i + 1]], m_arrVerts_AddMesh[pIndices[i + 2]],
            m_arrVerts3dWS_AddMesh[pIndices[i + 0]], m_arrVerts3dWS_AddMesh[pIndices[i + 1]], m_arrVerts3dWS_AddMesh[pIndices[i + 2]]
        );
    }
}
...
bool CCoverageBuffer::IsBBoxVisible(const Vec3d& mins, const Vec3d& maxs) {
    ...
    Point2d verts[8] =  { // transform into screen space
        ProjectToScreen(mins.x,mins.y,mins.z),
        ProjectToScreen(mins.x,maxs.y,mins.z),
        ProjectToScreen(maxs.x,mins.y,mins.z),
        ProjectToScreen(maxs.x,maxs.y,mins.z),
    ...
    for (int i = 0; i < 8; i++) {
        if (verts[i].x < min2d.x) min2d.x = verts[i].x;
        if (verts[i].x > max2d.x) max2d.x = verts[i].x;
        if (verts[i].y < min2d.y) min2d.y = verts[i].y;
        if (verts[i].y > max2d.y) max2d.y = verts[i].y;
    }
...
void CCoverageBuffer::DrawDebug(int nStep) { // project buffer to the screen
    if (!nStep)
        return;
...
int CCoverageBuffer::ClipEdge(const Vec3d& v1, const Vec3d& v2, const Plane& ClipPlane, Vec3d& vRes1, Vec3d& vRes2) {
    ...
    if (d1 >= 0 && d2 < 0) { // from vis to no vis
        vRes1 = vIntersectionPoint;
        return 1;
    } else if (d1 < 0 && d2 >= 0) { // from novis to vis
        vRes1 = vIntersectionPoint;
        vRes2 = v2;
        return 2;
    }
...
void CCoverageBuffer::ScanTriangleWithCliping(Point2d p1, Point2d p2, Point2d p3, const Vec3d& v1, const Vec3d& v2, const Vec3d& v3) {
    ...
    if (!bClipingNeeded) { // just use already calculated 2d points
    ...
    assert(!IsEquivalent(v1, v2, VEC_EPSILON) && !IsEquivalent(v1, v3, VEC_EPSILON) && !IsEquivalent(v2, v3, VEC_EPSILON));
    ...
    for (int i = 1; i < arrTriangle.Count(); i++) {
        if (IsEquivalent(arrTriangle.GetAt(i), arrTriangle.GetAt(i - 1), VEC_EPSILON)) {
    ...
    if (arrTriangle.Count() > 2 && arrTriangle.Count() < 8) {
        ...
        for (int i = 0; i < arrTriangle.Count(); i++) { // transform into screen space
