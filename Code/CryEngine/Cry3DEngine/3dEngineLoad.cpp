...
bool C3DEngine::LoadLevel(const char* szFolderName, const char* szMissionName, bool bEditorMode) {
    ...
    if (!szFolderName || !szFolderName[0]) {
        Warning(0, 0, "C3DEngine::LoadLevel: Level name is not specified");
        return 0;
    }
    ...
    if (!bEditorMode) { // check is LevelData.xml file exist
        ...
        if (!CXFile::IsFileExist(sMapFileName)) {
            UpdateLoadingScreen("Error: Level not found: %s", sMapFileName);
            return 0;
        }
    }
    ...
    char header[512];
    snprintf(header, sizeof(header), "\001---------------------- Loading level %s, mission %s ------------------------------------", szFolderName, szMissionName);
    header[100] = 0;
    ...
    if (m_pTerrain && !m_pTerrain->LoadTerrain(bEditorMode)) { // terrain not found
        delete m_pTerrain;
        m_pTerrain = 0;
    }
    ...
    if (m_pTerrain && !m_pTerrain->m_pWater)
        m_pTerrain->InitTerrainWater(bEditorMode, m_pTerrainWaterShader, m_nWaterBottomTexId, m_pSunRoadShader, m_fWaterTranspRatio, m_fWaterReflectRatio, m_fWaterBumpAmountX,
                                     m_fWaterBumpAmountY, m_fWaterBorderTranspRatio);
    ...
    if (GetCVars()->e_stream_areas) { // all data can be streamed from disk now
        for (int i = 0; i < m_pObjManager->m_lstBrushContainer.Count(); i++)
...
void C3DEngine::LoadTerrainSurfacesFromXML(void* _pDoc) {
    ...
    if (!_pDoc) { // load current LevelData if pDoc not specified
    ...
    if (pDetTexTagList) {
        ...
        if (pDetTexList) {
            ...
            while (pDetTex = pDetTexList->nextNode()) {
                ...
                if (pDetailTextureName)
                    if (pScaleX)
                        if (pScaleY)
                            if (pSurfaceName) {
                                m_pTerrain->SetDetailTextures(nId, pDetailTextureName->getText(), (float)atof(pScaleX->getText()), (float)atof(pScaleY->getText()),
                        ...
                        if (pAttr) {
                            ...
                            //            if(pObjectsTagList)
                            {
                                ...
                                if (pAttr1 != 0 && pAttr2 != 0 && pAttr3 != 0 && pAttr4 != 0 && pAttr5 != 0) {
                                    ...
                                    volumeInfo.pShader = (char*)pAttr4->getText()[0] ? GetRenderer()->EF_LoadShader((char*)pAttr4->getText(), eSH_World, EF_SYSTEM)
                                                                                     : GetRenderer()->EF_LoadShader("FogLayer", eSH_World, EF_SYSTEM);
            ...
            if (pMissionTagList) {
                ...
                if (pMissionList) {
                    ...
                    while (pMission = pMissionList->nextNode()) {
                        ...
                        if (pName) {
                            if (!stricmp(pName->getText(), szMissionName)) { // get mission XML file name and open mission file
...
void C3DEngine::LoadMissionSettingsFromXML(XDOM::IXMLDOMNode* pInputNode, bool bEditorMode, bool bUpdateLightingOnVegetations) {
    ...
    m_pObjManager->m_vOutdoorAmbientColor = StringToVector(GetXMLAttribText(pInputNode, "Environment", "EnvState", "OutdoorAmbientColor", "64,64,64")) / 255;

    m_pObjManager->m_vSunColor = StringToVector(GetXMLAttribText(pInputNode, "Environment", "EnvState", "SunColor", "128,128,128")) / 255;
