...
bool CStatObj::CompileInNeeded() {
    ...
    snprintf(szRemoteCmdLine, sizeof(szRemoteCmdLine), RC_EXECUTABLE " \"%s\" /GeomName=%s /Stripify=%d /LoadAdditinalInfo=%d /KeepInLocalSpace=%d /StaticCGF=1 /refresh",
             m_szFileName, m_szGeomName, int(m_eVertsSharing == evs_ShareAndSortForCache), int(m_bLoadAdditinalInfo), int(m_bKeepInLocalSpace));
    ...
    if (f) {
        ...
        if (nReaded == sizeof(CCGFHeader)) {
            if (ftSourceFileTime.dwHighDateTime == fileHeader.SourceFileTime.dwHighDateTime && ftSourceFileTime.dwLowDateTime == fileHeader.SourceFileTime.dwLowDateTime) {
                if (strcmp(fileHeader.szVersion, CCGF_FILE_VERSION))
