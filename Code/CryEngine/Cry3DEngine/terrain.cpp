...
void CTerrain::PreCacheArea(const Vec3d& vPos, float fRadius) {
    ...
    GetLog()->UpdateLoadingScreen("Preloading terrain textures for (%.1f, %.1f, %.1f) ...", vPos.x, vPos.y, vPos.z);

    int nLoaded = 0;
    for (int x = 0; x < CTerrain::GetSectorsTableSize(); x++)
        for (int y = 0; y < CTerrain::GetSectorsTableSize(); y++) {
            m_arrSecInfoTable[x][y]->m_fDistance = GetDist2D(float(m_arrSecInfoTable[x][y]->m_nOriginX + (CTerrain::GetSectorSize() >> 1)),
                                                             float(m_arrSecInfoTable[x][y]->m_nOriginY + (CTerrain::GetSectorSize() >> 1)), vPos.x, vPos.y) - (CTerrain::GetSectorSize() >> 1);
...
void CTerrain::RefineSector(int x1, int x2, int y1, int y2, bool bAllIN) {
    ...
    if ((x2 - x1) > CTerrain::GetSectorSize()) {
        ...
        if (!m_pViewCamera->IsAABBVisible_hierarchical(AABB(Vec3d((float)x1 - TERRAIN_SECTORS_MAX_OVERLAPPING, (float)y1 - TERRAIN_SECTORS_MAX_OVERLAPPING, 0),
        ...
        if (info->m_bAllStaticsInFrustum)
            ...
            info->m_bGroundVisible = m_pViewCamera->IsAABBVisibleFast(
                AABB(Vec3d(fOrgX, fOrgY, info->m_fMinZ), Vec3d(fOrgX + CTerrain::GetSectorSize(), fOrgY + CTerrain::GetSectorSize(), max(info->m_fMaxZ, m_fGlobalWaterLevel))));
        }
...
void CTerrain::UnloadOldSectors(float fMaxViewDist) {
    ...
    for (int t = 0; t < 16; t++) {
...

        {
            ...
            float fDistanse = GetDist2D((float)info->m_nOriginX + (CTerrain::GetSectorSize() >> 1), (float)info->m_nOriginY + (CTerrain::GetSectorSize() >> 1),
                                        m_pViewCamera->GetPos().x, m_pViewCamera->GetPos().y) -
                              (CTerrain::GetSectorSize() >> 1);

            if ((fDistanse > (1.5f * fMaxViewDist)) || (info->m_cLastTimeUsed < (cCurTimeSec - 4))) { // try to release vert buffer if not in use int time
...
void CTerrain::ApplyForceToEnvironment(Vec3d vPos, float fRadius, float fAmountOfForce) {
    ...
    if (x2 >= CTerrain::GetTerrainSize())
        x2 = CTerrain::GetTerrainSize() - 1;
    if (y2 >= CTerrain::GetTerrainSize())
        y2 = CTerrain::GetTerrainSize() - 1;
...
void CTerrain::MoveAllEntitiesIntoList(list2<IEntityRender*>* plstVisAreasEntities, const Vec3d& vBoxMin, const Vec3d& vBoxMax) {
    // gat 2d bounds
...
bool CTerrain::PreloadResources() {
    ...
    if (pSecInfo) {
        pSecInfo->m_fDistance = GetDist2D(float(pSecInfo->m_nOriginX + (CTerrain::GetSectorSize() >> 1)), float(pSecInfo->m_nOriginY + (CTerrain::GetSectorSize() >> 1)),
                                          m_vCameraPos.x, m_vCameraPos.y) -
                                (CTerrain::GetSectorSize() >> 1);
