...
void CSectorInfo::SetLOD() {
...

    {
        ...
        if ((m_fDistance + camera_h) < CTerrain::GetSectorSize() + (CTerrain::GetSectorSize() >> 2))
            ...
            float allowed_error = (m_pTerrain->m_fLodLFactor * GetCVars()->e_terrain_lod_ratio * (m_fDistance + camera_h)) / (180.f) * 2.5f;
...
BOOL CSectorInfo::IsCanBeReflected()
{
  ...
  if(m_pTerrain->m_pViewCamera->GetPos().x<0 || m_pTerrain->m_pViewCamera->GetPos().y<0 || m_pTerrain->m_pViewCamera->GetPos().x>=CTerrain::GetTerrainSize() ||
m_pTerrain->m_pViewCamera->GetPos().y>=CTerrain::GetTerrainSize()) return 1;
...
void CSectorInfo::InitSectorBoundsAndErrorLevels(int _x1, int _y1, FILE* geom_file_to_read, FILE* geom_file_to_write) {
    ...
    } else // generate errors table, set min,max, mid Z values
    {
        ...
        for (int nLod = 0; nLod <= MAX_MML_LEVEL; nLod++) { // calculate max diffeence between this detail level and nolod
            float max_diff = 0;
    ...
    m_vBoxMax = Vec3d((float)m_nOriginX + CTerrain::GetSectorSize(), (float)m_nOriginY + CTerrain::GetSectorSize(), m_fMaxZ);
...
void CSectorInfo::SetDetailLayersPalette() {
    ...
    qsort(&lstSurfaceTypesInSector[0], MAX_SURFACE_TYPES_COUNT, sizeof(lstSurfaceTypesInSector[0]), CSectorInfo__Cmp_UnitsNum);
...
void CSectorInfo::RenderEntities(CObjManager* pObjManager, bool bNotAllInFrustum, char* fake, int nStatics) {
    if (m_lstEntities[nStatics].Count())
        DrawEntities(m_pFogVolume ? m_pFogVolume->nRendererVolumeID : 0, GetRenderer()->EF_GetHeatVision() ? m_nDynLightMaskNoSun : m_nDynLightMask, false, GetViewCamera(), nullptr,
                     nullptr, m_pFogVolume, bNotAllInFrustum, (m_nOriginX == 0 && m_nOriginY == 0) ? 0 : (m_fDistance - TERRAIN_SECTORS_MAX_OVERLAPPING), pObjManager,
                     m_fDistance < GetCVars()->e_area_merging_distance, "", nStatics);
}
