...
void CTerrain::ExplodeTerrain(Vec3d vExploPos, float fExploRadius, CObjManager* pObjManager, bool bDeformTerrain) {
    ...
    for (s = 0; s < lstNearSecInfos.Count(); s++) {
        ...
        for (int i = 0; i < pSecInfo->m_lstEntities[STATIC_ENTITIES].Count(); i++) {
            ...
            if (fDist < fExploRadius + fEntRadius && Overlap::Sphere_AABB(Sphere(vExploPos, fExploRadius), AABB(vEntBoxMin, vEntBoxMax))) {
                ...
                if (1) {
                    if (pEntityRender->GetEntityRenderType() == eERType_Vegetation && fEntRadius < fExploRadius) { // remove this object
                        pSecInfo->m_lstEntities[STATIC_ENTITIES].Delete(i);
                        delete pEntityRender;
                        i--;
                    } else { // if something was imposible to destroy - disable deformation
...
void CTerrain::GetObjectsAround(Vec3d vExploPos, float fExploRadius, list2<IEntityRender*>* pEntList) {
    ...
    if (y1 < 0)
        y1 = 0;
    if (x2 >= CTerrain::GetTerrainSize())
        x2 = CTerrain::GetTerrainSize() - nUnitSize;
    if (y2 >= CTerrain::GetTerrainSize())
        y2 = CTerrain::GetTerrainSize() - nUnitSize;
