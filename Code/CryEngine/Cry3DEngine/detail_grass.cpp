...
public:
    ...
    GrassType(char* fname) { // load object and make buffers
        ...
        if (m_pObject->GetLeafBuffer()->m_pMats->Count() && m_pObject->GetLeafBuffer()->m_pMats->Get(0)->shaderItem.m_pShader &&
...
CDetailGrass::CDetailGrass(XDOM::IXMLDOMNodeListPtr pDetTexTagList) {
                ...
                while (pDetailObject = pDetailObjectsList->nextNode()) {
                    ...
                    if (pObjList) {
                        ...
                        while (pObj = pObjList->nextNode()) {
                            ...
                            if (l >= m_GrassModelsArray.Count()) { // not found
    ...
    for (int t = 0; t < m_GrassModelsArray.Count(); t++) {
        if (m_GrassModelsArray[t]->nTexID && m_GrassModelsArray[t]->nTexID != 4096) { // if some valid texture found - ise it for all objects
    ...
    m_pLeafBuffer = GetRenderer()->CreateLeafBufferInitialized(0 /*m_pGrassVertices*/, DETAIL_GRASS_PIP_BUFFER_SIZE, VERTEX_FORMAT_P3F_COL4UB_TEX2F, m_GrassIndices.GetElements(),
                                                               m_GrassIndices.Count(), R_PRIMV_TRIANGLES, "Grass", eBT_Dynamic, 1, 0, PrepareBufferCallback, this);
    ...
    //  if(pTerrain->m_nRenderStackLevel==0)
    {
        ...
        if (d2 > 16 && pThis->m_pTerrain) {
            ...
            if (pLeafBuffer->m_pVertexBuffer && pLeafBuffer->m_pVertexBuffer->m_vertexformat == VERTEX_FORMAT_P3F_COL4UB_TEX2F) {
                ...
                pThis->CreateSectorGrass(int(CAMERA_GRASS_SHIFT + 2), int(1), (struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F*)pLeafBuffer->m_pVertexBuffer->m_VS[VSF_GENERAL].m_VData);
...
void CDetailGrass::AddIndexedArray(GrassType* o, float X, float Y, float Z, float fObjBr, float fSizeRatio, float fXSign, float fYSign, int nSwapXY,
...
void CDetailGrass::CreateSectorGrassInUnit(const int x, const int y, const int nStep, struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F* pGrassVertices) {
...
void CDetailGrass::RenderDetailGrass(CTerrain* pTerrain) {
    ...
    */
    ...
    else { // nothing to draw now but we must to add render element to be able to generate mesh right before rendering
        m_pLeafBuffer->AddRenderElements(pObj, nDLMask, -1, (nFogId < pTerrain->m_lstFogVolumes.Count()) ? pTerrain->m_lstFogVolumes[nFogId].nRendererVolumeID : 0);
    }
}
