...
void CStatObj::ProcessStreamOnCompleteError() { // file was not loaded successfully
    m_eCCGFStreamingStatus = ecss_NotLoaded;

    // try to rebuild compiled file if not tried yet
    /*
    if(!m_bCompilingNotAllowed) {
        char szCompiledFileName[MAX_PATH_LENGTH];
        MakeCompiledFileName(szCompiledFileName, MAX_PATH_LENGTH);
        bool bCompRes = CompileObject(szCompiledFileName, m_szFileName, m_szGeomName, m_eVertsSharing, m_bLoadAdditinalInfo, m_bKeepInLocalSpace);
...
void CStatObj::StreamOnComplete(IReadStream* pStream, unsigned nError) {
    ...
    if (pStream->IsError()) { // file was not loaded successfully
    ...
    if (!pFileHeader->nDataSize) { // should happend only in case of sync loading
...
void CStatObj::CheckLoaded() {
    ...
    if (m_eCCGFStreamingStatus == ecss_NotLoaded) { // load now
        assert(m_bUseStreaming == true);
        m_fStreamingTimePerFrame -= GetTimer()->GetAsyncCurTime();
        bool bRes = Load(m_szFileName, m_szGeomName[0] ? m_szGeomName : 0, m_eVertsSharing, m_bLoadAdditinalInfo, m_bKeepInLocalSpace, false, m_bMakePhysics);
        m_bUseStreaming = true;
        m_fStreamingTimePerFrame += GetTimer()->GetAsyncCurTime();
    } else if (m_eCCGFStreamingStatus == ecss_LoadingInProgress) { // finish now
        m_pReadStream->Wait();
    } else { // object is ready
        assert(m_eCCGFStreamingStatus == ecss_Ready);
...
void CStatObj::MakeCompiledFileName(char* szCompiledFileName, int nMaxLen) {
    ...
    // remove extension
    for (int i = strlen(szFileNameNoExt) - 1; i > 0; i--)
        if (szFileNameNoExt[i] == '.') {
            szFileNameNoExt[i] = 0;
            break;
        }
    ...
    _snprintf(szCompiledFileName, nMaxLen, "%s\\%s_%s_%d_%d_%d.ccgf", CCGF_CACHE_DIR_NAME, szFileNameNoExt, szGeomName, int(m_eVertsSharing == evs_ShareAndSortForCache),
