...
void CLMSerializationManager2::AddRawLMData(const DWORD indwWidth, const DWORD indwHeight, const std::vector<int>& _vGLM_IDs_UsingPatch, BYTE* _pColorLerp4, BYTE* _pHDRColorLerp4,
                                            BYTE* _pDomDirection3, BYTE* _pOccl2) {
...
void CLMSerializationManager2::AddTexCoordData(const std::vector<TexCoord2Comp>& vTexCoords, int iGLM_ID_UsingTexCoord, const DWORD indwHashValue,
                                               const std::vector<std::pair<EntityId, EntityId>>& rOcclIDs) {
...
bool CLMSerializationManager2::_Load(const char* pszFileName, std::vector<IEntityRender*>* inpIGLMs, const bool cbLoadTextures) {
    ...
    for (UINT iCurLMTexData = 0; iCurLMTexData < sHeader.iNumLM_Pairs; ++iCurLMTexData) {
        ...
        if (inpIGLMs) {
            // Create lightmap
            RenderLMData_AutoPtr pNewLM = sHeader.iVersion == 2 ? CreateLightmap(strDirName, iCurLMTexData, LM.iWidth, LM.iHeight, false /*don't load occl maps*/)
    ...
    for (UINT iNumTexCoordSets = 0; iNumTexCoordSets < sHeader.iNumTexCoordSets; iNumTexCoordSets++) {
        ...
        for (UINT iCurTexCrd = 0; iCurTexCrd < vTexCoords.size(); iCurTexCrd++) {
            if (vTexCoords[iCurTexCrd].s >= 0.0f && vTexCoords[iCurTexCrd].s <= 1.0f && vTexCoords[iCurTexCrd].t >= 0.0f && vTexCoords[iCurTexCrd].t <= 1.0f)
                continue;
            ...
            for (itGLM = inpIGLMs->begin(); itGLM != inpIGLMs->end(); ++itGLM) {
                ...
                if (pICurGLM->GetEditorObjectId() == pUVh->nIdGLM && pICurGLM->GetEntityRS()) {
                    ...
                    if (sHeader.iVersion >= 3 && (strcmp(pICurGLM->GetEntityClassName(), "Brush") == 0)) {
                        ...
                        std::vector<std::pair<EntityId, EntityId>> vIDs;
                        vIDs.resize(puvh->ucOcclCount);
...
unsigned int CLMSerializationManager2::Save(const char *pszFilePath, LMGenParam rParam, const bool cbAppend) {
    ...
    for (UINT iCurLMTexData = 0; iCurLMTexData < m_vLightPatches.size(); ++iCurLMTexData) {
        ...
        if (iCurLMTexData >= uiStartIndex) {
            ...
            DDS_HEADER ddsh;
            if (0) { // save uncompressed version
                szName[0] = 'c';
                ...
                *MemBufferCounter = ' ';
                ...
                for (uint32 i = 0; i < sizeof(ddsh); i++) {
                    *MemBufferCounter = hsrcDXT3[i];
                    MemBufferCounter++;
                }

                if (!GetSystem()->GetIRenderer()->DXTCompress(&pColorMaps32[0x80], width, height,
                                                              eTF_DXT3,                // DXT-format
                                                              false,                   // use hardware
                                                              false,                       // mipmaps on/off
                                                              4,                       // bytes per pixel
            ...
            *MemBufferCounter = ' ';
            ...
            for (uint32 i = 0; i < sizeof(ddsh); i++) {
                *MemBufferCounter = hsrcDXT1[i];
                MemBufferCounter++;
            }

#ifdef USE_ALPHA_FOR_LOWSPEC
            if (!GetSystem()->GetIRenderer()->DXTCompress(&pSimpleLightmaps32[0x80], width, height,
                                                          eTF_DXT5,                // DXT-format
                                                          false,                   // use hardware
                                                          false,                   // mipmaps on/off
                                                          4,                       // bytes per pixel
...
RenderLMData *CLMSerializationManager2::CreateLightmap(const char *pszFileName, int nItem, UINT iWidth, UINT iHeight,
                                                       BYTE *pColorLerp4, BYTE *pHDRColorLerp, BYTE *pDomDirection3,
                                                       BYTE *pOccl2) {
    // Create a DOT3 Lightmap object
    IRenderer *pIRenderer = GetSystem()->GetIRenderer();
    int iColorLerpTex = 0, iHDRColorLerpTex = 0, iDomDirectionTex = 0, iOcclTex = 0;
...
void CLMSerializationManager2::RawLMData::initFromBMP(BitmapEnum t, const void* pSource) {
    ...
    case TEX_COLOR:
        ...
        break;
    case TEX_DOMDIR:
        pBMP = &m_DomDirection3;
        break;
    case TEX_OCCL:
        pBMP = &m_Occl2;
        break;
    case TEX_HDR:
        pBMP = &m_HDRColorLerp4;
        break;
    default:
        return;
    }
