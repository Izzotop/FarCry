...
void CObjManager::RegisterEntity(IEntityRender* pEntityRS) {
    ...
    */
    ...
    if (vCenter.x < 0 || vCenter.y < 0 || x < 0 || x >= CTerrain::GetSectorsTableSize() || y < 0 || y >= CTerrain::GetSectorsTableSize() ||
...
bool CObjManager::UnRegisterEntity(IEntityRender* pEntityRS) {
    ...
    if (nStatic) { // remove references to this entity
            ...
            if (pVegetList) {
                ...
                while (pVeget = pVegetList->nextNode()) {
...

                    {
                        ...
                        if (pFileName)
                            siGroup.pStatObj = MakeObject(pFileName->getText(), nullptr, evs_ShareAndSortForCache, true, false, false);

                        if (siGroup.pStatObj)
                            siGroup.pStatObj->CheckValidVegetation();

                        if (siGroup.pStatObj && siGroup.pStatObj->GetLeafBuffer() && siGroup.pStatObj->GetLeafBuffer()->m_pMats &&
                            siGroup.pStatObj->GetLeafBuffer()->m_pMats->Count() > 4)
                            GetLog()->Log("Warning: Number of materials in distributed object is %d", siGroup.pStatObj->GetLeafBuffer()->m_pMats->Count());
                        ...
                        if (siGroup.pStatObj->GetLeafBuffer() && !((CStatObj*)siGroup.pStatObj)->IsSpritesCreated() && !GetSystem()->IsDedicated())
                            ((CStatObj*)siGroup.pStatObj)
                                ->UpdateCustomLightingSpritesAndShadowMaps(m_vOutdoorAmbientColor, siGroup.nSpriteTexRes, siGroup.fBackSideLevel, siGroup.bCalcLighting);
...
void CObjManager::LoadVegetationModels(const char* szMissionName, bool bEditorMode) {
    if (GetCVars()->e_vegetation) {
        ...
        UpdateLoadingScreen("%d of %d static objects loaded in %.2f seconds", m_lstLoadedObjects.size() - nCountStart, m_lstLoadedObjects.size(), GetCurAsyncTimeSec() - time);
    }
}
...
CStatObj* CObjManager::MakeObject(const char* __szFileName, const char* _szGeomName, EVertsSharing eVertsSharing, bool bLoadAdditinalInfo, bool bKeepInLocalSpace,
                                  bool bLoadLater) {
    AUTO_PROFILE_SECTION(GetTimer(), CObjManager::m_dMakeObjectTime);

    if (!strcmp(__szFileName, "NOFILE")) { // make ampty object to be filled from outside
    ...
    *pszDest = 0;
    ...
    if (!_szGeomName || !*_szGeomName || strcmp(_szGeomName, "cloth")) { // [Anton] - always use new cgf for objects used for cloth simulation
...
CObjManager::CObjManager(C3DEngine* p3DEngine) : m_pDefaultCGF(nullptr) {
...
void CObjManager::UpdateCustomLighting(const Vec3d& vLight) {
    ...
    for (int i = 0; i < m_lstStaticTypes.Count(); i++) {
        if (m_lstStaticTypes[i].GetStatObj()) {
            m_lstStaticTypes[i].GetStatObj()->UpdateCustomLightingSpritesAndShadowMaps(m_vOutdoorAmbientColor, m_lstStaticTypes[i].nSpriteTexRes,
                                                                                       m_lstStaticTypes[i].fBackSideLevel, m_lstStaticTypes[i].bCalcLighting);
...
void CObjManager::AddPolygonToRenderer(const int nTexBindId, IShader* pShader, const int nDynLMask, Vec3d right, Vec3d up, const UCol& ucResCol, const ParticleBlendType eBlendType,
                                       const Vec3d& vAmbientColor, Vec3d vPos, const SColorVert* pTailVerts, const int nTailVertsNum, const byte* pTailIndices,
                                       const int nTailIndicesNum, const float fSortId, const int dwCCObjFlags, IMatInfo* pCustomMaterial, CStatObjInst* pStatObjInst,
                                       list2<struct ShadowMapLightSourceInstance>* pShadowMapCasters) {
    if (pStatObjInst && pStatObjInst->m_fFinalBending) { // transfer decal into object space
        Matrix44 objMat;
    ...
    if (pTailVerts && nTailVertsNum && pTailIndices && nTailIndicesNum)
        GetRenderer()->EF_AddSpriteToScene(pShader->GetID(), nTailVertsNum, (SColorVert*)pTailVerts, pOb, (byte*)pTailIndices, nTailIndicesNum);
}
