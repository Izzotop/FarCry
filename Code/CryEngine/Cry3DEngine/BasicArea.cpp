...
void CBasicArea::DrawEntities(int nFogVolumeID, int nDLightMask, bool bLMapGeneration, const CCamera& EntViewCamera, Vec3d* pvAmbColor, Vec3d* pvDynAmbColor,
                              VolumeInfo* pFogVolume, bool bNotAllInFrustum, float fSectorMinDist, CObjManager* pObjManager, bool bAllowBrushMerging, char* fake, uint nStatics) {
    ...
    if (nStatics && m_StaticEntitiesSorted && GetCVars()->e_optimized_render_object && !bLMapGeneration && pObjManager->m_fZoomFactor >= 0.99f) { // render statics compiled
        ...
        static list2<IEntityRenderInfo*> TmpEntList;
        TmpEntList.Clear();
        list2<struct IEntityRender*>& SrcEntList = m_lstEntities[STATIC_ENTITIES];

        if (GetCVars()->e_vegetation) { // fill simple vegetations
            ...
            for (int i = 0; i < m_lstStatEntInfoVegetNoCastersNoVolFog.Count(); i++) {
                ...
                if (bNotAllInFrustum && PlaneR.DistFromPlane(inf.m_vWSCenter) > inf.m_fWSRadius || PlaneR.DistFromPlane(inf.m_vWSCenter) > inf.m_fWSRadius)
                    continue;
            ...
            for (int i = 0; i < TmpEntList.Count(); i++) {
                ...
                if (i + 1 < TmpEntList.Count()) { // prefech next element
                ...
                pObjManager->RenderObjectVegetationNonCastersNoFogVolume(pEntityRender, nDLightMaskNoSun, EntViewCamera, bNotAllInFrustum, pEntityRender->m_fWSMaxViewDist,
                                                                         TmpEntList.GetAt(i));
            }
        }
        ...
        if (GetCVars()->e_vegetation || GetCVars()->e_brushes) { // fill complex objects
            ...
            for (int i = 0; i < m_lstStatEntInfoOthers.Count(); i++) {
                ...
                // early sphere test agains left and right camera planes
                if (bNotAllInFrustum && PlaneR.DistFromPlane(inf.m_vWSCenter) > inf.m_fWSRadius + TERRAIN_SECTORS_MAX_OVERLAPPING ||
            ...
            for (int i = 0; i < TmpEntList.Count(); i++) {
                ...
                pObjManager->RenderObject(pEntityRender, nFogVolumeID, nDLightMask, bLMapGeneration, EntViewCamera, pvAmbColor, pvDynAmbColor, pFogVolume, bNotAllInFrustum,
        ...
        for (int i = 0; i < EntList.Count(); i++) {
            ...
            if (i + 1 < EntList.Count()) { // prefech next element
        ...
        if ((GetFrameID() & 63) == 0 && fr != GetFrameID()) {
            fr = GetFrameID();
            if (t0 + t1 + t2)
                GetLog()->Log("<128 = %.1f, >128 = %.1f, NEG = %.1f", 0.1f * float((t0 * 1000 / (t0 + t1 + t2))), 0.1f * float((t1 * 1000 / (t0 + t1 + t2))),
                              0.1f * float((t2 * 1000 / (t0 + t1 + t2))));
        }
#endif                                               // VEGETATION_MEM_STATS
    } else if (nStatics || GetCVars()->e_entities) { // render dynamics or statics uncompiled
...
void CBasicArea::SortStaticInstancesBySize(VolumeInfo* pFogVolume) {
    ...
    for (int i = 0; i < m_lstEntities[STATIC_ENTITIES].Count(); i++) {
        m_lstEntities[STATIC_ENTITIES][i]->GetEntityRS()->fTmpDistance = -m_lstEntities[STATIC_ENTITIES][i]->GetMaxViewDist();
    ...
    if (m_lstEntities[STATIC_ENTITIES].Count())
        qsort(m_lstEntities[STATIC_ENTITIES].GetElements(), m_lstEntities[STATIC_ENTITIES].Count(), sizeof(m_lstEntities[STATIC_ENTITIES][0]), CObjManager__Cmp_EntTmpDistance);
    ...
    for (int i = 0; i < m_lstEntities[STATIC_ENTITIES].Count(); i++) {
        ...
        if (pFogVolume) { // fog is set only for outdoors
        ...
        if (!(pEntityRender->GetRndFlags() & (ERF_CASTSHADOWMAPS | ERF_CASTSHADOWVOLUME | ERF_RECVSHADOWMAPS | ERF_SELFSHADOW)) &&
            pEntityRender->GetEntityRenderType() == eERType_Vegetation && !bInFogVolume)
...
int __cdecl CBasicArea__Cmp_MatChunks(const void* v1, const void* v2) {
    ...
    /*
       ...
       : ""; const char * pName2 = ((*(((*(pMat2)).shaderItem).m_pShaderResources)).m_Textures)[0] ?
       (*(((*(((*(pMat2)).shaderItem).m_pShaderResources)).m_Textures)[0])).m_Name.c_str() : "";
...
void CBasicArea::MakeAreaBrush() {
    ...
    while (nEntityId < m_lstEntities[STATIC_ENTITIES].Count() || nNextStartId >= 0) {
        static list2<struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F> lstVerts;
        lstVerts.Clear();
        static list2<LMTexCoord> lstLMTexCoords;
        lstLMTexCoords.Clear();
        static list2<ushort> lstIndices;
        lstIndices.Clear();
        static list2<ushort> lstIndicesSorted;
        lstIndicesSorted.Clear();
        static list2<CMatInfo> lstChunks;
        lstChunks.Clear();
        static list2<CMatInfo> lstChunksMerged;
        lstChunksMerged.Clear();
        static list2<SPipTangents> lstTangBasises;
        ...
        for (; nEntityId < m_lstEntities[STATIC_ENTITIES].Count(); nEntityId++) {
            ...
            if (pEntityRender->m_dwRndFlags & ERF_MERGED || pEntityRender->m_dwRndFlags & ERF_CASTSHADOWVOLUME || pEntityRender->m_dwRndFlags & ERF_CASTSHADOWMAPS)
                continue;
            ...
            if (eType == eERType_Vegetation) {
                ...
                mathCalcMatrix(mat, pEntityRender->GetPos(), Vec3d(0, 0, 0), Vec3d(pEntityRender->GetScale(), pEntityRender->GetScale(), pEntityRender->GetScale()),
            ...
            for (int m = 0; m < pLB->m_pMats->Count(); m++) {
                ...
                if (GetCVars()->e_materials) { // Override default material
        ...
        CBrush* pAreaBrush = 0;
        if (!lstVerts.Count()) { // make empty brush - no geometry in sector
        ...
        // sort
        if (lstChunks.Count())
            qsort(lstChunks.GetElements(), lstChunks.Count(), sizeof(lstChunks[0]), CBasicArea__Cmp_MatChunks);

        // merge chunks
        for (int nChunk = 0; nChunk < lstChunks.Count(); nChunk++) {
            if (!nChunk || CBasicArea__Cmp_MatChunks(&lstChunks[nChunk], &lstChunks[nChunk - 1])) { // not equal materials - add new chunk
        ...
        // make leaf buffer
        CLeafBuffer* pAreaLB = GetRenderer()->CreateLeafBufferInitialized(lstVerts.GetElements(), lstVerts.Count(), VERTEX_FORMAT_P3F_COL4UB_TEX2F, lstIndices.GetElements(),
                                                                          lstIndices.Count(), R_PRIMV_TRIANGLES, "AreaLB", eBT_Static, lstChunks.Count());

        pAreaLB->UpdateTangBuffer(lstTangBasises.GetElements());

        for (int i = 0; i < lstChunks.Count(); i++) {
            pAreaLB->SetChunk(lstChunks[i].GetShaderItem().m_pShader, lstChunks[i].nFirstVertId, lstChunks[i].nNumVerts, lstChunks[i].nFirstIndexId, lstChunks[i].nNumIndices, i,
                              true);
...
int CBasicArea::GetLastStaticElementIdWithInMaxViewDist(float fMaxViewDist) { // use binary search to find range of elements visible on fMaxViewDist
