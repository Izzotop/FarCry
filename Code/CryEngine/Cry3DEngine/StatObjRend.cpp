...
void CStatObj::Render(const SRendParams& rParams, const Vec3& t, int nLodLevel) {
    ...
    pObj->m_Color = CFColor(rParams.vColor.x, rParams.vColor.y, rParams.vColor.z, rParams.fAlpha);

    if (rParams.pShaderParams && rParams.pShaderParams->Num())
        pObj->m_ShaderParams = rParams.pShaderParams;
    else if (m_ShaderParams.Num())
    ...
    for (int i = 0; i < m_lstLSources.Count(); i++)
        if (m_lstLSources[i].m_Flags & DLF_LOCAL)
            if (((m_lstLSources[i].m_Flags & DLF_HEATSOURCE) && pRend->EF_GetHeatVision()) || ((m_lstLSources[i].m_Flags & DLF_LIGHTSOURCE) && !pRend->EF_GetHeatVision()))
                if (rParams.fDistance < 256) {
                    if (rParams.pMatrix) {
                        m_lstLSources[i].m_Origin = rParams.pMatrix->TransformPointOLD(m_lstLSources[i].m_vObjectSpacePos);
                    } else {
    ...
    */
    ...
    if (GetCVars()->e_overlay_geometry >= 2) {
        bNotCurArea = (!rParams.pCaller || !GetVisAreaManager() ||
                       ((CVisArea*)rParams.pCaller->GetEntityVisArea() &&
                        !((CVisArea*)rParams.pCaller->GetEntityVisArea())->FindVisArea((CVisArea*)GetVisAreaManager()->m_pCurArea, 1, true)));
...
void CStatObj::RenderDebugInfo(const SRendParams& rParams, const CCObject* pObj) {
    ...
    pRend->Draw3dBBox(m_vBoxMin * rParams.fScale + rParams.vPos, m_vBoxMax * rParams.fScale + rParams.vPos);

    // scaled bbox (frustum culling check)
    if (GetCVars()->e_vegetation_debug == 2) {
        pRend->SetMaterialColor(1, 0, 1, 0.5f);
        pRend->Draw3dBBox(m_vBoxMin * rParams.fScale * 1.5f + rParams.vPos, m_vBoxMax * rParams.fScale * 1.5f + rParams.vPos);
    }
    ...
    { // cgf's name and tris num
        ...
        pRend->DrawLabelEx(rParams.vPos /*+GetCenter()*rParams.fScale*/, 0.75f, color, false, true, "%s(%d tris, %d mats)", szName, m_nLoadedTrisCount,
