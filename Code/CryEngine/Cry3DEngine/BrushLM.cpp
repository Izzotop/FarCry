...
void CBrush::SetLightmap(RenderLMData* pLMData, float* pTexCoords, UINT iNumTexCoords, int nLod) {
    ...
    if (pLeafBuffer->m_SecVertCount != iNumTexCoords) {
        char szBuffer[1024];
        sprintf(szBuffer,
                "Error: CBrush::SetLightmap: Object at position (%f, %f, %f) has"
                " texture mismatch (%i coordinates supplied, %i required)\r\n",
                GetPos().x, GetPos().y, GetPos().z, iNumTexCoords, pLeafBuffer->m_SecVertCount);
    ...
    m_arrLMData[nLod].m_pLMTCBuffer =
        GetRenderer()->CreateLeafBufferInitialized(&vTexCoord2[0], pLeafBuffer->m_SecVertCount, VERTEX_FORMAT_TEX2F, 0 /*pLeafBuffer->GetIndices(0)*/,
                                                   0 /*pLeafBuffer->m_Indices.m_nItems*/, R_PRIMV_TRIANGLES, "LMapTexCoords", eBT_Static, 1, 0, nullptr, nullptr, false, false);

    C3DEngine* pEng = (C3DEngine*)Get3DEngine();
    m_arrLMData[nLod].m_pLMTCBuffer->SetChunk(pEng->m_pSHDefault, 0, pLeafBuffer->m_SecVertCount, 0, pLeafBuffer->m_Indices.m_nItems);
}
