    ...
    if (m_nSegs > 0) {
        for (i = 0; i <= m_nSegs; i++) {
            if (m_segs[i].pContactEnt && m_segs[i].pContactEnt->m_iSimClass == 7) {
                m_segs[i].pContactEnt = nullptr;
            }
        }
        for (i = 0; i < 2; i++) {
            if (m_pTiedTo[i] && m_pTiedTo[i]->m_iSimClass == 7) {
                m_pTiedTo[i] = nullptr;
            }
        }
    }
...
int CRopeEntity::SetParams(pe_params* _params) {
    ...
    if (_params->type == pe_params_rope::type_id) {
        ...
        for (i = 0; i < 2; i++)
            if (!is_unused(params->pEntTiedTo[i])) {
                if (m_pTiedTo[i])
                    m_pTiedTo[i]->Release();
                m_pTiedTo[i] =
                    params->pEntTiedTo[i] == WORLD_ENTITY ? &g_StaticPhysicalEntity : (params->pEntTiedTo[i] ? ((CPhysicalPlaceholder*)params->pEntTiedTo[i])->GetEntity() : 0);
                if (!is_unused(params->idPartTiedTo[i])) {
                    for (m_iTiedPart[i] = 0; m_iTiedPart[i] < m_pTiedTo[i]->m_nParts && params->idPartTiedTo[i] != m_pTiedTo[i]->m_parts[m_iTiedPart[i]].id; m_iTiedPart[i]++)
                        ;
        ...
        if (m_pTiedTo[0] && (m_pTiedTo[0] == this || m_pTiedTo[0]->m_iSimClass == 3 || m_pTiedTo[0]->GetType() == PE_ROPE || m_pTiedTo[0]->GetType() == PE_SOFT) ||
            m_pTiedTo[1] && (m_pTiedTo[1] == this || m_pTiedTo[1]->m_iSimClass == 3 || m_pTiedTo[1]->GetType() == PE_ROPE ||
                             m_pTiedTo[1]->GetType() == PE_SOFT)) { // rope cannot be attached to such objects
            if (m_pTiedTo[0])
                m_pTiedTo[0]->Release();
            if (m_pTiedTo[1])
                m_pTiedTo[1]->Release();
...
int CRopeEntity::Step(float time_interval) {
    ...
    if (m_pTiedTo[0] && m_pTiedTo[1] && (ptend[1] - ptend[0]).len2() > sqr(m_length * 0.99f)) {
        ...
        m_BBox[0].x = min(m_segs[0].pt.x, m_segs[m_nSegs].pt.x);
        m_BBox[1].x = max(m_segs[0].pt.x, m_segs[m_nSegs].pt.x);
        m_BBox[0].y = min(m_segs[0].pt.y, m_segs[m_nSegs].pt.y);
        m_BBox[1].y = max(m_segs[0].pt.y, m_segs[m_nSegs].pt.y);
        m_BBox[0].z = min(m_segs[0].pt.z, m_segs[m_nSegs].pt.z);
        m_BBox[1].z = max(m_segs[0].pt.z, m_segs[m_nSegs].pt.z);
        a = max(max(m_BBox[1].x - m_BBox[0].x, m_BBox[1].y - m_BBox[0].y), m_BBox[1].z - m_BBox[0].z) * 0.01f + m_collDist * 2;
        m_BBox[0] -= vectorf(a, a, a);
        m_BBox[1] += vectorf(a, a, a);
    ...
    for (i = 0; i <= m_nSegs; i++) {
        ...
        m_BBox[0].x = min(m_BBox[0].x, m_segs[i].pt.x);
        m_BBox[1].x = max(m_BBox[1].x, m_segs[i].pt.x);
        m_BBox[0].y = min(m_BBox[0].y, m_segs[i].pt.y);
        m_BBox[1].y = max(m_BBox[1].y, m_segs[i].pt.y);
        m_BBox[0].z = min(m_BBox[0].z, m_segs[i].pt.z);
        m_BBox[1].z = max(m_BBox[1].z, m_segs[i].pt.z);
    }
    a = max(max(m_BBox[1].x - m_BBox[0].x, m_BBox[1].y - m_BBox[0].y), m_BBox[1].z - m_BBox[0].z) * 0.01f + m_collDist * 2;
    m_BBox[0] -= vectorf(a, a, a);
    m_BBox[1] += vectorf(a, a, a);
    ...
    if (m_flags & rope_collides) {
        ...
        nEnts = m_pWorld->GetEntitiesAround(m_BBox[0], m_BBox[1], pentlist,
                                            (m_flags & rope_collides_with_terrain ? ent_terrain : 0) | ent_static | ent_sleeping_rigid | ent_rigid | ent_living | ent_sort_by_mass |
                                                ent_ignore_noncolliding | ent_triggers,
                                            this);
            ...
            if (pentlist[i] != this)
                for (j = 0; j < pentlist[i]->m_nParts; j++) {
                    ...
                    if (box_box_overlap_check(&boxrope, &checkParts[nCheckParts].bbox)) {
                        ...
                        pentlist[i]->m_parts[j].pPhysGeomProxy->pGeom->PrepareForRayTest(pentlist[i]->m_parts[j].scale == 1.0f ? seglen : seglen / pentlist[i]->m_parts[j].scale);
        ...
        for (i = iStart; i != iEnd; i += iDir) {
            ...
            if (pent = m_segs[iseg].pContactEnt) {
                ...
                if (!m_segs[iseg].bRecheckContact &&
                    pent->m_parts[m_segs[iseg].iContactPart].pPhysGeomProxy->pGeom->FindClosestPoint(&gwd, m_segs[iseg].iPrim, m_segs[iseg].iFeature, m_segs[i + iDir].pt,
                                                                                                     m_segs[i].pt, ptres) >= 0 &&
            ...
            if (!m_segs[iseg].pContactEnt)
                for (j = nLocChecks = 0; j < nCheckParts && nLocChecks < 6; j++) {
                    ...
                    if (box_ray_overlap_check(&checkParts[j].bbox, &aray.m_ray)) {
                        ...
                        if (checkParts[j].pent->m_parts[checkParts[j].ipart].pPhysGeomProxy->pGeom->Intersect(&aray, &gwd, 0, &ip, pcontact) && pcontact->iUnprojMode == 1) {
    ...
    if (bHasContacts) {
        ...
        if (!m_pTiedTo[0])
            m_segs[0].vel += m_segs[0].dir * m_segs[0].P;
        if (!m_pTiedTo[1])
            m_segs[m_nSegs].vel -= m_segs[m_nSegs - 1].dir * m_segs[m_nSegs - 1].P;
