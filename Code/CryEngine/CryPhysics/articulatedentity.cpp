...
void CArticulatedEntity::RemoveGeometry(int id) {
    ...
    if (m_parts[i].mass > 0) {
        ...
        if (m_body.M < 1E-8)
            m_body.M = 0;
        if (m_joints[m_infos[i].iJoint].body.M < 1E-8)
            m_joints[m_infos[i].iJoint].body.M = 0;
    ...
    for (i = iStart; i <= iEnd; i++) {
        ...
        if (m_parts[i].mass > 0) {
            ...
            if (m_joints[m_infos[i].iJoint].body.M == 0) {
                m_joints[m_infos[i].iJoint].body.Create(bodypos, m_parts[i].pPhysGeom->Ibody * sqr(m_parts[i].scale) * cube(m_parts[i].scale), bodyq, V, m_parts[i].mass, m_qrot,
                                                        m_pos);
                m_joints[i].body.pOwner = this;
                m_joints[i].body.softness[0] = m_softness[0];
                m_joints[i].body.softness[1] = m_softness[1];
            } else
                m_joints[m_infos[i].iJoint].body.Add(bodypos, m_parts[i].pPhysGeom->Ibody * sqr(m_parts[i].scale) * cube(m_parts[i].scale), bodyq, V, m_parts[i].mass);
        }
    }
}
...
int CArticulatedEntity::SetParams(pe_params* _params) {
    ...
    if (_params->type == pe_params_joint::type_id) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (!is_unused(params->qext[i])) {
                ...
                nChanges++;
                if (!(m_joints[op[1]].flags & angle0_locked << i) && isneg(m_joints[op[1]].limits[0][i] - m_joints[op[1]].qext[i]) +
                                                                             isneg(m_joints[op[1]].qext[i] - m_joints[op[1]].limits[1][i]) +
                                                                             isneg(m_joints[op[1]].limits[1][i] - m_joints[op[1]].limits[1][i]) <
                                                                         2) { // qext violates limits; adjust the limits
                    float diff[2];
    ...
    if (_params->type == pe_params_articulated_body::type_id) {
        ...
        if (!is_unused(params->bExpandHinges))
            m_bExpandHinges = params->bExpandHinges;
...
int CArticulatedEntity::GetParams(pe_params* _params) {
    ...
    if (_params->type == pe_params_joint::type_id) {
        ...
        if (params->pMtx0)
            ((matrix3x3f&)*params->pMtx0) = matrix3x3f(m_joints[i].quat0);
        // if (params->pMtx0T) m_joints[i].quat0.getmatrix((matrix3x3Tf&)*params->pMtx0T);  //Q2M_IVO
        if (params->pMtx0T)
            ((matrix3x3Tf&)*params->pMtx0T) = matrix3x3f(m_joints[i].quat0);
...
int CArticulatedEntity::Action(pe_action* _action) {
    if (_action->type == pe_action_impulse::type_id) {
        ...
        m_joints[j].Pimpact += P;
        m_joints[j].Limpact += L;
...
void CArticulatedEntity::CheckForGimbalLock(int idx) {
    ...
    if (!(m_joints[idx].flags & angle0_locked * 5) && sqr(m_joints[idx].rotaxes[0] * m_joints[idx].rotaxes[2]) > 0.999f * 0.999f) {
        if (!(m_joints[idx].flags & angle0_locked * 7) && cry_fabsf(m_joints[idx].limits[1][0] - m_joints[idx].limits[0][0]) > 10 &&
            fabsf(m_joints[idx].limits[1][1] - m_joints[idx].limits[0][1]) > 10 &&
            fabsf(m_joints[idx].limits[1][2] - m_joints[idx].limits[0][2]) > 10) { // if joint is 3dof w/o limits, just rotate its quat0 to avoid gimbal lock
...
void CArticulatedEntity::SyncBodyWithJoint(int idx, int flags) {
    ...
    if (flags & 2) { // sync velocities
        if (m_joints[idx].iParent >= 0) {
            ...
            m_joints[idx].body.v =
                m_joints[m_joints[idx].iParent].body.v + (m_joints[m_joints[idx].iParent].body.w ^ m_joints[idx].body.pos - m_joints[m_joints[idx].iParent].body.pos);
        } else {
        ...
        if (!(m_joints[idx].flags & (angle0_locked | angle0_gimbal_locked) * 7)) {
            ...
            if (j == 1)
                ...
                if (!(m_joints[idx].flags & (angle0_locked | angle0_gimbal_locked) << 1)) {
                    ...
                    m_joints[idx].dq[iAxes[0]] =
                        wrel * m_joints[idx].rotaxes[iAxes[0]] - (m_joints[idx].rotaxes[iAxes[0]] * m_joints[idx].rotaxes[iAxes[1]]) * m_joints[idx].dq[iAxes[1]];
                }
            }
        }
    }
...
inline masktype getContactMask(int iContact) {
    return iContact >= 0 ? getmask(iContact) : 0;
}
...
int CArticulatedEntity::StepJoint(int idx, float time_interval, masktype& contact_mask, int& bBounced, int bFlying) {
    ...
    if (m_iSimTypeCur && m_bCollisionResp) {
        ...
        R = matrix3x3f(m_joints[idx].body.q);
        m_joints[idx].I = R * m_joints[idx].body.Ibody * R.T();
        m_joints[idx].dv_body.Set(0, 0, 0);
        m_joints[idx].dw_body.Set(0, 0, 0);
        ...
        for (i = 0; i < 3; i++)
            if (!(m_joints[idx].flags & (angle0_locked | angle0_gimbal_locked) << i)) {
                ...
                if (isneg(qlim[0] - q[i]) + isneg(q[i] - qlim[1]) + isneg(qlim[1] - qlim[0]) < 2) {
                    ...
                    diff[0] = q[i] - qlim[0];
                    tdiff = diff[0] - sgn(diff[0]) * (2 * pi);
                    if (fabsf(tdiff) < fabsf(diff[0]))
                        diff[0] = tdiff;
                    diff[1] = q[i] - qlim[1];
                    tdiff = diff[1] - sgn(diff[1]) * (2 * pi);
    ...
    pivot = m_joints[idx].iParent < 0 ? m_posPivot : m_joints[m_joints[idx].iParent].body.pos + m_joints[m_joints[idx].iParent].quat * m_joints[idx].pivot[0];
            ...
            for (i = 0; i < ncont; i++)
                if (pcontacts[i].vel == 0) {
                    ...
                    if (j == pcontacts[i].nborderpt /*idx>0*/) // register border center only for penetration contacts in marginal joints
                        contact_mask |= m_joints[idx].iContacts |=
                            getContactMask(RegisterContactPoint(contact_mask, i, pcontacts[i].center, pcontacts, pcontacts[i].iPrim[0], pcontacts[i].iFeature[0],
                                                                pcontacts[i].iPrim[1], pcontacts[i].iFeature[1], contact_new, pcontacts[i].t));
                    else {
                        ...
                        for (j = 0, prevpt.zero(); j < pcontacts[i].nborderpt; j++)
                            if ((pcontacts[i].ptborder[j] - prevpt).len2() > e) {
                                contact_mask |= m_joints[idx].iContacts |=
                                    getContactMask(RegisterContactPoint(contact_mask, i, pcontacts[i].ptborder[j], pcontacts, pcontacts[i].iPrim[0], pcontacts[i].iFeature[0],
                                                                        pcontacts[i].iPrim[1], pcontacts[i].iFeature[1], contact_new, pcontacts[i].t));
                    ...
                    m_parts[m_joints[idx].iStartPart].pPhysGeomProxy->pGeom->GetBBox(&bbox);
                    e = sqr(max(max(bbox.size.x, bbox.size.y), bbox.size.z) * 0.8f * (iszero(idx) ^ 1));
                    prevpt.zero();
                    for (j = 0; j < pcontacts[i].parea->npt; j++)
                        if ((pcontacts[i].parea->pt[j] - prevpt).len2() > e) {
                            contact_mask |= m_joints[idx].iContacts |=
                                getContactMask(RegisterContactPoint(contact_mask, i, pcontacts[i].parea->pt[j], pcontacts, pcontacts[i].parea->piPrim[0][j],
                    ...
                    contact_mask |= m_joints[idx].iContacts |= getContactMask(RegisterContactPoint(contact_mask, i, pcontacts[i].pt, pcontacts, pcontacts[i].iPrim[0],
                                                                                                   pcontacts[i].iFeature[0], pcontacts[i].iPrim[1], pcontacts[i].iFeature[1]));
        }
    }
...
int CArticulatedEntity::Step(float time_interval) {
    ...
    if (m_bCheckCollisions) {
        ...
        for (i = 0; i < m_nCollEnts; i++)
            if (m_pCollEntList[i]->m_iSimClass > 2 && m_pCollEntList[i]->GetType() != PE_ARTICULATED)
                m_pCollEntList[i]->Awake();

        if (m_iSimClass <= 2) {
            for (i = j = 0; i < m_nCollEnts; i++)
                if (m_pCollEntList[i] == this ||
                    m_pCollEntList[i]->m_iSimClass <= 2 && (m_pCollEntList[i]->m_iGroup == m_iGroup && m_pCollEntList[i]->m_bMoved || !m_pCollEntList[i]->IsAwake() ||
                                                            m_pWorld->m_pGroupNums[m_pCollEntList[i]->m_iGroup] < m_pWorld->m_pGroupNums[m_iGroup]))
...
int CArticulatedEntity::CollectPendingImpulses(int idx, int& bNotZero) {
    ...
    if (idx == 0) {
        ...
        vectornf(6, (float*)(m_Ya_vec[0])) =
            ::operator*((const matrix_tpl<float>&)matrixf(6, 6, 0, m_joints[0].fs->Ia_s_qinv_sT[0]), (const vectorn_tpl<float>&)vectornf(6, m_joints[0].fs->Ya_vec[0]));
#else
...
void CArticulatedEntity::GetJointTorqueResponseMatrix(int idx, matrix3x3f& K) {
    ...
    (K = Lw) *= -1.0;
    if (m_joints[idx].iParent >= 0) {
        matrix3x3in6x6f &ww_up((matrix3x3in6x6f&)*(matrix3x3in6x6f*)m_joints[idx].fs->s_qinv_sT_Ia[0]), &vw_up((matrix3x3in6x6f&)m_joints[idx].fs->s_qinv_sT_Ia[0][3]),
            &LL_down((matrix3x3in6x6f&)m_joints[idx].fs->Ia_s_qinv_sT[3][3]), &LP_down((matrix3x3in6x6f&)m_joints[idx].fs->Ia_s_qinv_sT[0][3]),
            &Lv_parent((matrix3x3in6x6f&)m_joints[m_joints[idx].iParent].fs->Iinv[3][3]), &Lw_parent((matrix3x3in6x6f&)m_joints[m_joints[idx].iParent].fs->Iinv[0][3]),
            ...
            if ((unsigned int)j - 1u < 2u) {
                ...
                if (m_iSimTypeCur && j == 1 && m_joints[idx].flags & joint_expand_hinge) {
                    if (is_unused(m_joints[idx].hingePivot[0])) {
                        ...
                        cyl.center.zero();
                        cyl.r = (bbox.size.x + bbox.size.y + bbox.size.z) * m_parts[i].scale * 0.05f;
                        cyl.hh = cyl.r * 0.2f;
...
int CArticulatedEntity::Update(float time_interval, float damping) {
    ...
    for (i = 0; i < m_nJoints; i++) {
        ...
        if (m_joints[i].iParent >= 0) {
            vectorf pivot[2], dpivot, dvel;
            int j = m_joints[i].iParent;
            ...
            dvel = m_joints[i].body.v + (m_joints[i].body.w ^ pivot[1] - m_joints[i].body.pos) - m_joints[j].body.v - (m_joints[j].body.w ^ pivot[0] - m_joints[j].body.pos);
