...
float COBBTree::BuildNode(int iNode, int iTriStart, int nTris, int nDepth) {
    ...
    for (iAxis = 0; iAxis < 3; iAxis++) {
        ...
        for (i = iTriStart; i < iTriStart + nTris; i++) {
            center = m_pMesh->m_pVertices[m_pMesh->m_pIndices[i * 3]] + m_pMesh->m_pVertices[m_pMesh->m_pIndices[i * 3 + 1]] + m_pMesh->m_pVertices[m_pMesh->m_pIndices[i * 3 + 2]];
        ...
        iMode[iAxis] = idxmax3(diff);
        nTrisAx[iAxis] = numtris[iMode[iAxis]];
...
void COBBTree::GetNodeChildrenBVs(const matrix3x3f& Rw, const vectorf& offsw, float scalew, const BV* pBV_parent, BV*& pBV_child1, BV*& pBV_child2) {
...
int COBBTree::GetNodeContents(int iNode, BV* pBVCollider, int bColliderUsed, int bColliderLocal, geometry_under_test* pGTest, geometry_under_test* pGTestOp) {
    return m_pMesh->GetPrimitiveList(m_pNodes[iNode].ichild, m_pNodes[iNode].ntris, pBVCollider->type, *pBVCollider, bColliderLocal, pGTest, pGTestOp, pGTest->primbuf,
                                     pGTest->idbuf);
}
...
void COBBTree::MarkUsedTriangle(int itri, geometry_under_test* pGTest) {
    ...
    if (!(pGTest->pUsedNodesMap[iNode >> 5] & 1 << (iNode & 31)) && max(max(m_pNodes[iNode].size.x, m_pNodes[iNode].size.y), m_pNodes[iNode].size.z) < m_maxSkipDim) {
        do {
