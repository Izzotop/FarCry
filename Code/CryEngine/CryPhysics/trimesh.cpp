...
CTriMesh* CTriMesh::CreateTriMesh(strided_pointer<const vectorf> pVertices, index_t* pIndices, const short* pIds, int nTris, int flags, bool bCopyTriangles, bool bCopyVertices,
                                  int nMinTrisPerNode, int nMaxTrisPerNode, float favorAABB) {
    __gtrimesh++;
    int i;
    m_nTris = nTris;
    ...
    if (flags & mesh_SingleBB) {
        ...
        if (flags & mesh_AABB) {
            ...
            if (m_pIds)
                memcpy(pTreeIds[nTrees - 1] = new short[m_nTris], m_pIds, sizeof(m_pIds[0]) * m_nTris);
            pTree = new CAABBTree;
    ...
    for (i = 0; i < m_nTris * 3; i++)
        pIndices[i] = pVtx[m_pIndices[i]].id;
    delete[] pVtx;

    for (i = 0; i < m_nTris; i++) // precalculate normals
        m_pNormals[i] = (m_pVertices[pIndices[i * 3 + 1]] - m_pVertices[pIndices[i * 3]] ^ m_pVertices[pIndices[i * 3 + 2]] - m_pVertices[pIndices[i * 3]]).normalized();
    ...
    for (i = 0; i < m_nTris; i++)
        for (j = 0; j < 3; j++) {
            for (k = 0; k < 3; k++) {
                ...
                for (; ibuddy > 0 && pIndices[vsort[k][ibuddy - 1] * 3 + k] == pIndices[i * 3 + j]; ibuddy--)
                    ; // find first such triangle if there are several
                for (; ibuddy < m_nTris && pIndices[vsort[k][ibuddy] * 3 + k] == pIndices[i * 3 + j]; ibuddy++)
                    if (vsort[k][ibuddy] != i && pIndices[i * 3 + inc_mod3[j]] == pIndices[vsort[k][ibuddy] * 3 + dec_mod3[k]]) {
                        m_pTopology[i].ibuddy[j] = vsort[k][ibuddy];
                        goto nextedge; // find triangle that shares this edge with the current one
                    }
            }
        nextedge:;
        }
    ...
    if (m_nTris > 0) {
        ...
        for (i = 0, j = 1; i < j; i++) {
            for (k = 0; k < 3; k++)
                if (m_pTopology[pIndices[i]].ibuddy[k] >= 0 && vsort[0][m_pTopology[pIndices[i]].ibuddy[k]] == 0)
                    if (j < m_nTris)
...
int CTriMesh::TraceTriangleInters(int iop, primitive* pprims[], int idx_buddy, int type_buddy, prim_inters* pinters, geometry_under_test* pGTest, border_trace* pborder) {
    ...
    do {
        ...
        if ((itri | iop << 31) == pborder->itri_end && GetEdgeByBuddy(itri, itri0) == pborder->iedge_end ||
            (pborder->npt > 3 && (pinters->pt[1] - pborder->pt_end).len2() < pborder->end_dist2) || itri < 0 || pborder->npt >= pborder->szbuf)
            return 2;
    ...
    have_inters:
        ...
        if ((pborder->pt[pborder->npt] - pborder->pt[pborder->iMark]).len2() < pborder->end_dist2 * 0.01f)
            return 0;
        --pborder->nLoop;
        pborder->iMark = pborder->iMark & ~(pborder->nLoop >> 31) | pborder->npt & pborder->nLoop >> 31;
...
int CTriMesh::GetUnprojectionCandidates(int iop, const contact* pcontact, primitive*& pprim, int*& piFeature, geometry_under_test* pGTest) {
    ...
    short idnullptr = -1, *pIds = (short*)((intptr_t)m_pIds & idmask | (intptr_t)&idnullptr & ~idmask);
...
int CTriMesh::GetPrimitiveList(int iStart, int nPrims, int typeCollider, primitive* pCollider, int bColliderLocal, geometry_under_test* pGTest, geometry_under_test* pGTestOp,
                               primitive* pRes, short* pResId) {
    intptr_t mask;
...
int CTriMesh::RegisterIntersection(primitive* pprim1, primitive* pprim2, geometry_under_test* pGTest1, geometry_under_test* pGTest2, prim_inters* pinters) {
    ...
    if ((pinters->iFeature[1][0] | pinters->iFeature[1][1]) & 0x80 && !pGTest[0]->pParams->bNoBorder) { // if at least one triangle ended
        ...
        do {
            ...
            res = ((CTriMesh*)pGTest[iop]->pGeometry)->TraceTriangleInters(iop, pprims, idx_prim[iop ^ 1], pGTest[iop ^ 1]->typeprim, pinters, pGTest[iop], &border);
    ...
    border.n_sum[1].Flip();
    n_avg = (border.ntris[0] < border.ntris[1] && border.ntris[0] || !border.ntris[1] ? border.n_sum[0] : border.n_sum[1]).normalized();
    ...
    for (i = 0; i < border.npt; i++) {
        vrel_loc = pGTest[0]->v - pGTest[1]->v + (pGTest[0]->w ^ border.pt[i] - pGTest[0]->centerOfMass) - (pGTest[1]->w ^ border.pt[i] - pGTest[1]->centerOfMass);
    ...
    end_unproj:
        ...
        if (!bUseLSNormal && unproj.imode == 1 && (contact_best.pt - unproj.center ^ unproj.dir).len2() < sqr(pGTest[0]->pParams->minAxisDist)) {
            ...
            unproj.vel = unproj.dir *
                         (pGTest[1]->v + (pGTest[1]->w ^ contact_best.pt - pGTest[1]->centerOfMass) - pGTest[0]->v - (pGTest[0]->w ^ contact_best.pt - pGTest[0]->centerOfMass));
            ...
            if (pinters->nBestPtVal > border.npt) {
                ...
                if (ntest.len2() < sqr(m_minVtxDist))
                    ...
                    for (i = 0, maxdet = 0; i < 3; i++) {
                        det = inters_C(inc_mod3[i], inc_mod3[i]) * inters_C(dec_mod3[i], dec_mod3[i]) - inters_C(dec_mod3[i], inc_mod3[i]) * inters_C(inc_mod3[i], dec_mod3[i]);
                    ...
                    if (fabs(maxdet) < 1E-20) // normal is ill defined
                        ...
                        det = 1.0 / maxdet;
                        n[j] = 1;
                        n[inc_mod3[j]] = -(inters_C(inc_mod3[j], j) * inters_C(dec_mod3[j], dec_mod3[j]) - inters_C(dec_mod3[j], j) * inters_C(inc_mod3[j], dec_mod3[j])) * det;
                        n[dec_mod3[j]] = -(inters_C(inc_mod3[j], inc_mod3[j]) * inters_C(dec_mod3[j], j) - inters_C(dec_mod3[j], inc_mod3[j]) * inters_C(inc_mod3[j], j)) * det;
                        n.normalize();
    ...
    if (bNoUnprojection) {
        ...
        if (bSurfaceSurfaceContact || bSurfaceEdgeContact) {
            ...
            if (bSurfaceSurfaceContact) {
                ...
                npt1 = pGTest[0]->pGeometry->PreparePolygon(&surface, pGTest[0]->surfaces[i].idx, pGTest[0]->surfaces[i].iFeature, pGTest[0], ptbuf1, idbuf1[0], idbuf1[1]);
                npt2 = pGTest[1]->pGeometry->PreparePolygon(&surface, pGTest[1]->surfaces[j].idx, pGTest[1]->surfaces[j].iFeature, pGTest[1], ptbuf2, idbuf2[0], idbuf2[1]);
                ...
                npt1 = pGTest[iop]->pGeometry->PreparePolygon(&surface, pGTest[iop]->surfaces[i].idx, pGTest[iop]->surfaces[i].iFeature, pGTest[iop], ptbuf1, idbuf1[0], idbuf1[1]);
                npt2 = pGTest[iop ^ 1]->pGeometry->PreparePolyline(&surface, pGTest[iop ^ 1]->edges[j].idx, pGTest[iop ^ 1]->edges[j].iFeature, pGTest[iop ^ 1], ptbuf2, idbuf2[0],
                                                                   idbuf2[1]);
    ...
    if (*pGTest[0]->pnContacts >= pGTest[0]->nMaxContacts || pGTest[0]->pParams->bStopAtFirstTri || pGTest[0]->pParams->bBothConvex)
        ...
        if (!pGTest[0]->pParams->bNoAreaContacts && g_nAreas < sizeof(g_AreaBuf) / sizeof(g_AreaBuf[0]) && g_nAreaPt < sizeof(g_AreaPtBuf) / sizeof(g_AreaPtBuf[0])) {
...
int CTriMesh::PreparePolygon(coord_plane* psurface, int iPrim, int iFeature, geometry_under_test* pGTest, vector2df*& ptbuf, int*& pVtxIdBuf, int*& pEdgeIdBuf) {
    int *pUsedVtxMap, UsedVtxIdx[16], nUsedVtx, *pUsedTriMap, UsedTriIdx[16], nUsedTri;
    int i, ihead, itail, ntri, nvtx, itri, itri_parent, ivtx0, ivtx, itri_new, iedge, iedge_open, ivtx0_new, ivtx_start, ivtx_end, ivtx_end1, ivtx_head, iorder, nSides;
    ...
    while (ntri > 0) {
        ...
        if (pUsedVtxMap[ivtx >> 5] & 1 << (ivtx & 31)) {
            ...
            if (nSides == 2) { // if triangle touches both neighbours, it must have filled interior hole, trace it and "glue" sides
                ...
                for (; g_VtxList[ivtx_start].ivtx == g_VtxList[ivtx_end].ivtx; ivtx_start = g_VtxList[ivtx_start].ibuddy[0], ivtx_end = g_VtxList[ivtx_end].ibuddy[1])
                    ;
...
int CTriMesh::PreparePolyline(coord_plane* psurface, int iPrim, int iFeature, geometry_under_test* pGTest, vector2df*& ptbuf, int*& pVtxIdBuf, int*& pEdgeIdBuf) {
    ...
    for (nvtx = 0, ivtx = iprev; ivtx >= 0 && nvtx < sizeof(g_VtxList) / sizeof(g_VtxList[0]); ivtx = g_VtxList[ivtx].ibuddy[1], nvtx++) {
        ...
        pEdgeIdBuf[nvtx] = g_VtxList[ivtx].ibuddy[1] >= 0 ? GetNeighbouringEdgeId(g_VtxList[ivtx].id, g_VtxList[g_VtxList[ivtx].ibuddy[1]].ivtx) : -1;
    }
    ptbuf[nvtx] = ptbuf[0];
    pVtxIdBuf[nvtx] = pVtxIdBuf[0];
    pEdgeIdBuf[nvtx] = pEdgeIdBuf[0];
...
int CTriMesh::FindClosestPoint(geom_world_data* pgwd, int& iPrim, int& iFeature, const vectorf& ptdst0, const vectorf& ptdst1, vectorf* ptres, int nMaxIters) {
...
void CTriMesh::PrepareForRayTest(float raylen) {
    if (m_nHashPlanes == 0) {
        ...
        for (i = 0; i < 3; i++, iPlane = inc_mod3[iPlane]) {
            ...
            HashTrianglesToPlane(hashplane, vector2df(bbox.size[inc_mod3[iPlane]] * 2, bbox.size[dec_mod3[iPlane]] * 2), m_hashgrid[i], m_pHashGrid[i], m_pHashData[i], rcellsize);
        }
        m_nHashPlanes = i;
    }
}

void CTriMesh::HashTrianglesToPlane(const coord_plane& hashplane, const vector2df& hashsize, grid& hashgrid, index_t*& pHashGrid, index_t*& pHashData, float rcellsize) {
...
void CTriMesh::CalcVolumetricPressure(geom_world_data* gwd, const vectorf& epicenter, float k, float rmin, const vectorf& centerOfMass, vectorf& P, vectorf& L) {
    int i, j, npt, iter;
...
int CTriMesh::DrawToOcclusionCubemap(const geom_world_data* pgwd, int iStartPrim, int nPrims, int iPass, int* pGrid[6], int nRes, float rmin, float rmax, float zscale) {
    vectorf pt[3];
