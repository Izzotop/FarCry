...
IGeometry* CGeomManager::CreateMesh(strided_pointer<const vectorf> pVertices, index_t* pIndices, const short* pIds, int nTris, int flags, bool bCopyTriangles, bool bCopyVertices,
                                    float approx_tolerance, int nMinTrisPerNode, int nMaxTrisPerNode, float favorAABB) {
...
IGeometry* CGeomManager::CreatePrimitive(int type, const primitive* pprim) {
    switch (type) {
    case cylinder::type:
        return (new CCylinderGeom)->CreateCylinder((cylinder*)pprim);
    case sphere::type:
        return (new CSphereGeom)->CreateSphere((sphere*)pprim);
    case box::type:
        return (new CBoxGeom)->CreateBox((box*)pprim);
    case heightfield::type:
        return (new CHeightfield)->CreateHeightfield((heightfield*)pprim);
    case ray::type:
        return new CRayGeom((ray*)pprim);
    }
    return 0;
}
