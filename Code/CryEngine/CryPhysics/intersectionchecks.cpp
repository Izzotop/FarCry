...
int tri_cylinder_intersection(const triangle* ptri, const cylinder* pcyl, prim_inters* pinters) {
    ...
    if (iStartBest < 0) { // triangle edges do not intersect cylinder
        ...
        if (inrange((pt - ptri->pt[0]) * ptri->n, (pcyl->hh + pcyl->r) * -0.2f, 0.0f) && (pt - ptri->pt[0] ^ ptri->pt[1] - ptri->pt[0]) * ptri->n < 0 &&
            (pt - ptri->pt[1] ^ ptri->pt[2] - ptri->pt[1]) * ptri->n < 0 && (pt - ptri->pt[2] ^ ptri->pt[0] - ptri->pt[2]) * ptri->n < 0 && pinters->nborderpt < pinters->nbordersz)
        ...
        for (i = 0, pt2d.set(1, 0); i < 8; i++) {
            ...
            pt2d.y = (t + pt2d.y) * (sqrt2 / 2); // rotate by 45 degrees
        }
        return 1;
    }
    ...
    if ((pcyl->axis ^ ptri->n).len2() > 0.001) {
        ...
        if ((pt - ptri->pt[0]) * ptri->n > (pcyl->hh + pcyl->r) * -0.2f && (pt - ptri->pt[0] ^ ptri->pt[1] - ptri->pt[0]) * ptri->n < 0 &&
            (pt - ptri->pt[1] ^ ptri->pt[2] - ptri->pt[1]) * ptri->n < 0 && (pt - ptri->pt[2] ^ ptri->pt[0] - ptri->pt[2]) * ptri->n < 0 &&
            pinters->nborderpt < pinters->nbordersz) {
            pinters->ptbest = pt;
            pinters->n = ptri->n;
            pinters->nBestPtVal = 1000;
            pinters->ptborder[pinters->nborderpt++] = pt;
...
int ray_tri_intersection(const ray* pray, const triangle* ptri, prim_inters* pinters) {
    ...
    if (sqr(t.y) > pinters->minPtDist2 * sqr(1E-4f)) {
        ...
        edge = ptri->pt[1] - ptri->pt[0];
        bOutside |= isneg(sqr_signed(ptri->n * (edge ^ pt - ptri->pt[0] * t.y)) + pinters->minPtDist2 * edge.len2() * nlen2);
        edge = ptri->pt[2] - ptri->pt[1];
        bOutside |= isneg(sqr_signed(ptri->n * (edge ^ pt - ptri->pt[1] * t.y)) + pinters->minPtDist2 * edge.len2() * nlen2);
        edge = ptri->pt[0] - ptri->pt[2];
        bOutside |= isneg(sqr_signed(ptri->n * (edge ^ pt - ptri->pt[2] * t.y)) + pinters->minPtDist2 * edge.len2() * nlen2);
...
int box_plane_intersection(const box* pbox, const plane* pplane, prim_inters* pinters) {
    vectorf dir = pbox->Basis.GetRow(0) * pbox->size.x * sgnnz(pbox->Basis.GetRow(0) * pplane->n) +
                  pbox->Basis.GetRow(1) * pbox->size.y * sgnnz(pbox->Basis.GetRow(1) * pplane->n) + pbox->Basis.GetRow(2) * pbox->size.z * sgnnz(pbox->Basis.GetRow(2) * pplane->n);
...
int cylinder_cylinder_intersection(const cylinder* pcyl1, const cylinder* pcyl2, prim_inters* pinters) {
    ...
    if (sina > 0.003f) {
        ...
        if (isneg(fabs_tpl(dc * axisx) - pcyl[0]->r - pcyl[1]->r) & isneg(fabs_tpl((dc ^ pcyl[1]->axis) * axisx) - pcyl[0]->hh * sina) &
    ...
    for (icyl = 0; icyl < 2; icyl++) {
        ...
        if (a > 0.0001f) {
            ...
            d = b * b - a * c;
            if (d > 0 && (root_inrange(a, 1, b, 0, d, pcyl[icyl]->hh) && root_inrange(a, cosa, b, -(center[0] * pcyl[icyl ^ 1]->axis) * a, d, pcyl[icyl ^ 1]->hh) ||
...
int cylinder_ray_intersection(const cylinder* pcyl, const ray* pray, prim_inters* pinters) {
    ...
    if (!bHit) {
        ...
        if (d >= 0) {
            d = sqrt_tpl(d);
            t.set(-b - d, a);
            bHitSide = bHit = isneg(fabs_tpl(t.x * 2 - t.y) - t.y) & isneg(fabs_tpl(((pray->origin - pcyl->center) * t.y + pray->dir * t.x) * pcyl->axis) - pcyl->hh * t.y);
            t.x += d * (bHit ^ 1) * 2;
            bHitSide = bHit = isneg(fabs_tpl(t.x * 2 - t.y) - t.y) & isneg(fabs_tpl(((pray->origin - pcyl->center) * t.y + pray->dir * t.x) * pcyl->axis) - pcyl->hh * t.y);
        }
    }
