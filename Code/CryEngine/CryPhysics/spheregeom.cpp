...
int CSphereGeom::GetPrimitiveList(int iStart, int nPrims, int typeCollider, primitive* pCollider, int bColliderLocal, geometry_under_test* pGTest, geometry_under_test* pGTestOp,
...
int CSphereGeom::FindClosestPoint(geom_world_data* pgwd, int& iPrim, int& iFeature, const vectorf& ptdst0, const vectorf& ptdst1, vectorf* ptres, int nMaxIters) {
...
void CSphereGeom::CalculateMediumResistance(const plane* pplane, const geom_world_data* pgwd, vectorf& dPres, vectorf& dLres) {
    ...
    if (vxn < 0.01f) {
        ...
        axisy = GetOrthogonal(vn).normalized();
        l = sgnnz(x) * r;
...
void CSphereGeom::CalcVolumetricPressure(geom_world_data* gwd, const vectorf& epicenter, float k, float rmin, const vectorf& centerOfMass, vectorf& P, vectorf& L) {
    vectorf dc = gwd->R * m_sphere.center * gwd->scale + gwd->offset - epicenter;
    P += dc * (k / (dc.len() * max(dc.len2(), sqr(rmin))));
}

int CSphereGeom::DrawToOcclusionCubemap(const geom_world_data* pgwd, int iStartPrim, int nPrims, int iPass, int* pGrid[6], int nRes, float rmin, float rmax, float zscale) {
    ...
    axisx = GetOrthogonal(center).normalized();
...
void CSphereGeom::DrawWireframe(void (*DrawLineFunc)(float*, float*), geom_world_data* gwd, int iLevel) {
    if (iLevel == 0) {
        ...
        for (iz = 0; iz < 3; iz++) {
            ix = inc_mod3[iz];
            iy = dec_mod3[iz];
            pt0[ix] = m_sphere.r * gwd->scale;
            pt0[iy] = pt0[iz] = pt1[iz] = 0;
            for (i = 0; i < 16; i++) {
