...
inline Vec3_tpl<short> EncodeVec6b(const vectorf& vec) {
    return Vec3_tpl<short>(float2int(min_safe(100.0f, max_safe(-100.0f, vec.x)) * (32767 / 100.0f)), float2int(min_safe(100.0f, max_safe(-100.0f, vec.y)) * (32767 / 100.0f)),
...
CLivingEntity::CLivingEntity(CPhysicalWorld* pWorld) : CPhysicalEntity(pWorld) {
    ...
    m_iHist = 0;
    int i;
    m_history = m_history_buf;
    m_szHistory = sizeof(m_history_buf) / sizeof(m_history_buf[0]);
    m_actions = m_actions_buf;
    m_szActions = sizeof(m_actions_buf) / sizeof(m_actions_buf[0]);
    for (i = 0; i < m_szHistory; i++) {
        m_history[i].dt = 1E10;
        m_history[i].pos.zero();
        m_history[i].v.zero();
        m_history[i].bFlying = 0;
        m_history[i].q.SetIdentity();
        m_history[i].timeFlying = 0;
        m_history[i].idCollider = -2;
        m_history[i].posColl.zero();
    }
    for (i = 0; i < m_szActions; i++) {
        m_actions[i].dt = 1E10;
        MARK_UNUSED m_actions[i].dir;
        m_actions[i].iJump = 0;
    }
    m_iAction = 0;
    m_bIgnoreCommands = 0;
...
CLivingEntity::~CLivingEntity() {
    ...
    if (m_pSensors)
        delete[] m_pSensors;
    if (m_pSensorsPoints)
        delete[] m_pSensorsPoints;
    if (m_pSensorsSlopes)
        delete[] m_pSensorsSlopes;
...
int CLivingEntity::SetParams(pe_params* _params) {
    ...
    if (res = CPhysicalEntity::SetParams(_params)) {
        if (_params->type == pe_params_pos::type_id) {
            if ((prevpos - m_pos).len2() > sqr(m_size.z * 0.01)) {
                ...
                m_vel.zero();
                m_dh = 0;
                for (int i = 0; i < m_szHistory; i++)
                    m_history[i].pos = m_pos;
            }
            if (sqr(m_qrot.v * prevq.v) < m_qrot.v.len2() * prevq.v.len2() * sqr(0.998f) && ((pe_params_pos*)_params)->bRecalcBounds && m_bActive &&
                !IsPositionFree(m_BBox, m_hCyl - m_hPivot, m_size)) {
                m_qrot = prevq;
                ComputeBBox();
                return 0;
            }
    ...
    if (_params->type == pe_player_dynamics::type_id) {
        ...
        if (!is_unused(params->gravity))
            m_gravity.Set(0, 0, -params->gravity);
        ...
        if (!is_unused(params->mass)) {
            m_mass = params->mass;
            m_massinv = 2.0f / m_mass;
        }
        if (!is_unused(params->surface_idx))
            m_surface_idx = m_parts[0].surface_idx = params->surface_idx;
        if (!is_unused(params->minSlideAngle))
            m_slopeSlide = cos_tpl(params->minSlideAngle * (pi / 180.0f));
        if (!is_unused(params->maxClimbAngle))
            m_slopeClimb = cos_tpl(params->maxClimbAngle * (pi / 180.0f));
        if (!is_unused(params->maxJumpAngle))
            m_slopeJump = cos_tpl(params->maxJumpAngle * (pi / 180.0f));
        if (!is_unused(params->minFallAngle))
            m_slopeFall = cos_tpl(params->minFallAngle * (pi / 180.0f));
...
int CLivingEntity::SetStateFromSnapshot(CStream& stm, int flags) {
    ...
    if (flags & ssf_no_update) {
        ...
        stm.Read(bnz);
        if (bnz)
            stm.Seek(stm.GetReadPos() + ((m_flags & lef_snap_velocities) ? 6 : sizeof(vectorf)) * 8);
        stm.Read(bnz);
        if (bnz)
            stm.Seek(stm.GetReadPos() + ((m_flags & lef_snap_velocities) ? 4 : sizeof(vectorf)) * 8);
        stm.Read(bnz);
        if (bnz)
    ...
    if (flags & ssf_compensate_time_diff && dt_state > 0 && dt_state < 3.0f && m_bActive) {
        ...
        float dt, dt_action;
        for (dt_action = dt_state, nActions = 0; dt_action > 0.00001f && nActions < m_szActions; dt_action -= m_actions[m_iAction].dt, --m_iAction &= m_szActions - 1, nActions++)
            ;
        for (dt = dt_state, nSteps = 0; dt > 0.00001f && nSteps < m_szHistory && m_history[m_iHist].dt < 1E9; dt -= m_history[m_iHist].dt, --m_iHist &= m_szHistory - 1, nSteps++)
            ;
        m_bStateReading = 1;
...
float CLivingEntity::ShootRayDown(CPhysicalEntity** pentlist, int nents, const vectorf& pos, vectorf& nslope, float time_interval, bool bUseRotation, bool bUpdateGroundCollider,
    ...
    if (bUseRotation)
        ...
        R.SetIdentity();
    CRayGeom aray;
    aray.CreateRay(pos + R * vectorf(0, 0, m_hCyl - m_hPivot), R * vectorf(0, 0, -m_hCyl - m_size.x), &(R * vectorf(0, 0, -1)));
    geom_world_data gwd;
    geom_contact* pcontacts;
                    ...
                    if ((ncont = pentlist[i]->m_parts[j].pPhysGeomProxy->pGeom->Intersect(&aray, &gwd, 0, 0, pcontacts)) && pcontacts[ncont - 1].pt.z > h) {
                        ...
                        if (pentlist[i]->m_iSimClass == 3) {
                            ...
                            if (nslope.len2() > 1E-4f)
                                ...
                                nslope.Set(1, 0, 0);
                            nslope *= 0.965925826f;
                            nslope.z = 0.2588190451f; // make it act like a 75 degrees slope
                        }
...
void CLivingEntity::AddLegsImpulse(const vectorf& vel, const vectorf& nslope, bool bInstantChange) {
    ...
    (ssca.ptTest = m_pos).z -= m_hPivot;
    ssca.dirTest = m_gravity;

    if (m_pLastGroundCollider && m_flags & lef_push_objects && (unsigned int)m_pLastGroundCollider->m_iSimClass - 1u < 2u &&
        m_pLastGroundCollider->m_flags & pef_pushable_by_players &&
        ((pbody = m_pLastGroundCollider->GetRigidBody(m_iLastGroundColliderPart))->Minv < m_massinv * 2 || (ncoll = m_pLastGroundCollider->GetColliders(pColliders)) == 0 ||
         (ncoll == 1 && pColliders[0] == m_pLastGroundCollider)) &&
        ...
        K(0, 0) += m_massinv * 0.5f;
        K(1, 1) += m_massinv * 0.5f;
        K(2, 2) += m_massinv * 0.5f;
...
void CLivingEntity::SyncWithGroundCollider(float time_interval) {
    ...
    if (m_pLastGroundCollider) {
        ...
        if ((newpos - m_pos).len2() > sqr(time_interval * 0.01f)) {
            ...
            m_BBox[0] += (newpos - m_pos);
            m_BBox[1] += (newpos - m_pos);
            m_pos = newpos;
        }
...
int CLivingEntity::Step(float time_interval) {
    ...
    if (m_bActive && (!(m_vel.len2() == 0 && m_velRequested.len2() == 0 && (!m_bFlying || m_gravity.len2() == 0) && m_dhSpeed == 0 && m_dhAcc == 0 &&
        ...
        if (m_pWorld->m_vars.iCollisionMode != 0 && m_pWorld->m_vars.bFlyMode) {
            ...
            movelen = move.len();
            h = (movelen + m_pWorld->m_vars.maxContactGapPlayer) * 1.5f;
            ...
            nents = m_pWorld->GetEntitiesAround(BBoxOuter[0], BBoxOuter[1], pentlist,
                                                ent_terrain | ent_static | ent_sleeping_rigid | ent_rigid | ent_living | ent_independent | ent_triggers | ent_sort_by_mass, this);
            ...
            for (i = j = bHasFastPhys = 0, vrel = 0; i < nents; i++) {
                ...
                if (idmat == PE_SOFT || idmat == PE_ROPE)
                    pentlist[i]->Awake();
                else if (pentlist[i]->m_iSimClass < 4 && (idmat != PE_LIVING || /*m_parts[0].flags&*/ pentlist[i]->m_parts[0].flags & geom_colltype_player &&
            ...
            for (i = nUnproj = 0; i < nents; i++)
                if (pentlist[i]->GetType() != PE_LIVING || pentlist[i]->m_nParts > 1) {
                            ...
                            if (pentlist[i]->m_parts[j].flags & geom_colltype_player) {
                                ...
                                if (pCyl[iCyl]->Intersect(pentlist[i]->m_parts[j].pPhysGeomProxy->pGeom, gwd, gwd + 1, &ip, pcontacts)) {
                                    if (iCyl == 0 && bHeavy) {
                                        if (m_pWorld->m_bWorldStep == 2) { // this means step induced by rigid bodies moving around
                                            ...
                                            for (icnt = 0; icnt < nUnproj - bPushOther; icnt++)
                                                if (unproj[icnt].n * pcontacts[0].n < 0) {
                                    ...
                                    if (pentlist[i]->m_iSimClass == 2) {
                                        K.SetZero();
                                        GetContactMatrix(pcontacts[0].center, 0, K);
            ...
            if (movelen > m_size.x * 1E-4f) {
                ...
                do {
                    ...
                    for (i = 0; i < nents; i++)
                        if (pentlist[i]->GetType() != PE_LIVING || pentlist[i]->m_nParts > 1) {
                            for (j = 0; j < pentlist[i]->m_nParts; j++)
                                if (pentlist[i]->m_parts[j].flags & geom_colltype_player) {
                                    ...
                                    if (m_bUseSphere && (ncont = m_SphereGeom.Intersect(pentlist[i]->m_parts[j].pPhysGeomProxy->pGeom, gwd, gwd + 1, &ip, pcontacts)) &&
                                        ...
                                        imin = i;
                                        jmin = j;
                                        idmat = pentlist[i]->m_parts[j].surface_idx & pcontacts[ncont - 1].id[1] >> 31 | max(pcontacts[ncont - 1].id[1], 0);
                                    }
                            ...
                            if (kd >= 0) {
                                ...
                                kd = sqrt_tpl(kd);
                                tfirst = -kb + kd;
                                ncoll = m_pos + gwd[0].v * tfirst - pentlist[i]->m_pos;
                                ...
                                if (tfirst > -m_size.x && tfirst < tmin && ncoll * move2d < 0 &&
                                    min(zup0 + gwd[0].v.z * tfirst, zup1) > max(zdown0 + gwd[0].v.z * tfirst, zdown1)) {
                                    tmin = tfirst;
                                    ncontact.Set(ncoll.x, ncoll.y, 0).normalize();
                                    ptcontact = m_pos + gwd[0].v * tfirst - ncontact * m_size.x;
                                    imin = i;
                                    jmin = 0;
                                    idmat = m_surface_idx;
                                }
                                ...
                                if (fabs_tpl(gwd[0].v.z) > m_size.z * 1E-5f) {
                                    ...
                                    if (inrange(tfirst, -m_size.x * gwd[0].v.z, tmin * gwd[0].v.z) &&
                                        ...
                                        ncontact.Set(0, 0, -j);
                                        (ptcontact = m_pos + gwd[0].v * tfirst).z += m_size.z * j;
                                        imin = i;
                                        jmin = 0;
                                        idmat = m_surface_idx;
                                    }
                    ...
                    if (tmin <= ip.time_interval) {
                        ...
                        if (pentlist[imin]->m_iSimClass > 0 || ncontact.z > m_slopeClimb || ncontact.z < -m_slopeFall || m_vel.z < -1.0f) {
                            K.SetZero();
                            GetContactMatrix(ptcontact, 0, K);
                            bPushOther =
                                (m_flags & (pentlist[imin]->GetType() == PE_LIVING ? lef_push_players : lef_push_objects)) && (pentlist[imin]->m_flags & pef_pushable_by_players);
                            if (bPushOther)
                ...
                if (bUnprojected || !(m_flags & lef_loosen_stuck_checks)) {
                    ...
                    for (i = 0; i < nents; i++)
                        if (pentlist[i]->GetType() != PE_LIVING && pentlist[i]->GetMassInv() * 0.4f < m_massinv) {
                            for (j = 0; j < pentlist[i]->m_nParts; j++)
                                if (pentlist[i]->m_parts[j].flags & geom_colltype_player) {
                                    //(pentlist[i]->m_qrot*pentlist[i]->m_parts[j].q).getmatrix(gwd[1].R); //Q2M_IVO
                                    gwd[1].R = matrix3x3f(pentlist[i]->m_qrot * pentlist[i]->m_parts[j].q);
            ...
            m_bFlying = m_pWorld->m_vars.bFlyMode || m_gravity.z > 0 || m_bSwimming ||
                        ((bGroundContact ^ 1) && !(m_bStuck || ncontactSum.len2() > 0.01f && sqr_signed(ncontactSum.z) > sqr(0.7f) * ncontactSum.len2()));
            ...
            if (m_bFlying) {
                ...
                if (m_nslope.z < m_slopeFall && !m_bStuck) {
                    m_bFlying = 1;
                    m_minFlyTime = 0.5f;
                    m_vel += vectorf(m_nslope.x, m_nslope.y, 0);
                }
            ...
            for (i = 0; i < m_nSensors; i++) {
                ...
                m_pSensorsPoints[i] = pt;
                m_pSensorsPoints[i].z = m_pos.z;
...
void CLivingEntity::StepBackEx(float time_interval, bool bRollbackHistory) {
    float dt;
    int idx, nSteps;
    for (dt = 0, idx = m_iHist, nSteps = 0; dt + 0.00001f < time_interval && nSteps < m_szHistory; dt += m_history[idx].dt, idx = idx - 1 & m_szHistory - 1, nSteps++)
        ;
    ...
    if (fabsf(a1) < 0.00001f || m_history[idx + 1 & m_szHistory - 1].dt == 0) {
        ...
        rdt = 1.0f / m_history[idx + 1 & m_szHistory - 1].dt;
        a0 *= rdt;
        a1 *= rdt;
    }
    ...
    SetGroundCollider(m_history[idx].idCollider > -2 ? (CPhysicalEntity*)m_pWorld->GetPhysicalEntityById(m_history[idx].idCollider) : 0);
...
void CLivingEntity::GetMemoryStatistics(ICrySizer* pSizer) {
    ...
    if (m_nSensors > 0) {
        if (m_pSensors)
            pSizer->AddObject(m_pSensors, m_nSensors * sizeof(m_pSensors[0]));
        if (m_pSensorsPoints)
            pSizer->AddObject(m_pSensorsPoints, m_nSensors * sizeof(m_pSensorsPoints[0]));
        if (m_pSensorsSlopes)
            pSizer->AddObject(m_pSensorsSlopes, m_nSensors * sizeof(m_pSensorsSlopes[0]));
    }
}