...
int CRigidEntity::AddGeometry(phys_geometry* pgeom, pe_geomparams* params, int id) {
    ...
    if (M > 0) {
        if (m_body.M == 0 || m_nParts == 1) {
            ...
            m_body.softness[0] = m_softness[0];
            m_body.softness[1] = m_softness[1];
        } else
...
int CRigidEntity::SetParams(pe_params* _params) {
    ...
    if (_params->type == pe_simulation_params::type_id) {
        ...
        if (!is_unused(params->softness)) {
            m_softness[0] = m_body.softness[0] = params->softness;
            bRecompute = true;
        }
        if (!is_unused(params->softnessAngular)) {
            m_softness[1] = m_body.softness[1] = params->softnessAngular;
            bRecompute = true;
        }
        if (bRecompute)
            RecomputeMassDistribution();
...
int CRigidEntity::Action(pe_action* _action) {
    if (_action->type == pe_action_impulse::type_id) {
        ...
        if (action->iApplyTime == 0) {
            ...
            m_Pext[action->iApplyTime - 1] += P;
            m_Lext[action->iApplyTime - 1] += L;
        }
    ...
    if (_action->type == pe_action_add_constraint::type_id) {
        ...
        if (is_unused(qframe[0] = action->qframe[0]))
            qframe[0].SetIdentity();
        if (is_unused(qframe[1] = action->qframe[1]))
        ...
        if (!is_unused(action->yzlimits[0]) && action->yzlimits[0] >= action->yzlimits[1]) {
            ...
            m_pConstraintInfos[i].qframe_rel[0] = qframe[0];
            m_pConstraintInfos[i].qframe_rel[1] = qframe[1];
            m_pConstraintInfos[i].limits[0] = action->yzlimits[0];
...
int CRigidEntity::AddCollider(CPhysicalEntity* pCollider) {
    ...
    if (m_nCollidersAlloc > nCollidersAlloc) {
        ...
        if (pColliderContacts)
            delete[] pColliderContacts;
        ...
        if (pColliderConstraints)
            delete[] pColliderConstraints;
    }
...
int CRigidEntity::GetPotentialColliders(CPhysicalEntity**& pentlist) {
    ...
    if (m_flags & ref_use_simple_solver)
        for (i = 0; i < m_nColliders; i++)
            m_pColliders[i]->m_bProcessed =
                m_pWorld->m_vars.bSkipRedundantColldet & iszero(m_pColliders[i]->m_nParts * 2 + m_nParts - 3) &
                ...
                isneg(3 - g_bitcount[m_pColliderContacts[i] & 0xFF] - g_bitcount[m_pColliderContacts[i] >> 8 & 0xFF] - g_bitcount[m_pColliderContacts[i] >> 16 & 0xFF] -
                      g_bitcount[m_pColliderContacts[i] >> 24 & 0xFF]);

    for (i = j = 0; i < nents; i++)
        if (!pentlist[i]->m_bProcessed) {
            ...
            } else if (((pentlist[i]->m_BBox[1] - pentlist[i]->m_BBox[0]).len2() == 0 || AABB_overlap(m_BBox, pentlist[i]->m_BBox)) && pentlist[i] != this &&
                       ((bSameGroup = iszero(pentlist[i]->m_iGroup - m_iGroup)) & pentlist[i]->m_bMoved || pentlist[i]->m_iGroup == -1 ||
                        //! pentlist[i]->IsAwake() ||
            ...
            for (ient = 0; ient < nents; ient++)
                for (j = 0, bCheckBBox = (pentlist[ient]->m_BBox[1] - pentlist[ient]->m_BBox[0]).len2() > 0; j < pentlist[ient]->m_nParts; j++)
                    if ((pentlist[ient]->m_parts[j].flags & m_parts[i].flagsCollider) && !(pentlist[ient] == this && !CheckSelfCollision(i, j)) &&
                        ...
                        pip->ptOutsidePivot[0] = m_parts[i].maxdim < pentlist[ient]->m_parts[j].maxdim ? prevOutsidePivot : vectorf(1E11f, 1E11f, 1E11f);
...
int CRigidEntity::RemoveContactPoint(CPhysicalEntity* pCollider, const vectorf& pt, float mindist2) {
    ...
    for (i = 0; i < m_nColliders && m_pColliders[i] != pCollider; i++)
        ;
    if (i < m_nColliders) {
        for (j = 0; j < NMASKBITS && getmask(j) <= m_pColliderContacts[i] && !(m_pColliderContacts[i] & getmask(j) && isneg((m_pContacts[j].pt[0] - pt).len2() - mindist2)); j++)
            ;
...
int CRigidEntity::RegisterContactPoint(masktype& contact_mask, int idx, const vectorf& pt, const geom_contact* pcontacts, int iPrim0, int iFeature0, int iPrim1, int iFeature1,
                                       int flags, float penetration) {
    ...
    for (i = 0; i < NMASKBITS && getmask(i) <= contact_mask &&
                !(contact_mask & getmask(i) && (m_pContacts[i].flags & contact_new || m_pContacts[i].penetration == 0) && m_pContacts[i].pent[1] == g_CurColliders[idx] &&
                  m_pContacts[i].pbody[1] == pbody1 &&
                  ((m_pContacts[i].iPrim[0] - iPrim0 | m_pContacts[i].iFeature[0] - iFeature0 | m_pContacts[i].iPrim[1] - iPrim1 | m_pContacts[i].iFeature[1] - iFeature1 |
                    bUseSimpleSolver ^ 1) == 0 ||
                   ((m_pContacts[i].flags & contact_new) ? (m_pContacts[i].pt[0] - pt).len2()
    ...
    if (i == NMASKBITS || getmask(i) > contact_mask) { // no existing point that is close enough to this one
        ...
        contact_mask |= getmask(i);
               ...
               (m_pContacts[i].penetration == 0 || m_pContacts[i].flags & contact_new) && (m_pContacts[i].penetration >= penetration || flags & contact_2b_verified))
        return -1;
    ...
    m_pContacts[i].pbody[1] = pbody1;
    m_pContacts[i].iPrim[0] = iPrim0;
    m_pContacts[i].iFeature[0] = iFeature0;
    m_pContacts[i].iPrim[1] = iPrim1;
    m_pContacts[i].iFeature[1] = iFeature1;
    m_pContacts[i].penetration = penetration;

    m_pContacts[i].vrel = m_pContacts[i].pbody[0]->v + (m_pContacts[i].pbody[0]->w ^ m_pContacts[i].pt[0] - m_pContacts[i].pbody[0]->pos) - m_pContacts[i].pbody[1]->v -
                          (m_pContacts[i].pbody[1]->w ^ m_pContacts[i].pt[0] - m_pContacts[i].pbody[1]->pos);
    ...
    m_pContacts[i].friction =
        0.5f * max(0.0f, m_pContacts[i].vrel.len2() < sqr(m_pWorld->m_vars.maxContactGap * 5)
    ...
    if (bUseSimpleSolver) {
        ...
        if (!(flags & contact_inexact)) {
            m_pContacts[i].ptloc[0] = ptloc[0];
            m_pContacts[i].ptloc[1] = ptloc[1];
        } else {
            vectorf ptfeat[4];
        ...
        float e = (m_pWorld->m_BouncinessTable[pcontacts[idx].id[0] & NSURFACETYPES - 1] + m_pWorld->m_BouncinessTable[pcontacts[idx].id[1] & NSURFACETYPES - 1]) * 0.5f;
        if ( // m_body.M<2 &&  // bounce only smalll objects
            ...
            !bUseSimpleSolver && e > 0 && m_pContacts[i].vrel * m_pContacts[i].n < -m_pWorld->m_vars.minBounceSpeed) { // apply bounce impulse if needed
            ...
            Action(&ai);
            ai.impulse.Flip();
            ai.partid = g_CurColliders[idx]->m_parts[m_pContacts[i].ipart[1]].id;
        ...
        if (penetration > 0) {
            ...
            m_pContacts[i].vreq =
                penetration > m_pWorld->m_vars.maxContactGap
                    ? m_pContacts[i].n * min(m_pWorld->m_vars.maxUnprojVel, (penetration - m_pWorld->m_vars.maxContactGap /*0.5f*/) * m_pWorld->m_vars.unprojVelScale)
                    : vectorf(zero);
...
void CRigidEntity::UpdatePenaltyContacts(float time_interval) {
    ...
    for (i = nContacts = 0; i < NMASKBITS && getmask(i) <= contact_mask; i++)
        if (contact_mask & getmask(i)) {
...
int CRigidEntity::UpdatePenaltyContact(int i, float time_interval) //, int bResolveInstantly,entity_contact **pContacts,int &nContacts)
{
    ...
    if (dpn < m_pWorld->m_vars.maxContactGapSimple) { //,(m_pContacts[i].r*n)*-2)) {
        ...
        if (dP * n > min(0.0f, (m_pContacts[i].dP * n) * -2.1f * bCanPull)) {
            ...
            for (j = 0; j < 2; j++, dP.flip()) {
                r = m_pContacts[i].pt[j] - m_pContacts[i].pbody[j]->pos;
                m_pContacts[i].pbody[j]->P += dP;
                m_pContacts[i].pbody[j]->L += r ^ dP;
...
int CRigidEntity::VerifyExistingContacts(float maxdist) {
            ...
            if (m_pColliderContacts[i] & getmask(j) && IsAwake(m_pContacts[j].ipart[0]) + m_pColliders[i]->IsAwake(m_pContacts[j].ipart[1]) > 0) {
                if (!(m_pContacts[j].flags & contact_new)) {
                    ...
                    if (m_pContacts[j].penetration == 0) {
                        ptres[0].Set(1E9f, 1E9f, 1E9f);
                        ptres[1].Set(1E11f, 1E11f, 1E11f);

                        pent = m_pContacts[j].pent[0];
                        ipart = m_pContacts[j].ipart[0];
                        ...
                        if ((m_pContacts[j].iFeature[0] & 0xFFFFFF60) != 0) {
                            ...
                            if (pent->m_parts[ipart].pPhysGeomProxy->pGeom->FindClosestPoint(&gwd, m_pContacts[j].iPrim[0], m_pContacts[j].iFeature[0], ptres[1], ptres[1],
                                                                                             ptclosest) < 0)
                                goto endcheck;
                            ptres[0] = ptclosest[0];
                        } else {
                        ...
                        pbody = m_pContacts[j].pbody[0];
                        m_pContacts[j].ptloc[0] = (ptres[0] - pbody->pos) * pbody->q;
                        m_pContacts[j].pt[0] = m_pContacts[j].pt[1] = ptres[0];

                        pent = m_pContacts[j].pent[1];
                        ipart = m_pContacts[j].ipart[1];
                        //(pent->m_qrot*pent->m_parts[ipart].q).getmatrix(gwd.R);  //Q2M_IVO
                        gwd.R = matrix3x3f(pent->m_qrot * pent->m_parts[ipart].q);
                        gwd.offset = pent->m_pos + pent->m_qrot * pent->m_parts[ipart].pos;
                        gwd.scale = pent->m_parts[ipart].scale;
                        if ((m_pContacts[j].iFeature[1] & 0xFFFFFF60) != 0) {
                            if (pent->m_parts[ipart].pPhysGeomProxy->pGeom->FindClosestPoint(&gwd, m_pContacts[j].iPrim[1], m_pContacts[j].iFeature[1], ptres[0], ptres[0],
                                                                                             ptclosest) < 0)
                                goto endcheck;
                            ptres[1] = ptclosest[0];
                        } else {
                        ...
                        pbody = m_pContacts[j].pbody[1];
                        m_pContacts[j].ptloc[1] = (ptres[1] - pbody->pos) * pbody->q;
                        ...
                        m_pContacts[j].friction = 0.5f * max(0.0f, m_pContacts[j].vrel.len2() < sqr(m_pWorld->m_vars.maxContactGap * 5)
                                                                       ? m_pWorld->m_FrictionTable[m_pContacts[j].id[0] & NSURFACETYPES - 1] +
                                                                             m_pWorld->m_FrictionTable[m_pContacts[j].id[1] & NSURFACETYPES - 1]
                                                                       : m_pWorld->m_DynFrictionTable[m_pContacts[j].id[0] & NSURFACETYPES - 1] +
                                                                             m_pWorld->m_DynFrictionTable[m_pContacts[j].id[1] & NSURFACETYPES - 1]);
                        ...
                        if (iszero(m_pContacts[j].iFeature[0] & 0x60) | iszero(m_pContacts[j].iFeature[0] & 0x60)) {
                            ...
                            for (j1 = 0; j1 < j; j1++)
                                if (m_pColliderContacts[i] & getmask(j1) &&
    ...
    for (i = m_nColliders - 1; i >= 0; i--)
        if ((m_pColliderContacts[i] | m_pColliderConstraints[i]) == 0 && !HasContactsWith(m_pColliders[i]) && !m_pColliders[i]->HasContactsWith(this)) {
            CPhysicalEntity* pCollider = m_pColliders[i];
            pCollider->RemoveCollider(this);
            RemoveCollider(pCollider);
        }
...
float CRigidEntity::CalcEnergy(float time_interval) {
    ...
    int i;
    masktype contact_mask, constraint_mask;
    ...
    if (time_interval > 0) {
        ...
        if (!(m_flags & ref_use_simple_solver))
            for (i = 0; i < NMASKBITS && getmask(i) <= contact_mask; i++)
                if (contact_mask & getmask(i)) {
        ...
        for (i = 0; i < NMASKBITS && getmask(i) <= constraint_mask; i++)
            if (constraint_mask & getmask(i) && (m_pConstraints[i].flags & contact_constraint_3dof || m_pConstraintInfos[i].flags & constraint_rope)) {
                v += m_pConstraints[i].n *
                     max(0.0f, max(0.0f, m_pConstraints[i].vreq * m_pConstraints[i].n - max(0.0f, m_pConstraints[i].vrel * m_pConstraints[i].n)) - m_pConstraints[i].n * v);
...
int CRigidEntity::Step(float time_interval) {
    ...
    int bUseSimpleSolver = isneg(-((int)m_flags & ref_use_simple_solver)), bUseDistanceContacts = m_pWorld->m_vars.bUseDistanceContacts & (bUseSimpleSolver ^ 1);
        ...
        if (ncontacts < 6)
            for (i = 0; i < ncontacts - 1; i++)
                for (j = i + 1; j < ncontacts; j++)
                    if (g_CurColliders[i] == g_CurColliders[j] && g_CurCollParts[i][1] == g_CurCollParts[j][1] && fabs_tpl(pcontacts[i].vel) + fabs_tpl(pcontacts[j].vel) == 0 &&
        ...
        if (i >= 0) {                          // first unproject tmax contact to exact collision time (to get ptloc fields in entity_contact filled correctly)
            if (pcontacts[i].iUnprojMode == 0) // check if unprojection conflicts with existing contacts
                ...
                for (j = 0; j < NMASKBITS && getmask(j) <= contact_mask; j++)
                    if (contact_mask & getmask(j)) {
                        axis = pcontacts[i].dir ^ m_pContacts[j].pt[0] - ip.centerOfRotation;
                        r = axis.len2();
                    ...
                    if (pcontacts[i].vel > 0 && pcontacts[i].t > tmax - e) { // timed contact in unprojection gap range
                        if (pcontacts[i].parea && pcontacts[i].parea->npt > 1) {
                            ...
                            for (j = 0; j < pcontacts[i].parea->npt; j++) // do not set contact_new flag for areas since contacts are not precise and should be verified
                                RegisterContactPoint(contact_mask, i, pcontacts[i].parea->pt[j], pcontacts, pcontacts[i].parea->piPrim[0][j], pcontacts[i].parea->piFeature[0][j],
                                                     pcontacts[i].parea->piPrim[1][j], pcontacts[i].parea->piFeature[1][j], contact_inexact);
                        } else
                            RegisterContactPoint(contact_mask, i, pcontacts[i].pt, pcontacts, pcontacts[i].iPrim[0], pcontacts[i].iFeature[0], pcontacts[i].iPrim[1],
                                                 pcontacts[i].iFeature[1]);
                    } else
                ...
                if (pcontacts[i].t >= 0) { // penetration contacts - register points and add additional penalty impulse in solver
                    ...
                    if (pcontacts[i].parea && pcontacts[i].parea->npt > 2) {
                        for (j = 0; j < pcontacts[i].parea->npt; j++)
                            RegisterContactPoint(contact_mask, i, pcontacts[i].parea->pt[j], pcontacts, pcontacts[i].parea->piPrim[0][j], pcontacts[i].parea->piFeature[0][j],
                                                 pcontacts[i].parea->piPrim[1][j], pcontacts[i].parea->piFeature[1][j], contact_2b_verified | contact_new | contact_inexact);
                        ...
                        RegisterContactPoint(contact_mask, i, pcontacts[i].pt, pcontacts, pcontacts[i].iPrim[0], pcontacts[i].iFeature[0], pcontacts[i].iPrim[1],
                                             pcontacts[i].iFeature[1], contact_2b_verified | contact_new);
                    if (!bUseSimpleSolver)
                        for (j = 0; j < pcontacts[i].nborderpt; j++)
                            if ((itmax = RegisterContactPoint(contact_mask, i, pcontacts[i].ptborder[j], pcontacts, pcontacts[i].iPrim[0], pcontacts[i].iFeature[0],
                                                              pcontacts[i].iPrim[1], pcontacts[i].iFeature[1], contact_new,
                                                              max(0.001f, (float)pcontacts[i].t + axis * pcontacts[i].ptborder[j] - r))) >= 0)
            ...
            if (!bComplexCollider) {
                ...
                if (bStable && dist < min(mindim, mindim1) * 0.05f) {
                    ...
                    if (pContactLin && pContactAng) {
                        ...
                        pContactLin->vreq =
                            pContactLin->n * min(m_pWorld->m_vars.maxUnprojVel, max(0.0f, (penetration - m_pWorld->m_vars.maxContactGap)) * m_pWorld->m_vars.unprojVelScale);
...
void CRigidEntity::UpdateContactsAfterStepBack(float time_interval) {
    ...
    for (i = 0; i < NMASKBITS && getmask(i) <= contact_mask; i++)
        if (contact_mask & getmask(i)) {
            diff = m_pContacts[i].pbody[0]->q * m_pContacts[i].ptloc[0] + m_pContacts[i].pbody[0]->pos - m_pContacts[i].pbody[1]->q * m_pContacts[i].ptloc[1] -
                   m_pContacts[i].pbody[1]->pos;
...
int CRigidEntity::Update(float time_interval, float damping) {
    ...
    for (i = 0, contacts_mask = 0, collFlags = 0; i < m_nColliders; i++) {
        ...
        collFlags |= m_pColliders[i]->m_flags;
    }
    m_body.v *= damping;
    m_body.w *= damping;
    m_body.P *= damping;
    ...
    if (m_body.Minv > 0) {
        ...
        if (m_flags & ref_use_simple_solver)
            E = min(E, m_Estep);
        if (m_bStable < 2 &&
            g_bitcount[m_nRestMask & 0xFF] + g_bitcount[m_nRestMask >> 8 & 0xFF] + g_bitcount[m_nRestMask >> 16 & 0xFF] + g_bitcount[m_nRestMask >> 24 & 0xFF] < 20)
...
int CRigidEntity::WriteContacts(CStream& stm, int flags) {
    int i, j, imax;
    masktype contacts;
    for (i = 0, contacts = 0; i < m_nColliders; i++)
    ...
    for (i = 0; i < m_nColliders; i++) {
        ...
        for (j = 0, contacts = 0; j < imax; j++)
            if (m_pColliderContacts[i] & getmask(j) && m_pContacts[j].penetration == 0)
                contacts |= getmask(j);
        // contacts = m_pColliderContacts[i];
        WritePacked(stm, contacts);
        WritePacked(stm, m_pColliderConstraints[i]);
        for (j = 0; j < NMASKBITS; j++)
            if (contacts & getmask(j)) {
...
int CRigidEntity::SetStateFromSnapshot(CStream& stm, int flags) {
    ...
    if (m_pWorld->m_iTimeSnapshot[2] != 0) {
        if (m_bAwake && m_checksums[iBound[0] & NCHECKSUMS - 1].iPhysTime != m_pWorld->m_iTimeSnapshot[2])
            m_pWorld->m_pLog->Log("Rigid Entity: time not in list (%d, bounds %d-%d) (id %d)", m_pWorld->m_iTimeSnapshot[2], m_checksums[iBound[0] & NCHECKSUMS - 1].iPhysTime,
                                  m_checksums[iBound[1] & NCHECKSUMS - 1].iPhysTime, m_id);
    ...
    if (!bnz) {
        ...
        if (!(flags & ssf_no_update)) {
            ...
            stm.Seek(stm.GetReadPos() + sizeof(vectorf) * 8 + (m_pWorld->m_vars.bMultiplayer ? CMP_QUAT_SZ + CMP_VEL_SZ * 2 : (sizeof(quaternionf) + 2 * sizeof(vectorf)) * 8));
            stm.Read(bnz);
            if (bnz)
...
int CRigidEntity::ReadContacts(CStream& stm, int flags) {
    ...
    if (!(flags & ssf_no_update)) {
        ...
        for (i = 0; i < m_nColliders; i++) {
            ...
            for (j = 0; j < NMASKBITS; j++)
                if (m_pColliderContacts[i] & getmask(j)) {
                    ...
                    stm.Read(bnz);
                    m_pContacts[j].flags = bnz ? contact_2b_verified : 0;
                    ...
                    m_pContacts[j].pt[0] = m_pContacts[j].pt[1] = m_pContacts[j].pbody[0]->q * m_pContacts[j].ptloc[0] + m_pContacts[j].pbody[0]->pos;
                }
        }
        ...
        for (i = 0; i < nColliders; i++) {
            ReadPacked(stm, id);
            ReadPacked(stm, iContacts);
            ReadPacked(stm, dummy);
            for (j = 0; j < NMASKBITS; j++)
                if (iContacts & getmask(j)) {
...
int CRigidEntity::PostSetStateFromSnapshot() {
    if (m_bJustLoaded) {
        ...
        for (i = contact_mask = 0; i < m_nColliders; i++)
            contact_mask |= m_pColliderContacts[i];
            ...
            if (contact_mask & getmask(i)) {
                ...
                if (m_pContacts[i].pent[1] && (unsigned int)m_pContacts[i].ipart[0] < (unsigned int)m_pContacts[i].pent[0]->m_nParts &&
                    ...
                    if (m_pContacts[i].pent[1]->GetStatus(&si)) {
                        ...
                        m_pContacts[i].friction = 0.5f * max(0.0f, m_pWorld->m_FrictionTable[m_pContacts[i].id[0] & NSURFACETYPES - 1] +
                                                                       m_pWorld->m_FrictionTable[m_pContacts[i].id[1] & NSURFACETYPES - 1]);
                    } else
                        contact_mask &= ~getmask(i);
                } else
                    contact_mask &= ~getmask(i);
        ...
        for (i = m_nColliders - 1; i >= 0; i--) {
            ...
            if (!(m_pColliders[i] = (CPhysicalEntity*)m_pWorld->GetPhysicalEntityById((int)m_pColliders[i])) || !(m_pColliderContacts[i] | m_pColliderConstraints[i])) {
...
unsigned int CRigidEntity::GetStateChecksum() {
    return //*(unsigned int*)&m_testval;
        float2int(m_pos.x * 1024) ^ float2int(m_pos.y * 1024) ^ float2int(m_pos.z * 1024) ^ float2int(m_qrot.v.x * 1024) ^ float2int(m_qrot.v.y * 1024) ^
        float2int(m_qrot.v.z * 1024) ^ float2int(m_body.v.x * 1024) ^ float2int(m_body.v.y * 1024) ^ float2int(m_body.v.z * 1024) ^ float2int(m_body.w.x * 1024) ^
        float2int(m_body.w.y * 1024) ^ float2int(m_body.w.z * 1024);
}
    ...
    for (int i = 0; i < m_nParts; i++)
        if (m_parts[i].flags & geom_floats) {
            ...
            if (m_waterResistance > 0) {
                ...
                ai.impulse *= m_waterResistance * time_interval;
                ai.momentum *= m_waterResistance * time_interval;
            ...
            if (dist > -r) {
                ...
                ai.impulse += dP;
                ai.momentum += center - pbody->pos ^ dP;
                m_bFloating = 1;
            } else {
...
int CRigidEntity::CompactContactBlock(masktype& contact_mask, float maxPlaneDist, int nMaxContacts, int& nContacts, vectorf& n, float& maxDist, const vectorf& ptTest,
                                      const vectorf& dirTest) {
    int i, nEdges;
    matrix3x3f C, Ctmp;
    ...
    for (i = nContacts = 0, p_avg.zero(); i < NMASKBITS && getmask(i) <= contact_mask; i++)
        if (contact_mask & getmask(i)) {
            p_avg += m_pContacts[i].pt[0];
            pts[nContacts++].iContact = i;
        }
    ...
    if (nMaxContacts) { // chop off vertices until we have only the required amount left
        ...
        for (i = 0; i < nEdges; i++) {
            ...
            edges[i].areanorm2 = edge[0].len2() * edge[1].len2();
            edges[i].idx = i;
        }
        ...
        for (pedge = pminedge, i = 1; i < nEdges; i++) {
            ...
            pedge = pedge->next1 = edges + edges[i].idx;
        }
        ...
        while (nEdges > 2 && (nEdges > nMaxContacts || sqr(pminedge->area) < sqr(0.15f) * pminedge->areanorm2)) {
            ...
            for (i = 0; i < 2; i++) {
                ...
                pedge[i]->areanorm2 = edge[0].len2() * edge[1].len2(); // update area
