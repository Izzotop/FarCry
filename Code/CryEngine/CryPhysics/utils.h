...
template <class ftype, int stridei, int stridej> void OffsetInertiaTensor(Matrix33_tpl<ftype, stridei, stridej>& I, const Vec3_tpl<ftype>& center, ftype M) {
...
inline void LeftOffsetSpatialMatrix(matrixf& mtx, const vectorf& offset) {
    matrix3x3f rmtx;
    matrix3x3in6x6f &A((matrix3x3in6x6f&)mtx.data[0]), &B((matrix3x3in6x6f&)mtx.data[3]), &C((matrix3x3in6x6f&)mtx.data[18]), &D((matrix3x3in6x6f&)mtx.data[21]);
...
int BreakPolygon(vector2df* ptSrc, int nPt, int nCellx, int nCelly, int maxPatchTris, vector2df*& ptout, int*& nPtOut, float jointhresh = 0.5f, int seed = -1);
...
template <class CellChecker> int DrawRayOnGrid(primitives::grid* pgrid, vectorf& origin, vectorf& dir, CellChecker& cell_checker) {
    ...
    ishort = ilong ^ 1;
    dshort = fabs_tpl(dir[ishort]);
    dlong = fabs_tpl(dir[ilong]);
...
template <class T> inline T* _align16(T* ptr) {
    return (T*)(((INT_PTR)ptr - 1 & ~15) + 16);
}
#if !defined(LINUX)
template <class dtype> bool is_valid(const dtype& op) {
...
#define VALIDATORS_START                                                                                                                                                           \
    bool validate(const char* strSource, ILog* pLog, const vectorf& pt, IPhysicsStreamer* pStreamer, void* param0, int param1, int param2) {                                       \
        bool res = true;                                                                                                                                                           \
        char errmsg[256];
#define VALIDATOR(member)                                                                                                                                                          \
    if (!is_unused(member) && !is_valid(member)) {                                                                                                                                 \
        res = false;                                                                                                                                                               \
        sprintf(errmsg, "\002%s: (%.50s @ %.1f,%.1f,%.1f) Validation Error: %s is invalid", strSource, pStreamer->GetForeignName(param0, param1, param2), pt.x, pt.y, pt.z,        \
                #member);                                                                                                                                                          \
        VALIDATOR_LOG(pLog, errmsg);                                                                                                                                               \
    }
#define VALIDATOR_NORM(member)                                                                                                                                                     \
    if (!is_unused(member) && !(is_valid(member) && fabs_tpl((member | member) - 1.0f) < 0.01f)) {                                                                                 \
        res = false;                                                                                                                                                               \
        sprintf(errmsg, "\002%s: (%.50s @ %.1f,%.1f,%.1f) Validation Error: %s is invalid or unnormalized", strSource, pStreamer->GetForeignName(param0, param1, param2), pt.x,    \
                pt.y, pt.z, #member);                                                                                                                                              \
        VALIDATOR_LOG(pLog, errmsg);                                                                                                                                               \
    }
#define VALIDATOR_NORM_MSG(member, msg, member1)                                                                                                                                   \
    if (!is_unused(member) && !(is_valid(member) && fabs_tpl((member | member) - 1.0f) < 0.01f)) {                                                                                 \
        res = false;                                                                                                                                                               \
        sprintf(errmsg, "\002%s: (%.50s @ %.1f,%.1f,%.1f) Validation Error: %s is invalid or unnormalized %s", strSource, pStreamer->GetForeignName(param0, param1, param2), pt.x, \
                pt.y, pt.z, #member, msg);                                                                                                                                         \
        if (!is_unused(member1))                                                                                                                                                   \
            sprintf(errmsg + strlen(errmsg), " " #member1 ": %.1f,%.1f,%.1f", member1.x, member1.y, member1.z);                                                                    \
        VALIDATOR_LOG(pLog, errmsg);                                                                                                                                               \
    }
#define VALIDATOR_RANGE(member, minval, maxval)                                                                                                                                    \
    if (!is_unused(member) && !(is_valid(member) && member >= minval && member <= maxval)) {                                                                                       \
        res = false;                                                                                                                                                               \
        sprintf(errmsg, "\002%s: (%.50s @ %.1f,%.1f,%.1f) Validation Error: %s is invalid or out of range", strSource, pStreamer->GetForeignName(param0, param1, param2), pt.x,    \
                pt.y, pt.z, #member);                                                                                                                                              \
        VALIDATOR_LOG(pLog, errmsg);                                                                                                                                               \
    }
#define VALIDATOR_RANGE2(member, minval, maxval)                                                                                                                                   \
    if (!is_unused(member) && !(is_valid(member) && member * member >= minval * minval && member * member <= maxval * maxval)) {                                                   \
        res = false;                                                                                                                                                               \
        sprintf(errmsg, "\002%s: (%.50s @ %.1f,%.1f,%.1f) Validation Error: %s is invalid or out of range", strSource, pStreamer->GetForeignName(param0, param1, param2), pt.x,    \
                pt.y, pt.z, #member);                                                                                                                                              \
        VALIDATOR_LOG(pLog, errmsg);                                                                                                                                               \
...
#define ENTITY_VALIDATE(strSource, pStructure)                                                                                                                                     \
    if (!pStructure->validate(strSource, m_pWorld->m_pLog, m_pos, m_pWorld->m_pPhysicsStreamer, m_pForeignData, m_iForeignData, m_iForeignFlags)) {                                \
        if (m_pWorld->m_vars.bBreakOnValidation)                                                                                                                                   \
            DoBreak return 0;                                                                                                                                                      \
    }
#define ENTITY_VALIDATE_ERRCODE(strSource, pStructure, iErrCode)                                                                                                                   \
    if (!pStructure->validate(strSource, m_pWorld->m_pLog, m_pos, m_pWorld->m_pPhysicsStreamer, m_pForeignData, m_iForeignData, m_iForeignFlags)) {                                \
        if (m_pWorld->m_vars.bBreakOnValidation)                                                                                                                                   \
            DoBreak return iErrCode;                                                                                                                                               \
    }
#endif // LINUX
