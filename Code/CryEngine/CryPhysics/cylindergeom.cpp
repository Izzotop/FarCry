...
int CCylinderGeom::GetPrimitiveList(int iStart, int nPrims, int typeCollider, primitive* pCollider, int bColliderLocal, geometry_under_test* pGTest, geometry_under_test* pGTestOp,
...
int CCylinderGeom::PreparePolygon(coord_plane* psurface, int iPrim, int iFeature, geometry_under_test* pGTest, vector2df*& ptbuf, int*& pVtxIdBuf, int*& pEdgeIdBuf) {
    int i, icap;
    float ang, step;
...
int CCylinderGeom::PreparePolyline(coord_plane* psurface, int iPrim, int iFeature, geometry_under_test* pGTest, vector2df*& ptbuf, int*& pVtxIdBuf, int*& pEdgeIdBuf) {
...
int CCylinderGeom::FindClosestPoint(geom_world_data* pgwd, int& iPrim, int& iFeature, const vectorf& ptdst0, const vectorf& ptdst1, vectorf* ptres, int nMaxIters) {
...
void CCylinderGeom::CalcVolumetricPressure(geom_world_data* gwd, const vectorf& epicenter, float k, float rmin, const vectorf& centerOfMass, vectorf& P, vectorf& L) {
    vectorf c, dc;
    float blendf, area, r;
...
float CCylinderGeom::CalculateBuoyancy(const plane* pplane, const geom_world_data* pgwd, vectorf& massCenter) {
    ...
    float r = m_cyl.r * pgwd->scale, h = m_cyl.hh * pgwd->scale * 2, r2 = r * r, rinv = 1.0f / r, denom, k, a, b, z0, z1, ya, yb, anglea, angleb, V[4], Vaccum;
    ...
    if (b > a + r * 0.01f) {
        ...
        com[nPieces] = axes.z * (k * k * 0.5f *
                                     ((b * yb + r2 * angleb) * (b * b - a * a) + 0.5f * (b * cube(yb) - a * cube(ya)) -
                                      r2 * (0.75f * (b * yb - a * ya) - 0.25f * r2 * (angleb - anglea) + b * b * angleb - a * a * anglea)) +
                                 V[nPieces] * (z0 - a * k));
        com[nPieces++] += axes.x * ((2.0f / 3) * k * (0.25f * (b * cube(yb) - a * cube(ya)) + (3.0f / 8) * r2 * (b * yb - a * ya + r2 * (angleb - anglea)) - cube(yb) * (b - a)));
    }
...
int crop_polyarc2d_with_line(const vector2df* pt, const int* itype, int npt, float r, vector2df* ptout, int* itypeout, const vector2df& p0, const vector2df& l) {
    if (npt == 0)
        return 0;
    vector2df d0, c, ptt;
...
void CCylinderGeom::CalculateMediumResistance(const plane* pplane, const geom_world_data* pgwd, vectorf& dPres, vectorf& dLres) {
    ...
    for (i = 0; i < 12; i++) {
        ...
        CalcMediumResistance(ptside, 4, vectorf(x1, y1, 0).rotated(rotax, n.z, -sina), *pplane, pgwd->v, pgwd->w, pgwd->centerOfMass, dPres, dLres);
        dx = x1;
        x1 = (x1 * sqrt3 - y1) * 0.5f;
        y1 = (y1 * sqrt3 + dx) * 0.5f;
    }
    ...
    for (icap = -1; icap <= 1; icap += 2, n.flip(), center += n * (hh * 2), rotax.flip()) {
        ...
        if (dir.len2() > 0.001f)
            npt = crop_polyarc2d_with_line(pt, itype, npt, r, pt1, itype1, vector2df((org + dir * (((center - org) * n) / (dir * n)) - center).rotated(rotax, n.z, sina)),
        ...
        if (dir.len2() > w.len2() * 0.001f)
            npt = crop_polyarc2d_with_line(pt1, itype1, npt, r, pt, itype, vector2df((dir * ((pgwd->v * n - center * dir) / dir.len2())).rotated(rotax, n.z, sina)), vector2df(w));
        else {
            ptt = pt;
            pt = pt1;
            pt1 = ptt;
...
int CCylinderGeom::DrawToOcclusionCubemap(const geom_world_data* pgwd, int iStartPrim, int nPrims, int iPass, int* pGrid[6], int nRes, float rmin, float rmax, float zscale) {
    CBoxGeom geombox;
...
void CCylinderGeom::DrawWireframe(void (*DrawLineFunc)(float*, float*), geom_world_data* gwd, int iLevel) {
    if (iLevel == 0) {
        ...
        axes[0] = GetOrthogonal(axes[2]).normalized();
        axes[1] = axes[2] ^ axes[0];
        axes[0] *= m_cyl.r * gwd->scale;
        axes[1] *= m_cyl.r * gwd->scale;
