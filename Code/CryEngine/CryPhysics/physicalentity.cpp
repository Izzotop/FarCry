...
void CPhysicalEntity::ComputeBBox() {
    if (m_nParts == 0)
        ...
        box abox;
        vectorf sz, pos;
        m_BBox[0] = vectorf(MAX);
        m_BBox[1] = vectorf(MIN);
...
int CPhysicalEntity::GetParams(pe_params* _params) {
    ...
    if (_params->type == pe_params_part::type_id) {
        ...
        params->density = m_parts[i].pPhysGeomProxy->V > 0 ? m_parts[i].mass / (m_parts[i].pPhysGeomProxy->V * cube(m_parts[i].scale)) : 0;
...
int CPhysicalEntity::GetStatus(pe_status* _status) {
    ...
    if (_status->type == pe_status_id::type_id) {
        ...
        if ((unsigned int)status->iPrim >= (unsigned int)pgeom->pGeom->GetPrimitiveCount() || pgeom->pGeom->GetType() == GEOM_TRIMESH && status->iFeature > 2)
            return 0;
...
int CPhysicalEntity::IsPointInside(vectorf pt) {
    ...
    for (int i = 0; i < m_nParts; i++)
        if ((m_parts[i].flags & geom_collides) && m_parts[i].pPhysGeom->pGeom->PointInsideStatus((pt - m_parts[i].pos) * m_parts[i].q))
            return 1;
    return 0;
}
...
void CPhysicalEntity::AlertNeighbourhoodND() {
    ...
    if (m_nRefCount > 0 || m_flags & pef_always_notify_on_deletion) {
        ...
        for (i = m_pWorld->GetEntitiesAround(m_BBox[0] - inflator, m_BBox[1] + inflator, pentlist, ent_sleeping_rigid | ent_rigid | ent_living | ent_independent | ent_triggers) -
                 1;
             i >= 0; i--)
            pentlist[i]->Awake();
    }
