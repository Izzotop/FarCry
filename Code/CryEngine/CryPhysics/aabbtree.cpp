...
float CAABBTree::BuildNode(int iNode, int iTriStart, int nTris, vectorf center, vectorf size, int nDepth) {
    ...
    for (iAxis = 0; iAxis < 3; iAxis++) {
        ...
        for (i = iTriStart; i < iTriStart + nTris; i++) {
            c = m_pMesh->m_pVertices[m_pMesh->m_pIndices[i * 3]] + m_pMesh->m_pVertices[m_pMesh->m_pIndices[i * 3 + 1]] + m_pMesh->m_pVertices[m_pMesh->m_pIndices[i * 3 + 2]];
...
void CAABBTree::GetNodeChildrenBVs(const matrix3x3f& Rw, const vectorf& offsw, float scalew, const BV* pBV_parent, BV*& pBV_child1, BV*& pBV_child2) {
...
int CAABBTree::GetNodeContents(int iNode, BV* pBVCollider, int bColliderUsed, int bColliderLocal, geometry_under_test* pGTest, geometry_under_test* pGTestOp) {
