...
enum rentity_flags_int { ref_contact_overflow = 0x4000000 };
...
public:
    ...
    virtual void GetContactMatrix(const vectorf& pt, int ipart, matrix3x3f& K) {
        m_body.GetContactMatrix(pt - m_body.pos, K);
    }
    ...
    virtual int GetSnapshotVersion() {
    ...
    virtual float GetLastTimeStep(float time_interval) {
    ...
    int RegisterContactPoint(masktype& contact_mask, int idx, const vectorf& pt, const geom_contact* pcontacts, int iPrim0, int iFeature0, int iPrim1, int iFeature1,
                             int flags = contact_new, float penetration = 0);
    ...
    int CompactContactBlock(masktype& contact_mask, float maxPlaneDist, int nMaxContacts, int& nContacts, vectorf& n, float& maxDist, const vectorf& ptTest = vectorf(zero),
                            const vectorf& dirTest = vectorf(zero));
