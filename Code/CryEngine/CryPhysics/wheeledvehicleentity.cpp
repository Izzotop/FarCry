...
int CWheeledVehicleEntity::SetParams(pe_params* _params) {
    ...
    if (_params->type == pe_params_car::type_id) {
        ...
        if (!is_unused(params->engineMaxRPM))
            m_engineMaxw = params->engineMaxRPM * (2 * pi / 60.0);
        if (!is_unused(params->maxSteer))
            m_maxSteer = params->maxSteer;
        if (!is_unused(params->iIntegrationType))
            m_iIntegrationType = params->iIntegrationType;
        if (!is_unused(params->maxTimeStep))
        ...
        if (!is_unused(params->engineMinRPM))
            m_engineMinw = params->engineMinRPM * (2 * pi / 60.0);
        ...
        if (!is_unused(params->engineStartRPM))
            m_engineStartw = params->engineStartRPM * (2 * pi / 60.0);
        if (!is_unused(params->engineShiftUpRPM))
            m_engineShiftUpw = params->engineShiftUpRPM * (2 * pi / 60.0);
        if (!is_unused(params->engineShiftDownRPM))
            m_engineShiftDownw = params->engineShiftDownRPM * (2 * pi / 60.0);
        if (!is_unused(params->engineIdleRPM))
            m_engineIdlew = params->engineIdleRPM * (2 * pi / 60.0);
        ...
        if (!is_unused(params->gearDirSwitchRPM))
            m_gearDirSwitchw = params->gearDirSwitchRPM * (2 * pi / 60.0);
        ...
        if (!is_unused(action->steer) || !is_unused(action->dsteer)) {
            ...
            if (m_steer != 0) { // apply Ackerman steering to front wheels
                ...
                for (i = 0; i < m_nParts - m_nHullParts; i++)
                    if (m_susp[i].pt.y < pt[j = isneg(cm.x - m_susp[i].pt.x)].y)
                        pt[j] = m_susp[i].pt;
                rarax = (pt[1] - pt[0]).normalized(); // rear wheels axis (pt[0,1] - rear wheels positions
                for (i = 0; i < m_nParts - m_nHullParts; i++)
                    if (m_susp[i].pt.y > cm.y && (m_susp[i].pt.x - cm.x) * m_steer > 0) {
                        m_susp[i].steer = m_steer;
                        ha = (m_susp[i].pt - pt[0] ^ rarax).len();
                        la = (m_susp[i].pt - pt[0]) * rarax;
                        for (j = 0; j < m_nParts - m_nHullParts; j++)
                            if (fabsf(m_susp[j].pt.y - m_susp[i].pt.y) < ha * 0.05f && (m_susp[j].pt.x - cm.x) * m_steer < 0) {
...
int CWheeledVehicleEntity::GetStatus(pe_status* _status) {
    ...
    if (_status->type == pe_status_pos::type_id) {
        ...
        if ((unsigned int)iwheel < (unsigned int)m_nParts - m_nHullParts) {
            ...
            prevq = m_parts[i].q;
            m_parts[i].q = GetRotationAA(m_susp[iwheel].steer, vectorf(0, 0, -1)) * GetRotationAA(m_susp[iwheel].rot, vectorf(-1, 0, 0)) * m_susp[iwheel].q0;
...
void CWheeledVehicleEntity::RecalcSuspStiffness() {
    ...
    for (i = 0; i < m_nParts - m_nHullParts; i++)
        for (j = i + 1; j < m_nParts - m_nHullParts; j++)
...
int CWheeledVehicleEntity::AddGeometry(phys_geometry* pgeom, pe_geomparams* _params, int id) {
    ...
    if (!is_unused(params->bDriving)) {
        flags0 = params->flags;
        flagsCollider0 = params->flagsCollider;
...
void CWheeledVehicleEntity::RemoveGeometry(int id) {
    ...
    if (i < m_nParts) {
        if (i < m_nHullParts)
            ...
            memmove(m_susp + i - m_nHullParts, m_susp + i - m_nHullParts + 1, m_nParts - i - 1);
    }
...
void CWheeledVehicleEntity::CheckAdditionalGeometry(float time_interval, masktype& contact_mask) {
    ...
    for (i = j = 0; i < nents; i++)
        if (pentlist[i] != this &&
            (pentlist[i]->m_iSimClass < 2 || pentlist[i]->GetMassInv() < m_body.Minv * 10 || pentlist[i]->m_nParts == 1 && pentlist[i]->GetMassInv() < m_body.Minv * 100))
            ...
            for (j = 0; j < pentlist[ient]->m_nParts; j++)
                if (pentlist[ient]->m_parts[j].flags & m_susp[iwheel].flagsCollider0) {
                    ...
                    for (icont = 0; icont < ncontacts; icont++)
                        if (pcontacts[icont].n * gwd[0].v > 0.5f) { // || pcontacts[icont].n*gwd[0].v<-0.01f) {
                            ...
                            if ((ip.bSweepTest || pcontacts[icont].vel > 0) && tcur > tmax) {
                                ...
                                m_susp[iwheel].surface_idx[1] =
                                    pentlist[ient]->m_parts[j].surface_idx & pcontacts[icont].id[1] >> 31 | pcontacts[icont].id[1] & ~(pcontacts[icont].id[1] >> 31);
            ...
            for (j = 0; j < NMASKBITS && getmask(j) <= contact_mask; j++)
                if (contact_mask & getmask(j) && m_pContacts[j].ipart[0] == i && m_pContacts[j].pent[1] == m_susp[iwheel].pent) {
                    ...
                    for (icoll = 0; icoll < m_nColliders && m_pColliders[icoll] != m_susp[iwheel].pent; icoll++)
                        ;
                    if (icoll < m_nColliders && !((m_pColliderContacts[icoll] &= ~getmask(j)) | m_pColliderConstraints[icoll]) && tmax == 0 &&
                        !m_pColliders[icoll]->HasContactsWith(this)) {
    ...
    if (m_iCurGear != 1) {
        ...
        if (m_wengine > m_engineMinw) { // m_enginePedal!=0) {
            ...
            if (m_clutch > 1.0f) {
                ...
                */
                m_clutch = 1.0f;
                m_wengine = wwheel * m_gears[m_iCurGear];
            }
        }
    }
...
void CWheeledVehicleEntity::AddAdditionalImpulses(float time_interval) {
    ...
    axisz = R * vectorf(0, 0, 1);
    axisx = R * vectorf(1, 0, 0);
    ...
    if (m_iIntegrationType) { // solve for (delta)v = (I-AK)^-1*(A*(Pexplicit - ks*v*dt^2 + Pexternal))
        ...
        Pexp = Pexp * m_body.Minv + m_gravity * time_interval;
        Lexp = m_body.Iinv * Lexp; // Pexp = body delta v, Lexp = body delta w
        ...
        for (i = 0; i < nContacts; i++) {
            for (j = 0; j < nContacts; j++) { // build I - A*(-ks*dt^2-kd*dt) matrix
                ...
                if (m_susp[idx[j]].iBuddy >= 0 && m_kStabilizer > 0)
                    I_AK[i][j] -= (m_body.Minv - (m_susp[idx[i]].rworld ^ (m_body.Iinv * (m_susp[m_susp[idx[j]].iBuddy].rworld ^ axisz))) * axisz) * m_susp[idx[j]].kStiffness *
                                  m_kStabilizer * sqr(time_interval);
            }
        ...
        for (i = 0; i < nContacts; i++) {
            ...
            if (m_susp[i].iBuddy >= 0 && m_kStabilizer > 0)
                fN -= (m_susp[i].curlen - m_susp[m_susp[i].iBuddy].curlen + (m_susp[idx[i]].vworld - m_susp[m_susp[idx[i]].iBuddy].vworld) * time_interval) * m_susp[i].kStiffness *
                      m_kStabilizer;
    ...
    Pexp.zero();
    Lexp.zero();
    for (i = nfr = 0; i < m_nParts - m_nHullParts; i++)
        if (m_susp[i].bContact) {
            dP = axisz * m_susp[i].PN;
            Pexp += dP;
            Lexp += m_susp[i].rworld ^ dP;
            ...
            friction = min(m_susp[i].maxFriction, max(m_susp[i].minFriction, (m_pWorld->m_FrictionTable[m_susp[i].surface_idx[0] & NSURFACETYPES - 1] +
                                                                              m_pWorld->m_FrictionTable[m_susp[i].surface_idx[1] & NSURFACETYPES - 1]) *
                                                                                 0.5f));
            ...
            if (m_bHandBrake & m_susp[i].bCanBrake) {
                ...
                frdir[nfr] = axisz ^ axis;
                frdir[nfr] *= -sgnnz(frdir[nfr] * m_susp[i].ncontact);
                sina = pulldir * axisz;
    ...
    if (nfr > 0) {
        ...
        vectorn v(nfr, buf + nfr * nfr), p(nfr, buf + nfr * (nfr + 1)), v1(nfr, buf + nfr * (nfr + 2)), p1(nfr, buf + nfr * (nfr + 3)), dv(nfr, buf + nfr * (nfr + 5)),
            dp(nfr, buf + nfr * (nfr + 6));
        float sign;
        int d, prev_j, bSliding = 0;
        quotient s, s1, sd;
        real sval;
        ...
        for (i = 0; i < nfr; i++) {
            ...
            v[i] = (m_body.v + (m_body.w ^ r) - m_susp[idx[i]].pbody->v - (m_susp[idx[i]].pbody->w ^ m_susp[idx[i]].ptcontact - m_susp[idx[i]].pbody->pos)) * frdir[i];
        ...
        p1.zero();
        A.conjugate_gradient(p1, v1, 0, 0.0001);
        for (i = bSliding = 0; i < nfr; i++)
            bSliding += isneg(frmax[i] - fabs_tpl(p1[i]));
        if (!bSliding) {
            for (i = 0; i < nfr; i++)
                p[i] = p1[i];
            d = nfr;
        } else // .. invoke LCP solver otherwise
            for (d = iter = 0; d < nfr; d++)
                if (!slide[d]) {
                    prev_j = -1;
                    do {
                        for (i = nfr1 = 0; i < d; i++)
                        ...
                        if (v[d] * dv[d] < 0) {
                            j = d;
                            sd = s.set(-v[d], dv[d]).fixsign();
                        }
                        for (i = 0; i <= d; i++)
                            if (slide[i] == 0 && (s1.set(frmax[i] - p[i], dp[i]).fixsign() >= 0 && s1 < s || s1.set(frmin[i] - p[i], dp[i]).fixsign() >= 0 && s1 < s) ||
        ...
        for (i = 0; i < d; i++) {
            dP = frdir[i] * p[i];
            m_Ffriction += dP;
            m_Tfriction += m_susp[idx[i]].ptcontact - m_body.pos ^ dP;
        }
        ...
        Pbefore = m_body.P;
        Lbefore = m_body.L;
        (m_body.P += m_Ffriction) *= damping;
        (m_body.L += m_Tfriction) *= damping;
        m_Ffriction = m_body.P - Pbefore;
        m_Tfriction = m_body.L - Lbefore;
        ...
        m_Ffriction *= time_interval;
        m_Tfriction *= time_interval;
    }
...
int CWheeledVehicleEntity::SetStateFromSnapshot(CStream& stm, int flags) {
    ...
    if (res && res != 2) {
        ...
        if (!(flags & ssf_no_update)) {
            ...
            stm.Seek(stm.GetReadPos() + (m_pWorld->m_vars.bMultiplayer ? (m_nParts - m_nHullParts) * 14 + 12 * 3 + 14 + 1
                                                                       : (m_nParts - m_nHullParts) * sizeof(float) * 8 + 4 * sizeof(float) * 8 + 1));
            stm.Read(bnz);
            if (bnz)
