...
IPhysicalEntity* CPhysicalWorld::CreatePhysicalEntity(pe_type type, float lifeTime, pe_params* params, void* pForeignData, int iForeignData, int id,
    ...
    if (res) {
        ...
        if (m_pCurEntityHost && lifeTime > 0) {
            ...
            res->m_ig[0].x = m_pCurEntityHost->m_ig[0].x;
            res->m_ig[1].x = m_pCurEntityHost->m_ig[1].x;
            res->m_ig[0].y = m_pCurEntityHost->m_ig[0].y;
            res->m_ig[1].y = m_pCurEntityHost->m_ig[1].y;
        } else {
            res->m_bPermanent = 1;
...
int CPhysicalWorld::DestroyPhysicalEntity(IPhysicalEntity* _pent, int mode) {
    ...
    if (idx = IsPlaceholder(ppc)) {
        ...
        if (ppc->m_pGridThunks)
            delete[] ppc->m_pGridThunks;
        --idx;
    ...
    if (!pent->m_pEntBuddy) {
        ...
        if (pent->m_pGridThunks)
            delete[] pent->m_pGridThunks;
    }
...
int CPhysicalWorld::GetEntitiesAround(const vectorf& ptmin, const vectorf& ptmax, CPhysicalEntity**& pList, int objtypes, CPhysicalEntity* pPetitioner) {
    FUNCTION_PROFILER(GetISystem(), PROFILE_PHYSICS);
        ...
        for (iy = igy[0]; iy <= igy[1]; iy++) {
            for (thunk = m_pEntGrid[m_entgrid.getcell_safe(ix, iy)]; thunk; thunk = thunk_next, nGridEnts++) {
                ...
                if ((thunk->pent->m_bProcessed ^ 1) & (objtypes >> thunk->pent->m_iSimClass) & 1) {
                    ...
                    bbox1[0] = pGridEnt->m_BBox[0];
                    bbox1[1] = pGridEnt->m_BBox[1];
                    ...
                    if (bContact) {
                        if (pGridEnt->m_iSimClass != 6) {
                            ...
                            if (bContact)*/
                            m_pEventClient->OnBBoxOverlap(pGridEnt, pGridEnt->m_pForeignData, pGridEnt->m_iForeignData, pPetitioner, pPetitioner->m_pForeignData,
                                                          pPetitioner->m_iForeignData);
                        }
                    }
                    nEntsChecked++;
                }
    ...
    if (bSortRequired) {
        ...
        for (i = 0; i < nout; i++)
            m_pTmpEntList[i]->m_bProcessed_aux = 1;
        ...
        for (i = 0; i < nout; i++)
            m_pTmpEntList[i]->m_bProcessed_aux = 0;
    ...
    if (objtypes & ent_sort_by_mass) {
        for (i = 0; i < nout; i++)
            m_pMassList[i] = m_pTmpEntList[i]->GetMassInv();
                        ...
                        for (i = 0; i < nGroups; i++) {
                            ...
                            if (ipass == 0) {
                                ...
                                for (pent = m_pTmpEntList1[i], bStepValid = 1; pent; pent = pent->m_next_coll) {
                                    bStepValid &= pent->Step(max_time_step);
                                    pent->m_bMoved = 1;
                                }
                                if (!bStepValid) {
                                    for (pent = m_pTmpEntList1[i]; pent; pent = pent->m_next_coll)
...
void CPhysicalWorld::DetachEntityGridThunks(CPhysicalPlaceholder* pobj) {
    for (int i = 0; i < pobj->m_nGridThunks; i++) {
        if (pobj->m_pGridThunks[i].next)
            pobj->m_pGridThunks[i].next->prev = pobj->m_pGridThunks[i].prev;
        if (pobj->m_pGridThunks[i].prev)
            pobj->m_pGridThunks[i].prev->next = pobj->m_pGridThunks[i].next;
...
void CPhysicalWorld::RepositionEntity(CPhysicalPlaceholder* pobj, int flags) {
    ...
    if ((unsigned int)pobj->m_iSimClass >= 7u)
        return; // entity is frozen

    if (flags & 1 && m_pEntGrid) {
        for (i = 0; i < 2; i++) {
            igx[i] = max(-1, min(m_entgrid.size.x, float2int((pobj->m_BBox[i][inc_mod3[m_iEntAxisz]] - m_entgrid.origin[inc_mod3[m_iEntAxisz]]) * m_entgrid.stepr.x - 0.5f)));
            igy[i] = max(-1, min(m_entgrid.size.y, float2int((pobj->m_BBox[i][dec_mod3[m_iEntAxisz]] - m_entgrid.origin[dec_mod3[m_iEntAxisz]]) * m_entgrid.stepr.y - 0.5f)));
        }
            ...
            pobj->m_ig[0].x != -3) // if m_igx[0] is -3, the entity should not be registered in grid at all
        {
            ...
            if (n <= 0 || n > 1024) {
                ...
                char buf[256];
                sprintf(buf, "\002Error: %s @ %.1f,%.1f,%.1f is too large or invalid",
                ...
                if (m_vars.bBreakOnValidation)
                    DoBreak pobj->m_ig[0].x = pobj->m_ig[1].x = pobj->m_ig[0].y = pobj->m_ig[1].y = -2;
                goto skiprepos;
            }
            if (n > pcurobj->m_nGridThunksAlloc) {
                // check the heap for integrity
                // assert (IsHeapValid());
                if (pcurobj->m_pGridThunks)
                    delete[] pcurobj->m_pGridThunks;
                pcurobj->m_pGridThunks = new pe_gridthunk[pcurobj->m_nGridThunksAlloc = n];
                for (i = 0; i < n; i++)
                    pcurobj->m_pGridThunks[i].pent = pcurobj;
            }
            for (ix = igx[0], i = 0; ix <= igx[1]; ix++)
                for (iy = igy[0]; iy <= igy[1]; iy++, i++) {
                    j = m_entgrid.getcell_safe(ix, iy);
                    ...
                    if (m_pEntGrid[j])
                        m_pEntGrid[j]->prev = pcurobj->m_pGridThunks + i;
        ...
        if (pent->m_iPrevSimClass != pent->m_iSimClass) {
            ...
            if (!pent->m_bPermanent) {
                ...
                if (m_pTypedEntsPerm[pent->m_iSimClass]) {
                    ...
                    for (pent->m_prev = m_pTypedEnts[pent->m_iSimClass]; pent->m_prev && pent->m_prev->m_next; pent->m_prev = pent->m_prev->m_next)
                        ;
                    pent->m_prev->m_next = pent;
                } else
                    pent->m_prev = 0;
...
struct entity_grid_checker {
    ...
    int check_cell(const vector2di& icell, int& ilastcell) {
        ...
        for (thunk = pWorld->m_pEntGrid[pWorld->m_entgrid.getcell_safe(icell.x, icell.y)]; thunk; thunk = thunk_next) {
            thunk_next = thunk->next;
            if (!thunk->pent->m_bProcessed && objtypes & 1u << thunk->pent->m_iSimClass && (!pSkipForeignData || thunk->pent->GetForeignData() != pSkipForeignData)) {
                    ...
                    gotcontacts:
                        ...
                        for (j--; j >= 0; j--)
                            if (pcontacts[j].t < phits[0].dist && (!(flags & rwi_ignore_back_faces) || pcontacts[j].n * aray.m_dirn < 0)) {
                                ihit = 0;
                                if (pcontacts[j].id[0] < 0)
                                    pcontacts[j].id[0] = pent->m_parts[i].surface_idx;
                                if ((flags & rwi_pierceability_mask) < (pWorld->m_SurfaceFlagsTable[pcontacts[j].id[0] & NSURFACETYPES - 1] & sf_pierceable_mask)) {
                                    ...
                                    ilastcell = float2int((pcontacts[j].pt[inc_mod3[pWorld->m_iEntAxisz]] - pWorld->m_entgrid.origin[inc_mod3[pWorld->m_iEntAxisz]]) *
...
int CPhysicalWorld::RayWorldIntersection(vectorf org, vectorf dir, int objtypes, unsigned int flags, ray_hit* hits, int nMaxHits, IPhysicalEntity* pSkipEnt,
                                         IPhysicalEntity* pSkipEntAux) {
    FUNCTION_PROFILER(GetISystem(), PROFILE_PHYSICS);
    ...
    if ((objtypes & ent_terrain) && m_pHeightfield) {
        ...
        if (m_pHeightfield->m_parts[0].pPhysGeom->pGeom->Intersect(&aray, &gwd, 0, 0, pcontacts) && (pcontacts->id[0] >= 0 || flags & rwi_ignore_terrain_holes)) {
    ...
    if (fabsf(origin_grid.x * m_entgrid.stepr.x * 2 - m_entgrid.size.x) > m_entgrid.size.x || fabsf(origin_grid.y * m_entgrid.stepr.y * 2 - m_entgrid.size.y) > m_entgrid.size.y ||
...
void CPhysicalWorld::SimulateExplosion(vectorf epicenter, vectorf epicenterImp, float rmin, float rmax, float r, float impulsive_pressure_at_r, int nOccRes, int nGrow,
                                       float rmin_occ, IPhysicalEntity** pSkipEnts, int nSkipEnts, int iTypes) {
    FUNCTION_PROFILER(GetISystem(), PROFILE_PHYSICS);
    ...
    if (nOccRes > 0) {
        ...
        for (nents = GetEntitiesAround(epicenter - vectorf(rmax, rmax, rmax), epicenter + vectorf(rmax, rmax, rmax), pents, ent_terrain | ent_static) - 1; nents >= 0; nents--)
            for (i = 0; i < pents[nents]->m_nParts; i++)
            ...
            if (pents[nents]->m_parts[i].flags & geom_colltype_explosion || pents[nents]->m_flags & pef_use_geom_callbacks) {
                ...
                if (kr > 0) {
                    if (!(pents[nents]->m_flags & pef_use_geom_callbacks)) {
                        ...
                        shockwave.momentum.zero();
                        pbody = pents[nents]->GetRigidBody(i);
                        pents[nents]->m_parts[i].pPhysGeomProxy->pGeom->CalcVolumetricPressure(&gwd, epicenterImp, kr, rmin, pbody->pos, shockwave.impulse, shockwave.momentum);
                        shockwave.impulse *= frac;
                        shockwave.momentum *= frac;
                        shockwave.ipart = i;
    ...
    if (m_vars.bLogActiveObjects) {
        ...
        for (pent = m_pTypedEnts[2]; pent; pent = pent->m_next, nCount++) {
            ...
            m_pLog->Log("\001%s @ %7.2f,%7.2f,%7.2f, mass %.2f, v %.1f, w %.1f, #polies %d, id %d",
                        m_pPhysicsStreamer->GetForeignName(pent->m_pForeignData, pent->m_iForeignData, pent->m_iForeignFlags), pent->m_pos.x, pent->m_pos.y, pent->m_pos.z,
                        pbody->M, pbody->v.len(), pbody->w.len(), nPrims, pent->m_id);
        }
...
void CPhysicalWorld::GetMemoryStatistics(ICrySizer* pSizer) {
    static char* entnames[] = {"static entities", "physical entities", "physical entities", "living entities", "detached entities", "", "", "deleted entities"};
    int i, j, n;
    CPhysicalEntity* pent;
...

    {
        ...
        for (i = n = 0; i < m_nPlaceholderChunks; i++)
            for (j = 0; j < PLACEHOLDER_CHUNK_SZ; j++)
        ...
        pSizer->AddObject(m_pPlaceholders, m_nPlaceholderChunks * (sizeof(CPhysicalPlaceholder) * PLACEHOLDER_CHUNK_SZ + sizeof(CPhysicalPlaceholder*)) + n * sizeof(pe_gridthunk));
...
void CPhysicalWorld::AddEntityProfileInfo(CPhysicalEntity* pent, int nTicks) {
    if (m_nProfiledEnts == sizeof(m_pEntProfileData) / sizeof(m_pEntProfileData[0]) && nTicks <= m_pEntProfileData[m_nProfiledEnts - 1].nTicks)
        return;

    int i, nCalls = 1, iBound[2] = {0, m_nProfiledEnts};
    for (i = 0; i < m_nProfiledEnts; i++)
        if (m_pEntProfileData[i].pEntity == pent) {
            nTicks += m_pEntProfileData[i].nTicks;
            nCalls += m_pEntProfileData[i].nCalls;
    ...
    m_pEntProfileData[i].pName = m_pPhysicsStreamer ? m_pPhysicsStreamer->GetForeignName(pent->m_pForeignData, pent->m_iForeignData, pent->m_iForeignFlags) : "noname";
