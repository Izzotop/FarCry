...
void RigidBody::Create(const vectorf& center, const vectorf& Ibody0, const quaternionf& q0, float volume, float mass, const quaternionf& qframe, const vectorf& posframe) {
...
void InvokeContactSolver(float time_interval, SolverSettings* pss) {
    ...
    for (i = nBodies = nConstraints = 0; i < g_nContacts; i++) {
        ...
        if (g_pContacts[i]->flags & contact_constraint_3dof)
            ...
            for (j = 0; j < 2; j++)
                for (k = 0; k < 2; k++)
                    mtx[j][k] = axes[j] * g_pContacts[i]->K * axes[k];
            ...
            g_pContacts[i]->C(0, 0) += 1.0f;
            g_pContacts[i]->C(1, 1) += 1.0f;
            g_pContacts[i]->C(2, 2) += 1.0f;
    ...
    if (g_bUsePreCG && g_nContacts < 20) {
        ...
        do {
            ...
            for (i = 0; i < g_nContacts; i++) {
                ...
                if (!(g_pContacts[i]->flags & contact_angular)) {
                    r0 = g_pContacts[i]->pt[0] - body0->pos;
                    r1 = g_pContacts[i]->pt[1] - body1->pos;
                    body0->Fcollision += g_pContacts[i]->dP;
                    body0->Tcollision += r0 ^ g_pContacts[i]->dP;
                    body1->Fcollision -= g_pContacts[i]->dP;
                    body1->Tcollision -= r1 ^ g_pContacts[i]->dP;
                } else {
                    body0->Tcollision += g_pContacts[i]->dP;
                    body1->Tcollision -= g_pContacts[i]->dP;
                }
            }
            for (i = 0; i < g_nContacts; i++) {
                body0 = g_pContacts[i]->pbody[0];
        ...
        for (i = 0; i < g_nContacts; i++) {
            ...
            if (!(g_pContacts[i]->flags & contact_angular)) {
                ...
                body0->Tcollision += g_pContacts[i]->pt[0] - body0->pos ^ g_pContacts[i]->P;
                body1->Fcollision -= g_pContacts[i]->P;
                body1->Tcollision -= g_pContacts[i]->pt[1] - body1->pos ^ g_pContacts[i]->P;
            } else {
                body0->Tcollision += g_pContacts[i]->P;
                body1->Tcollision -= g_pContacts[i]->P;
            }
        }
        ...
        for (i = 0; i < g_nContacts; i++) {
            n = g_pContacts[i]->n;
            dPn = g_pContacts[i]->P * n;
            dPtang = (g_pContacts[i]->P - n * dPn).len2();
            if (!(g_pContacts[i]->flags & contact_angular)) {
            ...
            if (!(g_pContacts[i]->flags & contact_constraint) && (dp * n < -0.05f || // allow to pull bodies at contacts slightly
                                                                  dPtang > sqr(dPn * g_pContacts[i]->friction) + 0.001f || (g_pContacts[i]->r + g_pContacts[i]->vreq) * n < vdiff))
                break;
        }
    ...
    for (i = 0; i < g_nContacts; i++) {
        ...
        g_Contacts[i].iCountDst = ((entity_contact*)((char*)g_pContacts[i]->pBounceCount - ((char*)&g_pContacts[i]->iCount - (char*)g_pContacts[i])))->bProcessed;
    }
    for (i = 0; i < nBodies; i++) {
        ...
        do {
            ...
            for (i = istart; i != iend; i += istep) {
                if (g_Contacts[i].iCount >= (g_Contacts[i].flags & contact_count_mask)) {
                    ...
                    hbody0 = g_Bodies + g_Contacts[i].iBody[0];
                    hbody1 = g_Bodies + g_Contacts[i].iBody[1];
                    n = g_Contacts[i].n;
    ...
    if (bBounced && pss->nMaxLCPCGiters > 0) { // use several iterations of CG solver, solving only for non-separating contacts
        unsigned int iClass;
        int cgiter, bStateChanged, n1dofContacts, n2dofContacts, nAngContacts, nFric0Contacts, nFricInfContacts, nContacts, iSortedContacts[6], flags, bNoImprovement;
        ...
        for (i = bBounced = 0; i < g_nContacts; i++) {
            ...
            if (g_pContacts[i]->flags & contact_constraint_1dof)
                ...
                g_pContacts[i]->r0(g_pContacts[i]->n * g_pContacts[i]->vrel, 0, 0);
        ...
        if (bBounced) {
            {
                ...
                do {
                    ...
                    nFric0Contacts = iSortedContacts[3];
                    nFricInfContacts = iSortedContacts[4];
                    nContacts = iSortedContacts[5];
                    ...
                    do {
                        ...
                        for (; i < nContacts; i++) { // positional contacts
                            body0 = pContacts[i]->pbody[0];
                            body1 = pContacts[i]->pbody[1];
                            r0 = pContacts[i]->pt[0] - body0->pos;
                            r1 = pContacts[i]->pt[1] - body1->pos;
                            body0->Fcollision += pContacts[i]->dP;
                            body0->Tcollision += r0 ^ pContacts[i]->dP;
                            body1->Fcollision -= pContacts[i]->dP;
                            body1->Tcollision -= r1 ^ pContacts[i]->dP;
                        }
                    ...
                    for (i = n1dofContacts; i < nFric0Contacts; i++)
                        pContacts[i]->P = pContacts[i]->n * pContacts[i]->P.x;
                    if (!bStateChanged)
                        break;
                    ...
                    for (; i < nContacts; i++) { // positional contacts
                        ...
                        r0 = pContacts[i]->pt[0] - body0->pos;
                        r1 = pContacts[i]->pt[1] - body1->pos;
                        body0->Fcollision += pContacts[i]->P;
                        body0->Tcollision += r0 ^ pContacts[i]->P;
                        body1->Fcollision -= pContacts[i]->P;
                        body1->Tcollision -= r1 ^ pContacts[i]->P;
                    }
                    for (i = 0; i < g_nContacts; i++)
                ...
                if (cgiter < pss->nMaxLCPCGiters) {
                    ...
                    if (bBounced) {
                        for (; i < nContacts; i++) { // positional contacts
                            body0 = pContacts[i]->pbody[0];
                            body1 = pContacts[i]->pbody[1];
                            body0->P += pContacts[i]->P;
                            body0->L += r0 ^ pContacts[i]->P;
            ...
            if (bBounced) {
                ...
                do {
                    ...
                    do {
                        ...
                        i = (g_infos[g_sandwichbuf[j].iBread[0]].iLevel >> 31 & 1 | g_infos[g_sandwichbuf[j].iBread[1]].iLevel >> 31 & 2 |
                             g_infos[g_sandwichbuf[j].iMiddle].iLevel >> 31 & 4) ^
                            7;
                        if (i == 7) {                                                                                             // all participating bodies already have levels
                            ...
                            g_infos[g_sandwichbuf[j].iBread[0]].iLevel = g_infos[g_sandwichbuf[j].iBread[1]].iLevel = g_infos[g_sandwichbuf[j].iMiddle].iLevel + 1;
                            add_route_follower(g_sandwichbuf[j].iMiddle, g_sandwichbuf[j].iBread[0]);
                            add_route_follower(g_sandwichbuf[j].iMiddle, g_sandwichbuf[j].iBread[1]);
                ...
                for (i = 0; i < nBodies; i++) {
                    ...
                    g_infos[g_pBodies[i]->bProcessed].v_unproj.zero();
                    g_infos[g_pBodies[i]->bProcessed].w_unproj.zero();
                }
                    ...
                    if (iter = nContacts) {
                        do {
                            ...
                            for (; i < nContacts; i++) { // positional contacts
                                ...
                                body0->Tcollision += r0 ^ pContacts[i]->dP * pContacts[i]->dPn;
                            }
                            for (i = 0; i < nAngContacts; i++) // angular contacts
                                pContacts[i]->vrel = body0->Iinv * body0->Tcollision;
                            for (; i < nContacts; i++) { // positional contacts
                        ...
                        for (; i < nContacts; i++) { // positional contacts
                            ...
                            body0->Tcollision += r0 ^ pContacts[i]->dP * pContacts[i]->P.x;
                        }
                        ...
                        for (; i < nContacts; i++) {
                            ...
                            r2 += sqr(pContacts[i]->r0.x);
                            r2new += sqr((g_infos[body0->bProcessed].v_unproj + (g_infos[body0->bProcessed].w_unproj ^ pContacts[i]->pt[iop] - body0->pos)) * pContacts[i]->dP -
                                         pContacts[i]->r0.x);
                        }
                        iop = 0;
                        iop = 1;
