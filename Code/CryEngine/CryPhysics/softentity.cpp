...
int CSoftEntity::AddGeometry(phys_geometry* pgeom, pe_geomparams* params, int id) {
    ...
    for (i = 0; i < m_nVtx; i++) {
        ...
        m_vtx[i].volume = vtxvol;
        m_vtx[i].vel.zero();
        m_vtx[i].bSeparating = m_vtx[i].iSorted = m_vtx[i].bAttached = 0;
    ...
    for (i = 0; i < pMesh->m_nTris; i++)
        for (j = 0; j < 3; j++)
            if ((iedge = pInfo[i][j]) >= 0) {
                ...
                m_edges[pInfo[i][j]].kd = m_vtx[i0].massinv + m_vtx[i1].massinv > 0 ? sqrt_tpl(m_ks / (m_vtx[i0].massinv + m_vtx[i1].massinv)) * 2.0f * m_kdRatio : 0;
            }
    ...
    for (i = nVtxEdges = 0; i < pMesh->m_nTris; i++)
        for (j = 0; j < 3; j++)
            if (!m_vtx[ivtx = pMesh->m_pIndices[i * 3 + j]].iSorted) {
                ...
                m_BBox[0].x = min(m_BBox[0].x, m_vtx[ivtx].pos.x);
                m_BBox[1].x = max(m_BBox[1].x, m_vtx[ivtx].pos.x);
                m_BBox[0].y = min(m_BBox[0].y, m_vtx[ivtx].pos.y);
                m_BBox[1].y = max(m_BBox[1].y, m_vtx[ivtx].pos.y);
                m_BBox[0].z = min(m_BBox[0].z, m_vtx[ivtx].pos.z);
                m_BBox[1].z = max(m_BBox[1].z, m_vtx[ivtx].pos.z);
            }
    delete[] pInfo;
...
int CSoftEntity::SetParams(pe_params* _params) {
    ...
    if (res) {
        if (_params->type == pe_params_pos::type_id && !is_unused(((pe_params_pos*)_params)->q)) {
            if (m_nVtx > 0 && (m_qrot0 | m_qrot) < 0.998f) {
                ...
                int i;
                quaternionf qrot = m_bMeshUpdated ? m_qrot * !m_qrot0 : m_qrot;
                for (i = 0; i < m_nVtx; i++) {
    ...
    if (_params->type == pe_params_softbody::type_id) {
        ...
        if (!is_unused(params->ks))
            m_ks = params->ks;
        if (!is_unused(params->kdRatio))
            m_kdRatio = params->kdRatio;
        if (!is_unused(params->ks) || !is_unused(params->kdRatio)) {
            for (int i = 0; i < m_nEdges; i++)
                if (m_vtx[m_edges[i].ivtx[0]].massinv + m_vtx[m_edges[i].ivtx[1]].massinv > 0)
...
int CSoftEntity::Action(pe_action* _action) {
    ...
    if (_action->type == pe_action_attach_points::type_id) {
        ...
        CPhysicalEntity* pent = action->pEntity == WORLD_ENTITY ? &g_StaticPhysicalEntity : action->pEntity ? ((CPhysicalPlaceholder*)action->pEntity)->GetEntity() : 0;
        ...
        if (m_nVtx > 0) {
            ...
            for (i = 0; i < pMesh->m_nTris; i++)
                pMesh->m_pNormals[i] = (pMesh->m_pVertices[pMesh->m_pIndices[i * 3 + 1]] - pMesh->m_pVertices[pMesh->m_pIndices[i * 3]] ^
                                        pMesh->m_pVertices[pMesh->m_pIndices[i * 3 + 2]] - pMesh->m_pVertices[pMesh->m_pIndices[i * 3]])
                                           .normalized();
        }
        return 1;
    }
...
int CSoftEntity::Step(float time_interval) {
    ...
    nEnts = m_pWorld->GetEntitiesAround(m_BBox[0] - vectorf(m_thickness, m_thickness, m_thickness) * 2, m_BBox[1] + vectorf(m_thickness, m_thickness, m_thickness) * 2, pentlist,
        ...
        if (pentlist[i] != this)
            for (j = 0, pentlist[i]->m_bProcessed_aux = nCheckParts << 24; j < pentlist[i]->m_nParts; j++) {
                ...
                if (box_box_overlap_check(&boxent, &checkParts[nCheckParts].bbox)) {
                    ...
                    pentlist[i]->m_parts[j].pPhysGeomProxy->pGeom->PrepareForRayTest(pentlist[i]->m_parts[j].scale == 1.0f ? m_thickness * 2
                                                                                                                           : m_thickness * 2 / pentlist[i]->m_parts[j].scale);
...
enoughgeoms:
    ...
    for (i = nContactVtx = 0; i < m_nVtx; i++)
        if (!m_vtx[i].bAttached) { // detect collisions for free vertices
            // calculate normal
            for (j = m_vtx[i].iStartEdge, m_vtx[i].n.zero(); j < m_vtx[i].iEndEdge + m_vtx[i].bFullFan; j++) {
                ...
                m_vtx[i].n += (m_vtx[m_edges[m_pVtxEdges[j]].ivtx[1]].pos - m_vtx[m_edges[m_pVtxEdges[j]].ivtx[0]].pos) * (iszero(i ^ m_edges[m_pVtxEdges[j]].ivtx[0]) * 2 - 1) ^
            ...
            for (j = 0; j < nCheckParts; j++) {
                ...
                if (checkParts[j].bPrimitive) {
                    ...
                    if (box_ray_overlap_check(&checkParts[j].bbox, &aray.m_ray)) {
                        ...
                        gwd.offset.zero();
                        gwd.R.SetIdentity();
                        if (checkParts[j].pent->m_parts[checkParts[j].ipart].pPhysGeomProxy->pGeom->Intersect(&aray, &gwd, 0, &ip, pcontacts) && pcontacts->n * aray.m_dirn > 0) {
    ...
    for (i = 0; i < m_nVtx; i++) { // apply gravity, buoyancy, wind (or water resistance)
        ...
        m_vtx[i].r.zero();
        m_vtx[i].bSeparating = 0;
        m_vtx[i].pos0 = m_vtx[i].pos;
        m_vtx[i].vel0 = m_vtx[i].vel;
        m_vtx[i].iSorted0 = m_vtx[i].iSorted;
    }
...
reiterate:
    for (i = 0; i < nCheckParts; i++)
        checkParts[i].P = checkParts[i].L.zero();
    ...
    for (i = 1; i < m_nVtx; i++) {
        ...
        m_vtx[i].vel *= kr;
        m_BBox[0].x = min(m_BBox[0].x, m_vtx[i].pos.x);
        m_BBox[1].x = max(m_BBox[1].x, m_vtx[i].pos.x);
        m_BBox[0].y = min(m_BBox[0].y, m_vtx[i].pos.y);
        m_BBox[1].y = max(m_BBox[1].y, m_vtx[i].pos.y);
        m_BBox[0].z = min(m_BBox[0].z, m_vtx[i].pos.z);
        m_BBox[1].z = max(m_BBox[1].z, m_vtx[i].pos.z);
    }

    if (bUnstable) {
        for (i = 0; i < m_nVtx; i++) {
            m_vtx[i].pos = m_vtx[i].pos0;
            m_vtx[i].vel = m_vtx[i].vel0;
            m_vtx[i].iSorted = m_vtx[i].iSorted0;
        }
    ...
    if (m_collImpulseScale > 0)
        for (i = 0; i < nCheckParts; i++)
            if (checkParts[i].P.len2() + checkParts[i].L.len2() > 0 && checkParts[i].pent->GetType() == PE_LIVING) {
    ...
    if (m_parts[0].scale == 1.0f && m_parts[0].q.w == 1.0f)
        for (i = 0; i < m_nVtx; i++)
            pMesh->m_pVertices[i] = m_vtx[i].pos + d;
    else
        for (i = 0; i < m_nVtx; i++)
            pMesh->m_pVertices[i] = ((m_vtx[i].pos + d) * rscale) * m_parts[0].q;
    m_bMeshUpdated = 1;
    // for(i=0;i<pMesh->m_nTris;i++)
...
void CSoftEntity::DrawHelperInformation(void (*DrawLineFunc)(float*, float*), int flags) {
    ...
    if (flags & pe_helper_collisions)
        for (i = 0; i < m_nVtx; i++)
            if (m_vtx[i].pContactEnt && !m_vtx[i].bSeparating)
