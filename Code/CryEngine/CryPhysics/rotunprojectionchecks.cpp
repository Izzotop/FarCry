...
int tri_tri_rot_unprojection(unprojection_mode* pmode, const triangle* ptri1, int iFeature1, const triangle* ptri2, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    ...
    vectorr pt, ptz[2][3], ptx[2][3], pty[2][3], rotax, pt0, pt1, edge0, edge1, n0z, n0x, n0y, ptz0, ptx0, pty0, n, pt0_rot, pt1_rot, edge0_rot, ptz1, ptx1, pty1, dir0, dir1,
        norm[2];
                ...
                if (d >= 0) {
                    ...
                    for (j = 0; j < 2; j++, tsin.x += d * 2)
                            ...
                            isneg((ksin * tsin.x + k * tsin.y) * kcos))     // tcos in (0..1) (=eliminate foreign root)
                        {
                            ...
                            pt = ptx[itri ^ 1][inc_mod3[i]] * tcos.x + pty[itri ^ 1][inc_mod3[i]] * tsin.x +
                                 (ptz[itri ^ 1][inc_mod3[i]] + pmode->center - ptri[itri]->pt[0]) * tsin.y;
                            sg0 = pt * ptri[itri]->n + pmode->minPtDist * tsin.y;
                            pt = ptx[itri ^ 1][dec_mod3[i]] * tcos.x + pty[itri ^ 1][dec_mod3[i]] * tsin.x +
                                 (ptz[itri ^ 1][dec_mod3[i]] + pmode->center - ptri[itri]->pt[0]) * tsin.y;
        ...
        if (d >= 0) {
            ...
            for (j = 0; j < 2; j++, tsin.x += d * 2)
                    ...
                    isneg((ksin * tsin.x + k * tsin.y) * kcos))     // tcos in (0..1)
                {
                    ...
                    bContact = // point is inside itri
                        isneg((pt - ptri->pt[0] * tsin.y ^ ptri->pt[1] - ptri->pt[0]) * ptri->n) & isneg((pt - ptri->pt[1] * tsin.y ^ ptri->pt[2] - ptri->pt[1]) * ptri->n) &
