...
int qhull(strided_pointer<vectorf> pts, int npts, index_t*& pTris) {
    ...
    if (npts <= 4) {
        pTris = new index_t[6];
        pTris[0] = 0;
        pTris[1] = min(1, npts - 1);
        pTris[2] = min(2, npts - 1);
        pTris[3] = 0;
        pTris[4] = min(1, npts - 1);
        pTris[5] = min(4, npts - 1);
        return 2;
    }
...
int qhull2d(ptitem2d* pts, int nVtx, edgeitem* edges) {
    ...
    do {
        ...
        for (pstart = edges + i; pstart->prev != edges + i && (ppt_best->pt - pstart->prev->pvtx->pt ^ pstart->pvtx->pt - pstart->prev->pvtx->pt) > 0; pstart = pstart->prev)
            ;
        // trace contour from i ccw while edges are facing the point ppt_best (pend - edge after the last edge to be deleted)
        for (pend = edges[i].next; pend != edges + i && (ppt_best->pt - pend->pvtx->pt ^ pend->next->pvtx->pt - pend->pvtx->pt) > 0; pend = pend->next)
            ;
