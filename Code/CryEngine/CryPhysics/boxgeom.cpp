...
int CBoxGeom::CalcPhysicalProperties(phys_geometry* pgeom) {
    ...
    pgeom->Ibody.Set(sqr(m_box.size.y) + sqr(m_box.size.z), sqr(m_box.size.x) + sqr(m_box.size.z), sqr(m_box.size.x) + sqr(m_box.size.y)) *= pgeom->V * (1.0 / 3);
    return 1;
}
...
void CBoxGeom::PrepareBox(box* pbox, geometry_under_test* pGTest) {
    ...
    if (!m_box.bOriented)
        ...
        pbox->Basis = m_box.Basis * pGTest->R.T();
    pbox->bOriented = 1;
...
int CBoxGeom::PreparePrimitive(geom_world_data* pgwd, primitive*& pprim) {
    ...
    if (!m_box.bOriented)
        ...
        pbox->Basis = m_box.Basis * pgwd->R.T();
    pbox->bOriented = 1;
...
int CBoxGeom::GetFeature(int iPrim, int iFeature, vectorf* pt) {
    ...
    case 0: // vertex
        pt->x = m_box.size.x * ((iFeature << 1 & 2) - 1);
        pt->y = m_box.size.y * ((iFeature & 2) - 1);
        pt->z = m_box.size.z * ((iFeature >> 1 & 2) - 1);
        pt[0] = pt[0] * m_box.Basis + m_box.center;
        return 1;
    case 0x20: // edge
        iz = iFeature >> 2 & 3;
        ix = inc_mod3[iz];
        iy = dec_mod3[iz];
        pt[0][ix] = pt[1][ix] = m_box.size[ix] * ((iFeature << 1 & 2) - 1);
        pt[0][iy] = pt[1][iy] = m_box.size[iy] * ((iFeature & 2) - 1);
        pt[0][iz] = -m_box.size[iz];
        pt[1][iz] = m_box.size[iz];
        pt[0] = pt[0] * m_box.Basis + m_box.center;
        pt[1] = pt[1] * m_box.Basis + m_box.center;
        return 2;
    case 0x40: // face
        iz = iFeature >> 1 & 3;
        ix = inc_mod3[iz];
...
int CBoxGeom::GetPrimitiveList(int iStart, int nPrims, int typeCollider, primitive* pCollider, int bColliderLocal, geometry_under_test* pGTest, geometry_under_test* pGTestOp,
...
int CBoxGeom::PreparePolygon(coord_plane* psurface, int iPrim, int iFeature, geometry_under_test* pGTest, vector2df*& ptbuf, int*& pVtxIdBuf, int*& pEdgeIdBuf) {
    int i, sg;
...
int CBoxGeom::PreparePolyline(coord_plane* psurface, int iPrim, int iFeature, geometry_under_test* pGTest, vector2df*& ptbuf, int*& pVtxIdBuf, int*& pEdgeIdBuf) {
    int ix, iy, iz;
    vectorf pt, offs;
    matrix3x3f R;
    if (!m_box.bOriented)
...
int CBoxGeom::FindClosestPoint(geom_world_data* pgwd, int& iPrim, int& iFeature, const vectorf& ptdst0, const vectorf& ptdst1, vectorf* ptres, int nMaxIters) {
...
void CBoxGeom::BuildTriMesh(CTriMesh& mesh) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        if (m_box.bOriented)
            vtx[i] = vtx[i] * m_box.Basis;
        vtx[i] += m_box.center;
    }
    for (i = 0; i < 6; i++) {
        normals[i * 2].Set(0, 0, 0);
        normals[i * 2][i >> 1] = ((i & 1) << 1) - 1;
        if (m_box.bOriented)
            normals[i * 2] = normals[i * 2] * m_box.Basis;
...
void CBoxGeom::CalcVolumetricPressure(geom_world_data* gwd, const vectorf& epicenter, float k, float rmin, const vectorf& centerOfMass, vectorf& P, vectorf& L) {
    CTriMesh mesh;
    BuildTriMesh(mesh);
...
int CBoxGeom::DrawToOcclusionCubemap(const geom_world_data* pgwd, int iStartPrim, int nPrims, int iPass, int* pGrid[6], int nRes, float rmin, float rmax, float zscale) {
    int i, j, ix, iy, iz;
    vectorf pt[4], n, size = m_box.size * pgwd->scale, offset = pgwd->R * m_box.center * pgwd->scale + pgwd->offset;
    ;
    matrix3x3f R;
