...
int IntersectBVs(geometry_under_test* pGTest, BV* pBV1, BV* pBV2) {
    intptr_t mask;
    int iNode[2], *pUsedNodesMap[2], res = 0, nullptrmap = 0, bNodeUsed[2];
    mask = iszero_mask(pGTest[0].pUsedNodesMap); // mask = -(pUsedNodeMap==0)
    pUsedNodesMap[0] = (int*)((intptr_t)pGTest[0].pUsedNodesMap & ~mask | (intptr_t)&nullptrmap & mask);
    iNode[0] = pBV1->iNode & ~mask;
    mask = iszero_mask(pGTest[1].pUsedNodesMap); // mask = -(pUsedNodeMap==0)
    pUsedNodesMap[1] = (int*)((intptr_t)pGTest[1].pUsedNodesMap & ~mask | (intptr_t)&nullptrmap & mask);
    ...
    } else if (split2 > 0) { // split the second BV
...
int SweepTestBVs(geometry_under_test* pGTest, BV* pBV1, BV* pBV2) {
    ...
    if (split1 > split2 && split1 > 0) { // split the first BV
        ...
        for (i = 0, ptr[0] = pGTest[0].primbuf; i < nPrims1; i++, ptr[0] = (primitive*)((char*)ptr[0] + pGTest[0].szprim))
            for (j = 0, ptr[1] = pGTest[1].primbuf; j < nPrims2; j++, ptr[1] = (primitive*)((char*)ptr[1] + pGTest[1].szprim))
                if (g_Unprojector.Check(&unproj, pGTest[0].typeprim, pGTest[1].typeprim, ptr[0], -1, ptr[1], -1, &contact_cur) && contact_cur.n * pGTest[0].sweepdir > 0 &&
                    contact_cur.t <= pGTest[0].sweepstep && pGTest[0].sweepstep - contact_cur.t < pcontact->t) {
...
int CGeometry::RegisterIntersection(primitive* pprim1, primitive* pprim2, geometry_under_test* pGTest1, geometry_under_test* pGTest2, prim_inters* pinters) {
    unprojection_mode unproj;
    contact contact_cur;
    ...
    if (pGTest1->pParams->iUnprojectionMode == 0) {
        ...
        unproj.imode = 0;
        unproj.dir = pGTest2->v + (pGTest2->w ^ pinters->pt[1] - pGTest2->centerOfMass) - pGTest1->v - (pGTest1->w ^ pinters->pt[1] - pGTest1->centerOfMass);
...
int RadiusCheckBVs(radius_check_data* prcd, BV* pBV) {
    ...
    if (prcd->pBVtree->SplitPriority(pBV) > 0) {
        ...
        return prcd->pGeom->DrawToOcclusionCubemap(prcd->pgwd, iStartPrim, nPrims, prcd->iPass, prcd->pGrid, prcd->nRes, prcd->rmin, prcd->rmax, prcd->zscale);
    }
}
...
float CGeometry::BuildOcclusionCubemap(geom_world_data* pgwd, int iMode, int* pGrid0[6], int* pGrid1[6], int nRes, float rmin, float rmax, int nGrow) {
    ...
    for (i = 0; i < 6; i++)
        for (j = nRes * nRes - 1; j >= 0; j--) // if the highest bit is set, change cell z to irmin
...
int CPrimitive::Intersect(IGeometry* _pCollider, geom_world_data* pdata1, geom_world_data* pdata2, intersection_params* pparams, geom_contact*& pcontacts) {
    ...
    if (!pparams->bSweepTest) {
        ...
        if (!pparams->bNoIntersection) {
            if (g_Intersector.CheckExists(itype[0], itype[1])) {
                ...
                for (i = 0, pcontacts->center.zero(); i < inters.nborderpt; i++)
