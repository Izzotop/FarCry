...
struct BV {
    ...
    int iNode;
    operator primitive*() {
        return (primitive*)((char*)this + sizeof(type) + sizeof(iNode));
    }
};
...
public:
    ...
    virtual int GetNodeContents(int iNode, BV* pBVCollider, int bColliderUsed, int bColliderLocal, geometry_under_test* pGTest, geometry_under_test* pGTestOp) = 0;
