...
int CParticleEntity::IsAwake(int ipart) {
    ...
    return m_vel.len2() < sqr(500.0f) && m_pos.z > -1000.0f && m_bForceAwake != 0 &&
           (m_bForceAwake == 1 || (m_vel.len2() > sqr(m_pWorld->m_vars.minSeparationSpeed) || m_CollHistory[m_iCollHistory].age > 1E9 && !m_bSliding && gravity.len2() > 0));
}
...
int CParticleEntity::Step(float time_interval) {
    ...
    if (IsAwake()) {
        ...
        if (m_bSliding) {
            if (m_pWorld->RayWorldIntersection(m_pos, m_slide_normal * (m_dim * -1.1f), ent_all, m_iPierceability | (geom_colltype0 | geom_colltype_ray) << rwi_colltype_bit, hits,
                                               1, m_pColliderToIgnore, this)) {
                ...
                if (m_flags & particle_no_roll || m_slide_normal.z < 0.5f) { // always slide if the slope is steep enough
                    friction =
                        max(0.0f, (m_pWorld->m_FrictionTable[m_surface_idx & NSURFACETYPES - 1] + m_pWorld->m_FrictionTable[hits[0].surface_idx & NSURFACETYPES - 1]) * 0.5f);
                    ...
                    if (((CPhysicalEntity*)hits[0].pCollider)->m_iSimClass > 1) {
                        ...
                        vn0 = 0;
                    m_vel = vel0 = hits[0].n * max(vn0, vn) + vtang * (max(0.0f, vtang_len - max(0.0f, -(vn + (m_gravity * hits[0].n) * time_interval)) * friction) * rvtang_len);
                    m_wspin.zero();
                    m_qspin.SetIdentity();
                    if (!(m_flags & particle_constant_orientation))
        ...
        m_vel += (gravity + m_heading * m_accThrust - m_vel * kAirResistance + (m_heading ^ (m_heading ^ m_dirdown)).normalize() * (m_kAccLift * m_vel.len())) * time_interval;
        ...
        if (m_pWorld->m_bWorldStep == 2) {
            ...
            if (bHit) { // ignore collisions with moving bodies if they push us through statics
                ...
                bHit ^= m_pWorld->RayWorldIntersection(pos0, hits[0].pt - pos0 + heading0 * m_dim, ent_terrain | ent_static,
                                                       m_iPierceability | (geom_colltype0 | geom_colltype_ray) << rwi_colltype_bit | rwi_ignore_back_faces, hits, 1,
                                                       m_pColliderToIgnore, this);
            }
            if (nhits = bHit)
                pos0 = posFixed;
        }
        ...
        if (!bHit) {
            ...
            nhits = m_pWorld->RayWorldIntersection(pos0, m_pos - pos0 + heading0 * m_dim, ent_all,
                                                   m_iPierceability | (geom_colltype0 | geom_colltype_ray) << rwi_colltype_bit | rwi_ignore_back_faces, hits, 8,
                                                   m_pColliderToIgnore, this);
        ...
        if (bHit) {
            ...
            if (m_vel.len2() < sqr(m_pWorld->m_vars.minSeparationSpeed) || m_flags & particle_single_contact) {
                ...
                vel_next = m_vel + (gravity + m_heading * m_accThrust - m_vel * kAirResistance + (m_heading ^ (m_heading ^ m_dirdown)).normalize() * (m_kAccLift * m_vel.len())) *
                                       time_interval;
