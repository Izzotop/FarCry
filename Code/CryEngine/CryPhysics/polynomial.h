...
public:
    explicit polynomial_tpl() {
        denom = (ftype)1;
    };
    explicit polynomial_tpl(ftype op) {
        zero();
        data[degree] = op;
    }
    polynomial_tpl& zero() {
    ...
    polynomial_tpl(const polynomial_tpl<ftype, degree>& src) {
        *this = src;
    }
    ...
    polynomial_tpl<ftype, degree * 2> sqr() const {
        return *this * *this;
    }
...
template <class ftype> struct tagPolyE {
    inline static ftype polye() {
        return (ftype)1E-10;
    }
};

inline float tagPolyE<float>::polye() {
    return 1e-6f;
}

template <class ftype> inline ftype polye() {
    return tagPolyE<ftype>::polye();
}
...
template <class ftype, int degree> polynomial_tpl<ftype, degree * 2> psqr(const polynomial_tpl<ftype, degree>& op) {
    return op * op;
}
...
template <class ftype, int degree1, int degree2> polynomial_tpl<ftype, degree1>& operator+=(polynomial_tpl<ftype, degree1>& op1, const polynomial_tpl<ftype, degree2>& op2) {
...
void polynomial_divide(const polynomial_tpl<ftype, 8>& num, const polynomial_tpl<ftype, 8>& den, polynomial_tpl<ftype, 8>& quot, polynomial_tpl<ftype, 8>& rem, int degree1,
                       int degree2) {
    int i, j, k, l;
    ftype maxel;
    for (i = 0; i <= degree1; i++)
    ...
    for (maxel *= polye<ftype>(); degree1 >= 0 && fabs_tpl(num.data[degree1]) < maxel; degree1--)
        ;
    for (i = 1, maxel = fabs_tpl(den.data[0]); i <= degree1; i++)
...
polynomial_tpl<ftype, degree1 - degree2> operator/(const polynomial_tpl<ftype, degree1>& num, const polynomial_tpl<ftype, degree2>& den) {
    ...
    polynomial_divide((polynomial_tpl<ftype, 8>&)num, (polynomial_tpl<ftype, 8>&)den, (polynomial_tpl<ftype, 8>&)quot, (polynomial_tpl<ftype, 8>&)rem, degree1, degree2);
    return quot;
}
template <class ftype, int degree1, int degree2>
...
template <class ftype, int degree> void polynomial_tpl<ftype, degree>::calc_deriviative(polynomial_tpl<ftype, degree>& deriv, int curdegree) const {
    for (int i = 0; i < curdegree; i++)
        deriv.data[i] = data[i + 1] * (i + 1);
    deriv.denom = denom;
}

template <class ftype, int degree> int polynomial_tpl<ftype, degree>::nroots(ftype start, ftype end) const {
    polynomial_tpl<ftype, degree> f[degree + 1];
    int i, j, sg_a, sg_b;
    ftype val, prevval;

    calc_deriviative(f[0]);
    polynomial_divide((polynomial_tpl<ftype, 8>&)*this, (polynomial_tpl<ftype, 8>&)f[0], (polynomial_tpl<ftype, 8>&)f[degree], (polynomial_tpl<ftype, 8>&)f[1], degree, degree - 1);
    f[1].denom = -f[1].denom;
    for (i = 2; i < degree; i++) {
        polynomial_divide((polynomial_tpl<ftype, 8>&)f[i - 2], (polynomial_tpl<ftype, 8>&)f[i - 1], (polynomial_tpl<ftype, 8>&)f[degree], (polynomial_tpl<ftype, 8>&)f[i],
                          degree + 1 - i, degree - i);
        f[i].denom = -f[i].denom;
...
template <class ftype> inline ftype cubert_tpl(ftype x) {
    return fabs_tpl(x) > 1E-20 ? exp_tpl(log_tpl(fabs_tpl(x)) * (ftype)(1.0 / 3)) * sgnnz(x) : x;
}
template <class ftype> inline ftype pow_tpl(ftype x, ftype pow) {
    return fabs_tpl(x) > 1E-20 ? exp_tpl(log_tpl(fabs_tpl(x)) * pow) * sgnnz(x) : x;
}
template <class ftype> inline void swap(ftype* ptr, int i, int j) {
    ftype t = ptr[i];
    ptr[i] = ptr[j];
    ptr[j] = t;
}

template <class ftype, int maxdegree> int polynomial_tpl<ftype, maxdegree>::findroots(ftype start, ftype end, ftype* proots, int nIters, int degree) const {
    int i, j, nRoots = 0;
    ftype maxel;
    ...
    if (degree == 1) {
        ...
        p3aux.set(kp3aux);
        p3aux.findroots(-1E10, 1E10, subroots);
        y = subroots[0];
