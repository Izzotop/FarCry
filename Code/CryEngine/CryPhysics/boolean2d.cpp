...
int boolean2d(booltype type, vector2df* ptbuf1, int npt1, vector2df* ptbuf2, int npt2, int bClosed, vector2df*& ptres, int*& pidres) {
    ...
    if (bClosed)
        iobj = isneg((ptmax[1].x - ptmin[1].x + ptmax[1].y - ptmin[1].y) * npt[0] - (ptmax[0].x - ptmin[0].x + ptmax[0].y - ptmin[0].y) * npt[1]);
    else
        iobj = 1;
    ...
    for (idx = bPrevInside = 0; idx < ninters; idx++) {
        ...
        if (bInside | bClosed) {
            ...
            bool bForceFirstStep = i == g_BoolInters[idx_next].iedge[iobj1] && (g_BoolInters[idx].pt - ptsrc[iobj1][i]) * dp > (g_BoolInters[idx_next].pt - ptsrc[iobj1][i]) * dp;
            for (; bForceFirstStep || i != g_BoolInters[idx_next].iedge[iobj1] && (iobj1 == iobj || i != g_BoolInters[idx_prev].iedge[iobj1]);
