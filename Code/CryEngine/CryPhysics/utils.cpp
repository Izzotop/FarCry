...
real ComputeMassProperties(strided_pointer<const vectorf> points, const index_t* faces, int nfaces, vectorr& center, matrix3x3& I) {
    ...
    for (nfaces--; nfaces >= 0; nfaces--, faces += 3) {
        ...
        for (i = 0; i < 3; i++)
            p[i] = points[faces[i]].permutated(g);
        p[3] = p[0];
        n = n.permutated(g);
...
int ChoosePrimitiveForMesh(strided_pointer<const vectorf> pVertices, const index_t* pIndices, int nTris, const vectorr* eigen_axes, const vectorr& center, int flags,
                           float tolerance, primitive*& pprim) {
    ...
    if (flags & mesh_approx_cylinder) {
        ...
        for (i = 1; i < nTris * 3; i++) {
            ...
            ptmin.x = min(ptmin.x, ptloc.x);
            ptmax.x = max(ptmax.x, ptloc.x);
            ptmin.y = min(ptmin.y, ptloc.y);
            ptmax.y = max(ptmax.y, ptloc.y);
            ptmin.z = min(ptmin.z, ptloc.z);
            ptmax.z = max(ptmax.z, ptloc.z);
        }
...
int BreakPolygon(vector2df* ptSrc, int nPt, int nCellx, int nCelly, int maxPatchTris, vector2df*& ptout, int*& nPtOut, float jointhresh, int seed) {
    int i, j, k, i0, i1, ix, iy, nPatches, nSeedTris, nPatchTris, iTri, iTriNew, nCells, ihead, itail, ipt0, ipt1, nOut, iMotherPatch, szQueue, nPatchPt, nOutAlloc, bOut, iEdge,
        iPatch, iCurPatch, ilist[32], nInters, iPass, ix0, iy0, ix1, iy1, bHasInclusions, nPatchesOut, iExit, iEdge0, ipt, iEnter;
    vector2di sz;
    ...
    for (i = 1, ptmin = ptmax = ptSrc[0]; i < nPt; i++) {
        ptmin.x = min(ptmin.x, ptSrc[i].x);
        ptmin.y = min(ptmin.y, ptSrc[i].y);
        ptmax.x = max(ptmax.x, ptSrc[i].x);
        ptmax.y = max(ptmax.y, ptSrc[i].y);
    }
    ...
    for (ix = 0; ix < sz.x; ix++)
        for (iy = 0; iy < sz.y; iy++) // generate jittered grid points
            pt[ix + iy * sz.x].set((float2int((ix * step.x + frand(step.x * 0.45f) - snap * 0.5f) * rsnap) + 0.5f) * snap,
    ...
    // mark borders
    for (ix = 0; ix < sz.x; ix++)
        pTris[ix * 2] = pTris[ix * 2 + 1] = pTris[(ix + sz.x * (sz.y - 1)) * 2] = pTris[(ix + sz.x * (sz.y - 1)) * 2 + 1] = 3 << 29;
    for (iy = 0; iy < sz.y; iy++)
        pTris[iy * sz.x * 2] = pTris[iy * sz.x * 2 + 1] = pTris[(iy * sz.x + sz.x - 1) * 2] = pTris[(iy * sz.x + sz.x - 1) * 2 + 1] = 3 << 29;

    // unite grid triangles into patches by randomly growing them
    pTris[pPatchSeeds[0] = queue[0] = ((sz.y >> 1) + (sz.x >> 1) * sz.x) * 2] = 0;
    itail = 0;
    ihead = 1;
    nSeedTris = 0;
    nPatches = 0;
    do {
        ...
        for (i = 0; i < 3; i++)
            if (pTris[iTri = get_neighb(queue[itail], i, pCellDiv, sz.x)] == -1)
        ...
        if (nPatchTris > 0)
            do { // grow iCurPatch around a seed triangle
                ...
                if (pTris[iTri] == nPatches)
                    for (i = 0; i < 3; i++)
                        if (pTris[get_neighb(iTri, i, pCellDiv, sz.x)] < 0) {
                            queue[ihead] = get_neighb(iTri, i, pCellDiv, sz.x);
    ...
    do {
        for (iCurPatch = bHasInclusions = 0; iCurPatch < nPatches; iCurPatch++)
            if (pPatchMothers[iCurPatch] >= -1) {
                pTris[queue[0] = pPatchSeeds[iCurPatch]] |= 1 << 30;
                itail = 0;
                ihead = 1;
                iMotherPatch = -1;
        ...
        for (i = nCells * 2 - 1; i >= 0; i--)
            pTris[i] &= ~(1 << 30);
        if (bHasInclusions)
            do { // merge nested inclusions
                for (i = j = 0; i < nPatches; i++)
                    if (pPatchMothers[i] >= 0 && pPatchMothers[pPatchMothers[i]] >= 0) {
                        pPatchMothers[i] = pPatchMothers[pPatchMothers[i]];
                        j++;
                    }
            } while (j);
    ...
    for (iCurPatch = 0; iCurPatch < nPatches; iCurPatch++) {
        ...
        ymin.set(1, 0);
        bOut = 1;
        for (ipt = ipt0; ipt < nCells; ipt += sz.x)
            for (i = pGrid[ipt]; i < pGrid[ipt + 1]; i++)
        ...
        do {
            ...
            d0 = pt[ipt1] - pt[ipt0];
            nInters = 0;
            for (ix = -1, iy = 0; ix <= 1; ix++)
                for (iy = -sz.x; iy <= sz.x; iy += sz.x) // ix,iy - rectange dimensions
                    if (ipt1 == ipt0 + ix + iy)
                        goto foundixiy;
        foundixiy:
            for (i0 = iszero(ix) ^ 1; i0 >= 0; i0--)
                for (i1 = iszero(iy) ^ 1; i1 >= 0; i1--)
                    for (i = pGrid[ipt0 + (ix & -i0) + (iy & -i1)]; i < pGrid[ipt0 + (ix & -i0) + (iy & -i1) + 1]; i++) {
                        d1 = ptin[pHash[i] + 1] - ptin[pHash[i]];
                        dc = ptin[pHash[i]] - pt[ipt0];
                        t0.y = t1.y = d0 ^ d1;
                        t0.x = dc ^ d1;
                        t1.x = dc ^ d0;
                        if (t0.isin01() & t1.isin01()) { // we have a valid intersection
                            for (j = 0; j < nInters && tlist[j] < t0; j++)
                                ; // keep a sorted array of intersection t's
...
void RasterizePolygonIntoCubemap(const vectorf* pt, int npt, int iPass, int* pGrid[6], int nRes, float rmin, float rmax, float zscale) {
    int iPlane, iPlaneEnd, ipt, ipt1, i, j, iBrd[2], iBrdNext[2], idx, lxdir, iSign, ixlim[2], iylim, imask, idcell, iz, irmin, iyend, iOrder, nCells, ixleft, iEnter, nPlanes;
    vectori ic;
...
void CalcMediumResistance(const vectorf* ptsrc, int npt, const vectorf& n, const plane& waterPlane, const vectorf& vworld, const vectorf& wworld, const vectorf& com, vectorf& P,
                          vectorf& L) {
    int i;
...
struct ComputeBitCount {
    ComputeBitCount() {
        for (int i = 0; i < 256; i++)
            for (int j = g_bitcount[i] = 0; j < 8; j++)
                g_bitcount[i] += (i >> j) & 1;
    }
};
...
int CoverPolygonWithCircles(strided_pointer<vector2df> pt, int npt, bool bConsecutive, const vector2df& center, vector2df*& centers, float*& radii, float minCircleRadius) {
    intptr_t imask;
    ...
    do {
        ...
        for (pvtx = (pvtx_left = pvtx_max)->next; pvtx != pvtx_max && sqr(pvtx->pt ^ pvtx_max->pt) < 0.25f * pvtx->pt.len2() * len2 && pvtx->pt * pvtx_max->pt > 0;
             pvtx = pvtx->next) {
        ...
        // find the farthest from the center vertex in -30 degrees vicinity of the global maximum
        for (pvtx = (pvtx_right = pvtx_max)->prev; pvtx != pvtx_max && sqr(pvtx->pt ^ pvtx_max->pt) < 0.25f * pvtx->pt.len2() * len2 && pvtx->pt * pvtx_max->pt > 0;
             pvtx = pvtx->prev) {
