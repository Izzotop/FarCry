...
int CHeightfield::PrepareForIntersectionTest(geometry_under_test* pGTest, CGeometry* pCollider, geometry_under_test* pGTestColl, bool bKeepPrevContacts) {
    ...
    for (i = ix, maxh = m_minHeight; i <= ix + sx; i++)
        for (j = iy; j <= iy + sy; j++)
    ...
    for (itri = 0; itri < m_nTris; itri++)
        if (m_pIds[itri] == -1)
            for (j = 0; j < 3; j++)
                if ((i = m_pTopology[itri].ibuddy[0]) >= 0) // remove connectivity for 'hole' triangles
...
int CHeightfield::FindClosestPoint(geom_world_data* pgwd, int& iPrim, int& iFeature, const vectorf& ptdst0, const vectorf& ptdst1, vectorf* ptres, int nMaxIters) {
    triangle tri;
    float h[4];
...
void CHeightfield::CalcVolumetricPressure(geom_world_data* gwd, const vectorf& epicenter, float k, float rmin, const vectorf& centerOfMass, vectorf& P, vectorf& L) {
    return;
}

int CHeightfield::DrawToOcclusionCubemap(const geom_world_data* pgwd, int iStartPrim, int nPrims, int iPass, int* pGrid[6], int nRes, float rmin, float rmax, float zscale) {
    ...
    for (ix = irect[0].x, nTris = 0; ix < irect[1].x; ix++)
        for (iy = irect[0].y; iy < irect[1].y; iy++) {
            ...
            if (m_hf.gettype(idcell) >= 0) {
                ...
                tri.pt[0] = tri.pt[2];
                tri.pt[2] += pgwd->R * vectorf(pgwd->scale * m_hf.step.x, 0, 0);
