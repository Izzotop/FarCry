...
int tri_tri_lin_unprojection(unprojection_mode* pmode, const triangle* ptri1, int iFeature1, const triangle* ptri2, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    ...
    for (itri = 0; itri < 2; itri++) { // check for vertex-face contacts (itri provides face, itri^1 - vertex)
        ...
        bContact = isneg((dp - ptri[itri]->pt[0] * t.y ^ ptri[itri]->pt[1] - ptri[itri]->pt[0]) * ptri[itri]->n) &
...
int tri_box_lin_unprojection(unprojection_mode* pmode, const triangle* ptri, int iFeature1, const box* pbox, int iFeature2, contact* pcontact, geom_contact_area* parea) {
...
int box_tri_lin_unprojection(unprojection_mode* pmode, const box* pbox, int iFeature1, const triangle* ptri, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    pmode->dir.Flip();
...
int tri_cylinder_lin_unprojection(unprojection_mode* pmode, const triangle* ptri, int iFeature1, const cylinder* pcyl, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    ...
    t.y *= nlen;
    bContact = 1 ^ (isneg((ptri->pt[1] - ptri->pt[0] ^ ptcyl - ptri->pt[0] * t.y) * ptri->n) | isneg((ptri->pt[2] - ptri->pt[1] ^ ptcyl - ptri->pt[1] * t.y) * ptri->n) |
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (sqr(edge * pcyl->axis) < edge.len2() * 1E-4) { // potential traingle edge - cylinder cap contact
                ...
                t.set((dp * pcyl->axis) * edge.len2() - (edge * pcyl->axis) * (edge * dp), // calculate when the closest point on edge will touch the cap
                      (pmode->dir * pcyl->axis) * edge.len2() - (edge * pcyl->axis) * (edge * pmode->dir))
                    .fixsign();
                if (t > tmax && t < pmode->tmax) {
                    pcontact->t = t.val();
...
int cylinder_tri_lin_unprojection(unprojection_mode* pmode, const cylinder* pcyl, int iFeature1, const triangle* ptri, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    pmode->dir.Flip();
...
int tri_sphere_lin_unprojection(unprojection_mode* pmode, const triangle* ptri, int iFeature1, const sphere* psphere, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    ...
    bContact = 1 ^ (isneg((ptri->pt[1] - ptri->pt[0] ^ center - ptri->pt[0] * t.y) * ptri->n) | isneg((ptri->pt[2] - ptri->pt[1] ^ center - ptri->pt[1] * t.y) * ptri->n) |
...
int sphere_tri_lin_unprojection(unprojection_mode* pmode, const sphere* psphere, int iFeature1, const triangle* ptri, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    pmode->dir.Flip();
...
int tri_ray_lin_unprojection(unprojection_mode* pmode, const triangle* ptri, int iFeature1, const ray* pray, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    ...
    for (i = 0, pt = pray->origin; i < 2; i++, pt += pray->dir) {
        ...
        bContact = 1 ^ (isneg((ptri->pt[1] - ptri->pt[0] ^ ptnew - ptri->pt[0] * t.y) * ptri->n) | isneg((ptri->pt[2] - ptri->pt[1] ^ ptnew - ptri->pt[1] * t.y) * ptri->n) |
...
int ray_tri_lin_unprojection(unprojection_mode* pmode, const ray* pray, int iFeature1, const triangle* ptri, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    pmode->dir.Flip();
...
int tri_plane_lin_unprojection(unprojection_mode* pmode, const triangle* ptri, int iFeature1, const plane* pplane, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    quotient t;
    real dist, mindist;
    int i, bBest;
...
int plane_tri_lin_unprojection(unprojection_mode* pmode, const plane* pplane, int iFeature1, const triangle* ptri, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    pmode->dir.Flip();
...
int box_box_lin_unprojection(unprojection_mode* pmode, const box* pbox1, int iFeature1, const box* pbox2, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    ...
    if (parea) {
        ...
        for (idir1 = 0; idir1 < 3; idir1++)
            if (fabs_tpl(axes[idir1].x) + fabs_tpl(axes[idir1].y) + fabs_tpl(axes[idir1].z) < (real)1.05) { // axes[i] is close to some coordinate axis
                ...
                bBest = -isneg((real)0.997 - fabs_tpl(axes[idir1].x));
                iz = iz & ~bBest;
                bBest = -isneg((real)0.997 - fabs_tpl(axes[idir1].y));
                iz = iz & ~bBest | 1 & bBest;
                bBest = -isneg((real)0.997 - fabs_tpl(axes[idir1].z));
                iz = iz & ~bBest | 2 & bBest;
                if (iz < 0)
                    continue;

                diff0 = fabs_tpl(center[0][iz] - dir[0][iz] * pcontact->t) -
                        (pbox[0]->size[iz] + fabs_tpl(axes[0][iz]) * pbox[1]->size.x + fabs_tpl(axes[1][iz]) * pbox[1]->size.y + fabs_tpl(axes[2][iz]) * pbox[1]->size.z);
            ...
            if (fabs_tpl(axes[idir1][iz]) < (real)0.02) { // check all near-zeros in Basis21 buffer
                ...
                for (ibox = 0; ibox < 2; ibox++) {
                    if (fabs_tpl(fabs_tpl(center[ibox][iz] - dir[ibox][iz] * pcontact->t) -
                        ...
                        min(pbox[ibox]->size[iz], pbox[1]->size[ix1] + pbox[ibox ^ 1]->size[iy1]) * 0.01f) { // confirmed face(ibox.iz) - edge(ibox^1.idir1) contact
...
int box_cylinder_lin_unprojection(unprojection_mode* pmode, const box* pbox, int iFeature1, const cylinder* pcyl, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    ...
    if (parea) {
                ...
                if (fabs_tpl(fabs_tpl(c[ic.z]) - size[ic.z]) < min(size[ic.z], hh) * tol && dlen < r * r) {
                    ...
                    for (i = 0; i < 4; i++)
                        if ((kd = r * r - sqr((pt[ic[i & 1]] = size[ic[i & 1]] * ((i & 2) - 1)) - c[ic[i & 1]])) > 0)
                            for (j = 0, pt[ic[i & 1 ^ 1]] = c[ic[i & 1 ^ 1]] - (kd = sqrt_tpl(kd)); j < 2; j++, pt[ic[i & 1 ^ 1]] += kd * 2)
                                if (fabs_tpl(pt[ic[i & 1 ^ 1]]) < size[ic[i & 1 ^ 1]]) {
                                    parea->pt[parea->npt] = pt * pbox->Basis + pbox->center + pmode->dir * pcontact->t;
                                    parea->piPrim[0][parea->npt] = parea->piPrim[1][parea->npt] = 0;
        ...
        for (ic.z = 0; ic.z < 3; ic.z++)
            if (fabs_tpl(axis[ic.z]) < (real)0.04) {
...
int cylinder_box_lin_unprojection(unprojection_mode* pmode, const cylinder* pcyl, int iFeature1, const box* pbox, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    pmode->dir.Flip();
...
int box_sphere_lin_unprojection(unprojection_mode* pmode, const box* pbox, int iFeature1, const sphere* psph, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    ...
    if (pmode->dir.len2() == 0) { // minimum unprojection dir requested
        ...
        case 1:
            ...
            dir.zero()[iz] = -isg.z;
            pcontact->t = r - fabs_tpl(center[iz] - size[iz] * isg.z);
...
int sphere_box_lin_unprojection(unprojection_mode* pmode, const sphere* psph, int iFeature1, const box* pbox, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    pmode->dir.Flip();
...
int cylinder_sphere_lin_unprojection(unprojection_mode* pmode, const cylinder* pcyl, int iFeature1, const sphere* psph, int iFeature2, contact* pcontact,
...
int sphere_cylinder_lin_unprojection(unprojection_mode* pmode, const sphere* psph, int iFeature1, const cylinder* pcyl, int iFeature2, contact* pcontact,
                                     geom_contact_area* parea) {
    pmode->dir.Flip();
...
int sphere_sphere_lin_unprojection(unprojection_mode* pmode, const sphere* psph1, int iFeature1, const sphere* psph2, int iFeature2, contact* pcontact, geom_contact_area* parea) {
...
int cyl_cyl_lin_unprojection(unprojection_mode* pmode, const cylinder* pcyl1, int iFeature1, const cylinder* pcyl2, int iFeature2, contact* pcontact, geom_contact_area* parea) {
    /*box abox;
...
#define calcQ                                                                                                                                                                      \
    cx2 = sqr(c2d.x + dir2d.x * t.x);                                                                                                                                              \
    cy2 = sqr(c2d.y + dir2d.y * t.x);                                                                                                                                              \
    a = a0 - cx2 - cy2;                                                                                                                                                            \
    b = b0 - sqr(r0) * (cx2 * sqr(cosa) + cy2);                                                                                                                                    \
    p = b - a * a * (1.0f / 3);                                                                                                                                                    \
    ...
    case 0x40: // cap edge - side
        ...
        i = (idbest * 2 & 2) - 1;
        pcontact->t = tmax.val();
        pcontact->pt = pcyl[icyl]->center + pcyl[icyl]->axis * (pcyl[icyl]->hh * i) + pmode->dir * (pcontact->t * (1 - icyl)) + axisx * ptbest2d.x +
                       (pcyl[icyl]->axis ^ axisx) * (ptbest2d.y * sgnnz(cosa));
                ...
                for (icyl = 0; icyl < 2; icyl++)
                    for (i = -1; i <= 1; i += 2)
                        if (fabs_tpl((dc * (1 - icyl * 2) + pcyl[icyl]->axis * (pcyl[icyl]->hh * i)) * pcyl[icyl ^ 1]->axis) < pcyl[icyl ^ 1]->hh * 1.002f) {
                            parea->pt[parea->npt] =
                                pcyl[icyl]->center + pmode->dir * (pcontact->t * (1 - icyl)) + pcyl[icyl]->axis * (pcyl[icyl]->hh * i) + dp * (pcyl[icyl]->r * (1 - icyl * 2));
