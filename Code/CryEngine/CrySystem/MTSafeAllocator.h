...
// in multi-threaded environment. STL-Compatible
template <class _Ty, class _THeap = CMTSafeHeap> class CMTSafeAllocator {
public:
    ...
    template <class _Other> struct rebind { // convert an CMTSafeAllocator<_Ty> to an CMTSafeAllocator <_Other>
    ...
    CMTSafeAllocator(_THeap* pHeap) : m_pHeap(pHeap) {}

    CMTSafeAllocator(const CMTSafeAllocator<_Ty>& rThat) : m_pHeap(rThat.m_pHeap) {}

    template <class _Other> CMTSafeAllocator(const CMTSafeAllocator<_Other, _THeap>& rThat) {
        m_pHeap = rThat.GetHeap();
    }

    template <class _Other> CMTSafeAllocator<_Ty>& operator=(const CMTSafeAllocator<_Other>& rThat) {
    ...
    void deallocate(void* _Ptr, size_type) { // deallocate object at _Ptr, ignore size
        Deallocate(_Ptr);
    }
    ...
    size_t max_size() const { // estimate maximum array size
...
template <class _Ty, class _THeap> _Ty* CMTSafeAllocator<_Ty, _THeap>::Allocate(size_t _Count, _Ty*) { // allocate storage for _Count elements of type _Ty
...
template <class _Ty, class _THeap> void CMTSafeAllocator<_Ty, _THeap>::Deallocate(void* p) {
    m_pHeap->Free(p);
}

template <class _Ty, class _Other, class _THeap>
inline bool operator==(const CMTSafeAllocator<_Ty, _THeap>& left, const CMTSafeAllocator<_Other, _THeap>& right) { // test for allocator equality (always true)
    return left.GetHeap() == right.GetHeap();
}

template <class _Ty, class _Other, class _THeap>
inline bool operator!=(const CMTSafeAllocator<_Ty, _THeap>& left, const CMTSafeAllocator<_Other, _THeap>& right) { // test for allocator inequality (always false)
    return left.GetHeap() != right.GetHeap();
}
