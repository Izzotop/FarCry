...
void CScriptSink::OnScriptError(const char* sSourceFile, const char* sFuncName, int nLineNum, const char* sErrorDesc) {
    ...
    {
        m_pSystem->Warning(VALIDATOR_MODULE_SCRIPTSYSTEM, VALIDATOR_ERROR, VALIDATOR_FLAG_SCRIPT, sSourceFile, "$3#SCRIPT ERROR File: %s, Line [%03d], Function: %s,\n%s",
                           sSourceFile, nLineNum, sFuncName, sErrorDesc);
    } else {
        m_pSystem->Warning(VALIDATOR_MODULE_SCRIPTSYSTEM, VALIDATOR_ERROR, VALIDATOR_FLAG_SCRIPT, sSourceFile, "$3#SCRIPT ERROR File: %s, Function: %s,\n%s", sSourceFile,
                           sFuncName, sErrorDesc);
    }
    ...
    if (m_pSystem->GetLuaDebugger()) {
        if (sSourceFile != nullptr && _stricmp(sSourceFile, "__script_buffer__") != 0 && _stricmp(sSourceFile, "=C") != 0 && _stricmp(sSourceFile, "undefined") != 0 &&
            nLineNum != -1 && sSourceFile[0] != '\0') {
            // char szMessage[2048];
...
void CScriptSink::OnExecuteLine(ScriptDebugInfo& sdiDebugInfo) {
    ...
    */
    ...
    if (m_pSystem->GetLuaDebugger()) {
        m_pSystem->ShowDebugger(sdiDebugInfo.sSourceName, sdiDebugInfo.nCurrentLine, "Breakpoint Hit");
    }
}
