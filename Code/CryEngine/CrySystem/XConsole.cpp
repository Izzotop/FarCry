...
void CXConsole::ProcessInput(int nKeyCode, const char* sKeyName) {
    ...
    case XKEY_DOWN: {
        ...
        ////////////////////////////////////////////////////////////
    case XKEY_TAB:
        if ((!m_pSystem->GetIInput()->KeyDown(XKEY_LALT)) && (!m_pSystem->GetIInput()->KeyDown(XKEY_RALT)) && (!m_pSystem->GetIInput()->KeyDown(XKEY_ALT))) {
...
void CXConsole::DumpCommandsVars(char* prefix) {
    ...
    for (ConsoleCommandsMapItor itrCmd = m_mapCommands.begin(); itrCmd != m_mapCommands.end(); ++itrCmd) {
        ...
        if (hasprefix(cmd.m_sName.c_str(), prefix)) {
            ...
            if (cmd.m_dwFlags & VF_CHEAT)
                sFlags += " CHEAT";
            if (cmd.m_dwFlags & VF_REQUIRE_NET_SYNC)
                sFlags += " REQUIRE_NET_SYNC";
            if (cmd.m_dwFlags & VF_SAVEGAME)
                sFlags += " SAVEGAME";
            if (cmd.m_dwFlags & VF_READONLY)
                sFlags += " READONLY";
    ...
    for (ConsoleVariablesMapItor itrVar = m_mapVariables.begin(); itrVar != m_mapVariables.end(); ++itrVar) {
        ...
        if (hasprefix(var->GetName(), prefix)) {
            ...
            if (var->GetFlags() & VF_CHEAT)
                sFlags += " CHEAT";
            if (var->GetFlags() & VF_REQUIRE_NET_SYNC)
                sFlags += " REQUIRE_NET_SYNC";
            if (var->GetFlags() & VF_SAVEGAME)
                sFlags += " SAVEGAME";
            if (var->GetFlags() & VF_READONLY)
                sFlags += " READONLY";
...
void CXConsole::ExecuteString(const char* command, bool bNeedSlash, bool bIgnoreDevMode) {
    ...
    if (command[0] == '#') {
        if (!bIgnoreDevMode)
            AddLine(sTemp); // don't put line in console if we are overriding devmode: TRay 3/4/04
...
void CXConsole::Exit(const char* szExitComments, ...) {
    ...
    if (szExitComments) { // make result string
        va_list arglist;
...
void CXConsole::GetMemoryUsage(class ICrySizer* pSizer) {
    pSizer->AddObject(this, sizeof(*this) + sizeOf(m_dqConsoleBuffer) + sizeOf(m_dqHistory) + sizeOf(m_sInputBuffer) + sizeOf(m_sPrevTab));
