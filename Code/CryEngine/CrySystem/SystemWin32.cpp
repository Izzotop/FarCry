...
const char* g_szModuleGroups[][2] = {
    {"FarCry.exe", g_szGroupGame},         {"FarCry_WinSV.exe", g_szGroupGame},    {"Editor.exe", g_szGroupGame},
    {"CrySystem.dll", g_szGroupCore},      {"CryScriptSystem.dll", g_szGroupGame}, {"CryNetwork.dll", g_szGroupGame},
    {"CryPhysics.dll", g_szGroupCore},     {"CryMovie.dll", g_szGroupCore},        {"CryInput.dll", g_szGroupCore},
...
#endif
    {"CryFont.dll", g_szGroupCore},        {"CryAISystem.dll", g_szGroupGame},     {"CryEntitySystem.dll", g_szGroupCore},
    {"Cry3DEngine.dll", g_szGroupCore},    {"CryGame.dll", g_szGroupGame},         {"CryAnimation.dll", g_szGroupCore},
    {"XRenderD3D9.dll", g_szGroupCore},    {"XRenderOGL.dll", g_szGroupCore},      {"XRendernullptr.dll", g_szGroupCore}
};
...
void CSystem::DebugStats(bool checkpoint, bool leaks) {
    ...
    */
    ...
    for (int i = 0; i < realnumh; i++) {
        ...
        CryLogAlways("* heap %8x: %6d (or ~%6d) K in use, %6d..%6d K uncommitted, %6d K overhead (%s)\n", handles[i], nCommittedPieces / 1024, nCommitted / 1024,
                     nUncommittedPieces / 1024, nUncommitted / 1024, nOverhead / 1024, hinfo);
...
public:
    bool operator()(const char* szLeft, const char* szRight) const {
        return stricmp(szLeft, szRight) < 0;
    }
};
...
void CSystem::DumpWinHeaps() {
    ...
    if (Heap32ListFirst(hSnapshot, &hl)) {
        ...
        CryLogAlways("-------------------------------------------------");
        CryLogAlways("$6          %6.3f %6.3f %6.3f (%.3f) Mbytes", double(stats.dwFixed) / 0x100000, double(stats.dwFree) / 0x100000, double(stats.dwMoveable) / 0x100000,
                     double(stats.dwUnknown) / 0x100000);
        CryLogAlways("-------------------------------------------------");
    } else
...
// return pointer to the nullptr terminated error string or 0
...
bool CSystem::GetSSFileInfo(const char* inszFileName, char* outszInfo, const DWORD indwBufferSize) {
    ...
    {
        const char* szDatabase = nullptr;
        const char* szRoot = nullptr;
        ...
        if (::_GetSSFileInfo(
                szDatabase, // sourcesafe file
                szRoot,     // project sourcesafe path
                szMasterCD, // project folder path
                inszFileName, name, comment, date, iSize)
        ) {
