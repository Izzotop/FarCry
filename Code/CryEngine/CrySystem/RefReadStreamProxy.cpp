...
CRefReadStreamProxy::CRefReadStreamProxy(const char* szSource, CRefReadStream* pStream, IStreamCallback* pCallback, StreamReadParams* pParams)
    : m_strClient(szSource), m_pStream(pStream), m_pCallback(pCallback), m_bError(false), m_bFinished(false), m_bFreeBuffer(false), m_bPending(false), m_pBuffer(nullptr),
      m_numBytesRead(0), m_numRetries(0) {
    if (pParams)
        m_Params = *pParams;
...
VOID CALLBACK CRefReadStreamProxy::FileIOCompletionRoutine(DWORD dwErrorCode,               // completion code
...
void CRefReadStreamProxy::FinalizeIO() {
    if (m_pCallback) {
        ...
        g_System->GetILog()->LogToFile("\006io(%s) err %d%s%s%s%s piece(%d:%d) read %d %s userdata %d, pri %d, flags %x, offs %d, size %d", m_strClient.c_str(), m_nIOError,
                                       m_bError ? " Error" : "", m_bFinished ? " Finished" : "", m_bFreeBuffer ? " FreeBuffer" : "", m_bPending ? " Pending" : "", m_nPieceOffset,
                                       m_nPieceLength, m_numBytesRead, m_pStream->GetFileName().c_str(), m_Params.dwUserData, m_Params.nPriority, m_Params.nFlags, m_Params.nOffset,
                                       m_Params.nSize);
#endif
...
string CRefReadStreamProxy::Dump() {
    char szDump[0x300];
    _snprintf(szDump, sizeof(szDump), "%s: callback %p, %s%s%s %d bytes read, offset=%d, size=%d, flags=%x", m_strClient.c_str(), m_pCallback, m_bPending ? "PENDING " : "",
              m_bFinished ? "FINISHED " : "", m_bError ? "ERROR " : "", m_numBytesRead, m_Params.nOffset, m_Params.nSize, m_Params.nFlags);
    return szDump;
}
