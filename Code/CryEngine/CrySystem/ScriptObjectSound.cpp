...
int CScriptObjectSound::Load3DSound(IFunctionHandler* pH) {
    ...
    if (m_pSoundSystem) {
        ...
        if (pSound) {
            ...
            m_pSystem->Warning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, VALIDATOR_FLAG_SOUND, sSound, "Sound %s Failed to Load", sSound);
        }
    }
...
int CScriptObjectSound::SetEaxEnvironment(IFunctionHandler* pH) {
    ...
    */
    ...
    if (pH->GetParam(1, pObj)) {
        ...
        pObj->GetValueChain("nEnvironment", nTemp);
        pProps.Environment = (unsigned int)nTemp;
        ...
        vVec = oVec.Get();
        pProps.ReflectionsPan[0] = vVec.x;
        pProps.ReflectionsPan[1] = vVec.y;
        pProps.ReflectionsPan[2] = vVec.z;
        ...
        vVec = oVec.Get();
        pProps.ReverbPan[0] = vVec.x;
        pProps.ReverbPan[1] = vVec.y;
        pProps.ReverbPan[2] = vVec.z;
        ...
        pObj->GetValueChain("nFlags", nTemp);
        pProps.Flags = (unsigned int)nTemp;
        pObj->EndSetGetChain();
...
int CScriptObjectSound::GetMusicStatus(IFunctionHandler* pH) {
    ...
    for (TPatternStatusVecIt It = pStatus->m_vecPlayingPatterns.begin(); It != pStatus->m_vecPlayingPatterns.end(); ++It) {
        ...
        pLog->LogToConsole("    %s (Layer: %s; Volume: %s)", PatternStatus.sName.c_str(),
                           (PatternStatus.nLayer == MUSICLAYER_MAIN) ? "Main" : ((PatternStatus.nLayer == MUSICLAYER_RHYTHMIC) ? "Rhythmic" : "Incidental"), PatternStatus.nVolume);
    }
    return pH->EndFunction();
}
