...
class FindData {
public:
    FindData(DirHeader* pRoot) : m_pRoot(pRoot), m_pDirHeader(nullptr) {}
...
public:
    FindFile(Cache* pCache) : FindData(pCache->GetRoot()) {}
    FindFile(DirHeader* pRoot) : FindData(pRoot) {}
...
public:
    FindDir(Cache* pCache) : FindData(pCache->GetRoot()) {}
    FindDir(DirHeader* pRoot) : FindData(pRoot) {}
    // if bExactFile is passed, only the file is searched, and besides with the exact name as passed (no wildcards)
    bool FindFirst(const char* szWildcard);
