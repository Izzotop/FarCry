...
CCryPak::CCryPak(IMiniLog* pLog, PakVars* pPakVars)
    : m_pLog(pLog), m_setCachedFiles(CCachedFileDataOrder(), CachedFileDataAllocator(g_pSmallHeap)), m_bRememberOpenedFiles(false), m_pPakVars(pPakVars ? pPakVars : &g_PakVars),
...
void CCryPak::RemoveMod(const char* szMod) {
    ...
    for (it = m_arrMods.begin(); it != m_arrMods.end(); ++it) {
        ...
        if (strstr(sMOD.c_str(), szMod) == nullptr) {
            m_arrMods.erase(it);
            break;
        }
    } // it
...
char* CCryPak::BeautifyPath(char* dst) {
    ...
    if (*p == g_cNonNativeSlash || *p == g_cNativeSlash)
        ++p, ++q; // start normalization/beautification's from the second symbol; if it's a slash, we'll add it, too
...
const char* CCryPak::AdjustFileName(const char* src, char* dst, unsigned nFlags, bool* bFoundInPak) {
    ...
    } else if (nFlags & FLAGS_ONLY_MOD_DIRS) // only in MOD dirs, but no MODs found
        return nullptr;
...
bool CCryPak::OpenPackCommon(const char* szBindRoot, const char* szFullPath, unsigned nFlags) {
    AUTO_LOCK(m_csZips);
    {
        ...
        for (ZipArray::iterator it = m_arrZips.begin(); it != m_arrZips.end(); ++it)
            if (!stricmp(it->pZip->GetFilePath(), szFullPath) && !stricmp(it->strBindRoot.c_str(), szBindRoot))
...
ICryArchive* CCryPak::OpenArchive(const char* szPath, unsigned nFlags) {
    ...
    try {
        ...
        m_pLog->LogError("can't create the archive \"%s\"==\"%s\": error %s (code %d) in %s at %s:%d. %s.", szPath, szFullPath, e.getError(), e.nError, e.szFunction, e.szFile,
                         e.nLine, e.getDescription());
        return nullptr;
    }
}
