...
CHTTPDownloader::CHTTPDownloader()
    ...
    : m_hThread(nullptr),
#endif
      m_hINET(nullptr), m_hUrl(nullptr), m_iFileSize(0), m_pBuffer(nullptr), m_iState(HTTP_STATE_NONE), m_pSystem(nullptr), m_pParent(nullptr) {
}
...
void CHTTPDownloader::CreateThread() {
    DWORD dwThreadId = 0;
    m_hThread = ::CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE)DownloadProc, this, 0, &dwThreadId);
}

DWORD CHTTPDownloader::DoDownload() {
    m_iState = HTTP_STATE_WORKING;

    m_hINET = InternetOpen("", INTERNET_OPEN_TYPE_PRECONFIG, nullptr, nullptr, 0);
    if (!m_hINET) {
    ...
    m_hUrl = InternetOpenUrl(m_hINET, m_szURL.c_str(), nullptr, 0, dwFlags, 0);
    ...
    char szBuffer[64] = {0};
    DWORD dwSize = 64;
    int bQuery = HttpQueryInfo(m_hUrl, HTTP_QUERY_CONTENT_LENGTH, szBuffer, &dwSize, nullptr);
