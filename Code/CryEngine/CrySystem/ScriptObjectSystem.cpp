...
int CScriptObjectSystem::FrameProfiler(IFunctionHandler* pH) {
    ...
    m_pSystem->SetFrameProfiler(on, display, const_cast<char*>(prefix)); // todo (Izzotop): const_cast
    return pH->EndFunction();
};
...
int CScriptObjectSystem::LoadObject(IFunctionHandler* pH) {
    ...
    if (!szFileName || strlen(szFileName) == 0) {
        m_pSystem->Warning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, 0, 0, "Script method System:LoadObject(filename) called with Empty filename");
        return pH->EndFunctionnullptr();
    }
...
int CScriptObjectSystem::DeformTerrain(IFunctionHandler* pH) {
    ...
    if (nTid && (nCookie == USER_DATA_TEXTURE)) {
        ...
        // check if e_deformable_terrain for some reason is nullptr, maybe at the init time it wasnt already created.
...
int CScriptObjectSystem::SetScreenFxParamInt(IFunctionHandler* pH) {
    CHECK_PARAMETERS(3);
    const char *pszEffectName, *pszEffectParam;
    int iValue;
...
int CScriptObjectSystem::SetScissor(IFunctionHandler* pH) {
    ...
    m_pSystem->GetIRenderer()->SetScissor(m_pSystem->GetIRenderer()->ScaleCoordX((float)x), m_pSystem->GetIRenderer()->ScaleCoordY((float)y),
                                          m_pSystem->GetIRenderer()->ScaleCoordX((float)w), m_pSystem->GetIRenderer()->ScaleCoordY((float)h));
    ...
    for (i = 0; i < Formats.Num(); i++) {
        ...
        for (int j(0); j < pDispArray->Count(); ++j) {
            ...
            if (false != pDispArray->GetAt(j + 1, pDispCmp)) {
                ...
                if (pForm->m_Width == iWidthCmp && pForm->m_Height == iHeightCmp && pForm->m_BPP == iBppCmp) {
                    bInsert = false;
                    break;
                }
            }
...
int CScriptObjectSystem::DumpCommandsVars(IFunctionHandler* pH) {
    ...
    m_pSystem->GetIConsole()->DumpCommandsVars(const_cast<char*>(arg));
    return pH->EndFunction();
}
...
int CScriptObjectSystem::SetWaterVolumeOffset(IFunctionHandler* pH) {
    ...
    const char* pszWaterVolumeName = nullptr;
    ...
    IWaterVolume* pWaterVolume = pszWaterVolumeName ? m_p3DEngine->FindWaterVolumeByName(pszWaterVolumeName) : nullptr;
    if (!pWaterVolume) {
        m_pSystem->Warning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, 0, nullptr, "CScriptObjectSystem::SetWaterVolumeOffset: Water volume not found: %s",
                           pszWaterVolumeName ? pszWaterVolumeName : "Name is not set");
...
int CScriptObjectSystem::RayWorldIntersection(IFunctionHandler* pH) {
    ...
    int nHits = m_pPhysicalWorld->RayWorldIntersection(src, dst, iEntTypes, geom_colltype0 << rwi_colltype_bit | rwi_stop_at_pierceable, RayHit, nMaxHits, skipPhys1, skipPhys2);
...
int CScriptObjectSystem::RayTraceCheck(IFunctionHandler* pH) {
    ...
    int nHits =
        m_pPhysicalWorld->RayWorldIntersection(src, dst - src, ent_static | ent_terrain, rwi_ignore_noncolliding | rwi_stop_at_pierceable, &RayHit, 1, skipPhys1, skipPhys2);
...
int CScriptObjectSystem::GetGPUQuality(IFunctionHandler* pH) {
            ...
            if (SUCCEEDED(fpDirectDrawCreateEx(0, &pDD7, IID_IDirectDraw7, 0))) {
                ...
                if (SUCCEEDED(pDD7->GetDeviceIdentifier(&DeviceIdentifier, 0))) {
                    ...
                    case 0x1002: // ATI
                    {
                        ...
                        case 0x496e:
                            s_iGPUQuality = 0;
                            break; // Radeon 9000 - Secondary
                        case 0x514d:
                            s_iGPUQuality = 0;
                            break; // Radeon 9100
                        case 0x5834:
                            s_iGPUQuality = 0;
                            break; // Radeon 9100 IGP
                        case 0x4242:
                            s_iGPUQuality = 0;
                            break; // Radeon 8500 DV
                        case 0x4152:
                            s_iGPUQuality = 1;
                            break; // Radeon 9600
                        case 0x4172:
                            s_iGPUQuality = 1;
                            break; // Radeon 9600 - Secondary
                        case 0x4164:
                            s_iGPUQuality = 1;
                            break; // Radeon 9500 - Secondary
                        case 0x4144:
                            s_iGPUQuality = 2;
                            break; // Radeon 9500
                        case 0x4e45:
                            s_iGPUQuality = 2;
                            break; // Radeon 9500 Pro / 9700
                        case 0x4150:
                            s_iGPUQuality = 1;
                            break; // Radeon 9600 Pro
                        case 0x4151:
                            s_iGPUQuality = 1;
                            break; // Radeon 9600
                        case 0x4170:
                            s_iGPUQuality = 1;
                            break; // Radeon 9600 Pro - Secondary
                        case 0x4171:
                            s_iGPUQuality = 1;
                            break; // Radeon 9600 - Secondary
                        case 0x4e46:
                            s_iGPUQuality = 1;
                            break; // Radeon 9600 TX
                        case 0x4e66:
                            s_iGPUQuality = 1;
                            break; // Radeon 9600 TX - Secondary
                        case 0x4e44:
                            s_iGPUQuality = 3;
                            break; // Radeon 9700 Pro
                        case 0x4e64:
                            s_iGPUQuality = 3;
                            break; // Radeon 9700 Pro - Secondary
                        case 0x4e65:
                            s_iGPUQuality = 2;
                            break; // Radeon 9500 Pro / 9700 - Secondary
                        case 0x4e49:
                            s_iGPUQuality = 3;
                            break; // Radeon 9800
                        case 0x4e69:
                            s_iGPUQuality = 3;
                            break; // Radeon 9800 - Secondary
                        case 0x4148:
                            s_iGPUQuality = 3;
                            break; // Radeon 9800
                        case 0x4168:
                        ...
                        case 0x4e48:
                            s_iGPUQuality = 3;
                            break; // Radeon 9800 Pro
                        case 0x4e68:
                            s_iGPUQuality = 3;
                            break; // Radeon 9800 Pro - Secondary
                        case 0x4e4a:
                            s_iGPUQuality = 3;
                            break; // Radeon 9800 XT
                        case 0x4e6a:
                            s_iGPUQuality = 3;
                            break; // Radeon 9800 XT - Secondary
                        case 0x5960:
                            s_iGPUQuality = 1;
                            break; // Radeon 9200 Pro
                        case 0x5940:
                            s_iGPUQuality = 1;
                            break; // Radeon 9200 Pro - Secondary
                        case 0x5961:
                            s_iGPUQuality = 0;
                            break; // Radeon 9200
                        case 0x5941:
                            s_iGPUQuality = 1;
                            break; // Radeon 9200 - Secondary
                        case 0x5964:
                            s_iGPUQuality = 1;
                            break; // Radeon 9200SE
                        case 0x514c:
                        ...
                        case 0x4a49:
                            s_iGPUQuality = 3;
                            break; // Radeon X800 Pro
                        case 0x4a4a:
                            s_iGPUQuality = 3;
                            break; // Radeon X800 SE
                        case 0x4a4b:
                            s_iGPUQuality = 3;
                            break; // Radeon X800
                        case 0x4a4c:
                            s_iGPUQuality = 3;
                            break; // Radeon X800 Series
                        case 0x4a50:
                            s_iGPUQuality = 3;
                            break; // Radeon X800 XT
                        case 0x4a69:
                            s_iGPUQuality = 3;
                            break; // Radeon X800 Pro Secondary
                        case 0x4a6a:
                            s_iGPUQuality = 3;
                            break; // Radeon X800 SE Secondary
                        case 0x4a6b:
                            s_iGPUQuality = 3;
                            break; // Radeon X800 Secondary
                        case 0x4a6c:
                            s_iGPUQuality = 3;
                            break; // Radeon X800 Series Secondary
                        case 0x4a70:
                            s_iGPUQuality = 3;
                            break; // Radeon X800 XT Secondary
                        }
                    }
                    ...
                    case 0x10de: {
                        ...
                        case 0x0152:
                            s_iGPUQuality = 0;
                            break; // GeForce2 Ultra
                        case 0x0153:
                            s_iGPUQuality = 0;
                            break; // Quadro2 Pro
                        case 0x0170:
                            s_iGPUQuality = 0;
                            break; // GeForce4 MX 460
                        case 0x0171:
                            s_iGPUQuality = 0;
                            break; // GeForce4 MX 440
                        case 0x0172:
                            s_iGPUQuality = 0;
                            break; // GeForce4 MX 420
                        case 0x0173:
                            s_iGPUQuality = 0;
                            break; // GeForce4 MX 440-SE
                        case 0x0174:
                            s_iGPUQuality = 0;
                            break; // GeForce4 Go 440
                        case 0x0175:
                            s_iGPUQuality = 0;
                            break; // GeForce4 Go 420
                        case 0x0176:
                            s_iGPUQuality = 0;
                            break; // GeForce4 Go 420
                        case 0x0178:
                            s_iGPUQuality = 0;
                            break; // Quadro4 550 XGL
                        case 0x0179:
                            s_iGPUQuality = 0;
                            break; // GeForce4 Go 440
                        case 0x017a:
                        ...
                        case 0x017b:
                            s_iGPUQuality = 0;
                            break; // Quadro 550 XGL
                        case 0x0181:
                            s_iGPUQuality = 0;
                            break; // GeForce4 MX 440 with AGP8X
                        case 0x0182:
                            s_iGPUQuality = 0;
                            break; // GeForce4 MX 440SE with AGP8X
                        case 0x0183:
                            s_iGPUQuality = 0;
                            break; // GeForce4 MX 420 with AGP8X
                        case 0x0188:
                            s_iGPUQuality = 0;
                            break; // Quadro4 580 XGL
                        case 0x018a:
                            s_iGPUQuality = 0;
                            break; // Quadro NVS with AGP8X
                        case 0x018b:
                            s_iGPUQuality = 0;
                            break; // Quadro4 380 XGL
                        case 0x01f0:
                            s_iGPUQuality = 0;
                            break; // GeForce4 MX Integrated GPU (nForce2)
                        case 0x0200:
                            s_iGPUQuality = 0;
                            break; // GeForce3
                        case 0x0201:
                            s_iGPUQuality = 0;
                            break; // GeForce3 Ti200
                        case 0x0202:
                            s_iGPUQuality = 0;
                            break; // GeForce3 Ti500
                        case 0x0203:
                            s_iGPUQuality = 0;
                            break; // Quadro DCC
                        case 0x0250:
                            s_iGPUQuality = 1;
                            break; // GeForce4 Ti 4600
                        case 0x0251:
                            s_iGPUQuality = 1;
                            break; // GeForce4 Ti 4400
                        case 0x0253:
                            s_iGPUQuality = 0;
                            break; // GeForce4 Ti 4200
                        case 0x0258:
                            s_iGPUQuality = 0;
                            break; // Quadro4 900 XGL
                        case 0x0259:
                            s_iGPUQuality = 0;
                            break; // Quadro4 750 XGL
                        case 0x025b:
                            s_iGPUQuality = 0;
                            break; // Quadro4 700 XGL
                        case 0x0280:
                            s_iGPUQuality = 1;
                            break; // GeForce4 Ti 4800
                        case 0x0281:
                            s_iGPUQuality = 0;
                            break; // GeForce4 Ti4200 with AGP8X
                        case 0x0282:
                            s_iGPUQuality = 0;
                            break; // GeForce4 Ti 4800 SE
                        case 0x0288:
                            s_iGPUQuality = 0;
                            break; // Quadro4 980 XGL
                        case 0x0289:
                            s_iGPUQuality = 0;
                            break; // Quadro4 780 XGL
                        case 0x02a0:
                            s_iGPUQuality = 0;
                            break; // GeForce3 XBOX
                        case 0x0301:
                            s_iGPUQuality = 1;
                            break; // GeForce FX 5800 Ultra
                        case 0x0302:
                            s_iGPUQuality = 1;
                            break; // GeForce FX 5800
                        case 0x0308:
                            s_iGPUQuality = 1;
                            break; // Quadro FX 2000
                        case 0x0309:
                            s_iGPUQuality = 1;
                            break; // Quadro FX 1000
                        case 0x030a:
                            s_iGPUQuality = 1;
                            break; // ICE FX 2000
                        case 0x0311:
                            s_iGPUQuality = 1;
                            break; // GeForce FX 5600 Ultra
                        case 0x0312:
                            s_iGPUQuality = 1;
                            break; // GeForce FX 5600
                        case 0x0313:
                            s_iGPUQuality = 1;
                            break; // NV31
                        case 0x0314:
                            s_iGPUQuality = 1;
                            break; // GeForce FX 5600XT
                        case 0x031a:
                            s_iGPUQuality = 1;
                            break; // GeForce FX Go5600
                        case 0x0321:
                            s_iGPUQuality = 1;
                            break; // GeForce FX 5200 Ultra
                        case 0x0322:
                            s_iGPUQuality = 1;
                            break; // GeForce FX 5200
                        case 0x0323:
                            s_iGPUQuality = 1;
                            break; // GeForce FX 5200SE
                        case 0x032b:
                            s_iGPUQuality = 1;
                            break; // Quadro FX 500
                        case 0x032f:
                        ...
                        case 0x0330:
                            s_iGPUQuality = 3;
                            break; // GeForce FX 5900 Ultra
                        case 0x0331:
                            s_iGPUQuality = 2;
                            break; // GeForce FX 5900
                        case 0x0332:
                            s_iGPUQuality = 2;
                            break; // NV35
                        case 0x0333:
                            s_iGPUQuality = 3;
                            break; // GeForce FX 5950 Ultra
                        case 0x0338:
                            s_iGPUQuality = 2;
                            break; // Quadro FX 3000
                        case 0x0341:
                            s_iGPUQuality = 2;
                            break; // GeForce FX 5700 Ultra
                        case 0x0342:
                            s_iGPUQuality = 2;
                            break; // GeForce FX 5700
                        case 0x034e:
                            s_iGPUQuality = 1;
                            break;   // Quadro FX 1100
                        ...
                        case 0x00f9:
                            s_iGPUQuality = 3;
                            break; // GeForce 6800 Ultra
                        case 0x00fa:
                            s_iGPUQuality = 2;
                            break; // GeForce PCX 5750
                        case 0x00fb:
                            s_iGPUQuality = 2;
                            break; // GeForce PCX 5900
                        case 0x00fc:
                            s_iGPUQuality = 1;
                            break; // GeForce PCX 5300
                        case 0x00fd:
                            s_iGPUQuality = 2;
                            break; // Quadro PCI-E Series
                        case 0x00fe:
                            s_iGPUQuality = 2;
                            break; // Quadro FX 1300
                        case 0x00ff:
                            s_iGPUQuality = 1;
                            break; // GeForce PCX 4300
                        }
                    }
                    }
                }
    ...
    if (s_iGPUQuality == 666) {
        if (m_pRenderer->GetFeatures() & RFT_HW_PS20) {
            s_iGPUQuality = 2;
        } else if (m_pRenderer->GetFeatures() & RFT_HW_TS) {
            s_iGPUQuality = 1;
        } else {
            s_iGPUQuality = 0;
        }
...
int CScriptObjectSystem::SetSystemShaderRenderFlags(IFunctionHandler* pH) {
    ...
    static flagsDesc arrFlags[] = {{DLD_TERRAIN_WATER, "DrawWater"},
