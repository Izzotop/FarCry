...
CLUADbg::CLUADbg() {
    ...
    m_pIPak = nullptr;
    m_pIVariable = nullptr;
    m_hRoot = nullptr;
    m_bDisabled = false;
    m_pTreeToAdd = nullptr;
}
...
LRESULT CLUADbg::OnCreate(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    m_wndStatus.Create(0, nullptr, this);

    // Client area window
    _TinyVerify(m_wndClient.Create(nullptr, _T(""), WS_CHILD | WS_VISIBLE, 0, &wrect, this));
    m_wndClient.NotifyReflection(TRUE);

    // Splitter dividing source/file and watch windows
    m_wndMainHorzSplitter.Create(&m_wndClient, nullptr, nullptr, true);
    ...
    _TinyVerify(m_wScriptWindow.Create(WS_VISIBLE | WS_CHILD | ES_WANTRETURN | WS_HSCROLL | WS_VSCROLL | ES_AUTOHSCROLL | ES_AUTOVSCROLL | ES_MULTILINE, 0, &erect, &m_wndSrcEditSplitter));
...
bool CLUADbg::LoadFile(const char* pszFile, bool bForceReload) {
    FILE* hFile = nullptr;
    char *pszScript = nullptr, *pszFormattedScript = nullptr;
    ...
    if (strncmp(szMasterCD, pszFile, strlen(szMasterCD) - 1) == 0) {
        ...
        std::transform(strLuaFormatFileName.begin(), strLuaFormatFileName.end(), strLuaFormatFileName.begin(), (int (*)(int))tolower);
    } else
    ...
    const char szKeywords[][32] = {
        "function", "do", "for", "end", "and", "or", "not", "while", "return", "if", "then", "else", "elseif", "self", "local", "in", "nil", "repeat", "until", "break",
    };
    ...
    while (pszScript[iSrcBufPos] != '\0') {
        ...
        while (pszScript[iSrcBufPos] != ' ' && pszScript[iSrcBufPos] != '\n' && pszScript[iSrcBufPos] != '\r' && pszScript[iSrcBufPos] != '\t' && pszScript[iSrcBufPos] != '\0' &&
               pszScript[iSrcBufPos] != '(' && pszScript[iSrcBufPos] != ')' && pszScript[iSrcBufPos] != '[' && pszScript[iSrcBufPos] != ']' && pszScript[iSrcBufPos] != '{' &&
               pszScript[iSrcBufPos] != '}' && pszScript[iSrcBufPos] != ',' && pszScript[iSrcBufPos] != '.' && pszScript[iSrcBufPos] != ';' && pszScript[iSrcBufPos] != ':' &&
               pszScript[iSrcBufPos] != '=' && pszScript[iSrcBufPos] != '==' && pszScript[iSrcBufPos] != '*' && pszScript[iSrcBufPos] != '+' && pszScript[iSrcBufPos] != '/' &&
               pszScript[iSrcBufPos] != '~' && pszScript[iSrcBufPos] != '"') {
            iSrcBufPos++;
            iNumChars++;
...
bool CLUADbg::Reshape(int w, int h) {
    ...
    */
    ...
    m_wndStatus.SetWindowPos(0, h - STATUS_BAR_HEIGHT, w, STATUS_BAR_HEIGHT, 0);
...
HTREEITEM CLUADbg::AddVariableToTree(const char* sName, ScriptVarType type, HTREEITEM hParent) {
    ...
    if (m_pTreeToAdd == nullptr) {
        _TinyAssert(m_pTreeToAdd != nullptr);
        return nullptr;
    }
    ...
    HTREEITEM hNewItem = m_pTreeToAdd->AddItemToTree(szBuf, 0, hParent);
...
void CLUADbg::OnElementFound(const char* sName, ScriptVarType type) {
    ...
    IScriptObject* pIOldTbl = nullptr;
    HTREEITEM hOldRoot = nullptr;
