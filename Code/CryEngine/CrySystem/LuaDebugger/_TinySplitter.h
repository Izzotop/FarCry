...
public:
    virtual BOOL Create(_TinyWindow* pParentWnd = nullptr, _TinyWindow* pPan0 = nullptr, _TinyWindow* pPan1 = nullptr, bool bVertical = false, const RECT* pRect = nullptr) {
        BOOL bRes = _TinyWindow::Create(_T("_default_TinyWindowClass"), _T(""), WS_VISIBLE | WS_CHILD | WS_CLIPCHILDREN, 0, pRect, pParentWnd);
        if (!bRes)
            return FALSE;
    ...
    bool Reshape(int w, int h) {
        if (m_pPan0 == nullptr || m_pPan1 == nullptr)
            return false;
    ...
    UINT GetSplitterPos() const {
        return m_iSplitterPos;
    };
    void SetSplitterPos(UINT iPos) {
        m_iSplitterPos = iPos;
    };
...
protected:
    ...
    LRESULT OnMouseMove(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
        ...
        HCURSOR hCur = LoadCursor(nullptr, m_bVertical ? IDC_SIZENS : IDC_SIZEWE);
        _TinyAssert(hCur != nullptr);
        SetCursor(hCur);
