...
public:
    void SetLineMarker(UINT iLine) {
        m_iLineMarkerPos = iLine;
    };
    ...
    BOOL Create(DWORD dwStyle = WS_VISIBLE, DWORD dwExStyle = 0, const RECT* pRect = nullptr, _TinyWindow* pParentWnd = nullptr, ULONG nID = 0) {
        _TinyRect erect;
        erect.left = BORDER_SIZE;
        erect.right = pRect->right;
        erect.top = 0;
        erect.bottom = pRect->bottom;
        if (!_TinyWindow::Create(nullptr, _T("asd"), WS_CHILD | WS_VISIBLE, 0, pRect, pParentWnd, nID))
            return FALSE;
        if (!m_wEditWindow.Create(IDC_SOURCE, WS_VISIBLE | WS_CHILD | ES_WANTRETURN | WS_HSCROLL | WS_VSCROLL | ES_AUTOHSCROLL | ES_AUTOVSCROLL | ES_MULTILINE | ES_NOHIDESEL,
                                  WS_EX_CLIENTEDGE, &erect, this))
            return FALSE;
    ...
    const char* GetSourceFile() {
        return m_strSourceFile.c_str();
    };

    void AddBreakpoint() {
        // Adds a breakpoint for the current file in the current line
        _TinyAssert(m_pIScriptSystem);
        HBREAKPOINT hBreakPt = m_pIScriptSystem->AddBreakPoint(m_strSourceFile.c_str(), m_wEditWindow.LineFromChar(m_wEditWindow.GetSel()));
        _TinyVerify(::InvalidateRect(m_wEditWindow.m_hWnd, nullptr, FALSE));
    }
...
protected:
    ...
    LRESULT OnDebugRun(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
        if (m_pIScriptSystem->GetBreakState() == bsStepNext || m_pIScriptSystem->GetBreakState() == bsStepInto) {
    ...
    HTREEITEM AddVariableToTree(const char* sName, ScriptVarType type, HTREEITEM hParent = nullptr);
