...
__inline void _TinyCheckLastError(const char* pszFile, int iLine) {
    if (GetLastError() != ERROR_SUCCESS) {
        ...
        FormatMessage(FORMAT_MESSAGE_ARGUMENT_ARRAY | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, GetLastError(),
                      MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), // Default language
                      szMessageBuf, 2048, nullptr);
        sprintf(szLineFileInfo, "Error catched in file %s line %i", pszFile, iLine);
        strcat(szMessageBuf, szLineFileInfo);

#ifdef _DEBUG
        MessageBox(nullptr, szMessageBuf, "Tiny Framework Error", MB_OK | MB_ICONERROR);
#else
...
/*
 if (!TranslateAccelerator(
        ...
        {
 */
inline int _Tiny_MainLoop(HACCEL hAccelTable = nullptr, HWND hAccelTarget = nullptr) {
    MSG msg;

    //_TinyAssert((hAccelTable == nullptr && hAccelTarget == nullptr) ||
    //  (hAccelTable != nullptr && hAccelTarget != nullptr));

    while (GetMessage(&msg, nullptr, 0, 0)) {
        if (hAccelTable == nullptr || TranslateAccelerator(hAccelTarget, hAccelTable, &msg) == 0) {
