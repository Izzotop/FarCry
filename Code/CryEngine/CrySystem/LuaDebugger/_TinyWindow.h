...
public:
    ...
    virtual BOOL Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle = WS_VISIBLE, DWORD dwExStyle = 0, const RECT* pRect = nullptr, _TinyWindow* pParentWnd = nullptr,
                        ULONG_PTR nID = 0) {
        ...
        // create
        m_hWnd = ::CreateWindowEx(dwExStyle, lpszClassName, lpszWindowName, dwStyle, x, y, width, height, hParent, (HMENU)nID, _Tiny_GetInstance(), nullptr);
        if (!m_hWnd) {
...
public:
    ...
    virtual LRESULT SetTimer(UINT nIDEvent, UINT uElapse) {
        return (LRESULT)::SetTimer(m_hWnd, nIDEvent, uElapse, nullptr);
    }
    ...
    virtual LRESULT InvalidateRect(_TinyRect* pRect = nullptr, BOOL bErase = FALSE) {
    ...
    virtual HWND SetParent(_TinyWindow* pParent) {
        if (pParent)
            ...
            return ::SetParent(m_hWnd, nullptr);
    }
...
public:
    ...
    int DoModal(LPCTSTR lpTemplate, _TinyWindow* pParent = nullptr) {
        INT_PTR nRet = DialogBoxParam(_Tiny_GetResourceInstance(), lpTemplate, (pParent != nullptr) ? pParent->m_hWnd : nullptr, (DLGPROC)_TinyDlgProc, (LPARAM)this);
        if (nRet == -1) {
            _TINY_CHECK_LAST_ERROR
            return 0;
        }
...
public:
    virtual BOOL Create(ULONG nID = 0, DWORD dwStyle = WS_VISIBLE, DWORD dwExStyle = 0, const RECT* pRect = nullptr, _TinyWindow* pParentWnd = nullptr) {
    ...
    virtual int GetSel() {
        return SendMessage(EM_GETSEL, 0, 0L);
    }
...
public:
    virtual BOOL Create(ULONG nMenuID = 0, DWORD dwStyle = WS_CHILD | WS_VISIBLE | TBSTYLE_TOOLTIPS, DWORD dwStyleEx = 0, const RECT* pRect = nullptr,
                        _TinyWindow* pParentWnd = nullptr) {
...
public:
    ...
    virtual void SetViewStyle(DWORD dwView) {
        ...
        if ((dwStyle & LVS_TYPEMASK) != dwView)
            SetWindowLong(m_hWnd, GWL_STYLE, (dwStyle & ~LVS_TYPEMASK) | dwView);
    }
    ...
    virtual int InsertItem(int nCol, LPCTSTR pszText, int iSubItem = 0, int nParam = 0) {
        LVITEM lvi;
...
public:
    virtual BOOL Create(ULONG nID = 0, DWORD dwStyle = WS_VISIBLE | WS_CHILD | WS_BORDER | HDS_BUTTONS | HDS_HORZ, DWORD dwStyleEx = 0, const RECT* pRect = nullptr,
                        _TinyWindow* pParentWnd = nullptr) {
    ...
    virtual int InsertItem(int iInsertAfter, int nWidth, LPSTR lpsz) {
        ...
        index = SendMessage(HDM_INSERTITEM, (WPARAM)iInsertAfter, (LPARAM)&hdi);
...
public:
    virtual BOOL Create(ULONG nID = 0, DWORD dwStyle = WS_CHILD | TVS_HASLINES | TVS_HASBUTTONS | WS_VISIBLE, DWORD dwStyleEx = 0, const RECT* pRect = nullptr,
                        _TinyWindow* pParentWnd = nullptr) {
    ...
    virtual HTREEITEM AddItemToTree(LPTSTR lpszItem, LPARAM ud = 0, HTREEITEM hParent = nullptr, UINT iImage = 0) {
        TVITEM tv;
