...
float CTimer::MeasureTime(LPCSTR szComment) {
    ...
    if (szComment && m_nCurProfLine >= 0 && m_nCurProfLine < TIME_PROFILE_PARAMS && strlen(szComment) > 0) {
        ...
        m_nCurProfLine++;
    } else if (m_e_time_profiling) { // detail stats
        ...
        for (int i = 0; i < m_nCurProfLine; i++) {
            ...
            if (m_TimeInfoTable[i].param_name[0]) {
                ...
                m_pSystem->GetIRenderer()->TextToScreenColor((int)(fOffs - (nCollumns - nColumn / nLinesInCollumn) * 20), (int)(25 + ((((nLine++) % nLinesInCollumn) * 2))),
                                                             (m_TimeInfoTable[i].param_name[0] == '3') * fColorK, fColorK, fColorK, 1, "%12s %.02f",
                                                             m_TimeInfoTable[i].param_name + (m_TimeInfoTable[i].param_name[0] == '3'), m_TimeInfoTable[i].value);
        ...
        m_pSystem->GetIRenderer()->TextToScreen(fOffs - (nCollumns - nColumn / nLinesInCollumn) * 16, (float)(25 + ((((nLine++) % nLinesInCollumn) * 2))), "%12s %.02f", "* Summ *",
                                                fSumm);
