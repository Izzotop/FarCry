...
class FindDataRW {
public:
    FindDataRW(FileEntryTree* pRoot) : m_pRoot(pRoot), m_pDirHeader(nullptr) {}
...
public:
    FindFileRW(FileEntryTree* pRoot) : FindDataRW(pRoot) {}
...
public:
    FindDirRW(FileEntryTree* pRoot) : FindDataRW(pRoot) {}
    // if bExactFile is passed, only the file is searched, and besides with the exact name as passed (no wildcards)
    bool FindFirst(const char* szWildcard);
