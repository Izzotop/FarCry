...
CRefReadStream::CRefReadStream(const string& strFileName, CRefStreamEngine* pEngine)
    : m_pEngine(pEngine), m_bError(false), // no error yet because we didn't try to do anything
      m_strFileName(strFileName), m_nFileSize(0), m_nSectorSize(0), m_hFile(INVALID_HANDLE_VALUE), m_bOverlapped(false), m_pZipEntry(nullptr) {
...
bool CRefReadStream::Activate() {
    ...
    if (m_pZipEntry == 0 && m_hFile == INVALID_HANDLE_VALUE)
#endif
        m_hFile = CreateFile(m_strFileName.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, m_bOverlapped ? FILE_FLAG_OVERLAPPED : 0, nullptr);
#if !defined(LINUX64)
    if (m_pZipEntry == 0 && m_hFile == INVALID_HANDLE_VALUE)
#else
    if (m_pZipEntry == 0 && m_hFile == INVALID_HANDLE_VALUE)
#endif
    {
        ...
        if (m_pZipEntry) {
            ...
            if (!m_pZipEntry->GetData(false) && m_pZipEntry->GetFileEntry()->nMethod == ZipFile::METHOD_STORE
                ...
                m_hFile = CreateFile(szPakFile, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, m_bOverlapped ? FILE_FLAG_OVERLAPPED : 0, nullptr);
            }
