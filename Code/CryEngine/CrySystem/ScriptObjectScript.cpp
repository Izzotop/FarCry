...
void CScriptObjectScript::Debug_Full_recursive(IScriptObject* pCurrent, string& sPath, std::set<const void*>& setVisited) {
    ...
    while (pCurrent->MoveNext()) {
        ...
        if (type == svtObject) // svtnullptr,svtFunction,svtString,svtNumber,svtUserData,svtObject
        {
            const void* pVis;
        ...
        } else if (type == svtFunction) // svtnullptr,svtFunction,svtString,svtNumber,svtUserData,svtObject
        {
        ...
        } else // svtnullptr,svtFunction,svtString,svtNumber,svtUserData,svtObject
        {
...
DWORD CScriptObjectScript::Debug_Buckets_recursive(IScriptObject* pCurrent, string& sPath, std::set<const void*>& setVisited, const DWORD dwMinBucket) {
    assert(pCurrent);
        ...
        if (type == svtObject) // svtnullptr,svtFunction,svtString,svtNumber,svtUserData,svtObject
        {
...

            {
                ...
                if (dwSubTableCount >= dwMinBucket) {
                    ...
                    dwTableElementCount += dwSubTableCount;
            }
        }
        /*    else if(type==svtFunction)    // svtnullptr,svtFunction,svtString,svtNumber,svtUserData,svtObject
                        {
