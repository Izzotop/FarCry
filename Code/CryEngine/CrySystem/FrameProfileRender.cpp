...
void CFrameProfileSystem::RenderMemoryInfo() {
    ...
    float col3 = col2 + 7;
    float col4 = col3 + 7;
    DrawLabel(col, row++, HeaderColor, 0, "------------------------------------------------------------------------------------------------------------------------------------",
              fLabelScale);
    ...
    if (hSnapshot != INVALID_HANDLE_VALUE) {
        ...
        if (Module32First(hSnapshot, &me)) {
            ...
            do {
                ...
                if (m_bLogMemoryInfo)
                    pLog->Log("   %6d Kb (Static: %6d KB) (Num: %6d) (TotalAlloc: %8I64d KB) %s", usedInModule / 1024, staticModuleSize / 1024, memInfo.num_allocations,
                              memInfo.allocated, me.szModule);
                row++;
