...
public:
    ...
    void Register(CRefReadStreamProxy* pProxy) {
        this->AddRef();
        m_setProxies.insert(pProxy);
    }
    // the proxy deallocates, don't take it into account (decrease ref counter)
    void Unregister(CRefReadStreamProxy* pProxy) {
        m_setProxies.erase(pProxy);
        this->Release();
    }
...
private:
    ...
    // if this is not nullptr, the file actually resides in a virtual file system inside a zip file.
    // m_hFile may not or may be nullptr. if m_hFile != nullptr and this is not nullptr, m_hFile is the handle
