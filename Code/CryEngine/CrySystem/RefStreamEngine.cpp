...
CRefStreamEngine::CRefStreamEngine(CCryPak* pPak, IMiniLog* pLog, unsigned useWorkerThreads, bool bOverlappedIO)
    : m_pPak(pPak), m_pLog(pLog), m_nMaxReadDepth(16), m_nMaxQueueLength(4 * 1024), m_nMaxIOMemPool(128 * 1024 * 1024),
#if defined(LINUX)
      m_hIOWorker(INVALID_HANDLE_VALUE), // only diff is here, but what can i do?
#else
      m_hIOWorker(nullptr),
#endif
      m_dwWorkerThreadId(0), m_queIOJobs(ProxyPtrAllocator(g_pSmallHeap)), m_setIOPending(ProxyPtrPredicate(), ProxyPtrAllocator(g_pSmallHeap)),
      m_queIOExecuted(ProxyPtrAllocator(g_pSmallHeap)), m_bEnableOverlapped(bOverlappedIO), m_nSuspendCallbackTimeQuota(0) {
...
IReadStream_AutoPtr CRefStreamEngine::StartRead(const char* szSource, const char* szFilePathPC, IStreamCallback* pCallback, StreamReadParams* pParams) {
    ...
    else if (nFlags & SRP_FLAGS_MAKE_TRANSIENT)
...
void CRefStreamEngine::AddIOJob(CRefReadStreamProxy* pJobProxy) {
    if (!m_hIOWorker) { // its very simple with single-threaded model: we just put the job to the queue
...
bool CRefStreamEngine::IsCallbackTimeQuota(unsigned nFlags) {
    if (m_nSuspendCallbackTimeQuota == 0 && !(nFlags & FLAGS_DISABLE_CALLBACK_TIME_QUOTA) && m_nCallbackTimeQuota <= 0)
        return false;
    return true;
}
...
unsigned CRefStreamEngine::StartIOJobs() {
    ...
    AUTO_LOCK(m_csIOJobs);
    {
        ...
        while (!m_queIOJobs.empty() && m_setIOPending.size() < m_nMaxReadDepth && !IsSuspended()) {
...
unsigned CRefStreamEngine::GetDriveSectorSize(char cDrive) {
    ...
    char szBuf[4] = "X:\\";
    szBuf[0] = cDrive;
    // determine the pointer to the cached value of the sector size (nullptr if there's no such)
