...
void CScriptObjectEntity::SetEntity(IEntity* pEntity) {
    ...
    if (m_pEntity != nullptr) {
        ...
        assert(m_pScriptThis);
        m_pScriptThis->SetTonullptr("id");
        assert(m_pScriptThis);
        m_pScriptThis->SetTonullptr("classid");
        assert(m_pScriptThis);
        m_pScriptThis->SetTonullptr("classname");
        assert(m_pScriptThis);
    }
    assert(m_pScriptThis);
}
...
void CScriptObjectEntity::SetContainer(IScriptObject* pContainer) {
    ...
    if (pContainer != nullptr)
        ...
        m_pScriptThis->SetTonullptr("cnt");
...
int CScriptObjectEntity::StartAnimation(IFunctionHandler* pH) {
    ...
    if (!pH->GetParam(2, animname)) {
        m_pISystem->Warning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, 0, 0, "CScriptObjectEntity::StartAnimation, animation name not specified, in Entity %s",
                            m_pEntity->GetName());
...
int CScriptObjectEntity::SetEntityPhysicParams(IPhysicalEntity* pe, IFunctionHandler* pH, int iOffs, ICryCharInstance* pIChar) {
    ...
    case PHYSICPARAM_SIMULATION:
        ...
        gravity.Set(0, 0, 0);
        if (pTable->GetValue("freefall_gravityx", gravity.x) | pTable->GetValue("freefall_gravityy", gravity.y) | pTable->GetValue("freefall_gravityz", gravity.z))
    ...
    case PHYSICPARAM_ARTICULATED:
        ...
        if (pTable->GetValue("lying_gravityx", gravity.x) | pTable->GetValue("lying_gravityy", gravity.y) | pTable->GetValue("lying_gravityz", gravity.z))
    ...
    if (bUseOnArms) {
        if (pEntChar && pEntChar->GetCharacter(1)) {
            ...
            if (!bOnArmsAttached) {
                ...
                pEntChar->GetCharacter(1)->SetShaderTemplateName("Templnullptr", 1, 0, m_pEntity->GetMaterial(), 0);
...
int CScriptObjectEntity::GotoState(IFunctionHandler* pH) {
    ...
    if (sState) {
        if (!m_pEntity->IsStateClientside() && !GetISystem()->GetIGame()->GetModuleState(EGameServer)) {
            ...
            CryLog("\001ERROR: ScriptObjectEntity:GotoState on the client! (EntityClass:'%s' Name:'%s', State:'%s')", m_pEntity->GetEntityClassName(), m_pEntity->GetName(),
                   sState);
        }
...
int CScriptObjectEntity::DoRandomExpressions(IFunctionHandler* pH) {
    ...
    if (m_pEntity && m_pEntity->GetCharInterface()) {
        ...
        if (!pLipSync) {
            m_pISystem->Warning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, 0, 0, "Could not create lip-sync interface ! Does this entity %s have a character ?",
                                m_pEntity->GetName());
            return pH->EndFunctionnullptr();
        }
        if (!pLipSync->LoadRandomExpressions(pszFilename, bRaiseError)) {
            m_pISystem->Warning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, 0, 0, "CLipSync::LoadDialog failed for Entity %s", m_pEntity->GetName());
            return pH->EndFunctionnullptr();
        }
    }
...
int CScriptObjectEntity::DoExpression(IFunctionHandler* pH) {
    ...
    if (m_pEntity && m_pEntity->GetCharInterface()) {
        ...
        if (!pLipSync->DoExpression(pszMorphTarget, MorphParams)) {
            m_pISystem->Warning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, 0, 0, "CLipSync::DoExpression failed, Entity: %s", m_pEntity->GetName());
            return pH->EndFunctionnullptr();
        }
    }
    return pH->EndFunction((int)1);
                        ...
                        if ((iCollTypes == -1 ? pp[0].flagsColliderOR : iCollTypes) & pp[1].flagsOR) {
                            ...
                            for (nCont = pp[0].pPhysGeomProxy->pGeom->Intersect(pp[1].pPhysGeomProxy->pGeom, gwd + 0, gwd + 1, &ip, pContacts) - 1;
                                ...
                                if (psoEnt)
                                    ...
                                    psoContacts[nTotCont]->SetTonullptrChain("collider");
...
int CScriptObjectEntity::NoExplosionCollision(IFunctionHandler* pH) {
    ...
    if (m_pEntity->GetPhysics()) {
        ...
        do {
            ++ppart.ipart;
        } while (m_pEntity->GetPhysics()->SetParams(&ppart));
    }
    return pH->EndFunction();
}
