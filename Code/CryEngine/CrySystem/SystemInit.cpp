...
//////////////////////////////////////////////////////////////////////////
    ...
    #define DLL_NULLRENDERER "xrendernullptr.so"
#else
    ...
    #define DLL_NULLRENDERER "XRendernullptr.dll"
#endif
...
#ifdef WIN32
    if (m_pRenderer) {
        m_hWnd = m_pRenderer->Init(0, 0, m_rWidth->GetIVal(), m_rHeight->GetIVal(), m_rColorBits->GetIVal(), m_rDepthBits->GetIVal(), m_rStencilBits->GetIVal(),
                                   m_rFullscreen->GetIVal() ? true : false, hinst, hwnd);
        if (m_hWnd)
            return true;
        return false;
    }
#else
    if (m_pRenderer) {
        WIN_HWND h = m_pRenderer->Init(0, 0, m_rWidth->GetIVal(), m_rHeight->GetIVal(), m_rColorBits->GetIVal(), m_rDepthBits->GetIVal(), m_rStencilBits->GetIVal(),
                                       m_rFullscreen->GetIVal() ? true : false, hinst, hwnd);
        if (h)
            return true;
        return false;
...
bool CSystem::InitPhysics() {
    ...
    pConsole->Register("p_collision_mode", &pVars->iCollisionMode, (float)pVars->iCollisionMode, VF_CHEAT, "This variable is obsolete.\n");
    ...
    pConsole->Register("p_prohibit_unprojection", &pVars->bProhibitUnprojection, (float)pVars->bProhibitUnprojection, 0, "This variable is obsolete.");
    pConsole->Register("p_enforce_contacts", &pVars->bEnforceContacts, (float)pVars->bEnforceContacts, 0, "This variable is obsolete.");
    ...
    pConsole->Register("p_profile_entities", &pVars->bProfileEntities, (float)pVars->bProfileEntities, 0, "Enables per-entity time step profiling");
    pConsole->Register("p_GEB_max_cells", &pVars->nGEBMaxCells, (float)pVars->nGEBMaxCells, 0, "Specifies the cell number threshold after which GetEntitiesInBox issues a warning");
    pConsole->Register("p_max_velocity", &pVars->maxVel, pVars->maxVel, 0, "Clamps physicalized objects' velocities to this value");
    pConsole->Register("p_max_player_velocity", &pVars->maxVelPlayers, pVars->maxVelPlayers, 0, "Clamps players' velocities to this value");

    pConsole->Register("p_max_MC_iters", &pVars->nMaxMCiters, (float)pVars->nMaxMCiters, VF_REQUIRE_NET_SYNC, "Specifies the maximum number of microcontact solver iterations");
    pConsole->Register("p_accuracy_MC", &pVars->accuracyMC, pVars->accuracyMC, 0, "Desired accuracy of microcontact solver (velocity-related, m/s)");
    pConsole->Register("p_accuracy_LCPCG", &pVars->accuracyLCPCG, pVars->accuracyLCPCG, 0, "Desired accuracy of LCP CG solver (velocity-related, m/s)");
    ...
    pConsole->Register("p_max_LCPCG_iters", &pVars->nMaxLCPCGiters, (float)pVars->nMaxLCPCGiters, VF_REQUIRE_NET_SYNC, "Maximum number of LCP CG iterations");
    ...
    pConsole->Register("p_max_unproj_vel", &pVars->maxUnprojVel, pVars->maxUnprojVel, 0, "Limits the maximum unprojection velocity request");
    pConsole->Register("p_penalty_scale", &pVars->penaltyScale, pVars->penaltyScale, 0, "Scales the penalty impulse for objects that use the simple solver");
    ...
    pConsole->Register("p_max_world_step", &pVars->maxWorldStep, pVars->maxWorldStep, 0, "Specifies the maximum step physical world can make (larger steps will be truncated)");
...
public:
    CCommandLineSink_EarlyCommands(CSystem& rSystem) : m_rSystem(rSystem), m_bRelaunching(false), m_bDevMode(false) {}
...
void CSystem::CreateSystemVars() {
    ...
    m_cvPakLogMissingFiles = attachVariable(
        "sys_PakLogMissingFiles", &m_PakVar.nLogMissingFiles,
    ...
    m_sys_profile = GetIConsole()->CreateVariable("profile", "0", 0,
        "Enable profiling.\n"
        "Usage: profile #\n"
    ...
    m_sys_profile_graph = GetIConsole()->CreateVariable("profile_graph", "0", 0, "Enable drawing of profiling graph.\n");
    ...
    m_sys_profile_pagefaultsgraph = GetIConsole()->CreateVariable("profile_pagefaults", "0", 0, "Enable drawing of page faults graph.\n");
    m_sys_profile_network = GetIConsole()->CreateVariable("profile_network", "0", 0, "Enables network profiling.\n");
    m_sys_profile_peak = GetIConsole()->CreateVariable("profile_peak", "10", 0, "Profiler Peaks Tollerance in Milliseconds.\n");
    m_sys_profile_memory = GetIConsole()->CreateVariable("MemInfo", "0", 0, "Display memory information by modules");

    m_sys_skiponlowspec = GetIConsole()->CreateVariable("sys_skiponlowspec", "0", VF_DUMPTODISK | VF_SAVEGAME, "avoids loading of expendable entites.\n");

    m_sys_spec = GetIConsole()->CreateVariable("sys_spec", "1", VF_DUMPTODISK | VF_SAVEGAME, "Tells the system cfg spec.\n");

    m_sys_firstlaunch = GetIConsole()->CreateVariable("sys_firstlaunch", "0", VF_DUMPTODISK, "Indicates that the game was run for the first time.\n");
}
...
void CSystem::InitScriptDebugger() {
    ...
    if (sys_script_debugger->GetIVal() != 0) {
        ...
        _TinyVerify(
            m_pLuaDebugger->Create(nullptr, _T("Lua Debugger"), WS_OVERLAPPEDWINDOW, 0, nullptr, nullptr, (ULONG_PTR)LoadMenu(_Tiny_GetResourceInstance(), MAKEINTRESOURCE(IDC_LUADBG))));
