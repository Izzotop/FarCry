...
ZipDir::CacheFactory::CacheFactory(CMTSafeHeap* pHeap, InitMethodEnum nInitMethod, unsigned nFlags) : m_pHeap(pHeap) {
    m_nCDREndPos = 0;
    m_f = nullptr;
...
ZipDir::CacheRWPtr ZipDir::CacheFactory::NewRW(const char* szFileName) {
    ...
    if (m_nFlags & FLAGS_READ_ONLY) {
        ...
        if (m_f)
            Read(*pCache);
        else if ((m_f = fopen(szFileName, "w+b")) != nullptr) {
...
void ZipDir::CacheFactory::Prepare() {
    ...
    // we don't support multi volume archives
    if (m_CDREnd.nDisk != 0 || m_CDREnd.nCDRStartDisk != 0 || m_CDREnd.numEntriesOnDisk != m_CDREnd.numEntriesTotal) {
    ...
    if (m_CDREnd.lCDROffset > m_nCDREndPos || m_CDREnd.lCDRSize > m_nCDREndPos || m_CDREnd.lCDROffset + m_CDREnd.lCDRSize > m_nCDREndPos) {
    ...
    if (m_bBuildFileEntryMap && m_CDREnd.numEntriesTotal != m_mapFileEntries.size()) {
        THROW_ZIPDIR_ERROR(
            ...
            "The number of parsed files does not match the declared number of entries in the central directory, the pak is probably corrupt, try to repare or delete the file"
        );
    }
    ...
    if (m_bBuildFileEntryTree && m_CDREnd.numEntriesTotal != numFilesFound)
        THROW_ZIPDIR_ERROR(
            ZD_ERROR_CDR_IS_CORRUPT,
            "The number of parsed files does not match the declared number of entries in the central directory."
            " The pak does not appear to be corrupt, but perhaps there are some duplicated or missing file entries, try to repare the file"
        );
}
...
void ZipDir::CacheFactory::FindCDREnd() {
    ...
    if (nFileSize < sizeof(ZipFile::CDREnd)) {
        THROW_ZIPDIR_ERROR(
            ZD_ERROR_NO_CDR,
            "The file is too small, it doesn't even contain the CDREnd structure. Please check and delete the file. Truncated files are not deleted automatically"
        );
    }
    ...
    while (true) {
        ...
        if (nNewBufPos >= nOldBufPos) {
            ...
            THROW_ZIPDIR_ERROR(
                ZD_ERROR_NO_CDR,
                "Cannot find Central Directory Record in pak. This is either not a pak file, or a pak file without Central Directory."
                " It does not mean that the data is permanently lost, but it may be severely damaged."
                " Please repair the file with external tools, there may be enough information left to recover the file completely"
            );
        }
            ...
            if (pEnd->lSignature == pEnd->SIGNATURE) {
                if (pEnd->nCommentLength == nFileSize - nScanPos - sizeof(CDREnd)) {
                    ...
                    THROW_ZIPDIR_ERROR(
                        ...
                        "Central Directory Record is followed by a comment of inconsistent length. This might be a minor misconsistency, please try to repair the file."
                        " However, it is dangerous to open the file because I will have to guess some structure offsets,"
...
void ZipDir::CacheFactory::BuildFileEntryMap() {
    ...
    while ((pFileName = (const char*)(pFile + 1)) <= pEndOfData && pFile->lSignature == pFile->SIGNATURE) {
...
void ZipDir::CacheFactory::AddFileEntry(const string& strFilePath, const ZipFile::CDRFileHeader* pFileHeader) {
    if (pFileHeader->lLocalHeaderOffset > m_CDREnd.lCDROffset)
        THROW_ZIPDIR_ERROR(ZD_ERROR_CDR_IS_CORRUPT, "Central Directory contains file descriptors pointing outside the archive file boundaries. The archive file is either "
                                                    "truncated or damaged. Please try to repair the file"); // the file offset is beyond the CDR: impossible

    if (pFileHeader->nMethod == METHOD_STORE && pFileHeader->desc.lSizeUncompressed != pFileHeader->desc.lSizeCompressed)
        THROW_ZIPDIR_ERROR(ZD_ERROR_VALIDATION_FAILED, "File with STORE compression method declares its compressed size not matching its uncompressed size. File descriptor is "
                                                       "inconsistent, archive content may be damaged, please try to repair the archive");
...
void ZipDir::CacheFactory::InitDataOffset(FileEntry& fileEntry, const ZipFile::CDRFileHeader* pFileHeader) {
    ...
    */
    ...
    if (pFileHeader->desc != pLocalFileHeader->desc || pFileHeader->nMethod != pLocalFileHeader->nMethod || pFileHeader->nFileNameLength != pLocalFileHeader->nFileNameLength
        ...
        THROW_ZIPDIR_ERROR(ZD_ERROR_VALIDATION_FAILED, "The local file header descriptor doesn't match the basic parameters declared in the global file header in the file. The "
                                                       "archive content is misconsistent and may be damaged. Please try to repair the archive");
    ...
    if (memicmp((const char*)&pBuffer[sizeof(LocalFileHeader)], (const char*)pFileHeader + 1, pFileHeader->nFileNameLength)) {
        ...
        THROW_ZIPDIR_ERROR(ZD_ERROR_VALIDATION_FAILED, "The local file header contains file name which does not match the file name of the global file header. The archive content "
                                                       "is misconsistent with its directory. Please repair the archive");
    }
    ...
    if (fileEntry.nFileDataOffset >= m_nCDREndPos)
        THROW_ZIPDIR_ERROR(
            ...
            "The global file header declares the file which crosses the boundaries of the archive. The archive is either corrupted or truncated, please try to repair it");
...
void ZipDir::CacheFactory::Seek(ZipFile::ulong nPos, int nOrigin) { // throw
    if (fseek(m_f, nPos, nOrigin)) {
        THROW_ZIPDIR_ERROR(
            ZD_ERROR_IO_FAILED,
            "Cannot fseek() to the new position in the file. This is unexpected error and should not happen under any circumstances."
            " Perhaps some network or disk failure error has caused this"
        );
    }
}
...
long ZipDir::CacheFactory::Tell() // throw
{
    ...
    if (nPos == -1) {
        THROW_ZIPDIR_ERROR(
            ZD_ERROR_IO_FAILED,
            "Cannot ftell() position in the archive. This is unexpected error and should not happen under any circumstances."
            " Perhaps some network or disk failure error has caused this"
        );
    }
    return nPos;
