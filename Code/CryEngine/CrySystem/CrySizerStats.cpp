...
CrySizerStatsBuilder::CrySizerStatsBuilder(CrySizerImpl* pSizer, int nMinSubcomponentBytes)
    : m_pSizer(pSizer), m_nMinSubcomponentBytes(nMinSubcomponentBytes < 0 || nMinSubcomponentBytes > 0x10000000 ? 0 : nMinSubcomponentBytes) {}
...
CrySizerStatsRenderer::CrySizerStatsRenderer(ISystem* pSystem, CrySizerStats* pStats, unsigned nMaxSubcomponentDepth, int nMinSubcomponentBytes)
    : m_pStats(pStats), m_pRenderer(pSystem->GetIRenderer()), m_pLog(pSystem->GetILog()), m_pFont(pSystem->GetIConsole()->GetFont()),
      m_nMinSubcomponentBytes(nMinSubcomponentBytes < 0 || nMinSubcomponentBytes > 0x10000000 ? 0x8000 : nMinSubcomponentBytes), m_nMaxSubcomponentDepth(nMaxSubcomponentDepth) {}
...
void CrySizerStatsRenderer::render(bool bRefreshMark) {
    ...
    m_pRenderer->WriteXY(m_pFont, (int)fLeft, (int)(fTop), fCharScaleX, fCharScaleY, 0.9f, 0.85f, 1, 0.85f, "%-*s   TOTAL   partial  count", nNameWidth,
                         bRefreshMark ? "Memory usage (refresh*)" : "Memory usage (refresh )");
    m_pRenderer->WriteXY(m_pFont, (int)fLeft, (int)(fTop + fVStep * 0.25f), fCharScaleX, fCharScaleY, 0.85f, 0.9f, 1, 0.85f, "%*s   _____   _______  _____", nNameWidth, "");
    ...
    float fColors[] = {fLightGray, fLightGray, fGray, 1, 1,     1,          1, 1, fGray, 1, 1,     1, 1, fGray, 1,     1,
                       1,          1,          fGray, 1, fGray, fLightGray, 1, 1, fGray, 1, fGray, 1, 1, fGray, fGray, 1};
    ...
    for (unsigned i = 0; i < m_pStats->size(); ++i) {
        ...
        m_pRenderer->WriteXY(m_pFont, (int)fLeft, (int)(fTop), fCharScaleX, fCharScaleY, pColor[0], pColor[1], pColor[2], pColor[3], "%s%-*s:%s%s", szDepth,
                             nNameWidth - rComp.nDepth, rComp.strName.c_str(), szSize, szCount);
    }

    fTop += 0.25f * fVStep;
    m_pRenderer->WriteXY(m_pFont, (int)fLeft, (int)(fTop), fCharScaleX, fCharScaleY, fLightGray, fLightGray, fLightGray, 1, "%-*s %s", nNameWidth, "___________________________",
                         "________________");
    fTop += fVStep;

    const char* szOverheadNames[CrySizerStats::g_numTimers] = {".Collection", ".Transformation", ".Cleanup"};
    ...
    for (unsigned i = 0; i < CrySizerStats::g_numTimers; ++i) {
        ...
        if (!bOverheadsHeaderPrinted) {
            m_pRenderer->WriteXY(m_pFont, (int)fLeft, (int)(fTop), fCharScaleX, fCharScaleY, fLightGray, fLightGray, fLightGray, 1, "%-*s", nNameWidth, "Overheads");
            fTop += fVStep;
            bOverheadsHeaderPrinted = true;
        }

        m_pRenderer->WriteXY(m_pFont, (int)fLeft, (int)(fTop), fCharScaleX, fCharScaleY, fLightGray, fLightGray, fLightGray, 1, "%-*s:%7.1f ms", nNameWidth, szOverheadNames[i],
                             fTime);
        fTop += fVStep;
    }
}
