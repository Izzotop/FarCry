...
#define AddToWord(c)                                                                                                                                                               \
    {                                                                                                                                                                              \
        if (p - word >= MAXWORDSIZE - 1)                                                                                                                                           \
            Error("increase max word size");                                                                                                                                       \
        *p++ = c;                                                                                                                                                                  \
    }
        ...
        for (wchar_t* p = word;;)
            ...
            case '<':
                ...
                if (ch == '!') {
                    GetC();
                    if (ch == '[') {
                        if (GetC() != 'C' || GetC() != 'D' || GetC() != 'A' || GetC() != 'T' || GetC() != 'A' || GetC() != '[')
                            Error("CDATA block expected");
                        GetC();
                        if (ch == '\r') {
                            GetC();
                            if (ch == '\n')
                        ...
                        for (;;) {
                            if (ch <= 0)
                                Error("EOF inside CDATA block");
                            if (ch == '\n')
                                NewLine();
                            AddToWord(ch);
                            GetC();
                            if (p - word >= 3 && p[-1] == '>' && p[-2] == ']' && p[-3] == ']')
                                break;
                        };
                        p -= 3;
                        *p++ = 0;
    ...
    void ParseNode() {
        ...
        for (;;) {
            ...
            Lex();
            ExpectWord();
            if (wcscmp(word, elemname))
                Error("closing tag and opening don't match");
            sink->EndElement();
            Lex();
            Expect('>');
            return;
