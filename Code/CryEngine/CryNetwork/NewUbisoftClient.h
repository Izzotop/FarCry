...
public:
    ...
    //
    /////////////////////////////////////////////
    void Server_CheckCDKeys(bool bCheck) {
        m_bCheckCDKeys = bCheck;
    };
    ...
    void Client_GameServer(int iLobbyID, int iRoomID, const char* szServerName, const char* szIPAddress, const char* szLANIPAddress, int iMaxPlayers, int iNumPlayers);
    ...
    void RcvActivationID(PREPLY_INFORMATION psReplyInfo, PVALIDATION_SERVER_INFO psValidationServerInfo, GSubyte* pucActivationID, GSubyte* pucGlobalID);
    // Callback that receives the Authorization ID from CDKeyServer.  This ID must be sent to the Game Server.
    void RcvAuthorizationID(PREPLY_INFORMATION psReplyInfo, PVALIDATION_SERVER_INFO psValidationServerInfo, GSubyte* pucAuhorizationID);
    // Callback that receives if a players cdkey is valid and the server should allow the player to connect
    void RcvValidationResponse(PREPLY_INFORMATION psReplyInfo, PVALIDATION_SERVER_INFO psValidationServerInfo, GSubyte* pucAuhorizationID, CDKEY_PLAYER_STATUS eStatus,
                               GSubyte* pucGlobalID);
...
private: // -----------------------------------------------------------------
    ...
    bool GetRouterAddress(int iIndex, char* szIPAddress, unsigned short* pusClientPort, unsigned short* pusRegServerPort);
    ...
    GSvoid GameServerCB(GSint iLobbyID, GSint iRoomID, GSshort siGroupType, GSchar* szGroupName, GSint iConfig, GSchar* szMaster, GSchar* szAllowedGames, GSchar* szGames,
                        GSchar* szGameVersion, GSchar* szGSVersion, GSvoid* vpInfo, GSint iSize, GSuint uiMaxPlayer, GSuint uiNbrPlayer, GSuint uiMaxVisitor, GSuint uiNbrVisitor,
    ...
    GSvoid JoinFinishedCB(GSint iLobbyID, GSint iRoomID, GSvoid* vpGameData, GSint iSize, GSchar* szIPAddress, GSchar* szAltIPAddress, GSushort usPort);
    GSvoid AlternateInfoCB(GSint iLobbyID, GSint iRoomID, const GSvoid* pcAltGroupInfo, GSint iAltGroupInfoSize);
    GSvoid RequestMOTDCB(GSubyte ubType, GSchar* szUbiMOTD, GSchar* szGameMOTD, GSint iReason);
    ...
    GSvoid RegServerRcv_LoginRouterResult(GSubyte ucType, GSint lReason, const GSchar* szIPAddress);
    GSvoid RegServerRcv_RouterDisconnection();
    GSvoid RegServerRcv_RegisterServerResult(GSubyte pucType, GSint plReason, GSint iGroupID, const GSchar* szAddress, GSushort usPort, const GSchar* szSessionName);
    GSvoid RegServerRcv_RequestParentGroupResult(GSubyte ucType, GSint lReason, GSint iServerID, GSint iGroupID, const GSchar* szGroupName, GSuint uiNbPlayers,
                                                 GSuint uiMaxPlayers);
    GSvoid RegServerRcv_LobbyServerLoginResults(GSubyte ucType, GSint iReason, GSint iLobbyServerID, GSint iGroupID);
    GSvoid RegServerRcv_LobbyServerUpdateGroupSettingsResults(GSubyte ucType, GSint iReason, GSint iGroupID);
    GSvoid RegServerRcv_LobbyServerDisconnection();
    GSvoid RegServerRcv_LobbyServerMemberNew(const GSchar* szMember, GSbool bSpectator, const GSchar* szIPAddress, const GSchar* szAltIPAddress, const GSvoid* pPlayerInfo,
                                             GSuint uiPlayerInfoSize, GSushort usPlayerStatus);
    GSvoid RegServerRcv_LobbyServerMemberLeft(const GSchar* szMember);
    GSvoid RegServerRcv_LobbyServerNewGroup(GSushort usRoomType, const GSchar* szRoomName, GSint iGroupID, GSint iLobbyServerID, GSint iParentGroupID, GSint uiGroupConfig,
                                            GSshort sGroupLevel, const GSchar* szMaster, const GSchar* szAllowedGames, const GSchar* szGame, const GSvoid* pGroupInfo,
                                            GSuint GroupInfoSize, GSuint uiMatchEventID, GSuint uiMaxPlayers, GSuint uiNbPlayers, GSuint uiMaxSpectators, GSuint uiNbSpectators,
                                            const GSchar* szGameVersion, const GSchar* szGSGameVersion, const GSchar* szIPAddress, const GSchar* szAltIPAddress);
    ...
    virtual GSvoid RegServerRcv_LobbyServerMemberUpdateInfo(const GSchar* szMember, const GSvoid* pPlayerInfo, GSuint uiPlayerInfoSize){};
