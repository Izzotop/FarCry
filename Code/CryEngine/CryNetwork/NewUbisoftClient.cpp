...
NewUbisoftClient::NewUbisoftClient(const char* szLocalIPAddress)
    : m_strUsername(""), m_iJoinedLobbyID(0), m_iJoinedRoomID(0), m_bDownloadedGSini(false), m_eServerState(NoUbiServer), m_eClientState(NoUbiClient), m_hCDKey(0),
      m_pCDKeyServer(nullptr), m_bCheckCDKeys(false), m_dwNextServerAbsTime(0), m_dwNextClientAbsTime(0), m_dwAccountCreateTime(0), m_bDisconnecting(0), sv_authport(0),
      sv_regserver_port(0), m_pLog(0), m_pSystem(0), m_usGamePort(0) {
...
void NewUbisoftClient::Init(ISystem* inpSystem) {
    ...
    sv_authport = pConsole->GetCVar("sv_authport");
    assert(sv_authport);
    sv_regserver_port = pConsole->GetCVar("sv_regserver_port");
    assert(sv_regserver_port);
}
...
bool NewUbisoftClient::Update() {
    ...
    {
        bool bCheckCDKey = m_pSystem->GetIGame()->GetModuleState(EGameMultiplayer) && m_pSystem->GetINetwork() && m_pSystem->GetINetwork()->GetServerByPort(m_usGamePort) &&
...
void NewUbisoftClient::Client_GameServer(int iLobbyID, int iRoomID, const char* szServerName, const char* szIPAddress, const char* szLANIPAddress, int iMaxPlayers,
                                         int iNumPlayers) {
...
bool NewUbisoftClient::GetRouterAddress(int iIndex, char* szIPAddress, unsigned short* pusClientPort, unsigned short* pusRegServerPort) {
    char szKey[50];
    char szPort[50];
