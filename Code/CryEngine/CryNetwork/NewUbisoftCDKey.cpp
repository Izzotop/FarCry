...
static GSvoid __stdcall CDKey_RcvActivationID(PREPLY_INFORMATION psReplyInfo, PVALIDATION_SERVER_INFO psValidationServerInfo, GSubyte* pucActivationID, GSubyte* pucGlobalID) {
    if (g_pUbisoftClient)
        g_pUbisoftClient->RcvActivationID(psReplyInfo, psValidationServerInfo, pucActivationID, pucGlobalID);
}

static GSvoid __stdcall CDKey_RcvAuthorizationID(PREPLY_INFORMATION psReplyInfo, PVALIDATION_SERVER_INFO psValidationServerInfo, GSubyte* pucAuhorizationID) {
    if (g_pUbisoftClient)
        g_pUbisoftClient->RcvAuthorizationID(psReplyInfo, psValidationServerInfo, pucAuhorizationID);
}

static GSvoid __stdcall CDKey_RcvValidationResponse(PREPLY_INFORMATION psReplyInfo, PVALIDATION_SERVER_INFO psValidationServerInfo, GSubyte* pucAuhorizationID,
                                                    CDKEY_PLAYER_STATUS eStatus, GSubyte* pucGlobalID) {
    if (g_pUbisoftClient)
        g_pUbisoftClient->RcvValidationResponse(psReplyInfo, psValidationServerInfo, pucAuhorizationID, eStatus, pucGlobalID);
}

static GSvoid __stdcall CDKey_RcvPlayerStatusRequest(PVALIDATION_SERVER_INFO psValidationServerInfo, GSubyte* pucAuhorizationID) {
    if (g_pUbisoftClient)
...
void NewUbisoftClient::CopyIDToVector(CDKeyIDVector& stVector, const GSubyte* pubArray, unsigned int uiSize) {
    stVector.clear();
...
GSvoid NewUbisoftClient::RcvActivationID(PREPLY_INFORMATION psReplyInfo, PVALIDATION_SERVER_INFO psValidationServerInfo, GSubyte* pucActivationID, GSubyte* pucGlobalID) {
...
GSvoid NewUbisoftClient::RcvAuthorizationID(PREPLY_INFORMATION psReplyInfo, PVALIDATION_SERVER_INFO psValidationServerInfo, GSubyte* pucAuthorizationID) {
...
bool NewUbisoftClient::Server_CheckPlayerAuthorizationID(BYTE bPlayerID, const BYTE* pubAuthorizationID) {
    ...
    if (!pubAuthorizationID) {
        m_pLog->Log("\001Ubi.com: CDKey CheckPlayerAuthorizationID: Authorization ID nullptr");
        return false;
    }
...
void NewUbisoftClient::RcvValidationResponse(PREPLY_INFORMATION psReplyInfo, PVALIDATION_SERVER_INFO psValidationServerInfo, GSubyte* pucAuthorizationID,
                                             CDKEY_PLAYER_STATUS eStatus, GSubyte* pucGlobalID) {
    CDKeyIDVector stID;
...
void NewUbisoftClient::SaveCDKey(const GSchar* szCDKey) {
    if (szCDKey && (((strlen(szCDKey) + 1) % 8) == 0)) {
        ...
        sprintf(szCDKeyHex, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", szEncCDKey[0], szEncCDKey[1], szEncCDKey[2],
                szEncCDKey[3], szEncCDKey[4], szEncCDKey[5], szEncCDKey[6], szEncCDKey[7], szEncCDKey[8], szEncCDKey[9], szEncCDKey[10], szEncCDKey[11], szEncCDKey[12],
                szEncCDKey[13], szEncCDKey[14], szEncCDKey[15], szEncCDKey[16], szEncCDKey[17], szEncCDKey[18], szEncCDKey[19], szEncCDKey[20], szEncCDKey[21], szEncCDKey[22],
                szEncCDKey[23]);
...
void NewUbisoftClient::SaveActivationID(const GSubyte* pubActivationID) {
    if (pubActivationID) {
        ...
        sprintf(szActivationHex, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", pubActivationID[0], pubActivationID[1], pubActivationID[2], pubActivationID[3],
                pubActivationID[4], pubActivationID[5], pubActivationID[6], pubActivationID[7], pubActivationID[8], pubActivationID[9], pubActivationID[10], pubActivationID[11],
