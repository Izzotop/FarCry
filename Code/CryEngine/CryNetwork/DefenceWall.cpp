...
#ifdef LOGEVENTS
        if (m_bLog)
            CryLog("<DefenceWall> CreatedExeProbe[%d] %s %I64x", ctx.nRequestId, ctx.probe.sFilename.c_str(), ctx.probe.nCode);
#endif
    } else {
        return false;
    }
...
#ifdef LOGEVENTS
    if (m_bLog)
        CryLog("<DefenceWall> CreatedFileProbe[%d] %s %I64x", ctx.nRequestId, ctx.probe.sFilename.c_str(), ctx.probe.nCode);
#endif
#endif
    return true;
}
...
#ifdef LOGEVENTS
    if (m_bLog)
        CryLog("<DefenceWall> IssueRequest[%d] %s %I64x (ofs=%d,size=%d)", pCtx->nRequestId, pCtx->probe.sFilename.c_str(), pCtx->probe.nCode, pCtx->probe.nOffset,
               pCtx->probe.nSize);
#endif
...
#ifdef LOGEVENTS
            if (m_bLog)
                CryLog("<DefenceWall> Sending Stream to Client %d bytes", stm.GetSize() / 8);
#endif
...
#ifdef LOGEVENTS
            if (m_bLog)
                CryLog("Unknown Server Request Code");
#endif
            break;
        }
    }
...
#ifdef LOGEVENTS
        if (m_bLog)
            CryLog("<DefenceWall> OnClientResponse[%d] %s %I64x", ctx.nRequestId, ctx.probe.sFilename.c_str(), ctx.probe.nCode);
#endif

        if (clientResponse.nClientStatusFlags != nServerStatusFlags) {
#ifdef LOGEVENTS
            if (m_bLog)
                CryLog("<DefenceWall> Wrong Vars! %d!=%d", clientResponse.nClientStatusFlags, nServerStatusFlags);
#endif
...
#ifdef LOGEVENTS
                if (m_bLog)
                    CryLog("<DefenceWall> Wrong Code! %d!=%d", clientResponse.probe.nCode, pServerCtx->probe.nCode);
#endif
                // Wrong Code!
...
#ifdef LOGEVENTS
                if (m_bLog)
                    CryLog("<DefenceWall> Wrong Num Paks! %d!=%d", clientResponse.nNumOpenedPaks, pServerCtx->nNumOpenedPaks);
#endif
...
#ifdef LOGEVENTS
                if (m_bLog)
                    CryLog("<DefenceWall> Too Big Request Id! %d!=%d", clientResponse.nClientStatusFlags, nServerStatusFlags);
#endif
...
#ifdef LOGEVENTS
            if (m_bLog)
                CryLog("<DefenceWall> PAK: %s", pPakInfo->arrPaks[i].szFilePath);
#endif
...
#ifdef LOGEVENTS
        if (m_bLog)
            CryLog("Server Sent Unknown request");
#endif
        return;
    }
...
#ifdef LOGEVENTS
    if (m_bLog)
        CryLog("<DefenceWall> OnServerRequest[%d] %s %I64x (ofs=%d,size=%d)", ctx.nRequestId, ctx.probe.sFilename.c_str(), ctx.probe.nCode, ctx.probe.nOffset, ctx.probe.nSize);
#endif
...
#ifdef LOGEVENTS
                if (m_bLog)
                    CryLog("<DefenceWall> Repeat Request[%d] (Retry:%d)", ctx.nRequestId, ctx.nRetries);
#endif
                ctx.nRetries++;
...
#ifdef LOGEVENTS
                    if (m_bLog)
                        CryLog("<DefenceWall> Delete Request[%d] Unknown Client!", ctx.nRequestId);
#endif
...
#ifdef LOGEVENTS
                if (m_bLog)
                    CryLog("<DefenceWall> No Resonse from Client for Request(%d)", ctx.nRequestId);
#endif
