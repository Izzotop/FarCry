...
extern "C" void PBsdk_SendClPacket(int datalen, char* data) {
    ...
    if (PBisLocalServer()) {
        ...
        for (i = 0; i < PB_MAX_CLIENTS; i++)
            if (!stricmp("localhost", pbsdk->pbsv.m_client[i].pbc.ip))
                break;
        if (i < PB_MAX_CLIENTS)
            PbSvAddEvent(PB_EV_PACKET, i, datalen, data);
    } else {
...
extern "C" char* PBsdk_GetCvarValue(char* var_name) {
    // special cases
    if (!stricmp(var_name, "version"))
        return (strcpy(var_name, PBgameVer()));
    ...
    if (!stricmp(var_name, "server"))
        return PBserverIp(true);

    // redirects
    if (!stricmp(var_name, "mapname"))
        strcpy(var_name, "g_levelName");
...
extern "C" int PBsdk_GetClientInfo(int svsIndex, stPb_Sv_Client* c) {
    ...
    if (svsIndex < 0 || svsIndex >= PB_MAX_CLIENTS)
        return 0;                                             // return on invalid index
    if (*pbsdk->pbsv.m_client[svsIndex].pbc.ip) {             // player in this slot?
        ...
        if (gci) {
            ...
            *c->ip = 0; // player not set up yet (i.e. GetSlotInfo returned false)
    }
    return 1;
}
#endif
...
extern "C" int PBsdk_GetClientStats(int index, char* data) {
    if (index < 0 || index >= PB_MAX_CLIENTS)
        return 0; // return on invalid index
    if (*pbsdk->pbsv.m_client[index].pbc.ip == 0)
        return 0; // no client at this index
...
int PbSvAddClient(char* addr, char* name, char* guid) {
    ...
    for (i = 0; i < PB_MAX_CLIENTS; i++) {
        ...
        if (*pbsdk->pbsv.m_client[i].pbc.ip == 0 && j < 0)
            j = i;
    }
    if (j < 0)
...
int PbSvRemoveClient(char* addr) {
    ...
    int i;
    for (i = 0; i < PB_MAX_CLIENTS; i++) {
        if (strcmp(pbsdk->pbsv.m_client[i].pbc.ip, addr))
            continue;
...
extern "C" void PBsdk_SetPointers(void* pbinterface) {
    ...
    if (pbsdk != nullptr)
        return; // already accomplished

    if (tries >= 3)
        return; // after 3 tries, give up
    ...
    if (pbsdk == nullptr)
        return; // failed to get pointer to struct instance
