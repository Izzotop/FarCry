...
char* __cdecl PbClGameCommand(char* Cmd, char* Result) {
    ...
    if (!stricmp(Cmd, "set_cl_punkbuster"))
        pbsdk->pb_SetClPunkBuster(Result);
...
char* __cdecl PbClGameQuery(int Qtype, char* Data) {
    ...
    switch (Qtype) {
    case PB_Q_CVAR:
        strncpy(Data, pbsdk->pb_GetCvarValue(Data), PB_Q_MAXRESULTLEN);
        break;
    case PB_Q_SINFO:
        strncpy(Data, pbsdk->pb_GetKeyValue(pbsdk->pb_GetServerInfo(), Data), PB_Q_MAXRESULTLEN);
        break;
    case PB_Q_SADDR:
        strncpy(Data, pbsdk->pb_GetServerAddr(), PB_Q_MAXRESULTLEN);
        break;
    case PB_Q_SEARCHBINDINGS:
        n = pbsdk->pb_GetMaxKeys();
        for (i = atoi(Data); i < n; i++)
            if (stristr(pbsdk->pb_GetKeyBinding(i), arg2) != nullptr) {
                itoa(i, Data, 10);
                return nullptr;
            }
        itoa(-1, Data, 10);
    ...
    case PB_Q_GETBINDING:
        strncpy(Data, pbsdk->pb_GetKeyBinding(atoi(Data)), PB_Q_MAXRESULTLEN);
        break;
    case PB_Q_KEYNAME:
        strncpy(Data, pbsdk->pb_GetKeyName(atoi(Data)), PB_Q_MAXRESULTLEN);
        break;
    case PB_Q_SEARCHCVARS:
        ...
        while (pbsdk->pb_CvarWalk(&name, &string, &flags, &resetString)) {
            ...
            if (!(*name) || !(*string))
                continue;
            if (stristr(string, Data) != nullptr) {
        ...
        *Data = 0;
        break;
    case PB_Q_CVARVALID:
        return pbsdk->pb_CvarValidate(Data);
    case PB_Q_CVARFLAGS: // note: this query type returns nullptr when done only, cvar name otherwise
        if (pbsdk->pb_CvarWalk(&name, &string, &flags, &resetString) == 0)
            return nullptr;
        if (name == nullptr)
            name = "";
        itoa(flags, Data, 10);
        return name;
    case PB_Q_CVARDEFAULTS: // note: this query type returns nullptr when done only, cvar name otherwise
        if (pbsdk->pb_CvarWalk(&name, &string, &flags, &resetString) == 0)
            return nullptr;
        if (name == nullptr || string == nullptr || resetString == nullptr)
            name = string = resetString = "";
        if (!strcmp(string, resetString))
            *Data = 0;
        else {
...
#endif
    case PB_Q_DLLHANDLE:
        return (char*)pbsdk->pb_DllHandle(Data);
    default:
        *Data = 0;
        break;
    }
...
int __cdecl PbTrapPreExecCmd(char* cmdtext) // return 0 if game should continue exec'ing the command, 1 if not
{
    if (!isPBmultiplayerMode())
        return 0;
    if (pbsdk->pbcl.m_TrapPreExecCmd == nullptr)
        return 0;
...
void __cdecl PbClientTrapConsole(char* msg, int msglen) {
    if (!isPBmultiplayerMode())
        return;
    if (pbsdk->pbcl.m_TrapConsole == nullptr)
        return;
...
void __cdecl PbClientInitialize(void* exeInst) {
    ...
    if (pbsdk->pbcl.m_ClInstance == nullptr)
        pbsdk->pb_SetClPunkBuster("0");
}
...
void __cdecl PbClientConnecting(int status, char* pkt, int* pktlen) {
    if (!isPBmultiplayerMode())
        return;
    if (pbsdk->pbcl.m_ClientConnect == nullptr)
        return;
...
{
    sprintf(textbuf, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", (int)m->digest[0], (int)m->digest[1], (int)m->digest[2], (int)m->digest[3],
            (int)m->digest[4], (int)m->digest[5], (int)m->digest[6], (int)m->digest[7], (int)m->digest[8], (int)m->digest[9], (int)m->digest[10], (int)m->digest[11],
...
int __cdecl isPbClEnabled(void) {
    if (!isPBmultiplayerMode())
        return 0;
    return (int)pbsdk->pbcl.AddPbEvent(PB_EV_ISENABLED, 0, nullptr);
}

int __cdecl getPbGuidAge(void) {
    if (!isPbClEnabled())
        return -2;
    return (int)pbsdk->pbcl.AddPbEvent(PB_EV_GUIDAGE, 0, nullptr); // returns -1 if bad/missing cdkey
}

void __cdecl EnablePbCl(void) {
    if (!isPBmultiplayerMode())
        return;
    pbsdk->pbcl.AddPbEvent(PB_EV_ENABLE, 0, nullptr);
}

void __cdecl DisablePbCl(void) {
    if (!isPBmultiplayerMode())
        return;
    pbsdk->pbcl.AddPbEvent(PB_EV_DISABLE, 0, nullptr);
}
