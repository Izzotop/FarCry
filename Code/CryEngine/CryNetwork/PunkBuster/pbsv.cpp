...
void __cdecl PbServerInitialize(void) {
    ...
    if (pbsdk->pbsv.m_AddPbEvent == nullptr)
        pbsdk->pb_SetSvPunkBuster("0");
}
...
void __cdecl PbPassConnectString(char* fromAddr, char* connectString) {
    if (!isPBmultiplayerMode())
        return;
    if (pbsdk->pbsv.m_PassConnectString == nullptr)
        return; // means PB not installed/enabled
...
char* __cdecl PbAuthClient(char* fromAddr, int cl_pb, char* cl_guid) {
    if (!isPBmultiplayerMode())
        return nullptr;
    if (pbsdk->pbsv.m_AuthClient == nullptr)
        return nullptr; // means PB not installed/enabled
...
int __cdecl isPbSvEnabled(void) {
    if (!isPBmultiplayerMode())
        return 0;
    return (int)pbsdk->pbsv.AddPbEvent(PB_EV_ISENABLED, -1, 0, nullptr);
}

void __cdecl EnablePbSv(void) {
    if (!isPBmultiplayerMode())
        return;
    pbsdk->pbsv.AddPbEvent(PB_EV_ENABLE, -1, 0, nullptr);
}

void __cdecl DisablePbSv(void) {
    if (!isPBmultiplayerMode())
        return;
    pbsdk->pbsv.AddPbEvent(PB_EV_DISABLE, -1, 0, nullptr);
}
...
void __cdecl PbCaptureConsoleOutput(char* msg, int msglen) {
    ...
    if (pbsdk->pbcl.m_ReloadClient == 0)
        PbClientTrapConsole(msg, msglen);

    if (pbsdk->pbsv.m_ReloadServer == 0)
        if (pbsdk->pbsv.m_TrapConsole != nullptr)
            pbsdk->pbsv.m_TrapConsole(&pbsdk->pbsv, msg, msglen);
    ...
    if (!strnicmp(msg, "pb_", 3)) {
        if (!strnicmp(msg + 3, "sv_", 3)) {
            if (pbsdk->pbsv.m_TrapConsole != nullptr)
                PbSvAddEvent(PB_EV_CMD, -1, msglen, msg);
        } else {
            if (pbsdk->pbcl.m_ReloadClient == 0)
                PbClAddEvent(PB_EV_CMD, msglen, msg);
        }
    }

    if (pbsdk->ConsoleCaptureBuf == nullptr)
        return;
    int sl = strlen(pbsdk->ConsoleCaptureBuf);
    if (sl + (int)strlen(msg) + 1 >= pbsdk->ConsoleCaptureBufLen)
        return;
...
char* __cdecl PbSvGameCommand(char* Cmd, char* Result) {
    if (!isPBmultiplayerMode())
        return nullptr;
    if (!stricmp(Cmd, "set_sv_punkbuster"))
        pbsdk->pb_SetSvPunkBuster(Result);
    else if (!stricmp(Cmd, "pakNames"))
        PBpakNames(Result); // note: Result must be 1025+ bytes for this one
    else if (!stricmp(Cmd, "ConCapBufLen"))
        pbsdk->ConsoleCaptureBufLen = (int)Result;
    else if (!stricmp(Cmd, "ConCapBuf"))
        ...
        if (!stricmp(Cmd, "DropClient"))
            pbsdk->pb_DropClient(atoi(Result), arg1);
...
char* __cdecl PbSvGameQuery(int Qtype, char* Data) {
    ...
    int i;
    switch (Qtype) {
    case PB_Q_MAXCLIENTS:
        itoa(PB_MAX_CLIENTS, Data, 10);
        break;
    case PB_Q_CLIENT:
        i = atoi(Data);
        if (!pbsdk->pb_GetClientInfo(i, (stPb_Sv_Client*)Data))
            return PbsQueryFail;
        break;
    case PB_Q_CVAR:
    ...
    case PB_Q_STATS:
        i = atoi(Data);
        if (!pbsdk->pb_GetClientStats(i, Data))
            return PbsQueryFail;
        break;
    }
    return nullptr;
