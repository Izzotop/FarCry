...
struct stPbSv {
    ...
    inline char* getHomePath(void) {
        ...
        if (!(*m_cwd))
            getcwd(m_cwd, PB_Q_MAXRESULTLEN - 4);
        if (*m_cwd && m_cwd[strlen(m_cwd) - 1] != *pbDIRSEP)
...
#endif
#ifdef __PBMAC__
                if (m_cwd[strlen(m_cwd) - 1] == *pbDIRSEP)
                    m_cwd[strlen(m_cwd) - 1] = 0;
                mkdir(m_cwd, 511);
                strcat(m_cwd, pbDIRSEP);
#endif
    ...
    inline void UnloadAgentDll(void) {
        ...
        if (m_AgInstance != nullptr)
            FreeLibrary((HMODULE)m_AgInstance);
#endif
#ifdef __PBLINUX__
        if (m_AgInstance != nullptr)
    ...
    inline void uninitialize(void) { // also initializes game-side func ptrs
        ...
        m_GameQuery = nullptr;
        m_GameMsg = nullptr;
        m_SendToClient = nullptr;
        m_SendToAddrPort = nullptr;
        m_PassConnectString = nullptr;
    ...
    inline void UnloadServerDll(void) {
        ...
        if (m_SvInstance != nullptr)
            FreeLibrary((HMODULE)m_SvInstance);
#endif
#ifdef __PBLINUX__
        if (m_SvInstance != nullptr)
    ...
    inline char* AddPbEvent(int type, int clientIndex, int datalen, char* data, int retry = 0) {
        ...
        if (m_ReloadServer || m_SvInstance == nullptr) {
            if (m_SvInstance != nullptr) {
                UnloadServerDll();
                return nullptr;
            }
