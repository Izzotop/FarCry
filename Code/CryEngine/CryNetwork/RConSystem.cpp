...
public:
    ...
    virtual void Print(const char* inszText) {
        ...
        if (hRes == SOCKET_ERROR) {
            INetwork* pNetwork = m_Ref.m_pSystem->GetINetwork();
            assert(pNetwork);
...
void CRConSystem::OnReceivingPacket(const unsigned char inPacketID, CStream& stmPacket, CIPAddress& ip) {
    ...
    ICVar* pVar = pConsole->GetCVar("sv_rcon_password");
    assert(pVar);
    ...
    if (memcmp(nServerPassCode, pccp.m_nRConPasswordCode, sizeof(nServerPassCode)) != 0 && memcmp(m_nDevPassCode, pccp.m_nRConPasswordCode, sizeof(nServerPassCode)) != 0) {
...
void CRConSystem::ExecuteRConCommand(const char* inszCommand) {
    ...
    ICVar* pVar1 = pConsole->GetCVar("cl_rcon_serverip");
    assert(pVar1);
    string serverip = pVar1->GetString();

    ICVar* pVar2 = pConsole->GetCVar("cl_rcon_port");
    assert(pVar2);
    WORD wPort = pVar2->GetIVal();

    ICVar* pVar3 = pConsole->GetCVar("cl_rcon_password");
    assert(pVar3);
