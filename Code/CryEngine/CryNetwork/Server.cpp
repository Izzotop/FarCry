...
// todo (Izztop): #pragma comment(lib, "ASEQuerySDK.lib")

static CServer* g_pServer = nullptr;

extern "C" {
// todo (Izztop): #include "ASEQuerySDK.h"

void ASEQuery_wantstatus() {
    if (!g_pServer || !g_pServer->GetServerSlotFactory()) {
        // todo (Izztop): ASEQuery_status("", "", "", "", 1, 0, 0);
        return;
    }
    ...
    // todo (Izztop): ASEQuery_status(szName.c_str(), szGameType.c_str(), szMap.c_str(), szVersion.c_str(), bPassword, nPlayers, nMaxPlayers);
}

void ASEQuery_wantrules() {
    IScriptSystem* pSS = GetISystem()->GetIScriptSystem();

    _SmartScriptObject QueryHandler(pSS, true);

    if (!pSS->GetGlobalValue("QueryHandler", (IScriptObject*)QueryHandler)) {
        return;
    }

    _SmartScriptObject ServerRules(pSS, true);
    ...
    for (int i = 1; i <= ServerRules->Count(); i++) {
        _SmartScriptObject Rule(pSS, true);

        if (ServerRules->GetAt(i, (IScriptObject*)Rule)) {
            const char* szRuleName = nullptr;
            const char* szRuleValue = nullptr;

            Rule->GetAt(1, szRuleName);
            Rule->GetAt(2, szRuleValue);

            if (szRuleValue && szRuleName) {
                // todo (Izztop): ASEQuery_addrule(szRuleName, szRuleValue);
            }
        }
    }
}
...
void ASEQuery_wantplayers() {
    ...
    for (int i = 1; i <= PlayerStats->Count(); i++) {
        ...
        if (PlayerStats->GetAt(i, (IScriptObject*)Player)) {
            ...
            // todo (Izztop): ASEQuery_addplayer(szName, szTeam, szSkin, szScore, szPing, szTime);
        }
    }
}
} // extern "C"
...
bool CServer::Init(IServerSlotFactory* pFactory, WORD wPort, bool listen) {
    ...
    if (m_bListen = listen) { // todo (Izzotop): Using the result of an assignment as a condition without parentheses
        CIPAddress ipLocal;

        ipLocal.m_Address.ADDR = m_pNetwork->GetLocalIP();

        // only create the multicast socket if it's not internet server
        ICVar* sv_ServerType = GetISystem()->GetIConsole()->GetCVar("sv_ServerType");
        assert(sv_ServerType);
        ...
        */
        ...
        if (NET_FAILED(m_socketMain.Listen(wPort, nullptr, &ipLocal)))
            return false;
...
#if !defined(WIN64) && !defined(LINUX64) && !defined(NOT_USE_ASE_SDK)
        // todo (Izztop): ASEQuery_initialize((int)wPort, m_MPServerType != eMPST_LAN ? 1 : 0, szIP);
#endif
    }
...
void CServer::Update(unsigned int nTime) {
    ...
    if (m_bListen)
        do {
            buf.Reset();
            nRecvBytes = 0;
            m_socketMain.Receive(buf.GetPtr(), (int)BITS2BYTES(buf.GetAllocatedSize()), nRecvBytes, ipFrom);
    ...
    if (m_bMulticastSocket && m_bListen && m_MPServerType == eMPST_LAN) {
        do {
            buf.Reset();
            nRecvBytes = 0;
            m_socketMulticast.Receive(buf.GetPtr(), (int)BITS2BYTES(buf.GetAllocatedSize()), nRecvBytes, ipFrom);
