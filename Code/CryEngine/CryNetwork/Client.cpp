...
CClient::CClient(CNetwork* pNetwork) : m_ctpEndpoint(pNetwork), m_ctpEndpoint2(pNetwork), cl_timeout(0), m_pbAuthorizationID(nullptr), m_uiAuthorizationSize(0), m_bWaiting(0) {
    m_pNetwork = pNetwork;
...
bool CClient::Init(IClientSink* pSink) {
    ...
    if (NET_FAILED(m_socketMain.Create()))
        return false;
    if (NET_FAILED(m_socketMain.Listen(0)))
        return false;
...
bool CClient::Send(CStream& stm) {
    ...
    if (FAILED(m_socketMain.Send(stm.GetPtr(), BITS2BYTES(stm.GetSize()), &m_ipServer)))
        return false;
    return true;
#else
...
bool CClient::IsReady() {
    return (m_smCCPMachine.GetCurrentStatus() == STATUS_READY);
}
...
#endif // NOT_USE_UBICOM_SDK
    {
        OnCDKeyAuthorization(nullptr); // 0 -> fake AuthorizationID is generated
    }
}
