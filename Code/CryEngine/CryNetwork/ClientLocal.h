...
class CClientLocal : public IClient {
public:
    ...
    void OnConnect(CNPServerVariables sv) {
        if (m_pSink)
            m_pSink->OnXConnect();
    }
    void OnContextSetup(CStream& stm) {
        if (m_pSink)
    ...
    void OnDestructServerSlot() {
        m_pServerSlot = 0;
    }
    virtual CIPAddress GetServerIP() const {
        return CIPAddress();
    };
