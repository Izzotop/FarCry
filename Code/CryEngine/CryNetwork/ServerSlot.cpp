...
CServerSlotImpl::CServerSlotImpl(CNetwork* pNetwork, _IServerServices* pParent) : CServerSlot(pNetwork), m_ctpEndpoint(pNetwork), m_ctpEndpoint2(pNetwork) {
...
bool CServerSlotImpl::IsReady() {
    return m_smCCPMachine.GetCurrentStatus() == STATUS_SRV_READY;
}
...
CServerSlotLocal::CServerSlotLocal(CServer* pServer, CClientLocal* pClient, CIPAddress& ip, CNetwork* pNetwork) : CServerSlot(pNetwork) {
    m_cClientID = 0;
    m_pClient = pClient;
    m_pServer = pServer;
    m_ipAddress = ip;
...
void CServerSlot::OnPlayerAuthorization(bool bAllow, const char* szError, const BYTE* pGlobalID, unsigned int uiGlobalIDSize) {
