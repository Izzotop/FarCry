...
bool NewUbisoftClient::Client_Login(const GSchar* szUsername, const GSchar* szPassword, bool bSavePassword) {
    ...
    if (GetRouterAddress(iIndex, szIPAddress, &usClientPort, &usRegServerPort)) {
        while (clMSClientClass::Initialize(szIPAddress, usClientPort, szUsername, szPassword, UBISOFT_GAME_VERSION) == GS_FALSE) {
            iIndex++;
            if (!GetRouterAddress(iIndex, szIPAddress, &usClientPort, &usRegServerPort)) {
...
bool NewUbisoftClient::Client_JoinGameServer(int iLobbyID, int iRoomID) {
    ...
    case GameServerDisconnected: {
        if (clMSClientClass::JoinGameServer(iLobbyID, iRoomID, "", GSGAMEVERSION, GAME_NAME, nullptr, 0)) {
            m_iJoinedLobbyID = iLobbyID;
            m_iJoinedRoomID = iRoomID;
...
bool NewUbisoftClient::Client_CreateAccount(const char* szUsername, const char* szPassword) {
    ...
    if (GetRouterAddress(iIndex, szIPAddress, &usClientPort, &usRegServerPort)) {
        while (clMSClientClass::CreateAccount(szIPAddress, usClientPort, UBISOFT_GAME_VERSION, szUsername, szPassword, "", "", "", "") == GS_FALSE) {
            iIndex++;
            if (!GetRouterAddress(iIndex, szIPAddress, &usClientPort, &usRegServerPort))
                return false;
        }
...
GSvoid NewUbisoftClient::GameServerCB(GSint iLobbyID, GSint iRoomID, GSshort siGroupType, GSchar* szGroupName, GSint iConfig, GSchar* szMaster, GSchar* szAllowedGames,
                                      GSchar* szGames, GSchar* szGameVersion, GSchar* szGSVersion, GSvoid* vpInfo, GSint iSize, GSuint uiMaxPlayer, GSuint uiNbrPlayer,
                                      GSuint uiMaxVisitor, GSuint uiNbrVisitor, GSchar* szIPAddress, GSchar* szAltIPAddress, GSint iEventID) {
    ...
    Client_GameServer(iLobbyID, iRoomID, szGroupName, szGameIPAddress, szGameLANIPAddress, uiMaxPlayer, uiNbrPlayer);
}
...
GSvoid NewUbisoftClient::JoinFinishedCB(GSint iLobbyID, GSint iRoomID, GSvoid* vpGameData, GSint iSize, GSchar* szIPAddress, GSchar* szAltIPAddress, GSushort usPort) {
...
GSvoid NewUbisoftClient::AlternateInfoCB(GSint iLobbyID, GSint iRoomID, const GSvoid* pcAltGroupInfo, GSint iAltGroupInfoSize) {}
