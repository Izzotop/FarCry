...
CPunkBusterInterface::CPunkBusterInterface(CNetwork* pNetwork) {
    ...
    m_pClient = nullptr;
    m_pClientLocal = nullptr;
    m_pServer = nullptr;
    sys_punkbuster_loaded = nullptr;
...
void CPunkBusterInterface::Update(bool bClient) {
    if (!sys_punkbuster_loaded)
        return; // no PB processing when PB has been shut down or is not yet initialized
...
bool CPunkBusterInterface::ReadStringFromRegistry(const string& szKeyName, const string& szValueName, string& szValue) {
    ...
    DWORD dwSize = 0;
    if (RegQueryValueEx(hKey, szValueName.c_str(), nullptr, nullptr, nullptr, &dwSize) != ERROR_SUCCESS) {
        RegCloseKey(hKey);
        return false;
    }
    szValue.resize(dwSize);
    RegQueryValueEx(hKey, szValueName.c_str(), nullptr, nullptr, (LPBYTE)szValue.c_str(), &dwSize);
    RegCloseKey(hKey);
...
void CPunkBusterInterface::OnAddClient(CIPAddress& clientIP) {
    ...
    //  assert( m_pServer );
    if (!m_pServer)
        return; // happens in single-player mode
...
bool CPunkBusterInterface::CheckPBPacket(CStream& stmPacket, CIPAddress& ip) {
    ...
    if (len >= 7)                       // Must be at least 7 bytes int.
    {
        ...
        if (!memcmp(pBuf, "\xff\xff\xff\xffPB_", 7)) {
            // if not ded
            if (pBuf[7] != 'S' && pBuf[7] != '2' && pBuf[7] != 'G' && pBuf[7] != 'I' && pBuf[7] != 'Y' && pBuf[7] != 'B' && pBuf[7] != 'L')
...
void CPunkBusterInterface::OnCCPPunkBusterMsg(CIPAddress& ipAddress, CStream& stm) {
    ...
    if (m_pServer) {
        ...
        for (i = 0; i < PB_MAX_CLIENTS; i++)
            if (!strcmp(addr, pbsdk->pbsv.m_client[i].pbc.ip))
                break;
        if (i < PB_MAX_CLIENTS) {
            BYTE* pByte = stm.GetPtr();
...
void PBoutgame(char* text, int hudAlso) {
    ...
    for (;;) { // loop in case of multiple lines (to simulate linefeeds)
        ...
        if (!hold)
            break; // if end of line, then break out of loop
        *cp = hold;
        te = cp + 1;
    }
}
...
void Sys_PBSendUdpPacket(char* addr, unsigned short port, int datalen, char* data, int isFromClient) {
    ...
    if (isFromClient) { // from client
        if (pip->m_pClient != nullptr)
            pip->m_pClient->SendTo(cip, stm); // no need to check for local due to PB's internal handling
    } else {                                  // from server
        if (pip->m_pServer != nullptr)
            pip->m_pServer->Send(stm, cip);
    }
}
...
int PBgetClientInfo(stPb_Sv_Client* c) {
    ...
    if (*playerInfo.playerName == 0)
        return 0; // player not set up yet
...
void PBcomputeHash(char* guid, CServerSlot* pSlot) {
    ...
    _snprintf(guid, PB_GUIDLEN, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", mc.digest[0], mc.digest[1], mc.digest[2], mc.digest[3], mc.digest[4],
              mc.digest[5], mc.digest[6], mc.digest[7], mc.digest[8], mc.digest[9], mc.digest[10], mc.digest[11], mc.digest[12], mc.digest[13], mc.digest[14], mc.digest[15]);
}
...
char* PBcvar_VariableString(const char* var_name) {
    if (pbsdk == nullptr)
        return "ERROR: nullptr POINTER";
    CPunkBusterInterface* pip = (CPunkBusterInterface*)pbsdk->pbinterface;
    if (pip == nullptr)
        return "ERROR: nullptr POINTER";

    if (pip->m_pSystem == nullptr)
        return "ERROR: nullptr POINTER";
...
int isPBmultiplayerMode() {
    ...
    if (pip->m_pNetwork == nullptr)
        return 0;
    if (pip->m_pServer == nullptr && pip->m_pClient == nullptr && pip->m_pClientLocal == nullptr)
        return 0;
    return 1;
}
...
char* PBserverIp(int bServer) {
    ...
    if (!bServer) {
        if (pip->m_pClient == nullptr) {
            if (pip->m_pClientLocal == nullptr)
                return "bot";
            return "localhost";
        }
        cip = pip->m_pClient->GetServerIP();
    } else {
        if (pip->m_pServer == nullptr || pip->m_pNetwork == nullptr)
            return "???";
...
char* PBqueryGL(int type) {
    ...
    case PB_GL_RGB:
        return nullptr; //(char *) GL_RGB ;
    case PB_GL_UB:
        return nullptr; //(char *) GL_UNSIGNED_BYTE ;
    case PB_GL_D3DDEV:
    ...
    *buf = 0;
    ...
    for (i = 0; i < pPakInfo->numOpenPaks; i++) {
        ...
        if (stricmp(priorRoot, pPakInfo->arrPaks[i].szBindRoot)) {    // new bind root ?
            ...
            priorRoot[512] = 0;
            if (strlen(priorRoot) + slb < 1020)
                _snprintf(buf + slb, 1025 - slb, "*\"%s\" ", priorRoot);
            slb = strlen(buf);
        }
        const char* cp = pPakInfo->arrPaks[i].szFilePath;
        if (strlen(cp) > strlen(priorRoot))
            cp += strlen(priorRoot); // this should always happen
        if (strlen(cp) + slb > 1020)
            continue;                                    // skip it, too many already
