...
private: // ---------------------------------------------------------
    struct SNode {
        ...
        SNode(const unsigned char inValue) : m_Value(inValue) {
            m_dwIndexZero = 0xffffffff;
            m_dwIndexOne = 0xffffffff; // unused
        }

        //! constructor for a node
        SNode(const DWORD indwIndexZero, const DWORD indwIndexOne) : m_dwIndexZero(indwIndexZero), m_dwIndexOne(indwIndexOne) {
            m_Value = 0; // unused
        }
    ...
    struct SPriIndex {
            SPriIndex(const DWORD indwIndex, const DWORD indwPriority) : m_dwIndex(indwIndex), m_dwPriority(indwPriority) {}
