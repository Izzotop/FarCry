...
//
////////////////////////////////////////////////////////////////////
GSvoid NewUbisoftClient::RegServerRcv_LoginRouterResult(GSubyte ucType, GSint lReason, const GSchar* szIPAddress) {
    if (ucType == GSFAIL) {
...
GSvoid NewUbisoftClient::RegServerRcv_RegisterServerResult(GSubyte ucType, GSint plReason, GSint iGroupID, const GSchar* szAddress, GSushort usPort, const GSchar* szSessionName) {
    if (ucType == GSSUCCESS) {
...
GSvoid NewUbisoftClient::RegServerRcv_RequestParentGroupResult(GSubyte ucType, GSint lReason, GSint iServerID, GSint iGroupID, const GSchar* szGroupName, GSuint uiNbPlayers,
                                                               GSuint uiMaxPlayers) {
    if (ucType == GSSUCCESS) {
        ...
        if (iServerID <= 0) {
            ...
            CRegisterServer::RegServerSend_RegisterServerOnLobby(0, 0, m_strGameServerName.c_str(), GAME_NAME, ROOM_UBI_CLIENTHOST_REGSERVER, m_uiMaxPlayers, 0, "", szData,
                                                                 sizeof(szData), nullptr, 0, nullptr, 0, m_usGamePort, "", GSGAMEVERSION, GS_FALSE, GS_FALSE);
        }
    }
}

GSvoid NewUbisoftClient::RegServerRcv_LobbyServerLoginResults(GSubyte ucType, GSint iReason, GSint iLobbyServerID, GSint iGroupID) {
    if (ucType == GSSUCCESS) {
        RegServerSend_RouterDisconnect();
...
GSvoid NewUbisoftClient::RegServerRcv_LobbyServerUpdateGroupSettingsResults(GSubyte ucType, GSint iReason, GSint iGroupID) {}
...
GSvoid NewUbisoftClient::RegServerRcv_LobbyServerMemberNew(const GSchar* szMember, GSbool bSpectator, const GSchar* szIPAddress, const GSchar* szAltIPAddress,
...
GSvoid NewUbisoftClient::RegServerRcv_LobbyServerNewGroup(GSushort usRoomType, const GSchar* szRoomName, GSint iGroupID, GSint iLobbyServerID, GSint iParentGroupID,
                                                          GSint uiGroupConfig, GSshort sGroupLevel, const GSchar* szMaster, const GSchar* szAllowedGames, const GSchar* szGame,
                                                          const GSvoid* pGroupInfo, GSuint GroupInfoSize, GSuint uiMatchEventID, GSuint uiMaxPlayers, GSuint uiNbPlayers,
                                                          GSuint uiMaxSpectators, GSuint uiNbSpectators, const GSchar* szGameVersion, const GSchar* szGSGameVersion,
