...
public:
    ...
    virtual void Advise(IServerSlotSink* pSink) {
        m_pSink = pSink;
    }
    virtual void ResetBandwidthStats();
    virtual void GetBandwidthStats(SServerSlotBandwidthStats& out) const;
    virtual unsigned int GetClientIP() const {
        return m_ipAddress.GetAsUINT();
    };
    virtual void OnPlayerAuthorization(bool bAllow, const char* szError, const BYTE* pGlobalID, unsigned int uiGlobalIDSize);

    //////////////////////////////////////////////////////////////////////////
    // Returns client specific flags.
    virtual unsigned int GetClientFlags() {
        return 0;
    };

    virtual bool IsLocalSlot() const {
        return false;
    };
...
public:
    ...
    virtual void SendUnreliable(CStream& stm);
    virtual bool IsReady() {
        return ((m_pClient && m_pSink) ? true : false);
    }
    virtual void Release();
    virtual unsigned int GetPacketsLostCount() {
        return 0;
