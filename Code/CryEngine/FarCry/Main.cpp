    ...
    if (0 < nBufferSize) {
        ...
        while (true) {
            ...
            if (DRIVE_CDROM == GetDriveType(&rawDriveLetters[i])) {
                ...
                if (FALSE != GetVolumeInformation(&rawDriveLetters[i], &cdVolumeName[0], (DWORD)cdVolumeName.size(), 0, 0, 0, 0, 0)) {
            ...
            ++i; // skip nullptr termination of current drive

            // check if we're out of drive letters
            if (0 == rawDriveLetters[i]) {
                // double nullptr termination found, bail out
                break;
            }
        }
    }
...
bool RegisterWindow(HINSTANCE hInst) {
    ...
    return RegisterClass(&wc) != 0 ? true : false;
}
#endif
...
string TryFormatWinError(DWORD dwError) {
    ...
    DWORD dwFormattedMsgLen = FormatMessage(
    ...
    if (!dwFormattedMsgLen)
        ...
        return string();
    else { // the lpMsgBuf contains allocated by the system call message that is to be returned.
...
static void InvokeExternalConfigTool() {
#if defined(WIN32) || defined(WIN64)
    try {
        // build tmp working directory
        char tmpWorkingDir[MAX_PATH];
        GetModuleFileName(nullptr, tmpWorkingDir, MAX_PATH);
...
bool RunGame(HINSTANCE hInstance, const char* sCmdLine) {
    ...
    do {
        ...
        if (!hWnd && !RegisterWindow(hInstance)) {
            if (!hWnd && RegisterWindow(hInstance)) {
                MessageBox(nullptr, "Cannot Register Window\n", "Error", MB_OK | MB_DEFAULT_DESKTOP_ONLY);
                return false;
            }
        }
        ...
        if (!g_hSystemHandle) {
            ...
            MessageBox(nullptr, ("CrySystem.dll Loading Failed:\n" + TryFormatWinError(dwLastError)).c_str(), "FarCry Error", MB_OK | MB_ICONERROR);
            return false;
        }

        PFNCREATESYSTEMINTERFACE pfnCreateSystemInterface = (PFNCREATESYSTEMINTERFACE)::GetProcAddress(g_hSystemHandle, "CreateSystemInterface");
    ...
    if (bRelaunch) {
        ...
        CreateProcess(nullptr, szBuf, nullptr, nullptr, FALSE, 0, nullptr, szMasterCDFolder, &si, &pi);
