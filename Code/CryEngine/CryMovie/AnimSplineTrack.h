 ...
 */
template <class ValueType> class TAnimTcbTrack : public IAnimTrack {
public:
    TAnimTcbTrack() {
        AllocSpline();
        m_flags = 0;
    ...
    virtual EAnimTrackType GetType() {
        assert(0); return static_cast<EAnimTrackType>(0);
    }
    virtual EAnimValue GetValueType() {
        assert(0); return static_cast<EAnimValue>(0);
    }
...
template <class T> bool TAnimTcbTrack<T>::Serialize(XmlNodeRef& xmlNode, bool bLoading, bool bLoadEmptyTracks) {
    if (bLoading) {
...
template <> void TAnimTcbTrack<float>::GetValue(float time, float& value) {
    m_spline->interpolate(time, value);
}
template <> EAnimTrackType TAnimTcbTrack<float>::GetType() {
    return ATRACK_TCB_FLOAT;
}
template <> EAnimValue TAnimTcbTrack<float>::GetValueType() {
    return AVALUE_FLOAT;
}
...
template <> void TAnimTcbTrack<Vec3>::GetValue(float time, Vec3& value) {
    m_spline->interpolate(time, value);
}
template <> EAnimTrackType TAnimTcbTrack<Vec3>::GetType() {
    return ATRACK_TCB_VECTOR;
}
template <> EAnimValue TAnimTcbTrack<Vec3>::GetValueType() {
...
template <> void TAnimTcbTrack<Quat>::GetValue(float time, Quat& value) {
    m_spline->interpolate(time, value);
}
template <> EAnimTrackType TAnimTcbTrack<Quat>::GetType() {
    return ATRACK_TCB_QUAT;
}
template <> EAnimValue TAnimTcbTrack<Quat>::GetValueType() {
    return AVALUE_QUAT;
}
