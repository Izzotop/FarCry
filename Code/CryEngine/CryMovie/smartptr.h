...
//
///////////////////////////////////////////////////////////////////////////////
template <class _T> class TSmartPtr {
    _T* p;

public:
    TSmartPtr() : p(nullptr) {}
    TSmartPtr(_T* p_) : p(p_) {
        if (p)
            (p)->AddRef();
    }
    TSmartPtr(const TSmartPtr<_T>& p_) : p(p_.p) {
        if (p)
            (p)->AddRef();
    } // Copy constructor.
    TSmartPtr(int dummy) : p(nullptr) {}
    ~TSmartPtr() {
        if (p)
            (p)->Release();
    }
    ...
    bool operator==(const _T* p2) const {
        return p == p2;
    };
    bool operator==(_T* p2) const {
        return p == p2;
    };
    bool operator!=(const _T* p2) const {
        return p != p2;
    ...
    bool operator==(const TSmartPtr<_T>& p2) const {
        return p == p2.p;
    };
    bool operator!=(const TSmartPtr<_T>& p2) const {
        return p != p2.p;
...
template <class T> inline bool operator==(const TSmartPtr<T>& p1, int dummy) {
    return !(bool)p1;
}
...
template <class T> inline bool operator==(int dummy, const TSmartPtr<T>& p1) {
    return !(bool)p1;
}
