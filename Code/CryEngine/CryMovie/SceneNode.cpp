...
//////////////////////////////////////////////////////////////////////////
CAnimSceneNode::CAnimSceneNode(IMovieSystem* sys) : CAnimNode(sys) {
    m_dwSupportedTracks = PARAM_BIT(APARAM_CAMERA) | PARAM_BIT(APARAM_EVENT) | PARAM_BIT(APARAM_SOUND1) | PARAM_BIT(APARAM_SOUND2) | PARAM_BIT(APARAM_SOUND3) |
    ...
    */
    ...
    for (int paramIndex = 0; paramIndex < paramCount; paramIndex++) {
        ...
        case APARAM_EVENT:
            ...
            break;
        case APARAM_SOUND1:
            pSoundTrack[0] = (CSoundTrack*)pTrack;
            break;
        case APARAM_SOUND2:
            pSoundTrack[1] = (CSoundTrack*)pTrack;
            break;
        case APARAM_SOUND3:
            pSoundTrack[2] = (CSoundTrack*)pTrack;
            break;
        case APARAM_SEQUENCE:
            pSequenceTrack = (CSelectTrack*)pTrack;
            break;
        case APARAM_CONSOLE:
...
void CAnimSceneNode::ApplyCameraKey(ISelectKey& key, SAnimContext& ec) {
    ...
    if (cameraNode) {
        ...
        if (strlen(key.szSelection) > 0) {
            m_pMovie->GetSystem()->Warning(VALIDATOR_MODULE_MOVIE, VALIDATOR_WARNING, 0, 0, "[CryMovie] Camera entity %s not found", (const char*)key.szSelection);
        }
    }
