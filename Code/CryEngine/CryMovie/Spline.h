...
****************************************************************************/
template <class T> class TCBSpline : public TSpline<TCBSplineKey<T>, HermitBasis> {
protected:
...
template <class T> inline void TCBSpline<T>::compMiddleDeriv(int curr) {
...
template <class T> inline void TCBSpline<T>::compFirstDeriv() {
    key_type& k = key(0);
    Zero(k.ds);
    k.dd = 0.5f * (1.0f - k.tens) * (3.0f * (value(1) - k.value) - ds(1));
}

template <class T> inline void TCBSpline<T>::compLastDeriv() {
    int last = num_keys() - 1;
    key_type& k = key(last);
...
template <class T> inline void TCBSpline<T>::comp2KeyDeriv() {
    key_type& k1 = key(0);
    key_type& k2 = key(1);
...
template <class T> inline void TCBSpline<T>::comp_deriv() {
    if (num_keys() > 1) {
...
template <class T> inline float TCBSpline<T>::calc_ease(float t, float a, float b) {
    float k;
    float s = a + b;
...
template <class T> inline void TCBSpline<T>::interp_keys(int from, int to, float u, T& val) {
...
template <> inline void TCBSpline<Quat>::compMiddleDeriv(int curr) {}

template <> inline void TCBSpline<Quat>::compFirstDeriv() {}

template <> inline void TCBSpline<Quat>::compLastDeriv() {}

template <> inline void TCBSpline<Quat>::comp2KeyDeriv() {}
