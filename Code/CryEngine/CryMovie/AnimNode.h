...
public:
    ...
    void SetName(const char* name) {
        ...
        if (nLen > sizeof(m_name) - 1)
            nLen = sizeof(m_name) - 1;
        m_name[nLen] = 0;
    ...
    virtual IAnimNode* GetParent() const {
        return m_parent;
    }
    ...
    void RegisterCallback(IAnimNodeCallback* callback) {
        m_callback = callback;
        m_pMovieSystem->Callback(CBR_REGISTERNODECB);
    };
    void UnregisterCallback(IAnimNodeCallback* callback) {
        m_callback = nullptr;
        m_pMovieSystem->Callback(CBR_UNREGISTERNODECB);
    };
