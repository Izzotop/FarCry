...
void* CnullptrRenderer::GetDynVBPtr(int nVerts, int& nOffs, int Pool) {
    return m_DynVB;
}

void CnullptrRenderer::DrawDynVB(int nOffs, int Pool, int nVerts) {}

void CnullptrRenderer::DrawDynVB(struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F* pBuf, ushort* pInds, int nVerts, int nInds, int nPrimType) {}

// allocates vertex buffer
void CnullptrRenderer::CreateBuffer(int size, int vertexformat, CVertexBuffer* buf, int Type, const char* szSource) {
...
CVertexBuffer* CnullptrRenderer::CreateBuffer(int vertexcount, int vertexformat, const char* szSource, bool bDynamic) {
...
///////////////////////////////////////////
void CnullptrRenderer::DrawBuffer(CVertexBuffer* src, SVertexStream* indicies, int numindices, int offsindex, int prmode, int vert_start, int vert_stop, CMatInfo* mi) {}

void CnullptrRenderer::SetFenceCompleted(CVertexBuffer* buffer) {}

///////////////////////////////////////////
// Updates the vertex buffer dest with the data from src
// NOTE: src may be nullptr, in which case the data will not be copied
void CnullptrRenderer::UpdateBuffer(CVertexBuffer* dest, const void* src, int vertexcount, bool bUnlock, int offs, int Type) {
    ...
    if (src && vertexcount) {
        ...
        if (vertexcount > dest->m_NumVerts) {
            iLog->Log("CnullptrRenderer::UpdateBuffer: vertexcount>dest->m_NumVerts");
            return;
        }
...
///////////////////////////////////////////
void CnullptrRenderer::ReleaseBuffer(CVertexBuffer* bufptr) {
    if (bufptr) {
...
///////////////////////////////////////////
void CnullptrRenderer::DrawTriStrip(CVertexBuffer* src, int vert_num) {}

void CLeafBuffer::DrawImmediately() {}

void CnullptrRenderer::CreateIndexBuffer(SVertexStream* dest, const void* src, int indexcount) {
...
void CnullptrRenderer::UpdateIndexBuffer(SVertexStream* dest, const void* src, int indexcount, bool bUnLock) {
...
void CnullptrRenderer::ReleaseIndexBuffer(SVertexStream* dest) {
    SAFE_DELETE_ARRAY(dest->m_VData);
    dest->Reset();
}
