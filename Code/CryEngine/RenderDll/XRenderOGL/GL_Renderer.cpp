...
void CGLRenderer::Update() {
    ...
    /*if (m_TexMan->m_Text_ScreenLuminosityMap->m_Flags & FT_ALLOCATED)
    {
      ...
      sDrawImage(0, 0, 1, 1, m_TexMan->m_Text_ScreenLuminosityMap->m_Bind, 0, 0, m_TexMan->m_Text_ScreenLuminosityMap->m_Width, m_TexMan->m_Text_ScreenLuminosityMap->m_Height,
    1,1,1,1);
...
const char* CGLRenderer::ErrorString(GLenum errorCode) {
    static char* tess_error[] = {"missing gluEndPolygon", "missing gluBeginPolygon", "misoriented contour", "vertex/edge intersection", "misoriented or self-intersecting loops",
                                 "coincident vertices",   "colinear vertices",       "intersecting edges",  "not coplanar contours"};
    static char* nurbs_error[] = {"spline order un-supported",
                                  "too few knots",
                                  ...
                                  "nullptr control array or knot vector",
...
//////////////////////////////////////////////////////////////////////
unsigned int CGLRenderer::DownLoadToVideoMemory(unsigned char* data, int w, int h, ETEX_Format eTFSrc, ETEX_Format eTFDst, int nummipmap, bool repeat, int filter, int Id,
    ...
    if (nummipmap) {
        glTexImage2D(tgt, 1 - nummipmap, dstformat, w, h, 0, srcformat, GL_UNSIGNED_BYTE, data);
    } else {
        ...
        if (eTFDst == eTF_DXT1) {
            ...
            glTexImage2D(tgt,
                         0, // 1-mlevels,
                         dstformat, w, h, 0, srcformat, GL_UNSIGNED_BYTE, data);
    }
...
//////////////////////////////////////////////////////////////////////
void CGLRenderer::Draw2dImage(float xpos, float ypos, float w, float h, int texture_id, float s0, float t0, float s1, float t1, float angle, float r, float g, float b, float a,
                              float z) {
    PROFILE_FRAME(Draw_2DImage);
...
void CGLRenderer::SetFog(float density, float fogstart, float fogend, const float* color, int fogmode) {
    ...
    else if (fogmode == R_FOGMODE_EXP2)
...
//////////////////////////////////////////////////////////////////////
void CGLRenderer::ProjectToScreen(float ptx, float pty, float ptz, float* sx, float* sy, float* sz) {
    ...
    SGLFuncs::gluProject(ptx, pty, ptz, g_ModelMatrix, g_ProjectionMatrix, g_Viewport, sx, sy, sz);
    (*sx) = (*sx) * 100 / m_width;
    (*sy) = 100 - (*sy) * 100 / m_height;
}

int CGLRenderer::UnProject(float sx, float sy, float sz, float* px, float* py, float* pz, const float modelMatrix[16], const float projMatrix[16], const int viewport[4]) {
    return SGLFuncs::gluUnProject(sx, sy, sz, modelMatrix, projMatrix, viewport, px, py, pz);
}

//////////////////////////////////////////////////////////////////////
int CGLRenderer::UnProjectFromScreen(float sx, float sy, float sz, float* px, float* py, float* pz) {
    float g_ProjectionMatrix[16];
    float g_ModelMatrix[16];
    int g_Viewport[4];
    ...
    int res = SGLFuncs::gluUnProject(sx, sy, sz, g_ModelMatrix, g_ProjectionMatrix, g_Viewport, px, py, pz);
    CheckError("unproj");
    return (res);
}
    ...
    for (int i = 0; i < m_numtmus; i++) {
        ...
        if (!i) {
            ...
            if (m_RP.m_ClipPlaneEnabled != 1 || i != 3) {
                EnableTexGen(false);
            } else if (m_RP.m_ClipPlaneEnabled == 1 && SUPPORTS_GL_NV_texture_shader) {
                if (i == 3)
...
char* CGLRenderer::GetStatusText(ERendStats type) {
    ...
    sprintf(sStatusText, "Busy=%d(%.1fmb), All=%d(%.1fmb), BuffSize=%.1fmb", num_used, float(size_used / 1024) / 1024, num_all, float(size_all / 1024) / 1024,
...
int crtAllocHook(int nAllocType, void* pvData, size_t nSize, int nBlockUse, long lRequest, const unsigned char* szFileName, int nLine) {
