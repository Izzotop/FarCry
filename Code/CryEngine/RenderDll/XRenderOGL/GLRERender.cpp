...
bool CREOcclusionQuery::mfDraw(SShader* ef, SShaderPass* sfm) {
    ...
    } else if (SUPPORTS_GL_HP_occlusion_test && CGLRenderer::CV_gl_hp_occlusion_test) {
        if (m_nCheckFrame != nFrame) {
            if (m_nCheckFrame)
...
bool CREFlashBang::mfDraw(SShader* ef, SShaderPass* sfm) {
    ...
    static struct_VERTEX_FORMAT_P3F_TEX2F pData[] = {
        Vec3(800, 600, 0), 1, 1.f - 1, Vec3(800, 0, 0), 1, 1.f - 0, Vec3(0, 600, 0), 0, 1.f - 1, Vec3(0, 0, 0), 0, 1.f - 0,
    };
...
bool CREGlare::mfDraw(SShader* ef, SShaderPass* sfm) {
    ...
    // screen aligned quad
    static struct_VERTEX_FORMAT_P3F_TEX2F pData[] = {
        Vec3(800, 600, 0), 1, 0, Vec3(800, 0, 0), 1, 1, Vec3(0, 600, 0), 0, 0, Vec3(0, 0, 0), 0, 1,
    };
...
bool CREOcLeaf::mfDraw(SShader* ef, SShaderPass* sl) {
    ...
    if (!lb->m_pVertexBuffer) {
        iLog->Log("Warning: CREOcLeaf::mfDraw m_pBuffer->m_pVertexBuffer==0 '%s'", lb->m_sSource != 0 ? lb->m_sSource : "<nullptr>");
        return true;
    }
...
bool CREFlare::mfCheckVis(CCObject* obj) {
    ...
    if (m_Importance <= CRenderer::CV_r_coronas) {
        ...
        gluPerspective(rd->m_prevCamera.GetFov() / (gf_PI / 180.0f) * rd->m_prevCamera.GetProjRatio(), 1.0f / rd->m_prevCamera.GetProjRatio(), rd->m_prevCamera.GetZMin(),
                       rd->m_prevCamera.GetZMax());
