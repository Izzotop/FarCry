...
void CGLRenderer::CreateBuffer(int size, int vertexformat, CVertexBuffer* buf, int Type, const char* szSource) {
        ...
        if (!data) {
            ...
            while (!data) {
                if (pLB == &CLeafBuffer::m_Root) {
                    ...
                    iConsole->Exit("Error: Pipeline buffer overflow. Current geometry is too-oo-oo big (%s) (See LogVBuffers.txt for more info)",
                                   gRenDev->GetStatusText(eRS_VidBuffer));
                }
    ...
    } else if (SUPPORTS_GL_ARB_vertex_buffer_object) {
    ...
    if (IsVarPresent()) {
        ...
        if (!vtemp->m_VS[VSF_GENERAL].m_VData) {
            ...
            while (!vtemp->m_VS[VSF_GENERAL].m_VData) {
                if (pLB == &CLeafBuffer::m_Root) {
                    GenerateVBLog("LogVBuffers.txt");
                    iConsole->Exit("Error: Pipeline buffer overflow. Current geometry cannot fit in video memory (%s) (See LogVBuffers.txt for more info)",
                                   gRenDev->GetStatusText(eRS_VidBuffer));
                }
...
void CGLRenderer::InitVAR() {
    ...
    while (1) { // if first try fails - try to allocate less memory
        iLog->Log("Allocating %.2f MB in %s memory ... ", (float)(m_pip_buffer_size / 1024 / 1024), (pip_allow_VAR == 1.f) ? "Video" : "AGP");
...
void* CGLRenderer::AllocateVarShunk(int bytes_count, const char* szSource) {
    ...
    if (best_i >= 0) { // use best free chunk
...
void CGLRenderer::GenerateVBLog(const char* szName) {
    ...
    for (i = 0; i < LVs.Num(); i++) {
        ...
        else if (strcmp(szLast, LVs[i].szSource)) {
