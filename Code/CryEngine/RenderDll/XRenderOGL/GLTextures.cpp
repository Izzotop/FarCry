...
void STexPic::SaveTGA(const char* nam, bool bMips) {
    ...
    const GLenum cubefaces[6] = {
        GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT, GL_TEXTURE_CUBE_MAP_NEGATIVE_X_EXT, GL_TEXTURE_CUBE_MAP_POSITIVE_Y_EXT,
        GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_EXT, GL_TEXTURE_CUBE_MAP_POSITIVE_Z_EXT, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_EXT,
    };
...
int SShaderTexUnit::mfSetTexture(int nt) {
            ...
            if (bind >= TX_FIRSTBIND) {
                ...
                case TO_ENVIRONMENT_TEX: {
                    ...
                    if ((gRenDev->m_RP.m_pShader->m_Flags3 & (EF3_CLIPPLANE_FRONT | EF3_REFLECTION)))
                        bReflect = true;
                    cm = gRenDev->m_cEF.mfFindSuitableEnvTex(Pos, Angs, true, 0, false, gRenDev->m_RP.m_pShader, gRenDev->m_RP.m_pShaderResources, gRenDev->m_RP.m_pCurObject,
                                                             bReflect, gRenDev->m_RP.m_pRE);
                    if (cm)
                        cm->m_Tex->Set();
                    else
                        return false;
                ...
                default: {
                    if (bind >= TO_CUSTOM_CUBE_MAP_FIRST && bind <= TO_CUSTOM_CUBE_MAP_LAST) {
                        ...
                        cm->m_Tex->Set();
                    } else if (pSTU->m_TexPic->m_TargetType)
                        pSTU->m_TexPic->Set();
                } break;
                }
            }
    ...
    } else if (CGLTexMan::m_TUState[gRenDev->m_TexMan->m_CurStage].m_Target) {
...
bool CGLTexMan::SetFilter(char* tex) {
    ...
    static textype tt[] = {{"GL_NEAREST", GL_NEAREST, GL_NEAREST},
...
_inline uint filter3x3(uint* data, int x, int y, int dx, int dy) {
    ...
    static int filter[3][3] = {{0, 1, 0}, {1, 2, 1}, {0, 1, 0}};
...
static inline bool IsDXTFormat(int format) {
    if (format == GL_COMPRESSED_RGB_S3TC_DXT1_EXT || format == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT || format == GL_COMPRESSED_RGBA_S3TC_DXT3_EXT ||
...
void CGLTexMan::BuildMips(GLenum tgt, byte* src, int wdt, int hgt, int depth, STexPic* ti, int srcFormat, int dstFormat, int blockSize, int DXTSize, int nMips) {
    ...
    if (nMips) {
        ...
        if (dstFormat == GL_DSDT_MAG_NV) {
        ...
        } else if (dstFormat == GL_COLOR_INDEX8_EXT && srcFormat == GL_COLOR_INDEX) {
            for (int l = 0; l < nMips; l++) {
                if (!w)
                    w = 1;
                if (!h)
            ...
            } else if (IsDXTFormat(dstFormat) && dstFormat == srcFormat) {
                int l = 0;
                int size;
                while (w > 0 || h > 0) {
                    if (offset >= DXTSize)
    ...
    } else if (CRenderer::CV_r_texhwmipsgeneration && SUPPORTS_GL_SGIS_generate_mipmap) {
...
STexPic* CGLTexMan::CreateTexture(const char* name, int wdt, int hgt, int depth, uint flags, uint flags2, byte* dst, ETexType eTT, float fAmount1, float fAmount2, int DXTSize,
                ...
                if (ti->m_eTT == eTT_Bumpmap) {
                    if (CRenderer::CV_r_texbumpquality == 0) {
                        if (ti->m_Flags & FT_HASALPHA)
                            ...
                            dstFormat = GL_RGB8;
                    } else if (CRenderer::CV_r_texbumpquality == 1 && SUPPORTS_GL_EXT_paletted_texture) {
                    ...
                    } else if ((CRenderer::CV_r_texbumpquality == 2 || strstr(ti->m_SourceName.c_str(), "_cct")) && SUPPORTS_GL_ARB_texture_compression) {
                        dstFormat = GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
                    ...
                    if (SUPPORTS_GL_NV_texture_shader)
                        ...
                        dstFormat = GL_RGB8;
                } else if (ti->m_Flags & FT_SKY) {
                    if (CRenderer::CV_r_texskyquality == 0)
                        dstFormat = GL_RGB8;
                    else if (CRenderer::CV_r_texskyquality == 1)
                        dstFormat = GL_RGB5;
                    else if (CRenderer::CV_r_texskyquality == 2 && SUPPORTS_GL_ARB_texture_compression)
                        dstFormat = GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
                    else
                        dstFormat = GL_RGB8;
                } else if (!(ti->m_Flags & FT_HASALPHA)) {
                    if (CRenderer::CV_r_texquality == 0)
                        dstFormat = GL_RGB8;
                    else if (CRenderer::CV_r_texquality == 1)
                        dstFormat = GL_RGB5;
                    else if (CRenderer::CV_r_texquality == 2 && SUPPORTS_GL_ARB_texture_compression)
                        dstFormat = GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
                    else
                        dstFormat = GL_RGB8;
...
inline void _multMatrices(float* dst, const float* a, const float* b) {
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            dst[i * 4 + j] = b[i * 4 + 0] * a[0 * 4 + j] + b[i * 4 + 1] * a[1 * 4 + j] + b[i * 4 + 2] * a[2 * 4 + j] + b[i * 4 + 3] * a[3 * 4 + j];
        }
    }
}
...
void CGLTexMan::Update() {
    ...
    if (CRenderer::CV_r_texresolution != m_CurTexResolution || CRenderer::CV_r_texbumpresolution != m_CurTexBumpResolution || CRenderer::CV_r_texquality != m_CurTexQuality ||
        CRenderer::CV_r_texbumpquality != m_CurTexBumpQuality || CRenderer::CV_r_texskyquality != m_CurTexSkyQuality || CRenderer::CV_r_texskyresolution != m_CurTexSkyResolution ||
        CRenderer::CV_r_texmaxsize != m_CurTexMaxSize || CRenderer::CV_r_texminsize != m_CurTexMinSize) {
        for (i = 0; i < m_Textures.Num(); i++) {
            ...
            if (tp->m_eTT == eTT_Cubemap) {
                // if (tp->m_CubeSide > 0)
                continue;
                if (CRenderer::CV_r_texmaxsize != m_CurTexMaxSize || CRenderer::CV_r_texminsize != m_CurTexMinSize || CRenderer::CV_r_texresolution != m_CurTexResolution ||
                    CRenderer::CV_r_texquality != m_CurTexQuality) {
                ...
                if (CRenderer::CV_r_texmaxsize != m_CurTexMaxSize || CRenderer::CV_r_texminsize != m_CurTexMinSize || CRenderer::CV_r_texbumpresolution != m_CurTexBumpResolution ||
                    CRenderer::CV_r_texbumpquality != m_CurTexBumpQuality) {
                    gRenDev->EF_LoadTexture(tp->m_SearchName.c_str(), tp->m_Flags, tp->m_Flags2 | FT2_RELOAD, tp->m_eTT, tp->m_fAmount1, tp->m_fAmount2, tp->m_Id);
                }
            } else {
                if (tp->m_Flags & FT_SKY) {
                    if (CRenderer::CV_r_texmaxsize != m_CurTexMaxSize || CRenderer::CV_r_texminsize != m_CurTexMinSize ||
                        CRenderer::CV_r_texskyresolution != m_CurTexSkyResolution || CRenderer::CV_r_texskyquality != m_CurTexSkyQuality) {
                        gRenDev->EF_LoadTexture(tp->m_SearchName.c_str(), tp->m_Flags, tp->m_Flags2 | FT2_RELOAD, tp->m_eTT, tp->m_fAmount1, tp->m_fAmount2, tp->m_Id);
                    }
                } else {
                    if (CRenderer::CV_r_texmaxsize != m_CurTexMaxSize || CRenderer::CV_r_texminsize != m_CurTexMinSize || CRenderer::CV_r_texresolution != m_CurTexResolution ||
    ...
    if (CRenderer::CV_r_logusedtextures == 1 || CRenderer::CV_r_logusedtextures == 3 || CRenderer::CV_r_logusedtextures == 4 || CRenderer::CV_r_logusedtextures == 5) {
        ...
        static char* sTexType[] = {"Base", "Cubemap", "AutoCubemap", "Bump", "DSDTBump", "Rectangle"};
        ...
        if (CRenderer::CV_r_logusedtextures == 1 || CRenderer::CV_r_logusedtextures == 3 || CRenderer::CV_r_logusedtextures == 5) {
            for (i = 0; i < m_Textures.Num(); i++) {
                if (CRenderer::CV_r_logusedtextures == 3 && m_Textures[i] && m_Textures[i]->m_bBusy && m_Textures[i]->m_Bind == gRenDev->m_TexMan->m_Text_NoTexture->m_Bind) {
                    ...
                    if (m_Textures[i]->m_eTT != eTT_Cubemap || !m_Textures[i]->m_CubeSide)
                        Texs.AddElem(m_Textures[i]);
                } else if (CRenderer::CV_r_logusedtextures == 5 && m_Textures[i] && m_Textures[i]->m_bBusy &&
                           m_Textures[i]->m_Bind != gRenDev->m_TexMan->m_Text_NoTexture->m_Bind && !(m_Textures[i]->m_Flags2 & FT2_WASLOADED)) {
                    if (m_Textures[i]->m_eTT != eTT_Cubemap || !m_Textures[i]->m_CubeSide)
                        Texs.AddElem(m_Textures[i]);
                }
            ...
            else if (CRenderer::CV_r_logusedtextures == 1)
...
bool CGLRenderer::EF_SetLightHole(Vec3d vPos, Vec3d vNormal, int idTex, float fScale, bool bAdditive) {
    ...
    SGLFuncs::gluLookAt(PosL.x, PosL.y, PosL.z, TargL.x, TargL.y, TargL.z, pLight->m_Orientation.m_vUp[0], pLight->m_Orientation.m_vUp[1], pLight->m_Orientation.m_vUp[2],
                        viewMatr);
...
void CGLTexMan::GenerateFogMaps() {
    int i, j;
    {
        ...
        gRenDev->m_TexMan->m_Text_Fog =
            CreateTexture("$Fog", 128, 128, 1, FT_CLAMP | FT_NOMIPS | FT_NOREMOVE | FT_HASALPHA, FT2_NODXT, &Data1[0][0][0], eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8888);
...
void CGLTexMan::GenerateDepthLookup() {
    ...
    gRenDev->m_TexMan->m_Text_DepthLookup =
        CreateTexture("$DepthMap", 2048, 1, 1, FT_CLAMP | FT_NOREMOVE | FT_NOMIPS | FT_PROJECTED, FT2_NODXT, (byte*)&data[0], eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8888);
    ...
    gRenDev->m_TexMan->m_Text_WhiteShadow =
        CreateTexture("$WhiteShadow", 4, 4, 1, FT_CLAMP | FT_NOREMOVE | FT_HASALPHA | FT_NOMIPS, FT2_NODXT, (byte*)&data2[0], eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8888);
    ...
    gRenDev->m_TexMan->m_Text_Gradient =
        CreateTexture("$AlphaGradient", 256, 1, 1, FT_CLAMP | FT_NOREMOVE | FT_HASALPHA | FT_NOMIPS, FT2_NODXT, (byte*)&data3[0], eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8000);
}
