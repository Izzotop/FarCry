...
bool CShader::mfCompileHWShadeLayer(SShader* ef, char* scr, TArray<SShaderPassHW>& Layers) {
    ...
    static tokenDesc commands[] = {{eLayer, "Layer"},
    ...
    while ((cmd = shGetObject(&scr, commands, &name, &params)) > 0) {
        ...
        case eLightType:
            ...
            else if (!strnicmp(data, "Point", 5))
                sm->m_LightFlags |= DLF_POINT;
            else if (!strnicmp(data, "Project", 7)) {
                ef->m_Flags |= EF_USEPROJLIGHTS;
                sm->m_LightFlags |= DLF_PROJECT;
            } else if (!strnicmp(data, "OnlySpec", 8))
...
void CShader::mfCompileLayers(SShader* ef, char* scr, TArray<SShaderPassHW>& Layers, EShaderPassType eType) {
    ...
    static tokenDesc commands[] = {{eShadeLayer, "ShadeLayer"},
                                   {ePass, "Pass"},
...
void CShader::mfCompileHWConditions(SShader* ef, char* scr, SShaderTechnique* hs, int Id) {
    ...
    static tokenDesc commands[] = {{eMultipleLights, "MultipleLights"},
...
#define new DEBUG_CLIENTBLOCK
    ...
    static tokenDesc commands[] = {{eDeclareCGScript, "DeclareCGScript"},
...
void SGenTC_ObjectLinear::mfCompile(char* scr, SShader* ef) {
    ...
    static tokenDesc commands[] = {{eMask, "Mask"}, {ePlaneS, "PlaneS"}, {ePlaneT, "PlaneT"}, {ePlaneR, "PlaneR"}, {ePlaneQ, "PlaneQ"}, {eComponents, "Components"},
...
void SGenTC_EyeLinear::mfCompile(char* scr, SShader* ef) {
    ...
    static tokenDesc commands[] = {{eMask, "Mask"}, {ePlaneS, "PlaneS"}, {ePlaneT, "PlaneT"}, {ePlaneR, "PlaneR"}, {ePlaneQ, "PlaneQ"},
...
bool CShader::mfCompileTexGen(char* name, char* params, SShader* ef, SShaderTexUnit* ml) {
    ...
    } else if (!strnicmp(name, "HW_Reflection", 13)) {
        ml->m_GTC = (SGenTC*)new SGenTC_ReflectionMap;
        ml->m_GTC->mfCompile(params, ef);
    } else if (!stricmp(name, "HW_ObjectLinear")) {
        ml->m_GTC = (SGenTC*)new SGenTC_ObjectLinear;
        ml->m_GTC->mfCompile(params, ef);
    } else if (!stricmp(name, "HW_EyeLinear")) {
        ml->m_GTC = (SGenTC*)new SGenTC_EyeLinear;
        ml->m_GTC->mfCompile(params, ef);
    } else if (!stricmp(name, "HW_SphereMap")) {
        ml->m_GTC = (SGenTC*)new SGenTC_SphereMap;
        ml->m_GTC->mfCompile(params, ef);
    } else if (!stricmp(name, "HW_EmbossMap")) {
...
void CShader::mfCompileMatrixOp(TArray<SMatrixTransform>* List, char* scr, char* nmMat, SShader* ef) {
    ...
    if (!stricmp(nmMat, "GL_TEXTURE"))
        Matr = GL_TEXTURE;
    else if (!stricmp(nmMat, "GL_MATRIX0_NV"))
        Matr = GL_MATRIX0_NV;
    else if (!stricmp(nmMat, "GL_MATRIX1_NV"))
        Matr = GL_MATRIX1_NV;
    else if (!stricmp(nmMat, "GL_MATRIX2_NV"))
        Matr = GL_MATRIX2_NV;
    else if (!stricmp(nmMat, "GL_MATRIX3_NV"))
        Matr = GL_MATRIX3_NV;
    else if (!stricmp(nmMat, "GL_MATRIX4_NV"))
        Matr = GL_MATRIX4_NV;
    else if (!stricmp(nmMat, "GL_MATRIX5_NV"))
        Matr = GL_MATRIX5_NV;
    else if (!stricmp(nmMat, "GL_MATRIX6_NV"))
        Matr = GL_MATRIX6_NV;
    else if (!stricmp(nmMat, "GL_MATRIX7_NV"))
        Matr = GL_MATRIX7_NV;
    else {
...
void SArrayPointer_Vertex::mfSet(int Id) {
    if (gRenDev->m_RP.m_pRE) {
        ...
        } else if (p != m_pLastPointer || m_nFrameCreateBuf != gcpOGL->m_nFrameCreateBuf) {
            m_nFrameCreateBuf = gcpOGL->m_nFrameCreateBuf;
            m_pLastPointer = p;
...
void SArrayPointer_Normal::mfSet(int Id) {
    ...
    if (gRenDev->m_RP.m_pRE) {
        ...
        } else if (p && (p != m_pLastPointer || m_nFrameCreateBuf != gcpOGL->m_nFrameCreateBuf)) {
            m_nFrameCreateBuf = gcpOGL->m_nFrameCreateBuf;
            m_pLastPointer = p;
...
void SArrayPointer_Texture::mfSet(int Id) {
    ...
    if (gRenDev->m_RP.m_pRE) {
        ...
        } else if (p != m_pLastPointer[Stage] || m_nFrameCreateBuf[Stage] != gcpOGL->m_nFrameCreateBuf) {
            m_nFrameCreateBuf[Stage] = gcpOGL->m_nFrameCreateBuf;
