...
#define TEXEL_TO_SCREEN(size) ((size) == 0) ? 0.0f : 0.5f / ((float)(size))
...
bool BlurTextureHw(CScreenVars* pVars, CGLRenderer* pRenderer, STexPic*& pTex, int iBlurType, int iBlurAmount, bool bDefineRT, bool bRestoreRT) {
    ...
    struct_VERTEX_FORMAT_P3F_TEX2F pScreenBlur[] = {
        Vec3(1, 1, 0),
        (float)pTex->m_Width + s_off,
        0,
        Vec3(1, 0, 0),
        (float)pTex->m_Width + s_off,
        (float)pTex->m_Height + t_off,
        Vec3(0, 1, 0),
        0,
        0,
        Vec3(0, 0, 0),
...
bool ResizeTextureHw(CScreenVars* pVars, CGLRenderer* pRenderer, STexPic*& pSrc, STexPic*& pDst) {
    ...
    // setup screen aligned quad
    struct_VERTEX_FORMAT_P3F_TEX2F pScreenBlur[] = {
        Vec3(1, 1, 0), (float)pSrc->m_Width, 0, Vec3(1, 0, 0), (float)pSrc->m_Width, (float)pSrc->m_Height, Vec3(0, 1, 0), 0, 0, Vec3(0, 0, 0), 0, (float)pSrc->m_Height,
    };
...
inline float InterpolateCubic(float fCp1, float fCp2, float fCp3, float fCp4, float fTime) {
    ...
    return (((-fCp1 * 2.0f) + (fCp2 * 5.0f) - (fCp3 * 4) + fCp4) / 6.0f) * fTimeSquare * fTime + (fCp1 + fCp3 - (2.0f * fCp2)) * fTimeSquare +
           (((-4.0f * fCp1) + fCp2 + (fCp3 * 4.0f) - fCp4) / 6.0f) * fTime + fCp2;
}
...
bool CREScreenProcess::mfDrawLowSpec(SShader* ef, SShaderPass* sfm) {
    ...
    struct_VERTEX_FORMAT_P3F_TEX2F pScreenQuad[] = {
        Vec3(0, 0, 0), fSoff, fToff, Vec3(0, 1, 0), fSoff, 1 + fToff, Vec3(1, 0, 0), 1 + fSoff, fToff, Vec3(1, 1, 0), 1 + fSoff, 1 + fToff,
    };
    ...
    if (m_pVars->m_iNightVisionActive) {
        ...
        struct_VERTEX_FORMAT_P3F_TEX2F pScrQuad[] = {
            Vec3(0, 0, 0), 0, fNoise, Vec3(0, 1, 0), 0, 2 + fNoise, Vec3(1, 0, 0), 2, 0 + fNoise, Vec3(1, 1, 0), 2, 2 + fNoise,
        };
...
bool CREScreenProcess::mfDraw(SShader* ef, SShaderPass* sfm) {
    ...
    // setup shared screen aligned quad
    struct_VERTEX_FORMAT_P3F_TEX2F pScreenQuad[] = {
        Vec3(1, 1, 0), 1, 0, Vec3(1, 0, 0), 1, 1, Vec3(0, 1, 0), 0, 0, Vec3(0, 0, 0), 0, 1,
    };

    // setup shared screen sized aligned quad
    struct_VERTEX_FORMAT_P3F_TEX2F pScreenSizeQuad[] = {
        Vec3(1, 1, 0),
        (float)pScreenTex->m_Width,
        0,
        Vec3(1, 0, 0),
        (float)pScreenTex->m_Width,
        (float)pScreenTex->m_Height,
        Vec3(0, 1, 0),
        0,
        0,
        Vec3(0, 0, 0),
    ...
    {
        STexPic *pScreenLow = gRenDev->m_TexMan->m_Text_ScreenLowMap, *pScreenAvg = gRenDev->m_TexMan->m_Text_ScreenAvg1x1;
        // update screen texture..
    ...
    if ((m_pVars->m_iNightVisionActive && m_pVars->m_bCartoonActive) || (m_pVars->m_bGlareActive && !m_pVars->m_bCartoonActive) || m_pVars->m_iNightVisionActive) {
        ...
        if (!m_pVars->m_iNightVisionActive) {
            ...
            STexPic *pTex = gRenDev->m_TexMan->m_Text_Glare, *pScreenLuminosityTex = gRenDev->m_TexMan->m_Text_ScreenLuminosityMap,
                    *pScreenLow = gRenDev->m_TexMan->m_Text_ScreenLowMap, *pScreenAvg = gRenDev->m_TexMan->m_Text_ScreenAvg1x1;
            ...
            // get data
            STexPic *pTex = gRenDev->m_TexMan->m_Text_Glare, *pScreenLow = gRenDev->m_TexMan->m_Text_ScreenLowMap, *pHeatTexture = gRenDev->m_TexMan->m_Text_ScreenLowMap;
