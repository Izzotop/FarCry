...
void CGLRenderer::ScanOutSpaceCube(uint& nTexID, const Vec3d& vPos, int* pResult) {
    ...
    // is images valid
    if (GetDistance(vPrevPos[nSide], iSystem->GetViewCamera().GetPos()) < 1.0f && GetDistance(vPrevPos[nSide2], iSystem->GetViewCamera().GetPos()) < 1.0f)
    ...
    static const GLenum OutSpaceCubeFaces[6] = {
        GL_TEXTURE_CUBE_MAP_POSITIVE_Z_EXT, GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_EXT,
        GL_TEXTURE_CUBE_MAP_NEGATIVE_X_EXT, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_EXT, GL_TEXTURE_CUBE_MAP_POSITIVE_Y_EXT,
    };

    static const float sAngles[6][5] = {{0, 180, 0, 0, 0}, {0, 180, 90, 1, 0}, {0, 180, 180, 2, 0}, {0, 180, 270, 0, 1}, {-90, 180, 0, 1, 1}, {90, 180, 0, 2, 1}};

    if (!nTexID) { // create empty if not created
...
bool CREOutSpace::mfDraw(SShader* ef, SShaderPass* sfm) {
    ...
    { // top
        struct_VERTEX_FORMAT_P3F_TEX2F data[] = {
            D, -D, D, 1, 1.f - 1, -D, -D, D, 0, 1.f - 1, D, D, D, 1, 1.f - 0, -D, D, D, 0, 1.f - 0,
        };
    ...
    { // s
        struct_VERTEX_FORMAT_P3F_TEX2F data[] = {
            -D, -D, D, 1.0, 1.f - 1.0, D, -D, D, 0.0, 1.f - 1.0, -D, -D, -D, 1.0, 1.f - 0.0, D, -D, -D, 0.0, 1.f - 0.0,
        };
    ...
    { // e
        struct_VERTEX_FORMAT_P3F_TEX2F data[] = {
            -D, D, D, 1.0, 1.f - 0.0, -D, -D, D, 0.0, 1.f - 0.0, -D, D, -D, 1.0, 1.f - 0.0, -D, -D, -D, 0.0, 1.f - 0.0,
        };
