        ...
        for (float y = 0; nShadowId < MAX_DYNAMIC_SHADOW_MAPS_COUNT && y < height - 10; y += fPicDimY) {
            ...
            if (pInf->nTexId && pInf->pOwner) {
                ...
                if (tp) {
                    ...
                    TextToScreen(x / width * 100.f, y / height * 100.f, "%8s", pInf->pOwner->GetName());
...
void CGLRenderer::BlurImage(int nSizeX, int nSizeY, int nType, int nTexIdSrc, int nTexIdTmp) {
    if (nType == 1) {
        ...
        struct_VERTEX_FORMAT_P3F_TEX2F pScreenBlur[] = {
            Vec3(0, 1, 0), 0, 0, Vec3(0, 0, 0), 0, 1, Vec3(1, 1, 0), 1, 0, Vec3(1, 0, 0), 1, 1,
        };
...
void CGLRenderer::PrepareDepthMap(ShadowMapFrustum* lof, bool make_new_tid) {
    ...
    glMatrixMode(GL_MODELVIEW);
    SGLFuncs::gluLookAt(lof->pLs->vSrcPos.x, lof->pLs->vSrcPos.y, lof->pLs->vSrcPos.z, lof->target.x, lof->target.y, lof->target.z, 0, 0, 1, lightViewMatrix);
    glLoadMatrixf(lightViewMatrix);
    // iLog->Log("Generate SM: From (%.3f,%.3f,%.3f) To (%.3f,%.3f,%.3f), FOV: %.3f, MinD: %.3f, MaxD: %.3f", lof->pLs->vSrcPos.x, lof->pLs->vSrcPos.y, lof->pLs->vSrcPos.z,
    // lof->target.x, lof->target.y, lof->target.z, lof->FOV, lof->min_dist, lof->max_dist);
    ...
    if (make_new_tid) { // new id for static objects
        ...
        if (lof->nTexIdSlot >= 0 && m_ShadowTexIDBuffer[lof->nTexIdSlot].nTexId == lof->depth_tex_id && m_ShadowTexIDBuffer[lof->nTexIdSlot].nTexSize == nShadowTexSize &&
    ...
    if (bDepth) {
        glCopyTexImage2D(GL_TEXTURE_2D, 0, bDepth ? (m_zbpp == 24 ? GL_DEPTH_COMPONENT24_SGIX : GL_DEPTH_COMPONENT16_SGIX) : GL_DEPTH_COMPONENT, 0, 0, nShadowTexSize,
                         nShadowTexSize, 0);
    } else if (lof->shadow_type == EST_PENUMBRA) { // make penumbra texture
        int width = nShadowTexSize;
        int height = nShadowTexSize;
...
void CGLRenderer::SetupShadowOnlyPass(int Num, ShadowMapFrustum* pFrustum, Vec3d* vShadowTrans, const float fShadowScale, Vec3d vObjTrans, float fObjScale, const Vec3d vObjAngles,
                                      Matrix44* pObjMat) {
    if (!pFrustum)
        return;
    ...
    if (vShadowTrans) { // make tmp matrix for this obj position if shadow frustum is not translated (translate original mats)
        ...
        SGLFuncs::gluLookAt(pFrustum->pLs->vSrcPos.x + mv_trans.x, pFrustum->pLs->vSrcPos.y + mv_trans.y, pFrustum->pLs->vSrcPos.z + mv_trans.z,
                            fShadowScale * pFrustum->target.x + mv_trans.x, fShadowScale * pFrustum->target.y + mv_trans.y, fShadowScale * pFrustum->target.z + mv_trans.z, 0, 0, 1,
                            mat);
...
void CGLRenderer::ConfigShadowTexgen(int Num, int rangeMap, ShadowMapFrustum* pFrustum, float* pLightFrustumMatrix, float* pLightViewMatrix) {
    ...
    if (rangeMap) {
        ...
        float Smatrix[16] = {0.5f, 0, 0, 0, 0, 0.5f, 0, 0, 0, 0, 0.5f, 0, 0.5f, 0.5f, 0.5f, 1.0f};
