...
bool same_color(const GLfloat* rgba1, const GLfloat* rgba2) {
    if (rgba1[0] == rgba2[0] && rgba1[1] == rgba2[1] && rgba1[2] == rgba2[2] && rgba1[3] == rgba2[3])
        return true;
    return false;
}
...
string general_combiner_portion(GLenum combiner, GLenum portion) {
    ...
    if (p.output[0] == GL_DISCARD_NV && p.output[1] == GL_DISCARD_NV && p.output[2] == GL_DISCARD_NV)
        return s;
    ...
    for (int i = 0; i < 2; i++) {
        if (p.output[i] != GL_DISCARD_NV || p.output[2] != GL_DISCARD_NV) {
            ...
            if ((p.input[2 * i] == GL_ZERO && p.mapping[2 * i] == GL_UNSIGNED_INVERT_NV) && (p.input[2 * i + 1] != GL_ZERO)) {
                s += operand_string(p.input[2 * i + 1], p.mapping[2 * i + 1], p.compusg[2 * i + 1], portion);
            } else if ((p.input[2 * i + 1] == GL_ZERO && p.mapping[2 * i + 1] == GL_UNSIGNED_INVERT_NV) && (p.input[2 * i] != GL_ZERO)) {
...
final_combiner_state get_final_combiner_state() {
    GLenum v[7] = {GL_VARIABLE_A_NV, GL_VARIABLE_B_NV, GL_VARIABLE_C_NV, GL_VARIABLE_D_NV, GL_VARIABLE_E_NV, GL_VARIABLE_F_NV, GL_VARIABLE_G_NV};
...
string final_combiner() {
    ...
    if (fc.input[0] == GL_E_TIMES_F_NV || fc.input[1] == GL_E_TIMES_F_NV || fc.input[2] == GL_E_TIMES_F_NV || fc.input[3] == GL_E_TIMES_F_NV) {
    ...
    if (fc.clamp_color_sum &&
        (fc.input[1] == GL_SPARE0_PLUS_SECONDARY_COLOR_NV || fc.input[2] == GL_SPARE0_PLUS_SECONDARY_COLOR_NV || fc.input[3] == GL_SPARE0_PLUS_SECONDARY_COLOR_NV))
...
extern "C" char* state_to_rc10() {
    ...
    GLenum combiner[8] = {GL_COMBINER0_NV, GL_COMBINER1_NV, GL_COMBINER2_NV, GL_COMBINER3_NV, GL_COMBINER4_NV, GL_COMBINER5_NV, GL_COMBINER6_NV, GL_COMBINER7_NV};
    string state;
