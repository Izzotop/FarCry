...
void CGLRenderer::CheckGammaSupport() {
...
void CGLRenderer::RestoreDeviceGamma() {
    if (!(GetFeatures() & RFT_HWGAMMA))
        return;
...
#define GL_EXT(name)                                                                                                                                                               \
    if (AllowExt)                                                                                                                                                                  \
        SUPPORTS##name = FindExt(TEXT(#name) + 1);
...
bool CGLRenderer::CheckOGLExtensions() {
    ...
    else if (CV_gl_arb_texture_compression && CV_r_supportcompressedtextures) {
    ...
    else if (CV_gl_arb_render_texture)
    ...
    else if (CV_gl_3dfx_gamma_control)
    ...
    else if (CV_gl_arb_pixel_format)
    ...
    else if (CV_gl_arb_buffer_region)
    ...
    else if (CV_gl_arb_texture_env_combine)
    ...
    } else if (CV_gl_sgix_depth_texture) {
#ifdef WIN64
    ...
    else if (CV_gl_sgis_generate_mipmap)
    ...
    else if (CV_gl_sgis_texture_lod)
    ...
    else if (CV_gl_arb_multitexture) {
    ...
    else if (CV_gl_nv_texgen_reflection) {
    ...
    else if (CV_gl_hp_occlusion_test) {
    ...
    else if (CV_gl_nv_multisample_filter_hint)
    ...
    else if (CV_gl_nv_texgen_emboss) {
    ...
    else if (CV_gl_ext_separate_specular_color) {
    ...
    else if (CV_gl_ext_texture_env_combine)
    ...
    else if (CV_gl_ext_multi_draw_arrays) {
    ...
    else if (CV_gl_nv_texture_env_combine4) {
    ...
    else if (CV_gl_nv_vertex_array_range)
    ...
    else if (CV_gl_ext_compiled_vertex_array)
    ...
    else if (CV_gl_ext_texture_env_add)
    ...
    else if (CV_gl_ext_texture_filter_anisotropic) {
    ...
    else if (CV_gl_ext_depth_bounds_test)
    ...
    else if (CV_gl_ext_secondary_color) {
    ...
    else if (CV_gl_ext_paletted_texture && CV_r_supportpalettedtextures) {
    ...
    else if (CV_gl_ext_stencil_two_side)
    ...
    else if (CV_gl_ext_stencil_wrap)
    ...
    else if (CV_gl_nv_register_combiners) {
    ...
    else if (CV_gl_nv_register_combiners2) {
    ...
    else if (CV_gl_nv_vertex_program) {
    ...
    else if (CV_gl_arb_vertex_program) {
    ...
    else if (CV_gl_nv_texture_shader) {
    ...
    else if (CV_gl_nv_texture_shader2) {
    ...
    else if (CV_gl_nv_texture_shader3) {
    ...
    else if (CV_gl_nv_texture_rectangle) {
    ...
    else if (CV_gl_ext_texture_rectangle && !SUPPORTS_GL_NV_texture_rectangle) {
    ...
    else if (CV_gl_ext_texture_cube_map) {
    ...
    else if (CV_gl_ati_separate_stencil) {
    ...
    else if (CV_gl_ati_fragment_shader) {
    ...
    else if (CV_gl_arb_vertex_buffer_object || !SUPPORTS_GL_NV_fence) {
        SUPPORTS_GL_ATI_fragment_shader = 0;
        SUPPORTS_GL_NV_fence = 0;
        SUPPORTS_GL_NV_vertex_array_range = 0;
    ...
    else if (CV_gl_nv_fragment_program) {
    ...
    else if (CV_gl_arb_fragment_program && !SUPPORTS_GL_NV_fragment_program) {
    ...
    else if (CV_gl_nv_vertex_program3) {
    ...
    if (!SUPPORTS_GL_ARB_texture_env_combine)
        iLog->Log("  ...GL_ARB_texture_env_combine not found.\n");
    else if (CV_gl_arb_fragment_program) {
        iLog->Log("  ...using GL_ARB_texture_env_combine.\n");
    } else {
        SUPPORTS_GL_ARB_texture_env_combine = 0;
        iLog->Log("  ...ignoring GL_ARB_texture_env_combine.\n");
...
bool CGLRenderer::SetupPixelFormat(unsigned char colorbits, unsigned char zbits, unsigned char sbits, SRendContext* rc) {
    ...
    if (DescribePixelFormat(m_CurrContext->m_hDC, pixelFormat, sizeof(PIXELFORMATDESCRIPTOR), &pfd)) {
        if (pfd.dwFlags & PFD_DRAW_TO_WINDOW)
            iLog->LogToFile("PFD_DRAW_TO_WINDOW\n");
        if (pfd.dwFlags & PFD_DRAW_TO_BITMAP)
            iLog->LogToFile("PFD_DRAW_TO_BITMAP\n");

        if (pfd.dwFlags & PFD_SUPPORT_GDI)
            iLog->LogToFile("PFD_SUPPORT_GDI\n");

        if (pfd.dwFlags & PFD_SUPPORT_OPENGL)
            iLog->LogToFile("PFD_SUPPORT_OPENGL\n");
        if (pfd.dwFlags & PFD_GENERIC_ACCELERATED)
            iLog->LogToFile("PFD_GENERIC_ACCELERATED\n");

        if (pfd.dwFlags & PFD_NEED_PALETTE)
            iLog->LogToFile("PFD_NEED_PALETTE\n");
        if (pfd.dwFlags & PFD_NEED_SYSTEM_PALETTE)
            iLog->LogToFile("PFD_NEED_SYSTEM_PALETTE\n");
        if (pfd.dwFlags & PFD_DOUBLEBUFFER)
            iLog->LogToFile("PFD_DOUBLEBUFFER\n");
        if (pfd.dwFlags & PFD_STEREO)
            iLog->LogToFile("PFD_STEREO\n");
        if (pfd.dwFlags & PFD_SWAP_LAYER_BUFFERS)
            iLog->LogToFile("PFD_SWAP_LAYER_BUFFERS\n");
...
HWND CGLRenderer::SetMode(int x, int y, int width, int height, unsigned int cbpp, int zbpp, int sbits, bool fullscreen, HINSTANCE hinst, HWND Glhwnd) {
    ...
    for (int mode = 0; mode < m_numvidmodes; mode++) {
        if (((int)(m_vidmodes[mode].dmPelsWidth) >= (int)(m_vidmodes[best_mode].dmPelsWidth)) && ((int)(m_vidmodes[mode].dmPelsWidth) <= width) &&
            ((int)(m_vidmodes[mode].dmPelsHeight) >= (int)(m_vidmodes[best_mode].dmPelsHeight)) && ((int)(m_vidmodes[mode].dmPelsHeight) <= height) &&
            ((int)(m_vidmodes[mode].dmBitsPerPel) >= (int)(m_vidmodes[best_mode].dmBitsPerPel)) && ((int)(m_vidmodes[mode].dmBitsPerPel) <= cbpp))
            best_mode = mode;
    }
    ...
    if (fullscreen) {
        if (!ChangeDisplay(m_vidmodes[best_mode].dmPelsWidth, m_vidmodes[best_mode].dmPelsHeight, m_vidmodes[best_mode].dmBitsPerPel))
            return false;
        Glhwnd = CreateWindowEx(WS_EX_APPWINDOW, "CryENGINE", szWinTitle, WS_POPUP | WS_CLIPSIBLINGS, 0, 0, width, height, nullptr, nullptr, hinst, nullptr);
        ...
        if (!Glhwnd) {
            ...
            Glhwnd = CreateWindowEx(WS_EX_APPWINDOW, "CryENGINE", szWinTitle, WS_BORDER | WS_DLGFRAME | WS_CLIPSIBLINGS | WS_SYSMENU,
                                    (GetSystemMetrics(SM_CXFULLSCREEN) - width) / 2, (GetSystemMetrics(SM_CYFULLSCREEN) - height) / 2, GetSystemMetrics(SM_CXDLGFRAME) * 2 + width,
                                    GetSystemMetrics(SM_CYCAPTION) + GetSystemMetrics(SM_CXDLGFRAME) * 2 + height, nullptr, nullptr, hinst, nullptr);
        }
    ...
    if (!m_FullScreen)
        SetWindowPos(Glhwnd, HWND_NOTOPMOST, (GetSystemMetrics(SM_CXFULLSCREEN) - width) / 2, (GetSystemMetrics(SM_CYFULLSCREEN) - height) / 2,
...
WIN_HWND CGLRenderer::Init(int x, int y, int width, int height, unsigned int cbpp, int zbpp, int sbits, bool fullscreen, WIN_HINSTANCE hinst, WIN_HWND Glhwnd, WIN_HDC Glhdc,
