...
public:
    ...
    int mfGetCGInstanceID(int Type, int LightMask) {
        ...
        cg.m_dwHandle = nullptr;
        cg.m_dwHandleExt = nullptr;
    ...
    void mfInit() {
        ...
        if (!gcpOGL->m_CGContext) {
            ...
            assert(gcpOGL->m_CGContext != nullptr);
        }
#endif
        m_dwFrame = 1;
        m_CurInst = -1;
    ...
    void mfBind() {
        ...
        } else if (m_CGProfileType == CG_PROFILE_ARBFP1) {
    ...
    if (pr == CG_PROFILE_FP30 || pr == CG_PROFILE_ARBFP1)
    {
      ...
      return pBuf;
        ...
        char szCmdLine[512];
        sprintf(szCmdLine, "cgc.exe -profile %s -o $$out.cg $$in.cg", m_CGProfileType == CG_PROFILE_FP20 ? "fp20" : "arbfp1");
    ...
    void mfLoad(const char* prog_text) {
        if (m_CGProfileType == CG_PROFILE_ARBFP1) {
            ...
            if (sStr) {
                ...
                bool bUseTS = false;
                for (i = 0; i < nS; i++) {
                    if (strcmp(sLine[i], "texture_2d();") && strcmp(sLine[i], "nop();") && strcmp(sLine[i], "texture_3d();") && strcmp(sLine[i], "texture_cube_map();") &&
                        strcmp(sLine[i], "texture_rectangle();")) {
                        bUseTS = true;
    ...
    bool mfIsValid(int Num) const {
        return (m_Insts[m_CurInst].m_dwHandle != 0);
    }
