...
public:
    ...
    void RestoreDeviceGamma();
    void CheckGammaSupport();
    ...
    int UnProject(float sx, float sy, float sz, float* px, float* py, float* pz, const float modelMatrix[16], const float projMatrix[16], const int viewport[4]);
    int UnProjectFromScreen(float sx, float sy, float sz, float* px, float* py, float* pz);
...
#define MAX_DYNAMIC_SHADOW_MAPS_COUNT 128
    ...
    void SetupShadowOnlyPass(int Num, ShadowMapFrustum* pFrustum, Vec3d* vShadowTrans, const float fShadowScale, Vec3d vObjTrans = Vec3d(0, 0, 0), float fObjScale = 1.f,
