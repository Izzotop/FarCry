...
void CCGVProgram_D3D::mfAddFXParameter(SFXParam* pr, const char* ParamName, SShader* ef) {
    ...
    if (pr->m_Assign.size()) {
        ...
        else if (!stricmp(pr->m_Assign.c_str(), "World"))
            mt.m_eCGParamType = ECGP_Matr_World;
        else if (!stricmp(pr->m_Assign.c_str(), "WorldView") || !stricmp(pr->m_Assign.c_str(), "View"))
            mt.m_eCGParamType = ECGP_Matr_View;
        else if (!stricmp(pr->m_Assign.c_str(), "WorldViewInverse") || !stricmp(pr->m_Assign.c_str(), "ViewInverse"))
            mt.m_eCGParamType = ECGP_Matr_View_I;
        else if (!strnicmp(pr->m_Assign.c_str(), "ObjMatrix", strlen("ObjMatrix")))
            mt.m_eCGParamType = ECGP_Matr_Obj;
        else if (!strnicmp(pr->m_Assign.c_str(), "InvObjMatrix", strlen("InvObjMatrix")))
            mt.m_eCGParamType = ECGP_Matr_Obj_I;
        else if (!strnicmp(pr->m_Assign.c_str(), "TranspObjMatrix", strlen("TranspObjMatrix")))
            mt.m_eCGParamType = ECGP_Matr_Obj_T;
        else if (!stricmp(pr->m_Assign.c_str(), "InvTranspObjMatrix"))
...
void CCGVProgram_D3D::mfGatherFXParameters(const char* buf, SShaderPassHW* pSHPass, std::vector<SFXParam>& Params, std::vector<SFXSampler>& Samplers,
                                           std::vector<SFXTexture>& Textures, SShader* ef) {
    assert(m_Insts.Num());
                ...
                if (s) {
                    ...
                    while (true) {
                        ...
                        if (!stricmp(str, "COLOR0"))
                            bCol = true;
                        else if (!stricmp(str, "COLOR1"))
                            bSecCol = true;
                        else if (!stricmp(str, "TEXCOORD0"))
                            bTC0 = true;
                        else if (!stricmp(str, "TEXCOORD1"))
                            bTC1 = true;
                        else if (!stricmp(str, "NORMAL"))
                            bNormal = true;
                        else if (!strnicmp(str, "TANGENT", 7) || !strnicmp(str, "BINORMAL", 8) || !strnicmp(str, "BLENDWEIGHT", 11))
                            bTangent = true;
                        s++;
                    }
                    break;
...
SCGScript* CCGVProgram_D3D::mfGenerateScriptVP(CVProgram* pPosVP) {
    ...
    else if (m_Insts[m_CurInst].m_Mask & VPVST_INSTANCING_NOROT)
        ...
        if (sNewStr) {
            ...
            while (true) {
                if (sNewStr[n] == '(') {
                    ...
                    if (nDepth == 1) {
                        n--;
                        while (sNewStr[n] == 0x20 || sNewStr[n] == 9 || sNewStr[n] == 0xa) {
                            n--;
                        }
                        if (sNewStr[n] == ',')
                            sNewStr[n] = 0x20;
...
static char* sSphereMapGenScriptVP[] = {sEyeSpaceCamVecVP,
                                        sEyeSpaceNormal,
                                        sReflectVec,
                                        ...
                                        nullptr};
static char* sReflectionMapGenScriptVP[] = {sEyeSpaceCamVecVP,
                                            sEyeSpaceNormal,
                                            sReflectVec,
                                            {"float4 tcRM;\n"
                                             "tcRM.xyz = tcRef.xyz;\n"
                                             "tcRM.w = vPos.w;\n"},
                                            nullptr};
static char* sNormalMapGenScriptVP[] = {sEyeSpaceNormal,
                                        {"float4 tcNM;\n"
...
char* CCGVProgram_D3D::mfGenerateTCScript(char* Script, int nt) {
    ...
    if (m_Insts[m_CurInst].m_Mask & (tcGOLMask | tcGRMMask | tcGNMMask | tcGSMMask | tcMMask)) {
        ...
        if (sNewStr) {
            ...
            n = 0;
            while (sNewStr[n] != 0xa && sNewStr[n] != 0) {
                sNewStr[n] = 0x20;
                n++;
            }
            ...
            else if (m_Insts[m_CurInst].m_Mask & tcGRMMask)
                pNewScripts = sReflectionMapGenScriptVP;
            else if (m_Insts[m_CurInst].m_Mask & tcGSMMask)
                pNewScripts = sSphereMapGenScriptVP;
            else if (m_Insts[m_CurInst].m_Mask & tcGNMMask)
            ...
            if (m_Insts[m_CurInst].m_Mask & (tcGOLMask | tcMMask | tcGRMMask | tcGSMMask | tcGNMMask)) {
                ...
                if (sNewStr) {
                    n = 0;
                    while (sNewStr[n] != 0x20 && sNewStr[n] != 8 && sNewStr[n] != 0xa && sNewStr[n] != '(') {
                        n++;
                    }
                    while (sNewStr[n] == 0x20 || sNewStr[n] == 8 || sNewStr[n] == 0xa) {
                        n++;
                    }
                    if (sNewStr[n] == '(') {
                        int m = 0;
...
char* CCGVProgram_D3D::mfCreateAdditionalVP(CVProgram* pPosVP) {
    ...
    while (true) {
        ...
        n = 0;
        while (sNewStr[n] && sNewStr[n] != 0x20 && sNewStr[n] != 8 && sNewStr[n] != 9 && sNewStr[n] != 0xa) {
            n++;
        }
        while (sNewStr[n] == 0x20 || sNewStr[n] == 8 || sNewStr[n] == 9 || sNewStr[n] == 0xa) {
            n++;
        }
    ...
    if ((Mask & VPVST_NOISE) && !strstr(newScr, "register(c30)")) {
        ...
        while (true) {
            ...
            if (!sNewStr)
                break;
            n = 0;
            while (sNewStr[n] != 0x20 && sNewStr[n] != 8 && sNewStr[n] != 9 && sNewStr[n] != 0xa && sNewStr[n] != '(') {
                n++;
            }
            while (sNewStr[n] == 0x20 || sNewStr[n] == 8 || sNewStr[n] == 9 || sNewStr[n] == 0xa) {
                n++;
    ...
    if (gRenDev->m_Features & RFT_FOGVP) {
        ...
        while (true) {
            ...
            if (!sNewStr)
                break;
            n = 0;
            while (sNewStr[n] != 0x20 && sNewStr[n] != 9 && sNewStr[n] != 8 && sNewStr[n] != 0xa && sNewStr[n] != '{') {
                n++;
            }
            while (sNewStr[n] == 0x20 || sNewStr[n] == 9 || sNewStr[n] == 8 || sNewStr[n] == 0xa) {
                n++;
            }
            if (sNewStr[n] == '{') {
                n++;
...
void CCGVProgram_D3D::mfCompileParamStateMatrix(char* scr, SShader* ef, TArray<SCGMatrix>* Params) {
    ...
    enum { eType = 1, eName };
    static tokenDesc commands[] = {{eType, "Type"},
                                   {eName, "Name"},
    ...
    while ((cmd = shGetObject(&scr, commands, &name, &params)) > 0) {
        ...
        case eType:
            ...
            else if (!stricmp(data, "View"))
                pr.m_eCGParamType = ECGP_Matr_View;
            else if (!stricmp(data, "View_I"))
                pr.m_eCGParamType = ECGP_Matr_View_I;
            else if (!stricmp(data, "View_T"))
                pr.m_eCGParamType = ECGP_Matr_View_T;
            else if (!stricmp(data, "View_IT"))
...
void CCGVProgram_D3D::mfCompileVertAttributes(char* scr, SShader* ef) {
    ...
    enum { eVertArray = 1 };
    static tokenDesc commands[] = {{eVertArray, "VertArray"},
...
bool CCGVProgram_D3D::mfCompile(char* scr) {
    ...
    enum {
        ...
        eSubrScript,
        eSupportsInstancing,
        eSupportsMultiLights,
        eInst_Param4f,
        eNoSpecular
    };
    static tokenDesc commands[] = {{eScript, "Script"},
...
void CCGVProgram_D3D::mfGetDstFileName(char* dstname, int nSize, bool bUseASCIICache) {
    ...
    if ((m_Insts[m_CurInst].m_Mask & VPVST_FOGGLOBAL) || m_CGProfileType == CG_PROFILE_VS_3_0)
        fog = "Fog";
    else if (m_Insts[m_CurInst].m_Mask & VPVST_NOFOG) {
        if (gRenDev->m_Features & RFT_FOGVP)
            fog = "Fog0";
        else
            fog = "NoFog";
    ...
    else if (m_Insts[m_CurInst].m_Mask & VPVST_INSTANCING_ROT)
            ...
            while (!Types[i]) {
                ...
                for (j = 0; j < i; j++) {
                    if ((sTypes[j] & 3) == SLMF_PROJECTED)
                        nProjs++;
                    else if ((sTypes[j] & 3) == SLMF_DIRECT)
                        nDirs++;
