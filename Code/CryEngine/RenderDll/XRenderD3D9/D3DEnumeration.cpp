...
HRESULT CD3DEnumeration::EnumerateDeviceCombos(D3DDeviceInfo* pDeviceInfo, TArray<D3DFORMAT>* pAdapterFormatList) {
    const D3DFORMAT backBufferFormatArray[] = {D3DFMT_A8R8G8B8, D3DFMT_X8R8G8B8, D3DFMT_A2R10G10B10, D3DFMT_R5G6B5, D3DFMT_A1R5G5B5, D3DFMT_X1R5G5B5};
    ...
    for (int iaf = 0; iaf < pAdapterFormatList->Num(); iaf++) {
        ...
        for (UINT ibbf = 0; ibbf < backBufferFormatArrayCount; ibbf++) {
            ...
            for (UINT iiw = 0; iiw < 2; iiw++) {
                ...
                if (pDeviceCombo->pDepthStencilFormatList == nullptr || pDeviceCombo->pMultiSampleTypeList == nullptr || pDeviceCombo->pMultiSampleQualityList == nullptr ||
                    pDeviceCombo->pDSMSConflictList == nullptr || pDeviceCombo->pVertexProcessingTypeList == nullptr || pDeviceCombo->pPresentIntervalList == nullptr) {
                    delete pDeviceCombo;
                    return E_OUTOFMEMORY;
                }
...
void CD3DEnumeration::BuildDepthStencilFormatList(D3DDeviceCombo* pDeviceCombo) {
    ...
    for (UINT idsf = 0; idsf < depthStencilFormatArrayCount; idsf++) {
        ...
        if (StencilBits(depthStencilFmt) < AppMinStencilBits)
            continue;
        if (SUCCEEDED(m_pD3D->CheckDeviceFormat(pDeviceCombo->AdapterOrdinal, pDeviceCombo->DevType, pDeviceCombo->AdapterFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE,
                                                depthStencilFmt))) {
            if (SUCCEEDED(m_pD3D->CheckDepthStencilMatch(pDeviceCombo->AdapterOrdinal, pDeviceCombo->DevType, pDeviceCombo->AdapterFormat, pDeviceCombo->BackBufferFormat,
                                                         depthStencilFmt))) {
...
void CD3DEnumeration::BuildMultiSampleTypeList(D3DDeviceCombo* pDeviceCombo) {
    const D3DMULTISAMPLE_TYPE msTypeArray[] = {
        D3DMULTISAMPLE_NONE,       D3DMULTISAMPLE_NONMASKABLE, D3DMULTISAMPLE_2_SAMPLES,  D3DMULTISAMPLE_3_SAMPLES,  D3DMULTISAMPLE_4_SAMPLES,  D3DMULTISAMPLE_5_SAMPLES,
        D3DMULTISAMPLE_6_SAMPLES,  D3DMULTISAMPLE_7_SAMPLES,   D3DMULTISAMPLE_8_SAMPLES,  D3DMULTISAMPLE_9_SAMPLES,  D3DMULTISAMPLE_10_SAMPLES, D3DMULTISAMPLE_11_SAMPLES,
        D3DMULTISAMPLE_12_SAMPLES, D3DMULTISAMPLE_13_SAMPLES,  D3DMULTISAMPLE_14_SAMPLES, D3DMULTISAMPLE_15_SAMPLES, D3DMULTISAMPLE_16_SAMPLES,
    };
    ...
    for (UINT imst = 0; imst < msTypeArrayCount; imst++) {
        ...
        if (SUCCEEDED(m_pD3D->CheckDeviceMultiSampleType(pDeviceCombo->AdapterOrdinal, pDeviceCombo->DevType, pDeviceCombo->BackBufferFormat, pDeviceCombo->IsWindowed, msType,
                                                         &msQuality))) {
...
void CD3DEnumeration::BuildPresentIntervalList(D3DDeviceInfo* pDeviceInfo, D3DDeviceCombo* pDeviceCombo) {
    const UINT piArray[] = {
        D3DPRESENT_INTERVAL_IMMEDIATE, D3DPRESENT_INTERVAL_DEFAULT, D3DPRESENT_INTERVAL_ONE, D3DPRESENT_INTERVAL_TWO, D3DPRESENT_INTERVAL_THREE, D3DPRESENT_INTERVAL_FOUR,
    };
    ...
    for (d = 0; d < pAdapter->pDeviceInfoList->Num(); d++) {
        ...
        for (idc = 0; idc < pDevice->pDeviceComboList->Num(); idc++) {
            ...
            if (m_bFullScreen) {
                ...
                // save it
                if (pBestDeviceCombo == nullptr || (pBestDeviceCombo->DevType != D3DDEVTYPE_HAL && pDevice->DevType == D3DDEVTYPE_HAL) ||
    ...
    if (m_bFullScreen) {
        ...
        for (int idm = 0; idm < pAdapter->pDisplayModeList->Num(); idm++) {
            ...
            if (thisCD <= 0)
                continue;
            int ThisError =
                abs((int)pdm->Width - (int)requiredDisplayMode.Width) + abs((int)pdm->Height - (int)requiredDisplayMode.Height) + abs((int)thisCD - (int)CRenderer::m_cbpp);
