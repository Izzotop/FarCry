...
char* resourceName[] = {"UNKNOWN", "Surfaces", "Volumes", "Textures", "Volume Textures", "Cube Textures", "Vertex Buffers", "Index Buffers"};
...
void CD3D9Renderer::BeginFrame() {
    ...
    if (!m_bDeviceSupportsInstancing) {
        ...
        } else if (bChanged30 && m_sm30path >= 0) {
            var = iConsole->GetCVar("r_GeomInstancing");
            if (var)
    ...
    if (!m_bEditor) {
        if (m_CVWidth && m_CVHeight && m_CVFullScreen && m_CVColorBits) {
            if (m_CVWidth->GetIVal() != CRenderer::m_width || m_CVHeight->GetIVal() != CRenderer::m_height || m_CVFullScreen->GetIVal() != (int)m_bFullScreen ||
                m_CVColorBits->GetIVal() != CRenderer::m_cbpp)
                ChangeResolution(m_CVWidth->GetIVal(), m_CVHeight->GetIVal(), m_CVColorBits->GetIVal(), 75, m_CVFullScreen->GetIVal() != 0);
        }
    }
    ...
    if (CRenderer::CV_r_logTexStreaming) {
        LogStrv(0, "******************************* EndFrame ********************************\n");
        LogStrv(0, "Loaded: %.3f Kb, UpLoaded: %.3f Kb, UploadTime: %.3fMs\n\n", gRenDev->m_TexMan->m_LoadBytes / 1024.0f, gRenDev->m_TexMan->m_UpLoadBytes / 1024.0f,
                m_RP.m_PS.m_fTexUploadTime);
    }
...
void CD3D9Renderer::Update() {
    ...
    if (CV_r_showtimegraph) {
        ...
        if (sMask & 4) {
            ...
            col = ColStreamSync;
            WriteXY(cf, 4, hgt - 280 + 16 * 2, 0.5f, 1, col.r, col.g, col.b, 1, "StreamMB (%d-%d)", (int)(gRenDev->m_TexMan->m_LoadBytes / 1024.0f / 1024.0f),
                    (int)fScaleStreamSync);
        }
        ...
        if (sMask & 32) {
            ...
            col = ColTotalMem;
            WriteXY(cf, 4, hgt - 280 + 16 * 5, 0.5f, 1, col.r, col.g, col.b, 1, "Cur Pool Size (Mb) (%d-%d)", (int)(gRenDev->m_TexMan->m_StatsCurTexMem / 1024.0f / 1024.0f),
                    (int)fScaleTotalMem);
        }
        if (sMask & 64) {
            ...
            col = ColCurMem;
            WriteXY(cf, 4, hgt - 280 + 16 * 6, 0.5f, 1, col.r, col.g, col.b, 1, "Cur Scene Size (Mb) (%d-%d)", (int)(m_RP.m_PS.m_TexturesSize / 1024.0f / 1024.0f),
                    (int)fScaleCurMem);
        }
    ...
    if (CV_r_measureoverdraw) {
        ...
        EF_SetStencilState(STENCOP_FAIL(FSS_STENCOP_KEEP) | STENCOP_ZFAIL(FSS_STENCOP_KEEP) | STENCOP_PASS(FSS_STENCOP_KEEP) | STENC_FUNC(FSS_STENCFUNC_LEQUAL), 0, -1);
        ...
        if (hr == D3D_OK) {
            ...
            if (!timeOut) {
                for (i = 0; i < D3DRTYPECOUNT; i++) {
                    iLog->Log("%s - Thrashing = %s, Approx Bytes Downloaded %d, Number Evictions %d", resourceName[i], resourceStats.stats[i].bThrashing ? "TRUE" : "FALSE",
                              resourceStats.stats[i].ApproxBytesDownloaded, resourceStats.stats[i].NumEvicts);
                    iLog->Log("    Number Video Creates %d, Last Priority %d", resourceStats.stats[i].NumVidCreates, resourceStats.stats[i].LastPri);
                    iLog->Log("    Number set to Device %d, Number used In Vid mem %d", resourceStats.stats[i].NumUsed, resourceStats.stats[i].NumUsedInVidMem);
...
void CD3D9Renderer::Draw2dImage(float xpos, float ypos, float w, float h, int textureid, float s0, float t0, float s1, float t1, float angle, float r, float g, float b, float a,
                                float z) {
    if (textureid > TX_FIRSTBIND && unsigned(textureid - TX_FIRSTBIND) >= m_TexMan->m_Textures.size()) {
        iLog->LogError("\001CD3D9Renderer::Draw2dImage(x=%g,y=%g,w=%g,h=%g,texid=%d): invalid texid, should be between %u and %u", xpos, ypos, w, h, textureid, TX_FIRSTBIND,
                       m_TexMan->m_Textures.size() + TX_FIRSTBIND);
        return;
    }
    ...
    w = (float)ScaleCoordX(w);
    ypos = (float)ScaleCoordY(ypos);
    h = (float)ScaleCoordY(h);
    float fx = xpos - 0.5f;
    float fy = ypos - 0.5f;
    float fw = w;
    float fh = h;
...
void CD3D9Renderer::Flush3dBBox(const Vec3d& mins, const Vec3d& maxs, const bool bSolid) {
    ...
    if (bSolid) {
        ...
        vQuad[0].xyz.x = maxs.x;
        vQuad[0].xyz.y = mins.y;
        vQuad[0].xyz.z = mins.z; // 3
        vQuad[1].xyz.x = maxs.x;
        vQuad[1].xyz.y = mins.y;
        vQuad[1].xyz.z = maxs.z; // 2
        vQuad[2].xyz.x = mins.x;
        vQuad[2].xyz.y = mins.y;
        vQuad[2].xyz.z = maxs.z; // 1
        vQuad[3].xyz.x = mins.x;
        vQuad[3].xyz.y = mins.y;
        vQuad[3].xyz.z = mins.z; // 0
        UnlockVB3D();
        m_pd3dDevice->DrawPrimitive(D3DPT_TRIANGLEFAN, nOffs, 2);

        vQuad = (struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F*)GetVBPtr3D(4, nOffs);
        hr = m_pd3dDevice->SetStreamSource(0, m_pVB3D[0], 0, sizeof(struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F));
        vQuad[0].xyz.x = mins.x;
        vQuad[0].xyz.y = mins.y;
        vQuad[0].xyz.z = mins.z; // 0
        vQuad[1].xyz.x = mins.x;
        vQuad[1].xyz.y = mins.y;
        vQuad[1].xyz.z = maxs.z; // 1
        vQuad[2].xyz.x = mins.x;
        vQuad[2].xyz.y = maxs.y;
        vQuad[2].xyz.z = maxs.z; // 6
        vQuad[3].xyz.x = mins.x;
        vQuad[3].xyz.y = maxs.y;
        vQuad[3].xyz.z = mins.z; // 4
        UnlockVB3D();
        m_pd3dDevice->DrawPrimitive(D3DPT_TRIANGLEFAN, nOffs, 2);
        ...
        hr = m_pd3dDevice->SetStreamSource(0, m_pVB3D[0], 0, sizeof(struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F));
        vQuad[0].xyz.x = mins.x;
        vQuad[0].xyz.y = maxs.y;
        vQuad[0].xyz.z = mins.z; // 4
        vQuad[1].xyz.x = mins.x;
        vQuad[1].xyz.y = maxs.y;
        vQuad[1].xyz.z = maxs.z; // 6
        vQuad[2].xyz.x = maxs.x;
        vQuad[2].xyz.y = maxs.y;
        vQuad[2].xyz.z = maxs.z; // 7
        vQuad[3].xyz.x = maxs.x;
        vQuad[3].xyz.y = maxs.y;
        vQuad[3].xyz.z = mins.z; // 5
        UnlockVB3D();
        m_pd3dDevice->DrawPrimitive(D3DPT_TRIANGLEFAN, nOffs, 2);
        ...
        vQuad[0].xyz.x = maxs.x;
        vQuad[0].xyz.y = maxs.y;
        vQuad[0].xyz.z = mins.z; // 54
        vQuad[1].xyz.x = maxs.x;
        vQuad[1].xyz.y = maxs.y;
        vQuad[1].xyz.z = maxs.z; // 73
        vQuad[2].xyz.x = maxs.x;
        vQuad[2].xyz.y = mins.y;
        vQuad[2].xyz.z = maxs.z; // 22
        vQuad[3].xyz.x = maxs.x;
        vQuad[3].xyz.y = mins.y;
        vQuad[3].xyz.z = mins.z; // 31
        UnlockVB3D();
        m_pd3dDevice->DrawPrimitive(D3DPT_TRIANGLEFAN, nOffs, 2);
        ...
        hr = m_pd3dDevice->SetStreamSource(0, m_pVB3D[0], 0, sizeof(struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F));
        vQuad[0].xyz.x = maxs.x;
        vQuad[0].xyz.z = maxs.z;
        vQuad[0].xyz.y = mins.y; // 3
        vQuad[1].xyz.x = maxs.x;
        vQuad[1].xyz.z = maxs.z;
        vQuad[1].xyz.y = maxs.y; // 2
        vQuad[2].xyz.x = mins.x;
        vQuad[2].xyz.z = maxs.z;
        vQuad[2].xyz.y = maxs.y; // 1
        vQuad[3].xyz.x = mins.x;
        vQuad[3].xyz.z = maxs.z;
        vQuad[3].xyz.y = mins.y; // 0
        UnlockVB3D();
        m_pd3dDevice->DrawPrimitive(D3DPT_TRIANGLEFAN, nOffs, 2);
        ...
        hr = m_pd3dDevice->SetStreamSource(0, m_pVB3D[0], 0, sizeof(struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F));
        vQuad[0].xyz.x = maxs.x;
        vQuad[0].xyz.z = mins.z;
        vQuad[0].xyz.y = mins.y; // 3
        vQuad[1].xyz.x = maxs.x;
        vQuad[1].xyz.z = mins.z;
        vQuad[1].xyz.y = maxs.y; // 2
        vQuad[2].xyz.x = mins.x;
        vQuad[2].xyz.z = mins.z;
        vQuad[2].xyz.y = maxs.y; // 1
        vQuad[3].xyz.x = mins.x;
        vQuad[3].xyz.z = mins.z;
        vQuad[3].xyz.y = mins.y; // 0
        UnlockVB3D();
        m_pd3dDevice->DrawPrimitive(D3DPT_TRIANGLEFAN, nOffs, 2);
    } else {
        ...
        vQuad[3].xyz.z = maxs.z;
        vQuad[3].xyz.y = mins.y; // 0
        vQuad[4].xyz.x = maxs.x;
        vQuad[4].xyz.z = maxs.z;
        vQuad[4].xyz.y = mins.y; // 3
        UnlockVB3D();
...
void CD3D9Renderer::DrawQuad3D(const Vec3d& v0, const Vec3d& v1, const Vec3d& v2, const Vec3d& v3, const CFColor& color, float ftx0, float fty0, float ftx1, float fty1) {
    LPDIRECT3DDEVICE9 dv = mfGetD3DDevice();
    DWORD col = D3DRGBA(color.r, color.g, color.b, color.a);
...
unsigned int CD3D9Renderer::DownLoadToVideoMemory(unsigned char* data, int w, int h, ETEX_Format eTFSrc, ETEX_Format eTFDst, int nummipmap, bool repeat, int filter, int Id,
