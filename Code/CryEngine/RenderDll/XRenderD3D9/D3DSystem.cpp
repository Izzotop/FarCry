...
void CD3D9Renderer::DestroyWindow() {
    if (m_hWnd) {
...
void CD3D9Renderer::RestoreGamma() {
...
void CD3D9Renderer::SetGamma(float fGamma, float fBrightness, float fContrast, bool bForce) {
    ...
    for (i = 0; i < 256; i++) {
        m_GammaTable[i] =
            CLAMP((int)((fContrast + 0.5f) * cry_powf((float)i / 255.f, 1.0f / fGamma) * 65535.f + (fBrightness - 0.5f) * 32768.f - fContrast * 32768.f + 16384.f), 0, 65535);
    }
...
HRESULT CD3D9Renderer::RestoreDeviceObjects() {
    ...
    // if( m_FSAA == 2 )
    //{
    //     hr = m_pd3dDevice->CreateDepthStencilSurface( m_width, m_height, m_d3dpp.AutoDepthStencilFormat, ConvertFSAASamplesToType( m_FSAA_samples ), m_FSAA_quality, TRUE,
    //     &m_pZBuffer, 0 ); if( SUCCEEDED( hr ) )
    //     {
    ...
    if (m_FSAA == 2) {
        hr = m_pd3dDevice->CreateDepthStencilSurface(m_width, m_height, m_d3dpp.AutoDepthStencilFormat, ConvertFSAASamplesToType(m_FSAA_samples), m_FSAA_quality, TRUE, &m_pZBuffer,
                                                     0);
        if (SUCCEEDED(hr)) {
            // iLog->Log( "HDR-FSAA: Created multi-sampled Z-Buffer for current FSAA settings (samples = %d / quality = %d).", m_FSAA_samples, m_FSAA_quality );
    ...
    hr = m_pd3dDevice->CreateVertexBuffer(m_nVertsDMesh2D * sizeof(struct_VERTEX_FORMAT_TRP3F_COL4UB_TEX2F), D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC,
                                          D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1, D3DPOOL_DEFAULT, &m_pVB2D, nullptr);
    if (FAILED(hr))
        return hr;
...
COMPRESSOR_ERROR (*CompressTextureATI)
(DWORD width, DWORD height, UNCOMPRESSED_FORMAT sourceFormat, COMPRESSED_FORMAT destinationFormat, void* inputData, void** dataOut, DWORD* outDataSize);
#endif

WIN_HWND CD3D9Renderer::Init(int x, int y, int width, int height, unsigned int cbpp, int zbpp, int sbits, bool fullscreen, WIN_HINSTANCE hinst, WIN_HWND Glhwnd, WIN_HDC Glhdc,
    ...
    iLog->Log(" NormalMaps compression: %s\n", m_bDeviceSupportsComprNormalmaps == 1   ? "3Dc"
                                               : m_bDeviceSupportsComprNormalmaps == 2 ? "V8U8"
                                               : m_bDeviceSupportsComprNormalmaps == 3 ? "CxV8U8"
                                                                                       : "Not supported");
    ...
    if (nGPU == RFT_HW_GF2)
        ...
        else if (m_bDeviceSupports_PS2X) {
            if (nGPU == RFT_HW_GFFX)
    ...
    for (int iai = 0; iai < m_D3DEnum.m_pAdapterInfoList->Num(); iai++) {
        ...
        for (int idi = 0; idi < pAdapterInfo->pDeviceInfoList->Num(); idi++) {
            ...
            for (int idc = 0; idc < pDeviceInfo->pDeviceComboList->Num(); idc++) {
                ...
                // save it
                if (pBestDeviceCombo == nullptr || pBestDeviceCombo->DevType != D3DDEVTYPE_HAL && pDeviceCombo->DevType == D3DDEVTYPE_HAL ||
    ...
    for (int iai = 0; iai < m_D3DEnum.m_pAdapterInfoList->Num(); iai++) {
        ...
        for (int idi = 0; idi < pAdapterInfo->pDeviceInfoList->Num(); idi++) {
            ...
            for (int idc = 0; idc < pDeviceInfo->pDeviceComboList->Num(); idc++) {
                ...
                // is better (because it's a HAL, and/or because formats match better),
                // save it
                if (pBestDeviceCombo == nullptr || (pBestDeviceCombo->DevType != D3DDEVTYPE_HAL && pDeviceInfo->DevType == D3DDEVTYPE_HAL) ||
...
EndFullscreenDeviceComboSearch:
    ...
    for (int idm = 0; idm < pBestAdapterInfo->pDisplayModeList->Num(); idm++) {
        ...
        if (pdm->Format != pBestDeviceCombo->AdapterFormat)
            continue;
        if (pdm->Width == bestAdapterDesktopDisplayMode.Width && pdm->Height == bestAdapterDesktopDisplayMode.Height &&
            pdm->RefreshRate == bestAdapterDesktopDisplayMode.RefreshRate) {
            // found a perfect match, so stop
            bestDisplayMode = *pdm;
            break;
        } else if (pdm->Width == bestAdapterDesktopDisplayMode.Width && pdm->Height == bestAdapterDesktopDisplayMode.Height && pdm->RefreshRate > bestDisplayMode.RefreshRate) {
...
HRESULT CD3D9Renderer::Initialize3DEnvironment() {
    ...
    if (pDeviceInfo->Caps.PrimitiveMiscCaps & D3DPMISCCAPS_NULLREFERENCE) {
        // Warn user about nullptr ref device that can't render anything
    ...
    // Create the device
    iLog->Log("Creating D3D device (Adapter format: %s, BackBuffer format: %s, Depth format: %s)", sD3DFMT(m_D3DSettings.AdapterFormat()), sD3DFMT(m_d3dpp.BackBufferFormat),
              sD3DFMT(m_d3dpp.AutoDepthStencilFormat));
...
void CD3D9Renderer::BuildPresentParamsFromSettings() {
    ...
    if (CV_r_fsaa) {
        ...
        if (!FAILED(hr) && nQualityLevel >= (DWORD)Formats[nNum].nQuality) {
            HRESULT hr = m_pD3D->CheckDeviceMultiSampleType(m_D3DSettings.AdapterOrdinal(), m_D3DSettings.DevType(), m_d3dpp.AutoDepthStencilFormat, m_d3dpp.Windowed, nType,
                                                            &nQualityLevel);
            if (!FAILED(hr) && nQualityLevel >= (DWORD)Formats[nNum].nQuality) {
                if (CV_r_fsaa == 1) {
...
bool CD3D9Renderer::ChooseDevice() {
    HRESULT hr;
...
HRESULT CD3D9Renderer::InitDeviceObjects() {
    ...
    // Hardware-specific initialization and workarounds for driver bugs.
    {
        ...
        if (CV_d3d9_nodeviceid) {
            ...
            if (ai->DeviceId == 0x4a49 || ai->DeviceId == 0x4a4a || ai->DeviceId == 0x4a4b || ai->DeviceId == 0x4a4c || ai->DeviceId == 0x4a50 || ai->DeviceId == 0x4a69 ||
                ai->DeviceId == 0x4a6a || ai->DeviceId == 0x4a6b || ai->DeviceId == 0x4a6c || ai->DeviceId == 0x4a70 || ai->DeviceId == 0x5b60 || ai->DeviceId == 0x5b70 ||
                ai->DeviceId == 0x3e50 || ai->DeviceId == 0x3e70 || ai->DeviceId == 0x5549 || ai->DeviceId == 0x5569 || ai->DeviceId == 0x554b || ai->DeviceId == 0x556b) {
    ...
    // Handle the texture formats we need.
    {
        ...
        // Find the needed texture formats.
        {
            ...
            if (!mFirstPixelFormat) {
                ...
                else if (SUCCEEDED(m_pD3D->CheckDeviceFormat(pAI->AdapterOrdinal, m_d3dCaps.DeviceType, m_D3DSettings.AdapterFormat(), 0, D3DRTYPE_TEXTURE, D3DFMT_X1R5G5B5)))
            ...
            if (SUCCEEDED(m_pD3D->CheckDeviceFormat(pAI->AdapterOrdinal, m_d3dCaps.DeviceType, m_D3DSettings.AdapterFormat(), 0, D3DRTYPE_TEXTURE,
    ...
    if (m_d3dCaps.MaxSimultaneousTextures <= 2) {
        ...
        if (m_d3dCaps.MaxSimultaneousTextures == 4 && D3DSHADER_VERSION_MAJOR(m_d3dCaps.PixelShaderVersion) == 1) {
            ...
            if (m_d3dCaps.MaxSimultaneousTextures >= 8 &&
                (D3DSHADER_VERSION_MAJOR(m_d3dCaps.PixelShaderVersion) >= 3 ||
                 (D3DSHADER_VERSION_MAJOR(m_d3dCaps.PixelShaderVersion) >= 2 && D3DSHADER_VERSION_MINOR(m_d3dCaps.PixelShaderVersion) >= 1))) {
                m_Features &= ~RFT_HW_MASK;
    ...
    m_bDeviceSupportsMRT = (m_d3dCaps.NumSimultaneousRTs >= 4) && SUCCEEDED(m_pD3D->CheckDeviceFormat(pAI->AdapterOrdinal, m_d3dCaps.DeviceType, m_D3DSettings.AdapterFormat(),
                                                                                                      D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE, D3DFMT_G16R16F));
    ...
    if (FAILED(m_pD3D->CheckDeviceFormat(pAI->AdapterOrdinal, m_d3dCaps.DeviceType, m_D3DSettings.AdapterFormat(), D3DUSAGE_RENDERTARGET | D3DUSAGE_QUERY_FILTER, D3DRTYPE_TEXTURE,
    ...
    int nHDRSupported = 1;
    {
        ...
        if (CV_r_Quality_BumpMapping < 3)
            nHDRSupported = 0;
        else if (FAILED(m_pD3D->CheckDeviceFormat(pDev->AdapterOrdinal, pDev->DevType, pDev->AdapterFormat, D3DUSAGE_RENDERTARGET | D3DUSAGE_QUERY_POSTPIXELSHADER_BLENDING,
                                                  D3DRTYPE_TEXTURE, D3DFMT_A16B16G16R16F)))
            nHDRSupported = 0;
        else if (FAILED(m_pD3D->CheckDeviceFormat(pDev->AdapterOrdinal, pDev->DevType, pDev->AdapterFormat, D3DUSAGE_RENDERTARGET | D3DUSAGE_QUERY_POSTPIXELSHADER_BLENDING,
                                                  D3DRTYPE_SURFACE, D3DFMT_A16B16G16R16F)))
            nHDRSupported = 0;
    ...
    if (!nHDRSupported && m_d3dCaps.NumSimultaneousRTs >= 2 && (m_d3dCaps.PrimitiveMiscCaps & D3DPMISCCAPS_MRTPOSTPIXELSHADERBLENDING) &&
        D3DSHADER_VERSION_MAJOR(m_d3dCaps.PixelShaderVersion) >= 2 && CV_r_Quality_BumpMapping >= 3)
        nHDRSupported = 2;
