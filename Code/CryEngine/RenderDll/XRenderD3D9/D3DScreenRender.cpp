...
bool CD3D9TexMan::PreloadScreenFxMaps() {
...
bool CREScreenProcess::mfDrawLowSpec(SShader* ef, SShaderPass* sfm) {
    ...
    struct_VERTEX_FORMAT_P3F_TEX2F pScreenQuad[] = {
        Vec3(-fSoff, -fToff, 0), 0, 0, Vec3(-fSoff, 1 - fToff, 0), 0, 1, Vec3(1 - fSoff, -fToff, 0), 1, 0, Vec3(1 - fSoff, 1 - fToff, 0), 1, 1,
    };
...
bool CREScreenProcess::mfDraw(SShader* ef, SShaderPass* sfm) {
    ...
    if ((m_pVars->m_iNightVisionActive && m_pVars->m_bCartoonActive) || (m_pVars->m_bGlareActive && !m_pVars->m_bCartoonActive) || m_pVars->m_iNightVisionActive) {
        ...
        if (!m_pVars->m_iNightVisionActive && ((CRenderer::CV_r_rendermode != 5) || (CRenderer::CV_r_rendermode == 5 && REQUIRES_PS20))) {
            // get data
            STexPic *pTex = gRenDev->m_TexMan->m_Text_Glare, *pScreenCurrLuminosityTex = gRenDev->m_TexMan->m_Text_ScreenCurrLuminosityMap,
            ...
            // get data
            STexPic *pTex = gRenDev->m_TexMan->m_Text_Glare, *pHeatTexture = gRenDev->m_TexMan->m_Text_ScreenLowMap;
    ...
    if (m_pVars->m_bFlashBangActive) {
        ...
        if (m_pVars->m_fFlashBangTimeOut == 1.0f || m_pVars->m_iFlashBangForce) {
            ...
            struct_VERTEX_FORMAT_P3F_TEX2F pFlashBangScreen[] = {
                Vec3(0, 0, 0), fOffX, fOffY, Vec3(0, 1, 0), fOffX, 1 + fOffY, Vec3(1, 0, 0), 1 + fOffX, fOffY, Vec3(1, 1, 0), 1 + fOffX, 1 + fOffY,
            };
    ...
    if (m_pVars->m_bBlurActive) {
        ...
        struct_VERTEX_FORMAT_P3F_TEX2F pScreenBluredQuad[] = {
            Vec3(0, 0, 0), fTexelWidth,     fTexelHeight, Vec3(0, 1, 0), fTexelWidth,     1 + fTexelHeight,
            Vec3(1, 0, 0), 1 + fTexelWidth, fTexelHeight, Vec3(1, 1, 0), 1 + fTexelWidth, 1 + fTexelHeight,
        };
