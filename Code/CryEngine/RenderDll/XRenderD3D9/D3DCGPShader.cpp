...
void CCGPShader_D3D::mfAddFXParameter(SFXParam* pr, const char* ParamName, SShader* ef) {
    ...
    if (pr->m_Assign.size()) {
        ...
        else if (!stricmp(translatedParam, "Diffuse"))
...
void CCGPShader_D3D::mfGatherFXParameters(const char* buf, SShaderPassHW* pSHPass, std::vector<SFXParam>& Params, std::vector<SFXSampler>& Samplers,
                ...
                if (ss != cgs) {
                    ...
                    if (!strncmp(ss, "# ifdef", 7)) {
                        ...
                        if (!strncmp(ss, "_3DC_A", 6) && !(m_Insts[m_CurInst].m_Mask & VPVST_3DC_A))
                            bIncr = false;
                        else if (!strncmp(ss, "_SPEC_ANTIALIAS", 15) && !(m_Insts[m_CurInst].m_Mask & VPVST_SPECANTIALIAS))
                            bIncr = false;
                        else if (!strncmp(ss, "_HDRLM", 6) && !(m_Insts[m_CurInst].m_Mask & VPVST_HDRLM))
                            bIncr = false;
                    }
                }
            }
...
void CCGPShader_D3D::mfSetVariables(bool bObj, TArray<SCGParam4f>* Parms) {
    if (m_Insts[m_CurInst].m_pHandle == nullptr || (INT_PTR)m_Insts[m_CurInst].m_pHandle == -1)
        return;
