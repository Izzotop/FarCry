...
public:
    ...
    char* mfLoadCG(const char* prog_text) {
        ...
        char szCmdLine[512];
        sprintf(szCmdLine, "fxc.exe /T %s /Zpr /DCGC=0 /Fc $$out.cg $$in.cg",
                pr == CG_PROFILE_VS_2_0   ? "vs_2_0"
                : pr == CG_PROFILE_VS_2_X ? "vs_2_a"
                : pr == CG_PROFILE_VS_3_0 ? "vs_3_0"
                                          : "vs_1_1");
    ...
    bool mfIsValid(int Num) const {
        return (m_Insts[Num].m_CGProgram != nullptr);
    }
