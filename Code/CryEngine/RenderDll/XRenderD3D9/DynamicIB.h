...
public:
    ...
    DynamicIB(const LPDIRECT3DDEVICE9 pD3D, const unsigned int& theElementsCount) {
        ...
        HRESULT hr = pD3D->CreateIndexBuffer(theElementsCount * sizeof(Type), D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC, sizeof(Type) == 2 ? D3DFMT_INDEX16 : D3DFMT_INDEX32,
                                             D3DPOOL_DEFAULT, &m_pIB, nullptr);
        assert((hr == D3D_OK) && (m_pIB));
    }

    LPDIRECT3DVERTEXBUFFER9 GetInterface() const {
        return m_pIB;
    }
