...
#define MAX_DYNVB3D_VERTS 4096
    ...
    struct_VERTEX_FORMAT_TRP3F_COL4UB_TEX2F* GetVBPtr2D(int nVerts, int& nOffs) {
        ...
        if (nVerts + m_nOffsDMesh2D > m_nVertsDMesh2D) {
            ...
            hr = m_pVB2D->Lock(m_nOffsDMesh2D * sizeof(struct_VERTEX_FORMAT_TRP3F_COL4UB_TEX2F), nVerts * sizeof(struct_VERTEX_FORMAT_TRP3F_COL4UB_TEX2F), (void**)&pVertices,
                               D3DLOCK_NOOVERWRITE);
...
public:
    LPDIRECT3DDEVICE9 mfGetD3DDevice() {
        return m_pd3dDevice;
    }
    LPDIRECT3D9 mfGetD3D() {
        return m_pD3D;
    }
    D3DSURFACE_DESC* mfGetZSurfaceDesc() {
        return &m_d3dsdZBuffer;
    }
    LPDIRECT3DSURFACE9 mfGetZSurface() {
        return m_pZBuffer;
    }
    LPDIRECT3DSURFACE9 mfGetBackSurface() {
        return m_pBackBuffer;
    }
    D3DCAPS9* mfGetD3DCaps() {
        return &m_d3dCaps;
    }
...
#define MAX_DYNAMIC_SHADOW_MAPS_COUNT 64
    ...
    virtual void SetupShadowOnlyPass(int Num, ShadowMapFrustum* pFrustum, Vec3* vShadowTrans, const float fShadowScale, Vec3 vObjTrans = Vec3(0, 0, 0), float fObjScale = 1.f,
                                     const Vec3 vObjAngles = Vec3(0, 0, 0), Matrix44* pObjMat = 0);
...
public:
#ifndef PS2
    virtual WIN_HWND Init(int x, int y, int width, int height, unsigned int cbpp, int zbpp, int sbits, bool fullscreen, WIN_HINSTANCE hinst, WIN_HWND Glhwnd = 0, WIN_HDC Glhdc = 0,
    ...
    virtual void Draw2dImage(float xpos, float ypos, float w, float h, int textureid, float s0 = 0, float t0 = 0, float s1 = 1, float t1 = 1, float angle = 0, float r = 1,
                             float g = 1, float b = 1, float a = 1, float z = 1);
    ...
    virtual unsigned int DownLoadToVideoMemory(unsigned char* data, int w, int h, ETEX_Format eTFSrc, ETEX_Format eTFDst, int nummipmap, bool repeat = true,
                                               int filter = FILTER_BILINEAR, int Id = 0, char* szCacheName = nullptr, int flags = 0);
    ...
    virtual void SetClipPlane(int id, float* params) {
        EF_SetClipPlane(params ? true : false, params, false);
    }
    ...
    void DrawQuad3D(const Vec3& v0, const Vec3& v1, const Vec3& v2, const Vec3& v3, const CFColor& color, float ftx0 = 0, float fty0 = 0, float ftx1 = 1, float fty1 = 1);
    ...
    _inline HRESULT EF_SetVertexDeclaration(int StreamMask, int nVFormat) {
        ...
        if (!m_RP.m_D3DFixedPipeline[StreamMask][nVFormat].m_pDeclaration) {
            if (FAILED(hr = m_pd3dDevice->CreateVertexDeclaration(&m_RP.m_D3DFixedPipeline[StreamMask][nVFormat].m_Declaration[0],
                                                                  &m_RP.m_D3DFixedPipeline[StreamMask][nVFormat].m_pDeclaration)))
                return hr;
        }
...
protected:
    virtual STexPic* CreateTexture(const char* name, int wdt, int hgt, int depth, uint flags, uint flags2, byte* dst, ETexType eTT, float fAmount1 = -1.0f, float fAmount2 = -1.0f,
                                   int DXTSize = 0, STexPic* ti = nullptr, int bind = 0, ETEX_Format eTF = eTF_8888, const char* szSourceName = nullptr);
...
public:
    ...
    void D3DCreateVideoTexture(int tgt, byte* src, int wdt, int hgt, int depth, D3DFORMAT SrcFormat, D3DFORMAT DstFormat, STexPicD3D* ti, bool bMips, int CubeSide,
    ...
    virtual bool PreloadScreenFxMaps();
