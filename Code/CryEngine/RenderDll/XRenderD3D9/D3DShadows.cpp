    ...
    if (nType == 1 || (nType > 1 && !(m_Features & RFT_HW_PS20))) {
        ...
        if (vpBlur && fpBlur) {
            ...
            struct_VERTEX_FORMAT_P3F_TEX2F pScreenBlur[] = {
                Vec3(-s_off, -t_off, 0), 0, 0, Vec3(-s_off, 1 - t_off, 0), 0, 1, Vec3(1 - s_off, -t_off, 0), 1, 0, Vec3(1 - s_off, 1 - t_off, 0), 1, 1,
            };
            int iBlurAmount = 1;
...
/*void CD3D9Renderer::DrawShadowGrid(const Vec3d & pos, const Vec3d & Scale, ShadowMapFrustum*lf, bool translate_projection, float alpha, IndexedVertexBuffer* pVertexBuffer, float
anim_angle)
{
}    */
...
void CD3D9Renderer::OnEntityDeleted(IEntityRender* pEntityRender) { // remove references to the entity
...
void CD3D9Renderer::PrepareDepthMap(ShadowMapFrustum* lof, bool make_new_tid) {
    ...
    if (make_new_tid) { // new id for static objects
        ...
        if (lof->nTexIdSlot >= 0 && m_ShadowTexIDBuffer[lof->nTexIdSlot].nTexId0 == lof->depth_tex_id && m_ShadowTexIDBuffer[lof->nTexIdSlot].nTexSize == nShadowTexSize &&
            m_ShadowTexIDBuffer[lof->nTexIdSlot].pOwner == lof->pOwner && m_ShadowTexIDBuffer[lof->nTexIdSlot].pOwnerGroup == lof->pOwnerGroup &&
            m_ShadowTexIDBuffer[lof->nTexIdSlot].dwFlags == lof->dwFlags) {
            nCurTexIdSlot = lof->nTexIdSlot;
        } else { // find oldest slot
            int nOldestSlot = -1;
    ...
    assert(m_ShadowTexIDBuffer[0].nTexId0 ? m_TexMan->GetByID(m_ShadowTexIDBuffer[0].nTexId0) != nullptr : 1);
    //    assert(m_ShadowTexIDBuffer[1].nTexId ? m_TexMan->GetByID(m_ShadowTexIDBuffer[1].nTexId)!=nullptr : 1);
...
void CD3D9Renderer::SetupShadowOnlyPass(int Num, ShadowMapFrustum* pFrustum, Vec3d* vShadowTrans, const float fShadowScale, Vec3d vObjTrans, float fObjScale,
    ...
    if (vShadowTrans) { // make tmp matrix for this obj position if shadow frustum is not translated (translate original mats)
        ...
        D3DXVECTOR3 Eye = D3DXVECTOR3(pFrustum->pLs->vSrcPos.x + mv_trans.x, pFrustum->pLs->vSrcPos.y + mv_trans.y, pFrustum->pLs->vSrcPos.z + mv_trans.z);
        D3DXVECTOR3 At =
            D3DXVECTOR3(fShadowScale * pFrustum->target.x + mv_trans.x, fShadowScale * pFrustum->target.y + mv_trans.y, fShadowScale * pFrustum->target.z + mv_trans.z);
        D3DXVECTOR3 Up = D3DXVECTOR3(0, 0, 1);
...
void CD3D9Renderer::ConfigShadowTexgen(int Num, int rangeMap, ShadowMapFrustum* pFrustum, float* pLightFrustumMatrix, float* pLightViewMatrix, float* ModelVPMatrix) {
    ...
    if (rangeMap) {
        ...
        float RSmatrix[16] = {0, 0, 0, 0, 0, 0, 0, 0, 0.5f, 128, 0, 0, 0.5f, 128, 0, 1.0f};
        ...
        if ((m_Features & RFT_DEPTHMAPS)) {
            ...
            float Smatrix[16] = {0.5f, 0, 0, 0, 0, -0.5f, 0, 0, 0, 0, 0.5f, 0, fOffsetX, fOffsetY, 0.5f, 1.0f};

            mtexSc = Smatrix;
        } else {
            static float Smatrix[16] = {0.5f, 0, 0, 0, 0, -0.5f, 0, 0, 0, 0, 0, 0, 0.5f, 0.5f, 1.0f, 1.0f};
    ...
    if (Num >= 0) {
        ...
        if (m_RP.m_pCurLight) {
            ...
            if (m_RP.m_ObjFlags & FOB_TRANS_MASK) {
                float fLen = m_RP.m_pCurObject->m_Matrix(0, 0) * m_RP.m_pCurObject->m_Matrix(0, 0) + m_RP.m_pCurObject->m_Matrix(0, 1) * m_RP.m_pCurObject->m_Matrix(0, 1) +
                             m_RP.m_pCurObject->m_Matrix(0, 2) * m_RP.m_pCurObject->m_Matrix(0, 2);
