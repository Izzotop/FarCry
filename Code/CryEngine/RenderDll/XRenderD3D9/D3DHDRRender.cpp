...
struct DescendingRenderTargetSort {
    bool operator()(DeferredRenderTarget* drtStart, DeferredRenderTarget* drtEnd) {
        return (drtStart->dwPitch * drtStart->fPriority) > (drtEnd->dwPitch * drtEnd->fPriority);
    }
};
...
void CD3D9TexMan::GenerateHDRMaps() {
    ...
    if (r->m_nHDRType == 2) {
        ...
        if (r->CV_r_fsaa == 2) {
            HRESULT hr = r->m_pd3dDevice->CreateRenderTarget(r->m_width, r->m_height, D3DFMT_A16B16G16R16F, ConvertFSAASamplesToType(r->m_FSAA_samples), r->m_FSAA_quality, FALSE,
                                                             &m_HDR_RT_FSAA, 0);
            // if( SUCCEEDED( hr ) )
...
public:
    ...
    HRESULT Initialize(const TCHAR* szStarName, int nStarLines, int nPasses, float fSampleLength, float fAttenuation, float fInclination, bool bRotation);
    ...
    //  Sunny cross filter
    HRESULT Initialize_SunnyCrossFilter(const TCHAR* szStarName = TEXT("SunnyCross"), float fSampleLength = 1.0f, float fAttenuation = 0.88f, float fLongAttenuation = 0.95f,
...
public:
    ...
    HRESULT Initialize(const TCHAR* szStarName, float fGlareLuminance, float fBloomLuminance, float fGhostLuminance, float fGhostDistortion, float fStarLuminance,
                       ESTARLIBTYPE eStarType, float fStarInclination, float fChromaticAberration,
                       float fAfterimageSensitivity, // Current weight
                       float fAfterimageRatio,       // After Image weight
                       float fAfterimageLuminance);

    HRESULT Initialize(const GLAREDEF& glareDef) {
        return Initialize(glareDef.szGlareName, glareDef.fGlareLuminance, glareDef.fBloomLuminance, glareDef.fGhostLuminance, glareDef.fGhostDistortion, glareDef.fStarLuminance,
                          glareDef.eStarType, glareDef.fStarInclination, glareDef.fChromaticAberration, glareDef.fAfterimageSensitivity, glareDef.fAfterimageRatio,
...
static GLAREDEF s_aLibGlareDef[NUM_GLARELIBTYPES] = {
    ...
    */
    ...
    {
        TEXT("Cine Camera Vertical Slits"),
        1.0f,
        2.0f,
        1.5f,
        0.00f,
        ...
        0.93f,
        1.0f,
    }, // GLT_CINECAM_VERTICALSLIT
    {
        TEXT("Cine Camera Horizontal Slits"),
        1.0f,
        2.0f,
        1.5f,
        0.00f,
...
/// generic simple star generation
HRESULT CStarDef::Initialize(const TCHAR* szStarName, int nStarLines, int nPasses, float fSampleLength, float fAttenuation, float fInclination, bool bRotation) {
    // Release the data
    Release();
...
//  Sunny cross filter
HRESULT CStarDef::Initialize_SunnyCrossFilter(const TCHAR* szStarName, float fSampleLength, float fAttenuation, float fLongAttenuation, float fInclination) {
    // Release the data
    Release();
    ...
    */
    D3DXCOLOR avColor[8] = {
        ...
        */
        ...
        D3DXCOLOR(0.8f, 0.3f, 0.3f, 0.0f), D3DXCOLOR(1.0f, 0.2f, 0.2f, 0.0f), // r
        D3DXCOLOR(0.5f, 0.2f, 0.6f, 0.0f), D3DXCOLOR(0.2f, 0.2f, 1.0f, 0.0f), // b
        D3DXCOLOR(0.2f, 0.3f, 0.7f, 0.0f), D3DXCOLOR(0.2f, 0.6f, 0.2f, 0.0f), // g
...
HRESULT CGlareDef::Initialize(const TCHAR* szStarName, float fGlareLuminance, float fBloomLuminance, float fGhostLuminance, float fGhostDistortion, float fStarLuminance,
                              ESTARLIBTYPE eStarType, float fStarInclination, float fChromaticAberration,
                              float fAfterimageSensitivity, // Current weight
                              float fAfterimageRatio,       // Afterimage weight
                              float fAfterimageLuminance) {
...
bool HDR_MeasureLuminance() {
    ...
    while (dwCurTexture > 0) {
        dv->SetRenderTarget(0, apSurfToneMap[dwCurTexture]);
        if (rd->m_bDeviceSupportsFP16Filter) {
            GetSampleOffsets_DownScale4x4Bilinear(rd->m_TexMan->m_Text_HDRToneMaps[dwCurTexture + 1]->m_Width, rd->m_TexMan->m_Text_HDRToneMaps[dwCurTexture + 1]->m_Height,
                                                  avSampleOffsets);
            ...
            GetSampleOffsets_DownScale4x4(rd->m_TexMan->m_Text_HDRToneMaps[dwCurTexture + 1]->m_Width, rd->m_TexMan->m_Text_HDRToneMaps[dwCurTexture + 1]->m_Height,
                                          avSampleOffsets);
    ...
    for (d = 0; d < starDef.m_nStarLines; d++) {
        ...
        for (p = 0; p < starLine.nPasses; p++) {
            ...
            if (rd->m_bDeviceSupportsMRT && srcDesc.Format == D3DFMT_G16R16F) {
                ...
                nBitPlanes = 2;
                bSplitOutput = FALSE;
            } else if (rd->m_bDeviceSupportsMRT && (srcDesc.Format == D3DFMT_A8R8G8B8)) {
