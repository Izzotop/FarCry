...
void STexPicD3D::ReleaseDriverTexture() {
    ...
    } else if (m_Bind == TX_FIRSTBIND && !m_Id) {
        if (m_LoadedSize)
            gRenDev->m_TexMan->m_StatsCurTexMem -= m_LoadedSize;
        else
            gRenDev->m_TexMan->m_StatsCurTexMem -= m_Size;
...
void STexPicD3D::Set(int nTexSlot) {
    ...
    if (m_RefTex.m_MinFilter != r->m_RP.m_TexStages[tmu].MinFilter || m_RefTex.m_MagFilter != r->m_RP.m_TexStages[tmu].MagFilter ||
...
int SShaderTexUnit::mfSetTexture(int nt) {
            ...
            if (bind >= TX_FIRSTBIND) {
                ...
                case TO_FROMLIGHT: {
                    ...
                    if (!bRes && !(rd->m_RP.m_PersFlags & RBPF_MULTILIGHTS))
                        Warning(VALIDATOR_FLAG_TEXTURE, 0, "Couldn't set projected texture for %d light source (Shader: '%s')\n", rd->m_RP.m_nCurLight,
                                rd->m_RP.m_pShader->m_Name.c_str());
                } break;
                ...
                case TO_ENVIRONMENT_TEX: {
                    ...
                    if ((rd->m_RP.m_pShader->m_Flags3 & (EF3_CLIPPLANE_FRONT | EF3_REFLECTION)))
                        bReflect = true;
                    cm = rd->m_cEF.mfFindSuitableEnvTex(Pos, Angs, true, 0, false, rd->m_RP.m_pShader, rd->m_RP.m_pShaderResources, rd->m_RP.m_pCurObject, bReflect,
                                                        gRenDev->m_RP.m_pRE);
                    if (cm)
                        cm->m_Tex->Set();
                    else
    ...
    if (m_nFlags & (FTU_CLAMP | FTU_NOMIPS | FTU_PROJECTED | FTU_FILTERBILINEAR | FTU_FILTERTRILINEAR | FTU_FILTERNEAREST)) {
        ...
        if ((m_nFlags & FTU_FILTERNEAREST) &&
            (rd->m_RP.m_TexStages[nt].nMipFilter != D3DTEXF_NONE || rd->m_RP.m_TexStages[nt].MinFilter != D3DTEXF_POINT || rd->m_RP.m_TexStages[nt].MagFilter != D3DTEXF_POINT)) {
    ...
    if (ti->m_nMips) {
        ...
        for (i = 0; i < ti->m_nMips; i++) {
            ...
            if (ti->m_Flags & FT_DXT) {
                ...
                offset += size;
            } else if (srcFormat == D3DFMT_X8R8G8B8 || srcFormat == D3DFMT_A8R8G8B8 || srcFormat == D3DFMT_X8L8V8U8 || srcFormat == D3DFMT_Q8W8V8U8) {
                size = w * h * 4;
                // Lock the texture to copy the image data into the texture
                hr = pID3DSrcTexture->LockRect(i, &d3dlr, nullptr, 0);
                // Copy data to the texture
...
void CD3D9TexMan::D3DCreateVideoTexture(int tgt, byte* src, int wdt, int hgt, int depth, D3DFORMAT SrcFormat, D3DFORMAT DstFormat, STexPicD3D* ti, bool bMips, int CubeSide,
                                        PALETTEENTRY* pe, int DXTSize) {
    int i = 0;
    int offset = 0;
    int size;
...
STexPic* CD3D9TexMan::CreateTexture(const char* name, int wdt, int hgt, int depth, uint flags, uint flags2, byte* dst, ETexType eTT, float fAmount1, float fAmount2, int DXTSize,
    ...
    if (dst) {
        ...
        if (eTF == eTF_8888 || eTF == eTF_RGBA) {
            ...
            if (gcpRendD3D->mFormatDepth16.BitsPerPixel) {
                ...
                return nullptr;
        } else if (eTF == eTF_DXT1 || eTF == eTF_DXT3 || eTF == eTF_DXT5) {
            if (eTF == eTF_DXT1)
                srcFormat = D3DFMT_DXT1;
            else if (eTF == eTF_DXT3)
                srcFormat = D3DFMT_DXT3;
            else if (eTF == eTF_DXT5)
                srcFormat = D3DFMT_DXT5;
        } else if (eTF == eTF_0565 || eTF == eTF_0555) {
            srcFormat = D3DFMT_R5G6B5;
        } else if (eTF == eTF_4444) {
        ...
        if (ti->m_Flags & FT_3DC) {
            ...
            } else if (gcpRendD3D->m_bDeviceSupportsComprNormalmaps > 1) {
            ...
            } else if (!(ti->m_Flags & FT_HASALPHA) || ti->m_eTT == eTT_Bumpmap || ti->m_eTT == eTT_DSDTBump) // 3 components;
            {
...
void CD3D9TexMan::ScanEnvironmentTexture(SEnvTexture* cm, SShader* pSH, SRenderShaderResources* pRes, int RendFlags, bool bUseExistingREs) {
    static float Smat[16] = {0.5f, 0, 0, 0, 0, -0.5f, 0, 0, 0, 0, 0.5f, 0, 0.5f, 0.5f, 0.5f, 1.0f};
...
void CD3D9TexMan::DrawToTexture(Plane& Pl, STexPic* Tex, int RendFlags) {
    ...
    if (!(Tex->m_Flags & FT_ALLOCATED) || (Tex->m_Flags & FT_HDR) != nHDRFl) {
...
#else
        STexPic* tp = r->m_TexMan->CreateTexture(nullptr, tex_size, tex_size, 1, FT_NOMIPS | FT_ALLOCATED, FT2_NODXT, data, eTT_Base, -1.0f, -1.0f, 0, Tex);
        ...
        HRESULT h = D3DXCreateTexture(((CD3D9Renderer*)gRenDev)->mfGetD3DDevice(), tex_size, tex_size, 1, D3DUSAGE_DYNAMIC, dc.Format, D3DPOOL_DEFAULT,
                                      ((LPDIRECT3DTEXTURE9*)&tp->m_RefTex.m_VidTex));
#endif
        delete[] data;
    }
...
void CD3D9TexMan::Update() {
    ...
    if (bChangedNormalMapCompressed || CRenderer::CV_r_texresolution != m_CurTexResolution || CRenderer::CV_r_texbumpresolution != m_CurTexBumpResolution ||
        CRenderer::CV_r_texquality != m_CurTexQuality || CRenderer::CV_r_texbumpquality != m_CurTexBumpQuality) {
        for (i = 0; i < m_Textures.Num(); i++) {
            ...
            } else if (tp->m_eTT == eTT_Bumpmap || tp->m_eTT == eTT_DSDTBump) {
    ...
    if (CRenderer::CV_r_logusedtextures == 1 || CRenderer::CV_r_logusedtextures == 3 || CRenderer::CV_r_logusedtextures == 4) {
        ...
        static char* sTexType[] = {"Base", "Cubemap", "AutoCubemap", "Bump", "DSDTBump", "Rectangle", "3D"};
        static char* sTexFormat[] = {"Unknown", "Index8", "HSV",           "0888",         "8888",        "RGBA", "8000",     "0565", "0555", "4444",   "1555", "DXT1",
                                     "DXT3",    "DXT5",   "SIGNED_HILO16", "SIGNED_HILO8", "SIGNED_RGB8", "RGB8", "DSDT_MAG", "DSDT", "V8U8", "V16U16", "0088", "DEPTH"};
        ...
        if (CRenderer::CV_r_logusedtextures == 1 || CRenderer::CV_r_logusedtextures == 3) {
            for (i = 0; i < m_Textures.Num(); i++) {
                ...
                } else if (CRenderer::CV_r_logusedtextures == 1 && m_Textures[i] && m_Textures[i]->m_bBusy &&
                           m_Textures[i]->m_Bind != gRenDev->m_TexMan->m_Text_NoTexture->m_Bind && (m_Textures[i]->m_Flags2 & FT2_WASLOADED) &&
                           !(m_Textures[i]->m_Flags2 & FT2_WASUNLOADED)) {
                    if (m_Textures[i]->m_eTT != eTT_Cubemap || !m_Textures[i]->m_CubeSide)
                        Texs.AddElem(m_Textures[i]);
                }
            }
            ...
            for (i = 0; i < Texs.Num(); i++) {
                fprintf(fp, "%d\t\tType: %s\t\tFormat: %s\t\t(%s)\n", Texs[i]->m_Size, sTexType[Texs[i]->m_eTT], (Texs[i]->m_Flags & FT_3DC) ? "3DC" : sTexFormat[Texs[i]->m_ETF],
                        *Texs[i]->m_SearchName);
                Size += Texs[i]->m_Size;
        ...
        for (i = 0; i < Texs.Num(); i++) {
            if (fp)
                fprintf(fp, "%.3fKb\t\tType: %s\t\tFormat: %s\t\t(%s)\n", Texs[i]->m_Size / 1024.0f, sTexType[Texs[i]->m_eTT],
                        (Texs[i]->m_Flags & FT_3DC) ? "3DC" : sTexFormat[Texs[i]->m_ETF], Texs[i]->m_SearchName.c_str());
            else {
                sprintf(buf, "%.3fKb  Type: %s  Format: %s  (%s)", Texs[i]->m_Size / 1024.0f, sTexType[Texs[i]->m_eTT],
                        (Texs[i]->m_Flags & FT_3DC) ? "3DC" : sTexFormat[Texs[i]->m_ETF], Texs[i]->m_SearchName.c_str());
        ...
        for (i = 0; i < Texs.Num(); i++) {
            ...
            if (Texs[i]->m_Flags2 & FT2_WASUNLOADED)
                continue;
            if (fp)
                fprintf(fp, "%.3fKb %d mips (%.3fKb)\t\tType: %s \t\tFormat: %s\t\t(%s)\n", Texs[i]->m_Size / 1024.0f, Texs[i]->m_nMips, Texs[i]->m_LoadedSize / 1024.0f,
                        sTexType[Texs[i]->m_eTT], (Texs[i]->m_Flags & FT_3DC) ? "3DC" : sTexFormat[Texs[i]->m_ETF], Texs[i]->m_SearchName.c_str());
            else {
                sprintf(buf, "%.3fKb  Type: %s  Format: %s  (%s)", Texs[i]->m_Size / 1024.0f, sTexType[Texs[i]->m_eTT],
                        (Texs[i]->m_Flags & FT_3DC) ? "3DC" : sTexFormat[Texs[i]->m_ETF], Texs[i]->m_SearchName.c_str());
                rd->TextToScreenColor(4, nY, 0, 1, 0, 1, buf);
        ...
        sprintf(buf, "All Normal Maps: %d (FullSize: %.3fMb, All Loaded Size: %.3fMb, Loaded MIPS: %.3fMb)", TexsNM.Num(), AllSizeNM / (1024.0f * 1024.0f),
                SizeNM / (1024.0f * 1024.0f), PartSizeNM / (1024.0f * 1024.0f));
        rd->TextToScreenColor(4, 19, 1, 1, 0, 1, buf);

        Texs.Free();
        for (i = 0; i < m_Textures.Num(); i++) {
            if (m_Textures[i] && m_Textures[i]->m_bBusy && m_Textures[i]->m_Bind != gRenDev->m_TexMan->m_Text_NoTexture->m_Bind &&
                m_Textures[i]->m_AccessFrame == rd->m_nFrameUpdateID) {
                if (m_Textures[i]->m_eTT != eTT_Cubemap || !m_Textures[i]->m_CubeSide)
                    Texs.AddElem(m_Textures[i]);
            }
        }
...
void CD3D9TexMan::GenerateFogMaps() {
    int i, j;
    {
        ...
        gRenDev->m_TexMan->m_Text_Fog = CreateTexture("$Fog", 128, 128, 1, FT_CLAMP | FT_NOMIPS | FT_NOREMOVE | FT_HASALPHA, FT2_NODXT | FT2_NOANISO, &Data1[0][0][0], eTT_Base,
        ...
        // gRenDev->m_TexMan->m_Text_Fog_Enter = DownloadTexture("(FogEnter)", 64, 64, FT_CLAMP | FT_NOMIPS | FT_NOREMOVE | FT_HASALPHA, FT2_NODXT, &Data2[0][0][0], eTT_Base, 0,
        // nullptr, 0, eTF_8888); gRenDev->m_TexMan->m_Text_Fog_Enter->SaveTGA("FogEnter.tga", false);
...
inline float frnd(float max = 1.0) {
    return max * INV_RAND_MAX * rand();
}
inline float frnd(float min, float max) {
...
void CFurNormalMap::Update(EShaderPassType eShPass, float dt, SShaderPassHW* slw, bool bUseSimulation) {
    ...
    if (vpNormGen && fpNormGen) {
        ...
        struct_VERTEX_FORMAT_P3F_TEX2F pScreen[] = {
            Vec3(-1, -1, 0), 0, 1, Vec3(-1, 1, 0), 0, 0, Vec3(1, -1, 0), 1, 1, Vec3(1, 1, 0), 1, 0,
        };
...
void CD3D9TexMan::GenerateFlareMap() {
    ...
    gRenDev->m_TexMan->m_Text_Flare =
        CreateTexture("$Flare", 32, 4, 1, FT_CLAMP | FT_NOREMOVE, FT2_NODXT | FT2_NOANISO, &data[0][0][0], eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8888);
}
...
//-----------------------------------------------------------------------------
static void InitWhiteNoiseArray() {
    int i, j, k;
...
void CD3D9TexMan::GenerateNoiseVolumeMap() {
    ...
    if (FAILED(hr = D3DXCreateVolumeTexture(gcpRendD3D->mfGetD3DDevice(), NOISE_DIMENSION, NOISE_DIMENSION, NOISE_DIMENSION, NOISE_MIP_LEVELS, 0, D3DFMT_A8, D3DPOOL_MANAGED,
                                            &pID3DVolTexture)))
        return;
...
void CD3D9TexMan::GenerateAttenMap() {
    ...
    if (rd->mFormatA8L8.BitsPerPixel)
        d3dFMT = D3DFMT_A8L8;
    else if (rd->mFormatA8.BitsPerPixel)
        d3dFMT = D3DFMT_A8;
...
void CD3D9TexMan::GenerateDepthLookup() {
    ...
    gRenDev->m_TexMan->m_Text_DepthLookup = CreateTexture("$DepthMap", 2048, 1, 1, FT_CLAMP | FT_NOREMOVE | FT_HASALPHA | FT_NOMIPS | FT_PROJECTED, FT2_NODXT | FT2_NOANISO,
                                                          (byte*)&data[0], eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8888);
    ...
    gRenDev->m_TexMan->m_Text_Depth =
        CreateTexture("$Depth", 4, 4, 1, FT_CLAMP | FT_NOREMOVE | FT_HASALPHA | FT_NOMIPS, FT2_NODXT, (byte*)&data2[0], eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8888);

    pMap = &data2[0][0];
    for (i = 0; i < 4 * 4; i++) {
        *pMap++ = D3DCOLOR_RGBA(0xff, 0xff, 0xff, 0);
    }
    gRenDev->m_TexMan->m_Text_WhiteShadow = CreateTexture("$WhiteShadow", 4, 4, 1, FT_CLAMP | FT_NOREMOVE | FT_HASALPHA | FT_NOMIPS, FT2_NODXT | FT2_NOANISO, (byte*)&data2[0],
                                                          eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8888);
    ...
    gRenDev->m_TexMan->m_Text_Gradient =
        CreateTexture("$AlphaGradient", 256, 1, 1, FT_NOREMOVE | FT_HASALPHA | FT_NOMIPS, FT2_NODXT | FT2_NOANISO, (byte*)&data3[0], eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8000);
