...
void CLeafBuffer::UpdateCustomLighting(float fBackSideLevel, Vec3d vStatObjAmbientColor, const Vec3d& vLight, bool bCalcLighting) {
    ...
    for (int i = 0; i < m_SecVertCount; i++) {
        ...
        if (bCalcLighting) {
            ...
            if (fBackSideLevel < 1.f) { // make back side darker
...
void CLeafBuffer::AddRenderElements(CCObject* pObj, int DLightMask, int nTemplate, int nFogVolumeID, int nSortId, IMatInfo* pIMatInfo) {
    ...
    for (int i = 0; i < m_pMats->Count(); i++) {
        ...
        if (pIMatInfo) { // Assume that the root material is the first material, sub materials start from index 1.
            if (i == 0)
...
void CLeafBuffer::Render(const SRendParams& rParams, CCObject* pObj, TArray<int>& ShaderTemplates, int e_overlay_geometry, bool bNotCurArea, IMatInfo* pMaterial,
                         bool bSupportDefaultShaderTemplates) {
    ...
    for (int i = 0; i < m_pMats->Count(); i++) {
        ...
        if (pREOcLeaf && pShader) {
            ...
            bool bTransparent = (pObj->m_Color.a < 1.f || !(pShader->mfGetTemplate(-1)->m_Flags2 & EF2_OPAQUE) || (sr && sr->m_Opacity != 1.0f));
            ...
            if (bTransparent) {
                ...
                if (!pObjTransp && pObj->m_DynLMMask != rParams.nStrongestDLightMask) { // make object for transparent geometry since it will use different light mask and
            ...
            if (nSortValue == EFSLIST_STENCIL && bTransparent)
                gRenDev->EF_AddEf_NotVirtual(rParams.nFogVolumeID, pREOcLeaf, pShader, sr, (bTransparent && pObjTransp) ? pObjTransp : pObj, nTempl, pStateShader, 0);
            else
                gRenDev->EF_AddEf_NotVirtual(rParams.nFogVolumeID, pREOcLeaf, pShader, sr, (bTransparent && pObjTransp) ? pObjTransp : pObj, nTempl, pStateShader, nSortValue);
        }
    } // i
}
