...
class CRERandom {
public:
    enum EMersenneTwisterInitialSeed { INITIAL_SEED = 4357U };
...
private:
    ...
    void Reload() {
        ...
        for (m_ulIndex = 0; m_ulIndex < N - M; ++m_ulIndex) {
            ulY = (m_ulState[m_ulIndex] & UPPER_MASK) | (m_ulState[m_ulIndex + 1] & LOWER_MASK);
    ...
    unsigned long TemperingShiftU(unsigned long ulX) { return (ulX >> 11); }

    unsigned long TemperingShiftS(unsigned long ulX) { return (ulX << 7); }

    unsigned long TemperingShiftT(unsigned long ulX) { return (ulX << 15); }

    unsigned long TemperingShiftL(unsigned long ulX) { return (ulX >> 18); }
...
public:
    ...
    void PostLoad(unsigned long ulSeed, float fWindDirection, float fWindSpeed, float fWaveHeight, float fDirectionalDependence, float fChoppyWavesFactor,
...
private:
    ...
    _inline float PhillipsSpectrum(float fKx, float fKy) {
        ...
        return (m_fWaveHeight * cry_expf(-1.0f / sqrf(fKLength * m_fLargestPossibleWave) - sqrf(fKLength * m_fSuppressSmallWaves)) *
                cry_powf(fKx * m_fWindX + fKy * m_fWindY, m_fDirectionalDependence) / cry_powf(fKLength, 4.0f));
    }
