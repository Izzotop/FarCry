...
SLightIndicies* CREOcLeaf::mfGetIndiciesForLight(CDLight* pDLight) {
    ...
    for (i = 0; i < m_LIndicies->Num(); i++) {
        ...
        //  if ((li->m_AssociatedLight.m_Flags & DLF_LIGHTTYPE_MASK) == (pDLight->m_Flags & DLF_LIGHTTYPE_MASK) && li->m_AssociatedLight.m_Origin == pDLight->m_Origin &&
        //  li->m_AssociatedLight.m_Orientation.m_vForward == objPos && li->m_AssociatedLight.m_Orientation.m_vRight == objAng && li->m_AssociatedLight.m_Orientation.m_vUp[0] ==
        //  fObjScale)
        if ((li->m_AssociatedLight.m_Flags & DLF_LIGHTTYPE_MASK) == (pDLight->m_Flags & DLF_LIGHTTYPE_MASK) && IsEquivalent(li->m_AssociatedLight.m_Origin, pDLight->m_Origin) &&
            IsEquivalent(li->m_AssociatedLight.m_Orientation.m_vForward, objPos) && IsEquivalent(li->m_AssociatedLight.m_Orientation.m_vRight, objAng) &&
            li->m_AssociatedLight.m_Orientation.m_vUp[0] == fObjScale) {
            if (li->m_AssociatedLight.m_NumCM) {
                ...
                if (pDLight->m_Flags & DLF_PROJECT) {
                    ...
                    if (li->m_AssociatedLight.m_fLifeTime + 1.0f < gRenDev->m_RP.m_RealTime) {
                        if (!(li->m_AssociatedLight.m_NumCM & 2))
                            bNeedList = true;
                    } else if (CRenderer::CV_r_cullgeometryforlights == 2)
                        bNeedList = true;
                    if (bNeedList) {
                ...
                } else if (pDLight->m_Flags & DLF_POINT) {
                    bool bNeedList = false;
...
bool CREOcLeaf::mfCheckUpdate(int nVertFormat, int Flags) {
    ...
    if (bWasReleased && !lb->m_pSecVertBuffer && !(Flags & FHF_FORANIM) && lb->m_pVertexBuffer) {
        ...
        if (gRenDev->m_RP.m_pCurObject && gRenDev->m_RP.m_pCurObject->m_pCharInstance) {
            ...
            double time0 = 0;
            ticks(time0);
            gRenDev->m_RP.m_pCurObject->m_pCharInstance->ProcessSkinning(Vec3(zero), gRenDev->m_RP.m_pCurObject->m_Matrix, gRenDev->m_RP.m_pCurObject->m_nTemplId,
                                                                         gRenDev->m_RP.m_pCurObject->m_nLod, true);
            unticks(time0);
...
void* CREOcLeaf::mfGetPointer(ESrcPointer ePT, int* Stride, int Type, ESrcPointer Dst, int Flags) {
    ...
    case eSrcPointer_TexLM:
        if (gRenDev->m_RP.m_pCurObject && gRenDev->m_RP.m_pCurObject->m_pLMTCBufferO) { // separate stream for lightmaps
