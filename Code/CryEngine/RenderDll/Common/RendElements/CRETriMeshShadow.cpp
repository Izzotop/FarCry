...
bool CRETriMeshShadow::mfCheckUpdate(int nVertFormat, int Flags) {
    if (m_bAnimatedObject) { // character animation: everything already calculated, always use slot 0
    ...
    if (!pSVInfo) // if not found - select new slot
    {
        ...
        for (int i = 0; i < MAX_SV_INSTANCES; i++)
            if (m_arrLBuffers[i].pVB && m_arrLBuffers[i].pLightOwner == fakeLight.m_pOwner && m_arrLBuffers[i].pShadowCaster == pObj->m_CustomData &&
        ...
        {             // find empty slot or slot with smallest frame id
            ...
            for (int i = 0; i < MAX_SV_INSTANCES; i++) {
                if (!m_arrLBuffers[i].pVB) { // search for free slot
                    pSVInfo = &m_arrLBuffers[i];
                    m_nCurrInst = i;
                    nSmalestFrameId = 0;
                    break;
                ...
                if (m_arrLBuffers[i].nFrameId < nSmalestFrameId) { // search for oldest slot at the same time
    ...
    if (pSVInfo->pVB && pSVInfo->pVB->m_pMats && pSVInfo->pVB->m_pMats->Count() && pSVInfo->pVB->m_pMats->Get(0)->pRE)
...
bool CRETriMeshShadow::mfCheckUnload() { // remove all not used leafbuffers
    for (int i = 0; i < MAX_SV_INSTANCES; i++)
...
void CRETriMeshShadow::PrintStats() {
    gRenDev->TextToScreenColor(8, 20, 0, 2, 0, 1, "Shadow volumes stats: Rebuild: %d, Used: %d, Allocated: %d", CRETriMeshShadow::GetAndResetRebuildsPerFrrameCounter(),
                               CRETriMeshShadow::GetAndResetShadowVolumesPerFrrameCounter(), CRETriMeshShadow::GetShadowVolumesAllocatedCounter());
}
