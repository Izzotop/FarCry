...
void CREPolyBlend_Base::mfSetVerts(CCObject* obj, Vec3d& orgo, uint c, SOrient* ori) {
    ...
    static float tverts[4][2] = {{0, 0}, {1, 0}, {1, 1}, {0, 1}};
...
void CREAnimPolyBlend::mfPrepare() {
    CCObject* obj = gRenDev->m_RP.m_pCurObject;
    if (!obj)
        return;
...
void CREPolyBlend_Base::mfCompileOrients(SShader* ef, int* nums, SOrient* Orients[], char* scr) {
    if (!scr || !scr[0]) {
        ...
        *nums = 1;
        ...
        int ors[16];
        *nums = sscanf(scr, "%i %i %i %i %i %i %i %i %i %i %i %i %i %i %i %i", &ors[0], &ors[1], &ors[2], &ors[3], &ors[4], &ors[5], &ors[6], &ors[7], &ors[8], &ors[9], &ors[10],
                       &ors[11], &ors[12], &ors[13], &ors[14], &ors[15]);
        if (!(*nums)) {
            Warning(0, 0, "Can't declare orient for effector '%s'\n", ef->m_Name.c_str());
            *nums = 1;
            Orients[0] = &gRenDev->m_cEF.m_Orients[0];
...
bool CREPolyBlend_Base::mfCompile(SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{eTrace, "Trace"},
                                   {eType, "Type"},
                                   {eScale, "Scale"},
                                   {eScalX, "ScalX"},
                                   {eScalY, "ScalY"},
