...
CREParticleSpray& CREParticleSpray::operator=(const CREParticleSpray& Orig) {
    ...
    // SET THE LAST PARTICLE TO POINT TO nullptr
...
bool CREParticleSpray::mfInitParticleSystem() {
    int loop;
...
bool CREParticleSpray::mfInitEmitter(SEmitter* emitter) {
    ...
    emitter->particle = nullptr; // nullptr TERMINATED LINKED LIST
    return TRUE;
}
...
bool CREParticleSpray::mfUpdateParticle(SParticle* particle, SEmitter* emitter) {
    ...
    if (particle != nullptr && particle->life > 0) {
        ...
        if (emitter->eCollisionType && !particle->bSpark) {
            ...
            } else if (emitter->eCollisionType == ePCollision_True) {
                Vec3d v0, v1, org;
        ...
        return TRUE;
    } else if (particle != nullptr && particle->life == 0) {
...
void CREParticleSpray::mfPrepare() {
            ...
            while (pt) {
                ...
                for (int i = 0; i < steps; i++) {
                    if (n + 4 >= gRenDev->m_RP.m_MaxVerts || gRenDev->m_RP.m_RendNumIndices + 2 * 3 >= gRenDev->m_RP.m_MaxTris * 3) {
                        ...
                        gRenDev->EF_Start(gRenDev->m_RP.m_pShader, gRenDev->m_RP.m_pStateShader, nullptr,
                                          gRenDev->m_RP.m_pFogVolume ? (gRenDev->m_RP.m_pFogVolume - &gRenDev->m_RP.m_FogVolumes[0]) : 0, this);
                        n = 0;
    ...
    if (em->Life) {
        if (em->Life < 0) {
            ...
            return;
        } else if (gRenDev->m_RP.m_RealTime - gRenDev->m_RP.m_pCurObject->m_StartTime >= em->Life) {
            // gRenDev->m_RP.m_pCurObject->m_ObjFlags |= FOB_NOTVISIBLE;
            return;
        }
    }
...
bool CREParticleSpray::mfCompileMoveTypeWave(SShader* ef, SPartMoveStage* pm, char* scr) {
    ...
    enum { eWaveForm = 1 };
    static tokenDesc commands[] = {{eWaveForm, "WaveForm"},
...
void CREParticleSpray::mfCompileCollision(SShader* ef, SEmitter* em, char* scr, char* Collide) {
    ...
    static tokenDesc commands[] = {{ePlaneAxis, "PlaneAxis"},
...
void CREParticleSpray::mfCompileParticleInfo(SShader* ef, SParticleInfo* pi, char* scr) {
    ...
    static tokenDesc commands[] = {{eYaw, "Yaw"},
                                   {eVarYaw, "VarYaw"},
                                   {ePitch, "Pitch"},
                                   {eVarPitch, "VarPitch"},
                                   {eSpeed, "Speed"},
    ...
    while ((cmd = shGetObject(&scr, commands, &name, &params)) > 0) {
        ...
        case eMove:
            ...
            } else if (mfCompileMove(ef, &pi->mMoves[pi->mNumMoves], pi, params)) {
                pi->mNumMoves++;
            }
            break;
        }
...
bool CREParticleSpray::mfCompile(SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{eTotalParticles, "TotalParticles"},
