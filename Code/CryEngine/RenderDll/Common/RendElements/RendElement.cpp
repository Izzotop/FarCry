...
static _inline int Compare(SRendItemPre& a, SRendItemPre& b) {
    ...
    if (a.SortVal.SortVal < b.SortVal.SortVal)
        return -1;
    else if (a.SortVal.SortVal > b.SortVal.SortVal)
        return 1;
    else
        return 0;
#else
...
bool CRendElement::mfCull(CCObject* obj) {
    return false;
}
bool CRendElement::mfCull(CCObject* obj, SShader* ef) {
    return false;
}
...
void SRendItem::mfCalcHalfAngles_Terrain(int type, byte* ha, int StrHA) {
    ...
    if (type == GL_FLOAT) {
        for (int i = 0; i < numVerts; i++, verts += StrVrt, lv += StrLV, ha += StrHA, normals += StrN) {
            ...
            float tang[3];
            tang[0] = binorm[1] * n[2] - binorm[2] * n[1];
            tang[1] = binorm[2] * n[0] - binorm[0] * n[2];
            tang[2] = binorm[0] * n[1] - binorm[1] * n[0];
...
void SRendItem::mfComputeTangent(const Vec3d& v0, const Vec3d& v1, const Vec3d& v2, const vec2_t t0, const vec2_t t1, const vec2_t t2, Vec3d& tangent, Vec3d& binormal, float& sign,
                                 Vec3d& face_normal) {
    Vec3d bi;
    Vec3d cp;
    Vec3d e0;
...
void SRendItem::mfCalcTangentSpaceVectors() {
    int i, j;
    int in[3];
