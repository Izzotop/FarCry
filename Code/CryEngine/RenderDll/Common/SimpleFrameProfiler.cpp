...
CSimpleFrameProfilerInfo::CSimpleFrameProfilerInfo(const char* szName) : m_szName(szName), m_nTickTimer(0) {
    m_nIndex = g_nCount * 2;
    assert(g_nCount < 64);
...
void CSimpleFrameProfilerInfo::drawStatistics(float fRow, float* fColor, const char* szLabel, CProfilerTimerHistory<float, 64>& rProfiler) {
    ...
    char szBuf[128];
    sprintf(szBuf, "%22s  %7.2f%7.2f%7.2f%7.2f", szLabel, rProfiler.getMax(), rProfiler.getAve(), rProfiler.getLast(), rProfiler.getMin());
