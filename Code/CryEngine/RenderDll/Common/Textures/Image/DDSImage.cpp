...
CImageDDSFile::CImageDDSFile(byte* ptr, long filesize) : CImageFile() {
    ...
    if ((ddsh->dwReserved1[0] & DDS_RESF1_NORMALMAP) || !stricmp(ext, ".ddn") || !stricmp(ext, ".ddp") ||
        (strlen(m_CurFileName) > 4 && (strstr(m_CurFileName, "_ddn") || strstr(m_CurFileName, "_ddp"))))
        mfSet_Flags(FIM_NORMALMAP);
    else if ((ddsh->dwReserved1[0] & DDS_RESF1_DSDT) || !stricmp(ext, ".ddt") || (strlen(m_CurFileName) > 4 && strstr(m_CurFileName, "_ddt"))) {
    ...
    for (int dpt = 0; dpt < nDepth; dpt++) {
        if (m_eFormat == eIF_DXT1 || m_eFormat == eIF_DXT3 || m_eFormat == eIF_DXT5) {
            ...
            nOffsSrc += size;
            nOffsDst += size;
        } else if (m_eFormat == eIF_DDS_LUMINANCE) {
            int size = mfSizeWithMips(filesize, sx, sy, numMips);
            cryMemcpy(&m_pByteImage[nOffsDst], &ptr[nOffsSrc], size);
            nOffsSrc += size;
            nOffsDst += size;
        } else if (m_eFormat == eIF_DDS_RGBA8 || m_eFormat == eIF_DDS_RGBA4) {
            int size = mfSizeWithMips(filesize, sx, sy, numMips);
            cryMemcpy(&m_pByteImage[nOffsDst], &ptr[nOffsSrc], size);
            nOffsSrc += size;
            nOffsDst += size;
        } else if (m_eFormat == eIF_DDS_RGB8) {
            int size = mfSizeWithMips(filesize, sx, sy, numMips);
            int n = size / 3;
            int sizeDst = n * 4;
            for (int i = 0; i < n; i++) {
        ...
        } else if (m_eFormat == eIF_DDS_DSDT) {
            int size = mfSizeWithMips(filesize, sx, sy, numMips);
            int n = size / 3;
            int sizeDst = n * 4;
            for (int i = 0; i < n; i++) {
...
void WriteDDS(byte* dat, int wdt, int hgt, int Size, const char* name, EImFormat eF, int NumMips) {
    ...
    if (len > 4) {
        ...
        else if (!stricmp(&name[len - 4], ".ddt"))
