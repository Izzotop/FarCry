...
CImageTgaFile::CImageTgaFile(byte* ptr, long filesize) : CImageFile() {
    ...
    if (tga_head.ImgType == TGA_Map || tga_head.ImgType == TGA_RLEMap || tga_head.ImgType == TGA_CompMap || tga_head.ImgType == TGA_CompMap4) { /* Color-mapped image */
    ...
    } else { /* Not colormap, so figure maxval from PixelSize. */
        mapped = 0;
...
static void get_map_entry(byte*& ptr, SRGBPixel* Value, int Size) {
    ...
    case 24: /* 8 bits each of blue green and red. */
        ...
        r = getbyte(ptr);
        if (Size == 32)
            ()getbyte(ptr); /* Read alpha byte & throw away. */
        break;
