...
CImageJpgFile::CImageJpgFile(byte* ptr, long filesize) : CImageFile() {
    ...
    ()my_jpeg_read_header(&cinfo, TRUE);
    ...
    ()my_jpeg_start_decompress(&cinfo);
    ...
    while (cinfo.output_scanline < cinfo.output_height) {
         ...
         */
        ()my_jpeg_read_scanlines(&cinfo, buffer, 1);
        ...
        } else if (cinfo.output_components == 3) { /* rgb triplets */
    ...
    ()my_jpeg_finish_decompress(&cinfo);
...
#if !defined(PS2) && !defined(WIN64) && !defined(nullptr_RENDERER)
    JPEG_CORE_PROPERTIES image;
