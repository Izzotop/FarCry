...
struct shColorBox {
    ...
    void ComputeVolume() {
        ...
        Volume = Sqr(Rx - Rm) * (R_COEF_SQ << HIST_SHIFT_R) + Sqr(Gx - Gm) * (G_COEF_SQ << HIST_SHIFT_G) + Sqr(Bx - Bm) * (B_COEF_SQ << HIST_SHIFT_B);
    }
...
void shQuantizePalette(SRGBPixel*& outpalette, int& maxcolors, SRGBPixel* transp) {
    ...
    while (boxcount < maxcolors) {
        ...
        enum { axisR, axisG, axisB } axis = (glen < rlen) ? ((rlen < blen) ? axisB : axisR) : ((glen < blen) ? axisB : axisG);
    ...
    memset(&outpalette[boxcount + delta], 0, (maxcolors - boxcount) * sizeof(SRGBPixel));
...
void shQuantizeRemap(SRGBPixel* image, int pixels, byte*& outimage, SRGBPixel* transp) {
    // Sanity check
...
void shQuantizeRemapDither(SRGBPixel* image, int pixels, int pixperline, SRGBPixel* palette, int colors, byte*& outimage, SRGBPixel* transp) {
    // Sanity check
    if (qState != qsCount && qState != qsRemap)
        return;
    ...
    if (qState == qsCount) {
        ...
        shInverseColormap(colors - delta, palette + delta, HIST_R_BITS, HIST_G_BITS, HIST_B_BITS, icmap);
        if (transp)
    ...
    while (count > 0) {
        ...
        for (int fspix = pixperline; fspix; fspix--, cursrc += dir, curdst += dir, curerr += dir3, nexterr += dir3) {
            ...
            byte pix = icmap[((r >> (8 - HIST_R_BITS)) << (HIST_G_BITS + HIST_B_BITS)) | ((g >> (8 - HIST_G_BITS)) << HIST_B_BITS) | ((b >> (8 - HIST_B_BITS)))];
            *curdst = pix;
...
void shQuantizeRGB(SRGBPixel* image, int pixels, int pixperline, byte*& outimage, SRGBPixel*& outpalette, int& maxcolors, bool dither) {
    shQuantizeBegin();
