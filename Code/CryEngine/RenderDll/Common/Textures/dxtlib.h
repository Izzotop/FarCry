...
HRESULT nvDXTcompress(unsigned char* raw_data, // pointer to data (24 or 32 bit)
                      ...
                      DWORD byte_pitch, CompressionOptions* options,
                      DWORD planes,                // 3 or 4
                      MIPcallback callback = nullptr, // callback for generated levels
                      RECT* rect = nullptr);          // subrect to operate on, nullptr is whole image
...
HRESULT LoadAllMipSurfaces(void * data, int iLevel)
{
    ...
    hr = pCurrentTexture->LockRect(iLevel, &lr, nullptr, 0);
    if (FAILED(hr))
        return hr;
    ...
    nvDXTcompress(raw_data, Width, Height, DXT3, true, 4, LoadAllMipSurfaces, nullptr);
...
unsigned char* nvDXTdecompress(int& w, int& h, int& depth, int& total_width, int& rowBytes, int& src_format, int SpecifiedMipMaps = 0);
