...
STexPic* CTexMan::TextureInfoForName(const char* nameTex, int numT, byte eTT, uint flags, uint flags2, int bind) {
    ...
    if (numT >= 0) {
        ...
        else if (m_Textures[i]->m_eTT == eTT_Cubemap)
...
STexPic* CTexMan::UploadImage(CImageFile* im, const char* name, uint flags, uint flags2, byte eTT, int bind, STexPic* ti, float fAmount1, float fAmount2) {
    ...
    case eIF_Bmp:
        if (im->mfGet_bps() == 32)
            txf = eTF_8888;
        else if (im->mfGet_bps() == 24)
            txf = eTF_0888;
        else if (im->mfGet_bps() == 8)
            txf = eTF_Index;
        break;
        ...
        if (eTT == eTT_Bumpmap && (ti->m_Flags & FT_3DC) && gRenDev->m_bDeviceSupportsComprNormalmaps) {
            ...
            if (bAllow) {
                ...
                for (nMip = 0; nMip < ti->m_nMips; nMip++) {
                    ...
                    } else if (gRenDev->m_bDeviceSupportsComprNormalmaps > 1) {
                        int nDst = Dst.Num();
                        Dst.Grow(wdt * hgt * 2);
                        byte* dst = &Dst[nDst];
                        if (!(ti->m_Flags & FT_3DC_A)) {
    ...
    for (i = 0; i < nT; i++) {
        for (int n = 0; gImgExt[nI][n]; n++) {
            ...
            if ((!bComp && !(flags2 & FT2_FORCEDXT)) || eTT == eTT_Bumpmap || eTT == eTT_DSDTBump) {
                if (!(im[i]->mfGet_Flags() & (FIM_NORMALMAP | FIM_DSDT)) &&
                    (im[i]->mfGetFormat() == eIF_DXT1 || im[i]->mfGetFormat() == eIF_DXT3 || im[i]->mfGetFormat() == eIF_DXT5)) {
...
#ifdef WIN64
            Warning(VALIDATOR_FLAG_TEXTURE, wasloaded[i],
                    "Error: CTexMan::LoadFromImage: Generate normal map from compressed texture: '%s', "
...
void CTexMan::ImagePreprocessing(CImageFile* im, uint flags, uint flags2, byte eTT, STexPic* ti) {
    ...
    if (!bPowerOfTwo) {
        if (imf == eIF_DXT1 || imf == eIF_DXT3 || imf == eIF_DXT5)
            Warning(VALIDATOR_FLAG_TEXTURE, im->m_FileName, "Error: CTexMan::ImagePreprocessing: Attempt to load of non-power-of-two compressed texture '%s' (%dx%d)",
                    im->m_FileName, width, height);
        else
            Warning(VALIDATOR_FLAG_TEXTURE, im->m_FileName, "Warning: CTexMan::ImagePreprocessing: Attempt to load of non-power-of-two texture '%s' (%dx%d)", im->m_FileName, width,
                    height);
        ...
        if (imf == eIF_DXT1 || imf == eIF_DXT3 || imf == eIF_DXT5) {
            ...
            if (bPowerOfTwo) {
                if (nMips > 1) {
                    ...
                    if (nLodDH != nLodDW) {
                        Warning(VALIDATOR_FLAG_TEXTURE, im->m_FileName,
                    ...
                    Warning(VALIDATOR_FLAG_TEXTURE, im->m_FileName, "Error: CTexMan::ImagePreprocessing: Scaling of '%s' compressed texture is dangerous (only one mip)",
                            im->m_FileName);
            }
#ifndef WIN64
            if (!bResample) {
...
byte* CTexMan::ConvertRGB_Gray(byte* src, STexPic* ti, int flags, ETEX_Format eTF) {
    ...
    else if (flags & FT_BUMP_DETALPHA)
        while (size) {
            *dst1 = src1[3];
            dst1++;
            src1 += 4;
    ...
    else if (CRenderer::CV_r_texgrayoverage) {
        while (size) {
    ...
    for (l = 1; l < nMips; l++) {
        ...
        for (j = 0; j < resh; j++) {
            for (i = 0; i < resw; i++) {
                ...
                int bavg;
                if (wmul == 1) {
                    avg = prev[wmul * i + hmul * j * reswp] + prev[wmul * i + (hmul * j + 1) * reswp];
                    bavg = bprev[wmul * i + hmul * j * reswp] + bprev[wmul * i + (hmul * j + 1) * reswp];
                    bavg /= 2;
                } else if (hmul == 1) {
                    avg = prev[wmul * i + hmul * j * reswp] + prev[wmul * i + 1 + hmul * j * reswp];
                    bavg = bprev[wmul * i + hmul * j * reswp] + bprev[wmul * i + 1 + hmul * j * reswp];
                    bavg /= 2;
                } else {
                    avg = prev[wmul * i + hmul * j * reswp] + prev[wmul * i + 1 + hmul * j * reswp] + prev[wmul * i + 1 + (hmul * j + 1) * reswp] +
                          prev[wmul * i + (hmul * j + 1) * reswp];
                    bavg = bprev[wmul * i + hmul * j * reswp] + bprev[wmul * i + 1 + hmul * j * reswp] + bprev[wmul * i + 1 + (hmul * j + 1) * reswp] +
...
void CTexMan::GenerateNormalMap(CImageFile** im, uint flags, uint flags2, byte eTT, float fAmount1, float fAmount2, STexPic* ti) {
    ...
    } else if ((im[0]->mfGet_Flags() & FIM_NORMALMAP) || (im[0]->mfGet_Flags() & FIM_DSDT))
        return;
    else {
        if (im[0]->m_pByteImage != dst[0]) {
            SAFE_DELETE_ARRAY(im[0]->m_pByteImage);
...
void CTexMan::ReloadAll(int nFlags) {
    ...
    for (i = 0; i < m_Textures.Num(); i++) {
        ...
        if (tp->m_pData32)
            CreateTexture(*tp->m_SearchName, tp->m_Width, tp->m_Height, tp->m_Depth, tp->m_Flags, tp->m_Flags2, tp->m_pData32, tp->m_eTT, tp->m_fAmount1, tp->m_fAmount2,
                          tp->m_DXTSize, tp, tp->m_Bind, tp->m_ETF);
        else
    ...
    for (i = 0; i < 256; i++) {
        ...
        if (i <= iInterpVal[0]) {
            ...
            while (true) {
                if (i <= iInterpVal[n + 1]) {
                    break;
                } else if (i <= iInterpVal[n + 2]) {
                    n++;
                    break;
                } else if (i <= iInterpVal[n + 3]) {
                    n += 2;
                    break;
                } else if (i <= iInterpVal[n + 4]) {
                    n += 3;
                    break;
                } else if (i <= iInterpVal[n + 5]) {
                    n += 4;
                    break;
                } else if (i <= iInterpVal[n + 6]) {
                    n += 5;
                    break;
                } else if (i <= iInterpVal[n + 7]) {
                    n += 6;
                    break;
                } else {
                    n += 7;
