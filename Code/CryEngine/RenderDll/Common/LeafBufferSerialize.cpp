...
bool CLeafBuffer::Serialize(int& nPos, uchar* pSerBuf, bool bSave, char* _szFolderName, char* _szFileName, double& dCIndexedMesh__LoadMaterial) { // Load
    int i;
    char szSignature[16] = "";
    ...
    for (i = 0; i < m_pMats->Count(); i++) {
        ...
        if (m_pMats->GetAt(i).m_dwNumSections) {
            ...
            LoadBuffer((void*)m_pMats->GetAt(i).m_pPrimitiveGroups, sizeof(SPrimitiveGroup) * m_pMats->GetAt(i).m_dwNumSections, pSerBuf, nPos);
        }
    }
...
bool CLeafBuffer::LoadMaterial(int m, const char* szFileName, const char* szFolderName, list2<CMatInfo>& lstMatTable, IRenderer* pRenderer, MAT_ENTITY* me, bool bFake) {
