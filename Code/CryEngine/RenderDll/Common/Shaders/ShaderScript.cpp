...
char** CShader::mfListInScript(char* scr) {
    ...
    static tokenDesc commands[] = {{eShader, "Shader"}, {eVersion, "Version"}, {eSubrScript, "SubrScript"}, {0, 0}};
...
char* CShader::mfPreprCheckMacros(char* buf, const char* nameFile) {
    ...
    while (true) {
        ...
        int nw = 0;
        word[0] = 0;
        while (nBuf[nPos] == 0x20 || nBuf[nPos] == 9 || nBuf[nPos] == 0xa || nBuf[nPos] == ',' || nBuf[nPos] == ';') {
            nPos++;
        }
        int nPrevPos = nPos;
        ...
        if (word[0]) {
            if (word[0] == '#') {
                if (!strcmp(&word[1], "if") || !strcmp(&word[1], "elif")) {
                    ...
                    while (nBuf[nPos] == 0x20 || nBuf[nPos] == 9) {
                        nPos++;
                    }
                    int nLog = 0;
