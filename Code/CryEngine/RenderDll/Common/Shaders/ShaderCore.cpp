            ...
            if (bOpaq) {
                ...
                if (!nT) {
                    if ((Passes[np].m_TUnits[nt].m_TexPic && (Passes[np].m_TUnits[nt].m_TexPic->m_Flags2 & FT2_DIFFUSETEXTURE)) ||
                        ...
                        bFound = true;
                } else if (Passes[np].m_TUnits[nt].m_TexPic || Passes[np].m_TUnits[nt].m_AnimInfo)
                    bFound = true;
            }
            if (bFound)
                break;
...
unsigned int SShader::GetUsedTextureTypes() {
    uint nMask = 0;
    int i, j, n;
    SShaderPass* ps;
    SShaderPassHW* psHW;
...
void CShader::mfClearAll() {
    ...
    for (i = 0; i < SLightMaterial::known_materials.Num(); i++) {
        if (SLightMaterial::known_materials[i] && SLightMaterial::known_materials[i]->name[0] == '$')
            iLog->Log("Warning: CShader::mfClearAll: Light material %s was not deleted (%d)", SLightMaterial::known_materials[i]->name,
                      SLightMaterial::known_materials[i]->m_nRefCounter);
...
static bool sNeedColorArray(EEvalRGB eRGB, EEvalAlpha eA) {
    if ((eRGB == eERGB_Fixed || eRGB == eERGB_Identity || eRGB == eERGB_NoFill || eRGB == eERGB_Wave || eRGB == eERGB_Object || eRGB == eERGB_OneMinusObject || eRGB == eERGB_RE ||
         eRGB == eERGB_OneMinusRE || eRGB == eERGB_World || eRGB == eERGB_Noise || eRGB == eERGB_Comps) &&
        (eA == eEALPHA_Fixed || eA == eEALPHA_Identity || eA == eEALPHA_NoFill || eA == eEALPHA_Wave || eA == eEALPHA_Object || eA == eEALPHA_OneMinusObject || eA == eEALPHA_RE ||
         eA == eEALPHA_OneMinusRE || eA == eEALPHA_World || eA == eEALPHA_Noise || eA == eEALPHA_Comps))
        return false;
    return true;
}
...
void CShader::mfOptimizeShaderHW(SShader* ef, TArray<SShaderPassHW>& Layers, int Stage) {
    ...
    if (Stage == 0) {
        ...
        if (Layers.Num()) {
            ...
            for (i = 0; i < Layers.Num(); i++) {
                ...
                for (j = 0; j < Layers[i].m_TUnits.Num(); j++) {
                    ...
                    // if (!ef->m_ePreprocess)
                    {
                        if (tl->m_TexPic == gRenDev->m_TexMan->m_Text_EnvCMap)
                        ...
                        else if (tl->m_TexPic == gRenDev->m_TexMan->m_Text_EnvLCMap)
                            ef->m_nPreprocess |= FSPR_SCANLCM;
                        else if (tl->m_TexPic == gRenDev->m_TexMan->m_Text_EnvTex)
                            ef->m_nPreprocess |= FSPR_SCANTEX;
                        else if (tl->m_TexPic == gRenDev->m_TexMan->m_Text_EnvScr)
                            ...
                            if (tl->m_TexPic == gRenDev->m_TexMan->m_Text_ScreenMap)
                                ...
                                else if (tl->m_TexPic == gRenDev->m_TexMan->m_Text_WaterMap)
                        ...
                        if (tl->m_TexPic) {
                            ...
                            else if (tl->m_TexPic->m_Bind >= TO_CUSTOM_TEXTURE_FIRST && tl->m_TexPic->m_Bind <= TO_CUSTOM_TEXTURE_LAST)
            ...
            else if (ef->m_Flags & EF_FOGSHADER)
...
SEnvTexture* CShader::mfFindSuitableEnvLCMap(Vec3d& Pos, bool bMustExist, int RendFlags, float fDistToCam, CCObject* pObj) {
    ...
    if (bMustExist)
        nUpdate = -2;
    else if (dist > MAX_ENVLIGHTCUBEMAPSCANDIST_THRESHOLD) {
        if (firstFree >= 0)
            nUpdate = firstFree;
        else
            nUpdate = -1;
    ...
    if (nUpdate == -2) {
        if (!bMustExist) {
            /*for (int i=0; i<6; i++)
            {
              if (gRenDev->m_TexMan->m_EnvLCMaps[firstForUse].m_nFrameCreated[i]>0 &&
            gRenDev->GetFrameID()-gRenDev->m_TexMan->m_EnvLCMaps[firstForUse].m_nFrameCreated[i]>=CRenderer::CV_r_envlightcmupdatefrequence)
              {
    ...
    if (nUpdate >= 0) {
        if (!(gRenDev->m_TexMan->m_EnvLCMaps[nUpdate].m_Tex->m_Flags & FT_ALLOCATED) ||
            (fDistToCam <= MAX_ENVLIGHTCUBEMAPSCANDIST_UPDATE && gRenDev->m_RP.m_PS.m_fEnvCMapUpdateTime < 0.1f)) {
            // Reuse old cube-map
...
SEnvTexture* CShader::mfFindSuitableEnvCMap(Vec3d& Pos, bool bMustExist, int RendFlags, float fDistToCam) {
    ...
    if (bMustExist)
        nUpdate = -2;
    else if (dist > MAX_ENVCUBEMAPSCANDIST_THRESHOLD) {
        if (firstFree >= 0)
            nUpdate = firstFree;
        else
            nUpdate = -1;
...
SEnvTexture* CShader::mfFindSuitableEnvTex(Vec3d& Pos, Vec3d& Angs, bool bMustExist, int RendFlags, bool bUseExistingREs, SShader* pSH, SRenderShaderResources* pRes,
                                           CCObject* pObj, bool bReflect, CRendElement* pRE) {
    SEnvTexture* cm = nullptr;
...
void SEfResTexture::Update(int nTU) {
    ...
    if (m_TexModificator.m_eUMoveType != ETMM_NoChange || m_TexModificator.m_eVMoveType != ETMM_NoChange || m_TexModificator.m_eRotType != ETMR_NoChange ||
        ...
        case ETMR_Fixed: {
            m_TexModificator.m_TexMatrix =
                m_TexModificator.m_TexMatrix * Matrix44(1, 0, 0, 0, 0, 1, 0, 0, m_TexModificator.m_RotOscCenter[0], m_TexModificator.m_RotOscCenter[1], 1, 0, 0, 0, 0, 1);
        ...
        if (m_TexModificator.m_Offs[0] != 0.0f || m_TexModificator.m_Offs[1] != 0.0f || m_TexModificator.m_Tiling[0] != 1.0f || m_TexModificator.m_Tiling[1] != 1.0f ||
            m_TexModificator.m_Rot[0] || m_TexModificator.m_Rot[1] || m_TexModificator.m_Rot[2]) {
            float du = m_TexModificator.m_Offs[0];
            float dv = m_TexModificator.m_Offs[1];
            float su = m_TexModificator.m_Tiling[0];
            float sv = m_TexModificator.m_Tiling[1];
            m_TexModificator.m_TexMatrix = m_TexModificator.m_TexMatrix * Matrix44(su, 0, 0, 0, 0, sv, 0, 0, du, dv, 1, 0, 0, 0, 0, 1);
...
SShaderCache* CShader::OpenCacheFile(const char* szName, float fVersion) {
    ...
    if (!rf->mfOpen(RA_READ)) {
        ...
        if (hd.m_SizeOf != sizeof(SShaderCacheHeader))
            bValid = false;
        else if (hd.m_MajorVer != (int)fVersion || hd.m_MinorVer != (int)(((float)fVersion - (float)(int)fVersion) * 10.1f))
            bValid = false;
        else if (rf->mfGetHolesSize() / rf->mfGetResourceSize() * 100 > 20)
            bValid = false;
        rf->mfClose();
        if (bValid) {
