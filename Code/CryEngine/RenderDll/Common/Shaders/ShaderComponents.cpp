...
bool CShader::mfGetParmComps(int comp, SParam* vpp, char* name, char* params, SShader* ef) {
    ...
    if (!name || !name[0]) {
        ...
        return true;
    } else if (!stricmp(name, "Wave")) {
        SParamComp_Wave p;
        gRenDev->m_cEF.mfCompileWaveForm(&p.m_WF, params);
        vpp->m_Comps[comp] = SParamComp::mfAdd(&p);
        return true;
    } else if (!stricmp(name, "ObjWaveX")) {
...
bool CShader::mfCompilePlantsTMoving(char* scr, SShader* ef, TArray<SParam>* Params, int reg) {
    ...
    static tokenDesc commands[] = {{eWaveX, "WaveX"},
                                   {eWaveY, "WaveY"},
...
void CShader::mfCompileParamMatrix(char* scr, SShader* ef, SParamComp_Matrix* pcm) {
    ...
    static tokenDesc commands[] = {{eIdentity, "Identity"},
...
bool CShader::mfCompileParam(char* scr, SShader* ef, TArray<SParam>* Params) {
    ...
    enum {
        ...
        eInvCameraMatrix,
        eCameraInvMatrix,
        eTranspCameraInvMatrix,
        eTranspInvCameraMatrix,
        eTranspCameraMatrix,
        eTexViewProjMatrix,
        eTranspTexViewProjMatrix,
        eWaterProjMatrix,
        eTranspWaterProjMatrix,
        eLightMatrix,
        eTranspLightMatrix,
        eFromRE0,
        eFromRE1,
    ...
    static tokenDesc commands[] = {{eReg, "Reg"},
                                   {eAll, "All"},
                                   {eComp, "Comp"},
    ...
    while ((cmd = shGetObject(&scr, commands, &name, &params)) > 0) {
        ...
        case eReg: {
            ...
            if (!name || !name[0])
                ...
                else if (!stricmp(name, "ConstColor1") || !stricmp(name, "GL_CONSTANT_COLOR1_NV"))
                    vpp.m_Reg = GL_CONSTANT_COLOR1_NV;
                else if (!stricmp(name, "GL_OFFSET_TEXTURE_2D_MATRIX_NV") || !stricmp(name, "OFFSET_TEXTURE_2D_MATRIX_NV"))
...
float SParamComp_LightIntens::mfGet() {
    ...
    if (gRenDev->m_RP.m_ObjFlags & FOB_TRANS_MASK) {
        float fLen = gRenDev->m_RP.m_pCurObject->m_Matrix(0, 0) * gRenDev->m_RP.m_pCurObject->m_Matrix(0, 0) +
                     gRenDev->m_RP.m_pCurObject->m_Matrix(0, 1) * gRenDev->m_RP.m_pCurObject->m_Matrix(0, 1) +
                     gRenDev->m_RP.m_pCurObject->m_Matrix(0, 2) * gRenDev->m_RP.m_pCurObject->m_Matrix(0, 2);
...
void SParamComp_LightsIntens::mfGet4f(vec4_t v) {
    ...
    if (rd->m_RP.m_ObjFlags & FOB_TRANS_MASK) {
        float fLen = rd->m_RP.m_pCurObject->m_Matrix(0, 0) * rd->m_RP.m_pCurObject->m_Matrix(0, 0) + rd->m_RP.m_pCurObject->m_Matrix(0, 1) * rd->m_RP.m_pCurObject->m_Matrix(0, 1) +
                     rd->m_RP.m_pCurObject->m_Matrix(0, 2) * rd->m_RP.m_pCurObject->m_Matrix(0, 2);
        unsigned int* n1 = (unsigned int*)&fLen;
        unsigned int n = 0x5f3759df - (*n1 >> 1);
        float* n2 = (float*)&n;
...
float SParamComp_LightsNum::mfGet() {
...
float SParamComp_LightBright::mfGet() {
    float fLuminance;
    if (gRenDev->m_RP.m_nCurLightParam >= 0)
        fLuminance = ((gRenDev->m_RP.m_pActiveDLights[gRenDev->m_RP.m_nCurLightParam]->m_Color.r * 0.3f) +
                      (gRenDev->m_RP.m_pActiveDLights[gRenDev->m_RP.m_nCurLightParam]->m_Color.g * 0.59f) +
                      (gRenDev->m_RP.m_pActiveDLights[gRenDev->m_RP.m_nCurLightParam]->m_Color.b * 0.11f));
    else
...
void SParamComp_LightMatrix::mfGet4f(vec4_t v) {
    ...
    if (dl && dl->m_pLightImage) {
        if (!i) {
            if (gRenDev->m_RP.m_FrameObject != sLastObjFrame || !IsEquivalent(dl->m_Origin, sLastLPos) || !IsEquivalent(dl->m_ProjAngles, sLastLAngles) ||
                dl->m_fLightFrustumAngle != sLastLFrust) {
...
void SParamComp_PlantsTMoving::mfGet4f(vec4_t v) {
    ...
    if (sPrevObjFrame != gRenDev->m_RP.m_FrameObject) {
        ...
        if (ax != m_LastAX || ay != m_LastAY) {
            m_LastAX = ax;
            m_LastAY = ay;
            SParamComp_PlantsTMoving::m_Matrix = GetTranslationMat(Vec3d(0.5f, 1.0f, 0)) * Matrix33::CreateRotationZ(ax / 180.0f * PI) *
                                                 Matrix33::CreateRotationY(-ay / 180.0f * PI); // IMPORTANT: radian-angle must be negated ;
...
void SParamComp_TexProjMatrix::mfGet4f(vec4_t v) {
    ...
    if (!tp || !tp->m_Matrix) {
        if (tp->m_Bind == TO_ENVIRONMENT_TEX) {
            ...
            Vec3d Angs = gRenDev->GetCamera().GetAngles();
            Vec3d Pos = gRenDev->GetCamera().GetPos();
            cm = gRenDev->m_cEF.mfFindSuitableEnvTex(Pos, Angs, true, 0, false, gRenDev->m_RP.m_pShader, gRenDev->m_RP.m_pShaderResources, gRenDev->m_RP.m_pCurObject, bReflect,
                                                     gRenDev->m_RP.m_pRE);
    ...
    if (gRenDev->m_RP.m_FrameObject != nLastObjFrame || pLastTex != tp || (!IsEquivalent(gRenDev->GetCamera().GetAngles(), pLastAngs, VEC_EPSILON)) ||
        (!IsEquivalent(gRenDev->GetCamera().GetPos(), pLastPos, VEC_EPSILON))) {
        pLastTex = tp;
...
float SParamComp_DofFocalParams::mfGet() {
    ...
    case 1:
        ...
        } else if (CRenderer::CV_r_doffocalsource == 1) {
            return (float)CRenderer::CV_r_doffocaldist_tag;
        } else if (CRenderer::CV_r_doffocalsource == 2) {
