...
void fxPreprocess(char** buf, const char* command) {
    if (!strcmp(&command[1], "include")) {
        ...
        *buf = tmpBuf;
    } else if (!strcmp(&command[1], "define")) {
        char name[256];
        TArray<char> macro;
        sFill(buf, name);
        while (**buf <= 0x20) {
    ...
    } else if (!strcmp(&command[1], "ifdef")) {
        char name[256];
        sFill(buf, name);
        int nID;
        ...
        if (sbIFDEF[nLevel] == true)
            fxIgnoreBlock(buf);
    } else if (!strcmp(&command[1], "endif")) {
        int nLevel = sbIFDEF.Num() - 1;
        if (nLevel < 0) {
...
void fxGetParams(char* annot, std::vector<SFXParam>& prms) {
    ...
    while (buf[0]) {
        ...
        else if (!strnicmp(buf, "float", sizeof("float") - 1))
            prm.m_Type = eType_FLOAT;
        else if (!strnicmp(buf, "int", sizeof("int") - 1))
            prm.m_Type = eType_INT;
        else if (!strnicmp(buf, "bool", sizeof("bool") - 1))
...
char* GetSubText(char** buf, char open, char close) {
    ...
    while ((theChar = **buf) != 0) {
        ...
        if (theChar == close) {
            if (--skip == 0) {
                **buf = 0; // nullptr terminate the result string
...
} sCols[] = {SColAsc("Aquamarine", Col_Aquamarine),
