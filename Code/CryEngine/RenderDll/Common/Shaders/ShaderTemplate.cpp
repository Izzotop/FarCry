...
SRenderShaderResources* CShader::mfCreateShaderResources(const SInputShaderResources* Res, bool bShare) {
    ...
    for (i = 0; i < EFTT_MAX; i++) {
        ...
        if (i == EFTT_DETAIL_OVERLAY && !RS.m_Textures[i].m_Name.empty()) {
            RS.m_Textures[i].m_TU.m_TexPic = mfLoadResourceTexture(RS.m_Textures[i].m_Name.c_str(), RS.m_TexturePath.c_str(), RS.m_Textures[i].m_TU.GetTexFlags(),
                                                                   RS.m_Textures[i].m_TU.GetTexFlags2(), eTT_Base, nullptr, &RS.m_Textures[i], RS.m_Textures[i].m_Amount);
            if (!RS.m_Textures[i].m_TU.m_TexPic->IsTextureLoaded()) {
                ...
                RS.m_Textures[i].m_TU.m_TexPic = mfLoadResourceTexture(CRenderer::CV_r_detaildefault->GetString(), RS.m_TexturePath.c_str(), RS.m_Textures[i].m_TU.GetTexFlags(),
                                                                       RS.m_Textures[i].m_TU.GetTexFlags2(), eTT_Base, nullptr, &RS.m_Textures[i], RS.m_Textures[i].m_Amount);
                if (!RS.m_Textures[i].m_TU.m_TexPic->IsTextureLoaded())
                    RS.m_Textures[i].m_TU.m_TexPic = mfLoadResourceTexture("Textures/detail/rock", RS.m_TexturePath.c_str(), RS.m_Textures[i].m_TU.GetTexFlags(),
                                                                           RS.m_Textures[i].m_TU.GetTexFlags2(), eTT_Base, nullptr, &RS.m_Textures[i], RS.m_Textures[i].m_Amount);
            }
        }
        ...
        if (i == EFTT_ATTENUATION2D) {
            ...
            if (!Tex->m_TU.m_TexPic) {
                if (!Tex->m_Name.empty())
                    Tex->m_TU.m_TexPic =
                        mfLoadResourceTexture(Tex->m_Name.c_str(), RS.m_TexturePath.c_str(), Tex->m_TU.GetTexFlags(), Tex->m_TU.GetTexFlags2(), eTT_Base, nullptr, Tex);
                if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded())
                    Tex->m_TU.m_TexPic =
                        mfLoadResourceTexture("Textures/Defaults/PointLight2D", RS.m_TexturePath.c_str(), Tex->m_TU.GetTexFlags(), Tex->m_TU.GetTexFlags2(), eTT_Base, nullptr, Tex);
            }
        }
            ...
            if (!Tex->m_TU.m_TexPic) {
                ...
                if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                    if (gRenDev->m_TexMan->m_Text_Atten1D)
                        ...
                        Tex->m_TU.m_TexPic = mfLoadResourceTexture("Textures/Defaults/PointLight1D", RS.m_TexturePath.c_str(), Tex->m_TU.GetTexFlags(), Tex->m_TU.GetTexFlags2(),
                                                                   eTT_Base, nullptr, Tex);
                }
            }
        }
    }
    ...
    for (i = 1; i < SShader::m_ShaderResources_known.Num(); i++) {
        ...
        if (!bShare || Res->m_ShaderParams.Num())
            continue;
        if (RS.m_ResFlags == pSR->m_ResFlags && RS.m_LMaterial == pSR->m_LMaterial && RS.m_Opacity == pSR->m_Opacity && RS.m_AlphaRef == pSR->m_AlphaRef &&
    ...
    if (!SShader::m_ShaderResources_known.Num()) {
        ...
        SRenderShaderResources* pSRnullptr = new SRenderShaderResources;
        pSRnullptr->m_nRefCounter = 1;
        SShader::m_ShaderResources_known[0] = pSRnullptr;
    } else if (SShader::m_ShaderResources_known.Num() >= MAX_SHADER_RES) {
...
SShaderItem CShader::mfShaderItemForName(const char* nameEf, EShClass Class, bool bShare, const char* tmplName, int flags, const SInputShaderResources* Res, uint64 nMaskGen) {
    ...
    while (true) {
        ...
        } else if ((SI.m_pShader->GetFlags3() & EF3_NOTEMPLATE) && templName)
            bTryNew = true;
        if (!bTryNew)
            break;
        n++;
...
STexPic* CShader::mfLoadResourceTexture(const char* nameTex, const char* path, int Flags, int Flags2, byte eTT, SShader* sh, SEfResTexture* Tex, float fAmount1, float fAmount2) {
    ...
    if ((!tx || !tx->IsTextureLoaded()) && nameTex && path) {
        if (strnicmp(nameTex, path, strlen(path))) {
            ...
            _splitpath(nameTex, nullptr, nullptr, name, ext);
            strcat(name, ext);
            ...
            if (Dst[i].m_TUnits[j].m_TexPic == &gRenDev->m_TexMan->m_Templates[EFTT_DIFFUSE]) {
                ...
                if (!Tex->m_TU.m_TexPic) {
                    ...
                    if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                        if (!stricmp(CRenderer::CV_r_glossdefault->GetString(), "$Diffuse"))
                            Tex->m_TU.m_TexPic =
                                mfLoadResourceTexture(Res->m_Textures[EFTT_DIFFUSE]->m_Name.c_str(), patch, Res->m_Textures[EFTT_DIFFUSE]->m_TU.GetTexFlags() | Flags,
                                                      Res->m_Textures[EFTT_DIFFUSE]->m_TU.GetTexFlags2(), eTT_Base, ef, Res->m_Textures[EFTT_GLOSS]);
                        else
                            Tex->m_TU.m_TexPic = mfLoadResourceTexture(CRenderer::CV_r_glossdefault->GetString(), patch, Tex->m_TU.GetTexFlags() | Flags, Tex->m_TU.GetTexFlags2(),
                                                                       eTT_Base, ef, Tex);
                    }
                }
                ...
                if (!Res->m_Textures[EFTT_DETAIL_OVERLAY])
                    ...
                    if (!Tex->m_TU.m_TexPic) {
                        ...
                        if (!Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                            Tex->m_TU.m_TexPic = mfLoadResourceTexture(CRenderer::CV_r_detaildefault->GetString(), patch, Tex->m_TU.GetTexFlags() | Flags, Tex->m_TU.GetTexFlags2(),
                                                                       eTT_Base, ef, Tex);
                            if (!Tex->m_TU.m_TexPic->IsTextureLoaded())
                ...
                if (!Res->m_Textures[EFTT_OPACITY])
                    ...
                    if (!Tex->m_TU.m_TexPic) {
                        Tex->m_TU.m_TexPic = mfLoadResourceTexture(Tex->m_Name.c_str(), patch, Tex->m_TU.GetTexFlags() | Flags, Tex->m_TU.GetTexFlags2(), eTT_Base, ef, Tex);
                        if (!Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                            Tex->m_TU.m_TexPic = mfLoadResourceTexture(CRenderer::CV_r_opacitydefault->GetString(), patch, Tex->m_TU.GetTexFlags() | Flags,
                                                                       Tex->m_TU.GetTexFlags2(), eTT_Base, ef, Tex);
                            if (!Tex->m_TU.m_TexPic->IsTextureLoaded())
                    ...
                    if (!Tex->m_TU.m_TexPic) {
                        ...
                        if (!Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                            if (Res->m_Textures[EFTT_DIFFUSE])
                                Tex->m_TU.m_TexPic =
                                    mfLoadResourceTexture(Res->m_Textures[EFTT_DIFFUSE]->m_Name.c_str(), patch, Res->m_Textures[EFTT_DIFFUSE]->m_TU.GetTexFlags() | Flags,
                                                          Res->m_Textures[EFTT_DIFFUSE]->m_TU.GetTexFlags2(), eTT_Base, ef, Tex);
                        }
                    }
                }
                ...
                if (Res->m_Textures[EFTT_BUMP] && Res->m_Textures[EFTT_NORMALMAP]) {
                    ...
                    Res->m_Textures[EFTT_BUMP]->m_TU.m_TexPic =
                        mfLoadResourceTexture(fullname, patch, Res->m_Textures[EFTT_BUMP]->m_TU.GetTexFlags() | Flags, Res->m_Textures[EFTT_BUMP]->m_TU.GetTexFlags2(), eTT_Bumpmap,
                                              ef, Res->m_Textures[EFTT_BUMP], (float)Res->m_Textures[EFTT_BUMP]->m_Amount, (float)Res->m_Textures[EFTT_NORMALMAP]->m_Amount);
                }
                ...
                if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                    Tex = Res->m_Textures[EFTT_BUMP];
                    if (!Tex->m_Name.empty())
                        Tex->m_TU.m_TexPic = mfLoadResourceTexture(Tex->m_Name.c_str(), patch, Tex->m_TU.GetTexFlags() | Flags, Tex->m_TU.GetTexFlags2(), eTT_Bumpmap, ef, Tex,
                                                                   (float)Tex->m_Amount);
                    if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                        if (Res->m_Textures[EFTT_NORMALMAP])
                            Tex->m_TU.m_TexPic =
                                mfLoadResourceTexture(Res->m_Textures[EFTT_NORMALMAP]->m_Name.c_str(), patch, Res->m_Textures[EFTT_NORMALMAP]->m_TU.GetTexFlags() | Flags,
                                                      Res->m_Textures[EFTT_NORMALMAP]->m_TU.GetTexFlags2(), eTT_Bumpmap, ef, Res->m_Textures[EFTT_BUMP],
                                                      (float)Res->m_Textures[EFTT_NORMALMAP]->m_Amount);
                    }
                    if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                ...
                if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                    if (!Tex->m_Name.empty())
                        Tex->m_TU.m_TexPic =
                            mfLoadResourceTexture(Tex->m_Name.c_str(), patch, Tex->m_TU.GetTexFlags() | Flags, Tex->m_TU.GetTexFlags2(), eTT_Base, ef, Tex, (float)Tex->m_Amount);
                    else
                        Tex->m_TU.m_TexPic =
                            mfLoadResourceTexture("Textures/white", patch, Tex->m_TU.GetTexFlags() | Flags, Tex->m_TU.GetTexFlags2(), eTT_Base, ef, Tex, (float)Tex->m_Amount);
                }
                ...
                if (!Tex->m_TU.m_TexPic) {
                    ...
                    if (!Tex->m_TU.m_TexPic->IsTextureLoaded())
                        Tex->m_TU.m_TexPic =
                            mfLoadResourceTexture("Textures/Defaults/PointLight2D", patch, Tex->m_TU.GetTexFlags() | Flags, Tex->m_TU.GetTexFlags2(), eTT_Base, ef, Tex);
                }
                ...
                if (!Tex->m_TU.m_TexPic) {
                    ...
                    if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                        if (gRenDev->m_TexMan->m_Text_Atten1D)
                            ...
                            Tex->m_TU.m_TexPic =
                                mfLoadResourceTexture("Textures/Defaults/PointLight1D", patch, Tex->m_TU.GetTexFlags() | Flags, Tex->m_TU.GetTexFlags2(), eTT_Base, ef, Tex);
                    }
                }
                ...
                if (!Tex->m_TU.m_TexPic) {
                    ...
                    if (!Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                        Tex->m_TU.m_TexPic =
                            mfLoadResourceTexture("Textures/Defaults/DefaultCM", patch, Tex->m_TU.GetTexFlags() | Flags, Tex->m_TU.GetTexFlags2(), eTT_Cubemap, ef, Tex);
                    }
                }
            }
        }
    ...
    for (int i = 0; i < Dst.Num(); i++) {
        for (int j = 0; j < Dst[i].m_TUnits.Num(); j++) {
            ...
            if (Dst[i].m_TUnits[j].m_TexPic == &gRenDev->m_TexMan->m_Templates[EFTT_DIFFUSE]) {
                ...
                if (!Tex->m_TU.m_TexPic) {
                    Tex->m_TU.m_TexPic =
                        mfLoadResourceTexture(Tex->m_Name.c_str(), patch, Tex->m_TU.GetTexFlags() | Flags, Tex->m_TU.GetTexFlags2(), Tex->m_TU.m_eTexType, ef, Tex);
                    if (Tex->m_TU.m_TexPic && Tex->m_TU.m_TexPic->IsTextureLoaded())
                        Tex->m_TU.m_TexPic->m_Flags2 |= FT2_DIFFUSETEXTURE;
                } else
                    mfCheckAnimatedSequence(&Res->m_Textures[EFTT_DIFFUSE]->m_TU, Res->m_Textures[EFTT_DIFFUSE]->m_TU.m_TexPic);
...
#endif
                if (Res->m_Textures[EFTT_BUMP] && !Res->m_Textures[EFTT_BUMP]->m_Name.empty() && Res->m_Textures[EFTT_NORMALMAP] &&
                    !Res->m_Textures[EFTT_NORMALMAP]->m_Name.empty()) {
                    char fullname[512];
                    sprintf(fullname, "%s+norm_%s", Res->m_Textures[EFTT_BUMP]->m_Name.c_str(), Res->m_Textures[EFTT_NORMALMAP]->m_Name.c_str());
                    Res->m_Textures[EFTT_BUMP]->m_TU.m_TexPic =
                        mfLoadResourceTexture(fullname, patch, Res->m_Textures[EFTT_BUMP]->m_TU.GetTexFlags() | Flags, Res->m_Textures[EFTT_BUMP]->m_TU.GetTexFlags2(), eTT_Bumpmap,
                ...
                if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                    ...
                    if (!Tex->m_Name.empty())
                        Tex->m_TU.m_TexPic =
                            mfLoadResourceTexture(Tex->m_Name.c_str(), patch, Tex->m_TU.GetTexFlags() | Flags, Tex->m_TU.GetTexFlags2(), eTT, ef, Tex, (float)Tex->m_Amount);
                    if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                        if (Res->m_Textures[EFTT_NORMALMAP]) {
                            Tex->m_TU.m_TexPic = mfLoadResourceTexture(
                                Res->m_Textures[EFTT_NORMALMAP]->m_Name.c_str(), patch, Res->m_Textures[EFTT_NORMALMAP]->m_TU.GetTexFlags() | Flags,
                                Res->m_Textures[EFTT_NORMALMAP]->m_TU.GetTexFlags2(), eTT, ef, Res->m_Textures[EFTT_BUMP], (float)Res->m_Textures[EFTT_NORMALMAP]->m_Amount);
                ...
                if (nameBump[0] && nameNorm[0]) {
                    ...
                    Res->m_Textures[EFTT_BUMP_DIFFUSE]->m_TU.m_TexPic = mfLoadResourceTexture(
                        fullname, patch, Res->m_Textures[EFTT_BUMP]->m_TU.GetTexFlags() | Flags, Res->m_Textures[EFTT_BUMP]->m_TU.GetTexFlags2(), eTT_Bumpmap, ef,
                        Res->m_Textures[EFTT_BUMP_DIFFUSE], (float)Res->m_Textures[EFTT_BUMP]->m_Amount, (float)Res->m_Textures[EFTT_NORMALMAP]->m_Amount);
                }
                Tex = Res->m_Textures[EFTT_BUMP_DIFFUSE];
                if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                    ETexType eTT = Dst[i].m_TUnits[j].m_eTexType == eTT_DSDTBump ? eTT_DSDTBump : eTT_Bumpmap;
                    if (nameBump[0])
                        Tex->m_TU.m_TexPic = mfLoadResourceTexture(nameBump, patch, Res->m_Textures[EFTT_BUMP]->m_TU.GetTexFlags() | Flags,
                                                                   Res->m_Textures[EFTT_BUMP]->m_TU.GetTexFlags2(), eTT, ef, Tex, (float)Res->m_Textures[EFTT_BUMP]->m_Amount);
                    if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                        if (nameNorm[0])
                            Tex->m_TU.m_TexPic =
                                mfLoadResourceTexture(nameNorm, patch, Res->m_Textures[EFTT_NORMALMAP]->m_TU.GetTexFlags() | Flags,
                                                      Res->m_Textures[EFTT_NORMALMAP]->m_TU.GetTexFlags2(), eTT, ef, Tex, (float)Res->m_Textures[EFTT_NORMALMAP]->m_Amount);
                    }
                    if (!Tex->m_TU.m_TexPic || !Tex->m_TU.m_TexPic->IsTextureLoaded()) {
                        Tex->m_TU.m_nFlags |= FTU_NOBUMP;
                        Tex->m_TU.m_TexPic = mfLoadResourceTexture("Textures/white_ddn", patch, Flags, 0, eTT_Bumpmap, ef, Tex);
...
void CShader::mfRefreshResources(SShader* eft, SRenderShaderResources* Res) {
    ...
    if (Res && Res->m_Textures[EFTT_BUMP] && Res->m_Textures[EFTT_BUMP]->m_TU.m_TexPic && (Res->m_Textures[EFTT_BUMP]->m_TU.m_TexPic->m_Flags & FT_3DC) &&
        (Res->m_Textures[EFTT_BUMP]->m_TU.m_TexPic->m_Flags & FT_HASALPHA) && (eft->m_Flags & EF_OFFSETBUMP)) {
        const char* patch = Res->m_TexturePath.c_str();
        if (!Res->m_Textures[EFTT_BUMP_HEIGHT])
            Res->AddTextureMap(EFTT_BUMP_HEIGHT);
        Res->m_Textures[EFTT_BUMP_HEIGHT]->m_TU.m_TexPic = mfLoadResourceTexture(
            Res->m_Textures[EFTT_BUMP]->m_Name.c_str(), patch, Res->m_Textures[EFTT_BUMP]->m_TU.GetTexFlags() | FT_3DC | FT_3DC_A, Res->m_Textures[EFTT_BUMP]->m_TU.GetTexFlags2(),
            eTT_Bumpmap, eft, Res->m_Textures[EFTT_BUMP_HEIGHT], (float)Res->m_Textures[EFTT_BUMP]->m_Amount);
    }
}
...
int CShader::mfTemplateNameToId(char* name) {
    ...
    if (!stricmp("Default", name))
        n = EFT_DEFAULT;
    else if (!stricmp("Decal", name))
        n = EFT_DECAL;
    else if (!stricmp("InvLight", name))
        n = EFT_INVLIGHT;
    else if (!stricmp("White", name))
        n = EFT_WHITE;
    else if (!stricmp("WhiteShadow", name))
        n = EFT_WHITESHADOW;
    else if (!stricmp("HeatVision", name))
        n = EFT_HEATVISION;
    else if (!strnicmp("User", name, 4))
...
void CShader::mfCompileTemplate(SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{eTexAttenuation2D, "TexAttenuation2D"},
