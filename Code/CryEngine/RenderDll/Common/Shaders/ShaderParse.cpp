...
void CShader::mfCompileFogParms(SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{eColor, "Color"}, {eDensity, "Density"}, {eAxis, "Axis"}, {0, 0}};
...
void CShader::mfCompileRGBNoise(SRGBGenNoise* cn, char* scr, SShader* ef) {
    ...
    static tokenDesc commands[] = {
        {eRRange, "RRange"}, {eRConst, "RConst"}, {eGRange, "GRange"}, {eGConst, "GConst"}, {eBRange, "BRange"}, {eBConst, "BConst"},
    };
...
void CShader::mfCompileWaveForm(SWaveForm* wf, char* scr) {
    ...
    static tokenDesc commands[] = {{eType, "Type"}, {eLevel, "Level"}, {eAmp, "Amp"}, {ePhase, "Phase"}, {eFreq, "Freq"}, {eClamp, "Clamp"}, {0, 0}};
    ...
    while ((cmd = shGetObject(&scr, commands, &name, &params)) > 0) {
        ...
        case eType:
            ...
            else if (!stricmp(params, "HalfSin"))
                wf->m_eWFType = eWF_HalfSin;
            else if (!stricmp(params, "InvHalfSin"))
                wf->m_eWFType = eWF_InvHalfSin;
            else if (!stricmp(params, "Square"))
                wf->m_eWFType = eWF_Square;
            else if (!stricmp(params, "Triangle"))
                wf->m_eWFType = eWF_Triangle;
            else if (!stricmp(params, "SawTooth"))
                wf->m_eWFType = eWF_SawTooth;
            else if (!stricmp(params, "InverseSawTooth") || !stricmp(params, "InvSawTooth"))
                wf->m_eWFType = eWF_InvSawTooth;
            else if (!stricmp(params, "Hill"))
                wf->m_eWFType = eWF_Hill;
            else if (!stricmp(params, "InverseHill") || !stricmp(params, "InvHill"))
...
void CShader::mfCompileDeform(SShader* ef, SDeform* df, char* dname, char* scr) {
    ...
    static tokenDesc commands[] = {{eDiv, "Div"}, {eDeformGen, "DeformGen"}, {eFlareSize, "FlareSize"}};

    EDeformType eType;
    if (!stricmp(dname, "Bulge"))
        eType = eDT_Bulge;
    else if (!stricmp(dname, "Wave"))
        eType = eDT_Wave;
    else if (!stricmp(dname, "Flare")) {
        ef->m_Flags3 |= EF3_SHAREVERTS;
        eType = eDT_Flare;
    } else if (!stricmp(dname, "Beam"))
        eType = eDT_Beam;
    else if (!stricmp(dname, "VerticalWave"))
        eType = eDT_VerticalWave;
    else if (!stricmp(dname, "Squeeze"))
        eType = eDT_Squeeze;
    else if (!stricmp(dname, "FromCenter"))
...
SShaderGenBit* CShader::mfCompileShaderGenProperty(SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{eName, "Name"}, {eProperty, "Property"}, {eDescription, "Description"}, {eMask, "Mask"}, {eHidden, "Hidden"},
...
bool CShader::mfCompileShaderGen(SShader* ef, SShaderGen* shg, char* scr) {
    ...
    static tokenDesc commands[] = {{eProperty, "Property"},
                                   {eVersion, "Version"},
...
bool CShader::mfCompileParams(SShader* ef, char* scr) {
    ...
    enum {
        ...
        eMotionBlurMap,
        eScreenProcess,
        eScreenLuminosityMap,
        eFlashBangMap,
        eScreenTexture,
        eIgnoreResourceState,
        eUseLightMaterial,
        eSkyLayerHeight,
        eDofMap,
        eDefaultVertexFormat,
        eOffsetBump,
        eAllow3DC
    };
    static tokenDesc commands[] = {{eNoDraw, "NoDraw"},
                                   {eNoLight, "NoLight"},
    ...
    while ((cmd = shGetObject(&scr, commands, &name, &params)) > 0) {
        ...
        case eClipPlane:
            ...
            else if (!stricmp(data, "Front"))
                ef->m_Flags3 |= EF3_CLIPPLANE_FRONT;
            else if (!stricmp(data, "WaterFront"))
                ef->m_Flags3 |= EF3_CLIPPLANE_WATER_FRONT;
            else if (!stricmp(data, "WaterBack"))
...
bool CShader::mfCompilePublic(SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{eByte, "Byte"}, {eShort, "Short"}, {eInt, "Int"}, {eFloat, "Float"}, {eString, "String"}, {eColor, "Color"}, {eVector, "Vector"},
...
bool CShader::mfCompileRenderParams(SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{eDrawWater, "DrawWater"},
...
void sGetBlend(char* data, SShader* ef, uint* src, uint* dst) {
    ...
    if (!stricmp(sbfsrc, "ONE"))
        bfsrc = GS_BLSRC_ONE;
    else if (!stricmp(sbfsrc, "ZERO"))
        bfsrc = GS_BLSRC_ZERO;
    else if (!stricmp(sbfsrc, "DST_COLOR"))
        bfsrc = GS_BLSRC_DSTCOL;
    else if (!stricmp(sbfsrc, "ONE_MINUS_DST_COLOR"))
        bfsrc = GS_BLSRC_ONEMINUSDSTCOL;
    else if (!stricmp(sbfsrc, "SRC_ALPHA") || !stricmp(sbfsrc, "SrcAlpha"))
        bfsrc = GS_BLSRC_SRCALPHA;
    else if (!stricmp(sbfsrc, "ONE_MINUS_SRC_ALPHA") || !stricmp(sbfsrc, "InvSrcAlpha"))
        bfsrc = GS_BLSRC_ONEMINUSSRCALPHA;
    else if (!stricmp(sbfsrc, "DST_ALPHA"))
        bfsrc = GS_BLSRC_DSTALPHA;
    else if (!stricmp(sbfsrc, "ONE_MINUS_DST_ALPHA"))
        bfsrc = GS_BLSRC_ONEMINUSDSTALPHA;
    else if (!stricmp(sbfsrc, "SRC_ALPHA_SATURATE"))
    ...
    if (!stricmp(sbfdst, "ONE"))
        bfdst = GS_BLDST_ONE;
    else if (!stricmp(sbfdst, "ZERO"))
        bfdst = GS_BLDST_ZERO;
    else if (!stricmp(sbfdst, "SRC_ALPHA") || !stricmp(sbfsrc, "SrcAlpha"))
        bfdst = GS_BLDST_SRCALPHA;
    else if (!stricmp(sbfdst, "ONE_MINUS_SRC_ALPHA") || !stricmp(sbfsrc, "InvSrcAlpha"))
        bfdst = GS_BLDST_ONEMINUSSRCALPHA;
    else if (!stricmp(sbfdst, "DST_ALPHA"))
        bfdst = GS_BLDST_DSTALPHA;
    else if (!stricmp(sbfdst, "ONE_MINUS_DST_ALPHA"))
        bfdst = GS_BLDST_ONEMINUSDSTALPHA;
    else if (!stricmp(sbfdst, "SRC_COLOR"))
        bfdst = GS_BLDST_SRCCOL;
    else if (!stricmp(sbfdst, "ONE_MINUS_SRC_COLOR"))
...
uint CShader::mfCompileRendState(SShader* ef, SShaderPass* sm, char* scr) {
    ...
    static tokenDesc commands[] = {{eBlend, "Blend"},
...
byte sGetColorOp(char* data, SShader* ef) {
    if (!stricmp(data, "NoSet"))
        return eCO_NOSET;
    else if (!stricmp(data, "Disable") || !stricmp(data, "None"))
        return eCO_DISABLE;
    else if (!stricmp(data, "Replace") || !stricmp(data, "SelectArg1"))
        return eCO_REPLACE;
    else if (!stricmp(data, "SelectArg2"))
        return eCO_ARG2;
    else if (!stricmp(data, "Decal"))
        return eCO_DECAL;
    else if (!stricmp(data, "BlendDiffuseAlpha"))
        return eCO_BLENDDIFFUSEALPHA;
    else if (!stricmp(data, "Modulate"))
        return eCO_MODULATE;
    else if (!stricmp(data, "Modulate4X"))
        return eCO_MODULATE4X;
    else if (!stricmp(data, "Modulate2X"))
        return eCO_MODULATE2X;
    else if (!stricmp(data, "Add"))
        return eCO_ADD;
    else if (!stricmp(data, "MultiplyAdd"))
        return eCO_MULTIPLYADD;
    else if (!stricmp(data, "AddSigned") || !stricmp(data, "Add_Signed"))
        return eCO_ADDSIGNED;
    else if (!stricmp(data, "AddSigned2X"))
        return eCO_ADDSIGNED2X;
    else if (!stricmp(data, "BumpEnvMap"))
        return eCO_BUMPENVMAP;
    else if (!stricmp(data, "MODULATEALPHA_ADDCOLOR"))
        return eCO_MODULATEALPHA_ADDCOLOR;
    else if (!stricmp(data, "MODULATECOLOR_ADDALPHA"))
        return eCO_MODULATECOLOR_ADDALPHA;
    else if (!stricmp(data, "MODULATEINVALPHA_ADDCOLOR"))
        return eCO_MODULATEINVALPHA_ADDCOLOR;
    else if (!stricmp(data, "MODULATEINVCOLOR_ADDALPHA"))
        return eCO_MODULATEINVCOLOR_ADDALPHA;
    else if (!stricmp(data, "DOTPRODUCT3"))
        return eCO_DOTPRODUCT3;
    else if (!stricmp(data, "LERP"))
        return eCO_LERP;
    else if (!stricmp(data, "SUBTRACT"))
        return eCO_SUBTRACT;
    else {
...
byte sGetColorArg(char* data, SShader* ef) {
    if (!stricmp(data, "Texture"))
        return eCA_Texture;
    else if (!stricmp(data, "Diffuse"))
        return eCA_Diffuse;
    else if (!stricmp(data, "Specular"))
        return eCA_Specular;
    else if (!stricmp(data, "Previous") || !stricmp(data, "Current"))
        return eCA_Previous;
    else if (!stricmp(data, "Constant") || !stricmp(data, "TFactor"))
        return eCA_Constant;
    else {
...
bool CShader::mfCompileLayer(SShader* ef, int nl, char* scr, SShaderPass* sm) {
    ...
    enum {
        ...
        eBumpCompressed,
        eBumpInverted,
        eSecondPassRendState,
        eRendState,
        eTexFilter,
        eTexNoScale,
        eHDRTexColorOp
    };
    static tokenDesc commands[] = {{eMap, "Map"},
                                   {eTexType, "TexType"},
                                   {eBlend, "Blend"},
    ...
    else if (Flags2 & FT2_FILTER_TRILINEAR)
        ml->m_nFlags |= FTU_FILTERTRILINEAR;
    else if (Flags2 & FT2_FILTER_NEAREST)
...
void CShader::mfCompileOrient(SShader* ef, int num, char* scr) {
    ...
    static tokenDesc commands[] = {{eX, "X"}, {eY, "Y"}, {eOrigin, "Origin"}, {0, 0}};
...
void CShader::mfCompileLightMove(SShader* ef, char* nameMove, SLightEval* le, char* scr) {
    ...
    static tokenDesc commands[] = {{eDirection, "Direction"}, {eWave, "Wave"}, {eSpeed, "Speed"}, {0, 0}};
    ...
    else if (!stricmp(nameMove, "Patch"))
...
void CShader::mfCompileEvalLight(SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{eStyle, "Style"},         {eUpdateStyle, "UpdateStyle"}, {eProjRotate, "ProjRotate"},   {eLightRotate, "LightRotate"},
                                   {eLightMove, "LightMove"}, {eLightPos, "LightPos"},       {eLightOffset, "LightOffset"}, {0, 0}};
    ...
    while ((cmd = shGetObject(&scr, commands, &name, &params)) > 0) {
        ...
        case eUpdateStyle:
            ...
            else if (!stricmp(data, "Color") || !stricmp(data, "RGB"))
...
void CShader::mfCompileStencil(SShader* ef, char* scr) {
    ...
    enum { eFunc = 1, eOp };
    static tokenDesc commands[] = {{eFunc, "Func"}, {eOp, "Op"}, {0, 0}};
    ...
    while ((cmd = shGetObject(&scr, commands, &name, &params)) > 0) {
        ...
        case eFunc: {
            ...
            else if (!stricmp(func[0], "NEVER"))
                sm->m_State |= FSS_STENCFUNC_NEVER;
            else if (!stricmp(func[0], "LESS"))
                sm->m_State |= FSS_STENCFUNC_LESS;
            else if (!stricmp(func[0], "LEQUAL"))
                sm->m_State |= FSS_STENCFUNC_LEQUAL;
            else if (!stricmp(func[0], "GREATER"))
                sm->m_State |= FSS_STENCFUNC_GREATER;
            else if (!stricmp(func[0], "GEQUAL"))
                sm->m_State |= FSS_STENCFUNC_GEQUAL;
            else if (!stricmp(func[0], "EQUAL"))
                sm->m_State |= FSS_STENCFUNC_EQUAL;
            else if (!stricmp(func[0], "NOTEQUAL"))
...
void CShader::mfCompileState(SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{eStencil, "Stencil"},
...
SShader* CShader::mfCompile(SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{ePublic, "Public"},
                                   {eParams, "Params"},
...
SShader* CShader::mfCompileShader(SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{eShader, "Shader"},
                                   {eVersion, "Version"},
...
bool CShader::mfCompileSequence(SShader* ef, SShaderTexUnit* tl, int nLayer, char* scr, ETexType eTT, int Flags, int Flags2, float fBumpAmount) {
    ...
    static tokenDesc commands[] = {{eMaps, "Maps"}, {eTime, "Time"}, {eRandom, "Random"}, {eLoop, "Loop"}, {0, 0}};
    ...
    else if (strchr(mapname, '#') || strchr(mapname, '$'))
...
int CShader::mfReadAllImgFiles(SShader* ef, SShaderTexUnit* tl, STexAnim* ta, char* name) {
    ...
    if (path) {
        if (!stricmp(path, "ModelsPath"))
            pt = m_ModelsPath;
        else if (!stricmp(path, "TexturesPath"))
            pt = m_TexturesPath;
        else if (!stricmp(path, "SystemPath"))
            pt = m_SystemPath;
...
bool CShader::mfCompileFlare(SShader* ef, SSunFlare* fl, char* scr) {
    ...
    static tokenDesc commands[] = {{eName, "Name"},
                                   {eLocation, "Location"},
                                   {eScale, "Scale"},
                                   {eColor, "Color"},
...
bool CShader::mfCompileSunFlares(SShader* ef, char* namefl, char* scr) {
    ...
    enum { eFlare = 1 };
    static tokenDesc commands[] = {{eFlare, "Flare"},
