...
enum EShadowType { EST_DEPTH_BUFFER, EST_2D_BUFFER, EST_PENUMBRA };
...
struct ShadowMapFrustum {
    ...
    void UnProject(float sx, float sy, float sz, float* px, float* py, float* pz, IRenderer* pRend) {
        ...
        pRend->UnProject(sx, sy, sz, px, py, pz, debugLightViewMatrix, debugLightFrustumMatrix, shadowViewport);
    }
    ...
    void DrawFrustum(IRenderer* pRend, Vec3d vPos, float fScale) {
        ...
        for (int i = 0; i <= 1; i++) {
            pRend->Draw3dBBox(vPos + fScale * UnProjectVertex3d(0, 0, i, vert1, pRend), vPos + fScale * UnProjectVertex3d(1, 0, i, vert2, pRend), true);
