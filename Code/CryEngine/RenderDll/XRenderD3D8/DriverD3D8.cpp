...
CD3D8Renderer::CD3D8Renderer() {
    ...
    CV_d3d8_texturefilter = iConsole->CreateVariable("d3d8_TextureFilter", "TRILINEAR", nullptr);
    iConsole->Register("d3d8_SquareTextures", &CV_d3d8_squaretextures, 0);
    CV_d3d8_device = iConsole->CreateVariable("d3d8_Device", "Auto", nullptr);
...
void CD3D8Renderer::DestroyWindow() {
...
void CD3D8Renderer::RestoreGamma() {
    if (!m_pd3dDevice)
        return;
...
bool CD3D8Renderer::SetWindow(int width, int height, bool fullscreen, WIN_HWND hWnd) {
    ...
    if (!hWnd) {
        m_hWnd = CreateWindowEx(exstyle, "CryENGINE", m_WinTitle, style, x, y, wdt, hgt, nullptr, nullptr, m_hInst, nullptr);
    } else
        m_hWnd = (HWND)hWnd;
    m_width = width;
    m_height = height;
    ...
    if (fullscreen) {
        ...
        SetWindowPos(m_hWnd, HWND_NOTOPMOST, (GetSystemMetrics(SM_CXFULLSCREEN) - width) / 2, (GetSystemMetrics(SM_CYFULLSCREEN) - height) / 2,
                     GetSystemMetrics(SM_CXDLGFRAME) * 2 + width, GetSystemMetrics(SM_CYCAPTION) + GetSystemMetrics(SM_CXDLGFRAME) * 2 + height, SWP_SHOWWINDOW);
...
#ifndef PS2
WIN_HWND CD3D8Renderer::Init(int x, int y, int width, int height, unsigned int cbpp, int zbpp, int sbits, bool fullscreen, WIN_HINSTANCE hinst, WIN_HWND Glhwnd, WIN_HDC Glhdc,
    ...
    else if (GetFeatures() & RFT_HW_GF3)
        iLog->Log(" Use Hardware Shaders for NV2x GPU\n");
    else if (GetFeatures() & RFT_HW_RADEON8500)
...
HRESULT CD3D8Renderer::BuildDevicesList() {
    ...
    for (UINT iAdapter = 0; iAdapter < m_pD3D->GetAdapterCount(); iAdapter++) {
        ...
        for (UINT iDevice = 0; iDevice < dwNumDeviceTypes; iDevice++) {
            ...
            } else if (bFormatConfirmed[0] && !(pDevice->d3dCaps.Caps2 & D3DCAPS2_CANRENDERWINDOWED))
...
HRESULT CD3D8Renderer::InitDeviceObjects() {
    ...
    iLog->Log("D3D Adapter: Driver version: %d.%02d.%02d.%04d\n", HIWORD(ai->DriverVersion.u.HighPart), LOWORD(ai->DriverVersion.u.HighPart), HIWORD(ai->DriverVersion.u.LowPart),
              LOWORD(ai->DriverVersion.u.LowPart));
    ...
    guardnw(InitTextureFormats);
    {
        ...
        guardnw(FindTextureFormats);
        {
            ...
            if (!mFirstPixelFormat) {
                ...
                else if (SUCCEEDED(m_pD3D->CheckDeviceFormat(m_dwAdapter, m_d3dCaps.DeviceType, m_d3dpp.BackBufferFormat, 0, D3DRTYPE_TEXTURE, D3DFMT_X1R5G5B5)))
    ...
    iLog->Log("D3D Driver: Textures (%ix%i)-(%ix%i), Max aspect %i\n", mMinTextureWidth, mMinTextureHeight, m_d3dCaps.MaxTextureWidth, m_d3dCaps.MaxTextureHeight,
              m_d3dCaps.MaxTextureAspectRatio);
    ...
    if (CV_d3d8_usebumpmap && (GetFeatures() & RFT_MULTITEXTURE)) {
        ...
        else if (type == 1 && (m_d3dCaps.TextureOpCaps & D3DTEXOPCAPS_DOTPRODUCT3) && (m_d3dCaps.TextureCaps & D3DPTEXTURECAPS_CUBEMAP) && mFormat8888.BitsPerPixel)
...
HRESULT CD3D8Renderer::Initialize3DEnvironment() {
    ...
    // Create the device
    hr = m_pD3D->CreateDevice(m_dwAdapter, pDeviceInfo->DeviceType, (HWND)m_CurrContext->m_hWnd, pModeInfo->dwBehavior, &m_d3dpp, &m_pd3dDevice);
    if (SUCCEEDED(hr)) {
        ...
        if (m_d3dCaps.MaxSimultaneousTextures == 4 && D3DSHADER_VERSION_MAJOR(m_d3dCaps.PixelShaderVersion) == 1 && D3DSHADER_VERSION_MINOR(m_d3dCaps.PixelShaderVersion) == 1 &&
            m_d3dCaps.MaxTextureBlendStages == 8)
            m_Features |= RFT_HW_GF3 | RFT_HW_TS | RFT_HW_VS;
                 ...
                 D3DSHADER_VERSION_MINOR(m_d3dCaps.PixelShaderVersion) >= 1 && m_d3dCaps.MaxTextureBlendStages >= 8)
            m_Features |= RFT_HW_GF3 | RFT_HW_TS | RFT_HW_VS;
        else if (m_d3dCaps.MaxSimultaneousTextures > 4 && D3DSHADER_VERSION_MAJOR(m_d3dCaps.PixelShaderVersion) == 1 &&
                 D3DSHADER_VERSION_MINOR(m_d3dCaps.PixelShaderVersion) >= 3 && m_d3dCaps.MaxTextureBlendStages >= 8)
...
bool CD3D8Renderer::SetRes() {
    ...
    if (!strnicmp(drv, "auto", 4))
        bAuto = true;
    else if (!strnicmp(drv, "primary", 7))
        numAdap = 0;
    else if (!strnicmp(drv, "3dfx", 4) || !strnicmp(drv, "second", 6) || !strnicmp(drv, "Voodoo", 6))
        numAdap = 1;
    else if (drv[0] && isdigit(drv[0]))
        numAdap = atol(drv);
    else {
...
void CD3D8Renderer::Update() {
    ...
    for (; itBBox != itBBoxEnd; ++itBBox) {
        ...
        SetMaterialColor(rBBox.fColor[0], rBBox.fColor[1], rBBox.fColor[2], rBBox.fColor[3]);
        ...
        case DPRIM_LINE:
            ...
            DWORD dwCol = (DWORD(rBBox.fColor[0] * 255) << 24) | (DWORD(rBBox.fColor[1] * 255) << 16) | (DWORD(rBBox.fColor[2] * 255) << 8) | (DWORD(rBBox.fColor[3] * 255));
            if (hr == S_OK) {
...
void CD3D8Renderer::Draw2dImage(float xpos, float ypos, float w, float h, int textureid, float s0, float t0, float s1, float t1, float angle, float r, float g, float b, float a,
                                float z) {
    //////////////////////////////////////////////////////////////////////
    ...
    xpos = (float)(int)ScaleCoordX(xpos);
    w = (float)(int)ScaleCoordX(w);
    ypos = (float)(int)ScaleCoordY(ypos);
    h = (float)(int)ScaleCoordY(h);
...
void CD3D8Renderer::Draw3dBBox(const Vec3d &mins,const Vec3d &maxs, int nPrimType)
{
  ...
  ID3DXMesh *pID3DBoxMesh = nullptr;

  ASSERT(m_pd3dDevice);

  // Create the box
  D3DXCreateBox(m_pd3dDevice, 10, 10, 10, &pID3DBoxMesh, nullptr);
...
void CD3D8Renderer::EnableTMU(bool enable) {
    ...
    if (!enable) {
        ...
        mStages[m_TexMan->m_CurStage].Texture = nullptr;
    }
}
...
void CD3D8Renderer::CreateIndexBuffer(SVertexStream* dest, const void* src, int indexcount) {
    ...
    dest->m_nItems = 0;
    if (indexcount) {
        IDirect3DIndexBuffer8* ibuf = nullptr;
...
void CD3D8Renderer::CreateBuffer(int size, int vertexformat, CVertexBuffer* buf, int Type, const char* szSource) {
    IDirect3DVertexBuffer8* vptr = nullptr;
...
void CD3D8Renderer::UpdateBuffer(CVertexBuffer* dest, const void* src, int vertexcount, bool bUnLock, int offs, int Type) {
    ...
    } else if (dest->m_VS[Type].m_bLocked && bUnLock) {
        tvert->Unlock();
        dest->m_VS[Type].m_bLocked = false;
    }
}
...
void CD3D8Renderer::ReleaseBuffer(CVertexBuffer* bufptr) {
    if (bufptr) {
        ...
        if (bufptr->m_VS[VSF_GENERAL].m_VertBuf.m_pPtr) {
            ...
            SAFE_RELEASE(vtemp);
            bufptr->m_VS[VSF_LMTC].m_VertBuf.m_pPtr = nullptr;
        }
...
void CD3D8Renderer::DrawObjSprites(list2<CStatObjInst*>* pList, float fMaxViewDist, CObjManager* pObjMan) {
    ...
    // struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F *vQuad = nullptr;
    ...
    for (int i = pList->Count() - 1; i >= 0; i--) {
        ...
        if (o->GetEntityRS()->nFogVolumeID > 0 && CV_r_VolumetricFog) {
            ...
            if (lastvpD3D != pVP_FV) {
                ...
                if (m_RP.m_RCSprites_FV) {
                    ...
                    param[0] = m_WorldColor[0];
                    param[1] = m_WorldColor[1];
                    param[2] = m_WorldColor[2];
                    param[3] = m_WorldColor[3];
                    CPShader_D3D::mfSetFloat4f(0, param);
                    param[0] = FogColor[0];
                    param[1] = FogColor[1];
                    param[2] = FogColor[2];
        ...
        *(DWORD*)(&vQuad[5].r) = cCol;
        ...
        /*if (nLast + 6 >= m_RP.MaxVerts)
        {
          ...
          pVB->Lock(nFirst*sizeof(struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F), 6*sizeof(struct_VERTEX_FORMAT_P3F_COL4UB_TEX2F), (BYTE **)vQuad, D3DLOCK_NOOVERWRITE |
        D3DLOCK_NOSYSLOCK);*/

        m_nPolygons += 4;
    }
    if (pVP)
        pVP->mfSet(false, nullptr);
...
void CD3D8Renderer::DrawQuad(const Vec3d& right, const Vec3d& up, const Vec3d& origin, int nFlipMode /*=0*/) {
    ...
    Verts[0].xyz[0] = curr[0];
    Verts[0].xyz[1] = curr[1];
    Verts[0].xyz[2] = curr[2];
    Verts[0].st[0] = -1;
    Verts[0].st[1] = 0;

    curr = origin + (right - up);
    Verts[1].xyz[0] = curr[0];
    Verts[1].xyz[1] = curr[1];
    Verts[1].xyz[2] = curr[2];
    Verts[1].st[0] = 0;
    Verts[1].st[1] = 0;

    curr = origin + (right + up);
    Verts[2].xyz[0] = curr[0];
    Verts[2].xyz[1] = curr[1];
    Verts[2].xyz[2] = curr[2];
    Verts[2].st[0] = 0;
    Verts[2].st[1] = 1;

    curr = origin + (-right + up);
    Verts[3].xyz[0] = curr[0];
    Verts[3].xyz[1] = curr[1];
    Verts[3].xyz[2] = curr[2];
    Verts[3].st[0] = -1;
    Verts[3].st[1] = 1;
...
uint CD3D8Renderer::MakeSprite(float object_scale, int tex_size, float angle, IStatObj* pStatObj, uchar* _pTmpBuffer, uint def_tid) {
    ...
    STexPic* tp = m_TexMan->CreateTexture(name, tex_size, tex_size, 1, flags, FT2_NODXT, data, eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8888);
    STexPicD3D* t = (STexPicD3D*)tp;
    LPDIRECT3DTEXTURE8 pID3DTexture = nullptr;
    LPDIRECT3DTEXTURE8 pID3DTargetTexture = nullptr;
    delete[] data;
    if (!t->m_RefTex)
        return 0;
    ...
    // render object
    if (m_sbpp)
        m_pd3dDevice->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER | D3DCLEAR_STENCIL, cColor, 1.0f, 0);
    else
        m_pd3dDevice->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, cColor, 1.0f, 0);
    ...
    // Copy data
    {
        ...
        h = m_pd3dDevice->CopyRects(pSrcSurf, nullptr, 0, pDstSurf, nullptr);
        h = pID3DSrcTexture->LockRect(0, &d3dlrSrc, nullptr, 0);
        h = pID3DTexture->LockRect(0, &d3dlrDst, nullptr, 0);
        byte* ds = new byte[tex_size * tex_size * 4];
#ifndef _XBOX
        // Copy data to the texture
        memcpy(ds, d3dlrSrc.pBits, tex_size * tex_size * 4);
#else
        XGUnswizzleRect(d3dlrSrc.pBits, tex_size, tex_size, nullptr, ds, d3dlrSrc.Pitch, nullptr, 4);
#endif
...
#ifdef _XBOX
        XGSwizzleRect(dst, 0, nullptr, d3dlrDst.pBits, tex_size, tex_size, nullptr, sizeof(DWORD));
        delete[] dst;
#endif
...
void CD3D8Renderer::DrawBall(float x, float y, float z, float radius) {
    ...
    HRESULT hr = D3DXCreateSphere(m_pd3dDevice, radius, 16, 16, &pMesh, nullptr);
    if (FAILED(hr))
        return;
    EF_PushMatrix();
    EnableTMU(false);
...
unsigned int CD3D8Renderer::DownLoadToVideoMemory(unsigned char* data, int w, int h, ETEX_Format eTFSrc, ETEX_Format eTFDst, int nummipmap, bool repeat, int filter, int Id) {
    ...
    if (eTFSrc == eTF_DXT1) {
        flags |= FT_DXT1;
        blockSize = 8;
    } else if (eTFSrc == eTF_DXT3) {
        flags |= FT_DXT3;
        blockSize = 16;
    } else if (eTFSrc == eTF_DXT5) {
        flags |= FT_DXT5;
        blockSize = 16;
    }
    if (blockSize) {
    ...
    STexPic* tp = m_TexMan->CreateTexture(name, w, h, 1, flags, FT2_NODXT, data, eTT_Base, -1.0f, -1.0f, DXTSize, nullptr, 0, eTFSrc);
...
void CD3D8Renderer::DrawQuad3D(const Vec3d& v0, const Vec3d& v1, const Vec3d& v2, const Vec3d& v3, const CFColor& color, float ftx0, float fty0, float ftx1, float fty1, float ftx2,
                               float fty2, float ftx3, float fty3) {
    LPDIRECT3DDEVICE8 dv = mfGetD3DDevice();
    DWORD col = (DWORD(color.a * 255) << 24) | (DWORD(color.r * 255) << 16) | (DWORD(color.g * 255) << 8) | DWORD(color.b * 255);
