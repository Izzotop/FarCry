...
void CShader::mfCompilePassConditions(SShader* ef, char* scr, SShaderPassHW* sl) {
    ...
    static tokenDesc commands[] = {{eAmbient, "Ambient"},
                                   {eHasLM, "HasLM"},
                                   {eVars, "Vars"},
    ...
    while ((cmd = shGetObject(&scr, commands, &name, &params)) > 0) {
        ...
        case ePSParam: {
            ...
            if (n != sm->m_RCParamsNoObj->Num()) {
                ...
                else if (sm->m_RCParamsNoObj->Get(n).m_Reg == GL_CONSTANT_COLOR1_NV)
...
void CShader::mfCompileLightLayers(SShader* ef, char* scr, TArray<SShaderPassHW>& Layers) {
    ...
    while ((cmd = shGetObject(&scr, commands, &name, &params)) > 0) {
        ...
        case ePass: {
            ...
            else if (Layers[n].m_RCParamsNoObj) {
...
#define new DEBUG_CLIENTBLOCK
    ...
    static tokenDesc commands[] = {{eVProgram, "VProgram"},
                                   {eCGVProgram, "CGVProgram"},
...
void CShader::mfCompileMatrixOp_Matrix(SMatrixTransform_Matrix* mt, SShader* ef, char* scr) {
    ...
    static tokenDesc commands[] = {{eRow0, "Row0"},
                                   {eRow1, "Row1"},
                                   {eRow2, "Row2"},
                                   {eRow3, "Row3"},
