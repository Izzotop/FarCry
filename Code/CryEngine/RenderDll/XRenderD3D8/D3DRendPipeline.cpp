...
void CCObject::CalcInvertMatrix() {
    ...
    if (m_ObjFlags & FOB_TRANS_USEROTATE) {
        D3DXMatrixInverse((D3DXMATRIX*)m.GetData(), nullptr, (D3DXMATRIX*)m_Matrix.GetData());
    } else if (m_ObjFlags & FOB_TRANS_USESCALE) {
        m = m_Matrix;
    ...
    } else if (m_ObjFlags & FOB_TRANS_USETRANSLATE) {
        m = m_Matrix;
        m(3, 0) = -m(3, 0);
        m(3, 1) = -m(3, 1);
        m(3, 2) = -m(3, 2);
...
void CD3D8Renderer::EF_Start(SShader* ef, SShader* efState, SRenderShaderResources* Res, CRendElement* re) {
    ...
    m_RP.m_pShader = ef;
    m_RP.mCurLightMaterial = nullptr;
...
bool CD3D8Renderer::EF_SetLights(int Flags, bool Enable) {
    ...
    for (int i = 0; i < m_RP.m_DLights.Num(); i++) {
        ...
        Pos[2] = lpos.z;
        cDiffuse.a = 1.0f;
        EF_SetHWLight(n, Pos, cDiffuse, dl->m_SpecColor, fCA, fLA, fQA, fRange);
        n++;
    }
...
void CD3D8Renderer::EF_DrawDetailOverlay() {
    ...
    if (!(sh->m_Flags & EF_HASVSHADER)) {
        ...
        m_RP.m_VPDetail->mfSet(true, nullptr, 1);
        ...
        m_RP.m_VPDetail->mfSet(false, nullptr, 1);
        if (m_RP.m_RCDetail)
...
void CD3D8Renderer::EF_Flush() {
    ...
    if (rd->m_LogFile) {
        ...
        rd->Logv(SRendItem::m_RecurseLevel, "%s: '%s', (St: %s) Id:%d, Obj:%d(%s), Tech: %d, Cp: %d, Fog:%d, VF:%d, NL:%d, LPas:%d, Pas:%d (time: %f, %d polys)\n", str,
                 ef->m_Name.c_str(), rd->m_RP.m_pStateShader ? rd->m_RP.m_pStateShader->m_Name.c_str() : "nullptr", ef->m_Id, rd->m_RP.m_pCurObject->m_VisId,
                 (rd->m_RP.m_pCurObject->m_ObjFlags & FOB_USEMATRIX) ? "Matr" : "NoMatr", rd->m_RP.m_CurTechnique, rd->m_RP.m_ClipPlaneEnabled,
...
void CD3D8Renderer::EF_EndEf3D(bool bSort) {
    ...
    //  EF_AddEf(0, m_RP.m_pREGlare, gcEf.m_GlareShader, nullptr, nullptr, -1);
...
void CD3D8Renderer::EF_PrintProfileInfo() {
    ...
    TextToScreenColor(8, 14, 0, 2, 0, 1, "Shad.: %d, VShad: %d, PShad: %d, RComb: %d, Text: %d, Lit Shaders: %d", m_RP.m_PS.m_NumRendShaders, m_RP.m_PS.m_NumVShaders,
                      m_RP.m_PS.m_NumPShaders, m_RP.m_PS.m_NumRCombiners, m_RP.m_PS.m_NumTextures, m_RP.m_PS.m_NumLitShaders);
    ...
    { // sort by interpolated time and print
        ...
        for (nLine = 0; nLine < m_RP.m_Profile.Num(); nLine++) {
            ...
            TextToScreenColor(4, (24 + (nLine * 3)), 1, 0, 0, 1, "%8.02f ms, %5d polys, '%s'", m_RP.m_Profile[nLine].Time, m_RP.m_Profile[nLine].NumPolys,
...
int CD3D8Renderer::EF_Preprocess(SRendItemPre* ri, int nums, int nume) {
    ...
    for (i = 0; i < m; i++) {
        ...
        } else if ((pr->m_Shader->m_Flags2 & EF2_PREPR_SCANSCR) && !m_RP.m_bDrawToTexture) { // Draw screen to texture
        } else if ((pr->m_Shader->m_Flags2 & EF2_PREPR_SCANTEX) && !m_RP.m_bDrawToTexture) { // Draw environment to texture
        ...
        } else if ((pr->m_Shader->m_Flags2 & EF2_PREPR_SHADOWMAPGEN)) {
            CCObject* obj = m_RP.m_VisObjects[pr->m_nObject];
            m_RP.m_pCurObject = obj;
        ...
        } else if ((pr->m_Shader->m_Flags2 & EF2_PREPR_CORONA) && pr->m_nObject > 0) {
        ...
        } else if (pr->m_Shader->m_Flags2 & EF2_PREPR_OUTSPACE) {
            CCObject* obj = m_RP.m_VisObjects[pr->m_nObject];
            m_RP.m_pCurObject = obj;
        ...
        } else if (pr->m_Shader->m_Flags2 & EF2_PREPR_PORTAL) {
            if (!CV_r_portals)
                continue;
