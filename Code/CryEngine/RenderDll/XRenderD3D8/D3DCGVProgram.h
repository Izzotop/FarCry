...
public:
    ...
    static void mfCgErrorCallback() {
        ...
        } else if (error == cgD3D8DebugTrace)
        ...
        if (pPosVP && pPosVP->m_PosScript) {
            ...
            if (cg.m_PosScriptName != m_PosScript->m_Name && pPosVP->m_ParamsNoObj.Num()) {
                ...
                for (int i = 0; i < pPosVP->m_ParamsNoObj.Num(); i++) {
                    ...
                    cg.m_ParamsNoObj->Get(cg.m_ParamsNoObj->Num() - 1).m_pBind = nullptr;
                ...
                for (int i = 0; i < pPosVP->m_ParamsObj.Num(); i++) {
                    ...
                    cg.m_ParamsObj->Get(cg.m_ParamsObj->Num() - 1).m_pBind = nullptr;
    ...
    const char* mfLoadCG(const char* prog_text) {
        ...
        // Assemble the shader
        if (FAILED(cgD3D8LoadProgram(m_Insts[m_CurInst].m_CGProgram, false, 0, 0,
                                     &gRenDev->m_RP.m_D3DFixedPipeline[m_Insts[m_CurInst].Streams][m_Insts[m_CurInst].VertFormat].m_Declaration[0]))) {
    ...
    void mfLoad(const char* prog_text) {
...
#ifdef _XBOX
        LPXGBUFFER pCode;
        LPXGBUFFER pBuffer = nullptr;
        hr = XGAssembleShader(m_Name.c_str(), prog_text, strlen(prog_text), 0, nullptr, &pCode, &pBuffer, nullptr, nullptr, nullptr, 0);
#else
        LPD3DXBUFFER pCode;
        LPD3DXBUFFER pBuffer = nullptr;
        hr = D3DXAssembleShader(prog_text, strlen(prog_text), 0, nullptr, &pCode, &pBuffer);
#endif
        if (FAILED(hr)) {
        ...
        hr = gcpRendD3D->mfGetD3DDevice()->CreateVertexShader(&gRenDev->m_RP.m_D3DFixedPipeline[m_Insts[m_CurInst].Streams][m_Insts[m_CurInst].VertFormat].m_Declaration[0],
                                                              (DWORD*)pCode->GetBufferPointer(), (DWORD*)&m_Insts[m_CurInst].m_dwHandle, 0);
        if (FAILED(hr)) {
    ...
    bool mfIsValid(int Num) const {
        return (m_Insts[Num].m_dwHandle != 0);
    }
