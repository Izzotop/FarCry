...
bool CREOcLeaf::mfPreDraw(SShaderPass* sl) {
    ...
    if (gRenDev->m_RP.m_FlagsModificators & RBMF_TANGENTSUSED) {
        ...
        gRenDev->m_RP.m_PersFlags &= ~RBPF_USESTREAM1;
        h = dv->SetStreamSource(1, nullptr, 0);
    }
    ...
    } else if (gRenDev->m_RP.m_PersFlags & RBPF_USESTREAM2) {
        gRenDev->m_RP.m_PersFlags &= ~RBPF_USESTREAM2;
        h = dv->SetStreamSource(2, nullptr, 0);
    }
...
void GlareQuad(const CFColor& color, CD3D8Renderer* gcpRendD3D) {
    /*
      ...
      glEnd ();
    */
    gcpRendD3D->DrawQuad3D(Vec3d(0.1f, 1, 1), Vec3d(0.1f, -1, 1), Vec3d(0.1f, -1, -1), Vec3d(0.1f, 1, -1), color, -1, 1, 0, 0);
}
    ...
    */
    ...
    if (CRenderer::CV_r_checkSunVis == 2) {
        ...
        if (!obj->m_TexId0) {
            ...
            STexPic* tp = gcpRendD3D->m_TexMan->CreateTexture(name, sizeMask, sizeMask, 1, FT_NOMIPS | FT_ALLOCATED, FT2_NODXT, data, eTT_Base, -1.0f, -1.0f, 0, nullptr);
            delete[] data;
        ...
        if (!obj->m_TexId1) {
            ...
            char name[128];
            sprintf(name, "$AutoCoronas_%d", gcpRendD3D->m_TexGenID++);
            STexPic* tp = gcpRendD3D->m_TexMan->CreateTexture(name, sizeTex, sizeTex, 1, FT_NOMIPS | FT_ALLOCATED, FT2_NODXT, data, eTT_Base, -1.0f, -1.0f, 0, nullptr);
            delete[] data;
    ...
    } else if (CRenderer::CV_r_checkSunVis == 3) {
        gRenDev->ResetToDefault();
        int sizeMask = 64;
        int sizeMask2 = sizeMask / 2;
...
void CREFlare::mfDrawCorona(SShader* ef, CFColor& col) {
    ...
    */
    ...
    } else if (CRenderer::CV_r_checkSunVis == 2) {
        gRenDev->SelectTMU(0);
...
bool CRESky::mfDraw(SShader* ef, SShaderPass* sfm) {
    ...
    if (bPrevClipPl)
        gcpRendD3D->EF_SetClipPlane(false, nullptr, false);

    if (sfm == &ef->m_Passes[1]) { // draw sky sphere vertices at pass 1
    ...
    { // top
        struct_VERTEX_FORMAT_P3F_TEX2F data[] = {{fSkyBoxSize, -fSkyBoxSize, fSkyBoxSize, 1, 1.f - 1},
    ...
    { // s
        struct_VERTEX_FORMAT_P3F_TEX2F data[] = {-fSkyBoxSize, -fSkyBoxSize, fSkyBoxSize, 1.0, 1.f - 1.0, fSkyBoxSize, -fSkyBoxSize, fSkyBoxSize, 0.0, 1.f - 1.0,
                                                 -fSkyBoxSize, -fSkyBoxSize, -P,          1.0, 1.f - 0.5, fSkyBoxSize, -fSkyBoxSize, -P,          0.0, 1.f - 0.5,
                                                 -fSkyBoxSize, -fSkyBoxSize, -D,          1.0, 1.f - 0.5, fSkyBoxSize, -fSkyBoxSize, -D,          0.0, 1.f - 0.5};
    ...
    { // e
        struct_VERTEX_FORMAT_P3F_TEX2F data[] = {-fSkyBoxSize, fSkyBoxSize, fSkyBoxSize, 1.0, 1.f - 0.0, -fSkyBoxSize, -fSkyBoxSize, fSkyBoxSize, 0.0, 1.f - 0.0,
                                                 -fSkyBoxSize, fSkyBoxSize, -P,          1.0, 1.f - 0.5, -fSkyBoxSize, -fSkyBoxSize, -P,          0.0, 1.f - 0.5,
                                                 -fSkyBoxSize, fSkyBoxSize, -D,          1.0, 1.f - 0.5, -fSkyBoxSize, -fSkyBoxSize, -D,          0.0, 1.f - 0.5};
