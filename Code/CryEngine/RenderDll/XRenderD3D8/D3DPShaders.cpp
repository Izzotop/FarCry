...
CPShader* CPShader::mfForName(char* name, bool bCGType) {
    ...
    if (statussrc != INVALID_HANDLE_VALUE) {
        GetFileTime(statussrc, nullptr, nullptr, &p->m_WriteTime);
...
bool CPShader_D3D::mfUnbind() {
    HRESULT hr;
    hr = gcpRendD3D->mfGetD3DDevice()->SetPixelShader(nullptr);
    if (FAILED(hr))
        return false;
    return true;
}
...
bool CPShader_D3D::mfActivate() {
...
#ifndef _XBOX
    hr = XGAssembleShader(m_Name.c_str(), m_pScript, strlen(m_pScript), 0, nullptr, &pCode, &pBuffer, nullptr, nullptr, nullptr, 0);
#else  //_XBOX
    char* pScriptTmp = PSToXboxPS(m_pScript);
    hr = XGAssembleShader(m_Name.c_str(), pScriptTmp, strlen(pScriptTmp), 0, nullptr, &pCode, &pBuffer, nullptr, nullptr, nullptr, 0);
    delete[] pScriptTmp;
#endif ///_XBOX
    ...
    hr = D3DXAssembleShader(m_pScript, strlen(m_pScript), 0, nullptr, &pCode, &pBuffer);
    if (FAILED(hr)) {
