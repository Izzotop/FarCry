...
public:
    ...
    virtual void SetColors(const D3DLOCKED_RECT& LockData, DWORD i, DWORD j, float fRed, float fGreen, float fBlue, float fAlpha) {
        assert(0);
    };
    ...
    virtual void SetLuminance(const D3DLOCKED_RECT& LockData, DWORD i, DWORD j, float Luminance) {
        assert(0);
    };

    virtual void GetVector(const D3DLOCKED_RECT& LockData, DWORD i, DWORD j, D3DXVECTOR3& inVector) {
        assert(0);
    };
...
public:
    enum { MAX_LOCK_LEVELS = 12 };

    D3D2DTextureLocker(LPDIRECT3DTEXTURE8 pTexture) : m_pTexture(pTexture), m_pDataFormat(nullptr) {
        m_pTexture->AddRef();
...
public:
    D3D2DSurfaceLocker(LPDIRECT3DSURFACE8 pSurface) : m_pSurface(pSurface), m_pDataFormat(nullptr) {
        m_pSurface->AddRef();
        m_bLocked = false;
...
public:
    ...
    static LPDIRECT3DTEXTURE8 CreateNormalMap(LPDIRECT3DDEVICE8 pD3DDev, LPDIRECT3DTEXTURE8 pSource, STexPic* ti, bool bMips, D3DXVECTOR3 Scale, D3DFORMAT Format = D3DFMT_Q8W8V8U8,
                                              D3DPOOL Pool = D3DPOOL_MANAGED) {
        LPDIRECT3DTEXTURE8 pNormalMap = nullptr;
        D3DSURFACE_DESC ddsdDescDest;
        D3DSURFACE_DESC ddsdDescSource;
        D3DXVECTOR3 Normal;
        HRESULT hr;
        DWORD i, j;
        LPDIRECT3DTEXTURE8 pNewTex = nullptr;
