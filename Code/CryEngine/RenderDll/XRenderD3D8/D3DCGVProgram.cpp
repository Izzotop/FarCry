...
CVProgram* CVProgram::mfForName(char* name, bool bCGType) {
    ...
    CVProgram* p = nullptr;
    {
        ...
        HANDLE statussrc = CreateFile(scrname, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, nullptr);
        if (statussrc != INVALID_HANDLE_VALUE) {
            GetFileTime(statussrc, nullptr, nullptr, &pr->m_WriteTime);
...
static char* sAdditionalVP[] = {{"OUT.FogC = IN.Fog.x;\n"
                                 "return OUT;\n"
                                 "}\n"},
...
char* CCGVProgram_D3D::mfCreateAdditionalVP(SCGScript* pPosScript) {
    ...
    if (!(m_Insts[m_CurInst].m_Mask & VPVST_NOFOG)) {
        ...
        while (true) {
            ...
            if (sNewStr[n] == '{') {
                n++;
                while (sNewStr[n] == 0x20 || sNewStr[n] == 8 || sNewStr[n] == 0xd || sNewStr[n] == 0xa) {
                    n++;
                }
...
bool CCGVProgram_D3D::mfActivate(CCGVProgram_D3D* pPosVP) {
    ...
    if (!m_Insts[m_CurInst].m_dwHandle) {
        ...
        statusdst = CreateFile(namedst, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, nullptr);
        if (statusdst == INVALID_HANDLE_VALUE)
            bCreate = true;
        else {
            statussrc = CreateFile(namesrc, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, nullptr);

            GetFileTime(statussrc, nullptr, nullptr, &writetimesrc);
            GetFileTime(statusdst, nullptr, nullptr, &writetimedst);
            CloseHandle(statussrc);
        ...
        if (bCreate) {
            ...
            if (code) {
                ...
                statusdst = CreateFile(namedst, GENERIC_WRITE, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, nullptr);
                statussrc = CreateFile(namesrc, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, nullptr);
                GetFileTime(statussrc, nullptr, nullptr, &writetimesrc);
                SetFileTime(statusdst, nullptr, nullptr, &writetimesrc);
                CloseHandle(statussrc);
                CloseHandle(statusdst);
            }
