...
void STexPicD3D::Set() {
    ...
    if (m_RefTex->bMips != r->mStages[tmu].UseMips || m_RefTex->m_MagFilter != r->mStages[tmu].MagFilter || m_RefTex->m_MinFilter != r->mStages[tmu].MinFilter ||
        m_RefTex->m_AnisLevel != r->mStages[tmu].Anisotropic) {
        ...
        } else if (m_RefTex->m_AnisLevel > 1) {
...
void CD3D8TexMan::SetTexture(int Id, ETexType eTT) {
    ...
    if (rt->bMips != r->mStages[tmu].UseMips || rt->m_MagFilter != r->mStages[tmu].MagFilter || rt->m_MinFilter != r->mStages[tmu].MinFilter ||
        rt->m_AnisLevel != r->mStages[tmu].Anisotropic) {
        ...
        if (CRenderer::CV_r_nobindtextures == 1)
            ...
            if (bind >= TX_FIRSTBIND) {
                ...
                case TO_ENVIRONMENT_CUBE_MAP: {
                    ...
                    cm = gcEf.mfFindSuitableEnvCMap(Vec3d(rd->m_RP.m_pCurObject->m_Matrix[3][0], rd->m_RP.m_pCurObject->m_Matrix[3][1], rd->m_RP.m_pCurObject->m_Matrix[3][2]),
                                                    true, 0);
                    if (cm)
                        cm->m_Tex->Set();
                    else
...
void STexPicD3D::SaveTGA(const char* name, bool bMips) {
    ...
    CD3D8Renderer* r = gcpRendD3D;
    LPDIRECT3DDEVICE8 dv = r->mfGetD3DDevice();
    IDirect3DTexture8* pID3DTexture = nullptr;
    IDirect3DCubeTexture8* pID3DCubeTexture = nullptr;
...
void CD3D8TexMan::D3DCreateVideoTexture(int tgt, byte* src, int wdt, int hgt, D3DFORMAT SrcFormat, D3DFORMAT DstFormat, STexPicD3D* ti, bool bMips, int CubeSide, PALETTEENTRY* pe,
                                        int DXTSize) {
    int i = 0;
    int offset = 0;
    int size;
    ...
    LPDIRECT3DTEXTURE8 pID3DTexture = nullptr;
    LPDIRECT3DCUBETEXTURE8 pID3DCubeTexture = nullptr;
    LPDIRECT3DTEXTURE8 pID3DSrcTexture = nullptr;
    ...
    if (tgt == TEXTGT_2D) {
        ...
        if (CubeSide == 0) {
            ...
            } else if (FAILED(h = D3DXCreateCubeTexture(dv, wdt, bMips ? D3DX_DEFAULT : 1, 0, DstFormat, D3DPOOL_MANAGED, &pID3DCubeTexture)))
                return;
    ...
    if ((SrcFormat == D3DFMT_DXT1 || SrcFormat == D3DFMT_DXT3 || SrcFormat == D3DFMT_DXT5) && SrcFormat == DstFormat) {
        ...
        if (pID3DCubeTexture)
            ...
            ti->m_RefTex->m_VidTex = pID3DTexture;
        return;
    } else if ((SrcFormat == D3DFMT_P8) && SrcFormat == DstFormat) {
#ifndef _XBOX
...
#ifdef _XBOX
        if ((SrcFormat == D3DFMT_LIN_A8R8G8B8 || SrcFormat == D3DFMT_LIN_X8R8G8B8 || SrcFormat == D3DFMT_A8R8G8B8 || SrcFormat == D3DFMT_X8R8G8B8 || SrcFormat == D3DFMT_X8L8V8U8 ||
             SrcFormat == D3DFMT_LIN_X8L8V8U8) &&
            (DstFormat == D3DFMT_A8R8G8B8 || DstFormat == D3DFMT_X8R8G8B8 || DstFormat == D3DFMT_X8L8V8U8 || DstFormat == D3DFMT_LIN_X8L8V8U8))
#else
        if ((SrcFormat == D3DFMT_A8R8G8B8 || SrcFormat == D3DFMT_X8R8G8B8 || SrcFormat == D3DFMT_X8L8V8U8) &&
            (DstFormat == D3DFMT_A8R8G8B8 || DstFormat == D3DFMT_X8R8G8B8 || DstFormat == D3DFMT_X8L8V8U8))
#endif
    {
        ti->m_Size = 0;
        while (wdt || hgt) {
    ...
    if (bMips && ti->m_nMips <= 1) {
        ...
        if (pID3DCubeTexture) {
            if (CubeSide == 5) {
#ifdef _XBOX
                h = D3DXFilterCubeTexture(pID3DCubeTexture, nullptr, 0, MipFilter);
#else
                h = D3DXFilterTexture((LPDIRECT3DTEXTURE8)pID3DCubeTexture, nullptr, 0, MipFilter);
#endif
            }
        } else
            h = D3DXFilterTexture(pID3DTexture, nullptr, 0, MipFilter);
    }
...
void CD3D8TexMan::D3DCompressTexture(int tgt, STexPicD3D* ti, int CubeSide) {
    ...
    LPDIRECT3DTEXTURE8 pID3DSrcTexture = nullptr;
    LPDIRECT3DCUBETEXTURE8 pID3DSrcCubeTexture = nullptr;
    LPDIRECT3DTEXTURE8 pID3DTexture = nullptr;
    LPDIRECT3DCUBETEXTURE8 pID3DCubeTexture = nullptr;
    if (tgt == TEXTGT_2D)
...
STexPic* CD3D8TexMan::CreateTexture(const char* name, int wdt, int hgt, int depth, uint flags, uint flags2, byte* dst, ETexType eTT, float fAmount1, float fAmount2, int DXTSize,
                                    STexPic* tp, int bind, ETEX_Format eTF, const char* szSourceName) {
    byte* dst1 = nullptr;
    int i;
    LPDIRECT3DDEVICE8 dv = gcpRendD3D->mfGetD3DDevice();
    ...
    if (!tp) {
        tp = TextureInfoForName(name, nullptr, -1, eTT, flags, flags2, bind);
    ...
    if (dst) {
        ...
        if (eTF == eTF_8888 || eTF == eTF_RGBA) {
            srcFormat = D3DFMT_A8R8G8B8;
            SizeSrc = wdt * hgt * 4;
        } else if (eTF == eTF_0888 || eTF == eTF_DSDT_MAG) {
            int size = 0;
            int w = wdt;
            int h = hgt;
            if (ti->m_nMips) {
        ...
        if (ti->m_Flags2 & FT2_FORCEDXT) {
            ...
            if (!(ti->m_Flags & FT_HASALPHA) || ti->m_eTT == eTT_Bumpmap || ti->m_eTT == eTT_DSDTBump) // 3 components;
            {
                if (ti->m_eTT == eTT_Bumpmap) {
                    if (CRenderer::CV_r_texbumpquality == 0)
                    ...
                    else if (CRenderer::CV_r_texbumpquality == 1) {
                        ...
                        format = D3DFMT_P8;
                        SizeSrc = wdt * hgt;
                    } else if (CRenderer::CV_r_texbumpquality == 2)
                        format = D3DFMT_DXT1;
                    else
                        format = D3DFMT_X8L8V8U8;
                } else if (ti->m_eTT == eTT_DSDTBump)
                    format = D3DFMT_X8L8V8U8;
                else {
                    if (CRenderer::CV_r_texquality == 0)
                        format = D3DFMT_X8R8G8B8;
                    else if (CRenderer::CV_r_texquality == 1)
                        format = D3DFMT_R5G6B5;
                    else if (CRenderer::CV_r_texquality == 2)
                        format = D3DFMT_DXT1;
                    else
                        format = D3DFMT_X8R8G8B8;
                }
        ...
        else if (srcFormat == D3DFMT_A8R8G8B8)
            srcFormat = D3DFMT_LIN_A8R8G8B8;
        else if (srcFormat == D3DFMT_X8R8G8B8)
            srcFormat = D3DFMT_LIN_X8R8G8B8;
        else if (srcFormat == D3DFMT_X8L8V8U8)
            srcFormat = D3DFMT_LIN_X8L8V8U8;
        else if (srcFormat == D3DFMT_A4R4G4B4)
            srcFormat = D3DFMT_LIN_A4R4G4B4;
        else if (srcFormat == D3DFMT_R5G6B5)
        ...
        {
            {
                if (ti->m_eTT == eTT_Cubemap) {
                    ...
                    if (!strcmp(&ti->m_SearchName.c_str()[n], "posx"))
                        CubeSide = 0;
                    else if (!strcmp(&ti->m_SearchName.c_str()[n], "negx"))
                        CubeSide = 1;
                    else if (!strcmp(&ti->m_SearchName.c_str()[n], "posy"))
                        CubeSide = 2;
                    else if (!strcmp(&ti->m_SearchName.c_str()[n], "negy"))
                        CubeSide = 3;
                    else if (!strcmp(&ti->m_SearchName.c_str()[n], "posz"))
                        CubeSide = 4;
                    else if (!strcmp(&ti->m_SearchName.c_str()[n], "negz"))
                        CubeSide = 5;
                    else
                        CubeSide = 0;
    ...
    if (pImage) {
        ...
        */

        {
            ...
            DWORD col = (DWORD(color.a * 255) << 24) | (DWORD(color.r * 255) << 16) | (DWORD(color.g * 255) << 8) | DWORD(color.b * 255);
...
void CD3D8TexMan::ScanEnvironmentCube(SEnvTexture* cm, int RendFlags) {
    ...
    if (!(cm->m_Tex->m_Flags & FT_ALLOCATED) || tex_size != cm->m_TexSize) {
        ...
        gRenDev->m_TexMan->CreateTexture(nullptr, tex_size, tex_size, 1, FT_NOMIPS, FT2_NODXT | FT2_RENDERTARGET, data, eTT_Cubemap, -1.0f, -1.0f, 0, cm->m_Tex);
...
void CD3D8TexMan::ScanEnvironmentTexture(SEnvTexture* cm, int RendFlags) {
    ...
    if (!(cm->m_Tex->m_Flags & FT_ALLOCATED) || tex_size != cm->m_TexSize) {
        ...
        cm->m_Tex->m_nMips = 0;
        byte* data = new byte[tex_size * tex_size * 4];
        gRenDev->m_TexMan->CreateTexture(nullptr, tex_size, tex_size, 1, FT_NOMIPS, FT2_NODXT | FT2_RENDERTARGET, data, eTT_Base, -1.0f, -1.0f, 0, cm->m_Tex);
        delete[] data;
    }
...
void CD3D8TexMan::DrawToTexture(Plane& Pl, STexPic* Tex, int RendFlags) {
    float Smat[16] = {0.5f, 0, 0, 0, 0, -0.5f, 0, 0, 0, 0, 0.5f, 0, 0.5f, 0.5f, 0.5f, 1.0f};
    ...
    if (!(Tex->m_Flags & FT_ALLOCATED)) {
        ...
        gRenDev->m_TexMan->CreateTexture(nullptr, tex_size, tex_size, 1, FT_NOMIPS | FT_CLAMP, FT2_NODXT | FT2_RENDERTARGET, data, eTT_Base, -1.0f, -1.0f, 0, Tex);
        delete[] data;
    }
    ...
    /*{
      ...
      h = r->m_pd3dDevice->CopyRects(pSrcSurf, nullptr, 0, pDstSurf, nullptr);
      h = pID3DSrcTexture->LockRect(0, &d3dlr, nullptr, 0);
      // Copy data to the texture
  #ifdef _XBOX
      DWORD *pSrcBits = new DWORD[tex_size*tex_size];
      //XGUnswizzleRect( d3dlr.pBits, tex_size, tex_size, nullptr, pSrcBits, d3dlr.Pitch, nullptr, 4 );
...
void CD3D8TexMan::DrawToTextureForGlare(int Id) {
    ...
    if (CRenderer::CV_r_glaresize != gRenDev->m_RP.m_GlareSize) {
        ...
        else if (CRenderer::CV_r_glaresize <= 64)
            CRenderer::CV_r_glaresize = 64;
        else if (CRenderer::CV_r_glaresize <= 128)
    ...
    ClearBuffer(width, height, true, nullptr, 0);
    ...
    // Generate texture with mip-maps
    {
        ...
        if (!sTexId) {
            ...
            */
            ...
            STexPic* tp = gcpRendD3D->m_TexMan->CreateTexture(name, width, height, 1, FT_NOMIPS | FT_ALLOCATED, FT2_NODXT, data, eTT_Base, -1.0f, -1.0f, 0, nullptr);
            delete[] data;
            sTexId = tp->m_Bind;
...
void CD3D8TexMan::StartHeatMap(int Id) {
    ...
    if (CRenderer::CV_r_heatsize != gRenDev->m_RP.m_HeatSize) {
        ...
        else if (CRenderer::CV_r_heatsize <= 128)
            CRenderer::CV_r_heatsize = 128;
        else if (CRenderer::CV_r_heatsize <= 256)
            CRenderer::CV_r_heatsize = 256;
        else if (CRenderer::CV_r_heatsize <= 512)
    ...
    if (!(Tex->m_Flags & FT_ALLOCATED)) {
        ...
        Tex->m_Flags2 |= FT2_NODXT | FT2_RENDERTARGET;
        AddToHash(Tex->m_Bind, Tex);
        gRenDev->m_TexMan->CreateTexture(nullptr, Tex->m_Width, Tex->m_Height, 1, FT_NOMIPS, FT2_NODXT | FT2_RENDERTARGET, data, eTT_Base, -1.0f, -1.0f, 0, Tex);
        delete[] data;
    }
    ...
    // DLD_TERRAIN_WATER | DLD_NEAR_OBJECTS; eng->DrawLowDetail(nDrawFlags);
}
...
void CD3D8TexMan::StartNightMap(int Id) {
    ...
    if (CRenderer::CV_r_nightsize != gRenDev->m_RP.m_NightSize) {
        ...
        else if (CRenderer::CV_r_nightsize <= 128)
            CRenderer::CV_r_nightsize = 128;
        else if (CRenderer::CV_r_nightsize <= 256)
            CRenderer::CV_r_nightsize = 256;
        else if (CRenderer::CV_r_nightsize <= 512)
...
void CD3D8TexMan::Update() {
    ...
    if (CRenderer::CV_r_logusedtextures == 1 || CRenderer::CV_r_logusedtextures == 3 || CRenderer::CV_r_logusedtextures == 4) {
        ...
        static char* sTexType[] = {"Base", "Bump", "DSDTBump", "Cubemap", "AutoCubemap", "Rectangle"};
        static char* sTexFormat[] = {"Unknown", "Index8", "HSV",  "0888",          "8888",         "RGBA",        "8000", "0565",     "0555", "4444", "1555",
                                     "DXT1",    "DXT3",   "DXT5", "SIGNED_HILO16", "SIGNED_HILO8", "SIGNED_RGB8", "RGB8", "DSDT_MAG", "DSDT", "0088"};
        ...
        if (CRenderer::CV_r_logusedtextures == 1 || CRenderer::CV_r_logusedtextures == 3) {
            for (i = 0; i < m_Textures.Num(); i++) {
                ...
                } else if (CRenderer::CV_r_logusedtextures == 1 && m_Textures[i] && m_Textures[i]->m_bBusy && m_Textures[i]->m_Bind != CTexMan::m_Text_NoTexture->m_Bind &&
                           (m_Textures[i]->m_Flags2 & FT2_WASLOADED) && !(m_Textures[i]->m_Flags2 & FT2_WASUNLOADED)) {
                    if (m_Textures[i]->m_eTT != eTT_Cubemap || !m_Textures[i]->m_CubeSide)
                        Texs.AddElem(m_Textures[i]);
                }
            }
        ...
        for (i = 0; i < Texs.Num(); i++) {
            if (fp)
                fprintf(fp, "%.3fKb\t\tType: %s\t\tFormat: %s\t\t(%s)\n", Texs[i]->m_Size / 1024.0f, sTexType[Texs[i]->m_eTT], sTexFormat[Texs[i]->m_ETF],
                        Texs[i]->m_SearchName.c_str());
            else {
    ...
    } else if (CRenderer::CV_r_logusedtextures == 2) {
        ...
        sprintf(buf, "All Normal Maps: %d (FullSize: %.3fMb, All MIPS: %.3fMb, Loaded MIPS: %.3fMb)", TexsNM.Num(), AllSizeNM / (1024.0f * 1024.0f), SizeNM / (1024.0f * 1024.0f),
                PartSizeNM / (1024.0f * 1024.0f));
        rd->TextToScreenColor(4, 19, 1, 1, 0, 1, buf);

        Texs.Free();
        for (i = 0; i < m_Textures.Num(); i++) {
            if (m_Textures[i] && m_Textures[i]->m_bBusy && (m_Textures[i]->m_Flags2 & FT2_WASLOADED) && m_Textures[i]->m_Bind != CTexMan::m_Text_NoTexture->m_Bind &&
                m_Textures[i]->m_AccessFrame == rd->GetFrameID()) {
                if (m_Textures[i]->m_eTT != eTT_Cubemap || !m_Textures[i]->m_CubeSide)
                    Texs.AddElem(m_Textures[i]);
            }
        }
        ...
        sprintf(buf, "Current tex. objects: %d (Size: %.3fMb, Loaded: %.3f, NonStreamed: %.3f)", Texs.Num(), Size / (1024.0f * 1024.0f), PartSize / (1024.0f * 1024.0f),
                NonStrSize / (1024.0f * 1024.0f));
...
char* CD3D8Renderer::GetTexturesStatusText() {
    ...
    for (i = 0; i < CTexMan::m_Textures.Num(); i++) {
        if (CTexMan::m_Textures[i] && CTexMan::m_Textures[i]->m_bBusy && CTexMan::m_Textures[i]->m_Bind != CTexMan::m_Text_NoTexture->m_Bind &&
            CTexMan::m_Textures[i]->m_AccessFrame == GetFrameID())
            Texs.AddElem(CTexMan::m_Textures[i]);
    }
    qsort(&Texs[0], Texs.Num(), sizeof(STexPic*), TexCallback);
    Size = 0;
...
void CD3D8TexMan::GenerateFogMaps() {
    int i, j;
    {
        ...
        CTexMan::m_Text_Fog = CreateTexture("$Fog", 128, 128, 1, FT_CLAMP | FT_NOMIPS | FT_NOREMOVE | FT_HASALPHA, FT2_NODXT, Data1, eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8888);
        delete[] Data1;
        ...
        // CTexMan::m_Text_Fog_Enter = DownloadTexture("(FogEnter)", 64, 64, FT_CLAMP | FT_NOMIPS | FT_NOREMOVE | FT_HASALPHA, FT2_NODXT, &Data2[0][0][0], eTT_Base, 0, nullptr, 0,
        // eTF_8888); CTexMan::m_Text_Fog_Enter->SaveTGA("FogEnter.tga", false);
...
void CD3D8TexMan::GenerateFlareMap() {
    ...
    CTexMan::m_Text_Flare = CreateTexture("$Flare", 32, 4, 1, FT_CLAMP | FT_NOREMOVE, FT2_NODXT, &data[0][0][0], eTT_Base, -1.0f, -1.0f, 0, nullptr, 0, eTF_8888);
}
...
static void MakeCubeMapXBox(LPDIRECT3DCUBETEXTURE8 pCubeMap, int dwSize, bool bMips) {
    ...
    for (DWORD i = 0; i < 6; i++) {
        ...
        XGSwizzleRect(pSourceBits, 0, nullptr, lock.pBits, dwSize, dwSize, nullptr, sizeof(DWORD));
