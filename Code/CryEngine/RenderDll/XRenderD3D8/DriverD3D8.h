...
public:
    LPDIRECT3DDEVICE8 mfGetD3DDevice() {
        return m_pd3dDevice;
    }
    LPDIRECT3D8 mfGetD3D() {
        return m_pD3D;
    }
    D3DSURFACE_DESC* mfGetZSurfaceDesc() {
        return &m_d3dsdZBuffer;
    }
    LPDIRECT3DSURFACE8 mfGetZSurface() {
        return m_pZBuffer;
    }
    LPDIRECT3DSURFACE8 mfGetBackSurface() {
        return m_pBackBuffer;
    }
    D3DCAPS8* mfGetD3DCaps() {
        return &m_d3dCaps;
    }
...
public:
    ...
    void DrawQuad3D(const Vec3d& v0, const Vec3d& v1, const Vec3d& v2, const Vec3d& v3, const CFColor& color, float ftx0 = 0, float fty0 = 0, float ftx1 = 1, float fty1 = 1,
...
public:
    ...
    virtual void Draw3dPrim(const Vec3d& mins, const Vec3d& maxs, int nPrimType = DPRIM_WHIRE_BOX, const float* fRGBA = nullptr);
    ...
    virtual void SetupShadowOnlyPass(int Num, ShadowMapFrustum* pFrustum, Vec3d* vShadowTrans, const float fShadowScale, Vec3d vObjTrans = Vec3d(0, 0, 0), float fObjScale = 1.f,
                                     const Vec3d vObjAngles = Vec3d(0, 0, 0), Matrix44* pObjMat = 0);
    //  virtual void DrawShadowGrid(const Vec3d & pos, const Vec3d & Scale, ShadowMapFrustum*lf, bool translate_projection, float alpha, IndexedVertexBuffer* pVertexBuffer, float
    //  anim_angle);
    ...
    virtual int GenBumpTexUnsignedNorMap(char* bump_filename) {
        return 0;
    };
...
public:
    ...
    void D3DCreateVideoTexture(int tgt, byte* src, int wdt, int hgt, D3DFORMAT SrcFormat, D3DFORMAT DstFormat, STexPicD3D* ti, bool bMips, int CubeSide, PALETTEENTRY* pe,
                               int DXTSize);
    ...
    static void BindNULL(int From) {
        ...
        for (; From < n; From++) {
            ...
            gcpRendD3D->mStages[From].Texture = nullptr;
        }
    }
