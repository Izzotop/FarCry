...
void CEntityClassRegistry::ResetClassRegistry() {
    if (m_pScriptSystem) {
        ...
        while (itor != m_vEntityClasses.end()) {
            m_pScriptSystem->SetGlobalTonullptr(itor->second.strClassName.c_str());
...
bool CEntityClassRegistry::InitRegistry() {
    ...
    while (pTable->GetAt(++i, *pLineObj)) {
        ...
        if (pLineObj->GetAt(1, entity_type) && pLineObj->GetAt(2, tablename) && pLineObj->GetAt(3, clsid) && pLineObj->GetAt(4, scriptfile)) {
