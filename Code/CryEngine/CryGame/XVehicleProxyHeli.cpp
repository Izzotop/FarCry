...
void CXVehicleProxy::MoveLikeAHelicopter(SOBJECTSTATE* state) {
    ...
    if (state->dodge) {
        ...
        float deltaA = UpdateAltitude(m_Movement, state->fValueAux, absMinAlt,
                                      //            (state->fStickDist>1.0f||state->bodystate==1 ? state->vTargetPos.z : m_MinAltitude),
                                      //// if attacking or dropping - don't go lower tham target
...
float CXVehicleProxy::UpdateAltitude(const Vec3d& moveDir, float desiredAltitude, float absMinAlt, bool useObjects) {
    Vec3d curPos = m_pEntity->GetPos();
...
Vec3d CXVehicleProxy::UpdateThreatHeli(void* threat) {
    ...
    if (physEnt && physEnt->GetStatus(&status)) {
        ...
        if (dir.x == 0 && dir.y == 0 && dir.z == 0) // rocket goes exactly at self pos (probably newer will happen) Jus move somewhere
            dir.x = 1;
        else if (GetLengthSquared(dir) > 120) // too far away
            return Vec3d(0, 0, 0);
...
bool CXVehicleProxy::IsTargetVisible(const Vec3d& selfPos, const Vec3d& targetPos) {
    ...
    ray_hit hit;
    if (!m_pGame->GetSystem()->GetIPhysicalWorld()->RayWorldIntersection(selfPos, targetPos - selfPos, ent_terrain | ent_static | ent_sleeping_rigid, rwi_stop_at_pierceable, &hit,
                                                                         1, m_pEntity->GetPhysics()))
        return true;
    return false;
}
