...
void CPlayer::UpdateBoatCamera() {
    ...
    if (m_bFirstPerson) {
        ...
        if (car) {
            ...
            else { // this is gunner at mounted weapon
            ...
            */
            ...
            //        m_vCurCamposVhcl = vEyePos;
                    ...
                    if(m_walkParams.shakeLElapsedTime < m_walkParams.shakeLTime)
                    {
                            ...
                            m_walkParams.shakeLOffset.x =
            m_walkParams.shakeLAmpl.x*amplScale*Fsin(m_walkParams.shakeLElapsedTime*m_walkParams.shakeLFreq.x*6.283185307179586476925286766559f); m_walkParams.shakeLOffset.y =
            m_walkParams.shakeLAmpl.y*amplScale*Fcos(m_walkParams.shakeLElapsedTime*m_walkParams.shakeLFreq.y*6.283185307179586476925286766559f); m_walkParams.shakeLOffset.z =
            m_walkParams.shakeLAmpl.z*amplScale*Fsin(m_walkParams.shakeLElapsedTime*m_walkParams.shakeLFreq.z*6.283185307179586476925286766559f + 1.13f);
                            ...
                            m_walkParams.shakeAOffset.z = shakeAAmpl.z*amplScale*Fsin(m_walkParams.shakeLElapsedTime*m_walkParams.shakeLFreq.z*6.283185307179586476925286766559f
            + 1.13f);
                    }
                    else
                    {
...
void CPlayer::InitCameraTransition(e_PCM mode, bool OnlyZtransition) {
    ...
    case PCM_ENTERINGVEHICLE:
        ...
        // transmition very fast (immidiate)
        {
            ...
            ray_hit RayHit;
            if (m_pGame->GetSystem()->GetIPhysicalWorld()->RayWorldIntersection(vEyeDestPos, m_vEyePos - vEyeDestPos, ent_all, 0, &RayHit, 1, GetEntity()->GetPhysics()))
                m_fCameraTime = 0.1f;
        }
    ...
    case PCM_LEAVINVEHICLE: {
        // filippo:when leave the vehicle store the delta from car pos and eye pos; because if we are changing sit position we need to know the last eye position for a smooth view
        // transition.
        if (m_pVehicle) {
            m_vDeltaEyeVehicle = m_pVehicle->GetEntity()->GetPos() - m_vCurCamposVhcl;
...
void CPlayer::UpdateAutoCenter() {
    ...
    float trh = .10f;
    if (curAngl.x < trh && curAngl.x > -trh && curAngl.y < trh && curAngl.y > -trh &&
        (m_AutoCenter == 2 && curAngl.z < trh && curAngl.z > -trh || m_AutoCenter == 1 && curAngl.z > 180.0f - trh && curAngl.z < -180.0f + trh)) {
        curAngl.Set(0, 0, 0);
...
CPlayer* CVehicle::GetUserInState(CPlayer::eInVehiclestate state) {
    ...
    for (curUser = m_UsersList.begin(); curUser != m_UsersList.end(); ++curUser) {
        ...
        if (pCurUserEntity && pCurUserEntity->GetContainer() && pCurUserEntity->GetContainer()->QueryContainerInterface(CIT_IPLAYER, (void**)&pCurUserPlayer) &&
...
void CVehicle::SetWeaponUser(int entId) {
    ...
    m_WeaponUser = entId;
    IEntity* pCurUserEntity = m_pGame->GetSystem()->GetIEntitySystem()->GetEntity(m_WeaponUser);
    if (pCurUserEntity && pCurUserEntity->GetContainer() && pCurUserEntity->GetContainer()->QueryContainerInterface(CIT_IPLAYER, (void**)&shooter)) {
...
void CVehicle::UpdateWeaponPosAngl() {
    ...
    if (!shooter) {
        ...
        float trh = 1.0f;
        if (vDiff.x < trh && vDiff.x > -trh && vDiff.y < trh && vDiff.y > -trh && vDiff.z < trh && vDiff.z > -trh)
    ...
    if (bAutoAim && m_bCrossOnScreen) {
        ...
        while (pEnt = It->Next()) {
            ...
            if ((Position.x >= m_vCrossScreen.x - autoaimWndSize) && (Position.y >= m_vCrossScreen.y - autoaimWndSize) && (Position.x <= m_vCrossScreen.x + autoaimWndSize) &&
                (Position.y <= m_vCrossScreen.y + autoaimWndSize) && (Position.z > 0.0f)) {
                ...
                if (dist < minDist) {
                    ...
                    if (m_pGame->GetSystem()->GetIPhysicalWorld()->RayWorldIntersection(vectorf(m_vWpnPos + offset), diff, ent_terrain | ent_static, 0, &RayHit, 1, pPE,
                                                                                        GetEntity()->GetPhysics()))
                        continue;
                    minDist = dist;
                    bestPoint3D = Center;
    ...
    if (m_bTargetIsLocked)
        ...
        ray_hit hits[1];
        int hit = m_pGame->GetSystem()->GetIPhysicalWorld()->RayWorldIntersection(shooterPos + offset, dir, ent_all & ~ent_living, rwi_stop_at_pierceable, hits, 1,
                                                                                  GetEntity()->GetPhysics(), shooter->GetEntity()->GetPhysics());
        if (hit == 0)
    ...
    // check vertical limits
    {
        ...
        angleX = ClampAngle(Snap_s360(m_AngleLimitBase.x + m_MinVAngle), Snap_s360(m_AngleLimitBase.x + m_MaxVAngle), Snap_s360(angleX));
    ...
    if (m_AngleLimitHFlag) {
        ...
        angleZ = ClampAngle(Snap_s360(m_AngleLimitBase.z + m_MinHAngle), Snap_s360(m_AngleLimitBase.z + m_MaxHAngle), Snap_s360(angleZ));
...
bool CVehicle::AnglesToLimit(Vec3& angl) {
    ...
    // check vertical limits
    {
        ...
        angl.x = ClampAngle(Snap_s360(m_AngleLimitBase.x + m_MinVAngle), Snap_s360(m_AngleLimitBase.x + m_MaxVAngle), Snap_s360(angl.x));
    ...
    if (m_AngleLimitHFlag) {
        ...
        angl.z = ClampAngle(Snap_s360(m_AngleLimitBase.z + m_MinHAngle), Snap_s360(m_AngleLimitBase.z + m_MaxHAngle), Snap_s360(angl.z));
