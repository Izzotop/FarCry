...
void CXGame::DrawRadar(float x, float y, float w, float h, float fRange, INT_PTR* pRadarTextures, _SmartScriptObject* pEntities, char* pRadarObjective) {
    ...
    // radar texture id's
    INT_PTR iRadarID = pRadarTextures[0], iRadarMaskID = pRadarTextures[1], iPlayerIconID = pRadarTextures[2], iEnemyInRangeIconID = pRadarTextures[3],
            iEnemyOutRangeIconID = pRadarTextures[4], iSoundIconID = pRadarTextures[5], iObjectiveIconID = pRadarTextures[6];
    ...
    m_pRenderer->Draw2dImage(fMapCenter.x - fPlayerSize * 0.5f, fMapCenter.y - fPlayerSize * 0.5f, fPlayerSize, fPlayerSize, iPlayerIconID, fTexOffset, 1 - fTexOffset,
                             1 - fTexOffset, fTexOffset, 0, fPlayerAlpha * fFadeAmount, fPlayerAlpha * fFadeAmount, fPlayerAlpha * fFadeAmount, fPlayerAlpha);
    ...
    if (pClient) //&& fLastAlpha>0.0f)
    {
        ...
        for (TSoundListIt It = SoundList.begin(); It != SoundList.end(); ++It) {
            ...
            float fCurrDist = cry_sqrtf((pPlayerPos.x - pSoundPos.x) * (pPlayerPos.x - pSoundPos.x) + (pPlayerPos.y - pSoundPos.y) * (pPlayerPos.y - pSoundPos.y) +
                                        (pPlayerPos.z - pSoundPos.z) * (pPlayerPos.z - pSoundPos.z));
            ...
            if (fCurrDist > fRange) {
                ...
                m_pRenderer->Draw2dImage(fMapCenter.x + pPos.x * fCurrDist - fSoundIconSize * 0.5f, fMapCenter.y + pPos.y * fCurrDist - fSoundIconSize * 0.5f, fSoundIconSize,
                                         fSoundIconSize, iSoundIconID, 0, 0, 1, 1, 0, fFadeAmount, fFadeAmount, fFadeAmount, fPhase);
            } else {
                fCurrDist /= 15.0f;
                if (fCurrDist < 1.2f && fCurrDist > 0.0f) {
                    fCurrDist = 1.2f;
                } else if (fCurrDist == 0.0f) {
                    fCurrDist = 1.0f;
                }
    ...
    if (iHaveObjective) {
        ...
        if (fCurrDist > fRange) {
            ...
            m_pRenderer->Draw2dImage(fMapCenter.x + fPosX * 52.0f - 10.0f * 0.5f, fMapCenter.y + fPosY * 52.0f * fScaleY - 10.0f * 0.5f, 10.0f, 10.0f, iEnemyOutRangeIconID, 0, 0,
                                     1, 1, fOutRangeAngle, r * fFadeAmount, g * fFadeAmount, b * fFadeAmount, a * 0.5f);
            ...
            m_pRenderer->Draw2dImage(fMapCenter.x + (pObjectiveScreenPos.x / fRange) - 10.0f * fCurrSize * 0.5f,
                                     fMapCenter.y + (pObjectiveScreenPos.y / fRange) - 10.0f * fCurrSize * 0.5f, 10.0f * fCurrSize, 10.0f * fCurrSize, iObjectiveIconID, 0, 0, 1, 1,
                                     0.0f, r * a * fFadeAmount, g * a * fFadeAmount, b * a * fFadeAmount, a);
        }
    }
    ...
    while ((*pEntities)->MoveNext()) {
        ...
        if (fAngleZ < -360.0f) {
            fAngleZ += 360;
        } else if (fAngleZ > 360.0f) {
            fAngleZ -= 360;
        }

        float fEnemyInRangeSize = 10.0f, fEnemyOutRangeSize = 5.0f;
        ...
        if (fCurrDist > fRange) {
            ...
            m_pRenderer->Draw2dImage(fMapCenter.x + (Pos.x * 0.98f / fRange) - fEnemyInRangeSize * 0.5f * fCurrDist,
                                     fMapCenter.y + (Pos.y * 0.98f / fRange) - fEnemyInRangeSize * 0.5f * fCurrDist, fEnemyInRangeSize * fCurrDist, fEnemyInRangeSize * fCurrDist,
                                     iEnemyInRangeIconID, 0, 0, 1, 1, fAngleZ + 180.0f, r * a * fFadeAmount, g * a * fFadeAmount, b * a * fFadeAmount, a);
        }
    }
