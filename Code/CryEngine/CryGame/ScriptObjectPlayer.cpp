...
IScriptObject* CScriptObjectPlayer::m_pTempObj = nullptr;
IScriptObject* CScriptObjectPlayer::m_pTempAng = nullptr;
IScriptObject* CScriptObjectPlayer::m_pWeaponSlots = nullptr;
IScriptObject* CScriptObjectPlayer::m_pTempBloodObj = nullptr;
IScriptObject* CScriptObjectPlayer::m_pBlindScreenPos = nullptr;
...
int CScriptObjectPlayer::GetWeaponsSlots(IFunctionHandler* pH) {
    ...
    for (int n = 0; n < 4; n++) {
        if (m_pPlayer->m_vWeaponSlots[n]) {
            ...
            if (pWC && pWC->GetScriptObject())
                m_pWeaponSlots->PushBack(pWC->GetScriptObject());
            else
                m_pWeaponSlots->PushBack(0);
        } else
            m_pWeaponSlots->PushBack(0);
    }
...
int CScriptObjectPlayer::GetViewIntersection(IFunctionHandler* pH) {
    ...
    ray_hit hit;
    float fDist = -1.0;
    if (m_pPlayer->GetGame()->GetSystem()->GetIPhysicalWorld()->RayWorldIntersection(vectorf(pos), vectorf(dir), ent_all, 13 /*pirceability*/, &hit, 1,
                                                                                     m_pPlayer->GetEntity()->GetPhysics())) {
        ...
        m_pTempObj->SetTonullptrChain("id");
        m_pTempObj->SetTonullptrChain("ent");
...
int CScriptObjectPlayer::StartDie(IFunctionHandler* pH) {
    ...
    if (!pH->GetParam(1, *pTable)) {
        CryError("CScriptObjectPlayer::StartDie parameter 1 failed");
        return pH->EndFunction();
    }
    ...
    if (!pH->GetParam(2, *pTable)) {
        CryError("CScriptObjectPlayer::StartDie parameter 2 failed");
        return pH->EndFunction();
    }
...
int CScriptObjectPlayer::SetDynamicsProperties(IFunctionHandler* pH) {
    ...
    if (pTable->GetValue("push_players", bPushPlayers) && pTable->GetValue("pushable_by_players", bPushableByPlayers)) {
        pe_params_flags ppf;
...
int CScriptObjectPlayer::GetTPVHelper(IFunctionHandler* pH) {
    ...
    } else if (m_pPlayer->m_pMountedWeapon) // if at mounted weapon - get position of bone from weapon character
    {
...
int CScriptObjectPlayer::GetFirePosAngles(IFunctionHandler* pH) {
    ...
    Vec3 tpos, tang;
    if (!pH->GetParam(1, pPos))
        return pH->EndFunctionnullptr();
    if (!pH->GetParam(2, pAng))
        return pH->EndFunctionnullptr();
...
//    fd.type
                ...
                if (centycontact && (!centycontact->IsGarbage()))
                        ...
                        m_pTempBloodObj->SetTonullptr("target_id");
...
int CScriptObjectPlayer::GetProjectedBloodPos(IFunctionHandler* pH) {
    ...
    if (m_pPlayer->GetGame()->GetSystem()->GetIPhysicalWorld()->RayWorldIntersection(
            vectorf(pos), vectorf(dir * fDist), dwObjTypes,
        ...
        if (dir.x == 0 && dir.y == 0) {
            for (float x = -1.f; x <= 1.f; x += 2.f)
                for (float y = -1.f; y <= 1.f; y += 2.f) {
                    ray_hit hitAux;
                    if (m_pPlayer->GetGame()->GetSystem()->GetIPhysicalWorld()->RayWorldIntersection(
                            vectorf(pos) + vectorf(Decal.fSize * x, Decal.fSize * y, 0), vectorf(dir * fDist), dwObjTypes, 0, &hitAux, 1, m_pPlayer->GetEntity()->GetPhysics()) &&
                        hitAux.pCollider != hit.pCollider && hitAux.n.Dot(dir) < 0) {
                        pe_params_foreign_data fd;
