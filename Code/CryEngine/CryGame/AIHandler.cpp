...
CAIHandler::CAIHandler()
    : m_pEntity(nullptr), m_pCharacter(nullptr), m_pDefaultCharacter(nullptr), m_pBehavior(nullptr), m_pPreviousBehavior(nullptr), m_pDefaultBehavior(nullptr), m_pDEFAULTDefaultBehavior(nullptr),
      m_pBehaviorTable(nullptr), m_pBehaviorTableAVAILABLE(nullptr), m_pBehaviorTableINTERNAL(nullptr), m_DamageGrenadeType(-1) {
    m_pLog = nullptr;
}
...

                {
                    if (m_pLog->GetVerbosityLevel()) {
                        ...
                        if (m_pEntity && m_pEntity->GetName() && behaviorName && nextBehaviorName && signalText)
                            m_pLog->LogToConsole("\004 entity %s changin behavior from %s to %s on signal %s", m_pEntity->GetName(), behaviorName, nextBehaviorName, signalText);
                    }
                }
        ...
        if (pCharacterTable->GetValue(signalText, nextBehaviorName)) {
            m_NextBehaviorName = nextBehaviorName;
            {
                if (m_pLog->GetVerbosityLevel()) {
                    ...
                    if (m_pEntity && m_pEntity->GetName() && behaviorName && nextBehaviorName && signalText)
                        m_pLog->Log("\004 entity %s changin behavior from %s to %s on signal %s [DEFAULT character]", m_pEntity->GetName(), behaviorName, nextBehaviorName,
                                    signalText);
                }
            }
            return true;
...
void CAIHandler::DoChangeBehavior() {
    ...
    //    !strcmp(nextBehaviorName, "PREVIOUS") )
    {
        ...
        if (!m_pBehaviorTable->GetValue(nextBehaviorName, pNextBehavior)) {
            ...
            if (m_pEntity && m_pEntity->GetName() && m_NextBehaviorName.c_str()) {
                m_pLog->LogToFile("\004 entity %s faild to change behavior to %s.", m_pEntity->GetName(), m_NextBehaviorName.c_str());
    ...
    if (pNextBehavior) {
        if (m_pBehavior) {
            ...
            m_pLog->Log("\001 [AIERROR] entity %s had 0 behaviour but behaviour name %s", m_pEntity->GetName(), m_CurrentBehaviorName.c_str());
        }
