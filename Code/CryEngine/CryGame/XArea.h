...
public:
    ...
    int GetGroup() const {
        return m_AreaGroupID;
    }

    void SetAreaType(const tAreaType type) {
        m_AreaType = type;
    }
    tAreaType GetAreaType() const {
        return m_AreaType;
    }
...
public:
    ...
    IXArea* CreateArea(const Vec3d* vPoints, const int count, const std::vector<string>& names, const int type, const int groupId, const float width = 0.0f,
                       const float height = 0.0f);
    IXArea* CreateArea(const Vec3d& min, const Vec3d& max, const Matrix44& TM, const std::vector<string>& names, const int type, const int groupId, const float width);
    IXArea* CreateArea(const Vec3d& center, const float radius, const std::vector<string>& names, const int type, const int groupId, const float width);
