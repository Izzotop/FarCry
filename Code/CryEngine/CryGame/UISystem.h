...
#define REGISTER_SCRIPTOBJECT_MEMBER(scriptsystem, classname, functionname)                                                                                                        \
    { classname::RegisterFunction(scriptsystem, #functionname, &classname::functionname); }
#define REGISTER_SCRIPT_CONSTANT(scriptsystem, constant) (scriptsystem)->SetGlobalValue(#constant, constant)
#define GET_SCRIPT_TYPE_STRING(itype)                                                                                                                                              \
    (itype == svtObject                                                                                                                                                            \
         ? "Object"                                                                                                                                                                \
         : (itype == svtString                                                                                                                                                     \
                ? "String"                                                                                                                                                         \
                : (itype == svtNumber ? "Number" : (itype == svtFunction ? "Function" : (itype == svtUserData ? "UserData" : (itype == svtnullptr ? "nullptr" : "Unknown"))))))
#define CHECK_SCRIPT_FUNCTION_PARAMCOUNT(scriptsystem, objectname, funcname, nparameters)                                                                                          \
    {                                                                                                                                                                              \
        if (pH->GetParamCount() != nparameters) {                                                                                                                                  \
            scriptsystem->RaiseError("%s:%s() Wrong number of parameters! Expected %d, but found %d!", objectname, #funcname, nparameters, pH->GetParamCount());                   \
            return pH->EndFunctionnullptr();                                                                                                                                          \
        }                                                                                                                                                                          \
    }
#define CHECK_SCRIPT_FUNCTION_PARAMCOUNT2(scriptsystem, objectname, funcname, nparameters, nparameters2)                                                                           \
    {                                                                                                                                                                              \
        if ((pH->GetParamCount() != nparameters) && (pH->GetParamCount() != nparameters2)) {                                                                                       \
            scriptsystem->RaiseError("%s:%s() Wrong number of parameters! Expected %d or %d, but found %d!", objectname, #funcname, nparameters, nparameters2,                     \
                                     pH->GetParamCount());                                                                                                                         \
            return pH->EndFunctionnullptr();                                                                                                                                          \
        }                                                                                                                                                                          \
    }
#define CHECK_SCRIPT_FUNCTION_PARAMTYPE(scriptsystem, objectname, funcname, idparam, type)                                                                                         \
    {                                                                                                                                                                              \
        if (pH->GetParamType(idparam) != type) {                                                                                                                                   \
            scriptsystem->RaiseError("%s:%s() Wrong type in parameter %d! Expected '%s', but found '%s'!", objectname, #funcname, idparam, GET_SCRIPT_TYPE_STRING(type),           \
                                     GET_SCRIPT_TYPE_STRING(pH->GetParamType(idparam)));                                                                                           \
            return pH->EndFunctionnullptr();                                                                                                                                          \
        }                                                                                                                                                                          \
    }

#define CHECK_SCRIPT_FUNCTION_PARAMTYPE2(scriptsystem, objectname, funcname, idparam, type, type2)                                                                                 \
    {                                                                                                                                                                              \
        if ((pH->GetParamType(idparam) != type) && (pH->GetParamType(idparam) != type2)) {                                                                                         \
            scriptsystem->RaiseError("%s:%s() Wrong type in parameter %d! Expected '%s' or '%s', but found '%s'!", objectname, #funcname, idparam, GET_SCRIPT_TYPE_STRING(type),   \
                                     GET_SCRIPT_TYPE_STRING(type2), GET_SCRIPT_TYPE_STRING(pH->GetParamType(idparam)));                                                            \
            return pH->EndFunctionnullptr();                                                                                                                                          \
        }                                                                                                                                                                          \
    }

#define CHECK_SCRIPT_FUNCTION_PARAMTYPE3(scriptsystem, objectname, funcname, idparam, type, type2, type3)                                                                          \
    {                                                                                                                                                                              \
        if ((pH->GetParamType(idparam) != type) && (pH->GetParamType(idparam) != type2) && (pH->GetParamType(idparam) != type3)) {                                                 \
            scriptsystem->RaiseError("%s:%s() Wrong type in parameter %d! Expected '%s', '%s' or '%s', but found '%s'!", objectname, #funcname, idparam,                           \
                                     GET_SCRIPT_TYPE_STRING(type), GET_SCRIPT_TYPE_STRING(type2), GET_SCRIPT_TYPE_STRING(type3),                                                   \
                                     GET_SCRIPT_TYPE_STRING(pH->GetParamType(idparam)));                                                                                           \
            return pH->EndFunctionnullptr();                                                                                                                                          \
        }                                                                                                                                                                          \
    }
...
#define RETURN_TEXTURE_FROM_SCRIPT(scriptsystem, objectname, functionname, varname)                                                                                                \
    ...
    CHECK_SCRIPT_FUNCTION_PARAMTYPE2(scriptsystem, objectname, functionname, 1, svtUserData, svtnullptr);                                                                             \
    int iCookie = 0;                                                                                                                                                               \
...
#define RETURN_COLOR_FROM_SCRIPT(scriptsystem, objectname, functionname, varname)                                                                                                  \
    ...
    if ((pH->GetParamCount() == 1) && (pH->GetParamType(1) == svtString)) {                                                                                                        \
        ...
        m_pUISystem->GetIScriptSystem()->RaiseError("%s:%s() Wrong type in parameter 1! Expected 'String' or 'Number', but found '%s'!", objectname, #functionname,                \
                                                    GET_SCRIPT_TYPE_STRING(pH->GetParamType(1)));                                                                                  \
        return pH->EndFunctionnullptr();                                                                                                                                              \
    }                                                                                                                                                                              \
    return pH->EndFunctionnullptr()
...
public:
    ...
    //-------------------------------------------------------------------------------------------------
    CScriptObjectUI* GetScriptObjectUI() {
        return m_pScriptObjectUI;
    };
    ...
    int DrawMultiLineText(const UIRect& pRect, int iHAlignment, int iVAlignment, IFFont* pFont, const std::vector<wstring>& vLines, float fHorizontalOffset = 0.0f,
    ...
    int ShouldSortTabStop() {
        return m_bSortTabStop;
    };
