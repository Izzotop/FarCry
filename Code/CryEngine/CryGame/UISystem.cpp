...
CUISystem::CUISystem() : m_pGame(0), m_pSystem(0), m_pScriptSystem(0), m_pRenderer(0), m_pInput(0) {
    Reset();
}
...
LRESULT CUISystem::DefaultUpdate(CUIWidget* pWidget, unsigned int iMessage, WPARAM wParam, LPARAM lParam) {
    ...
    case UIM_MOUSEMOVE: {
        if (pWidget->m_bMoving) {
            ...
            return (int)pWidget->m_pUISystem->SendMessage(pWidget, UIM_MOVE, UIM_FLOAT_TO_DWORD(pWidget->m_pRect.fLeft + (fNewX - fOldX)),
                                                          UIM_FLOAT_TO_DWORD(pWidget->m_pRect.fTop + (fNewY - fOldY)));
        }
    } break;
    default: {
    } break;
...
int CUISystem::IntersectRect(UIRect* pNewRect, const UIRect pRect1, const UIRect pRect2) {
    ...
    if ((pRect2.fLeft > pRect1.fLeft + pRect1.fWidth) || (pRect1.fLeft > pRect2.fLeft + pRect2.fWidth) || (pRect2.fTop > pRect1.fTop + pRect1.fHeight) ||
        (pRect1.fTop > pRect2.fTop + pRect2.fHeight)) {
...
CUIWidget* CUISystem::FindWidgetAt(float fX, float fY) {
    ...
    for (pItor = m_pChildList.begin(); pItor != m_pChildList.end(); ++pItor) {
        ...
        if (((pWidget->m_iFlags & UIFLAG_VISIBLE) == 0) || ((pWidget->m_iFlags & UIFLAG_ENABLED) == 0) || (pWidget->m_pScreen && (!IsScreenActive(pWidget->m_pScreen)))) {
            continue;
        }
...
int CUISystem::SetScissor(const UIRect* pRect) {
    if (!pRect) {
        if (m_bDrawing) {
            ...
            m_pCurrentDrawFont->SetClippingRect(x, y, x + (float)floor(m_fVirtualToRealX * m_pCurrentDrawRect.fWidth),
    ...
    if (m_pCurrentDrawFont) {
        ...
        m_pCurrentDrawFont->EnableClipping(1);
        m_pCurrentDrawFont->SetClippingRect(x, y, x + (float)floor(m_fVirtualToRealX * m_pScissorRect.fWidth), y + (float)floor(m_fVirtualToRealY * m_pScissorRect.fHeight));
    }
...
int CUISystem::DrawImage(const UIRect& pRect, int iTextureID, const float* vTexCoord, const color4f& cColor) {
    ...
    if (iTextureID > -1) {
        ...
        m_pRenderer->Draw2dImage(AdjustWidth(fX), AdjustHeight(fY), AdjustWidth(fR - fX), AdjustHeight(fB - fY), iTextureID, vClippedTexCoord[0], vClippedTexCoord[1],
                                 vClippedTexCoord[2], vClippedTexCoord[3], 0, cColor.v[0], cColor.v[1], cColor.v[2], cColor.v[3], 0);
    } else {
        m_pRenderer->Draw2dImage(AdjustWidth(fX), AdjustHeight(fY), AdjustWidth(fR - fX), AdjustHeight(fB - fY), -1, 0, 0, 0, 0, 0, cColor.v[0], cColor.v[1], cColor.v[2],
                                 cColor.v[3], 0);
    }
...
int CUISystem::RetrieveCommonAttribute(IScriptObject* pObject, CUIWidget* pWidget) {
    ...
    // font attributes
    if ((pObject->GetCurrentType() == svtObject) || (CHECKATTRIBUTE("classname", svtString)) || IsReserved(szAttributeName)) {
        return 1;
...
int CUISystem::CreateObjectFromTable(CUIWidget** pWidget, CUIWidget* pParent, CUIScreen* pScreen, IScriptObject* pObject, const string& szName) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        } else if ((pObj->GetValueType("left") == svtNumber) || (pObj->GetValueType("top") == svtNumber) || (pObj->GetValueType("width") == svtNumber) ||
...
bool CUISystem::IsReserved(const char* szName) {
    if ((strcmp(szName, "user") == 0) || (strcmp(szName, "skin") == 0)) {
        return 1;
    }
    return 0;
}
