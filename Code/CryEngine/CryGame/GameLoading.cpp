...
struct PropertyWriter : IScriptObjectDumpSink {
    ...
    void Element(const char* sName, int nIdx, ScriptVarType type, bool iskey) {
        ...
        case svtUserData:
        case svtFunction:
            TRACE("WARNING: userdata or function found in properties table (%s)", sName);
        case svtnullptr:
            break;
        case svtString: {
            const char* s = "";
            _VERIFY(iskey ? table->GetValue(sName, s) : table->GetAt(nIdx, s));
            stm.Write(s);
        }; break;
        case svtNumber: {
            float f = 0;
            _VERIFY(iskey ? table->GetValue(sName, f) : table->GetAt(nIdx, f));
            stm.Write(f);
        }; break;
        case svtObject: {
            _SmartScriptObject t(m_pScriptSystem, true);
            _VERIFY(iskey ? table->GetValue(sName, t) : table->GetAt(nIdx, t));
            // todo (Izzotop): Taking the address of a temporary object of type 'PropertyWriter'
...
void LoadProperties(IScriptObject* table, CStream& stm, IScriptSystem* ss, char* parent) {
    ...
    for (;;) {
        ...
        switch (what) {
        case svtnullptr: {
            iskey ? table->SetTonullptr(key.c_str()) : table->SetnullptrAt(idx);
            break;
        };
        case svtString: {
            string s;
            stm.Read(s);
            iskey ? table->SetValue(key.c_str(), s.c_str()) : table->SetAt(idx, s.c_str());
            break;
        };
        case svtNumber: {
            float f = 0;
            stm.Read(f);
            iskey ? table->SetValue(key.c_str(), f) : table->SetAt(idx, f);
            break;
        };
        case svtObject: {
            _SmartScriptObject t(ss);
            iskey ? table->SetValue(key.c_str(), *t) : table->SetAt(idx, *t);
            LoadProperties(t, stm, ss, (char*)key.c_str());
            break;
        };
        case svtUserData:
        case svtFunction:
            TRACE("WARNING: can't restore userdata or function in properties table (%s.%s)", parent, key.c_str());
        };
    };
};
...
bool CXGame::SaveToStream(CStream& stm, Vec3d* pos, Vec3d* angles, string sFilename) {
    ...
    if (!pPlayerEnt)
        ...
        CryError(
            "A checkpoint has been triggered to save data when the player is not existing yet, generally right after respawning. \n This is a data error, and must be fixed by the "
            "designer working on this map. \n Possible data errors are: \n - the first respawn point might be inside a checkpoint \n - something else other than shapes is used to "
    ...
    if (pPlayerEnt->GetContainer())
        pPlayerEnt->GetContainer()->QueryContainerInterface(CIT_IPLAYER, (void**)&pPlayer);
    // ASSERT(pPlayer);
    if (!pPlayer)
    ...
    */
    ...
    while ((pEnt = pEntities->Next()) != nullptr) {
        ...
        if (pPlayer && pPlayer->m_stats.health <= 0)
            continue;
        ...
        if (so->GetValue("Properties", props)) {
            // todo (Izzotop): Taking the address of a temporary object of type 'PropertyWriter'
            props->Dump(&PropertyWriter(props, stm, m_pScriptSystem));
        }
        stm.Write((char)TABLE_END);
        ...
        if (so->GetValue("PropertiesInstance", propsi)) {
            // todo (Izzotop): Taking the address of a temporary object of type 'PropertyWriter'
            propsi->Dump(&PropertyWriter(propsi, stm, m_pScriptSystem));
        }
        stm.Write((char)TABLE_END);
        ...
        if (so->GetValue("Events", events)) {
            // todo (Izzotop): Taking the address of a temporary object of type 'PropertyWriter'
            events->Dump(&PropertyWriter(events, stm, m_pScriptSystem));
        }
        stm.Write((char)TABLE_END);
...
bool CXGame::LoadFromStream(CStream& stm, bool isdemo) {
    ...
    if (stricmp(g_LevelName->GetString(), sLevelName.c_str()) != 0 || !m_pServer || (stricmp(m_pServer->m_GameContext.strMission.c_str(), sMissionName.c_str()) != 0)) {
        ...
        while ((pEnt = pEntities->Next()) != nullptr) {
            EntityClass* pClass = pECR->GetByClass(pEnt->GetEntityClassName());
            if (m_pWeaponSystemEx->IsProjectileClass(pClass->ClassId))
                continue;
#ifdef _DEBUG
...
void CXGame::LoadConfiguration(const string& sSystemCfg, const string& sGameCfg) {
    ...
    while (fgets(szLine, 512, pFile)) {
        ...
        if (strstr(szLine, "#")) {
            ...
            // valid command
            bValid = true;
        } else if (strstr(szLine, "Input:SetMouseSensitivity")) {
            // valid command
            bValid = true;
        } else if (strstr(szLine, "Input:SetInvertedMouse")) {
            // valid command
            bValid = true;
