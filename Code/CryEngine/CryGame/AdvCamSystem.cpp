...
void CAdvCamSystem::ProcessKeys(CXEntityProcessingCmd& epc) {
    if (m_eiPlayerA && m_eiPlayerB) {
        // get entities
        IEntitySystem* pEntitySystem = m_pGame->m_pSystem->GetIEntitySystem();
        assert(pEntitySystem);
        IEntity* pEntityA = pEntitySystem->GetEntity(m_eiPlayerA);
        assert(pEntityA);
        if (!pEntityA)
            return;
        IEntity* pEntityB = pEntitySystem->GetEntity(m_eiPlayerB);
        assert(pEntityB);
        if (!pEntityB)
            return;
        ...
        pEntityA->GetContainer()->QueryContainerInterface(CIT_IPLAYER, (void**)&pPlayerA);
        assert(pPlayerA);
        CPlayer* pPlayerB = nullptr;
        pEntityB->GetContainer()->QueryContainerInterface(CIT_IPLAYER, (void**)&pPlayerB);
        assert(pPlayerB);
...
bool CAdvCamSystem::Write(CStream& stm, EntityCloneState* cs) {
    if (!stm.Write(m_eiPlayerA))
        return false;
    if (!stm.Write(m_eiPlayerB))
        return false;
...
bool CAdvCamSystem::Read(CStream& stm) {
    if (!stm.Read(m_eiPlayerA))
        return false;
    if (!stm.Read(m_eiPlayerB))
        return false;
...
#define GETVAL(Name)                                                                                                                                                               \
    { scriptObject->GetValue(#Name, Name); }
#define GETVEC(Name)                                                                                                                                                               \
    {                                                                                                                                                                              \
        if (scriptObject->GetValue(#Name, *oVec))                                                                                                                                  \
            Name = oVec.Get();                                                                                                                                                     \
    }
    ...
    baseA_m.SetIdentity();
    baseB_m.SetIdentity();
    baseAB_m.SetIdentity();
    baseAB_fac.SetIdentity();

    if (m_eiPlayerA) {
        IEntitySystem* pEntitySystem = m_pGame->m_pSystem->GetIEntitySystem();
        assert(pEntitySystem);
        IEntity* pEntity = pEntitySystem->GetEntity(m_eiPlayerA);
        assert(pEntity);
        if (!pEntity)
            return;
    ...
    if (m_eiPlayerB) {
        IEntitySystem* pEntitySystem = m_pGame->m_pSystem->GetIEntitySystem();
        assert(pEntitySystem);
        IEntity* pEntity = pEntitySystem->GetEntity(m_eiPlayerB);
        assert(pEntity);
        if (!pEntity)
            return;
