...
struct PlayerDynamics {
    PlayerDynamics() {
        gravity = swimming_gravity = inertia = swimming_inertia = air_control = jump_gravity = 1E10f;
    }
...
public:
    ...
    void SetColor(const Vec3& invColor) {
        m_vColor = invColor;
    }
    ...
    */
    void SetSwayAmp(float fAmp) {
        m_walkParams.swayAmp = fAmp;
    }

    /*! Sets the swaying-frequency
                    @param fFreq frequency
    */
    void SetSwayFreq(float fFreq) {
        m_walkParams.swayFreq = fFreq;
    }
    ...
    */
    void SetAngleLimitBaseOnCamera() {
        m_AngleLimitBase = m_pEntity->GetAngles();
    }
    /*! Limits the vertical player angles - min value
                    @param fLimit - min angle value
    */
    void SetMinAngleLimitV(float const fLimit) {
        m_MinVAngle = fLimit;
    }
    /*! Limits the vertical player angles - max value
                    @param fLimit - max angle value
    */
    void SetMaxAngleLimitV(float const fLimit) {
        m_MaxVAngle = fLimit;
    }
    /*! Enables vertical player angles restriction
                    @param enabled
    */
    void EnableAngleLimitV(bool const enabled) {
        m_AngleLimitVFlag = enabled;
    }
    /*! Limits the horizontal player angles - min value
                    @param fLimit - min angle value
    */
    void SetMinAngleLimitH(float const fLimit) {
        m_MinHAngle = fLimit;
    }
    /*! Limits the horizontal player angles - max value
                    @param fLimit - max angle value
    */
    void SetMaxAngleLimitH(float const fLimit) {
        m_MaxHAngle = fLimit;
    }
    /*! Enables horizontal player angles restriction
                    @param enabled
    */
    void EnableAngleLimitH(bool const enabled) {
        m_AngleLimitHFlag = enabled;
    }

    bool IsAngleLimitVOn() const {
        return m_AngleLimitVFlag;
    }
    bool IsAngleLimitHOn() const {
        return m_AngleLimitHFlag;
    }
    ...
    virtual void OnEntityNetworkUpdate(const EntityId& idViewerEntity, const Vec3d& v3dViewer, uint32& inoutPriority, EntityCloneState& inoutCloneState) const;
    ...
    */
    void SetShake(const Vec3& shakeAxis, float shakeDegree, float shakeFreq, float shakeTime) {
        m_walkParams.shakeAxis = shakeAxis;
        m_walkParams.shakeDegree = shakeDegree;
        m_walkParams.shakeFreq = shakeFreq;
        m_walkParams.shakeTime = shakeTime;
        m_walkParams.shakeOffset = 0.0f;
        m_walkParams.shakeElapsedTime = 0.0f;
    }
    void SetShakeL(const Vec3& shakeAmpl, const Vec3& shakeFreq, const float shakeTime) {
        m_walkParams.shakeLAmpl = shakeAmpl;
        m_walkParams.shakeLFreq = shakeFreq;
        m_walkParams.shakeLTime = shakeTime;
        m_walkParams.shakeLElapsedTime = 0.0f;
    }
    ...
    int CountAvaliableWeapons() {
        int count = 0;
        for (int n = 0; n < PLAYER_MAX_WEAPONS; n++) {
            if (m_vWeaponSlots[n] != 0)
                count++;
        }
        return count;
    }
    ...
    /*! Retrieves the id of the currently selected weapon
                    @return weapon-id
    */
    int GetSelectedWeaponId() {
        return m_nSelectedWeaponID;
    }
    ...
    */
    void SetGravityOverride(float fNewGravity) {
        m_fGravityOverride = fNewGravity;
    }
...
protected:
    ...
    //! Player dimention
    enum ePlayerDimension { eDimNone, eDimNormal, eDimStealth, eDimCrouch, eDimProne };
...
public:
    ...
    /*! Retrieves the current walk-stats
                    @return walk-stats
    */
    CPlayer::SWalkParams GetWalkParams() {
        return m_walkParams;
    }
...
public:
    ...
    bool GetWaitForFireRelease() const {
        return m_bWaitForFireRelease;
    }
    void SetWaitForFireRelease(bool bVal) {
        m_bWaitForFireRelease = bVal;
    }
