...
void CPlayer::UpdateLean() {
    ...
    if (IsMyPlayer() && (IsLeaning() || m_walkParams.fCurrLean != 0)) {
        ...
        if (goesInside == 0) { // check if there is something in front of me
    ...
    if (/*IsLeaning( )*/ (m_walkParams.fLeanTarget - m_walkParams.fCurrLean != 0) && IsMyPlayer()) {
        ...
        if (m_walkParams.fLeanTarget == 0) {
            m_walkParams.fCurrLean += m_walkParams.fCurrLean < 0 ? min(m_pTimer->GetFrameTime() * (fMul * m_walkParams.leanSpeed * 3), fDiff * fMul)
                                                                 : max(m_pTimer->GetFrameTime() * (fMul * m_walkParams.leanSpeed * 3), fDiff * fMul);
        } else {
            m_walkParams.fCurrLean += m_walkParams.fLeanTarget > 0 ? min(m_pTimer->GetFrameTime() * (fMul * m_walkParams.leanSpeed * 3), fDiff * fMul)
                                                                   : max(m_pTimer->GetFrameTime() * (fMul * m_walkParams.leanSpeed * 3), fDiff * fMul);
        }
    ...
    //  if (IsAlive())
    {
        ...
        if (m_bFirstPerson && m_pVehicle) {
            ...
            if (m_pVehicle && m_pVehicle->GetEntity()) // in vehicle - third person - skip vehicle physiscs when trace camera
            {
                ...
                camera->SetThirdPersonMode(offset, angles, CAMERA_3DPERSON1, frameTime, camRange, !m_bFirstPerson, physEnt,
                                           //            nullptr,
                                           physEntCar, m_pGame->GetSystem()->GetI3DEngine());
            } else
...
void CPlayer::UpdateFirstPersonView() {
        ...
        if (lazy_intensity > 0.0f) {
            ...
            if (frameTime >= 0.0f) {
                ...
                if (m_vDeltaCamAngles.len2() > 0.0f || m_vDeltaCamPos.len2() > 0.0f) {
                    ...
                    if (m_vDeltaCamPos.z > 0.0f)
                        m_vDeltaCamPos.z *= 0.3f;
    ...
    if (m_walkParams.shakeLElapsedTime < m_walkParams.shakeLTime) {
        ...
        m_walkParams.shakeLOffset.x =
            m_walkParams.shakeLAmpl.x * amplScale * cry_sinf(m_walkParams.shakeLElapsedTime * m_walkParams.shakeLFreq.x * 6.283185307179586476925286766559f);
        m_walkParams.shakeLOffset.y =
            m_walkParams.shakeLAmpl.y * amplScale * cry_cosf(m_walkParams.shakeLElapsedTime * m_walkParams.shakeLFreq.y * 6.283185307179586476925286766559f);
        m_walkParams.shakeLOffset.z =
            m_walkParams.shakeLAmpl.z * amplScale * cry_sinf(m_walkParams.shakeLElapsedTime * m_walkParams.shakeLFreq.z * 6.283185307179586476925286766559f + 1.13f);
