...
bool CDivXPlayer::Load_DivX(CUIVideoPanel* pPanel, const string& szFileName) {
    ...
    if (divxFileInfo.audio_bytes) {
        ...
        if (m_pAudioBuffer == 0)
            m_pAudioBuffer = new unsigned char[MAX_AUD_BUFFER_SIZE]; // allocate the audio buffer
        for (uint32 x = 0; x < MAX_AUD_BUFFER_SIZE; x++) {
            m_pAudioBuffer[x] = 0;
        };
        ...
        if (hr == S_OK) {
            ...
            HRESULT hr = g_DivXPlayer.m_SoundManager.CreateStreaming(
    ...
    if (divxFileInfo.width == 640 && divxFileInfo.height == 480)
        pPanel->m_iTextureID = pPanel->GetUISystem()->GetIRenderer()->DownLoadToVideoMemory((uint8*)m_pFrameBuffer, divxFileInfo.width, divxFileInfo.height, eTF_0888, eTF_0888, 0,
                                                                                            0, FILTER_LINEAR, 0, "$VideoPanel", FT_DYNAMIC);
    else
        pPanel->m_iTextureID = pPanel->GetUISystem()->GetIRenderer()->DownLoadToVideoMemory((uint8*)m_pFrameBuffer, divxFileInfo.width, divxFileInfo.height, eTF_0888, eTF_0888, 0,
                                                                                            0, FILTER_LINEAR, 0, nullptr, FT_DYNAMIC);
    ...
    if (divxFile && pPanel->m_bPlaying) {
        ...
        if (FrameNo < divxFileInfo.numVideoFrames) {
            ...
            if (m_LastFramer != FrameNo) {
                ...
                if (pPanel->m_iTextureID != -1)
                    pPanel->GetUISystem()->GetIRenderer()->UpdateTextureInVideoMemory(pPanel->m_iTextureID, m_pFrameBuffer, 0, 0, divxFileInfo.width, divxFileInfo.height,
                                                                                      eTF_8888);
...
HRESULT CDivXPlayer::PlaySound() {
    ...
    if (g_DivXPlayer.m_StreamingSound.m_SoundEnabled == 0)
...
HRESULT CDivXPlayer::StopSound() {
    ...
    HRESULT hr = g_DivXPlayer.m_StreamingSound.Stop();
    assert(hr == S_OK);
    hr = g_DivXPlayer.m_StreamingSound.Reset();
    assert(hr == S_OK);
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CSoundManager::CreateStreaming(DWORD dwCreationFlags, GUID guid3DAlgorithm, DWORD dwNotifyCount, DWORD dwNotifySize, HANDLE hNotifyEvent) {
    HRESULT hr;
