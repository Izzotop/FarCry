...
enum VHC_TYPE { VHT_CAR, VHT_BOAT, VHT_BOATDEAD, VHT_PARAGLIDER };
...
public: // -------------------------------------------------------------------------------
    ...
    virtual IScriptObject* GetScriptObject() {
        return m_pScriptObject;
    }
    virtual void SetScriptObject(IScriptObject* pObject) {
    ...
    bool Save(CStream& stream, IScriptObject* pStream = nullptr) {
        return false;
    }
    bool Load(CStream& stream, IScriptObject* pStream = nullptr) {
        return false;
    }

    bool HasAccelerated() {
        return m_bAcceleratedLastUpdate && m_bAcceleratedFlagSetLastFrame;
    };
    bool IsBreaking() {
        return m_bIsBreaking;
    };

    //[filippo]
    //! added handbrakingvalue
    void SetDrivingParams(float pedalspeed, float steerspeed, float v0maxsteer, float kvmaxsteer, float v0steerrelax, float kvsteerrelax, float brake_vel_threshold,
    ...
    bool IsBoat() {
        return (m_Type == VHT_BOAT);
    }
    int GetType() {
        return (m_Type);
    }
    void SetType(VHC_TYPE type) {
        m_Type = type;
    }
