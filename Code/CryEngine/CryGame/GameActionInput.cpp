...
void CXGame::InitConsoleCommands() {
    ...
    if (!m_bEditor) {
        ...
        pConsole->AddCommand("start_server",
                             "local name=tonotnil(%%); "
        ...
        pConsole->AddCommand("stopdemo", "Game:StopDemoPlay()", 0, "Stop playing demo.\n");
        ...
        pConsole->CreateVariable("cl_saveubipassword", "1", VF_DUMPTODISK, "Enables/Disables saving of Ubi.com username and password for later use.\n");
...
void CXGame::InitConsoleVars() {
    ...
    pConsole->CreateVariable("sv_password", "", 0,
                             ...
                             "Specifying a nullptr string means, no password.\n"
    ...
    if (g_InstallerVersion->GetIVal() == 46) {
        ...
        g_Gore->ForceSet("0");
    } else if (g_InstallerVersion->GetIVal() == 48) {
        g_Gore = pConsole->CreateVariable("g_gore", "1", VF_DUMPTODISK | VF_READONLY);
        g_Gore->ForceSet("1");
    } else if (g_InstallerVersion->GetIVal() == 44) {
    ...
    */
    ...
    a_LogArea = pConsole->CreateVariable("a_log_area", "0", VF_CHEAT,
                                         ...
                                         "Usage: \n"
                                         "");
    pConsole->CreateVariable("game_DifficultyLevel", "1", VF_SAVEGAME, "0 = easy, 1 = normal, 2 = hard");
    ...
    pl_JumpNegativeImpulse =
        GetISystem()->GetIConsole()->CreateVariable("JumpNegativeImpulse", "0.0f", VF_REQUIRE_NET_SYNC,
