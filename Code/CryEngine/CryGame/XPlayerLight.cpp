...
void CPlayer::UpdateLightBlinding() {
    ...
    // return;
    if (m_pGame->m_PlayersWithLighs.empty() && ((IsMyPlayer() && m_vBlindingList.empty()) || (m_bIsAI && !m_stats.bIsBlinded)))
        return;
    ...
    for (; pl != m_pGame->m_PlayersWithLighs.end(); pl++) {
        ...
        if (m_pGame->GetSystem()->GetIPhysicalWorld()->RayWorldIntersection(blindingPos, m_vEyePos - blindingPos, ent_terrain | ent_static, rwi_stop_at_pierceable, &hit, 1,
                                                                            physic))
            continue;

        if (IsMyPlayer()) {
            Vec3d scrPos;
            GetGame()->GetSystem()->GetIRenderer()->ProjectToScreen(blindingPos.x, blindingPos.y, blindingPos.z, &scrPos.x, &scrPos.y, &scrPos.z);
