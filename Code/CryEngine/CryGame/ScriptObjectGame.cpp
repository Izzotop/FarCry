...
int CScriptObjectGame::ForceEntitiesToSleep(IFunctionHandler* pH) {
    ...
    while ((pEnt = pEntities->Next()) != nullptr) {
        ...
        // m_pSystem->GetILog()->Log("ENTITY class=%s/%d ent=%s/%d pos=(%.1f,%.1f,%.1f)", pClass->strClassName.c_str(), pClass->cTypeID, pEnt->GetName(), pEnt->GetId(), pos.x,
        // pos.y, pos.z);
    }
...
int CScriptObjectGame::GetLevelList(IFunctionHandler* pH) {
    ...
    if (pH->GetParamCount() >= 1 && !pH->GetParam(1, pszMissionFilter)) {
        m_pScriptSystem->RaiseError("CScriptObjectGame::GetLevelList : 1st (%s) of %d arguments couldn't be resolved as mission name string.",
                                    ScriptVarTypeAsCStr(pH->GetParamType(1)), pH->GetParamCount());
        return pH->EndFunctionnullptr();
    }
    ...
    do {
        ...
        if ((strncmp(c_file.name, ".", 1) != 0) && (c_file.attrib & _A_SUBDIR) && m_pGame->GetLevelMissions((sLevelsFolder + "/" + c_file.name).c_str(), arrMissions) &&
        ...
        if (!pData) {
            ...
            if (GetFileVersionInfo((char*)szFileName.c_str(), 0, dwVersionSize, pData)) {
                ...
                if (VerQueryValue(pData, szQueryInfoString, (LPVOID*)&pVersionBuffer, &dwSize) && dwSize == 4) {
                    ...
                    sprintf(szQueryInfoString, "\\StringFileInfo\\%02X%02X%02X%02X\\FileVersion", (dwLang & 0xff00) >> 8, dwLang & 0xff, (dwLang & 0xff000000) >> 24,
                            (dwLang & 0xff0000) >> 16);
                } else {
...
int CScriptObjectGame::GetEntitiesScreenSpace(IFunctionHandler* pH) {
    ...
    while (pEnt = It->Next()) {
        ...
        if (GetLengthSquared(diff) > 700 * 700)
            continue;
        if (m_pSystem->GetIPhysicalWorld()->RayWorldIntersection(vectorf(Cam.GetPos()), diff, ent_terrain | ent_static, 0, &RayHit, 1, pPE))
            continue;
        ...
        Position.z = (float)pz;
        if ((Position.x >= 0.0f) && (Position.y >= 0.0f) && (Position.x <= 800.0f) && (Position.y <= 600.0f) && (Position.z > 0.0f)) {
...
int CScriptObjectGame::DrawHalfCircleGauge(IFunctionHandler* pH) {
    if (pH->GetParamCount() < 10)
        CHECK_PARAMETERS(10);
    if (!m_pRenderer)
        return pH->EndFunctionnullptr(); // no renderer
    int nCookie = 0;
    ...
    if (!(pH->GetParamUDVal(9, nTid, nCookie) && (nCookie == USER_DATA_TEXTURE))) {
        return pH->EndFunctionnullptr(); // invalid texture handle
    }
    ...
    const _vtx_ vtxpos[6] = {{x + 1, y, 0.0f, dwColor, u, v},
...
int CScriptObjectGame::DumpEntities(IFunctionHandler* pH) {
    ...
    while ((pEnt = pEntities->Next()) != nullptr) {
        ...
        if (m_pGame->GetWeaponSystemEx()->IsProjectileClass(pClass->ClassId))
            continue;
        Vec3 pos = pEnt->GetPos();
        m_pSystem->GetILog()->Log("ENTITY class=%s/%d ent=%s/%d pos=(%.1f,%.1f,%.1f)", pClass->strClassName.c_str(), (int)pClass->ClassId, pEnt->GetName(), pEnt->GetId(), pos.x,
                                  pos.y, pos.z);
    }
    m_pSystem->GetILog()->Log("------------------------------------------------------------------------------------------");
    return pH->EndFunction();
...
int CScriptObjectGame::CreateExplosion(IFunctionHandler* pH) {
    ...
    m_pGame->CreateExplosion(pos, damage, rmin, rmax, radius, impulsive_pressure, fShakeFactor, fDeafnessRadius, fDeafnessTime, ImpactForceMul, ImpactForceMulFinal,
                             ImpactForceMulFinalTorso, rmin_occ, nOccRes, nGrow, shooter, shooterSSID, weapon, fTerrainDefSize, nTerrainDecalId);
    return pH->EndFunction();
}
...
int CScriptObjectGame::GetModsList(IFunctionHandler* pH) {
    ...
    if (!pMods)
        return pH->EndFunctionnullptr(); // game was not Init
