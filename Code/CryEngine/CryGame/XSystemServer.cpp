...
bool CXSystemServer::LoadLevel(const char* szLevelDir, const char* szMissionName, bool bEditor) {
    ...
    if (m_pGame->IsMultiplayer()) {
        for (int i = 0; i < 10; i++)
            m_pXServer->UpdateXServerNetwork(); // Do some network updates.
    }
    ...
    m_pSystem->GetINetwork()->OnAfterServerLoadLevel(m_pXServer->sv_name->GetString(), m_pXServer->sv_maxplayers->GetIVal(), m_pXServer->m_ServerInfos.nPort);
