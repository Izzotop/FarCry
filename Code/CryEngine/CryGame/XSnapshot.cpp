...
void CXSnapshot::BuildAndSendSnapshot() {
    ...
    if (m_pServer->sv_netstats->GetIVal() & 0x4) // log netentities sent/count
        GetISystem()->GetILog()->Log("netentities %d/%d level: %.2f (%dbps(~%dbps)/%dbps(%d)) carry:%d fac:%.2f", dwObjectSent, dwObjectSum, fPrioritySendLevel, dwEstimatedBps,
                                     nSnapshotSize * iActualSendPerSec, maxRateBps, iAvailableBps, m_iCarryOverBps, (float)(dwEstimatedBps) / (float)iAvailableBps);
