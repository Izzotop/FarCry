...
void CXGame::SetPlayerEquipPackName(const char* pszPackName) {
    // Pass nullptr or "" to remove all equipment from the player
    IScriptSystem* pIScriptSystem = GetScriptSystem();
    IEntity* pIMyPlayer = (m_pServer != nullptr) ? m_pServer->m_pISystem->GetLocalPlayer() : m_pClient->m_pISystem->GetLocalPlayer();
...
void CXGame::RestoreWeaponPacks() {
    ...
    if (m_pServer) {
        if (!m_pServer->m_pISystem) {
            m_pLog->LogError("CXGame::RestoreWeaponPacks Error1");
            return;
        }
    } else {
        if (!m_pClient) {
            m_pLog->LogError("CXGame::RestoreWeaponPacks Error2");
            return;
        }

        if (!m_pClient->m_pISystem) {
            m_pLog->LogError("CXGame::RestoreWeaponPacks Error3");
            return;
        }
    }
