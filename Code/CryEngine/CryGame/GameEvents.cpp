...
void EventPlayerCmd::Execute(CXGame* pGame) {
    ...
    IEntity* pEnt;
    CPlayer* pPlayer;
    if (pES && (pEnt = pES->GetEntity(idEntity)) && pEnt->GetContainer() && pEnt->GetContainer()->QueryContainerInterface(CIT_IPLAYER, (void**)&pPlayer))
...
void EventExplosion::Execute(CXGame* pGame) {
    ...
    pGame->CreateExplosion(pos, damage, rmin, rmax, radius, impulsivePressure, shakeFactor * (1.0f / 254), deafnessRadius * (20.0f / 254), deafnessTime * 0.25f, iImpactForceMul,
                           iImpactForceMulFinal, iImpactForceMulFinalTorso, rminOcc * (1.0f / 254), nOccRes, nGrow, pShooter, nShooterSSID, pWeapon, terrainDefSize * (20.0f / 254),
                           nTerrainDecalId, true);
}
...
void EventVehicleDamage::Execute(CXGame* pGame) {
    ...
    IEntity* pEnt;
    CVehicle* pVehicle;
    if (pES && (pEnt = pES->GetEntity(idVehicle)) && pEnt->GetContainer() && pEnt->GetContainer()->QueryContainerInterface(CIT_IVEHICLE, (void**)&pVehicle))
...
//////////////////////////////////////////////////////////////////////////
void CXGame::ScheduleEvent(int iPhysTime, const Vec3& pos, float fDamage, float rmin, float rmax, float radius, float fImpulsivePressure, float fShakeFactor, float fDeafnessRadius,
...
void CXGame::ExecuteScheduledEvents() {
    ...
    for (i = 0; i < sz && m_lstEvents[i].iPhysTime <= iCurTime; ++i) {
        if (m_lstEvents[i].iPhysTime < iCurTime)
            m_pLog->Log("Event missed the schedule by %.4f", (iCurTime - m_lstEvents[i].iPhysTime) * m_pSystem->GetIPhysicalWorld()->GetPhysVars()->timeGranularity);
