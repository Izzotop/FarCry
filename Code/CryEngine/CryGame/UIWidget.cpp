...
CUIWidget::CUIWidget()
    : m_iFlags(UIFLAG_DEFAULT), m_iStyle(0), m_pUISystem(0), m_pScreen(0), m_bMoving(0), m_cColor(color4f(1.0, 1.0f, 1.0f, 1.0f)), m_iMouseCursor(-1), m_iTabStop(0),
      m_cGreyedColor(0.3f, 0.3f, 0.3f, 0.5f), m_iGreyedBlend(0) {
...
int CUIWidget::SetRect(IFunctionHandler* pH) {
    ...
    if ((pH->GetParamCount() == 1) && (pH->GetParamType(1) == svtString)) {
        ...
        m_pUISystem->GetIScriptSystem()->RaiseError("%s:SetRect() Wrong type in parameter 1! Expected 'String' or 'Number', but found '%s'!", GetName().c_str(),
