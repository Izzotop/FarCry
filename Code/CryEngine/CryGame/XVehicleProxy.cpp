...
CXVehicleProxy::CXVehicleProxy(IEntity* pEntity, IScriptSystem* pScriptSystem, CXGame* pGame) : pSignalTable(pScriptSystem), m_MinAltitude(0.0f), m_bTargetInWater(true) {
...
void CXVehicleProxy::SendAuxSignal(int signalID, const char* szText) {
    ...
    pSignalTable->SetTonullptr("SignalText");
...
void CXVehicleProxy::MoveLikeACar(SOBJECTSTATE* state) {
    ...
    // GetISystem()->GetILog()->Log("distance from target for %s : %.3f (%s) %i",m_pEntity->GetName(),state->fDistanceFromTarget,state->bTargetEnabled?"enable":"not
    // enable",state->nTargetType);
    ...
    } else // drive!!!
    {
        ...
        if (state->back || doGoBack) {
            ...
            } else if (state->fDesiredSpeed < .5f && curVel > 1.0f)
...
void CXVehicleProxy::MoveLikeABoat(SOBJECTSTATE* state) {
    ...
    // drive!!!
    {
        ...
        if (!bApproachingDropPoint) {
            ...
            Vec3d vClosestPoint;
            bool intersectsForbidden =
                m_pGame->GetSystem()->GetAISystem()->IntersectsForbidden(m_pEntity->GetPos(), fwdPoint, vClosestPoint) && m_pEntity->GetAI()->IfNeedsPathOutdoor();
...
bool CXVehicleProxy::BoatPointInWater(const Vec3d& pos) {
    ...
    if (m_pGame->m_p3DEngine->GetWaterLevel(&pos) - m_pGame->m_p3DEngine->GetTerrainElevation(pos.x, pos.y) > .3f)
