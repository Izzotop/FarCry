...
CUIComboBox::CUIComboBox()
    : m_iSelectionIndex(0), m_iHAlignment(UIALIGN_LEFT), m_iVAlignment(UIALIGN_CENTER), m_cButtonColor(color4f(1.0f, 1.0f, 1.0f, 1.0f)), m_iState(0), m_iOldZ(0), m_iNewIndex(1),
      m_bNeedSort(0), m_iSortOrder(UISORT_ASCENDING), m_fLeftSpacing(0), m_iMaxItems(6), m_iFirstItem(0), m_pVScroll(0), m_bVerticalScrollBar(0),
      m_cItemBgColor(0.0f, 0.0f, 0.0f, 0.35f), m_iRollUp(0) {}
...
LRESULT CUIComboBox::Update(unsigned int iMessage, WPARAM wParam, LPARAM lParam) // AMD Port
{
    ...
    if (iMessage == UIM_DRAW) {
        ...
        if (bVScroll) {
            if (!m_pVScroll) {
                m_pUISystem->CreateScrollBar(&m_pVScroll, this, "vscrollbar", UIRect(0, 0, m_fButtonSize, m_iMaxItems * m_fItemHeight), UIFLAG_ENABLED, 0,
                                             UISCROLLBARTYPE_VERTICAL);
            }
            assert(m_pVScroll);
...
int CUIComboBox::AddItem(IFunctionHandler* pH) {
    ...
    if (pH->GetParamCount() > 1) {
        CHECK_SCRIPT_FUNCTION_PARAMTYPE2(m_pScriptSystem, GetName().c_str(), AddItem, 2, svtUserData, svtnullptr);
    ...
    if (pH->GetParamCount() > 2) {
        CHECK_SCRIPT_FUNCTION_PARAMTYPE2(m_pScriptSystem, GetName().c_str(), AddItem, 3, svtString, svtnullptr);
...
int CUIComboBox::InsertItem(IFunctionHandler* pH) {
    ...
    // get the texture first
    if (pH->GetParamCount() > 2) {
        CHECK_SCRIPT_FUNCTION_PARAMTYPE2(m_pScriptSystem, GetName().c_str(), InsertItem, 3, svtUserData, svtnullptr);
    ...
    // then get the texrect
    if (pH->GetParamCount() > 3) {
        CHECK_SCRIPT_FUNCTION_PARAMTYPE2(m_pScriptSystem, GetName().c_str(), AddItem, 4, svtString, svtnullptr);
