...
CVehicle::CVehicle(CXGame* pGame)
    : m_fPedalSpeed(0.0f), m_DriverID(0), m_fPrevFwvSpeedLen(0.0f), m_pHeadLight(nullptr), m_pHeadLightLeft(nullptr), m_pHeadLightRight(nullptr), m_pBreakLightLeft(nullptr),
      m_pBreakLightRight(nullptr), m_bHeadLightsOn(false), m_bBreakLightsOn(false), m_bAutoLights(false), m_bPhysAwaike(false), m_bBrakeOnNodriver(true), m_fNoDriverTime(0.0f),
      m_AngleLimitVFlag(false), m_AngleLimitHFlag(false), m_vWpnAng(0, 0, 180), m_vWpnAngDelta(0, 0, 0), m_bCrossOnScreen(false), m_WeaponUser(0), m_bForceHandBreak(false) {
    m_pGame = pGame;
    ...
    m_fv0MaxSteer = 40.0f;
    m_fkvMaxSteer = 0.0f;
    m_fv0SteerRelaxation = 0.0f;
    m_fkvSteerRelaxation = 5.0f;
    m_fMaxSteeringPedal = 0.015f;
    m_fPedalLimitSpeed = 10.0f;
    ...
    m_vCamStiffness[0](0, 0, 0);
    m_vCamStiffness[1](0, 0, 0);
    m_vCamLimits[0](1, 1, 1);
    m_vCamLimits[1](1, 1, 1);
    m_fCamDamping = 0.8f;
    m_fMaxCamTimestep = 0.01f;
    m_fCamSnapDist = 0.001f;
    m_fCamSnapVel = 0.01f;
...
void CVehicle::ProcessMovementBoat2(CXEntityProcessingCmd& cmd, float velScale) {
    ...
    */
    ...
    if (cmd.CheckAction(ACTION_MOVE_RIGHT) && bCanTurn) {
        control.momentum = -Vec3(-dir.x * m_btVelocity * m_BoatParams.m_fBtTitlTurn * dt, -dir.y * m_btVelocity * m_BoatParams.m_fBtTitlTurn * dt, fTurnImpuls * dt);
    } else if (cmd.CheckAction(ACTION_MOVE_LEFT) && bCanTurn) {
        control.momentum = Vec3(-dir.x * m_btVelocity * m_BoatParams.m_fBtTitlTurn * dt, -dir.y * m_btVelocity * m_BoatParams.m_fBtTitlTurn * dt, fTurnImpuls * dt);
    }
...
void CVehicle::SetDrivingParams(float pedalspeed, float steerspeed, float v0maxsteer, float kvmaxsteer, float v0steerrelax, float kvsteerrelax, float brake_vel_threshold,
                                float brake_axle_friction, float steerspeedVelScale, float steerspeedMin, float maxSteeringPedal, float pedalLimitSpeed, float fhandbrakingvalue,
                                float fmaxbrakingfrictionnodriver, float fhandbrakingvaluenodriver, float fstabilizejump, float fsteerspeedscale, float fsteerspeedscalemin) {
    m_fPedalSpeed = pedalspeed;
    m_fSteerSpeed = steerspeed;
    m_fv0MaxSteer = v0maxsteer;
    m_fkvMaxSteer = kvmaxsteer;
    m_fv0SteerRelaxation = v0steerrelax;
    m_fkvSteerRelaxation = kvsteerrelax;
    m_fBrakeVelThreshold = brake_vel_threshold;
    m_fBrakeAxleFriction = brake_axle_friction;
...
void CVehicle::SetCameraParams(Vec3 vCamStiffness[2], Vec3 vCamLimits[2], float fCamDamping, float fMaxCamTimestep, float fCamSnapDist, float fCamSnapVel) {
...
void CVehicle::UpdateCamera(float fTimeStep, bool bSynchronizing) {
    if (m_bUpdateCamera) {
        ...
        if (!bSynchronizing) {
            if (fTimeStep == 0 || (velTarget = (posTargetNew - m_vPrevCamTarget) / fTimeStep).len2() > sqr(200) || m_vCamStiffness[0].len2() + m_vCamStiffness[1].len2() == 0) {
...
void CVehicle::OnEntityNetworkUpdate(const EntityId& idViewerEntity, const Vec3d& v3dViewer, uint32& inoutPriority, EntityCloneState& inoutCloneState) const {
