...
private: // -----------------------------------------------------------------------
    struct STableEntry {
            STableEntry(const char* szString) : m_fValue(FLT_MAX), m_sValue(szString) {}

            STableEntry(const float fValue) : m_fValue(fValue) {}

        //! default constructor
        STableEntry() : m_fValue(FLT_MAX) {}
    ...
    struct STableLine {
        ...
        void SetX(const uint32 uiX, const STableEntry& rValue) {
            if (uiX == 0xff) {
                assert(0);
                return;
            } // (0xff is used to mark the whole line dirty)
