...
private:
    ...
    bool Init(CIngameDialogMgr* pMgr, int nId, ISystem* pSystem, int nFillId, const char* pszFontName, const char* pszEffectName, int nSize, string sText, wstring swText,
              float fTimeout);
...
public:
    CIngameDialogMgr();
    ~CIngameDialogMgr();
    int AddDialog(ISystem* pSystem, int nFillId, const char* pszFontName, const char* pszEffectName, int nSize, string sText, wstring swText,
                  float fTimeout = 0.0f); // return id to dialog
