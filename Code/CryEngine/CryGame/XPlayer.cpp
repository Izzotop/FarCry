...
CPlayer::~CPlayer() {
    ...
    GetEntity()->GetCharInterface()->SetCharacter(1, nullptr);
...
void CPlayer::UpdateRotateHead() {
    ...
    if (!m_bRotateHead) {
        if (m_pEntity->GetCurrentAnimation(0, 3) >= 0 || m_pEntity->IsBound() ||
...
void CPlayer::AutoAiming() {
    ...
    while (pEnt = It->Next()) {
        ...
        CPlayer* pPlayer;
        if (pEnt->GetContainer() && pEnt->GetContainer()->QueryContainerInterface(CIT_IPLAYER, (void**)&pPlayer) && !pPlayer->IsAlive())
            continue;
    ...
    if (m_pLastAiming) {
        CPlayer* pPlayer;
        if (m_pLastAiming->GetContainer() && m_pLastAiming->GetContainer()->QueryContainerInterface(CIT_IPLAYER, (void**)&pPlayer) && !pPlayer->IsAlive())
            m_pLastAiming = nullptr;
    }
...
void CPlayer::Update() {
    ...
    m_bRotateHead =
        m_pEntity->GetCurrentAnimation(0, 3) < 0 && m_pGame->p_RotateHead->GetIVal() != 0 && IsAlive() && !m_pEntity->IsBound() && !m_stats.onLadder // no rotatehead on ladders
...
void CPlayer::UpdatePhysics(float fDeltaTime) {
    ...
    if (m_stats.fKWater >= 1.0f && sl.velRequested.len2() > 0 && sqr(sl.velRequested * vDir) > sl.velRequested.len2() * 0.5f && m_stats.swimTime > 1.0f)
        movedyn.kAirControl = 1.0f;
...
void CPlayer::ProcessAngles(CXEntityProcessingCmd& ProcessingCmd) {
    ...
    {
        Angles.x = ClampAngle(Snap_s360(m_AngleLimitBase.x + m_MinVAngle), Snap_s360(m_AngleLimitBase.x + m_MaxVAngle), Snap_s360(Angles.x));
    ...
    if (m_AngleLimitHFlag) {
        ...
        Angles.z = ClampAngle(Snap_s360(m_AngleLimitBase.z + m_MinHAngle), Snap_s360(m_AngleLimitBase.z + m_MaxHAngle), Snap_s360(Angles.z), bClamped);
        if (m_bIsAI && bClamped)
    ...
    if (!m_pRedirected) {
        ...
        if (m_Dynamics.gravity != 0.0f) // only if there is gravity
        {
            ...
            m_vShake.x += (cry_cosf(m_walkParams.swayOfs * 15.9f + 0.3f) * m_walkParams.swayAmp * 0.1f +
                           cry_sinf(m_walkParams.swayOfs * 5.9f + 0.0f) * m_walkParams.swayAmp * 0.45f + cry_cosf(m_walkParams.swayOfs * 0.95f + 0.5f) * m_walkParams.swayAmp) *
                          0.33333f;
            m_vShake.z += (cry_cosf(m_walkParams.swayOfs * 14.9f + 0.7f) * m_walkParams.swayAmp * 0.12f +
                           cry_sinf(m_walkParams.swayOfs * 5.05f + 0.6f) * m_walkParams.swayAmp * 0.5f + cry_cosf(m_walkParams.swayOfs * 1.0f + 0.2f) * m_walkParams.swayAmp) *
                          0.33333f;
...
void CPlayer::ProcessMovements(CXEntityProcessingCmd& cmd, bool bScheduled) {
    ...
    */
    ...
    if ((cmd.CheckAction(ACTION_WALK) ^ m_bAlwaysRun) &&
        (cmd.CheckAction(ACTION_MOVE_RIGHT) || cmd.CheckAction(ACTION_MOVE_LEFT) || cmd.CheckAction(ACTION_MOVE_FORWARD) || cmd.CheckAction(ACTION_MOVE_BACKWARD)) &&
        !m_stats.bForceWalk) {
        if (!m_Running) {
    ...
    if (m_Running) {
        if (cmd.CheckAction(ACTION_RUNSPRINT) && m_stats.stamina > m_StaminaTable.DecoyRun * .5f || cmd.CheckAction(ACTION_RUNSPRINT) && m_Sprinting && m_stats.stamina > 1)
            m_Sprinting = true;
        else
            m_Sprinting = false;
        m_stats.running = true;
    ...
    } else if (cmd.CheckAction(ACTION_LEANRIGHT) && (!m_stats.lock_weapon) && (m_CurStance != eProne)) {
        if (IsMyPlayer()) {
            if (m_walkParams.leanEnd != -1.0f) {
                m_walkParams.leanStart = m_walkParams.leanAmount;
                m_walkParams.leanEnd = -1.0f;
    ...
    if ((cmd.CheckAction(ACTION_JUMP))) {
        ...
        if (m_stats.onLadder) {
            ...
            cmd.RemoveAction(ACTION_JUMP);
                   ...
                   //      && m_CurStance != eProne
                   && ((!m_stats.flying) || IsSwimming()) && ((m_stats.stamina > m_StaminaTable.DecoyJump && !m_stats.flying) || IsSwimming()) && m_JumpAniLenght <= 0) {
...
void CPlayer::SelectPrevWeapon() {
    ...
    while (iteration < PLAYER_MAX_WEAPONS && (m_vWeaponSlots[slot] == 0 || m_vWeaponSlots[slot] == curr)) {
        ...
        if (slot < 0) {
            ...
            while (slot >= 0 && (m_vWeaponSlots[slot] == 0))
                slot--;
            if (slot < 0)
                slot = 0;
        }
...
void CPlayer::ProcessWeapons(CXEntityProcessingCmd& cmd) {
    ...
    if (cmd.CheckAction(ACTION_RELOAD) && (!m_stats.lock_weapon) && (m_stats.underwater == 0.0f) && !m_bSwimming) {
        if (pSelectedWeapon) {
...
void CPlayer::FireGrenade(const Vec3d& origin, const Vec3d& angles, IEntity* pIShooter) {
    ...
    if (fWaterLevel > origin.z)
        ...
        pTable->SetTonullptr("underwater");
...
void CPlayer::SetWeapon(int iClsID) {
    ...
    if (pSelectedWeapon) {
        ...
        m_pScriptObject->SetTonullptr("weapon");
        m_pScriptObject->SetTonullptr("weaponid");
        m_stats.fSpeedScale = 1.0f;
        m_nSelectedWeaponID = -1;
    }
}
...
void CPlayer::UpdateMelee() {
    ...
    if (m_stats.melee_attack && (m_stats.weapon_busy <= 0) && (m_stats.melee_distance > 0.0f)) {
        ...
        m_stats.melee_attack = false;
        m_pEntity->GetScriptObject()->SetTonullptr("melee_target");
    }
}
///////////////////////////////////////////////
...
void CPlayer::UpdateWeapon() {
    ...
    if ((m_stats.ammo_in_clip <= 0 && m_stats.ammo > 0 && (!m_stats.reloading) && (m_stats.weapon_busy <= 0)) && (!m_stats.firing) && (m_stats.underwater == 0.0f) &&
        !m_bSwimming) {
        pSelectedWeapon->ScriptOnStopFiring(m_pEntity);
        pSelectedWeapon->ScriptReload(m_pEntity);
        wi.fireTime = m_pTimer->GetCurrTime();
                    ...
                    if (bullets = pSelectedWeapon->Fire(firePos, fireAngles, this, wi, GetRedirected() ? GetRedirected()->GetPhysics() : nullptr)) {
                        ...
                        if (!IsAI()) // IsMyPlayer())        // every player should have recoil
                        {
                            ...
                            if ((m_fRecoilXDelta + 1) < m_pGame->w_recoil_max_degree) {
                                ...
                                //                GetISystem()->GetILog()->Log(">> Recoil                           Recoil %d
                                //%d",(int)GetEntity()->GetId(),(int)(ucSeed));      // debug
        ...
        if (pChar) {
            ...
            */
            ...
            //      else if(m_aimLook)  //aiming
                ...
                //                                  -xAngle*.1f);//GetAngleDifference360(0,
                //-angles.x)*.5f);
            }
        }
    } else {
...
void CPlayer::UpdateFireAnimations() {
    ...
    } else if (m_aimLook && m_stats.canfire) {
...
void CPlayer::StartAnimation(const SPlayerUpdateContext& ctx) {
    ...
    //    m_stats.fInWater>1.0f)
    {
        ...
        if (m_pGame->GetSystem()->GetIPhysicalWorld()->RayWorldIntersection(pos, vectorf(0, 0, -1.5f), ent_all, rwi_stop_at_pierceable, &hit, 1, GetEntity()->GetPhysics()))
            depth = hit.dist;
        ...
        if (!m_bIsAI) {
            ...
            if (m_stats.flying) {
                if (m_JumpStage == 0 || m_JumpStage == 1 || m_JumpStage == 3) // if jumping immidiately after prev jump - still in landing state
                {
...
*/
void
CPlayer::SetDimCrouch(const pe_player_dimensions *const pDim) {
    if (!pDim) {
...
*/
void
CPlayer::SetDimProne(const pe_player_dimensions *const pDim) {
    if (!pDim) {
...
int CPlayer::GetArmDamage() const {
...
int CPlayer::GetLegDamage() const {
...
void CPlayer::SetDynamics(const PlayerDynamics* pDyn) {
    if (pDyn->air_control != 1E10f)
        m_Dynamics.air_control = pDyn->air_control;
    if (pDyn->gravity != 1E10f)
        m_Dynamics.gravity = pDyn->gravity;
    if (pDyn->swimming_gravity != 1E10f)
        m_Dynamics.swimming_gravity = pDyn->swimming_gravity;
    if (pDyn->inertia != 1E10f)
        m_Dynamics.inertia = pDyn->inertia;
    if (pDyn->swimming_inertia != 1E10f)
        m_Dynamics.swimming_inertia = pDyn->swimming_inertia;
    if (pDyn->jump_gravity != 1E10f)
        m_Dynamics.jump_gravity = pDyn->jump_gravity;
}
...
bool CPlayer::CanProne(bool ignoreSpam) {
    ...
    pos.z += .3f;
    pos.x += .1f;
    if (m_pGame->GetSystem()->GetIPhysicalWorld()->RayWorldIntersection(pos, vectorf(0, 0, -1.5f), ent_all - ent_independent,
                                                                        rwi_stop_at_pierceable | geom_colltype_player << rwi_colltype_bit, &hit, 1, GetEntity()->GetPhysics())) {
...
void CPlayer::UpdateCollisionDamage() {
    ...
    if (colN = physEnt->GetStatus(&status)) {
        ...
        if (damageValue >= 1.0f) {
            ...
            CScriptObjectVector oDir(m_pScriptSystem);
            oDir = history[imax].n;
...
void CPlayer::OnEntityNetworkUpdate(const EntityId& idViewerEntity, const Vec3d& v3dViewer, uint32& inoutPriority, EntityCloneState& inoutCloneState) const {
