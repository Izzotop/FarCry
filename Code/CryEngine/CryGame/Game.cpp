...
public:
    CTagPointManager(IGame* pGame) {
        m_pGame = (CXGame*)pGame;
    };
...
IXSystem* CXGame::GetXSystem() {
    return m_pServer ? m_pServer->m_pISystem : m_pClient ? m_pClient->m_pISystem : nullptr;
}
//////////////////////////////////////////////////////////////////////
...
bool CXGame::Update() {
    ...
    bool bRenderFrame = (!m_pSystem->GetViewCamera().GetPos().IsZero() || m_bMenuOverlay || m_bUIOverlay) && g_Render->GetIVal() != 0;
    ...
    if (!bPause || (m_pClient && !m_pClient->IsConnected())) { // network start
...
void CXGame::ProcessPMessages(const char* szMsg) {
    ...
    if ((stricmp(szMsg, "EndDemo") == 0) || (stricmp(szMsg, "EndDemoQuit") == 0)) // used for demos (e3, magazine demos)
    {
        ...
        return;
    } else if (stricmp(szMsg, "Quit-Yes") == 0) // quit message
    {
        m_bUpdateRet = false;
        return;
    ...
    } else if (strnicmp(szMsg, "LoadGame", 8) == 0) // load game
    {
        if (!m_bEditor) {
            m_p3DEngine->ResetScreenFx();
            // clear the current message
