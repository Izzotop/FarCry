...
CUIListView::CUIListView()
    : m_fHeaderHeight(16.0f), m_fItemHeight(18.0f), m_fVerticalOffset(0), m_fHorizontalOffset(0), m_bVerticalScrollBar(0), m_bHorizontalScrollBar(0),
      m_iSortOrder(UISORT_ASCENDING), m_iSortColumn(0), m_iNewIndex(1), m_bColumnSelect(0), m_iSelectedColumn(0), m_bNeedSort(0), m_pVScroll(0), m_pHScroll(0),
      m_fCellSpacing(0.0f), m_fCellPadding(0.0f), m_fTotalWidth(0.0f), m_fTotalHeight(0.0f), m_fColumnTextSpacing(0), m_cSelectionColor(0.0f, 0.0f, 0.8f, 0.8f), m_iNoSort(0),
      m_cSortByColor(0.0f, 0.0f, 0.85f, 1.0f), m_cSortByTextColor(1.0f, 1.0f, 1.0f, 1.0f) {}
...
int CUIListView::AddColumn(const wstring& szLabel, float fWidth, int iHTextAlignment, const color4f& cHeaderColor, const color4f& cBodyColor, const UISkinTexture& pHeaderTexture,
                           bool bNumeric, bool bSelectable, bool bImageSort) {
    UIListColumn pColumn;
...
int CUIListView::AddItem(IFunctionHandler* pH) {
    ...
    for (int i = 1; i <= iParamCount; i++) {
        if ((pH->GetParamType(i) != svtString) && (pH->GetParamType(i) != svtNumber) && (pH->GetParamType(i) != svtnullptr)) {
        ...
        if (pH->GetParamType(i) != svtnullptr) {
...
int CUIListView::AddColumn(IFunctionHandler* pH) {
    ...
    if (pH->GetParamCount() > 5) {
        CHECK_SCRIPT_FUNCTION_PARAMTYPE2(m_pScriptSystem, GetName().c_str(), AddColumn, 6, svtUserData, svtnullptr);

        if (pH->GetParamType(6) != svtnullptr) {
            int iCookie = 0;
    ...
    // get texture rect
    if ((pH->GetParamCount() > 6) && (pH->GetParamType(7) != svtnullptr) && (pH->GetParamType(6) != svtnullptr)) {
        CHECK_SCRIPT_FUNCTION_PARAMTYPE2(m_pScriptSystem, GetName().c_str(), AddColumn, 7, svtString, svtnullptr);
...
int CUIListView::AddImage(IFunctionHandler* pH) {
    ...
    CHECK_SCRIPT_FUNCTION_PARAMTYPE2(m_pScriptSystem, GetName().c_str(), AddImage, 1, svtUserData, svtnullptr);
...
int CUIListView::AddImageList(IFunctionHandler* pH) {
    ...
    for (int i = 1; i <= pImageList->Count(); i++) {
        ...
        if (!pImage->GetAtUD(1, (USER_DATA&)pListImage.iTextureID, iCookie) || !pImage->GetAt(2, pListImage.fWidth) || !pImage->GetAt(3, pListImage.fHeight)) {
            m_pUISystem->GetISystem()->GetILog()->Log("$4Warning:$1 Invalid image specification (%d) adding to listview(%s).", i, GetName().c_str());
