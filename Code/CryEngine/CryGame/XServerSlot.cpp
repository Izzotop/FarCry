...
bool CXServerSlot::IsEntityOffSync(EntityId id) {
    ...
    if (m_pParent->m_pGame->IsMultiplayer() && m_pParent->m_pGame->UseFixedStep()) {
        ...
        if (itor != m_mapOffSyncEnts.end())
            ...
            if (!(pEnt && (pContainer = pEnt->GetContainer()) && pContainer->QueryContainerInterface(CIT_IVEHICLE, (void**)&pVehicle) && pVehicle->HasDriver()))
                bOffSync = false;
        }
    }
    return bOffSync;
...
///////////////////////////////////////////////
void CXServerSlot::OnXPlayerAuthorization(bool bAllow, const char* szError, const BYTE* pGlobalID, unsigned int uiGlobalIDSize) {
...
void CXServerSlot::SendCommand(const char* sCmd, const Vec3& _invPos, const Vec3& _invNormal, const EntityId inId, const unsigned char incUserByte) {
    Vec3 invPos = _invPos;
...
void CXServerSlot::OnClientMsgPlayerProcessingCmd(CStream& stm) {
    ...
    if (pPhysEnt) {
        ...
        if (m_nDesyncFrames > 4 || fTimeDiff > fTimeThresh * 8 || !m_iClientWorldPhysTimeDelta) {
            ...
            if (iPing > 0 && fabs_tpl((iNewTimeDelta - m_iClientWorldPhysTimeDelta) * timeGran) < 3.0f)
                m_pLog->LogToConsole("Adjusting client clock by %+.4f, new delta %.4f (ping %.3f)", (iNewTimeDelta - m_iClientWorldPhysTimeDelta) * timeGran,
                                     iNewTimeDelta * timeGran, fPing);
    ...
    if (bHasVehicle) {
        ...
        if (!m_bLocalHost && pEnt && pEnt->GetContainer() && pEnt->GetContainer()->QueryContainerInterface(CIT_IVEHICLE, (void**)&pVehicle) &&
            pSlotPlayerEntity->GetContainer()->QueryContainerInterface(CIT_IPLAYER, (void**)&pPlayer) && pVehicle == pPlayer->GetVehicle() && pEnt->GetPhysics()->GetStatus(&sv) &&
            sv.nActiveColliders == 0) {
            pEnt->Read(stm);
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    Vec3 posClient;
    bool bClientPos, bInRange;
    stm.Read(bClientPos);
    if (bClientPos) {
        stm.Read(bInRange);
        if (bInRange) {
            unsigned int num;
            stm.ReadNumberInBits(num, 16);
            posClient.x = num * (1.0f / 16);
            stm.ReadNumberInBits(num, 16);
            posClient.y = num * (1.0f / 16);
            stm.ReadNumberInBits(num, 13);
            posClient.z = num * (1.0f / 16);
        } else
            stm.Read(posClient);
    }
