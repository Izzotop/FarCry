...
void CSynched2DTable::Update() {
    ...
    assert(pXGame);
    CXServer* pXServer = pXGame->GetServer();
    assert(pXServer);

    IServer* pIServer = pXServer->m_pIServer;
    assert(pIServer);
...
void CSynched2DTable::OnEntityNetworkUpdate(const EntityId& idViewerEntity, const Vec3d& v3dViewer, uint32& inoutPriority, EntityCloneState& inoutCloneState) const {
