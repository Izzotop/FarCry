...
CXClient::~CXClient() {
    ...
    */
    ...
    if (m_pScriptSystem) {
        m_pScriptSystem->SetGlobalTonullptr("ClientStuff");
        m_pScriptSystem->SetGlobalTonullptr("Client");
...
void CXClient::OnXContextSetup(CStream& stm) {
    ...
    if (!m_GameContext.IsVersionOk()) {
        m_pLog->LogError("CXClient::OnXContextSetup - Versions do not match.  Server version: %i.%d  Client version: %i.%d", m_GameContext.dwNetworkVersion,
                         (int)m_GameContext.ucServerInfoVersion, NETWORK_FORMAT_VERSION, (int)SERVERINFO_FORMAT_VERSION);
    ...
    if ((!m_pGame->m_bEditor) && (!m_pGame->IsServer()) && (m_pISystem->GetLevelDataCheckSum() != m_GameContext.wLevelDataCheckSum)) {
        ...
        if (!bSendToServer) {
...

            {
                ...
                if (bSyncToServer) {
                    ...
                    //          GetISystem()->GetILog()->Log(">> ClientCmd Write %d
                    //%d",(int)pPlayer->GetEntity()->GetId(),(int)pPlayer->m_SynchedRandomSeed.GetStartRandomSeedC());
                    //// debug
                }
            }
...
void CXClient::DrawNetStats() {
    ...
    pRen->TextToScreen(10, 75, "BANDWIDTH IN=%.2f/%.2f Kbits/sec (%d) OUT=%.2f/%.2f Kbits/sec (%d)", fIncomingKbPerSec, fIncomingKbPerSec + fPacketSize * nIncomingPacketsPerSec,
                       nIncomingPacketsPerSec, fOutgoingKbPerSec, fOutgoingKbPerSec + fPacketSize * nOutgoingPacketsPerSec, nOutgoingPacketsPerSec);
...
void CXClient::SetPlayerID(EntityId wPlayerID) {
    ...
    if (pPlayer)
        ...
        m_pScriptSystem->SetGlobalTonullptr("_localplayer");
    ...
    if (pEntity) {
        ...
        if (pCnt) {
            ...
            if (pPlayer) {
                ...
                m_pLog->Log("SET PLAYER [%d]b", id);
        } else
            m_pLog->Log("SET PLAYER [%d] failed1", id);
    } else
        m_pLog->Log("SET PLAYER [%d] failed2", id);
...
bool CXClient::OnServerMsgUpdateEntity(CStream& stm) {
    ...
    if (ent) {
        ...
        if (!m_bIgnoreSnapshot) {
            ...
            } else if (ent->GetPhysics()) { // the player will update her physics herself (to preserve perfect client time-slice based simuklation)
...
void CXClient::SoundEvent(EntityId idSrc, Vec3& pos, float fRadius, float fThreat) {
    ...
    if ((fDistance2 < (fMaxDistance * fMaxDistance)) && (fDistance2 > (fMinDistance * fMinDistance))) {
