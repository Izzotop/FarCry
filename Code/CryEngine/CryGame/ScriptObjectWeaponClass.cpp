...
@param pObj table of fireparams (accuracy, ammo, max_ammo, reload_time, fire_rate, distance, damage, blast_radius, bullet_per_shot, instant, projectile_mass, projectile_size,
projectile_velocity, acc_thrust, acc_lift, air_resistance, gravity, surface_idx, projectile_class, fx, fy, bullets_per_clip)
*/
...
input: x - horizontal distance to target (always positive), y - vertical distance to target (pisitive or negative), v - initial particle velocity (scalar, positive), g - gravity
(positive scalar, assume that it vector points down)
...
int CScriptObjectWeaponClass::DrawScopeFlare(IFunctionHandler* pH) {
    ...
    if (pITable->GetValueChain("shooterid", shooterid)) {
        ...
        if (pShooter->GetContainer())
            pShooter->GetContainer()->QueryContainerInterface(CIT_IPLAYER, (void**)&pShooterPlayer);
