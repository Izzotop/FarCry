...
CWeaponClass::CWeaponClass(CWeaponSystemEx& rWeaponSystem) : m_rWeaponSystem(rWeaponSystem) {
    m_pObject = nullptr;
...
void CWeaponClass::Reset() {
    ...
    // set the global table to nullptr, and garbage collect it
    if (!m_pScriptSystem)
        return;
    ...
    m_pScriptSystem->SetGlobalTonullptr(m_sName.c_str());
...
int CWeaponClass::Fire(const Vec3d& origin, const Vec3d& angles, CPlayer* pPlayer, WeaponInfo& winfo, IPhysicalEntity* pIRedirected) {
    ...
    if (fWaterLevel > origin.z) {
        ...
        m_ssoFireTable->SetTonullptrChain("underwater");
    ...
    for (bullet = 0; bullet < m_fireParams.nBulletpershot; bullet++) {
        ...
        // take into account weapon's accuracy(the first bullet is 100% accurate
        {
            ...
            //      GetISystem()->GetILog()->Log(">> Bullet                           Bullet %d %d",(int)pPlayer->GetEntity()->GetId(),(int)((bullet<<3)+ucSeed));
            //// debug
...
#define MAX_HITS 5
        ...
        if (!bMeleeHit) {
            ...
            for (int i = 0; i < res; i++) {
                m_rWeaponSystem.GetGame()->GetSystem()->GetILog()->Log("CWeaponClass::Fire hits: dist=%.2f type=%d pos=(%.2f %.2f %.2f)", hits[i].dist,
                                                                       hits[i].pCollider ? hits[i].pCollider->GetType() : -1, hits[i].pt.x, hits[i].pt.y, hits[i].pt.z);
            }
#endif
        }
...
void CWeaponClass::ProcessHitTarget(const SWeaponHit& hit) {
    ...
    if (hit.target != nullptr) {
#ifdef FIRE_DEBUG
        m_rWeaponSystem.GetGame()->GetSystem()->GetILog()->Log("hit.target!=nullptr id=%d target=%s %x", hit.target->GetId(), (const char*)hit.target->GetName(),
                                                               hit.target->GetScriptObject());
#endif
        ...
        if (hit.target->GetScriptObject())
            ...
            m_ssoProcessHit->SetTonullptrChain("target");
        m_ssoProcessHit->SetTonullptrChain("targetStat");
        ...
        m_ssoProcessHit->SetTonullptrChain("target");
        m_ssoProcessHit->SetTonullptrChain("target_id");
    } else {
#ifdef FIRE_DEBUG
...
void CWeaponClass::ProcessHit(const SWeaponHit& hit) {
    FUNCTION_PROFILER(GetISystem(), PROFILE_GAME);
#ifdef FIRE_DEBUG
    m_rWeaponSystem.GetGame()->GetSystem()->GetILog()->Log("CWeaponClass::ProcessHit target=%s pos=(%.2f %.2f %.2f)", hit.target ? hit.target->GetName() : "nil", hit.pos.x,
                                                           hit.pos.y, hit.pos.z);
#endif
    ...
    if (hit.pos.z < fWaterLevel && vShooter.z > fWaterLevel) {
        ...
        if (pTargetMaterial = m_rWeaponSystem.GetGame()->m_XSurfaceMgr.GetMaterialByName("mat_water")) {
            ...
            m_ssoProcessHit->SetTonullptrChain("target_material");
        }
    ...
    if (pTargetMaterial = m_rWeaponSystem.GetGame()->m_XSurfaceMgr.GetMaterialBySurfaceID(hit.surface_id)) {
        ...
        if (m_nLastMaterial == hit.surface_id) {
            m_ssoProcessHit->SetTonullptrChain("play_mat_sound");
        } else {
