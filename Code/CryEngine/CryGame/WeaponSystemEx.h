...
public:
    ...
    IScriptObject* GetWeaponClassesTable() const {
        return m_soWeaponClassesTable;
    }
    ...
    unsigned int GetNumWeaponClasses() const {
        return (unsigned)m_vWeaponClasses.size();
    }
...
public:
    ...
    CProjectileClass(const EntityClassId ClassID, const string& sName, IStatObj* pObject, CWeaponSystemEx& rWeaponSystem)
        : m_ClassID(ClassID), m_sName(sName), m_pObject(pObject), m_rWeaponSystem(rWeaponSystem) {}
