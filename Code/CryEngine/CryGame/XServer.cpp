...
CXServer::~CXServer() {
    ...
    //    m_ServerRules.ShutDown();
    m_pGame->GetScriptSystem()->SetGlobalTonullptr("GameRules"); // workaround to minimize risk
    ...
    if (m_pGame->GetScriptSystem()) {
        m_pGame->GetScriptSystem()->SetGlobalTonullptr("Server");
...
void CXServer::DrawNetStats(IRenderer* pRenderer) {
    ...
    if (sv_netstats->GetIVal() & 0x1) // display net statistics
    {
        ...
        while (i != m_mapXSlots.end()) {
            ...
            pRenderer->TextToScreen(10.0f, (float)y, "[%s] PING[%d] LOST[%d] ULOST[%d] MAXSNAP=%dbits LASTSNAP=%dbits", ss.name.c_str(), ss.ping, ss.packetslost, ss.upacketslost,
                                    ss.maxsnapshotbitsize, ss.lastsnapshotbitsize);
            y += 3;
            ++i;
        }
        ...
        pRenderer->TextToScreen(10.0f, (float)y, "BANDWIDTH IN=%.2f/%.2f Kbits/sec (%d) OUT=%.2f/%.2f Kbits/sec (%d)", fIncomingKbPerSec,
                                fIncomingKbPerSec + fPacketSize * nIncomingPacketsPerSec, nIncomingPacketsPerSec, fOutgoingKbPerSec,
                                fOutgoingKbPerSec + fPacketSize * nOutgoingPacketsPerSec, nOutgoingPacketsPerSec);
        ...
        if (pSlot) {
            ...
            if (pPlayerEntity) {
                ...
                for (NetEntityListItor eit = pSlot->m_Snapshot.m_lstNetEntities.begin(); eit != pSlot->m_Snapshot.m_lstNetEntities.end(); ++eit) {
                    ...
                    if (i >= 1000)
                        i = 1000 - 1; // clamp in max range
                    if (i < 0)
                        i = 0; // clamp in min range
...
void CXServer::BroadcastCommand(const char* sCmd, const Vec3& invPos, const Vec3& invNormal, const EntityId inId, const unsigned char incUserByte) {
    // now... broadcast !
    XSlotMap::iterator i;
...
bool CXServer::GetSlotInfo(const unsigned int dwIP, SSlotInfo& info, int nameOnly) {
    ...
    if (pSlot) {
        ...
        if (*info.playerName == 0 && pSlot->CanSpawn())
            strcpy(info.playerName, "*NO NAME*");
