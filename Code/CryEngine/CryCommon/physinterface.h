...
struct pe_params_part : pe_params { // Sets geometrical parameters of entity part
    ...
    pe_params_part() {
        type = type_id;
        MARK_UNUSED pos, q, scale, partid, ipart, mass, density, pPhysGeom, pPhysGeomProxy;
        pMtx3x3 = 0;
        pMtx3x3T = 0;
        pMtx4x4 = 0;
        pMtx4x4T = 0;
...
struct pe_simulation_params : pe_params { // Sets gravity and maximum time step
    ...
    pe_simulation_params() {
        type = type_id;
        MARK_UNUSED maxTimeStep, gravity, minEnergy, damping, iSimClass, softness, softnessAngular, dampingFreefall, gravityFreefall, mass, density;
    }
...
struct pe_params_foreign_data : pe_params {
    ...
    pe_params_foreign_data() {
        type = type_id;
        MARK_UNUSED pForeignData, iForeignData, iForeignFlags;
    }
...
struct pe_params_buoyancy : pe_params {
    ...
    pe_params_buoyancy() {
        type = type_id;
        MARK_UNUSED waterDensity, waterDamping, waterPlane.n, waterPlane.origin, waterEmin, waterResistance, waterFlow;
    };
...
enum phentity_flags {
    ...
    lef_push_objects = 0x20,
    lef_push_players = 0x40,
    lef_snap_velocities = 0x80,
    lef_loosen_stuck_checks = 0x100,
    ...
    pef_monitor_impulses = 0x4000,
    pef_never_affect_triggers = 0x8000,
    pef_checksum_received = 0x10000,
    pef_checksum_outofsync = 0x20000,
...
struct pe_params_flags : pe_params {
    ...
    pe_params_flags() {
        type = type_id;
        MARK_UNUSED flags, flagsOR, flagsAND;
    }
    unsigned int flags;
...
enum joint_flags {
    angle0_locked = 1,
    all_angles_locked = 7,
    angle0_limit_reached = 010,
    angle0_auto_kd = 0100,
    joint_no_gravity = 01000,
    joint_isolated_accelerations = 02000,
    joint_expand_hinge = 04000,
    angle0_gimbal_locked = 010000
};
...
struct pe_player_dimensions : pe_params {
    ...
    pe_player_dimensions() {
        type = type_id;
        MARK_UNUSED sizeCollider, heightPivot, heightCollider, heightEye, heightHead, headRadius;
    }
...
struct pe_player_dynamics : pe_params {
    ...
    pe_player_dynamics() {
        type = type_id;
        MARK_UNUSED kInertia, kAirControl, gravity, nodSpeed, mass, bSwimming, surface_idx, bActive;
...
struct pe_params_wheel : pe_params {
    ...
    pe_params_wheel() {
        type = type_id;
        iWheel = 0;
        MARK_UNUSED bDriving, iAxle, suspLenMax, minFriction, maxFriction, surface_idx;
    }
...
struct pe_params_rope : pe_params {
    ...
    pe_params_rope() {
        type = type_id;
        MARK_UNUSED length, mass, bCheckCollisions, collDist, surface_idx, friction, nSegments, pPoints, pVelocities;
...
struct pe_params_softbody : pe_params {
    ...
    pe_params_softbody() {
        type = type_id;
        MARK_UNUSED thickness, maxSafeStep, ks, kdRatio, airResistance, wind, nMaxIters, accuracy, friction, impulseScale, explosionScale, collisionImpulseScale,
            maxCollisionImpulse, collTypes;
    }
...
struct pe_action_impulse : pe_action {
    ...
    pe_action_impulse() {
        type = type_id;
        impulse.Set(0, 0, 0);
        MARK_UNUSED point, momentum, partid, ipart;
        iApplyTime = 2;
        iSource = 0;
    }
...
struct pe_action_remove_constraint : pe_action {
    enum entype { type_id = 6 };
    pe_action_remove_constraint() {
        type = type_id;
        MARK_UNUSED idConstraint;
    }
    int idConstraint;
};
...
struct pe_action_drive : pe_action {
    ...
    pe_action_drive() {
        type = type_id;
        MARK_UNUSED pedal, dpedal, steer, dsteer, bHandBrake, clutch, iGear;
    }
...
struct pe_status_timeslices : pe_status {
    enum entype { type_id = 11 };
    pe_status_timeslices() {
        type = type_id;
        pTimeSlices = 0;
        sz = 1;
        precision = 0.0001f;
        MARK_UNUSED time_interval;
    }
...
////////// common geometries
enum geom_flags {
    geom_colltype0 = 0x0001,
    geom_colltype1 = 0x0002,
    geom_colltype2 = 0x0004,
    geom_colltype3 = 0x0008,
    geom_colltype4 = 0x0010,
    geom_colltype5 = 0x0020,
    geom_colltype6 = 0x0040,
    geom_colltype7 = 0x0080,
    geom_colltype8 = 0x0100,
    geom_colltype9 = 0x0200,
    geom_colltype10 = 0x0400,
    geom_colltype11 = 0x0800,
...
struct pe_geomparams {
    ...
    pe_geomparams() {
        ...
        q.SetIdentity();
        bRecalcBBox = 1;
        flags = geom_collides | geom_floats;
...
struct pe_cargeomparams : pe_geomparams {
    enum entype { type_id = 1 };
    pe_cargeomparams() : pe_geomparams() {
        type = type_id;
        MARK_UNUSED bDriving, minFriction, maxFriction;
        bCanBrake = 1;
    }
...
public:
    ...
    virtual int FindClosestPoint(geom_world_data* pgwd, int& iPrim, int& iFeature, const vectorf& ptdst0, const vectorf& ptdst1, vectorf* ptres, int nMaxIters = 10) = 0;
    virtual void CalcVolumetricPressure(geom_world_data* gwd, const vectorf& epicenter, float k, float rmin, const vectorf& centerOfMass, vectorf& P, vectorf& L) = 0;
...
enum meshflags {
    ...
    mesh_AABB = 2,
    mesh_SingleBB = 4,
    mesh_multicontact0 = 8,
    mesh_multicontact1 = 16,
    mesh_multicontact2 = 32,
    mesh_approx_cylinder = 0x100,
    mesh_approx_box = 0x200,
    mesh_approx_sphere = 0x400,
    mesh_uchar_ids = 0x1000
};
...
class IPhysUtils {
public:
    virtual int BreakPolygon(vector2df* ptSrc, int nPt, int nCellx, int nCelly, int maxPatchTris, vector2df*& ptout, int*& nPtOut, float jointhresh = 0.5f, int seed = -1) = 0;
    virtual int CoverPolygonWithCircles(strided_pointer<vector2df> pt, int npt, bool bConsecutive, const vector2df& center, vector2df*& centers, float*& radii,
                                        float minCircleRadius) = 0;
...
public:
    virtual void OnBBoxOverlap(IPhysicalEntity* pEntity, void* pForeignData, int iForeignData, IPhysicalEntity* pCollider, void* pColliderForeignData,
                               int iColliderForeignData) = 0;
...
enum rwi_flags {
    ...
    rwi_separate_important_hits = sf_important,
    rwi_colltype_bit = 16
};
...
public:
    ...
    */
    ...
    virtual IPhysicalEntity* CreatePhysicalEntity(pe_type type, float lifeTime, pe_params* params = 0, void* pForeignData = 0, int iForeignData = 0, int id = -1,
                                                  IPhysicalEntity* pHostPlaceholder = 0) = 0;
    ...
    */
    virtual int RayWorldIntersection(vectorf org, vectorf dir, int objtypes, unsigned int flags, ray_hit* hits, int nMaxHits, IPhysicalEntity* pSkipEnt = 0,
                                     IPhysicalEntity* pSkipEntAux = 0) = 0;
    ...
    */
    virtual void SimulateExplosion(vectorf epicenter, vectorf epicenterImp, float rmin, float rmax, float r, float impulsive_pressure_at_r, int nOccRes = 0, int nGrow = 0,
                                   float rmin_occ = 0.1f, IPhysicalEntity** pSkipEnts = 0, int nSkipEnts = 0,
