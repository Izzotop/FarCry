...
enum EFindName { eFN_Find = 0, eFN_Add, eFN_Intrinsic };
...
public:
    ...
    CName& operator=(const CName& cFN) {
        mNameIndex = cFN.mNameIndex;
        return *this;
    }
    const char* operator*() const {
        return mNames[mNameIndex]->mName;
    }
    int operator!=(const CName& cFN) const {
        return (cFN.mNameIndex != mNameIndex);
    }
    int operator==(const CName& cFN) const {
        return (cFN.mNameIndex == mNameIndex);
    }
    ...
    unsigned long mfGetFlags() const {
        return mNames[mNameIndex]->mFlags;
    }
    ...
    void mfClearFlags(unsigned int flag) {
        mNames[mNameIndex]->mFlags &= ~flag;
    }
    void mfSetFlags(unsigned int flag) {
        mNames[mNameIndex]->mFlags |= flag;
    }
    int mfGetIndex() const {
        return mNameIndex;
    }
    int mfIsValid() {
    ...
    void mfInitTables();
    static void mfDisplayHash();
    static void mfExitSubsystem();
    static void mfInitSubsystem();
    static SNameEntry* mfGetEntry(int i) {
        return mNames[i];
    }
    void mfRegister(SNameEntry&);
    static int mfGetMaxNames() {
        return mNames.Num();
    }
    static int Size();
