...
struct CryCharMorphParams {
    CryCharMorphParams(float _fBlendIn = 0.15f, float _fLength = 0, float _fBlendOut = 0.15f, float _fAmplitude = 1, float _fStartTime = 0, float _fSpeed = 1, unsigned _nFlags = 0)
        : fBlendIn(_fBlendIn), fLength(_fLength), fBlendOut(_fBlendOut), fAmplitude(_fAmplitude), fStartTime(_fStartTime), fSpeed(_fSpeed), nFlags(_nFlags) {}
    // the blend-in time
    float fBlendIn;
