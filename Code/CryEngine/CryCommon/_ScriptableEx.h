...
template <class T> class _ScriptableEx {
public:
    _ScriptableEx() {
        // m_nFuncNum=0;
    ...
    enum PropertyType { PROPERTY_TYPE_BOOL, PROPERTY_TYPE_INT, PROPERTY_TYPE_CHAR, PROPERTY_TYPE_FLOAT, PROPERTY_TYPE_STRING, PROPERTY_TYPE_OBJECT };
    struct Property {
        Property() {
            val = 0;
        }
...
protected:
    static int FuncThunk(HSCRIPT h) {
        ...
        if (!pThis) {
            m_pSS->RaiseError("nullptr Self");
            ::OutputDebugString("nullptr Self\n");
            return 0;
        }
...
#else
#define _DECLARE_SCRIPTABLEEX(_class)                                                                                                                                              \
    IFunctionHandler* _ScriptableEx<_class>::m_pFunctionHandler = nullptr;                                                                                                            \
