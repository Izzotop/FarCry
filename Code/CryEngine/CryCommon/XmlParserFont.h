...
public:
    ...
    bool parse() {
        ...
        // if nullptr tag no data or last tag
        if (hasnullptrTag()) {
            // update cur position
        ...
        // parse the value if not a nullptr tag
        if (!hasnullptrTag())
            parseValue();
    ...
    bool parse(string& /*name*/, string& /*value*/, string& /*attributes*/, long& /*current*/) {
...
protected:
    ...
    bool parseName() {
        ...
        if (_firstTagStart < 0 || _firstTagEnd < 0 || _firstTagEnd <= _firstTagStart) {
            _nameStart = -1;
            _nameEnd = -1;
            return false;
        }
        ...
        // if nullptr tag then backup before
        // nullptr tag marker
        if (hasnullptrTag())
            _nameEnd -= 1;
    ...
    bool parseAttributes() {
        ...
        // no attribute case but has nullptr tag
        case 3:
            return false;
        }
        ...
        // if nullptr tag move before nullptr tag marker
    ...
    bool parseValue() {
        // if first tag search failed show failed
        if (_firstTagStart < 0 || _lastTagEnd < 0 || _lastTagEnd <= _firstTagStart) {
            _valueStart = -1;
            _valueEnd = -1;
            return false;
        }
...
public:
    ...
    string getName() {
        ...
        // if length invalid show nullptr string
        // else get string
        if (length <= 0)
            return string("");
        else
    ...
    long getLength(long startPos, long endPos) {
