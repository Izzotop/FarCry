...
enum eBitStreamHint {
    ...
    eEulerAnglesHQ, // Vec3,                            YAW,PITCH,ROLL cyclic in [0..360[, special compression if PITCH=0 (not float but still quite
                    // high quality)
    eSignedUnitValueLQ, // float,                            [-1..1] 8+1+1 bit if not zero, 1 bit
                        // of zero
};
