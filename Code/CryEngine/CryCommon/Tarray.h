...
template <typename T> class TAllocator16 {
public:
    ...
    TAllocator16()
        ...
        : m_szParentObject("TAllocator16"), m_nParentIndex(1)
#endif
    {
    }
...
public:
    ...
    TArray() {
        m_pElements = nullptr, m_nCount = m_nAllocatedCount = 0;
    }
    TArray(int Count) {
        m_pElements = nullptr;
        m_nCount = Count;
...
#error !!!!!
  ...
  */
    ...
    void push_back(const T& rSample) {
        Add(rSample);
    }
    T* begin() {
        return m_pElements;
    ...
    int GetMemoryUsage() {
        return (int)(m_nAllocatedCount * sizeof(T));
    }
};
