...
public:
    ...
    template <typename T> bool Add(const T* pId, size_t num) {
        return AddObject(pId, num * sizeof(T));
    }

    template <class T> bool Add(const T& rObject) {
    ...
    template <class String> bool AddString(const String& strText) {
        if (!strText.empty())
...
#ifdef _XSTRING_
    template <class Elem, class Traits, class Allocator> bool Add(const std::basic_string<Elem, Traits, Allocator>& strText) {
        AddString(strText);
    }
#endif
    ...
    template <class Container> bool AddContainer(const Container& rContainer) {
        if (!rContainer.empty())
...
public:
    ...
    CrySizerComponentNameHelper(ICrySizer* pSizer, const char* szComponentName, bool bSubcomponent) : m_pSizer(pSizer) {
        if (bSubcomponent)
