...
struct SMRendVert {
    SMRendVert() {}
    SMRendVert(float x, float y, float z) {
        vert[0] = x;
        vert[1] = y;
        vert[2] = z;
    }
...
public:
    ...
    void mfSetFlags(uint fl) {
        m_Flags = fl;
    }
    void mfUpdateFlags(uint fl) {
        m_Flags |= fl;
    }
    void mfClearFlags(uint fl) {
        m_Flags &= ~fl;
    }
