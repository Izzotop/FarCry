...
class CCFFileWriter {
public:
    CCFFileWriter(FILE* f) : m_pFile(f), m_nLastChunk(-1) {}

    CCFFileWriter() : m_pFile(nullptr), m_nLastChunk(-1) {}
...
public:
    ...
    CCFFileReader(FILE* f) : m_pFile(f) {
        Reset();
    }
...
public:
    ...
    // ASSUMES: the file pointer is exactly at some chunk header
    CCFMemReader(const void* pData, unsigned nSize) : m_pData((const char*)pData), m_pEnd((const char*)pData + nSize) {
        Reset();
    }
