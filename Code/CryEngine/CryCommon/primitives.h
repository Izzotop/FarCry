...
struct grid : primitive {
    ...
    int inrange(int ix, int iy) {
        return isneg(ix - size.x) & isnonneg(ix) & isneg(iy - size.y) & isnonneg(iy);
    }
    int getcell_safe(int ix, int iy) {
        int mask = -inrange(ix, iy);
        return iy * stride.y + ix * stride.x & mask | size.x * size.y & ~mask;
    }
};
...
struct heightfield : grid {
    ...
    float getheight(int ix, int iy) const {
        return getheight(ix * stride.x + iy * stride.y);
    }
    int gettype(int ix, int iy) const {
        return gettype(ix * stride.x + iy * stride.y);
    }
    float getheight(int icell) const {
        return pdata[icell] * heightscale;
    }
