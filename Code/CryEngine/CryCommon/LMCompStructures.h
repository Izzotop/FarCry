...
typedef enum ELMMode { ELMMode_ALL = 0, ELMMode_CHANGES, ELMMode_SEL } ELMMode;
...
struct RenderLMData : public _reference_target_t {
    ...
    int GetColorLerpTex() {
        return m_iColorLerpTex;
    };
    int GetHDRColorLerpTex() {
        return m_iHDRColorLerpTex;
    };
    int GetDomDirectionTex() {
        return m_iDomDirectionTex;
    };
    int GetOcclTex() {
        return m_iOcclTex;
    };
...
struct TexCoord2Comp {
    TexCoord2Comp(float _s, float _t) {
        s = _s;
        t = _t;
    };
...
typedef struct SOcclusionMapTexel {
    ...
    const SOcclusionMapTexel& operator=(const SOcclusionMapTexel& rTexel) {
        colour = rTexel.colour;
        return *this;
    }
    const bool operator==(const SOcclusionMapTexel& rTexel) const {
        return rTexel.colour == colour;
    }
    const bool operator!=(const SOcclusionMapTexel& rTexel) const {
        return rTexel.colour != colour;
    }
...
typedef struct GLMOcclLightInfo {
    ...
    const unsigned int UsedChannelCount() const {
        return uiLightCount;
    }
