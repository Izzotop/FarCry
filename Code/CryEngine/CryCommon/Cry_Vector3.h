...
public:
    ...
    // bracket operator
    void operator()(const F vx, const F vy, const F vz) {
        x = vx;
        y = vy;
        z = vz;
    };
    ...
    template <class T> explicit Vec3_tpl(const T* src) {
        x = src[0];
        y = src[1];
        z = src[2];
    }
    ...
    template <class T> ILINE Vec3_tpl(const Vec3_tpl<T>& v) {
        x = (F)v.x;
        y = (F)v.y;
        z = (F)v.z;
    }
    ...
    template <class T> ILINE Vec3_tpl& operator=(const Vec3_tpl<T>& v) {
        x = v.x;
        y = v.y;
        z = v.z;
        return *this;
    ...
    ILINE F operator[](int index) const {
        assert(index >= 0 && index <= 2);
        return ((F*)this)[index];
    }
    ...
    ILINE friend Vec3_tpl operator*(f32 f, const Vec3_tpl& vec) {
        return Vec3_tpl(f * vec.x, f * vec.y, f * vec.z);
    }
    ...
    // flip vector
    ILINE Vec3_tpl<F> operator-() const {
    ...
    ILINE f32 Normalize() {
        ...
        // assert(fLen>0.0f);
        if (fLen < 0.00001f)
            return (0); // no good idea! not everybody will check this
    ...
    ILINE Vec3_tpl GetNormalized() const {
        return normalized();
    } // vlad: all get functions should begin from Get...
     ...
     */
    ILINE static Vec3_tpl CreateProjection(const Vec3_tpl& i, const Vec3_tpl& n) {
        return i - n * (n | i);
    }
     ...
     */
    ILINE static Vec3_tpl CreateReflection(const Vec3_tpl& i, const Vec3_tpl& n) {
        return (n * (i | n) * 2) - i;
    }
};
...
inline Vec3_tpl<f32>::Vec3_tpl(type_min) {
    x = y = z = -3.3E38f;
}
inline Vec3_tpl<f32>::Vec3_tpl(type_max) {
    x = y = z = 3.3E38f;
}
inline Vec3_tpl<f64>::Vec3_tpl(type_min) {
    x = y = z = -1.7E308;
}
inline Vec3_tpl<f64>::Vec3_tpl(type_max) {
    x = y = z = 1.7E308;
}

template <class F> ILINE F GetLengthSquared(const Vec3_tpl<F>& v) {
    return v.x * v.x + v.y * v.y + v.z * v.z;
}
template <class F> ILINE F GetLength(const Vec3_tpl<F>& v) {
    return sqrt_tpl(v.x * v.x + v.y * v.y + v.z * v.z);
}
...
// returns a vector orthogonal to this one
template <class F> ILINE Vec3_tpl<F> GetOrthogonal(const Vec3_tpl<F>& v) {
    int i = isneg(square((F)0.9) * GetLengthSquared(v) - v.x * v.x);
    Vec3_tpl<F> res;
    res[i] = 0;
    res[incm3(i)] = v[decm3(i)];
    res[decm3(i)] = -v[incm3(i)];
    return res;
}
...
template <class F> struct Ang3_tpl : public Vec3_tpl<F> {
    Ang3_tpl() {}

    Ang3_tpl(const Vec3& v) {
        this->x = v.x;
        this->y = v.y;
        this->z = v.z;
    }
    Ang3_tpl& operator=(const Vec3& v) {
        this->x = v.x;
        this->y = v.y;
        this->z = v.z;
        return *this;
    }
    ILINE Ang3_tpl(const F vx, const F vy, const F vz) {
        this->x = vx;
        this->y = vy;
        this->z = vz;
    }
    ...
    ILINE void Rad2Deg() {
        this->x = RAD2DEG(this->x);
        this->y = RAD2DEG(this->y);
        this->z = RAD2DEG(this->z);
    }
    //! convert from degrees to radians
    // ILINE void  Deg2Rad() {   this->x=DEG2RAD(this->x);   this->y=DEG2RAD(this->y);   this->z=DEG2RAD(this->z);  }

    template <int SI, int SJ> ILINE void SetAnglesXYZ(const Matrix33_tpl<F, SI, SJ>& m) {
    ...
    template <int SI, int SJ> ILINE static Ang3_tpl<F> GetAnglesXYZ(const Matrix33_tpl<F, SI, SJ>& m) {
        Ang3_tpl<F> a;
        a.SetAnglesXYZ(m);
        return a;
    }
...
ILINE Ang3 Deg2Rad(const Ang3& a) {
    return Ang3(DEG2RAD(a.x), DEG2RAD(a.y), DEG2RAD(a.z));
}
...
template <class F> struct AngleAxis_tpl {
    ...
    AngleAxis_tpl(){};
    AngleAxis_tpl(const f32 a, const f32 ax, const f32 ay, const f32 az) {
        angle = a;
        axis.x = ax;
        axis.y = ay;
        axis.z = az;
    }
    AngleAxis_tpl(const f32 a, const Vec3_tpl<F>& n) {
        angle = a;
        axis = n;
    }
    void operator()(const f32 a, const Vec3_tpl<F>& n) {
        angle = a;
        axis = n;
    }
 ...
 *  CAngleAxis aa=mat33;
 */
template <class F> ILINE AngleAxis_tpl<F>::AngleAxis_tpl(const Quaternion_tpl<F>& q) {
 ...
 *   Vec3 result = aa*v;
 */
template <class F> ILINE const Vec3_tpl<F> AngleAxis_tpl<F>::operator*(const Vec3_tpl<F>& v) const {
...
struct Plane {
    ...
    ILINE Plane(const Vec3& normal, const f32& distance) {
        n = normal;
        d = distance;
    }
     ...
     *
     */
    ILINE f32 operator|(const Vec3& point) const {
        return (n | point) + d;
    }
