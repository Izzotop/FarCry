...
#define STREAM_VERIFY_TYPE_READ(a)                                                                                                                                                 \
    {                                                                                                                                                                              \
        if ((GetISystem()->GetStreamEngine()->GetStreamCompressionMask() & 0x80) != 0) {                                                                                           \
            unsigned char t1 = 0, t2 = (a);                                                                                                                                        \
            _Read(t1);                                                                                                                                                             \
            if (t1 != t2) {                                                                                                                                                        \
                char str[256];                                                                                                                                                     \
                sprintf(str, "Entity read typedcookie error %d!=%d file '%s' @ line %d", (DWORD)t1, (DWORD)t2, __FILE__, __LINE__);                                                \
                Debug(str);                                                                                                                                                        \
                assert(0);                                                                                                                                                         \
                return false;                                                                                                                                                      \
            }                                                                                                                                                                      \
...
public:
    ...
    bool WritePacked(size_t sz) {
        return WritePacked((unsigned int)sz);
    }
    bool ReadPacked(size_t& sz) {
        return ReadPacked((unsigned int&)sz);
    }
#endif
    ...
    bool WriteData(const void* pBuffer, int nSize) {
        return WriteBits((BYTE*)pBuffer, nSize * 8);
    };
    //! read data from stream.
    //! @param nSize size of buffer in bytes.
    bool ReadData(void* pBuffer, int nSize) {
        return ReadBits((BYTE*)pBuffer, nSize * 8);
    };
    ...
    int GetStreamVersion() const {
        return m_iStreamVersion;
    }
    //! used for to be able to load old savegames
    void SetStreamVersion(const int iVersion) {
        m_iStreamVersion = iVersion;
    }

private: // -----------------------------------------------------------------------
    unsigned int __htonl(unsigned int n) {
        return (unsigned int)(((n & 0xFF000000) >> 24) | ((n & 0x00FF0000) >> 8) | ((n & 0x0000FF00) << 8) | ((n & 0x000000FF) << 24));
    }
...
inline bool CStream::ReadNumberInBits(unsigned int& n, size_t nSize) {
    ...
    if (!ReadBits((BYTE*)&nSwapped, nSize))
        return false;
    n = __ntohl(nSwapped);
    n = n >> (32 - nSize);
    return true;
...
#define VERIFY_ENTITY_COOKIE_NO(stm, c)                                                                                                                                            \
    {                                                                                                                                                                              \
        if ((GetISystem()->GetStreamEngine()->GetStreamCompressionMask() & 0x8) == 0 || !GetISystem()->GetIGame()->GetModuleState(EGameMultiplayer)) {                             \
            ...
            if (cCookie != (BYTE)c) {                                                                                                                                              \
                stm.Debug();                                                                                                                                                       \
                m_pISystem->GetILog()->LogError("Entity read cookie error %x!=%x file '%s' @ line %d (%s of class %s)", (DWORD)cCookie, (DWORD)c, __FILE__, __LINE__, GetName(),   \
                                                GetEntityClassName());                                                                                                             \
                return false;                                                                                                                                                      \
            }                                                                                                                                                                      \
        }                                                                                                                                                                          \
