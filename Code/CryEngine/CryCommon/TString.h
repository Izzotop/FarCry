...
public:
    ...
    static char* strcat(const char* szLeft, const char* szRight) {
        if (szLeft[0] || szRight[0]) {
            ...
            // copy together with the terminating nullptr
    ...
    CryBasicString() : m_pString(nullptr) {}
...
protected:
    // nullptr if the string is empty
    char* m_pString;
#ifdef _DEBUG
...
template <> inline const char* constchar_cast(const CryBasicString& type) {
    return type.c_str();
}
} // namespace stl
