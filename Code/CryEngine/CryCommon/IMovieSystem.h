...
struct SAnimParam {
    ...
    SAnimParam(const char* _name, AnimParamType _id, IAnimTrack* pTrack = 0) {
        strcpy(name, _name);
        id = _id;
        track = pTrack;
    }
    SAnimParam(const SAnimParam& anim) {
        *this = anim;
    }
...
//! Callback-reasons
enum ECallbackReason { CBR_ADDNODE, CBR_REMOVENODE, CBR_CHANGENODE, CBR_REGISTERNODECB, CBR_UNREGISTERNODECB };
