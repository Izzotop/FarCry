...
struct AnimSinkEventData {
    AnimSinkEventData(void* _p = nullptr) : p(_p), n((INT_PTR)_p) {}
    ...
    bool operator==(const AnimSinkEventData& that) const {
        return p == that.p && n == that.n;
    }
};
...
enum AnimTwinMode { AnimTwinMode_Replace, AnimTwinMode_Add };
...
struct IAnimationSet {
    ...
    // Arguments:
    //   szAnimationName - nullptr terminated string holding the name of the animation.
    // Return Value:
    ...
    // Return Value:
    //   A nullptr terminated string holding the name of the animation. In case the
    ...
    // Return Value:
    //   A nullptr terminated string holding the name of the morph target. In case
    //   the specified id is out of range, the string
    //   "!MORPH TARGET ID OUT OF RANGE!" is returned.
    virtual const char* GetNameMorphTarget(int nMorphTargetId) {
...
struct ICryCharModel : public _i_reference_target_t {
    ...
    //     nId - Id of the bone
    // Return Value:
    //     A nullptr terminated char array which hold the name. If no bones of the specified id have been found, nullptr is returned.
    // Summary:
    ...
    enum ClassEnum { CLASS_UNKNOWN, CLASS_ANIMOBJECT, CLASS_CRYCHARBODY };
    ...
    // Return Value:
    //     A pointer to a nullptr terminated char string which contain the
    ...
    // Return Value:
    //     A pointer to a nullptr terminated char string which contain the value
    //     of the requested property.
    //     nullptr will be returned if there isn't any property attached with the
    //     specified name.
    // Summary:
    //     Get property which was set for the model
    virtual const char* GetProperty(const char* szName) {
        return "";
    }
};
...
struct ICryCharInstance : public IBindable {
    ...
    virtual bool ExecScriptCommand(int nCommand, void* pParams, void* pResult) {
        return false;
    }
    ...
    // Return Value:
    //     nullptr terminated char string which contain the name of the shader template
    // Summary:
    ...
    virtual const list2<struct CMatInfo>* getLeafBufferMaterials() {
        return nullptr;
    }
    ...
    //! Detaches all objects beforehand; if the object is nullptr, just detaches all objects from the bone
    ...
    // Return Value:
    //   An integer representing the number of binding detached.
    virtual unsigned DetachAllFromBone(unsigned nBone) {
        return 0;
    }
    ...
    virtual ObjectBindingHandle AttachToBone(IBindable* pObj, unsigned nBone, unsigned nFlags = 0) {
    ...
    virtual bool Detach(ObjectBindingHandle nHandle) {
        return false;
    }
    ...
    virtual bool IsBindingValid(ObjectBindingHandle nHandle) {
        return false;
    }
    ...
    enum UpdateEnum { flagDontUpdateBones = 1, flagDontUpdateAttachments = 1 << 1 };
    ...
    */
    ...
    virtual int AddParticleEmitter(struct ParticleParams& rInfo, const struct CryParticleSpawnInfo& rSpawnInfo) {
        return -1;
    }
    ...
    virtual ICryCharSubmesh* NewSubmesh(ICryCharModel* pModel, bool bVisible = false) {
        return nullptr;
    }
    // adds submesh to the specified slot; replaces submesh if there's some there
    virtual ICryCharSubmesh* NewSubmesh(unsigned nSlot, ICryCharModel* pModel, bool bVisible = false) {
        return nullptr;
    }
    ...
    virtual size_t NumSubmeshes() {
        return 0;
    }
    //! returns submesh from the i-th slot. Some submesh slots may be empty (nullptr)
    virtual ICryCharSubmesh* GetSubmesh(unsigned i) {
        return nullptr;
    }

    virtual ICryCharFxTrail* NewFxTrail(unsigned nSlot, const struct CryCharFxTrailParams&) {
        return nullptr;
    }
    virtual void RemoveFxTrail(unsigned nSlot) {}

    // returns the pointer to this character instance, if it's a character instance
    virtual struct ICryCharInstance* GetICryCharInstance() {
        return this;
    }
};
...
struct ICryCharManager {
    ...
    // Summary:
    //     Executes a script command
    virtual bool ExecScriptCommand(int nCommand, void* pParams = nullptr, void* pResult = nullptr) {
        return false;
    }
