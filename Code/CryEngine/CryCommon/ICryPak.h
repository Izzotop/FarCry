...
struct ICryArchive : public _reference_target_t {
    // Compression methods
    enum ECompressionMethods { METHOD_STORE = 0, METHOD_COMPRESS = 8, METHOD_DEFLATE = 8 };
    ...
    // Returns:
    //   nullptr if the file doesn't exist
...
struct ICryPak {
    ...
    enum EPathResolutionRules {
        ...
        // Search only in MODs; if not found, return nullptr; may not be used with FLAGS_IGNORE_MOD_DIRS
    ...
    // returns the pointer to the constructed path (can be either szSourcePath, or szDestPath, or nullptr in case of error
    ...
    virtual intptr_t FindFirst(const char* pDir, struct _finddata_t* fd) = 0;
    ...
    // returns nullptr if the file is a physical file, and "" if the path to archive is unknown (shouldn't ever happen)
