...
struct CLeafBuffer {
    ...
    _inline CLeafBuffer* GetVertexContainer() {
    ...
    virtual void InvalidateVideoBuffer(int flags = -1) {
        m_UpdateVBufferMask |= flags;
    };
    ...
    // compact buffer
    int FindInBuffer(struct_VERTEX_FORMAT_P3F_N_COL4UB_TEX2F& opt, SPipTangents& origBasis, uint nMatInfo, uint* uiInfo, struct_VERTEX_FORMAT_P3F_N_COL4UB_TEX2F* _vbuff,
    ...
    bool IsEmpty() {
        return !m_SecVertCount || !m_pSecVertBuffer || !m_SecIndices.Num();
    }
    virtual void RenderDebugLightPass(const Matrix44& mat, int nLightMask, float fAlpha);
    virtual void Render(const struct SRendParams& rParams, CCObject* pObj, TArray<int>& ShaderTemplates, int e_overlay_geometry, bool bNotCurArea, IMatInfo* pMaterial,
