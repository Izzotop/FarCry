...
enum EERType { eERType_Unknown, eERType_Brush, eERType_Vegetation };

struct OcclusionTestClient {
    OcclusionTestClient() {
        memset(this, 0, sizeof(OcclusionTestClient));
        bLastResult = true;
    }
...
struct IEntityRenderState {
    ...
    struct ShadowMapInfo {
        ShadowMapInfo() {
            memset(this, 0, sizeof(ShadowMapInfo));
        }
...
struct IEntityRender {
    ...
    virtual class CDLight* GetLight() {
        return 0;
    }
    virtual struct IEntityContainer* GetContainer() const {
        return 0;
    }
    ...
    // rendering flags
    virtual void SetRndFlags(unsigned int dwFlags) {
        m_dwRndFlags = dwFlags;
    }
    ...
    virtual unsigned int GetRndFlags() {
        return m_dwRndFlags;
    }
    int m_dwRndFlags;
    ...
    void SetDrawFrame(int nFrameID, int nRecursionLevel) {
        m_narrDrawFrames[nRecursionLevel] = nFrameID;
    }
    int GetDrawFrame(int nRecursionLevel = 0) const {
        return m_narrDrawFrames[nRecursionLevel];
    }
    int m_narrDrawFrames[2];

    // shadow draw frames (set if was drawn)
    void SetShadowFrame(unsigned short nFrameID, int nRecursionLevel) {
        m_narrShadowFrames[nRecursionLevel] = nFrameID;
    }
    unsigned short GetShadowFrame(int nRecursionLevel = 0) const {
        return m_narrShadowFrames[nRecursionLevel];
    }
    ...
    IEntityRenderState*& GetEntityRS() {
        return m_pEntityRenderState;
    }
    ...
    // Summary:
    //   Assigns a texture set reference for dot3 lightmapping. The object will Release() it at the end of its lifetime, special call from lightmap serializer/compiler to set
    //   occlusion map values
    virtual void SetLightmap(RenderLMData* pLMData, float* pTexCoords, UINT iNumTexCoords, const unsigned char cucOcclIDCount,
                             const std::vector<std::pair<EntityId, EntityId>>& aIDs){};
    ...
    //  Returns:
    //    Lightmap texture set for this object, or nullptr if there's none assigned. Don't release obtained copy, it's not a reference
    //  See Also:
    //    SetLightmap
    virtual RenderLMData* GetLightmap(int nLod) {
        return 0;
    };
    // Summary:
    //   Returns vertex buffer holding instance specific texture coordinate set for dot3 lightmaps
    virtual struct CLeafBuffer* GetLightmapTexCoord(int nLod) {
        return 0;
    };
    ...
    // Returns:
    //   Current VisArea or nullptr if in outdoors or entity was not registered in 3dengine
    IVisArea* GetEntityVisArea() {
        return (IVisArea*)m_pVisArea;
    }
    ...
    void SetViewDistRatio(int nViewDistRatio) {
        ucViewDistRatio = min(254, max(0, nViewDistRatio));
    }
    ...
    int GetViewDistRatio() {
        return (ucViewDistRatio == 255) ? 1000l : ucViewDistRatio;
    }

    //! return max view distance ratio
    virtual float GetViewDistRatioNormilized() {
        return 0.01f * GetViewDistRatio();
    }
    ...
    void SetLodRatio(int nLodRatio) {
        ucLodRatio = min(255, max(0, nLodRatio));
    }

    //! return lod distance ratio
    float GetLodRatioNormilized() {
        return 0.01f * ucLodRatio;
    }
    ...
    virtual int DestroyPhysicalEntityCallback(IPhysicalEntity* pent) {
        return 0;
    }
    ...
    virtual EERType GetEntityRenderType() {
        return eERType_Unknown;
    }
