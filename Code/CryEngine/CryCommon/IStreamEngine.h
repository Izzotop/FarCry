 ...
 * therefore the client allocates the buffer (to avoid extra copy). All the data transformations should take place in the Resource Compiler. If you have to allocate a lot of small
 * memory objects, you should revise this strategy in favor of one big allocation (again, that will be read directly from the compiled file). Anyway, we can negotiate that the
 * streaming engine allocates this memory. In the end, it could make use of a memory pool, and copying data is not the bottleneck in our engine
 *
...
struct StreamReadParams {
    StreamReadParams(
        ...
        DWORD_PTR _dwUserData = 0, int _nPriority = 0, unsigned _nLoadTime = 0, unsigned _nMaxLoadTime = 0, unsigned _nOffset = 0, unsigned _nSize = 0, void* _pBuffer = nullptr,
        unsigned _nFlags = 0)
        : // szFile (_szFile),
          // pCallback(_pCallback),
          dwUserData(_dwUserData), nPriority(_nPriority), nLoadTime(_nLoadTime), nMaxLoadTime(_nMaxLoadTime), pBuffer(_pBuffer), nOffset(_nOffset), nSize(_nSize), nFlags(_nFlags) {
    }
    ...
    // if this is nullptr, the streaming engine will supply the buffer
