...
inline char* CIPAddress::GetAsString(bool bPort) const {
    ...
    if (bPort)
#ifndef LINUX
        wsprintf(s, "%i.%i.%i.%i:%i", m_Address.sin_addr.S_un.S_un_b.s_b1, m_Address.sin_addr.S_un.S_un_b.s_b2, m_Address.sin_addr.S_un.S_un_b.s_b3,
                 m_Address.sin_addr.S_un.S_un_b.s_b4, __ntohs(m_Address.sin_port));
#else  // LINUX
        sprintf(s, "%s:%i", inet_ntoa(m_Address.sin_addr), ntohs(m_Address.sin_port));
#endif // LINUX
    else
#ifndef LINUX
        wsprintf(s, "%i.%i.%i.%i", m_Address.sin_addr.S_un.S_un_b.s_b1, m_Address.sin_addr.S_un.S_un_b.s_b2, m_Address.sin_addr.S_un.S_un_b.s_b3,
                 m_Address.sin_addr.S_un.S_un_b.s_b4);
#else  // LINUX
        sprintf(s, "%s", inet_ntoa(m_Address.sin_addr));
...
inline bool CIPAddress::operator==(const CIPAddress& s1) {
    return memcmp(&s1.m_Address, &m_Address, sizeof(_SOCKADDR_IN)) == 0;
}
...
inline bool CIPAddress::operator!=(const CIPAddress& s1) {
    return memcmp(&s1.m_Address, &m_Address, sizeof(sockaddr_in)) != 0;
}
