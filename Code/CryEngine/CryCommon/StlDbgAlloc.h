...
template <class T> class dbg_allocator : public TSimpleAllocator<T> { // generic allocator for objects of class T
public:
    ...
    template <class _Other> struct rebind { // convert an allocator<T> to an allocator <_Other>
    ...
    dbg_allocator<T>(const char* szParentObject = "cry_dbg_alloc", int nParentIndex = 0)
        : m_szParentObject(szParentObject), m_nParentIndex(nParentIndex) { // construct default allocator (do nothing)
    }

    dbg_allocator(const dbg_allocator<T>& rThat) { // construct by copying (do nothing)
    ...
    template <class _Other> dbg_allocator<T>& operator=(const dbg_allocator<_Other>& rThat) { // assign from a related allocator (do nothing)
        m_szParentObject = rThat.m_szParentObject;
        m_nParentIndex = rThat.m_nParentIndex;
        return (*this);
    }
    ...
    void construct(pointer _Ptr, const T& _Val) { // construct object at _Ptr with value _Val
    ...
    _SIZT max_size() const { // estimate maximum array size
...
template <class _Ty, class _Other> inline bool operator==(const dbg_allocator<_Ty>& rLeft, const dbg_allocator<_Other>& rRight) { // test for allocator equality (always true)
    return rLeft.m_szParentObject == rRight.m_szParentObject && rLeft.m_nParentIndex == rRight.m_nParentIndex;
}

template <class _Ty, class _Other> inline bool operator!=(const dbg_allocator<_Ty>&, const dbg_allocator<_Other>&) { // test for allocator inequality (always false)
    return rLeft.m_szParentObject != rRight.m_szParentObject && rLeft.m_nParentIndex != rRight.m_nParentIndex;
}
...
template <class T> class dbg_struct_allocator : public TSimpleStructAllocator<T> { // generic allocator for objects of class T
public:
    dbg_struct_allocator<T>() {}

    dbg_struct_allocator<T>(const char* szParentObject, int nParentIndex = 0)
        : TSimpleStructAllocator<T>(szParentObject, nParentIndex) { // construct default allocator (do nothing)
    }

    dbg_struct_allocator<T>(const dbg_struct_allocator<T>& rThat) : TSimpleStructAllocator<T>(rThat) { // construct by copying (do nothing)
    }

    template <class _Other> dbg_struct_allocator(const dbg_struct_allocator<_Other>& rThat) : TSimpleStructAllocator<T>(rThat) { // construct from a related allocator (do nothing)
    }

    template <class _Other> dbg_struct_allocator<T>& operator=(const dbg_struct_allocator<_Other>& rThat) { // assign from a related allocator (do nothing)
        *static_cast<TSimpleStructAllocator<T>*>(this) = rThat;
        return (*this);
    }
};
#else
template <class T> class dbg_allocator : public std::allocator<T> {
public:
    dbg_allocator(const char* szSource = "CryDbgAlloc", int nSource = 0) { // construct default allocator (do nothing)
    }

    dbg_allocator(const dbg_allocator<T>& rThat) : std::allocator<T>(rThat) { // construct by copying (do nothing)
    }
    /*
    template<class _Other>
    dbg_allocator(const dbg_allocator<_Other>&rThat):
    ...
    */
    template <class _Other> dbg_allocator(const std::allocator<_Other>& rThat) : std::allocator<T>(rThat) { // construct from a related allocator (do nothing)
    }

    template <class _Other> dbg_allocator<T>& operator=(const std::allocator<_Other>& rThat) : std::allocator<T>(rThat) { // assign from a related allocator (do nothing)
        return (*this);
    }
};
#endif
...
template <class Key, class Type, class Traits = std::less<Key>> class map : public std::map<Key, Type, Traits, dbg_allocator<std::pair<Key, Type>>> {
public:
    typedef dbg_allocator<std::pair<Key, Type>> _Allocator;
    typedef std::map<Key, Type, Traits, _Allocator> _Base;
    map(const char* szParentObject = "STL map", int nParentIndex = 0) : _Base(_Base::key_compare(), _Base::allocator_type(szParentObject, nParentIndex)) {}
};
#endif

#if (_MSC_VER >= 1300) && defined(_SET_)
template <class Key, class Traits = std::less<Key>> class set : public std::set<Key, Traits, dbg_allocator<Key>> {
public:
    typedef dbg_allocator<Key> _Allocator;
    typedef std::set<Key, Traits, _Allocator> _Base;
    set(const char* szParentObject = "STL set", int nParentIndex = 0) : _Base(_Base::key_compare(), _Allocator(szParentObject, nParentIndex)) {}
};
#endif

#if (_MSC_VER >= 1300) && defined(_VECTOR_)
template <class Type> class vector : public std::vector<Type, dbg_allocator<Type>> {
public:
    typedef dbg_allocator<Type> _Allocator;
    typedef std::vector<Type, _Allocator> _Base;
    typedef vector<Type> _Myt;
    vector(const char* szParentObject = "STL vector", int nParentIndex = 0) : _Base(_Allocator(szParentObject, nParentIndex)) {}
...
public:
    ...
    explicit string(const _Allocator allocator = _Allocator("STL string", 0)) : _Base(allocator) {}
    string(const char* szThat, const _Allocator allocator = _Allocator("STL string", 0)) : _Base(szThat, 0, _Base::npos, allocator) {}
    string(const char* szBegin, const char* szEnd, const _Allocator allocator = _Allocator("STL string", 0)) : _Base(szBegin, szEnd, allocator) {}
    string(const _Base& strRight, const _Allocator allocator = _Allocator("STL string", 0)) : _Base(strRight, 0, npos, allocator) {}
};
#endif
#else
// #if _MSC_VER >= 1300 && defined(_MAP_)
template <class Key, class Type, class Traits = std::less<Key>> class map : public std::map<Key, Type, Traits> {
public:
...
// #if (_MSC_VER >= 1300) && defined(_SET_)
template <class Key, class Traits = std::less<Key>> class set : public std::set<Key, Traits> {
public:
...
public:
    ...
    string(const char* szThat) : _Base(szThat) {}
    string(const char* szThat, const _Allocator allocator) : _Base(szThat) {}
    string(const char* szBegin, const char* szEnd) : _Base(szBegin, szEnd) {}
    string(const char* szBegin, const char* szEnd, const _Allocator allocator) : _Base(szBegin, szEnd) {}
    string(const _Base& strRight) : _Base(strRight) {}
    string(const _Base& strRight, const _Allocator allocator) : _Base(strRight) {}
};
// #endif
#endif
} // namespace cry
