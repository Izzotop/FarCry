...
// LEVEL 1 - Critical messages. Only events that are critical for the proper operation of the game need to be logged at this level. This includes features - event that are critical
// for a feature to work MUST be logged at this level. LEVEL 2 - Artifact filter - This level is for events that will not affect the basic operation of the game, but might
// introduce artifacts (visual or in the way the features work). LEVEL 3 - Startup / Shutdown logs - Basically information when systems start and when they shutdown as well as some
// diagnostic information if you want. LEVEL 4 - Events that are used for debugging purposes (like entity spawned there and there, which sector of the building we are loading etc)
