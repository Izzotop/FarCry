...
struct CCGFHeader {
    ...
    static int GetStructuresCheckSummm() {
        return sizeof(CDLight) + sizeof(list2<int>) + sizeof(CMatInfo) +                      // sizeof(MAT_ENTITY)+
               sizeof(CVertexBuffer) + sizeof(SPipTangents) + m_VertexSize[0] + sizeof(uint); // [Anton] - for m_arrVtxMap in CLeafBuffer
    }
...
public:
    ...
    CIndexedMesh(unsigned nFaces, const CryFace* pFaces, const CryTexFace* pTexFaces, unsigned nVertices, const Vec3d* pVertices, const Vec3d* pNormals, unsigned nUVs,
                 const CryUV* pUVs)
    ...
    CIndexedMesh(ISystem* pSystem, const char* szFileName, const char* szGeomName, // Vec3d * pvAngles,
    ...
    static bool LoadMaterial(const char* szFileName, const char* szFolderName, CMatInfo& MatInfo, IRenderer* pRenderer, MAT_ENTITY* me);
    ...
    list2<StatHelperInfo>* GetHelpers() {
        return &m_lstHelpers;
    }
    list2<CDLight*>* GetLightSourcesList() {
        return &m_lstLSources;
    }
    ...
    bool RemoveEdge(Vec3d* pVertsShared, struct VertInfo* pVertsInfo, TexCoord* pCoordShared, Vec3d* pNormsShared, UColor* pColorShared, UColor* pColorSharedSec,
                    list2<int>& lstIndices, int nVertCountShared, list2<struct TriEdge>& lstTriEdges, int nEdgeForRemoveId, const Vec3d& vBoundaryMin, const Vec3d& vBoundaryMax);
    ...
    void MergeVertexComponents(TriEdge& e0, Vec3d* pVertsShared, struct VertInfo* pVertsInfo, TexCoord* pCoordShared, Vec3d* pNormsShared, UColor* pColorShared,
                               UColor* pColorSharedSec);
