...
enum { FR_PLANE_NEAR, FR_PLANE_FAR, FR_PLANE_RIGHT, FR_PLANE_LEFT, FR_PLANE_TOP, FR_PLANE_BOTTOM, FRUSTUM_PLANES };
...
public:
    ...
    inline Ang3 GetAngles() const {
        return (m_GameAnglesDeg);
    }

    //  inline void  SetVCMatrix(const Matrix44& mat) { m_VCMatD3D9=mat; }
    inline const Matrix34 GetVMatrix() const {
        return (m_VMat);
    }
    inline const Matrix44 GetVCMatrixD3D9() const {
        return (m_VCMatD3D9);
    }
    ...
    inline void SetPos(const Vec3& newpos) {
        ...
        m_Position = newpos;
        m_OccPosition = newpos;
    }
    inline const Vec3& GetPos() const {
        return (m_Position);
    }
    inline const Vec3& GetPosPrec() const {
        return (m_OldPosition);
    }

    inline void SetOccPos(const Vec3& newpos) {
        m_OccPosition = newpos;
    }
    inline const Vec3& GetOccPos() const {
        return (m_OccPosition);
    }

    inline void SetFrustumPlane(int numplane, const Plane& plane) {
        m_frustum[numplane] = plane;
    }
    inline const Plane* GetFrustumPlane(int numplane) const {
        return (&m_frustum[numplane]);
    }

    inline void SetZMin(float zmin) {
        m_edge_nlt.y = zmin;
    }
    ...
    inline void SetZClip(float zmax) {
        m_edge_flt.y = zmax;
    }
    inline float GetZClip() const {
        return m_edge_flt.y;
    }

    inline void SetZMax(float zmax) {
        m_ZMax = zmax;
    }
    ...
    inline Vec3 GetEdgeP() const {
        return m_edge_plt;
    }
    inline Vec3 GetEdgeN() const {
        return m_edge_nlt;
    }
    inline Vec3 GetEdgeF() const {
        return m_edge_flt;
    }
    ...
    inline float GetProjRatio() const {
        return (m_ProjectionRatio);
    }
    inline void SetProjRatio(float fProjectionRatio) {
        m_ProjectionRatio = fProjectionRatio;
    }
    ...
    inline float GetViewSurfaceX() const {
        return (m_ViewSurfaceX);
    }
    inline float GetViewSurfaceZ() const {
        return (m_ViewSurfaceZ);
    }
...
inline void CCamera::Update(int nWidth = -1, int nHeight = -1) {
    ...
    if (1) {
        m_frustum[FR_PLANE_NEAR] = m_fp[FR_PLANE_NEAR];
        m_frustum[FR_PLANE_NEAR].d *= -1;
        m_frustum[FR_PLANE_LEFT] = m_fp[FR_PLANE_LEFT];
        m_frustum[FR_PLANE_LEFT].d *= -1;
        m_frustum[FR_PLANE_RIGHT] = m_fp[FR_PLANE_RIGHT];
        m_frustum[FR_PLANE_RIGHT].d *= -1;
        m_frustum[FR_PLANE_TOP] = m_fp[FR_PLANE_TOP];
        m_frustum[FR_PLANE_TOP].d *= -1;
        m_frustum[FR_PLANE_BOTTOM] = m_fp[FR_PLANE_BOTTOM];
        m_frustum[FR_PLANE_BOTTOM].d *= -1;
        m_frustum[FR_PLANE_FAR] = m_fp[FR_PLANE_FAR];
        m_frustum[FR_PLANE_FAR].d *= -1;
    }
}
...
__forceinline char CCamera::IsSphereVisible_hierarchical(const Sphere& s, bool* bAllIn = 0) const {
    ...
    if (bAllIn)
        *bAllIn = false;
    if ((_nc = m_fp[FR_PLANE_NEAR] | s.center) > s.radius)
        return CULL_EXCLUSION;
    if ((_rc = m_fp[FR_PLANE_RIGHT] | s.center) > s.radius)
        return CULL_EXCLUSION;
    if ((_lc = m_fp[FR_PLANE_LEFT] | s.center) > s.radius)
        return CULL_EXCLUSION;
    if ((_tc = m_fp[FR_PLANE_TOP] | s.center) > s.radius)
        return CULL_EXCLUSION;
    if ((_bc = m_fp[FR_PLANE_BOTTOM] | s.center) > s.radius)
        return CULL_EXCLUSION;
    if ((_cc = m_fp[FR_PLANE_FAR] | s.center) > s.radius)
        return CULL_EXCLUSION;
...
inline bool CCamera::IsAABBVisible_exact(const AABB& aabb) const {
    ...
    if (sideamount == 4) {
        ...
        //--------------------------------------------------------------------------
        if (!(((s0 | cltp) <= 0.0f) | ((s0 | crtp) <= 0.0f) | ((s0 | crbp) <= 0.0f) | ((s0 | clbp) <= 0.0f)))
            return CULL_EXCLUSION;
        if (!(((s1 | cltp) <= 0.0f) | ((s1 | crtp) <= 0.0f) | ((s1 | crbp) <= 0.0f) | ((s1 | clbp) <= 0.0f)))
            return CULL_EXCLUSION;
        if (!(((s2 | cltp) <= 0.0f) | ((s2 | crtp) <= 0.0f) | ((s2 | crbp) <= 0.0f) | ((s2 | clbp) <= 0.0f)))
            return CULL_EXCLUSION;
        if (!(((s3 | cltp) <= 0.0f) | ((s3 | crtp) <= 0.0f) | ((s3 | crbp) <= 0.0f) | ((s3 | clbp) <= 0.0f)))
            return CULL_EXCLUSION;
    }
    ...
    if (sideamount == 6) {
        ...
        if (!(((s3 | cltp) <= 0.0f) | ((s3 | crtp) <= 0.0f) | ((s3 | crbp) <= 0.0f) | ((s3 | clbp) <= 0.0f)))
            return CULL_EXCLUSION;
        if (!(((s4 | cltp) <= 0.0f) | ((s4 | crtp) <= 0.0f) | ((s4 | crbp) <= 0.0f) | ((s4 | clbp) <= 0.0f)))
            return CULL_EXCLUSION;
        if (!(((s5 | cltp) <= 0.0f) | ((s5 | crtp) <= 0.0f) | ((s5 | crbp) <= 0.0f) | ((s5 | clbp) <= 0.0f)))
            return CULL_EXCLUSION;
    }
...
inline char CCamera::IsAABBVisible_hierarchical(const AABB& aabb, bool* bAllIn = 0) const {
    ...
    for (int i = 0; i < FRUSTUM_PLANES; i++) {
        ...
        if (m_frustum[i].n.x >= 0) {
            dot1 += m_frustum[i].n.x * aabb.min.x;
            dot2 += m_frustum[i].n.x * aabb.max.x;
        } else {
            dot1 += m_frustum[i].n.x * aabb.max.x;
            dot2 += m_frustum[i].n.x * aabb.min.x;
        }

        if (m_frustum[i].n.y >= 0) {
            dot1 += m_frustum[i].n.y * aabb.min.y;
            dot2 += m_frustum[i].n.y * aabb.max.y;
        } else {
            dot1 += m_frustum[i].n.y * aabb.max.y;
            dot2 += m_frustum[i].n.y * aabb.min.y;
        }

        if (m_frustum[i].n.z >= 0) {
            dot1 += m_frustum[i].n.z * aabb.min.z;
            dot2 += m_frustum[i].n.z * aabb.max.z;
        } else {
            dot1 += m_frustum[i].n.z * aabb.max.z;
            dot2 += m_frustum[i].n.z * aabb.min.z;
        }
