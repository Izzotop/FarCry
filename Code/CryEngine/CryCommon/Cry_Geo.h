...
struct Lineseg {
    ...
    inline Lineseg() {}
    inline Lineseg(const Vec3& s, const Vec3& e) {
        start = s;
        end = e;
    }
    inline void operator()(const Vec3& s, const Vec3& e) {
        start = s;
        end = e;
    }
...
template <typename F> struct Triangle_tpl {
    ...
    // default Lineseg constructor (without initialisation)
    inline Triangle_tpl() {}
    inline Triangle_tpl(const Vec3_tpl<F>& a, const Vec3_tpl<F>& b, const Vec3_tpl<F>& c) {
        v0 = a;
        v1 = b;
        v2 = c;
    }
    inline void operator()(const Vec3_tpl<F>& a, const Vec3_tpl<F>& b, const Vec3_tpl<F>& c) {
        v0 = a;
        v1 = b;
        v2 = c;
    }
...
struct AABB {
    ...
    ILINE static AABB CreateTransformedAABB(const Matrix34& m34, const AABB& aabb) {
        AABB taabb;
        taabb.SetTransformedAABB(m34, aabb);
        return taabb;
    }

    // create an AABB using just the extensions of the OBB and ignore the orientation.
    template <typename F> ILINE void SetAABBfromOBB(const OBB_tpl<F>& obb) {
        min = obb.c - obb.h;
        max = obb.c + obb.h;
    }
    template <typename F> ILINE static AABB CreateAABBfromOBB(const OBB_tpl<F>& obb) {
     ...
     *  expanded AABB in world-space
     */
    template <typename F> ILINE void SetAABBfromOBB(const Vec3& wpos, const OBB_tpl<F>& obb, f32 scaling = 1.0f) {
        Vec3 pos = obb.m33 * obb.c * scaling + wpos;
        Vec3 sz = obb.m33.GetFabs() * obb.h * scaling;
        min = pos - sz;
        max = pos + sz;
    }
    template <typename F> ILINE static AABB CreateAABBfromOBB(const Vec3& wpos, const OBB_tpl<F>& obb, f32 scaling = 1.0f) {
        AABB taabb;
...
template <typename F> struct OBB_tpl {
    ...
    ILINE static OBB_tpl<F> CreateOBB(const Matrix33& m33, const Vec3& hlv, const Vec3& center) {
        OBB_tpl<f32> obb;
        obb.m33 = m33;
        obb.h = hlv;
        obb.c = center;
    ...
    ILINE static OBB_tpl<F> CreateOBBfromAABB(const Matrix33& m33, const AABB& aabb) {
        OBB_tpl<f32> obb;
        obb.SetOBBfromAABB(m33, aabb);
        return obb;
    }
...
struct Sphere {
    ...
    inline Sphere() {}
    inline Sphere(const Vec3& c, const float& r) {
        center = c;
        radius = r;
    }
    inline void operator()(const Vec3& c, const float& r) {
        center = c;
        radius = r;
    }
...
inline Vec3 SetMaxBB() {
    return Vec3(MAX_BB, MAX_BB, MAX_BB);
}
inline Vec3 SetMinBB() {
    return Vec3(MIN_BB, MIN_BB, MIN_BB);
}
