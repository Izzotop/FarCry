...
template <typename T> void TSimpleSwap(T& a, T& b) {
    T t = a;
    a = b;
    b = t;
}
...
//////////////////////////////////////////////////////////////////////////
template <typename T, class A = TSimpleAllocator<T>> class TElementaryArray {
public:
    ...
    TElementaryArray(const char* szParentObject, int nParentIndex = 0) : m_Allocator(szParentObject, nParentIndex), m_pData(nullptr) {
...
#if defined(_DEBUG) && defined(_INC_CRTDBG)
        : m_Allocator("TElementaryArray(n)", 1), m_nSize(numElements)
#endif
    {
...
#if defined(_DEBUG) && defined(_INC_CRTDBG)
          ,
          m_Allocator("TElementaryArray()", 2), m_nSize(0)
#endif
    {
    }
...
private:
    ...
    TElementaryArray(const TElementaryArray<T, A>& that) : m_pData(nullptr) {
        assert(0);
    }
...
////////////////////////////////////////////////////////////////////////////////////////
template <typename T, class A = TSimpleAllocator<T>> class TFixedArray {
public:
    typedef T value_type;
    typedef T& reference;
    typedef const T& const_reference;
    ...
    TFixedArray(const char* szParentObject, int nParentIndex = 0) : m_nSize(0), m_pData(nullptr), m_Allocator(szParentObject, nParentIndex) {}

    TFixedArray(size_t numElements) : m_nSize(numElements) {
        m_pData = numElements ? m_Allocator.allocate_construct(numElements) : nullptr;
    }

    TFixedArray() : m_nSize(0), m_pData(nullptr) {}

    // copy constructor: full copy implementation
    template <typename U, typename B> TFixedArray(const TFixedArray<U, B>& that) : m_nSize(that.size()), m_Allocator(that.m_Allocator) {
    ...
    // copy constructor: full copy implementation
    TFixedArray(const TFixedArray<T, A>& that) : m_nSize(that.size()), m_Allocator(that.m_Allocator) {
        m_pData = that.empty() ? nullptr : m_Allocator.allocate_construct(that.size());
        for (size_t i = 0; i < m_nSize; ++i)
            m_pData[i] = that[i];
    }
    ...
    TFixedArray(const char* szParentObject, int nParentIndex, const TFixedArray<U, B>& that) : m_nSize(that.size()), m_Allocator(szParentObject, nParentIndex) {
        m_pData = that.empty() ? nullptr : m_Allocator.allocate_construct(that.size());
        for (size_t i = 0; i < m_nSize; ++i)
            m_pData[i] = that[i];
    }
    ...
    // copy constructor: full copy implementation
    TFixedArray(const char* szParentObject, int nParentIndex, const TFixedArray<T, A>& that) : m_nSize(that.size()), m_Allocator(szParentObject, nParentIndex) {
        m_pData = that.empty() ? nullptr : m_Allocator.allocate_construct(that.size());
        for (size_t i = 0; i < m_nSize; ++i)
            m_pData[i] = that[i];
    }
    ...
    template <typename U, typename B> TFixedArray<T, A>& operator=(const TFixedArray<U, B>& src) {
        reinit(src.size());
        for (size_t i = 0; i < m_nSize; ++i)
            m_pData[i] = src[i];
        return *this;
...
public:
    ...
    TGrowArray() : TFixedArray<T, A>("TGrowArray()", 1), m_nSize(0) {}

    TGrowArray(const char* szParentObject, int nParentIndex = 0) : TFixedArray<T, A>(szParentObject, nParentIndex), m_nSize(0) {}

    TGrowArray(int numElements) : TFixedArray<T, A>(numElements, "TGrowArray(n)", 2), m_nSize(numElements) {}

    // copy constructor: full copy implementation
    // reserves enough data to contain the whole array
    template <typename U, typename B> TGrowArray(const TGrowArray<U, B>& that) : TFixedArray<T, A>(that), m_nSize(that.size()) {}

    // copy constructor: full copy implementation
    TGrowArray(const TGrowArray<T, A>& that) : TFixedArray<T, A>(that), m_nSize(that.size()) {}

    // copy constructor: full copy implementation
    // reserves enough data to contain the whole array
    template <typename U, typename B>
    TGrowArray(const char* szParentObject, int nParentIndex, const TGrowArray<U, B>& that) : TFixedArray<T, A>(szParentObject, nParentIndex, that), m_nSize(that.size()) {}

    // copy constructor: full copy implementation
    TGrowArray(const char* szParentObject, int nParentIndex, const TGrowArray<T, A>& that) : TFixedArray<T, A>(szParentObject, nParentIndex, that), m_nSize(that.size()) {}

    template <typename U, typename B> TGrowArray<T, A>& operator=(const TGrowArray<U, B>& src) {
        reinit(src.size());
        for (size_t i = 0; i < m_nSize; ++i)
