...
//! Draw mode
enum eDrawMode { ETY_DRAW_NONE = 0, ETY_DRAW_NORMAL = 1, ETY_DRAW_NEAR = 2 };

//! Object info flags
enum eObjInfoFlags { ETY_OBJ_INFO_DRAW = 1, ETY_OBJ_INFO_DRAW_NEAR = 2, ETY_OBJ_USE_MATRIX = 4, ETY_OBJ_IS_A_LINK = 8 };
...
// Misc
enum eMiscEnum { PLAYER_MODEL_IDX = 0, MAX_ANIMATED_MODELS = 2 };
...
struct SEntityUpdateContext {
    ...
    SEntityUpdateContext()
        : pScriptUpdateParams(nullptr), nFrameID(0), pCamera(0), fCurrTime(0), bProfileToLog(false), numVisibleEntities(0), numUpdatedEntities(0), fMaxViewDist(1e+8){};
};
...
*/
struct IEntity : public IEntityRender, public ICharInstanceSink {
public:
    ...
    */
    ...
    //     GetLight
    // Arguments:
    //     img - Filename of the projected texture, (DDS file) or nullptr if no projected texture.
    //     shader - Name of the light shader, or nullptr if not using light shader.
    ...
    */
    ...
    virtual bool CreateRigidBody(pe_type type, float density, float mass, int surface_id, Vec3* pInitialVelocity = nullptr, int slot = -1, bool bPermanent = false) = 0;
    ...
    virtual bool CreateLivingEntity(float mass, float height, float eye_height, float sphere_height, float radius, int nSurfaceID, float fGravity, float fAirControl,
                                    bool collide = false) = 0;
    ...
    */
    virtual bool CreateParticleEntity(float size, float mass, Vec3 heading, float acc_thrust = 0, float k_air_resistance = 0, float acc_lift = 0, float gravity = -9.8,
                                      int surface_idx = 0, bool bSingleContact = true) = 0;
    ...
    */
     ...
     */
    virtual void CalculateInWorld() = 0;
    ...
    */
    ...
    virtual int CreateEntityParticleEmitter(int nSlotId, const ParticleParams& PartParams, float fSpawnPeriod, Vec3 vOffSet, Vec3 vDir, IParticleEffect* pEffect = 0,
                                            float fSize = 1.0f) = 0;
...
enum ContainerInterfaceType { CIT_IPLAYER, CIT_IWEAPON, CIT_IVEHICLE, CIT_ICOMMANDER, CIT_ISPECTATOR, CIT_IADVCAMSYSTEM };
...
struct IEntityContainer {
    ...
    */
    ...
    virtual void OnEntityNetworkUpdate(const EntityId& idViewerEntity, const Vec3d& v3dViewer, uint32& inoutPriority, EntityCloneState& inoutCloneState) const = 0;
};
...
enum ThirdPersonMode { CAMERA_3DPERSON1, CAMERA_3DPERSON2 };
...
struct IEntityCamera {
    ...
    virtual void SetThirdPersonMode(const Vec3& pos, const Vec3& angles, int mode, float frameTime, float range, int dangleAmmount, IPhysicalEntity* physic,
                                    IPhysicalEntity* physicMore = nullptr, I3DEngine* p3DEngine = nullptr, float safe_range = 0.0f) = 0;
