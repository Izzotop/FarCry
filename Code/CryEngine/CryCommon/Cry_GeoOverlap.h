...
inline bool Ray_Triangle(const Ray& ray, const Vec3& v0, const Vec3& v1, const Vec3& v2) {
    ...
    if (((output - ray.origin) | ray.direction) < 0)
        return 0;
    return 1;
}
...
inline bool Lineseg_Triangle(const Lineseg& lineseg, const Vec3& v0, const Vec3& v1, const Vec3& v2) {
    ...
    if (((output - lineseg.start) | direction) < 0)
        return 0;
    if (((output - lineseg.end) | direction) > 0)
        return 0;
    return 1;
}
...
ILINE bool Sphere_AABB(const Sphere& s, const AABB& aabb) {
    ...
    if (s.center.x < aabb.min.x) {
        quat.x = s.center.x - aabb.min.x;
    } else if (s.center.x > aabb.max.x) {
        quat.x = s.center.x - aabb.max.x;
    }
    if (s.center.y < aabb.min.y) {
        quat.y = s.center.y - aabb.min.y;
    } else if (s.center.y > aabb.max.y) {
        quat.y = s.center.y - aabb.max.y;
    }
    if (s.center.z < aabb.min.z) {
        quat.z = s.center.z - aabb.min.z;
    } else if (s.center.z > aabb.max.z) {
        quat.z = s.center.z - aabb.max.z;
    }
...
//--- 1 = overlap                                                  -----------------
//----------------------------------------------------------------------------------
template <typename F> ILINE bool Sphere_Triangle(const Sphere& s, const Triangle_tpl<F>& t) {
    ...
    if (Sphere_Sphere(s, Sphere(middle, sqrt_tpl(SqRad1))) == 0)
        return 0; // overlap not possible
    //...and now the hardcore-test!
    if ((s.radius * s.radius) < Distance::Point_Triangle(s.center, t))
        return 0;
...
inline bool OBB_OBB(const Vec3& pos1, const OBB& obb1, const Vec3& pos2, const OBB& obb2) {
    ...
    if (fabsf(t | Vec3(M(0, 0), M(1, 0), M(2, 0))) > (ra + obb2.h.x))
        return 0;
    ra = fabsf(M(0, 1) * obb1.h.x) + fabsf(M(1, 1) * obb1.h.y) + fabsf(M(2, 1) * obb1.h.z);
    if (fabsf(t | Vec3(M(0, 1), M(1, 1), M(2, 1))) > (ra + obb2.h.y))
        return 0;
    ra = fabsf(M(0, 2) * obb1.h.x) + fabsf(M(1, 2) * obb1.h.y) + fabsf(M(2, 2) * obb1.h.z);
    if (fabsf(t | Vec3(M(0, 2), M(1, 2), M(2, 2))) > (ra + obb2.h.z))
        return 0;
