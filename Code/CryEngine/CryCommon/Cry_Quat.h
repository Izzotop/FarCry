...
#if defined(LINUX)
template <class F, int SI, int SJ> Quaternion_tpl<F> GetQuatFromMat33(const Matrix33_tpl<F, SI, SJ>& m);
#endif
...
template <class F> struct Quaternion_tpl {
    ...
    Quaternion_tpl(F W, F X, F Y, F Z) {
        w = W;
        v.x = X;
        v.y = Y;
        v.z = Z;
    ...
    template <class F1, int SI, int SJ> explicit ILINE Quaternion_tpl(const Matrix33_tpl<F1, SI, SJ>& m) {
        *this = GetQuatFromMat33(m);
    }
    template <class T> explicit ILINE Quaternion_tpl(const Matrix34_tpl<T>& m) {
        *this = GetQuatFromMat33(Matrix33(m));
    }
    template <class F1, int SI, int SJ> explicit ILINE Quaternion_tpl(const Matrix44_tpl<F1, SI, SJ>& m) {
        *this = GetQuatFromMat33(Matrix33_tpl<f32, 3, 1>(m));
    }

    // CONSTRUCTOR: implement the copy/casting/assignement constructor:
    template <class T> ILINE Quaternion_tpl(const Quaternion_tpl<T>& q) {
        w = q.w;
        v.x = q.v.x;
        v.y = q.v.y;
        v.z = q.v.z;
    ...
    */
    ...
    Quaternion_tpl<F> operator-() const {
        return Quaternion_tpl<F>(-w, -v);
    };

    Quaternion_tpl<F> operator!() const;

    void SetIdentity();
    static Quaternion_tpl<F> GetIdentity();
 ...
 */
template <class F> ILINE Quaternion_tpl<F>::Quaternion_tpl(const F angle, const Vec3_tpl<F>& axis) {
    w = angle;
    v = axis;
}
 ...
 *
 */
template <typename F1, typename F2> ILINE f32 operator|(const Quaternion_tpl<F1>& q, const Quaternion_tpl<F2>& p) {
 ...
 *
 */
template <class F1, class F2> ILINE Quaternion_tpl<F1> operator*(const Quaternion_tpl<F1>& q, const Quaternion_tpl<F2>& p) {
    return Quaternion_tpl<F1>(q.w * p.w - (q.v | p.v), q.v * p.w + q.w * p.v + (q.v % p.v));
}
template <class F1, class F2> ILINE void operator*=(Quaternion_tpl<F1>& q, const Quaternion_tpl<F2>& p) {
    F1 s0 = q.w;
    q.w = q.w * p.w - (q.v | p.v);
...
template <typename F1> ILINE Quaternion_tpl<F1> operator*(const Quaternion_tpl<F1>& q, f32 t) {
    return Quaternion_tpl<F1>(q.w * t, q.v * t);
};
//! Scale quaternion free function.
template <typename F1> ILINE Quaternion_tpl<F1> operator*(f32 t, const Quaternion_tpl<F1>& q) {
 ...
 *   Vec3 result = q*v;
 */
template <class F1, class F2> ILINE Vec3_tpl<F2> operator*(const Quaternion_tpl<F1>& q, const Vec3_tpl<F2>& v) {
#if !defined(LINUX)
 ...
 *   Vec3 result = v*q;
 */
template <class F1, class F2> ILINE Vec3_tpl<F1> operator*(const Vec3_tpl<F1>& v, const Quaternion_tpl<F2>& q) {
#if !defined(LINUX)
...
template <class F> ILINE void Quaternion_tpl<F>::Invert() {
    *this = !*this;
}
template <class F> ILINE Quaternion_tpl<F> Quaternion_tpl<F>::GetInverted(Quaternion_tpl<F>& q) {
    return !q;
}
template <class F> ILINE Quaternion_tpl<F> GetInverted(Quaternion_tpl<F>& q) {
    return !q;
}
...
template <class F> ILINE void Quaternion_tpl<F>::SetRotationAA(F rad, const Vec3_tpl<F>& axis) {
    F cs[2];
    sincos_tpl(rad * (F)0.5, cs);
    SetRotationAA(cs[0], cs[1], axis);
}
template <class F> ILINE Quaternion_tpl<F> Quaternion_tpl<F>::GetRotationAA(F rad, const Vec3_tpl<F>& axis) {
    Quaternion_tpl<F> q;
    q.SetRotationAA(rad, axis);
    return q;
}
template <class F> ILINE Quaternion_tpl<F> GetRotationAA(F rad, const Vec3_tpl<F>& axis) {
    Quaternion_tpl<F> q;
    q.SetRotationAA(rad, axis);
    return q;
}
...
template <class F> ILINE Quaternion_tpl<F> Quaternion_tpl<F>::GetRotationAA(F cosha, F sinha, const Vec3_tpl<F>& axis) {
    Quaternion_tpl<F> q;
    q.SetRotationAA(cosha, sinha, axis);
    return q;
}
template <class F> ILINE Quaternion_tpl<F> GetRotationAA(F cosha, F sinha, const Vec3_tpl<F>& axis) {
    Quaternion_tpl<F> q;
    q.SetRotationAA(cosha, sinha, axis);
    return q;
}
 ...
 */
template <class F> void Quaternion_tpl<F>::SetRotationV0V1(const Vec3_tpl<F>& v0, const Vec3_tpl<F>& v1) {
#if !defined(LINUX)
    ...
    if (w > 0.001) {
        ...
        v = GetOrthogonal(v0).normalized();
        w = 0;
    }
}
template <class F> ILINE Quaternion_tpl<F> Quaternion_tpl<F>::GetRotationV0V1(const Vec3_tpl<F>& v0, const Vec3_tpl<F>& v1) {
    Quaternion_tpl<F> q;
    q.SetRotationV0V1(v0, v1);
    return q;
}
template <class F> ILINE Quaternion_tpl<F> GetRotationV0V1(const Vec3_tpl<F>& v0, const Vec3_tpl<F>& v1) {
    Quaternion_tpl<F> q;
    q.SetRotationV0V1(v0, v1);
    return q;
}
 ...
 */
template <class F> ILINE void Quaternion_tpl<F>::SetRotationXYZ(const Ang3& a) {
    F csx[2];
    sincos_tpl(a.x * (F)0.5, csx);
    F csy[2];
    sincos_tpl(a.y * (F)0.5, csy);
    F csz[2];
    sincos_tpl(a.z * (F)0.5, csz);
    w = cx * cy * cz + sx * sy * sz;
    v.x = cz * cy * sx - sz * sy * cx;
    v.y = cz * sy * cx + sz * cy * sx;
    v.z = sz * cy * cx - cz * sy * sx;
}
template <class F> ILINE Quaternion_tpl<F> Quaternion_tpl<F>::GetRotationXYZ(const Ang3& a) {
    Quaternion_tpl<F> q;
    q.SetRotationXYZ(a);
    return q;
}
template <class F> ILINE Quaternion_tpl<F> GetRotationXYZ(const Ang3& a) {
    Quaternion_tpl<F> q;
    q.SetRotationXYZ(a);
    return q;
}
 ...
 *    q.Identity();
 */
template <class F> ILINE void Quaternion_tpl<F>::SetIdentity() {
    w = 1;
    v.x = 0, v.y = 0;
    v.z = 0;
}
template <class F> ILINE Quaternion_tpl<F> Quaternion_tpl<F>::GetIdentity() {
    return Quaternion_tpl(1, 0, 0, 0);
}
template <class F> ILINE Quaternion_tpl<F> GetIdentity() {
    return Quaternion_tpl<F>(1, 0, 0, 0);
}
 ...
 *
 */
template <class F> ILINE void Quaternion_tpl<F>::Normalize() {
#if !defined(LINUX)
...
template <class F> ILINE Quaternion_tpl<F> Quaternion_tpl<F>::GetNormalized(const Quaternion_tpl<F>& q) {
    Quaternion_tpl<F> t = q;
    t.Normalize();
    return t;
}
template <class F> ILINE Quaternion_tpl<F> GetNormalized(const Quaternion_tpl<F>& q) {
    Quaternion_tpl<F> t = q;
    t.Normalize();
    return t;
}
 ...
 *
 */
template <class F> ILINE F Quaternion_tpl<F>::GetLength(const Quaternion_tpl<F>& q) {
    return sqrt_tpl(q | q);
}
template <class F> ILINE F GetLength(const Quaternion_tpl<F>& q) {
    return sqrt_tpl(q | q);
}
 ...
 *
 */
template <class F> ILINE Quaternion_tpl<F> qlerp(const Quaternion_tpl<F>& p, const Quaternion_tpl<F>& q, f32 t) {
#if !defined(LINUX)
 ...
 *
 */
template <class F> ILINE Quaternion_tpl<F> Slerp(const Quaternion_tpl<F>& p, const Quaternion_tpl<F>& tq, f32 t) {
#if !defined(LINUX)
    assert((fabs_tpl(1 - (p | p))) < 0.001);   // check if unit-quaternion
...
template <class F> ILINE Quaternion_tpl<F> Squad(const Quaternion_tpl<F> p, const Quaternion_tpl<F> a, const Quaternion_tpl<F> b, const Quaternion_tpl<F> q, f32 t) {
    f32 k = 2.0f * (1.0f - t) * t;
...
template <class F> ILINE Quaternion_tpl<F> exp(const Quaternion_tpl<F>& q) {
    double d = sqrt(double(q.v.x * q.v.x + q.v.y * q.v.y + q.v.z * q.v.z));
    if (d > 1e-4) {
        double m = sin(d) / d;
        return Quaternion_tpl<F>((F)cos(d), F(q.v.x * m), F(q.v.y * m), F(q.v.z * m));
    } else
        return Quaternion_tpl<F>(F(1 - d * d), F(q.v.x), F(q.v.y), F(q.v.z)); // cos(d) ~= 1-d^2 when d->0
}

// logarithm of a quaternion, imaginary part (the real part of the logarithm is always 0)
template <class F> ILINE Quaternion_tpl<F> log(const Quaternion_tpl<F>& q) {
#if !defined(LINUX)
    ...
    if (d > 1e-7) {
        ...
        return Quaternion_tpl<F>(0, 0, 0, 0);
}

//////////////////////////////////////////////////////////////////////////
//! Logarithm of Quaternion difference.
template <class F> ILINE Quaternion_tpl<F> LnDif(const Quaternion_tpl<F>& q1, const Quaternion_tpl<F>& q2) {
#if !defined(LINUX)
 ...
 *
 */
template <class F> ILINE Quaternion_tpl<F> qslerp_a(const Quaternion_tpl<F>& p, const Quaternion_tpl<F>& tq, F t) {
#if !defined(LINUX)
    assert((fabs_tpl(1 - (p | p))) < 0.001);   // check if unit-quaternion
    assert((fabs_tpl(1 - (tq | tq))) < 0.001); // check if unit-quaternion
#endif
 ...
 * \endcode
 */
template <class F, int SI, int SJ> inline Quaternion_tpl<F> GetQuatFromMat33(const Matrix33_tpl<F, SI, SJ>& m) {
    ...
    if (tr > 0.0) {
        ...
        p = 0.5f / s;
        q.w = s * 0.5f;
        q.v((m.M21 - m.M12) * p, (m.M02 - m.M20) * p, (m.M10 - m.M01) * p);
        return q;
    }
    // diagonal is negativ. now we have to find the biggest element on the diagonal
    // check if "m00" is the biggest element
    if ((m.M00 >= m.M11) && (m.M00 >= m.M22)) {
        s = p = sqrt_tpl(m.M00 - m.M11 - m.M22 + 1.0f);
        if (s) {
            p = 0.5f / s;
        }
        q.w = (m.M21 - m.M12) * p;
        q.v(s * 0.5f, (m.M10 + m.M01) * p, (m.M20 + m.M02) * p);
        return q;
    }
    // check if "m11" is the biggest element
    if ((m.M11 >= m.M00) && (m.M11 >= m.M22)) {
        s = p = sqrt_tpl(m.M11 - m.M22 - m.M00 + 1.0f);
        if (s) {
            p = 0.5f / s;
        }
        q.w = (m.M02 - m.M20) * p;
        q.v((m.M01 + m.M10) * p, s * 0.5f, (m.M21 + m.M12) * p);
        return q;
    }
    // check if "m22" is the biggest element
    if ((m.M22 >= m.M00) && (m.M22 >= m.M11)) {
        s = p = sqrt_tpl(m.M22 - m.M00 - m.M11 + 1.0f);
        if (s) {
            p = 0.5f / s;
        }
        q.w = (m.M10 - m.M01) * p;
        q.v((m.M02 + m.M20) * p, (m.M12 + m.M21) * p, s * 0.5f);
        return q;
    }
#if !defined(LINUX)
    assert(0);
