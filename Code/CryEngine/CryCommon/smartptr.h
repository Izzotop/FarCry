...
template <class _I> inline bool operator==(const _smart_ptr<_I>& p1, int null) {
    return !(bool)p1;
}
template <class _I> inline bool operator!=(const _smart_ptr<_I>& p1, int null) {
    return (bool)p1;
}
template <class _I> inline bool operator==(int null, const _smart_ptr<_I>& p1) {
    return !(bool)p1;
}
template <class _I> inline bool operator!=(int null, const _smart_ptr<_I>& p1) {
    return (bool)p1;
...
// reference target for smart pointer
// implements AddRef() and Release() strategy using reference counter of the specified type
template <class Derived> class _reference_target_MT_novtbl {
public:
    _reference_target_MT_novtbl() : m_nRefCounter(0) {}
