...
template <typename T> class TSimpleStructAllocator {
public:
    ...
    TSimpleStructAllocator<T>()
        ...
        : m_szParentObject("CrySimpleDebugAlloc"), m_nParentIndex(1)
#endif
    {
    }
...
#if defined(_DEBUG) && defined(_INC_CRTDBG)
        : m_szParentObject(szParentObject), m_nParentIndex(nParentIndex)
#endif
    { // construct default allocator (do nothing)
    }

    TSimpleStructAllocator<T>(const TSimpleStructAllocator<T>& rThat) { // construct by copying (do nothing)
#if defined(_DEBUG) && defined(_INC_CRTDBG)
...
// this is the
template <typename T> class TSimpleAllocator : public TSimpleStructAllocator<T> {
public:
    // signals to the clients that the objects need construction/can be constructed (and destructed)
    enum { canConstruct = 1 };
    enum { canDestruct = 1 };
    ...
    TSimpleAllocator<T>(const char* szParentObject, int nParentIndex = 0) : TSimpleStructAllocator<T>(szParentObject, nParentIndex) { // construct default allocator (do nothing)
    }

    TSimpleAllocator<T>(const TSimpleAllocator<T>& rThat) : TSimpleStructAllocator<T>(rThat) { // construct by copying (do nothing)
    }
    /*
    template<class _Other>
    ...
    */
    void construct(pointer Ptr, const T& _Val) { // construct object at _Ptr with value _Val
                                                 // std::_Construct(_Ptr, _Val);
    }

    void destroy(pointer Ptr) { // destroy object at _Ptr
        Ptr->~T();
    }
};
