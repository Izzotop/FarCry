...
public: // --------------------------
    ...
    virtual bool Read(CStream& inStream) const {
        ...
        if (!inStream.ReadNumberInBits((unsigned int&)x, (size_t)20))
            return false;
        if (!inStream.ReadNumberInBits((unsigned int&)y, (size_t)20))
            return false;
        if (!inStream.ReadNumberInBits((unsigned int&)z, (size_t)18))
            return false;
    ...
    virtual bool Write(CStream& inStream) const {
        ...
        if (!inStream.Write(true))
            return false; // compressed

        if (!inStream.WriteNumberInBits((unsigned int)x, 20))
            return false;
        if (!inStream.WriteNumberInBits((unsigned int)y, 20))
            return false;
        if (!inStream.WriteNumberInBits((unsigned int)z, 18))
            return false;
...
public: // --------------------------
    ...
    virtual bool Read(CStream& inStream) const {
        ...
        if (!inStream.ReadNumberInBits((unsigned int&)dwWord, (size_t)16))
            return false;
    ...
    virtual bool Write(CStream& inStream) const {
        ...
        if (vAbs[iDominantDirection] != m_vData[iDominantDirection])
            iDominantDirection += 3;
        ...
        if (dwX >= 104)
            dwX = 104 - 1; // 104 values on a cube size
        if (dwY >= 104)
            dwY = 104 - 1; // 104 values on a cube size

        DWORD dwWord = iDominantDirection + 6 * (dwX + 104 * dwY); // 104 values on a cube size

        if (!inStream.WriteNumberInBits((unsigned int)dwWord, 16))
            return false;
