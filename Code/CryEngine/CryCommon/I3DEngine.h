 ...
 */
template <class T> struct SVariationValue {
    T value;
    ...
    T GetVariantValue() const {
        return value * (1 + variation * GenRand());
    }
...
//! Particle Blend Type
enum ParticleBlendType { ParticleBlendType_AlphaBased, ParticleBlendType_ColorBased, ParticleBlendType_Additive, ParticleBlendType_None };
...
struct IParticleEffect : public _i_reference_target_t {
    ...
    */
    ...
    //     Gets the name
    // Return Value:
    //     A nullptr terminated string which hold the name.
...
struct IVisArea {
    ...
    // Return Value:
    //     A nullptr terminated char array containing the name of the VisArea.
    virtual const char* GetName() = 0;
...
struct I3DEngine : public IProcess {
  ...
  and water volumes. For indoor objects global water level is ignored.
    ...
    //     case no effect has been found, the value nullptr will be returned.
    ...
    virtual class IEdgeConnectivityBuilder* GetNewConnectivityBuilder() = 0;
    ...
    virtual class IEdgeConnectivityBuilder* GetNewStaticConnectivityBuilder() = 0;

    // Summary:
    //   Used for precalculation in ShadowVolumes
    // ## added by M.M. (don't delete or Release the interface pointer)
    virtual class IEdgeDetector* GetEdgeDetector() = 0;
    ...
    //   bUseDeepness -
    //   bUseInIndoors -
    virtual void UpdateVisArea(IVisArea* pArea, const Vec3* pPoints, int nCount, const char* szName, float fHeight, const Vec3& vAmbientColor, bool bAfectedByOutLights,
                               bool bSkyOnly, const Vec3& vDynAmbientColor, float fViewDistRatio, bool bDoubleSide, bool bUseDeepness, bool bUseInIndoors) = 0;
    ...
    */
    ...
    //   Finds a material
    // Arguments:
    //   A nullptr terminated string which hold the name to search
    // Return Value:
    ...
    virtual INT_PTR AddStaticLightSource(const class CDLight& LSource, IEntityRender* pCreator, ICryCharInstance* pCryCharInstance = nullptr,
                                         const char* szBoneName = nullptr) = 0; // ## AMD Port
    ...
    virtual const char* GetFilePath(const char* szFileName) {
        return GetLevelFilePath(szFileName);
    }
    // DOC-IGNORE-END
