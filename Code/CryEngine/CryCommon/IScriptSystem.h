...
enum BreakState { bsStepNext, bsStepInto, bsContinue, bsNoBreak };
...
struct IScriptSystem {
     ...
     * @param bRaiseError if true the script engine will call IScirptSystemSink::OnError() if an error will occur
     * @param bForceReload
    ...
    */
    ...
    void EndCall(char*& sRet) { EndCall((const char*&)sRet); } // todo (Izzotop):
#if defined(WIN64) || defined(LINUX)
    inline void EndCall(char*& sRet) {
        EndCall((const char*&)sRet);
    }
#endif
    ...
    */
    ...
    virtual void SetGlobalTonullptr(const char* sKey) = 0;
    // ##@}
...
struct IScriptObject {
    ...
    */
    ...
    /*! Set the value of a member variable to nil
            @param sKey variable name
    */
    virtual void SetTonullptr(const char* sKey) = 0;
    ...
    */
    ...
    virtual void SetTonullptrChain(const char* sKey) = 0;
    virtual void EndSetGetChain() = 0;
    // ##@}

    /*!Get the vaue type of a table member
            @param sKey variable name
            @return the value type (svtnullptr if doesn't exist)
    */
    ...
    */
    ...
    /*! Set the value of a member variable to nil at the specified index
            @param nIdx index of the variable
    */
    virtual void SetnullptrAt(int nIdx) = 0;
    ...
    */
    ...
    bool GetAt(int nIdx, char*& sVal) { return GetAt(nIdx, (const char*&)sVal); } // todo (Izzotop):
    ...
    bool GetCurrent(char*& sVal) { return GetCurrent((const char*&)sVal); } // todo (Izzotop):
    ...
    bool GetCurrentKey(char*& sVal) { return GetCurrentKey((const char*&)sVal); } // todo (Izzotop):
...
struct IFunctionHandler {
    ...
    */
    ...
    bool GetParam(int nIdx, char*& s) { return GetParam(nIdx, (const char*&)s); } // todo (Izzotop):
    ...
    */
    // ##@{
    virtual int EndFunctionnullptr() = 0;
...
public:
    ...
    ~_HScriptFunction() {
        if (m_hFunc)
            m_pScriptSystem->ReleaseFunc(m_hFunc);
        m_hFunc = 0;
    }
