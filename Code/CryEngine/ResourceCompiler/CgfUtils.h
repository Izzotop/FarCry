...
public:
    ...
    // template name (nullptr if no template)
...
public:
    ...
    CMatEntityIndexSort(const CMatEntityNameTokenizer* pTokenizers, unsigned numSize) : m_pTokenizers(pTokenizers), m_numSize(numSize) {}
...
struct SBasisProperties {
    ...
    bool isOrthogonal() const {
        return fErrorDeg < 0.5;
    }
    double getLeftHandednessPercentage() const {
        return (100 - fErrorDeg * 100 / 90);
    }
};
