...
CryChunkedFile::CryChunkedFile(CChunkFileReader* pFile)
    : m_pFile(pFile), pTiming(nullptr), pRangeEntities(nullptr), pFileHeader(nullptr), m_numBoneLightBinds(0), m_pBoneLightBind(nullptr), numSceneProps(0), pSceneProps(nullptr) {
...
CryChunkedFile::MeshDesc::MeshDesc(const MESH_CHUNK_DESC* pChunk, unsigned nSize) : pVColors(nullptr) {
    this->pNode = nullptr;
            ...
            if (arrLinks.empty()) {
                ...
                for (u32 x = 0; x < size; x++) {
                    for (u32 y = x + 1; y < size; y++) {
                        if (arrLinks[x].BoneID == arrLinks[y].BoneID) {
                            ...
                            for (u32 z = y; z < size; z++)
                                arrLinks[z] = arrLinks[z + 1];
                            arrLinks.pop_back();
                            size = arrLinks.size();
                            y--;
                        }
                ...
                if (size == 1) {
                    ...
                    for (u32 i = 0; i < size; i++) {
                        ...
                        if (f < (0.0f + minval))
                            arrLinks[i].Blending = 0.0f;
                        if (f > (1.0f - minval))
                            arrLinks[i].Blending = 1.0f;
                    }

                    for (u32 x = 0; x < size; x++) {
                        if (arrLinks[x].Blending == 0.0f) {
                            // remove link from the list;
                            for (u32 z = x; z < size; z++)
                                arrLinks[z] = arrLinks[z + 1];
                            arrLinks.pop_back();
                            size = arrLinks.size();
                            x--;
                    ...
                    for (u32 i = 0; i < size; i++)
                        t += arrLinks[i].Blending;
                    // normalized blending
                    for (u32 i = 0; i < size; i++)
                        arrLinks[i].Blending /= t;
                }
...
// returns node pointer by the node chunk id; if chunkid is not node , returns nullptr
...
// if can't find it, returns nullptr
...
void CryChunkedFile::addChunkBoneLightBinding(const CHUNK_HEADER& chunkHeader, const BONELIGHTBINDING_CHUNK_DESC_0001* pChunkData, unsigned nChunkSize) {
    if (m_pBoneLightBind)
...
void CryChunkedFile::addChunkSceneProps(const CHUNK_HEADER& chunkHeader, const SCENEPROPS_CHUNK_DESC* pChunkData, unsigned nChunkSize) {
    ...
    if (nChunkSize < sizeof(*pChunkData) + sizeof(SCENEPROP_ENTITY) * pChunkData->nProps)
        throw Error("Truncated SCENEPROPS_CHUNK_DESC chunk, %d props, %d bytes, %d expected", pChunkData->nProps, nChunkSize,
                    sizeof(*pChunkData) + sizeof(SCENEPROP_ENTITY) * pChunkData->nProps);
...
void CryChunkedFile::MeshDesc::validateIndices() {
    for (unsigned nFace = 0; nFace < numFaces(); ++nFace) {
        const CryFace& Face = this->pFaces[nFace];
        if (Face.v0 < 0 || (unsigned)Face.v0 >= numVertices() || Face.v1 < 0 || (unsigned)Face.v1 >= numVertices() || Face.v2 < 0 || (unsigned)Face.v2 >= numVertices())
            throw Error("Face %d (v={%d,%d,%d}) has one or more indices out of range (%d vertices in the mesh)", nFace, Face.v0, Face.v1, Face.v2, numVertices());
    }
}
