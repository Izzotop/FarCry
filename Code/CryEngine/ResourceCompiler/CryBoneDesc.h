...
// together
#pragma pack(push, 4)
template <class TBonePhysics> struct TCryBoneDescData {
...
public:
    ...
    bool hasParent() const {
        return m_nOffsetParent != 0;
    }
    int getParentIndexOffset() const {
        return m_nOffsetParent;
    }
    int getFirstChildIndexOffset() const {
        return m_nOffsetChildren;
    }

    const Matrix44& getInvDefGlobal() const {
        return m_matInvDefGlobal;
    }
    ...
    BONE_PHYSICS& getPhysInfo(int nLod) {
    ...
    INT_PTR getPhysGeomId(unsigned nLOD) {
        return (INT_PTR)m_PhysInfo[nLOD].pPhysGeom;
    }
    ...
    // returns the number of required bytes for serialization, if the data pointer is nullptr
