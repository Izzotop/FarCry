...
ResourceCompiler::ResourceCompiler() {
    ...
    m_hLogFile = nullptr;
    m_hErrorLogFile = nullptr;
    m_hWarningLogFile = nullptr;
...
ResourceCompiler::~ResourceCompiler() {
    ...
    m_hLogFile = nullptr;
    if (m_hErrorLogFile)
        fclose(m_hErrorLogFile);
    m_hErrorLogFile = nullptr;
    if (m_hWarningLogFile)
        fclose(m_hWarningLogFile);
    m_hWarningLogFile = nullptr;
}
...
bool ResourceCompiler::Compile(Platform platform, IConfig* config, const char* filespec) {
    ...
    } else if (dwFileSpecAttr & FILE_ATTRIBUTE_DIRECTORY) {
...
// Izzotop: Added __stdcall
void __stdcall RegisterConvertors(IResourceCompiler* rc) {
    IRCLog* log = rc->GetIRCLog();
    assert(log);
...
HWND ResourceCompiler::GetEmptyWindow() {
    if (!m_hEmptyWindow) {
        ...
        m_hEmptyWindow = CreateWindow(szClassName, "DirectXEmpty", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 256, 256, nullptr, nullptr, wc.hInstance, nullptr);
    }
