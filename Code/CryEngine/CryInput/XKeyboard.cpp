...
CXKeyboard::CXKeyboard() {
    ...
    m_cvBufferedKeys = nullptr;
    m_cvDirectInputKeys = nullptr;
    m_pSystem = nullptr;
    m_modifiers = 0;
    m_iToggleState = 0;
}
...
bool CXKeyboard::Init(CInput* pInput, ISystem* pSystem, LPDIRECTINPUT8& g_pdi, HINSTANCE hinst, HWND hwnd) {
    ...
    if (g_pdi) {
        ...
        hr = g_pdi->CreateDevice(GUID_SysKeyboard, &m_pKeyboard, nullptr);
        if (FAILED(hr)) {
...
void CXKeyboard::SetExclusive(bool value, void* hwnd) {
    ...
    // hr = m_pKeyboard->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), nullptr, &dwItems, 0);
...
void CXKeyboard::ProcessKey(int cKey, bool bPressed, unsigned char* cTempKeys) {
    ...
    if (bPressed) {
        if (cKey == DIK_LSHIFT)
            m_modifiers |= XKEY_MOD_LSHIFT;
        if (cKey == DIK_RSHIFT)
            m_modifiers |= XKEY_MOD_RSHIFT;
        if (cKey == DIK_LCONTROL)
            m_modifiers |= XKEY_MOD_LCONTROL;
        if (cKey == DIK_RCONTROL)
            m_modifiers |= XKEY_MOD_RCONTROL;
        if (cKey == DIK_LALT)
            m_modifiers |= XKEY_MOD_LALT;
        if (cKey == DIK_RALT)
            m_modifiers |= XKEY_MOD_RALT;
