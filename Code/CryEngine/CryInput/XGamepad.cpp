...
CXGamepad::CXGamepad() {
    memset(m_hGamepads, nullptr, sizeof(HANDLE) * MAX_XBOX_CONTROLLERS);
    memset(m_xbStatus, nullptr, sizeof(XINPUT_STATE) * MAX_XBOX_CONTROLLERS);
    memset(m_xbLastStatus, nullptr, sizeof(XINPUT_STATE) * MAX_XBOX_CONTROLLERS);
...
void CXGamepad::Update() {
    ...
    for (i = 0; i < MAX_XBOX_CONTROLLERS; i++) {
        if (dwInsertions & (1 << i)) {
            ...
            m_hGamepads[i] = XInputOpen(XDEVICE_TYPE_GAMEPAD, NumToPort(i), XDEVICE_NO_SLOT, nullptr);

#ifdef _DEBUG
            if (m_hGamepads[i] != nullptr) {
    ...
    for (i = 0; i < MAX_XBOX_CONTROLLERS; i++) {
        if (IsControllerReady(i)) {
            ...
            if (dwReturn == ERROR_DEVICE_NOT_CONNECTED) {
                sprintf(szBuffer,
                        "INPUT: Can't get state of controller %i because it was"
                        " removed and the removal has not been handled yet\n",
                        i);
                OutputDebugString(szBuffer);
            } else if (dwReturn != ERROR_SUCCESS) {
                sprintf(szBuffer,
                        "INPUT: ERROR: Can't get state of controller %i"
                        ", error %i\n",
                        i, dwReturn);
                OutputDebugString(szBuffer);
            }
...
bool CXGamepad::IsDigitalButtonPressed(const eDigitalButton eWhichButton, const bool bOnce, const eController eWhichController) const {
    //////////////////////////////////////////////////////////////////////
    ...
    if (eWhichController == ePrimary) {
        ...
        bPressed = (m_xbStatus[GetPrimaryController()].Gamepad.wButtons & eWhichButton) != nullptr;
    } else if (eWhichController != eAll) {
        // Check the specified controller
        if (IsControllerReady(eWhichController))
            bPressed = (m_xbStatus[eWhichController].Gamepad.wButtons & eWhichButton) != nullptr;
        else
    ...
    if (bOnce) {
        ...
        if ((m_xbLastStatus[iLastController].Gamepad.wButtons & eWhichButton) != nullptr)
...
float CXGamepad::GetAnalogButtonValue(const eAnalogButton eWhichButton, const eController eWhichController) const {
    //////////////////////////////////////////////////////////////////////
...
void CXGamepad::GetAnalogStickValue(float& fX, float& fY, const eSide eWhichStick, const eController eWhichController) const {
    //////////////////////////////////////////////////////////////////////
    ...
    if (eWhichController == ePrimary) {
        ...
        ReadAnalogStickValue(GetPrimaryController(), eWhichStick, m_bExponentialStick, fX, fY);
        return;
    }
...
void CXGamepad::ReadAnalogStickValue(const unsigned __int8 iControllerID, const eSide eWhichSide, const bool bExponential, float& fX, float& fY) const {
    //////////////////////////////////////////////////////////////////////
