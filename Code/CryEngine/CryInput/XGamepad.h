...
private:
    bool IsControllerReady(UINT iNumController) const {
        assert(iNumController < MAX_XBOX_CONTROLLERS);
        return m_hGamepads[iNumController] != nullptr;
    };
    ...
    bool IsDigitalButtonPressed(const eDigitalButton eWhichButton, const bool bOnce, const eController eWhichController) const;

    float GetAnalogButtonValue(const eAnalogButton eWhichButton, const eController eWhichController) const;

    void GetAnalogStickValue(float& fX, float& fY, const eSide eWhichStick, const eController eWhichController) const;

    void ReadAnalogStickValue(const unsigned __int8 iControllerID, const eSide eWhichSide, const bool bExponential, float& fX, float& fY) const;
