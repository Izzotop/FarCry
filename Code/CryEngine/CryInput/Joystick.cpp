...
void CJoystick::Update() {
    ...
    if (m_hatdirs) {
        ...
        if (joyinfo.dwPOV == JOY_POVLEFT)
            m_hatdirs[JOY_DIR_LEFT] = 1;
        if (joyinfo.dwPOV == JOY_POVRIGHT)
            m_hatdirs[JOY_DIR_RIGHT] = 1;
        if (joyinfo.dwPOV == JOY_POVFORWARD)
            m_hatdirs[JOY_DIR_UP] = 1;
        if (joyinfo.dwPOV == JOY_POVBACKWARD)
            m_hatdirs[JOY_DIR_DOWN] = 1;
    }
    ...
    if (abs(iAnalog1X) < 100)
        iAnalog1X = 0;
    if (abs(iAnalog1Y) < 100)
        iAnalog1Y = 0;
    if (abs(iAnalog2X) < 100)
        iAnalog2X = 0;
    if (abs(iAnalog2Y) < 100)
        iAnalog2Y = 0;
