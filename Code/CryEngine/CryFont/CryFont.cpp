...
static ICVar* r_DumpFontTexture = nullptr;
static ICVar* r_DumpFontNames = nullptr;
...
CCryFont::CCryFont(ISystem* pSystem) {
    ...
    if (!r_DumpFontTexture) {
        r_DumpFontTexture = pSystem->GetIConsole()->CreateVariable(
            "r_DumpFontTexture", "0", 0,
...
CCryFont::~CCryFont() {
    m_pISystem->GetIConsole()->UnregisterVariable("r_DumpFontTexture", true);
    m_pISystem->GetIConsole()->UnregisterVariable("r_DumpFontNames", true);
    r_DumpFontTexture = nullptr;
    r_DumpFontNames = nullptr;
