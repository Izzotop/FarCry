...
static inline CFFont::eBlendMode GetBlendModeFromString(const string& str) {
    ...
    } else if (str == "srccolor" || str == "src_color") {
        blend = CFFont::BLEND_SRCCOLOR;
    } else if (str == "invsrccolor" || str == "inv_src_color") {
        blend = CFFont::BLEND_INVSRCCOLOR;
    } else if (str == "srcalpha" || str == "src_alpha") {
        blend = CFFont::BLEND_SRCALPHA;
    } else if (str == "invsrcalpha" || str == "inv_src_alpha") {
        blend = CFFont::BLEND_INVSRCALPHA;
    } else if (str == "dstalpha" || str == "dst_alpha") {
        blend = CFFont::BLEND_DSTALPHA;
    } else if (str == "invdstalpha" || str == "inv_dst_alpha") {
        blend = CFFont::BLEND_INVDSTALPHA;
    } else if (str == "dstcolor" || str == "dst_color") {
        blend = CFFont::BLEND_DSTCOLOR;
    } else if (str == "invdstcolor" || str == "inv_dst_color") {
...
public:
    CXmlFontShader(CFFont* pFont) {
        ...
        m_bNoRescale = 0, m_FontSmoothAmount = 0;
    ...
    void FoundElement(const string& name, const string& value) {
        ...
        } else if (name == "pos" || name == "offset") {
            m_nElement = ELEMENT_PASS_POSOFFSET;
        } else if (name == "blend" || name == "blending") {
    ...
    void FoundAttribute(const string& name, const string& value) {
        ...
        case ELEMENT_PASS_COLOR:
            if (!m_pPass)
                break;
            if (name == "r") {
                m_pPass->cColor.r = (float)atof(value.c_str());
            } else if (name == "g") {
                m_pPass->cColor.g = (float)atof(value.c_str());
            } else if (name == "b") {
                m_pPass->cColor.b = (float)atof(value.c_str());
            } else if (name == "a") {
                m_pPass->cColor.a = (float)atof(value.c_str());
            }
            break;
        ...
        case ELEMENT_PASS_SIZESCALE:
            ...
            if (name == "x") {
                ...
                m_pPass->vSizeScale.x = m_pPass->vSizeScale.y = (float)atof(value.c_str());
            }
            break;
