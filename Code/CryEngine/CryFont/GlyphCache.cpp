...
CGlyphCache::CGlyphCache() : m_dwUsage(1), m_iGlyphBitmapWidth(0), m_iGlyphBitmapHeight(0), m_fSizeRatio(0.8f), m_pScaleBitmap(0) {
    m_pCacheTable.clear();
    m_pSlotList.clear();
}
...
int CGlyphCache::PreCacheGlyph(wchar_t cChar) {
    ...
    if (m_pScaleBitmap) {
        ...
        m_pScaleBitmap->BlitScaledTo8(pSlot->pGlyphBitmap.GetBuffer(), 0, 0, m_pScaleBitmap->GetWidth(), m_pScaleBitmap->GetHeight(), 0, 0, pSlot->pGlyphBitmap.GetWidth(),
                                      pSlot->pGlyphBitmap.GetHeight(), pSlot->pGlyphBitmap.GetWidth());
    } else {
