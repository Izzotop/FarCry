...
CLMCompDialog::CLMCompDialog(CWnd* pParent /*=nullptr*/) : CDialog(CLMCompDialog::IDD, pParent) {}

CLMCompDialog::CLMCompDialog(ISystem* pISystem) : CDialog(CLMCompDialog::IDD, nullptr) {
    m_pISystem = pISystem;
...
#else
    sParam.m_bDebugBorders = false;
    sParam.m_bDontMergePolys = false;
#endif
...
afx_msg LRESULT CLMCompDialog::OnUpdateLightMapGenerationProgress(WPARAM wParam, LPARAM lParam) {
    CProgressCtrl* pCtrl((CProgressCtrl*)GetDlgItem(IDC_LM_GEN_PROGRESS));
    assert(0 != pCtrl);
...
afx_msg LRESULT CLMCompDialog::OnUpdateLightMapGenerationMemUsage(WPARAM wParam, LPARAM lParam) {
    CProgressCtrl* pCtrl((CProgressCtrl*)GetDlgItem(IDC_LM_MEMUSAGE));
    assert(0 != pCtrl);
    pCtrl->SetPos((int)wParam);
...
afx_msg LRESULT CLMCompDialog::OnUpdateLightMapGenerationMemUsageStatic(WPARAM wParam, LPARAM lParam) {
...
afx_msg LRESULT CLMCompDialog::OnUpdateBrushInfoEdit(WPARAM wParam, LPARAM lParam) {
    CEdit* pEditBrushName((CEdit*)GetDlgItem(IDC_EDIT_BRUSHNAME));
    assert(0 != pEditBrushName);
    CEdit* pEditBrushType((CEdit*)GetDlgItem(IDC_EDIT_BRUSHTYPE));
    assert(0 != pEditBrushType);
...
afx_msg void CLMCompDialog::OnReleasedCaptureTexelSize(NMHDR* pNMHDR, LRESULT* pResult) {
    CSliderCtrl* pCtrl((CSliderCtrl*)GetDlgItem(IDC_TEXEL_SIZE_SLIDER));
    assert(0 != pCtrl);
    CStatic* pStatic((CStatic*)GetDlgItem(IDC_TEXEL_SIZE));
    assert(0 != pStatic);
...
afx_msg void CLMCompDialog::OnReleasedCaptureAngleSlider(NMHDR* pNMHDR, LRESULT* pResult) {
