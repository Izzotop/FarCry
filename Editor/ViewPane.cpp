...
void CViewPane::OnDraw(CDC* pDC) {
    ...
    // active view
    VERIFY(cFont.CreateFont(-::MulDiv(8, GetDeviceCaps(dc.m_hDC, LOGPIXELSY), 72), 0, 0, 0, (active) ? FW_BOLD : FW_DONTCARE, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
...
BOOL CViewPane::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo) {
    ...
    // in the view. m_pActiveShape is nullptr if no shape object
    // is currently selected in the view.
    if ((m_viewport != nullptr) && m_viewport->OnCmdMsg(nID, nCode, pExtra, pHandlerInfo))
        return TRUE;
