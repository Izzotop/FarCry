...
template <class THemisphereSink> class CHeightmapAccessibility {
public:
    ...
    CHeightmapAccessibility(const DWORD indwWidth, const DWORD indwHeight, const DWORD indwAngleSteps, const float infAngleStart = 0.0f,
                            const float infAngleEnd = (float)(gf_PI_MUL_2))
        : m_Sink(indwAngleSteps) {
...
private:
    ...
    void CalcHeightmapAccessWedge(const float* inpHeightmap, const int iLineDirX, const int iLineDirY, const int iLineStepX, const int iLineStepY, const float infStep,
                                  const DWORD indwLineCount, const DWORD indwLineLength) {
        assert(inpHeightmap);
