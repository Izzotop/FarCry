...
CLayer::CLayer() {
    ...
    // Create the DCs
    m_dcLayerTexPrev.CreateCompatibleDC(nullptr);

    // Create the bitmap
    VERIFY(m_bmpLayerTexPrev.CreateBitmap(LAYER_TEX_PREVIEW_CX, LAYER_TEX_PREVIEW_CX, 1, 32, nullptr));
...
void CLayer::DrawLayerTexturePreview(LPRECT rcPos, CDC* pDC) {
    ...
    if (m_bmpLayerTexPrev.m_hObject) {
        ...
        pDC->StretchBlt(rcPos->left, rcPos->top, rcPos->right - rcPos->left, rcPos->bottom - rcPos->top, &m_dcLayerTexPrev, 0, 0, LAYER_TEX_PREVIEW_CX, LAYER_TEX_PREVIEW_CY,
                        SRCCOPY);
    } else {
...
bool CLayer::LoadTexture(CString strFileName) {
    ...
    VERIFY(dcLoad.CreateCompatibleDC(nullptr));
    ...
    m_dcLayerTexPrev.StretchBlt(0, 0, LAYER_TEX_PREVIEW_CX, LAYER_TEX_PREVIEW_CY, &dcLoad, 0, 0, m_texture.GetWidth(), m_texture.GetHeight(), SRCCOPY);
    dcLoad.DeleteDC();
...
bool CLayer::LoadTexture(DWORD* pBitmapData, UINT iWidth, UINT iHeight) {
    ...
    CDC dcLoad;
    CBitmap bmpLoad;
    DWORD *pPixData = nullptr, *pPixDataEnd = nullptr;
    ...
    // Copy it to the preview bitmap
    m_dcLayerTexPrev.SetStretchBltMode(COLORONCOLOR);
    m_dcLayerTexPrev.StretchBlt(0, 0, LAYER_TEX_PREVIEW_CX, LAYER_TEX_PREVIEW_CY, &dcLoad, 0, 0, iWidth, iHeight, SRCCOPY);
...
void CLayer::UpdateLayerMask16(float* pHeightmapPixels, UINT iHeightmapWidth, UINT iHeightmapHeight, bool bStreamFromDisk) {
    ////////////////////////////////////////////////////////////////////////
    ...
    uchar* pData = nullptr;
    uchar* pArrayEnd = nullptr;
    DWORD* pPreviewData = nullptr;
    DWORD* pPreviewDataStart = nullptr;
    unsigned int i, j;
    long iCurPos;
    float hVal = 0.0f;
    FILE* hLayerFile = nullptr;
    ...
    while (pData != pArrayEnd) {
        ...
        float fs = (fabs((*(pHeightmapPixels + 1)) - hVal) + fabs((*(pHeightmapPixels - 1)) - hVal) + fabs((*(pHeightmapPixels + iHeightmapWidth)) - hVal) +
                    fabs((*(pHeightmapPixels - iHeightmapWidth)) - hVal) + fabs((*(pHeightmapPixels + iHeightmapWidth + 1)) - hVal) +
                    fabs((*(pHeightmapPixels - iHeightmapWidth - 1)) - hVal) + fabs((*(pHeightmapPixels + iHeightmapWidth - 1)) - hVal) +
    ...
    if (m_bSmooth) {
        ...
        for (j = 1; j < iHeightmapHeight - 1; j++) {
            ...
            for (i = 1; i < iHeightmapWidth - 1; i++) {
                // Smooth it out
                pLayerMask[iCurPos] = ((uint)pLayerMask[iCurPos] + pLayerMask[iCurPos + 1] + pLayerMask[iCurPos + iHeightmapWidth] + pLayerMask[iCurPos + iHeightmapWidth + 1] +
                                       pLayerMask[iCurPos - 1] + pLayerMask[iCurPos - iHeightmapWidth] + pLayerMask[iCurPos - iHeightmapWidth - 1] +
                                       pLayerMask[iCurPos - iHeightmapWidth + 1] + pLayerMask[iCurPos + iHeightmapWidth - 1]) /
                                      9;
...
void CLayer::GenerateWaterLayer16(float* pHeightmapPixels, UINT iHeightmapWidth, UINT iHeightmapHeight, float waterLevel) {
    ...
    ASSERT(!IsBadWritePtr(pHeightmapPixels, iHeightmapWidth * iHeightmapHeight * sizeof(float)));
    ...
    if (m_bSmooth) {
        ...
        for (j = 1; j < iHeightmapHeight - 1; j++) {
            ...
            for (i = 1; i < iHeightmapWidth - 1; i++) {
                ...
                // Smooth it out
                pLayerMask[iCurPos] = ftoi((pLayerMask[iCurPos] + pLayerMask[iCurPos + 1] + pLayerMask[iCurPos + iHeightmapWidth] + pLayerMask[iCurPos + iHeightmapWidth + 1] +
                                            pLayerMask[iCurPos - 1] + pLayerMask[iCurPos - iHeightmapWidth] + pLayerMask[iCurPos - iHeightmapWidth - 1] +
                                            pLayerMask[iCurPos - iHeightmapWidth + 1] + pLayerMask[iCurPos + iHeightmapWidth - 1]) *
                                           0.11111111111f);
            }
...
void CLayer::AutogenLayerMask(const CRect& rc, const CFloatImage& hmap, CByteImage& mask) {
    ...
    for (y = rect.top; y < rect.bottom; y++) {
        for (x = rect.left; x < rect.right; x++, pos++) {
            ...
            // Calculate the slope for this point
            float fs = (fabs((*(h + 1)) - hVal) + fabs((*(h - 1)) - hVal) + fabs((*(h + hw)) - hVal) + fabs((*(h - hw)) - hVal) + fabs((*(h + hw + 1)) - hVal) +
                        fabs((*(h - hw - 1)) - hVal) + fabs((*(h + hw - 1)) - hVal) + fabs((*(h - hw + 1)) - hVal));
    ...
    if (m_bSmooth) {
        ...
        for (y = rect.top; y < rect.bottom; y++) {
            ...
            for (x = rect.left; x < rect.right; x++, pos++) {
                // Smooth it out
                pLayerMask[pos] = ((uint)pLayerMask[pos] + pLayerMask[pos + 1] + pLayerMask[pos + hw] + pLayerMask[pos + hw + 1] + pLayerMask[pos - 1] + pLayerMask[pos - hw] +
                                   pLayerMask[pos - hw - 1] + pLayerMask[pos - hw + 1] + pLayerMask[pos + hw - 1]) /
                                  9;
            }
        }
    }
