...
bool COBJExporter::WriteOBJ(const char* pszFileName) {
    ...
    for (itVec = m_vVertices.begin(); itVec != m_vVertices.end(); itVec++) {
        fprintf(hFile, "v %s %s %s\n", StripZero(ppszBuffer[0], (*itVec).fX), StripZero(ppszBuffer[1], (*itVec).fY), StripZero(ppszBuffer[2], (*itVec).fZ));
    }
    ...
    for (itTexCoord = m_vTexCoords.begin(); itTexCoord != m_vTexCoords.end(); itTexCoord++) {
        fprintf(hFile, "vt %s %s\n", StripZero(ppszBuffer[0], (*itTexCoord).fU), StripZero(ppszBuffer[1], (*itTexCoord).fV));
    }
    ...
    for (itFace = m_vFaces.begin(); itFace != m_vFaces.end(); itFace++) {
        fprintf(hFile, "f %i/%i %i/%i %i/%i\n", (*itFace).iVertexIndices[0] + 1, (*itFace).iTexCoordIndices[0] + 1, (*itFace).iVertexIndices[1] + 1,
                (*itFace).iTexCoordIndices[1] + 1, (*itFace).iVertexIndices[2] + 1, (*itFace).iTexCoordIndices[2] + 1);
    }
