...
namespace {
CBrushPanel* s_brushPanel = nullptr;
int s_brushPanelId = 0;

CPanelTreeBrowser s_treePanel;
CPanelTreeBrowser* s_treePanelPtr = nullptr;
int s_treePanelId = 0;
...
Matrix44 CBrushObject::GetBrushMatrix() const {
    if (m_prefabGeom == nullptr || m_brush == nullptr) {
        return GetWorldTM();
    }
    BBox box;
...
CMaterial* CBrushObject::GetMaterial() const {
    ...
    if (!m_pMaterial)
    {
        if (m_prefabGeom != nullptr && m_prefabGeom->GetMaterial())
