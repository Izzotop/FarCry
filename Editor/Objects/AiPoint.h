...
public:
    ...
    bool IsLinkSelected(int iLink) const {
        m_links[iLink].selected;
    };
    void SelectLink(int iLink, bool bSelect) {
        m_links[iLink].selected = bSelect;
    };

    void SetAIType(EAIPointType type);
    EAIPointType GetAIType() const {
        return m_aiType;
    };
