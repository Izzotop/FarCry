...
void CVisAreaShapeObject::UpdateGameArea(bool bRemove) {
    ...
    if (m_area) {
        ...
        if (GetPointCount() > 3) {
            ...
            GetIEditor()->Get3DEngine()->UpdateVisArea(m_area, &points[0], points.size(), GetName(), GetHeight(), vAmbClr, mv_bAffectedBySun, mv_bSkyOnly, vDynAmbClr,
                                                       mv_fViewDistRatio, true, false, false);
        }
    }
...
void CPortalShapeObject::UpdateGameArea(bool bRemove) {
    ...
    if (m_area) {
        ...
        if (GetPointCount() > 3) {
            ...
            Vec3d vAmbClr = mv_vAmbientColor;
            Vec3d vDynAmbClr = mv_vDynAmbientColor;
            GetIEditor()->Get3DEngine()->UpdateVisArea(m_area, &points[0], points.size(), name, GetHeight(), vAmbClr, mv_bAffectedBySun, mv_bSkyOnly, vDynAmbClr, mv_fViewDistRatio,
                                                       mv_bDoubleSide, mv_bUseDeepness, false);
        }
    }
    m_bAreaModified = false;
}
...
void COccluderShapeObject::UpdateGameArea(bool bRemove) {
    ...
    if (m_area) {
        ...
        if (GetPointCount() > 1) {
            ...
            Vec3d vDynAmbClr = mv_vDynAmbientColor;
            GetIEditor()->Get3DEngine()->UpdateVisArea(m_area, &points[0], points.size(), name, GetHeight(), vAmbClr, mv_bAffectedBySun, mv_bSkyOnly, vDynAmbClr, mv_fViewDistRatio,
                                                       mv_bDoubleSide, false, mv_bUseInIndoors);
        }
    }
    m_bAreaModified = false;
}
