...
public:
    ...
    CString GetTypeDescription() const {
    ...
    // CEntity interface.
    //////////////////////////////////////////////////////////////////////////
    virtual bool LoadScript(const CString& entityClass, bool bForceReload = false, bool bGetScriptProperties = true, XmlNodeRef xmlProperties = XmlNodeRef(),
                            XmlNodeRef xmlProperties2 = XmlNodeRef());
    virtual void SpawnEntity();
    virtual void DeleteEntity();
    virtual void UnloadScript();

    CString GetEntityClass() const {
    ...
    int GetEventTargetCount() const {
        return m_eventTargets.size();
    };
    CEntityEventTarget& GetEventTarget(int index) {
        return m_eventTargets[index];
    };
    ...
    bool IsCastShadow() const {
        return mv_castShadows;
    }
    bool IsSelfShadowing() const {
        return mv_selfShadowing;
    }
    bool IsCastShadowMaps() const {
        return mv_castShadowMaps;
    }
    bool IsRecvShadowMaps() const {
        return mv_recvShadowMaps;
    }
    bool IsCastLightmap() const {
        return mv_castLightmap;
    }
    bool IsRecvLightmap() const {
        return mv_recvLightmap;
    }
    bool IsHideable() const {
        return mv_recvLightmap;
    }
    float GetRatioLod() const {
        return mv_ratioLOD;
    };
    float GetRatioViewDist() const {
        return mv_ratioViewDist;
    }
...
public:
    ...
    CRuntimeClass* GetRuntimeClass() {
        return RUNTIME_CLASS(CSimpleEntity);
    };
    const char* GetFileSpec() {
        return "Objects\\*.cgf;*.ccgf;*.cga";
    };
    int GameCreationOrder() {
        return 201;
    };
