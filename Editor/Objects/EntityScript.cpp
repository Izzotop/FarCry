...
public:
    explicit CScriptPropertiesDump(CVarBlock* pVarBlock, IVariable* pParentVar = nullptr) {
...
void CEntityScript::SetEventsTable(CEntity* entity) {
    ...
    // If events target table is nullptr, set event table to nullptr either.
    if (entity->GetEventTargetCount() == 0) {
        if (m_haveEventsTable) {
            scriptObject->SetTonullptr("Events");
        }
        m_haveEventsTable = false;
        return;
    }
