...
CPropertiesPanel::CPropertiesPanel(CWnd* pParent /*=nullptr*/) : CDialog(CPropertiesPanel::IDD, pParent) {
...
void CPropertiesPanel::AddVars(CVarBlock* vb, const UpdateCallback& updCallback) {
    ...
    if (bNewBlock) {
        ...
        if (h > 400)
            h = 400;
        SetWindowPos(nullptr, 0, 0, rc.right, h + HEIGHT_OFFSET * 2 + 4, SWP_NOMOVE);
