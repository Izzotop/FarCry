...
bool CViewport::Create(CWnd* hWndParent, int id, const char* szTitle) {
    ...
    // Create the window
    /*
    bReturn = CreateEx(nullptr, AfxRegisterWndClass(CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW,
        AfxGetApp()->LoadStandardCursor(IDC_CROSS), nullptr, nullptr), szTitle, WS_CHILD
        | WS_VISIBLE | WS_CLIPCHILDREN, rcDefault, hWndParent, nullptr);
        */

    int wndStyle = (hWndParent != 0) ? WS_CHILD : WS_POPUP;

    bReturn = CreateEx(nullptr, AfxRegisterWndClass(CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW | CS_OWNDC, AfxGetApp()->LoadStandardCursor(IDC_ARROW), nullptr, nullptr), szTitle,
                       wndStyle | WS_CLIPCHILDREN, rcDefault, hWndParent, nullptr);
...
void CViewport::OnLButtonDown(UINT nFlags, CPoint point) {
    ...
    if (GetViewMode() == MoveMode || GetViewMode() == RotateMode || GetViewMode() == ScaleMode) {
        BeginUndo();
    }
