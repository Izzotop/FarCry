...
CTerrainLighting::CTerrainLighting(CWnd* pParent /*=nullptr*/) : CDialog(CTerrainLighting::IDD, pParent), m_texGen(LIGHTING_PREVIEW_RESOLUTION) {
...
void CTerrainLighting::OnGenerate() {
    ...
    if (!m_dcLightmap.m_hDC)
        VERIFY(m_dcLightmap.CreateCompatibleDC(nullptr));
    if (!m_bmpLightmap.m_hObject)
        VERIFY(m_bmpLightmap.CreateBitmap(LIGHTING_PREVIEW_RESOLUTION, LIGHTING_PREVIEW_RESOLUTION, 1, 32, nullptr));
