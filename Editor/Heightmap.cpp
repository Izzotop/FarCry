...
protected:
    ...
    virtual int GetSize() {
        return sizeof(*this) + m_undo.GetSize() + m_redo.GetSize();
    };
    virtual const char* GetDescription() {
        return "Heightmap Modify";
    };
...
void CHeightmap::SaveImage(LPCSTR pszFileName, bool bNoiseAndFilter) {
    uint* pImageData = nullptr;
    unsigned int i, j;
    uint8 iColor;
    t_hmap* pHeightmap = nullptr;
...
void CHeightmap::LoadPGM(const CString& pgmFile) {
    ...
    if (image.GetWidth() != m_iWidth || image.GetHeight() != m_iHeight) {
        MessageBox(nullptr, "PGM dimensions do not match dimensions of heighmap", "Warning", MB_OK | MB_ICONEXCLAMATION);
        return;
    }
...
void CHeightmap::SaveRAW(const CString& rawFile) {
    ...
    if (!file) {
        ...
        MessageBox(nullptr, str, "Warning", MB_OK | MB_ICONEXCLAMATION);
        return;
    }
    ...
    for (j = 1; j < m_iHeight - 1; j++) {
        ...
        for (i = 1; i < m_iWidth - 1; i++) {
            ...
            if (m_pHeightmap[iCurPos]) {
                ...
                m_pHeightmap[iCurPos] = __min(255.0f, __max(m_fWaterLevel, m_pHeightmap[iCurPos] + m_pNoise->m_Array[iNoiseX % NOISE_ARRAY_SIZE][iNoiseY % NOISE_ARRAY_SIZE]));
            }
        }
    }
...
void CHeightmap::Smooth(CFloatImage& hmap, const CRect& rect) {
    ...
    for (j = y1; j < y2; j++) {
        pos = j * w;
        for (i = x1; i < x2; i++) {
            pData[i + pos] = (pData[i + pos] + pData[i + 1 + pos] + pData[i - 1 + pos] + pData[i + pos + w] + pData[i + pos - w] + pData[(i - 1) + pos - w] +
                              pData[(i + 1) + pos - w] + pData[(i - 1) + pos + w] + pData[(i + 1) + pos + w]) *
                             (1.0f / 9.0f);
        }
    }
    /*
    ...
    for (i = 1; i < m_iWidth - 1; i++)
        for (j = 1; j < m_iHeight - 1; j++) {
            m_pHeightmap[i + j * m_iWidth] =
                (m_pHeightmap[i + j * m_iWidth] + m_pHeightmap[(i + 1) + j * m_iWidth] + m_pHeightmap[i + (j + 1) * m_iWidth] + m_pHeightmap[(i + 1) + (j + 1) * m_iWidth] +
                 m_pHeightmap[(i - 1) + j * m_iWidth] + m_pHeightmap[i + (j - 1) * m_iWidth] + m_pHeightmap[(i - 1) + (j - 1) * m_iWidth] +
                 m_pHeightmap[(i + 1) + (j - 1) * m_iWidth] + m_pHeightmap[(i - 1) + (j + 1) * m_iWidth]) /
                9.0f;
        }
    ...
    if (res) {
        ...
        for (int iY = 0; iY < h; iY++)
            for (int iX = 0; iX < w; iX++) {
                ...
                uint val = ftoi(*pH);
                pBitmapData[iX + iY * w] = (*pH < iWaterLevel) ? pWaterTexData[(iX % 128) + (iY % 128) * 128] : RGB(val, val, val);
...
void CHeightmap::GenerateTerrain(const SNoiseParams& noiseParam) {
    ...
    float* pArrayShortcut = nullptr;
    DWORD* pImageData = nullptr;
    DWORD* pImageDataStart = nullptr;
    ...
    for (i = 0; i < sParam.iPasses; i++) {
        ...
        cNoise.FracSynthPass(&cHeightmap, sParam.fFrequency, fYScale, sParam.iWidth, sParam.iHeight, FALSE);
...
void CHeightmap::SmoothSlope() {
    ...
    for (j = 1; j < m_iHeight - 1; j++) {
        ...
        for (i = 1; i < m_iWidth - 1; i++) {
            ...
            fAverage = (m_pHeightmap[iCurPos] + m_pHeightmap[iCurPos + 1] + m_pHeightmap[iCurPos + m_iWidth] + m_pHeightmap[iCurPos + m_iWidth + 1] + m_pHeightmap[iCurPos - 1] +
                        m_pHeightmap[iCurPos - m_iWidth] + m_pHeightmap[iCurPos - m_iWidth - 1] + m_pHeightmap[iCurPos - m_iWidth + 1] + m_pHeightmap[iCurPos + m_iWidth - 1]) *
                       0.11111111111f;
...
void CHeightmap::MakeBeaches() {
    ...
    unsigned int i, j;
    t_hmap* pHeightmapData = nullptr;
    t_hmap* pHeightmapDataStart = nullptr;
    double dCurHeight;
...
void CHeightmap::DrawSpot(unsigned long iX, unsigned long iY, uint8 iWidth, float fAddVal, float fSetToHeight, bool bAddNoise) {
    ////////////////////////////////////////////////////////////////////////
    ...
    for (j = (long)-iWidth; j < iWidth; j++) {
        ...
        for (i = (long)-iWidth; i < iWidth; i++) {
            ...
            if (iPosX < 0 || iPosY < 0 || iPosX > (long)m_iWidth - 1 || iPosY > (long)m_iHeight - 1) {
                continue;
            }
            ...
            if (fSetToHeight >= 0.0f) {
                ...
                // Noise brush
                if (fAddVal > 0.0f) {
                    m_pHeightmap[iIndex] += fAddVal / 100 * ((float)fabs(m_pNoise->m_Array[iPosX % NOISE_ARRAY_SIZE][iPosY % NOISE_ARRAY_SIZE])) * fAttenuation;
                } else {
                    m_pHeightmap[iIndex] += fAddVal / 100 * (float)(-fabs(m_pNoise->m_Array[iPosX % NOISE_ARRAY_SIZE][iPosY % NOISE_ARRAY_SIZE])) * fAttenuation;
                }
            } else {
...
void CHeightmap::SmoothSpot(int iX, int iY, int radius, float fHeight, float fHardness) {
    ...
    for (j = (long)-radius; j < radius; j++) {
        ...
        for (i = (long)-radius; i < radius; i++) {
            ...
            float h;
            h = (m_pHeightmap[pos] + m_pHeightmap[pos + 1] + m_pHeightmap[pos - 1] + m_pHeightmap[pos + m_iWidth] + m_pHeightmap[pos - m_iWidth] +
                 m_pHeightmap[pos + 1 + m_iWidth] + m_pHeightmap[pos + 1 - m_iWidth] + m_pHeightmap[pos - 1 + m_iWidth] + m_pHeightmap[pos - 1 - m_iWidth]) /
                9.0f;
