...
private:
    void OnAddNode(){};
    void OnRemoveNode() {
        if (m_pDlg && (!m_pDlg->m_bReloading))
            m_pDlg->ReloadSequences();
    }
    void OnChangeNode() {
        if (m_pDlg && (!m_pDlg->m_bReloading))
            m_pDlg->ReloadSequences();
...
CTrackViewDialog::CTrackViewDialog(CWnd* pParent /*=nullptr*/) : CToolbarDialog(CTrackViewDialog::IDD, pParent) {
    m_currSequence = 0;
    m_bRecord = false;
    m_bAutoRecord = false;
    m_bPause = false;
...
void CTrackViewDialog::OnRecordMenu(CPoint pos) {
    ...
    SScales Scales[] = {
        {" 1 sec ", 1.0f}, {"1/2 sec", 1.0f / 2}, {"1/5 sec", 1.0f / 5}, {"1/10 sec", 1.0f / 10}, {"1/25 sec", 1.0f / 25}, {"1/50 sec", 1.0f / 50}, {"1/100 sec", 1.0f / 100},
    };
...
void CTrackViewDialog::OnPlayMenu(CPoint pos) {
    ...
    SScales Scales[] = {
        {" 2 ", 2.0f}, {" 1 ", 1.0f}, {"1/2", 0.5f}, {"1/4", 0.25f}, {"1/8", 0.125f},
    };
    int nScales = sizeof(Scales) / sizeof(SScales);
    CMenu menu;
    menu.CreatePopupMenu();
...
BOOL CTrackViewDialog::OnInitDialog() {
    ...
    // Resize the toolbar
    GetClientRect(rc);
    m_cDlgToolBar.SetWindowPos(nullptr, 0, 0, rc.right, 70, SWP_NOZORDER);
    ...
    m_wndKeys.Create(nullptr, nullptr, WS_VISIBLE | WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_HSCROLL, rc, this, 2);
    ...
    // m_wndGraph.Create( nullptr,nullptr,WS_VISIBLE|WS_CHILD|WS_CLIPCHILDREN|WS_CLIPSIBLINGS|WS_HSCROLL,rc,this,2 );
    // m_wndGraph.SetTimeRange( 0,20 );
    // m_wndGraph.SetTimeScale( 100 );
    // m_wndGraph.SetCurrTime( 1 );
    // m_wndGraph.SetTrackDialog( &m_wndTrackProps );

    m_wndSpline.Create(nullptr, nullptr,
                       WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | LBS_NOINTEGRALHEIGHT | LBS_OWNERDRAWFIXED | LBS_WANTKEYBOARDINPUT | LBS_DISABLENOSCROLL | WS_HSCROLL, rc,
                       this, 3);
...
void CTrackViewDialog::OnDestroy() {
    ...
    if (GetMovieSystem())
        GetMovieSystem()->SetCallback(nullptr);
}
