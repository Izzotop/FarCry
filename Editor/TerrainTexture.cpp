...
//////////////////////////////////////////////////////////////////////////
CTerrainTexture::CTerrainTexture(CWnd* pParent /*=nullptr*/) : CDialog(CTerrainTexture::IDD, pParent) {
    ...
    if (!m_dcFinalTexPrev.m_hDC)
        m_dcFinalTexPrev.CreateCompatibleDC(nullptr);
    if (!m_bmpFinalTexPrev.m_hObject)
        VERIFY(m_bmpFinalTexPrev.CreateBitmap(FINAL_TEX_PREVIEW_PRECISION_CX, FINAL_TEX_PREVIEW_PRECISION_CY, 1, 32, nullptr));
    ...
    m_dcLayerPreview.CreateCompatibleDC(nullptr);
    // Create a new bitmap for the preview
    VERIFY(m_bmpLayerPreview.CreateBitmap(FINAL_TEX_PREVIEW_PRECISION_CX, FINAL_TEX_PREVIEW_PRECISION_CX, 1, 32, nullptr));
...
CTerrainTexture::~CTerrainTexture() {
    m_dcLayerPreview.SelectObject((CBitmap*)nullptr);
}
...
void CTerrainTexture::EnableControls() {
    ...
    GetMenu()->GetSubMenu(1)->EnableMenuItem(IDC_LOAD_TEXTURE, bEnable ? MF_ENABLED : MF_GRAYED);
    GetMenu()->GetSubMenu(1)->EnableMenuItem(ID_LAYER_EXPORTTEXTURE, bEnable ? MF_ENABLED : MF_GRAYED);
    GetMenu()->GetSubMenu(1)->EnableMenuItem(IDC_REMOVE_LAYER, bEnable ? MF_ENABLED : MF_GRAYED);
    ...
    GetMenu()->GetSubMenu(0)->EnableMenuItem(IDC_EXPORT, m_lstLayers.GetCount() ? MF_ENABLED : MF_GRAYED);
    GetMenu()->GetSubMenu(2)->EnableMenuItem(IDC_GENERATE_PREVIEW, m_lstLayers.GetCount() ? MF_ENABLED : MF_GRAYED);
    GetMenu()->GetSubMenu(2)->EnableMenuItem(ID_FILE_EXPORTLARGEPREVIEW, m_lstLayers.GetCount() ? MF_ENABLED : MF_GRAYED);
...
void CTerrainTexture::UpdateControlData() {
    ...
    if (m_pCurrentLayer) {
        ...
        if (m_pCurrentLayer->GetTextureFilename().GetLength())
            sprintf(szBuffer, "%s\n%i x %i", LPCTSTR(m_pCurrentLayer->GetTextureFilename()), m_pCurrentLayer->GetTextureWidth(), m_pCurrentLayer->GetTextureHeight());
        else
            szBuffer[0] = '\0';
...
void CTerrainTexture::OnPaint() {
    ...
    dc.StretchBlt(rcTex.left, rcTex.top, rcTex.right - rcTex.left, rcTex.bottom - rcTex.top, &m_dcFinalTexPrev, 0, 0, FINAL_TEX_PREVIEW_PRECISION_CX,
                  FINAL_TEX_PREVIEW_PRECISION_CY, SRCCOPY);
...
void CTerrainTexture::DrawLayerPreview(LPRECT rcPos, CDC* pDC) {
    ...
    if (m_bmpLayerPreview.m_hObject && m_bMaskPreviewValid) {
        ...
        pDC->StretchBlt(rcPos->left, rcPos->top, rcPos->right - rcPos->left, rcPos->bottom - rcPos->top, &m_dcLayerPreview, 0, 0, FINAL_TEX_PREVIEW_PRECISION_CX,
                        FINAL_TEX_PREVIEW_PRECISION_CX, SRCCOPY);
    } else {
...
void CTerrainTexture::ExportLargePreview() {
    ...
    CLogFile::FormatLine("Exporting large surface texture preview (%ix%i)...", cDialog.GetDimensions(), cDialog.GetDimensions());
...
bool CTerrainTexture::GenerateSurface(DWORD* pSurface, UINT iWidth, UINT iHeight, int flags, CBitArray* pLightingBits, float** ppHeightmapData) {
    ...
    // results of the terrain lighting if pLightingBit is not nullptr. Also,
    // if ppHeightmapData is not nullptr, the allocated heightmap data will
    ...
    DWORD* pTextureDataWrite = nullptr;
    CLayer* pLayerShortcut = nullptr;
    int iFirstUsedLayer;
    float* pHeightmapData = nullptr;
    ...
    if (bConvertToABGR) {
        ...
        for (iTexY = 0; iTexY < (int)iHeight; iTexY++) {
            for (iTexX = 0; iTexX < (int)iWidth; iTexX++) {
                *pTextureDataWrite++ = ((*pTextureDataWrite & 0x00FF0000) >> 16) | (*pTextureDataWrite & 0x0000FF00) | ((*pTextureDataWrite & 0x000000FF) << 16);
            }
        }
    }
...
void CTerrainTexture::OnLayerExportTexture() {
    ...
    CFileDialog dlg(FALSE, "bmp", nullptr, OFN_EXPLORER | OFN_OVERWRITEPROMPT | OFN_NOCHANGEDIR, szFilters);
    CFile cFile;
...
void CTerrainTexture::ClearTexturePreview() {
    ...
    if (m_dcFinalTexPrev.m_hDC) {
        ::SetRect(&rcPos, 0, 0, FINAL_TEX_PREVIEW_PRECISION_CX, FINAL_TEX_PREVIEW_PRECISION_CY);
...
void CTerrainTexture::OnFetch() {
    ...
    if (m_doc->IsModified()) {
        if (AfxMessageBox("The document contains unsaved data, really fetch old state ?", MB_ICONQUESTION | MB_YESNO, nullptr) != IDYES) {
            return;
        }
    }
