...
void CGameExporter::ExportHeightMap(char* pszGamePath) {
    ...
    t_hmap* pHeightmapBits = nullptr;
    t_hmap* pHeightmapBitsStart = nullptr;
    t_hmap* pHeightmapBitsEnd = nullptr;
    uint16* pSaveHeightmapData = nullptr;
    uint16* pSaveHeightmapDataStart = nullptr;
    uint16* pSaveHeightmapDataEnd = nullptr;
    CHeightmap& HeightMap = m_IEditor->GetDocument()->GetHeightmap();
    ...
    if (pSaveHeightmapDataStart) {
        ...
        pSaveHeightmapDataStart = nullptr;
    }
    ...
    for (iCurTexelY = 1; iCurTexelY < iExportTexWidth; iCurTexelY++) {
        for (iCurTexelX = 1; iCurTexelX < iExportTexWidth; iCurTexelX++) {
            ...
            if (iCurTexelX % lSectorDimensions == 0) {
                ...
                // Write the back
                pSurface[iCurTexelX + iCurTexelY * iExportTexWidth] = pSurface[iCurTexelX - 1 + iCurTexelY * iExportTexWidth] = dwBlendColor;
            }
            ...
            if (iCurTexelY % lSectorDimensions == 0) {
                ...
                // Write the back
                pSurface[iCurTexelX + iCurTexelY * iExportTexWidth] = pSurface[iCurTexelX + iCurTexelY * iExportTexWidth - iExportTexWidth] = dwBlendColor;
            }
        }
    }
