...
CMusicInfoDlg::CMusicInfoDlg(CWnd* pParent /*=nullptr*/) : CDialog(CMusicInfoDlg::IDD, pParent) {}
...
void CMusicInfoDlg::Resize() {
    ...
    if (m_wndPlayingFrame.GetSafeHwnd())
        m_wndPlayingFrame.SetWindowPos(nullptr, 0, 0, rcClient.Width() - (m_rcClient.Width() - m_rcPlayingFrame.Width()),
                                       rcClient.Height() - (m_rcClient.Height() - m_rcPlayingFrame.Height()), SWP_NOZORDER | SWP_NOMOVE);
    if (m_wndPlaying.GetSafeHwnd()) {
        m_wndPlaying.SetWindowPos(nullptr, 0, 0, rcClient.Width() - (m_rcClient.Width() - m_rcPlaying.Width()), rcClient.Height() - (m_rcClient.Height() - m_rcPlaying.Height()),
...
BOOL CMusicInfoDlg::OnInitDialog() {
    ...
    m_hTimer = SetTimer(1, 250, nullptr);
