...
CTcbKeyDialog::CTcbKeyDialog(CWnd* pParent /*=nullptr*/) : IKeyDlg(CTcbKeyDialog::IDD, pParent) {
    m_track = 0;
    m_node = 0;
    m_key = -1;
}
...
void CTcbKeyDialog::OnUpdateValue() {
    ...
    if (!bUndoRecorded) {
        if (key.tens != m_tcb[0].GetValue() || key.cont != m_tcb[1].GetValue() || key.bias != m_tcb[2].GetValue() || key.easeto != m_tcb[3].GetValue() ||
