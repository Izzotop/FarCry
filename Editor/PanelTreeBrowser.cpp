...
CPanelTreeBrowser::CPanelTreeBrowser(CWnd* pParent /*=nullptr*/) : CXTResizeDialog(CPanelTreeBrowser::IDD, pParent) {
...
BOOL CPanelTreeBrowser::OnInitDialog() {
    ...
    //SetWindowPos( nullptr,0,0,rc.right,h+80,SWP_NOMOVE );
    ...
    if (cTempFiles.StartEnumeration(szFolder, "*.*", &sFile)) {
        do {
            ...
            if (sFile.attrib & _A_SUBDIR) {
                ...
                if (_stricmp(sFile.name, ".") == 0 || _stricmp(sFile.name, "..") == 0) {
                    continue;
                }
...
void CPanelTreeBrowser::OnDblclkBrowserTree(NMHDR* pNMHDR, LRESULT* pResult) {
    *pResult = 0;
    ...
    if (m_tree.GetSelectedItem() == nullptr)
        return;
...
CString CPanelTreeBrowser::GetSelectedFile() {
    ...
    // currently selected file, just a nullptr terminated string will be
    // returned
    ////////////////////////////////////////////////////////////////////////
