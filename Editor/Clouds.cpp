...
bool CClouds::GenerateClouds(SNoiseParams* sParam, CWnd* pWndStatus) {
    ...
    // Create a new DC
    m_dcClouds.CreateCompatibleDC(nullptr);

    // Create a new bitmap
    VERIFY(m_bmpClouds.CreateBitmap(512, 512, 1, 32, nullptr));
...
void CClouds::DrawClouds(CDC* pDC, LPRECT prcPosition) {
    ...
    if (m_dcClouds.m_hDC) {
        ...
        pDC->StretchBlt(prcPosition->left, prcPosition->top, prcPosition->right, prcPosition->bottom, &m_dcClouds, 0, 0, m_iWidth, m_iHeight, SRCCOPY);
    } else {
        // Just draw a black rectangle
        pDC->StretchBlt(prcPosition->left, prcPosition->top, prcPosition->right, prcPosition->bottom, &m_dcClouds, 0, 0, m_iWidth, m_iHeight, BLACKNESS);
    }
}
...
void CClouds::CalcCloudsPerlin(SNoiseParams* sParam, CWnd* pWndStatus) {
    ...
    for (i = 0; i < sParam->iPasses; i++) {
        ...
        cNoise.FracSynthPass(&cCloud, sParam->fFrequency, fYScale, sParam->iWidth, sParam->iHeight, FALSE);
    ...
    for (i = 0; i < sParam->iWidth; i++)
        for (j = 0; j < sParam->iHeight; j++) {
            ...
            cCloud.m_Array[i][j] = CloudExpCurve(cCloud.m_Array[i][j], sParam->iCover, sParam->iSharpness);
    ...
    for (h = 0; h < sParam->iSmoothness; h++)
        for (i = 1; i < sParam->iWidth - 1; i++)
            for (j = 1; j < sParam->iHeight - 1; j++) {
                cCloud.m_Array[i][j] = (cCloud.m_Array[i][j] + cCloud.m_Array[i + 1][j] + cCloud.m_Array[i][j + 1] + cCloud.m_Array[i + 1][j + 1] + cCloud.m_Array[i - 1][j] +
                                        cCloud.m_Array[i][j - 1] + cCloud.m_Array[i - 1][j - 1] + cCloud.m_Array[i + 1][j - 1] + cCloud.m_Array[i - 1][j + 1]) /
                                       9.0f;
            }
    ...
    for (i = 0; i < sParam->iWidth; i++)
        for (j = 0; j < sParam->iHeight; j++) {
            ...
            if (sParam->bBlueSky) {
                ...
                clrfColor = ((int)(fScaledCol * 255 + (1 - fScaledCol) * 40)) | ((int)(fScaledCol * 255 + (1 - fScaledCol) * 90)) << 8 |
                            ((int)(fScaledCol * 255 + (1 - fScaledCol) * 180)) << 16;
            } else {
                fScaledCol = cCloud.m_Array[i][j] / 255.0f;
                clrfColor = ((int)(fScaledCol * 255 + (1 - fScaledCol))) | ((int)(fScaledCol * 255 + (1 - fScaledCol))) << 8 | ((int)(fScaledCol * 255 + (1 - fScaledCol))) << 16;
            }
