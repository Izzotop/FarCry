...
void CLogFile::LogString(const char* pszString, bool onlyFile) {
    ...
    if (m_hWndListBox) {
        ...
        SendMessage(m_hWndListBox, LB_SETTOPINDEX, SendMessage(m_hWndListBox, LB_GETCOUNT, 0, 0) - 1, 0);
    }
...
void CLogFile::AboutSystem() {
    ...
    */
    ...
    sprintf(szBuffer, "%dMB phys. memory installed, %dMB paging available", MemoryStatus.dwTotalPhys / 1048576 + 1, MemoryStatus.dwAvailPageFile / 1048576);
    LogLine(szBuffer);
    ...
    GetPrivateProfileString("boot.description", "display.drv", "(Unknown graphics card)", szProfileBuffer, sizeof(szProfileBuffer), "system.ini");
    sprintf(szBuffer, "Current display mode is %dx%dx%d, %s", DisplayConfig.dmPelsWidth, DisplayConfig.dmPelsHeight, DisplayConfig.dmBitsPerPel, szProfileBuffer);
    LogLine(szBuffer);
    ...
    if (!GetSystemMetrics(SM_MOUSEPRESENT))
        ...
        sprintf(szBuffer, " keyboard and %i+ button mouse installed", GetSystemMetrics(SM_CMOUSEBUTTONS));
...
//////////////////////////////////////////////////////////////////////////////////////////////////
// Checks the verbosity of the message and returns nullptr if the message must NOT be
...
const char* CLogFile::CheckAgainstVerbosity(const char* pText, bool& bOnlyFile) {
    ...
    if ((unsigned char)pText[0] > nMaxVerbosity) {
        ...
        if (nLogVerbosity >= (unsigned char)pText[0])
            // if the text is an empty string, a nullptr will be returned
            return pText[0] ? pText + 1 : nullptr;
        else {
            bOnlyFile = true;
            return pText[0] ? pText + 1 : nullptr;
        }
