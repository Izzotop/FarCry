...
protected:
    ...
    struct ItemDesc {
        ...
        //////////////////////////////////////////////////////////////////////////
        ItemDesc() : pParentItem(0), type(EITEM_THEME), layerType(ELAYER_MAIN), pItem(0), m_pTheme(0), m_pMood(0), m_pPatternSet(0), m_pPattern(0){};
        ItemDesc(ItemDesc& item, ELayerType lt)
            : pParentItem(&item), type(EITEM_LAYER), layerType(lt), pItem(item.pItem), m_pTheme(item.m_pTheme), m_pMood(item.m_pMood), m_pPatternSet(item.m_pPatternSet),
              m_pPattern(item.m_pPattern){};
        ItemDesc(ItemDesc& item)
            : pParentItem(&item), type(item.type), layerType(item.layerType), pItem(item.pItem), m_pTheme(item.m_pTheme), m_pMood(item.m_pMood), m_pPatternSet(item.m_pPatternSet),
              m_pPattern(item.m_pPattern){};
    };
    ...
    // Adding
    //////////////////////////////////////////////////////////////////////////
    SMusicTheme* NewTheme(const char* sBaseName = nullptr);
    SMusicMood* NewMood(SMusicTheme* pTheme, const char* sBaseName = nullptr, bool bNewPatternSet = true);
    SMusicPatternSet* NewPatternSet(SMusicMood* pMood);
    SPatternDef* NewPattern(SMusicPatternSet* pPatternSet, ELayerType layer, const char* sBaseName = nullptr, XmlNodeRef& node = XmlNodeRef(0));
