...
public:
    CMusicLoadSinkTemp(SMusicData* pMusicData, IScriptSystem* pScriptSystem, _SmartScriptObject* pObj, int nMode = MUSICLOAD_MODE_BASE, SPatternDef* pPattern = nullptr,
                       SMusicTheme* pTheme = nullptr, SMusicMood* pMood = nullptr, SMusicPatternSet* pPatternSet = nullptr, int nLayer = MUSICLAYER_MAIN) {
        ...
        case svtObject: {
            ...
            case MUSICLOAD_MODE_THEMES: {
                ...
                if (pObj->GetValue("Moods", pSubObj)) {
                    CMusicLoadSinkTemp MoodsSink(m_pMusicData, m_pScriptSystem, &pSubObj, MUSICLOAD_MODE_MOODS, nullptr, pTheme);
                    pSubObj->Dump(&MoodsSink);
                }
                if (pObj->GetValue("Bridges", pSubObj)) {
                    CMusicLoadSinkTemp BridgesSink(m_pMusicData, m_pScriptSystem, &pSubObj, MUSICLOAD_MODE_BRIDGES, nullptr, pTheme);
            ...
            case MUSICLOAD_MODE_MOODS: {
                ...
                if (pObj->GetValue("PatternSet", pSubObj)) {
                    CMusicLoadSinkTemp Sink(m_pMusicData, m_pScriptSystem, &pSubObj, MUSICLOAD_MODE_PATTERNSET, nullptr, m_pTheme, pMood);
                    pSubObj->Dump(&Sink);
                }
            ...
            case MUSICLOAD_MODE_PATTERNSET: {
                ...
                if (pObj->GetValue("MainLayer", pSubObj)) {
                    ...
                    if (pSubObj->GetValue("Patterns", pPatternsSubObj)) {
                        CMusicLoadSinkTemp Sink(m_pMusicData, m_pScriptSystem, &pPatternsSubObj, MUSICLOAD_MODE_MOODLAYERS, nullptr, m_pTheme, m_pMood, pPatternSet, MUSICLAYER_MAIN);
                ...
                if (pObj->GetValue("RhythmicLayer", pSubObj)) {
                    ...
                    _SmartScriptObject pPatternsSubObj(m_pScriptSystem, true);
                    if (pSubObj->GetValue("Patterns", pPatternsSubObj)) {
                        CMusicLoadSinkTemp Sink(m_pMusicData, m_pScriptSystem, &pPatternsSubObj, MUSICLOAD_MODE_MOODLAYERS, nullptr, m_pTheme, m_pMood, pPatternSet,
                                                MUSICLAYER_RHYTHMIC);
                        pPatternsSubObj->Dump(&Sink);
                    }
                }
...
void CMusicManager::LoadFromLua(CBaseLibrary* pLibrary) {
    ...
    pScriptSystem->SetGlobalTonullptr("DynamicMusic");
...
void CMusicManager::ReleaseMusicData() {
    ...
    if (GetIEditor()->GetSystem()->GetIMusicSystem())
        GetIEditor()->GetSystem()->GetIMusicSystem()->SetData(nullptr, true); // Clear music system data.
