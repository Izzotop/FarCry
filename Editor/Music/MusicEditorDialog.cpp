...
//////////////////////////////////////////////////////////////////////////
CMusicEditorDialog::CMusicEditorDialog(CWnd* pParent) : CBaseLibraryDialog(IDD_DB_ENTITY, pParent) {
...
void CMusicEditorDialog::InitToolbar() {
    VERIFY(m_toolbar.CreateEx(this, TBSTYLE_FLAT | TBSTYLE_WRAPABLE, WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC));
    VERIFY(m_toolbar.LoadToolBar24(IDR_DB_MUSIC_BAR));

    // Resize the toolbar
    CRect rc;
    GetClientRect(rc);
    m_toolbar.SetWindowPos(nullptr, 0, 0, rc.right, 70, SWP_NOZORDER);
...
void CMusicEditorDialog::OnUpdateProperties(IVariable* var) {
    ...
    if (m_publicVarsItems != nullptr && m_publicVars != nullptr)
    {
        if (!bShaderChanged)
        {
...
void CMusicEditorDialog::OnNotifyTreeRClick(NMHDR* pNMHDR, LRESULT* pResult) {
    *pResult = 0;
    ...
    if ((hItem != nullptr) && (TVHT_ONITEM & uFlags)) {
...
void CMusicEditorDialog::OnPaste() {
    ...
    if (node->isTag("Pattern")) {
        if (pItemDesc && pItemDesc->m_pPatternSet)
            NewPattern(pItemDesc->m_pPatternSet, pItemDesc->layerType, nullptr, node);
...
void CMusicEditorDialog::PastePatternSet(SMusicMood* pMood, XmlNodeRef& nodePtrnSet) {
    ...
    if (nodeMainLayer) {
        for (int j = 0; j < nodeMainLayer->getChildCount(); j++) {
            NewPattern(pPatternSet, ELAYER_MAIN, nullptr, nodeMainLayer->getChild(j));
        }
    }
    if (nodeRhythmicLayer) {
        for (int j = 0; j < nodeRhythmicLayer->getChildCount(); j++) {
            NewPattern(pPatternSet, ELAYER_RHYTHMIC, nullptr, nodeRhythmicLayer->getChild(j));
        }
    }
    if (nodeIncidentalLayer) {
        for (int j = 0; j < nodeIncidentalLayer->getChildCount(); j++) {
            NewPattern(pPatternSet, ELAYER_INCIDENTAL, nullptr, nodeIncidentalLayer->getChild(j));
        }
    }
    CalcProbabilities(pPatternSet);
}
...
void CMusicEditorDialog::DeleteTheme(ItemDesc* pItemDesc) {
    ...
    delete pTheme;
    m_pMusicSystem->UpdateTheme(nullptr); // Get newer themes, data.
    SetModified();
}
