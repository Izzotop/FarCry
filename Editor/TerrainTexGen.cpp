...
// Sector flags.
enum { eSectorHeightmapValid = 0x01, eSectorLightmapValid = 0x02, eSectorLayersValid = 0x04 };
...
CTerrainTexGen::CTerrainTexGen(int resolution) {
    m_pLightingBits = nullptr;
    m_bLog = true;
    m_waterLayer = nullptr;
    m_vegetationMap = nullptr;
...
// written by M.M.
float CTerrainTexGen::GetSunAmount(const float* inpHeightmapData, int iniX, int iniY, float infInvHeightScale, const Vec3& vSunShadowVector, const float infShadowBlur) const {
...
// Also give out the results of the terrain lighting if pLightingBit is not nullptr.
////////////////////////////////////////////////////////////////////////
bool CTerrainTexGen::GenerateSectorTexture(CPoint sector, const CRect& rect, int flags, CImage& surfaceTexture) {
    ...
    if (bNoTexture) {
        ...
        //////////////////////////////////////////////////////////////////////////
        // Setup water layer.
        if (bShowWater && m_waterLayer == nullptr) {
            // Apply water level.
...
bool CTerrainTexGen::GenerateLightmap(CPoint sector, LightingSettings* pSettings, CImage& lightmap, int genFlags, CBitArray* pLightingBits) {
    ...
    // bit you are supposed to pass a nullptr pointer if your don't want it.
    ////////////////////////////////////////////////////////////////////////
    int i, j;
    float fDP3;
            ...
            for (i = rc.left; i < rc.right; i++) {
                ...
                if (bHaveSkyColor) {
                    if (pSettings->iHemiSamplQuality) {
                        ...
                        fSkyWeight = vNormal.z * 0.6f + 0.4f; // approximate the sky accissibility without hills, only based only on surface slope
...
void CTerrainTexGen::GenerateShadowmap(CPoint sector, CByteImage& shadowmap, float shadowAmmount, const Vec3& sunVector) {
    ...
    for (int j = 0; j < sectorTexSize; j++) {
        ...
        for (int i = 0; i < sectorTexSize; i++) {
            ...
            color = (shadowValue * ((uint)(255 - sectorImage2[pos]) + (255 - sectorImage2[pos + 3]) + (255 - sectorImage2[pos + sectorTexSize2 * 3]) +
...
bool CTerrainTexGen::RefreshAccessibility(const LightingSettings* inpLSettings, int genFlags) {
    ...
    if (bTerrainShadows)
        if (!m_SunAccessiblity.GetData() || m_iCachedSunBlurLevel != inpLSettings->iShadowBlur || m_vCachedSunDirection != inpLSettings->GetSunVector()) {
    ...
    if (m_resolution < w)
        w = m_resolution; // use minimum needed size (fast in preview))
    if (m_resolution < h)
        h = m_resolution; // use minimum needed size (fast in preview)
