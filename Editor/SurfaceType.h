...
public:
    ...
    CSurfaceType(const CSurfaceType& st) {
        *this = st;
    }
    ...
    void SetDetailTexture(const CString& tex) {
        m_detailTexture = tex;
    };
    const CString& GetDetailTexture() const {
        return m_detailTexture;
    }

    void SetBumpmap(const CString& tex) {
        m_bumpmap = tex;
    };
    const CString& GetBumpmap() const {
        return m_bumpmap;
    }

    void SetDetailTextureScale(const Vec3& scale) {
        m_detailScale[0] = scale.x;
        m_detailScale[1] = scale.y;
    }
    Vec3 GetDetailTextureScale() const {
        return Vec3(m_detailScale[0], m_detailScale[1], 0);
    }

    void SetMaterial(const CString& mtl);
    const CString& GetMaterial() const {
        return m_material;
    }

    void AddDetailObject(const CString& name) {
        m_detailObjects.push_back(name);
    };
    int GetDetailObjectCount() const {
        return m_detailObjects.size();
    };
    void RemoveDetailObject(int i) {
        m_detailObjects.erase(m_detailObjects.begin() + i);
    };
    const CString& GetDetailObject(int i) const {
        return m_detailObjects[i];
    };

    //! Set detail texture projection axis.
    void SetProjAxis(int axis) {
        m_projAxis = axis;
    }

    //! Get detail texture projection axis.
    int GetProjAxis() const {
        return m_projAxis;
    }
