...
bool CMailer::SendMessage(const CString& subject, const CString& messageBody, const std::vector<CString>& _recipients, const std::vector<CString>& _attachments, bool bShowDialog) {
    ...
    */
    ...
    // Create a message. Most members are set to nullptr or 0 (the user may set them)
    /*
    MapiMessage message =  {0,                                // reserved, must be 0
                                                (const char*)_subject,      // subject
                                                    (const char*)_messageBody,  // message body
                                                    nullptr,           // nullptr = interpersonal message
    ...
    // Next, the client calls the MAPISendMail function and stores the return status so it can detect whether the call succeeded. You should use a more sophisticated error
    // reporting mechanism than the C library function printf.
