...
CDrawWnd::CDrawWnd() {
    ...
    // Create the DC
    m_dcBrushes.CreateCompatibleDC(nullptr);
...
void CDrawWnd::OnPaint() {
    ...
    */
    ...
    dcHeightmap.Ellipse(ptTransfMarker.x - 3, ptTransfMarker.y - 3, ptTransfMarker.x + 3, ptTransfMarker.y + 3);
...
bool CDrawWnd::Create(CWnd* pwndParent) {
    ...
    return CreateEx(nullptr, AfxRegisterWndClass(CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW, AfxGetApp()->LoadCursor(IDC_HAND_INTERNAL), nullptr, nullptr), "DrawWindow", WS_CHILD | WS_VISIBLE,
                    rcDefault, pwndParent, nullptr);
}
...
void CDrawWnd::MakeAlpha(CBitmap& ioBM) {
    ...
    for (UINT i = 0; i < numPixels; ++i) {
        if (pixels[i].rgbatBlue == 255 && pixels[i].rgbatGreen == 0 && pixels[i].rgbatRed == 255) {
...
bool CDrawWnd::SetZoomFactor(float fZoomFactor) {
    ...
    SetScrollOffset((long)(GetScrollOffset().x * fEnlargementFactor), (long)(GetScrollOffset().y * fEnlargementFactor));
...
void CDrawWnd::OnMouseMove(UINT nFlags, CPoint point) {
    ...
    if (nFlags & MK_LBUTTON || nFlags & MK_MBUTTON) {
        ...
        if (nFlags & MK_CONTROL || nFlags & MK_MBUTTON) {
            ...
            // Draw a spot
            m_heightmap->DrawSpot(point.y, point.x, (uint8)(10.0f + m_iCurBrush * 25.0f), m_fOpacity, m_fSetToHeight, m_bUseNoiseBrush);
        }
        ...
        // Draw a spot
        m_heightmap->DrawSpot(point.y, point.x, (uint8)(10.0f + m_iCurBrush * 25.0f), -m_fOpacity, m_fSetToHeight);
...
void CDrawWnd::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    ALPHABLEND pfnAlphaBlend = nullptr;
    BOOL bSuccess;
    HINSTANCE hLib;
    RECT rcInvalid;
