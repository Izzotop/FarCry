...
public:
    bool operator()(const std::pair<IEntityRender*, CBrushObject*>& rpIEtyRnd1, const std::pair<IEntityRender*, CBrushObject*>& rpIEtyRnd2) {
        ...
        if ((DWORD_PTR)pIEtyRnd1->GetEntityVisArea() < (DWORD_PTR)pIEtyRnd2->GetEntityVisArea())
            return true;
        if ((DWORD_PTR)pIEtyRnd1->GetEntityVisArea() > (DWORD_PTR)pIEtyRnd2->GetEntityVisArea())
            return false;
...
const bool CLightmapGen::Generate(ISystem* pSystem, std::vector<std::pair<IEntityRender*, CBrushObject*>>& nodes, ICompilerProgress* pICompilerProgress, const ELMMode Mode,
    ...
    ctx.pScriptUpdateParams = nullptr;
    ...
    if (!cLightScene.CreateFromEntity(IndInterface, m_sParam, nodes, cLights, pICompilerProgress, Mode, m_pSharedData, vSelectionIndices, bErrorsOccured)) {
...
const bool CLightmapGen::GenerateSelected(IEditor* pIEditor, ICompilerProgress* pICompilerProgress) {
    ...
    for (iCurObj = 0; iCurObj < iNumObjects; iCurObj++) {
        ...
        if (pIEtyRend != nullptr && IsLM_GLM(pIEtyRend)) {
