    ...
    for (int y = sy1; y <= sy2; y++) {
        for (int x = sx1; x <= sx2; x++) {
            ...
            if (si->first) {
                for (CVegetationInstance* obj = si->first; obj; obj = next) {
                    next = obj->next;
                    if (obj->object != object && object != nullptr)
                        continue;
            ...
            // Scale sector image down and store into the shadow map.
            {
                ...
                for (int j = 0; j < sectorSize; j++) {
                    ...
                    for (int i = 0; i < sectorSize; i++) {
                        ...
                        color = (shadowValue * ((uint)(255 - sectorImage2[pos]) + (255 - sectorImage2[pos + 3]) + (255 - sectorImage2[pos + sectorSize2 * 3]) +
...
void CVegetationMap::ImportBlock(CXmlArchive& ar, CPoint placeOffset) {
    ...
    ClearBrush(subRc, false, nullptr);
