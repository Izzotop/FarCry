...
public:
    ...
    explicit CCryFileDialog(BOOL bOpenFileDialog, // TRUE for FileOpen, FALSE for FileSaveAs
                            LPCTSTR lpszDefExt = nullptr, LPCTSTR lpszFileName = nullptr, DWORD dwFlags = OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, LPCTSTR lpszFilter = nullptr,
                            CWnd* pParentWnd = nullptr, DWORD dwSize = sizeof(OPENFILENAME)) {
        memset(&m_ofn, 0, sizeof(m_ofn)); // initialize structure to 0/nullptr
        m_szFileName[0] = '\0';
        m_szFileTitle[0] = '\0';
    ...
    CString GetNextPathName(POSITION& pos) const {
        ...
        if (!strPath.IsEmpty()) {
            ...
            if (lpsz != nullptr && (lpsz - (LPCTSTR)strPath) == strPath.GetLength() - 1) {
                ASSERT(*lpsz == '\\' || *lpsz == '/');
