...
public:
    ...
    bool GenerateSurface(DWORD* pSurface, UINT iWidth, UINT iHeight, int flags, CBitArray* pLightingBits = nullptr, float** ppHeightmapData = nullptr);
    void ReloadLayerList();
    void ExportLargePreview();

    CTerrainTexture(CWnd* pParent = nullptr); // standard constructor
    ~CTerrainTexture();
