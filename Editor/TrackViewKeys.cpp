...
enum ETVMouseMode { MOUSE_MODE_NONE = 0, MOUSE_MODE_SELECT = 1, MOUSE_MODE_MOVE, MOUSE_MODE_CLONE, MOUSE_MODE_DRAGTIME, MOUSE_MODE_DRAGSTARTMARKER, MOUSE_MODE_DRAGENDMARKER };
...
CTrackViewKeys::CTrackViewKeys() {
    ...
    m_descriptionFont->CreateFont(8, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH,
                                  "MS Sans Serif");
}
...
void CTrackViewKeys::DrawTimeline(CDC* dc, const CRect& rcUpdate) {
    ...
    */
    ...
    dc->SelectObject(GetStockObject(nullptr_BRUSH));
...
void CTrackViewKeys::RedrawItem(int item) {
    ...
    if (GetItemRect(item, rc) != LB_ERR) {
        RedrawWindow(rc, nullptr, RDW_INVALIDATE | RDW_ERASE);
    }
}
...
HBRUSH CTrackViewKeys::CtlColor(CDC* pDC, UINT nCtlColor) {
    ...
    // TODO:  Return a non-nullptr brush if the parent's handler should not be called
    return nullptr;
}
...
void CTrackViewKeys::OnSize(UINT nType, int cx, int cy) {
    ...
    if (m_offscreenBitmap.GetSafeHandle() != nullptr)
...
void CTrackViewKeys::SetCurrTime(float time) {
    ...
    if (bChange) {
        ...
        RedrawWindow(rc, nullptr, RDW_INVALIDATE);
        CRect rc1(x2 - 3, m_rcClient.top, x2 + 4, m_rcClient.bottom);
        RedrawWindow(rc1, nullptr, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE);
...
bool CTrackViewKeys::CanCopyPasteKeys() {
    IAnimTrack* pCopyFromTrack = nullptr;
