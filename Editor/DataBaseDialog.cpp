...
CDataBaseDialog::CDataBaseDialog(CWnd* pParent) : CToolbarDialog(IDD, pParent) {
    m_selectedCtrl = -1;
}
...
BOOL CDataBaseDialog::OnInitDialog() {
    ...
    m_menubar.CreateEx(this, TBSTYLE_FLAT,
                       WS_CHILD | WS_VISIBLE | CBRS_ALIGN_TOP | CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC | CBRS_BORDER_BOTTOM | CBRS_BORDER_LEFT | CBRS_BORDER_RIGHT);
...
void CDataBaseDialog::OnSize(UINT nType, int cx, int cy) {
    ...
    m_tabCtrl.MoveWindow(rcRollUp.left, rcRollUp.top, rcRollUp.right, rcRollUp.bottom);
    ...
    m_pwndRollUpCtrl->SetWindowPos(nullptr, rcRollUp.left + 3, rcRollUp.top + 3 + h, rcRollUp.right - 6,
