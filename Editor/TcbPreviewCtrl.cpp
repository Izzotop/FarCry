...
void CTcbPreviewCtrl::OnPaint() {
    ...
    dc.FillSolidRect(dc.m_ps.rcPaint.left, dc.m_ps.rcPaint.top, dc.m_ps.rcPaint.right, dc.m_ps.rcPaint.bottom, RGB(255, 255, 255));
...
void CTcbPreviewCtrl::DrawSpline(CDC& dc) {
    ...
    for (time = 0; time < 1.0f; time += dt) {
        ...
        int px = vec.x;
        int py = vec.y;
        if (px > m_rcClient.left && px < m_rcClient.right && py > m_rcClient.top && py < m_rcClient.bottom) {
            dc.LineTo(px, py);
        }
    }
