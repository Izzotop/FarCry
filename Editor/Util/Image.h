...
public:
    ...
    bool Uncompress(const CMemoryBlock& mem) {
        ...
        mem.Uncompress(*temp);
        bool bValid = (GetSize() == m_memory->GetSize()) || ((GetSize() + m_width * sizeof(T)) == m_memory->GetSize());
        if (bValid) {
            m_memory = temp;
            m_data = (T*)m_memory->GetBuffer();
        }
...
template <class T> void TImage<T>::Serialize(CXmlArchive& ar) {
    if (ar.bLoading) {
        // Loading
        ...
        void* pData = 0;
        int nDataSize = 0 bool bHaveBlock = ar.pNamedData->GetDataBlock(ar.root->getTag(), pData, nDataSize);
