...
bool CImageUtil::SaveBitmap(const CString& szFileName, CImage& inImage, bool inverseY) {
    ...
    DWORD dwBytes;
    unsigned int i;
    DWORD* pLine1 = nullptr;
    DWORD* pLine2 = nullptr;
    DWORD* pTemp = nullptr;
    ...
    WriteFile(hfile, &bitmapfileheader, sizeof(BITMAPFILEHEADER), &dwBytes, nullptr);
    WriteFile(hfile, &bitmapinfoheader, sizeof(BITMAPINFOHEADER), &dwBytes, nullptr);

    // Write the data
    DWORD written;
    WriteFile(hfile, pImage, (dwWidth * dwHeight * 3), &written, nullptr);
...
bool CImageUtil::SaveJPEG(const CString& strFileName, CImage& inImage) {
    ...
    unsigned char* pImageDataStart = nullptr;
...
bool CImageUtil::LoadJPEG(const CString& strFileName, CImage& outImage) {
    ...
    if (!trg) {
        Warning("CImageUtil::LoadJPEG Memory allocation faild\r\nFailed to allocate %dMb of RAM for Jpg %s", memSize / (1024 * 1024), (const char*)strFileName);
        return false;
    }
...
void CImageUtil::SmoothImage(CByteImage& image, int numSteps) {
    ...
    for (int steps = 0; steps < numSteps; steps++) {
        ...
        for (int y = 1; y < h - 1; y++) {
            ...
            for (int x = 1; x < w - 1; x++) {
                // Smooth it out
                *ptr = ((uint)ptr[1] + ptr[w] + ptr[-1] + ptr[-w] + ptr[w + 1] + ptr[w - 1] + ptr[-w + 1] + ptr[-w - 1]) >> 3;
