...
bool CFileUtil::CompileLuaFile(const char* luaFilename) {
    ...
    if (strlen(CompilerOutput) > 0) {
        ...
        if (MessageBox(nullptr, (CString("Error output from Lua compiler:\r\n") + CompilerOutput + CString("\r\nDo you want to edit the file ?")), "Lua Compiler",
...
void CFileUtil::EditTextFile(const char* txtFile, int line, ETextFileType fileType) {
    ...
    HINSTANCE hInst = ShellExecute(nullptr, "open", TextEditor, cmd, nullptr, SW_SHOWNORMAL);
...
bool CFileUtil::SelectFile(const CString& fileSpec, const CString& searchFolder, CString& fullFileName) {
    ...
    CFileDialog dlg(TRUE, nullptr, nullptr, OFN_ENABLESIZING | OFN_EXPLORER | OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST | OFN_NOCHANGEDIR, filter);
...
bool CFileUtil::SelectFiles(const CString& fileSpec, const CString& searchFolder, std::vector<CString>& files) {
    ...
    CFileDialog dlg(TRUE, nullptr, nullptr, OFN_ALLOWMULTISELECT | OFN_ENABLESIZING | OFN_EXPLORER | OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST | OFN_NOCHANGEDIR, filter);
    dlg.m_ofn.lpstrInitialDir = searchFolder;
...
bool CFileUtil::SelectSaveFile(const CString& fileFilter, const CString& defaulExtension, const CString& startFolder, CString& fileName) {
    CFileDialog dlg(FALSE, defaulExtension, nullptr, OFN_ENABLESIZING | OFN_EXPLORER | OFN_PATHMUSTEXIST | OFN_NOCHANGEDIR | OFN_OVERWRITEPROMPT, fileFilter);
...
static bool CheckOutFile( const char *filename )
{
    ...
    if (CreateProcess( nullptr,cmdLine,nullptr,nullptr,FALSE,CREATE_NEW_CONSOLE,nullptr,currDirectory,&si,&pi ))
    {
...
static bool CheckAndCreateDirectory(const char* path) {
    ...
    if (hFind == INVALID_HANDLE_VALUE) {
        return ::CreateDirectory(path, nullptr) == TRUE;
    } else {
