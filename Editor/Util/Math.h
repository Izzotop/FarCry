...
inline bool IsMatrixEqual(const Matrix44& tm1, const Matrix44& tm2) {
    for (int i = 0; i < 4; i++) {
        if (fabs(tm1[i][0] - tm2[i][0]) > FLOAT_EPSILON || fabs(tm1[i][1] - tm2[i][1]) > FLOAT_EPSILON || fabs(tm1[i][2] - tm2[i][2]) > FLOAT_EPSILON ||
...
*/
inline bool LineLineIntersect(const Vec3& p1, const Vec3& p2, const Vec3& p3, const Vec3& p4, Vec3& pa, Vec3& pb, float& mua, float& mub) {
    Vec3 p13, p43, p21;
