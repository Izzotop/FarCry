...
ThreadHandle createMutex(bool own) {
    return (ThreadHandle)CreateMutex(nullptr, own, nullptr);
}
...
ThreadHandle createSemaphore(uint initCount, uint maxCount) {
    return (ThreadHandle)CreateSemaphore(nullptr, initCount, maxCount, nullptr);
}

bool releaseSemaphore(ThreadHandle handle, int releaseCount) {
    if (ReleaseSemaphore((HANDLE)handle, releaseCount, nullptr) != 0)
        return true;
    return false;
}
...
ThreadHandle createEvent(bool manualReset, bool initialState) {
    return (ThreadHandle)CreateEvent(nullptr, manualReset, initialState, nullptr);
}
