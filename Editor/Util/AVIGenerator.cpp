...
CAVIGenerator::CAVIGenerator() : m_sFile(_T("Untitled.avi")), m_dwRate(30), m_pAVIFile(nullptr), m_pStream(nullptr), m_pStreamCompressed(nullptr) {
    memset(&m_bih, 0, sizeof(BITMAPINFOHEADER));
}

#ifdef _AVIGENERATOR_USE_MFC
CAVIGenerator::CAVIGenerator(LPCTSTR sFileName, CView* pView, DWORD dwRate) : m_sFile(sFileName), m_dwRate(dwRate), m_pAVIFile(nullptr), m_pStream(nullptr), m_pStreamCompressed(nullptr) {
    MakeExtAvi();
...
CAVIGenerator::CAVIGenerator(LPCTSTR sFileName, LPBITMAPINFOHEADER lpbih, DWORD dwRate)
    : m_sFile(sFileName), m_dwRate(dwRate), m_pAVIFile(nullptr), m_pStream(nullptr), m_pStreamCompressed(nullptr) {
    MakeExtAvi();
...
HRESULT CAVIGenerator::InitEngine(const char* codec) {
    ...
    hr = AVIFileOpen(&m_pAVIFile,          // Address to contain the new file interface pointer
                     (LPCSTR)m_sFile,      // nullptr-terminated string containing the name of the file to open
...
void CAVIGenerator::MakeExtAvi() {
    ...
    // finding avi
    if (_tcsstr((const char*)m_sFile, _T("avi")) == nullptr) {
        m_sFile += _T(".avi");
    }
}
