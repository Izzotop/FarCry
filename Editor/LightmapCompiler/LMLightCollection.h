...
struct LMCompLight {
    LMCompLight() : m_CompLightID(std::pair<EntityId, EntityId>(0, 0)) {
        ...
        // m_pPointLtRC = nullptr;

        m_pLastIntersectionRasterCube = nullptr;
...
public:
    ...
    std::vector<LMCompLight>& GetLights() {
        return m_vLights;
    }
    std::vector<CDLight*>& GetSrcLights() {
        return m_vSrcLights;
    };
    const unsigned int OcclLightSize() {
        return m_uiOcclLightSize;
    }
    //!
    void Release() {
        delete this;
