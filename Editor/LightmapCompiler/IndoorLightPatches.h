...
typedef struct SComplexOSDot3Texel {
    ...
    SComplexOSDot3Texel() : vDot3Light(), pSourcePatch(nullptr), fAlpha(0.f), fBeta(0.f), uiLightMask(0), bNotHit(false) {}
...
__inline int CalcPlaneType2(const Vec3d& n) {
    if (((1.0f - n.x) < 0.001f) && ((1.0f - n.x) >= 0))
        return PLANE_X;
    else if (((1.0f - n.y) < 0.001f) && ((1.0f - n.y) >= 0))
        return PLANE_Y;
    else if (((1.0f - n.z) < 0.001f) && ((1.0f - n.x) >= 0))
        return PLANE_Z;
    else {
        float ax = fabs(n[0]);
        float ay = fabs(n[1]);
...
public:
    ...
    static void ApplyBaryCoordsToVertex(CRadVertex& rDest, const CRadVertex& rSource0, const CRadVertex& rSource1, const CRadVertex& rSource2, float cfAlpha, float cfBeta,
                                        const bool cbCOmputeTS = true);
    ...
    void GatherSubSamples(const unsigned int cuiX, const unsigned int cuiY, const unsigned int cuiSubSamples, const unsigned int* cpaIndicator, const float* cpfColours,
                          const SComplexOSDot3Texel* pDot3, unsigned int& ruiMaxComponent, const SOcclusionMapTexel* cpfOccl, const GLMOcclLightInfo& rOcclInfo);
#else
    void GatherSubSamples(const unsigned int cuiX, const unsigned int cuiY, const unsigned int cuiSubSamples, const unsigned int* cpaIndicator, const float* cpfColours,
                          const SComplexOSDot3Texel* pDot3, const SOcclusionMapTexel* cpfOccl, const GLMOcclLightInfo& rOcclInfo);
#endif
    //!
    ...
    // lighpatch
    void AddWarningBorders();

#ifndef DISPLAY_MORE_INFO
    const unsigned int CalcExtent(CLightScene* pScene, const CString& rGLMName, const CString& rCGFName, bool bOriginal, const float fGridSize, const UINT iMinBlockSize,
                                  const UINT iMaxBlockSize, unsigned int& rHugePatchFoundNumber);
#else
    const unsigned int CalcExtent(CLightScene* pScene, const CString& rGLMName, const CString& rCGFName, bool bOriginal, const float fGridSize, const UINT iMinBlockSize,
                                  const UINT iMaxBlockSize);
#endif
...
#ifdef APPLY_COLOUR_FIX
    const unsigned int SetDot3LightmapTexel(const CRadVertex& rVertex, const float fColorRLamb, const float fColorGLamb, const float fColorBLamb, Vec3d& inLightDir,
                                            const float cfLM_DP3LerpFactor, SComplexOSDot3Texel& rDot3Texel, const SOcclusionMapTexel& rTexel, bool bHDR);
#else
    void SetDot3LightmapTexel(const CRadVertex& rVertex, const float fColorRLamb, const float fColorGLamb, const float fColorBLamb, Vec3d& inLightDir,
                              const float cfLM_DP3LerpFactor, SComplexOSDot3Texel& rDot3Texel, const SOcclusionMapTexel& rTexel, bool bHDR);
#endif
...
#endif
    // unsigned int consists of 4 chars as follows:  [0]=1st shared vertex of this [1]=2nd shared vertex of this [2]=1st shared vertex of shared triangle  [3]=2nd shared vertex of
    // shared triangle
...
public:
    ...
    void UpdateHash(const DWORD newHash) {
        CalcNCombineHash(newHash, m_dwHashValue);
    }
    ...
    CRadMesh()
        : m_uiCoarseTexelCount(0), pLightmapQualityVar(nullptr), m_fMaxVariance(0.f), m_fLMAccuracy(1.f), m_pESource(nullptr), m_dwHashValue(0), m_pClusterRC(nullptr),
          m_bReceiveLightmap(true), m_uiFlags(0), m_uiTexCoordsRequired(0) {}
...
private:
    ...
    DWORD CalcLocalLightHashValue() const;
};

/////////////////////////////////////////////////////////////////////
class CLightPatch {
public:
    CLightPatch(UINT iPatchResolution) : m_pLightmapImage(0), m_pHDRLightmapImage(0), m_pDominantDirImage(0), m_pOcclMapImage(nullptr), m_nTextureNum(0) {
        m_nPatchSpace.resize(iPatchResolution);
    }

    ~CLightPatch() {
        if (m_pLightmapImage) {
            delete[] m_pLightmapImage;
            m_pLightmapImage = nullptr;
        }
        if (m_pHDRLightmapImage) {
            delete[] m_pHDRLightmapImage;
            m_pHDRLightmapImage = nullptr;
        }
        if (m_pDominantDirImage) {
            delete[] m_pDominantDirImage;
            m_pDominantDirImage = nullptr;
        }
        if (m_pOcclMapImage) {
            delete[] m_pOcclMapImage;
            m_pOcclMapImage = nullptr;
        }
    }
...
public:
    ...
    bool CreateFromEntity(const IndoorBaseInterface& pInterface, LMGenParam sParam, std::vector<std::pair<IEntityRender*, CBrushObject*>>& vNodes, CLMLightCollection& cLights,
                          struct ICompilerProgress* pIProgress, const ELMMode Mode, volatile SSharedLMEditorData* pSharedData,
...
protected:
    void DoLightCalculation(unsigned int& ruiMaxColourComp, const std::vector<LMCompLight*>& vLights, const std::vector<LMCompLight*>& vOcclLights, SComplexOSDot3Texel& dot3Texel,
                            CRadMesh* pMesh, CRadPoly* pSource, const CRadVertex& Vert, const bool cbFirstPass, const unsigned int cuiSubSampleIndex = 0);
    ...
    void SelectObjectLMReceiverAndShadowCasterForChanges(std::vector<std::pair<IEntityRender*, CBrushObject*>>& vNodes, const std::vector<AABB>& vAABBs,
                                                         const Matrix33& rMatSunBasis, const std::vector<unsigned int>& rvNodeRadMeshIndices, const ELMMode Mode);
    void SelectObjectsFromChangedLMShadowCaster(std::vector<std::pair<IEntityRender*, CBrushObject*>>& vNodes, const std::vector<AABB>& vAABBs, const Matrix33& rMatSunBasis,
                                                const std::vector<unsigned int>& rvNodeRadMeshIndices);
    void ComputeSmoothingInformation(const unsigned int uiSmoothAngle = 45,
                                     const bool cbTSGeneratedByLM = false); // computes the sharing information and stores it into the respective per polygon vectors
    void MergeTangentSpace(CRadVertex& rVertex1, CRadVertex& rVertex2);     // merges two tangent spaces according to normal
    const unsigned int CheckNormals(float& rfMaxVariance,
                                    const CRadMesh* const pMesh); // returns true if some normals are not normalized, rfMaxVariance is the max variance encountered
    bool Create(const IndoorBaseInterface& pInterface, const Vec3d& vMinBB, const Vec3d& vMaxBB, volatile SSharedLMEditorData* pSharedData, const ELMMode Mode,
                const unsigned int cuiMeshesToProcessCount);
    void FlushAssignQueue();
#ifdef APPLY_COLOUR_FIX
    void PerformAdaptiveSubSampling(CRadMesh* pMesh, CRadPoly* pSource, const std::vector<LMCompLight*>& vLights, const std::vector<LMCompLight*>& vOcclLights,
                                    unsigned int uiMaxComponent);
#else
    void PerformAdaptiveSubSampling(CRadMesh* pMesh, CRadPoly* pSource, const std::vector<LMCompLight*>& vLights, const std::vector<LMCompLight*>& vOcclLights);
#endif
    void SetSubSampleDot3LightmapTexel(const unsigned int cuiSubSampleIndex, const float fColorRLamb, const float fColorGLamb, const float fColorBLamb, Vec3d& inLightDir,
    ...
    void InitSubSampling(const unsigned int cuiSampleCount) {
        m_puiIndicator = new unsigned int[cuiSampleCount];
        assert(m_puiIndicator);
        m_pSubSamples = new SComplexOSDot3Texel[cuiSampleCount];
        assert(m_pSubSamples);
        m_pfColours = new float[cuiSampleCount * 4 /*4 colour components*/];
        assert(m_pfColours);
        if (m_sParam.m_bGenOcclMaps) {
            m_pOcclSamples = new SOcclusionMapTexel[cuiSampleCount];
            assert(m_pOcclSamples);
        }
...
private:
    ...
    bool ComputeRasterCube(CRasterCubeImpl* pRasterCube, const std::set<IEntityRender*>& vGeom, const Matrix33* pDirLightTransf = nullptr);
...
#define ALREADY_TESTED_ARRARY_SIZE 1024
    ...
    protected:
        ...
        bool InsertAlreadyTested(RasterCubeUserT* inObject) {
            ...
            if (m_dwAlreadyTested < ALREADY_TESTED_ARRARY_SIZE - 1)
                ...
                assert(0); // ALREADY_TESTED_ARRARY_SIZE is not big enougth
    ...
    public:
        void SetupRay(const Vec3d& vRayDir, const Vec3d& vRayOrigin, float fRayLen, const float infGridSize, CRadPoly* pRayDest = nullptr) {
