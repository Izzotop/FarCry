...
template <class T> class CRasterTable {
public:
    CRasterTable();
...
private:
    ...
    //! callback function
    template <class T> class CPixelAddArrayElement : public CSimpleTriangleRasterizer::IRasterizeSink {
    public:
...
// constructor
template <class T> CRasterTable<T>::CRasterTable() {
...
// free data
template <class T> void CRasterTable<T>::DeInit() {
    delete[] m_pDataCounter;
    m_pDataCounter = 0;
...
// alloc raster, after that Phase 1 has started
template <class T> bool CRasterTable<T>::Init(const DWORD indwXSize, const DWORD indwYSize) {
...
template <class T> bool CRasterTable<T>::PreProcess() {
    assert(m_pDataCounter);
    assert(!m_pExtraMemoryPool);
...

    {
        ...
        for (DWORD i = 0; i < m_dwXSize * m_dwYSize; i++) {
            ...
            if (dwNumEl != 0)
                dwSum += dwNumEl + 1; // Elements + nullptrTermination
        }
    }
    ...
    // after filing in the triangles once again the pointer is on the first element
    {
        ...
        for (DWORD i = 0; i < m_dwXSize * m_dwYSize; i++) {
            ...
            if (dwNumEl != 0) {
                dest += dwNumEl;
                *ptr2++ = dest;
                dest++; // Elements + nullptrTermination
            } else
                *ptr2++ = 0;
        }
    }
...
// call this per triangle after Init and before PreProcess
// after PreProcess call it again for every triangle
template <class T> void CRasterTable<T>::PutInTriangle(float infX[3], float infY[3], T& inElement) {
    float fU[3], fV[3];
...
// returns pointer to zero terminated array of pointers
template <class T> T* CRasterTable<T>::GetElementsAt(int iniX, int iniY) {
    assert(iniX >= 0);
...
// returns pointer to zero terminated array of pointers
// IF YOU WANNA USE THIS FUNCTION, CLEAR IT FIRST BECAUSE IT SIMPLY DOES NOT WORK
template <class T> void CRasterTable<T>::Compress() {
    if (!m_pExtraMemoryPool)
        return;
    ...
    if (!newMem)
        return; // no memory - no compression
...
template <class T> DWORD CRasterTable<T>::CalcMemoryConsumption() {
    return (m_dwXSize * m_dwYSize * sizeof(m_pDataCounter) + m_ExtraMemorySize * sizeof(T));
}

template <class T> void CRasterTable<T>::Debug(const char* inPathFileName) const {
...
template <class T> DWORD* CRasterTable<T>::GetDebugData(DWORD& outPitchInBytes) const {
    outPitchInBytes = m_dwXSize * sizeof(DWORD);
    return (m_pDataCounter);
}

template <class T> DWORD CRasterTable<T>::GetWidth() {
    return (m_dwXSize);
}

template <class T> DWORD CRasterTable<T>::GetHeight() {
    return (m_dwYSize);
}
