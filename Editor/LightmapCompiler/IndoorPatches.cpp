...
const bool CLightScene::CAnyHit::ReturnElement(RasterCubeUserT& inObject, float& inoutfRayMaxDist) {
    ...
    if (!InsertAlreadyTested(&inObject)) {
        if (intersect_triangle((float*)m_vRayOrigin, (float*)m_vRayDir, inObject.fVertices[0], inObject.fVertices[1], inObject.fVertices[2], &fT, &fU, &fV)) {
            if (fT < m_fClosest) {
...
void CLightPatch::CreateDot3Lightmap(int nSizeX, int nSizeY, const bool cbGenerateHDRMaps, const bool cbGenerateOcclMaps) {
    m_nWidth = nSizeX;
    m_nHeight = nSizeY;
    m_pLightmapImage = new unsigned char[nSizeX * nSizeY * 4];
    assert(m_pLightmapImage);
#ifdef USE_DOT3_ALPHA
    ...
    if (cbGenerateOcclMaps) {
        m_pOcclMapImage = new SOcclusionMapTexel[nSizeX * nSizeY];
        assert(m_pOcclMapImage);
    }
    if (cbGenerateHDRMaps) {
        m_pHDRLightmapImage = new unsigned char[nSizeX * nSizeY * 4];
        assert(m_pHDRLightmapImage);
    }
    Reset();
}
...
void CRadMesh::PrepareLightmaps(CLightScene* pScene, bool bMergePolys, const float fGridSize, const Vec3d& vMinBB, const Vec3d& vMaxBB) {
    ...
    for (radpolyit i1 = m_lstOldPolys.begin(); i1 != m_lstOldPolys.end(); i1++) {
        ...
        IStatObj* pIGeom = m_pESource->GetEntityStatObj(0, nullptr);
    ...
    } else if (uiHugePatchFoundNumber > 1) {
...
bool CRadMesh::SaveLightmaps(CLightScene* pScene, bool bDebug) {
    ...
    for (radpolyit i1 = pScene->m_lstScenePolys.begin(); i1 != pScene->m_lstScenePolys.end(); i1++) {
        ...
        if (pPoly->m_dwFlags & NOLIGHTMAP_FLAG) {
            delete pPoly;
            *i1 = pPoly = nullptr; // does only apply to patches
            continue;
        }
...
///////////////////////////////////////////////////
void CRadPoly::FreeLightmaps() {
    if (m_pLightmapData) {
        delete[] m_pLightmapData;
        m_pLightmapData = nullptr;
    }
    if (m_pHDRLightmapData) {
        delete[] m_pHDRLightmapData;
        m_pHDRLightmapData = nullptr;
    }

    if (m_pDominantDirData) {
        delete[] m_pDominantDirData;
        m_pDominantDirData = nullptr;
    }
    if (m_pWSDominantDirData) {
        delete[] m_pWSDominantDirData;
        m_pWSDominantDirData = nullptr;
    }
    if (m_pOcclMapData) {
        delete[] m_pOcclMapData;
        m_pOcclMapData = nullptr;
    }
...
#ifdef APPLY_COLOUR_FIX
void CRadPoly::GatherSubSamples(const unsigned int cuiX, const unsigned int cuiY, const unsigned int cuiSubSamples, const unsigned int* cpaIndicator, const float* cpfColours,
...
#endif
{
    ...
    real r = (((real)(*pCenterColour)) * cfInvScale / 255.0f);
    real g = (((real)(*(pCenterColour + 1))) * cfInvScale / 255.0f);
    real b = (((real)(*(pCenterColour + 2))) * cfInvScale / 255.0f);
    real a = (((real)(*(pCenterColour + 3))) * cfInvScale);
...
#ifndef DISPLAY_MORE_INFO
const unsigned int CRadPoly::CalcExtent(CLightScene* pScene, const CString& rGLMName, const CString& rCGFName, bool bOriginal, const float fGridSize, const UINT iMinBlockSize,
...
#endif
{
    ...
    if (type == PLANE_X || type == PLANE_MX)
        m_nAxis = PLANE_X;
    else if (type == PLANE_Y || type == PLANE_MY)
        m_nAxis = PLANE_Y;
    else
        m_nAxis = PLANE_Z;
...
void CRadPoly::AllocateDot3Lightmap(const bool cbGenerateHDRMaps, const bool cbGenerateOcclMaps) {
    ...
    assert(m_pWSDominantDirData);
    if (cbGenerateHDRMaps) {
        m_pHDRLightmapData = new Vec3[cuiLMSize];
        assert(m_pHDRLightmapData);
        memset(m_pHDRLightmapData, 0, cuiLMSize * sizeof(Vec3)); // clear the image
    } else
        m_pHDRLightmapData = nullptr;
    if (cbGenerateOcclMaps) {
        m_pOcclMapData = new SOcclusionMapTexel[cuiLMSize];
        assert(m_pOcclMapData);
    } else
        m_pOcclMapData = nullptr; // just to make sure
...
void CRadPoly::GenerateImage() {
    ...
    for (radpolyit i3 = m_lstMerged.begin(); i3 != m_lstMerged.end(); i3++) {
        ...
        for (int v = 0; v < nNumOriginals; v++) {
            ...
            float w = (float)(m_nW - 1);
            if (w < 0)
                w = 0;
            float h = (float)(m_nH - 1);
            if (h < 0)
                h = 0;

            float divX = 0.0f;
            if (m_fX2 - m_fX1 > 0.001)
                divX = 1.0f / (m_fX2 - m_fX1);
            float divY = 0.0f;
            if (m_fY2 - m_fY1 > 0.001)
                divY = 1.0f / (m_fY2 - m_fY1);
            ...
            if (rv->m_fpX < 0.5f)
                rv->m_fpX = 0.5f;
            else if (rv->m_fpX > (m_nW - 0.5f))
                rv->m_fpX = (float)(m_nW - 0.50001f);
            if (rv->m_fpY < 0.5f)
                rv->m_fpY = 0.5f;
            else if (rv->m_fpY > (m_nH - 0.5f))
                rv->m_fpY = (float)(m_nH - 0.50001f);
        }
    } // i3
}

void CRadPoly::SetSimpleLightmapTexel(const float infX, const float infY, const int r, const int g, const int b, unsigned char iDP3Fac) {
    assert(m_pLightmapData);
...
void CRadPoly::SetHDRLightmapTexel(const float infX, const float infY, const float r, const float g, const float b) {
    assert(m_pHDRLightmapData);
    const unsigned int cuiIndex = RoundFromFloat(infY) * m_nW + RoundFromFloat(infX);
    assert(cuiIndex < m_nW * m_nH);
...
#ifdef APPLY_COLOUR_FIX
const unsigned int CRadPoly::SetDot3LightmapTexel(const CRadVertex& rVertex, const float fColorRLamb, const float fColorGLamb, const float fColorBLamb, Vec3d& inLightDir,
                                                  const float cfLM_DP3LerpFactor, SComplexOSDot3Texel& rDot3Texel, const SOcclusionMapTexel& rTexel, bool bHDR)
#else
void CRadPoly::SetDot3LightmapTexel(const CRadVertex& rVertex, const float fColorRLamb, const float fColorGLamb, const float fColorBLamb, Vec3d& inLightDir,
                                    const float cfLM_DP3LerpFactor, SComplexOSDot3Texel& rDot3Texel, const SOcclusionMapTexel& rTexel, bool bHDR)
#endif
{
    ...
    assert((int)fInfX >= 0);
    assert((int)fInfY >= 0);
    assert((int)fInfX < m_nW);
    assert((int)fInfY < m_nH);
    ...
    if (rDot3Texel.pSourcePatch == nullptr) // not set, no light hit pixel
#ifdef APPLY_COLOUR_FIX
        return cuiMax;
#else
            return;
...
#endif
#endif
    if (rDot3Texel.pSourcePatch == nullptr) // no light has hit texel
    {
    ...
    if (m_pOcclMapData) {
        ...
        for (int y = 0; y < m_nH; y++)
            for (int x = 1; x < m_nW; x++) {
                if (r != pRGBL[0] || g != pRGBL[1] || b != pRGBL[2] || l != pRGBL[3] || lx != pDir[0] || ly != pDir[1] || lz != pDir[2]
#ifdef USE_DOT3_ALPHA
                    || la != pDir[3]
#endif
                    || occl != *pOccl++)
