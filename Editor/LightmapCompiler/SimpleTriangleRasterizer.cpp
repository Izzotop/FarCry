    ...
    //    for(yy=m_iMinY; yy<=m_iMaxY; yy++)        // juhu
    {
        ...
        if (bFirstLine) {
            ...
            if (yy <= (int)floorf(y[1])) {
                start = min(fX1toX2, fX1toX3);
                end = max(fX1toX2, fX1toX3);
            } else {
                start = min(fX2toX3, fX1toX3);
                end = max(fX2toX3, fX1toX3);
            }
        }
