...
const Vec3d SComplexOSDot3Texel::TransformIntoTS(Vec3d& rSource) const {
    ...
    if (fabs(cfSqLength - 1.f) > 0.01f && cfSqLength > 0.01f) // leave nullptr vector as it is
        rSource.Normalize();
...
void CRadPoly::ApplyTangentSpaceToVertex(CRadVertex& outVertex, float cfAlpha, float cfBeta) {
    ...
    outVertex.m_vTNormal = m_lstOriginals[0].m_vTNormal * cfAlpha + m_lstOriginals[1].m_vTNormal * cfBeta + m_lstOriginals[2].m_vTNormal * cfGamma;
    outVertex.m_vTNormal.Normalize();
    outVertex.m_vBinormal = m_lstOriginals[0].m_vBinormal * cfAlpha + m_lstOriginals[1].m_vBinormal * cfBeta + m_lstOriginals[2].m_vBinormal * cfGamma;
    outVertex.m_vBinormal.Normalize();
    outVertex.m_vTangent = m_lstOriginals[0].m_vTangent * cfAlpha + m_lstOriginals[1].m_vTangent * cfBeta + m_lstOriginals[2].m_vTangent * cfGamma;
    outVertex.m_vTangent.Normalize();
}

void CRadPoly::ApplyBaryCoordsToVertex(CRadVertex& rDest, const CRadVertex& rSource0, const CRadVertex& rSource1, const CRadVertex& rSource2, float cfAlpha, float cfBeta,
                                       const bool cbCOmputeTS) {
    // correct the values to not let the sum go anywhere but 1.0f
    cfAlpha = (cfAlpha > 1.0f) ? 1.0f : cfAlpha;
    cfBeta = (cfBeta > 1.0f) ? 1.0f : cfBeta;
    float cfGamma = (1.0f - cfAlpha - cfBeta);
    ...
    rDest.m_vPos = rSource0.m_vPos * cfAlpha + rSource1.m_vPos * cfBeta + rSource2.m_vPos * cfGamma;
    rDest.m_vNormal = rSource0.m_vNormal * cfAlpha + rSource1.m_vNormal * cfBeta + rSource2.m_vNormal * cfGamma;
    rDest.m_vNormal.Normalize();
    if (cbCOmputeTS) {
        rDest.m_vTNormal = rSource0.m_vTNormal * cfAlpha + rSource1.m_vTNormal * cfBeta + rSource2.m_vTNormal * cfGamma;
        rDest.m_vTNormal.Normalize();
        rDest.m_vBinormal = rSource0.m_vBinormal * cfAlpha + rSource1.m_vBinormal * cfBeta + rSource2.m_vBinormal * cfGamma;
        rDest.m_vBinormal.Normalize();
        rDest.m_vTangent = rSource0.m_vTangent * cfAlpha + rSource1.m_vTangent * cfBeta + rSource2.m_vTangent * cfGamma;
        rDest.m_vTangent.Normalize();
    }
}

CRadPoly::CRadPoly()
    : m_pSource(nullptr), m_pMergeSource(nullptr), m_dwFlags(0), m_nX1(0), m_nY1(0), m_nX2(0), m_nY2(0), m_fX1(0.f), m_fY1(0.f), m_fX2(0.f), m_fY2(0.f), m_nW(0), m_nH(0), m_nAx1(0),
      m_nAx2(0), m_nAxis(0), m_pLightmapData(nullptr), m_pHDRLightmapData(nullptr), m_pDominantDirData(nullptr), m_pWSDominantDirData(nullptr), m_pOcclMapData(nullptr) {}
...
const float CRadPoly::PointInTriangle(const Vec3d& point, const int ciIndex0, const int ciIndex1) {
    ...
    for (int k = 0; k < 3; k++) {
        ...
        Ax = m_lstOriginals[k].m_vPos[ciIndex0];
        Bx = m_lstOriginals[kk2].m_vPos[ciIndex0];
        Ay = m_lstOriginals[k].m_vPos[ciIndex1];
        By = m_lstOriginals[kk2].m_vPos[ciIndex1];
...
CRadPoly::CRadPoly(CRadPoly* pSource)
    : m_Plane(pSource->m_Plane), m_dwFlags(pSource->m_dwFlags), m_pSource(pSource), m_fX1(pSource->m_fX1), m_fX2(pSource->m_fX2), m_fY1(pSource->m_fY1), m_fY2(pSource->m_fY2),
      m_nX1(0), m_nY1(0), m_nX2(0), m_nY2(0), m_nW(0), m_nH(0), m_nAx1(0), m_nAx2(0), m_nAxis(0), m_pLightmapData(nullptr), m_pHDRLightmapData(nullptr), m_pDominantDirData(nullptr),
      m_pWSDominantDirData(nullptr), m_pOcclMapData(nullptr) {}
...
void CRadPoly::CalculateTangentSpace(SUV vUV[3]) {
    ...
    if (div != 0.0f) {
        ...
        vU = (vEdge0 * a + vEdge1 * b) * fAreaMul2;
        vU.Normalize();
        vV = (vEdge0 * c + vEdge1 * d) * fAreaMul2;
        vV.Normalize();
    } else {
        vU = Vec3d(0, 0, 0);
        vV = Vec3d(0, 0, 0);
...
bool CRadPoly::InterpolateVertexAt(const float infX, const float infY, CRadVertex& outVertex, SComplexOSDot3Texel& rDot3Texel, const bool cbSubSample, const bool cbSnap) {
    ...
    if ((nearestpoly == nullptr) || !nearestpoly->ApplyBarycentricCoordinates(rvPoint, outVertex, rDot3Texel, false, cbSnap))
        return false;
    return true;
}
...
void CRadMesh::CreateEmpty(IEntityRender* pIEtyRend, const Matrix44& matEtyMtx) {
    IStatObj* pObj = pIEtyRend->GetEntityStatObj(0, nullptr);
...
const bool CRadMesh::FillInValues(IEntityRender* pIEtyRend, const CBrushObject* pBrushObject, const Matrix44& matEtyMtx) {
    ...
    for (iCurMat = 0; iCurMat < pMaterials->Count(); iCurMat++) {
        ...
        for (iCurTri = 0; iCurTri < cCurMat.nNumIndices / 3; iCurTri++) {
            ...
            pPoly->m_Plane.CalcPlane(matEtyMtx.TransformPointOLD(*reinterpret_cast<Vec3d*>(&(reinterpret_cast<byte*>(pVertices)[pIndices[iBaseIdx + 2] * iVtxStride]))),
                                     matEtyMtx.TransformPointOLD(*reinterpret_cast<Vec3d*>(&(reinterpret_cast<byte*>(pVertices)[pIndices[iBaseIdx + 1] * iVtxStride]))),
                                     matEtyMtx.TransformPointOLD(*reinterpret_cast<Vec3d*>(&(reinterpret_cast<byte*>(pVertices)[pIndices[iBaseIdx + 0] * iVtxStride]))));
            ...
            for (int v = 2; v >= 0; v--) {
                ...
                {
                    pVert.m_vPos = matEtyMtx.TransformPointOLD((*reinterpret_cast<Vec3d*>(&(reinterpret_cast<byte*>(pVertices)[pIndices[iBaseIdx + v] * iVtxStride]))));
                    vNormals[v] = pVert.m_vNormal =
                        GetTransposed44(matEtyMtx) * ((*reinterpret_cast<Vec3d*>(&(reinterpret_cast<byte*>(pNormals)[pIndices[iBaseIdx + v] * iNormalStride]))));
                    pVert.m_vBinormal =
                        GetTransposed44(matEtyMtx) * ((*reinterpret_cast<Vec3d*>(&(reinterpret_cast<byte*>(pBinormals)[pIndices[iBaseIdx + v] * iBinormalStride]))));
                    pVert.m_vTangent = GetTransposed44(matEtyMtx) * ((*reinterpret_cast<Vec3d*>(&(reinterpret_cast<byte*>(pTangents)[pIndices[iBaseIdx + v] * iTangentStride]))));
                    ...
                    pVert.m_vTNormal = GetTransposed44(matEtyMtx) * ((*reinterpret_cast<Vec3d*>(&(reinterpret_cast<byte*>(pTNormals)[pIndices[iBaseIdx + v] * iTNormalStride]))));
            ...
            if (bIsDecalMat) {
                ...
                if (fabs(vNormals[0] * vNormals[1] + 1.0f) < 0.01f || fabs(vNormals[2] * vNormals[0] + 1.0f) < 0.01f || fabs(vNormals[1] * vNormals[2] + 1.0f) < 0.01f) {
                    SUV uv[3];
...
inline DWORD CRadMesh::CalcLocalLightHashValue() const {
    DWORD dwRet = 0;
...
const float DistToLine(const Vec3d& rX0, const Vec3d& rX1, const Vec3d& rPoint) {
    ...
    return fabs((rX0 - rPoint).GetLengthSquared() * (rX1 - rX0).GetLengthSquared() - cfDot * cfDot) / (rX1 - rX0).GetLengthSquared();
}
...
const bool CRadPoly::SnapVertex(Vec3d& inPosition, float& outfAlpha, float& outfBeta, const float cfTriangleArea) {
    ...
    const int ciNearestEdgeIndex = (cfDistLV01 < cfDistLV02) ? (cfDistLV01 < cfDistLV12) ? 0 : 2 : (cfDistLV12 < cfDistLV02) ? 2 : 1;
...
const bool CRadPoly::SmoothVertex(CRadVertex& outVertex, const Vec3d& inPosition, const float cfArea3d, SComplexOSDot3Texel& rDot3Texel) {
    ...
    // now retrieve index of nearest edge
    const int ciNearestVertexIndex = (cfDistV01 < cfDistV02) ? (cfDistV01 < cfDistV03) ? 0 : 2 : (cfDistV03 < cfDistV02) ? 2 : 1;
    const int ciFarestVertexIndex = (cfDistV01 > cfDistV02) ? (cfDistV01 > cfDistV03) ? 0 : 2 : (cfDistV03 > cfDistV02) ? 2 : 1;
    ...
    for (SharedIter sharedIter = m_SharingPolygons.begin(); sharedIter != m_SharingPolygons.end(); ++sharedIter) {
        ...
        if (!cbOnlyOneVertexShared) {
            ...
            vNewDir = (cfDIst0 < cfDIst1) ? RotateIntoPlane(vPlaneNormal, vSharedPlaneNormal, inPosition, rSource0)
        ...
        if (cbBaryFailCond == true) {
            ...
            for (SharedIter sharedSharedIter = pSharedPoly->m_SharingPolygons.begin(); sharedSharedIter != pSharedPoly->m_SharingPolygons.end(); ++sharedSharedIter) {
                ...
                // try to rotate vertex into plane of this shared triangle, if rotations fails, it will just get projected
                if (!cbOnlyOneVertexShared) {
                    vNewDir = (cfDIst0 < cfDIst1) ? RotateIntoPlane(vPlaneNormal, vSharedSharedPlaneNormal, inPosition, rSource0)
