...
template <class T> _Vector3dC<T>::_Vector3dC() {}

template <class T> _Vector3dC<T>::_Vector3dC(T x, T y, T z) {
    p[0] = x;
    p[1] = y;
    p[2] = z;
}

template <class T> _Vector3dC<T>::_Vector3dC(const T& x) {
...
template <class T> _Vector3dC<T>::_Vector3dC(const _Vector3dC<T>& x) {
    p[0] = x.p[0];
    p[1] = x.p[1];
    p[2] = x.p[2];
}

template <class T> _Vector3dC<T>::_Vector3dC(const _Vector2dC<T>& x) {
    p[0] = x.p[0];
    p[1] = x.p[1];
    p[2] = 0;
}

template <class T> void _Vector3dC<T>::Set(T x, T y, T z) {
    p[0] = x;
    p[1] = y;
    p[2] = z;
}

template <class T> _Vector3dC<T> _Vector3dC<T>::operator-(const _Vector3dC<T>& b) const {
    return _Vector3dC<T>(p[0] - b.p[0], p[1] - b.p[1], p[2] - b.p[2]);
};

template <class T> _Vector3dC<T> _Vector3dC<T>::operator-=(const _Vector3dC<T>& b) {
    p[0] -= b.p[0];
    p[1] -= b.p[1];
    p[2] -= b.p[2];
...
template <class T> _Vector3dC<T> _Vector3dC<T>::operator*(const T f) const {
    return _Vector3dC<T>(p[0] * f, p[1] * f, p[2] * f);
}

template <class T> T _Vector3dC<T>::operator*(const _Vector3dC<T>& b) const {
    return (p[0] * b.p[0] + p[1] * b.p[1] + p[2] * b.p[2]);
}

template <class T> bool _Vector3dC<T>::operator==(const _Vector3dC<T>& other) const {
    if (p[0] != other.p[0])
        return false;
    if (p[1] != other.p[1])
        return false;
...
template <class T> _Vector3dC<T> _Vector3dC<T>::operator*=(const T f) {
    p[0] *= f;
    p[1] *= f;
    p[2] *= f;
...
template <class T> _Vector3dC<T> CrossProd(const _Vector3dC<T>& a, const _Vector3dC<T>& b) {
    _Vector3dC<T> ret;
...
template <class T> void _Vector3dC<T>::Normalize() {
    T len = 1.0f / Length();
...
template <class T> T _Vector3dC<T>::Length() const {
    return ((T)sqrt(p[0] * p[0] + p[1] * p[1] + p[2] * p[2]));
}

// unary negation
template <class T> const _Vector3dC<T> _Vector3dC<T>::operator-() const {
    return (_Vector3dC<T>(-p[0], -p[1], -p[2]));
}

typedef _Vector3dC<double> CVector3D;

template <class T> class CPossibleIntersectionSink {
public:
    //! return one element from the bucket
    //! /param inObject
    //! /param inoutfRayMaxDist in and out value for max. shooting distance or FLT_MAX, if you found a interesection you may update this because further intersections are no longer
    //! interesting /return true=do further processing, false=stop I found what I need (e.g. any intersection for shadow testing)
...
template <class T, bool bBreakAfterFirstHit = false, bool bUseXRaster = true> class CRasterCube {
public:
    //! default constructor
    CRasterCube();
    ...
    template <class Sink> const bool GatherElementsAt(int iniPos[3], Sink& inSink, float& rfRet) {
        rfRet = FLT_MAX;
    ...
    template <class Sink> void GatherRayHitsDirection(const CVector3D _invStart, const CVector3D _invDir, Sink& inSink, const float infRayMax = FLT_MAX) {
    ...
    template <class Sink> void GatherRayHitsTo(const CVector3D _invStart, const CVector3D _invEnd, Sink& inSink) {
        CVector3D vDir = _invEnd - _invStart;
        float fRayMax = (float)vDir.Length();
...
// constructor
template <class T, bool bBreakAfterFirstHit, bool bUseXRaster> CRasterCube<T, bBreakAfterFirstHit, bUseXRaster>::CRasterCube() {
    DeInit();
}
...
// free data
template <class T, bool bBreakAfterFirstHit, bool bUseXRaster> void CRasterCube<T, bBreakAfterFirstHit, bUseXRaster>::DeInit() {
    if (bUseXRaster)
        m_XRaster.DeInit();
    m_YRaster.DeInit();
    m_ZRaster.DeInit();
...
// Has to be called in Phase1, after that Phase 2 has started
template <class T, bool bBreakAfterFirstHit, bool bUseXRaster> bool CRasterCube<T, bBreakAfterFirstHit, bUseXRaster>::PreProcess(const bool inbDebug) {
    if (inbDebug) {
        if (bUseXRaster)
...
// Has to be called in Phase1, after that Phase 2 has started
template <class T, bool bBreakAfterFirstHit, bool bUseXRaster> void CRasterCube<T, bBreakAfterFirstHit, bUseXRaster>::Compress() {
    if (bUseXRaster)
        m_XRaster.Compress();
    m_YRaster.Compress();
    m_ZRaster.Compress();
...
template <class T, bool bBreakAfterFirstHit, bool bUseXRaster> DWORD CRasterCube<T, bBreakAfterFirstHit, bUseXRaster>::CalcMemoryConsumption(DWORD outdwSize[3]) {
