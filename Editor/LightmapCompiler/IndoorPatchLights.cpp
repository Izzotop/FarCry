...
ICompilerProgress* g_pIProgress = nullptr;
...
void CRasterCubeManager::AddReference(const CRadMesh* cpRadMesh) {
    if (cpRadMesh->m_pClusterRC == nullptr)
        return;
...
const bool CRasterCubeManager::RemoveReference(CRadMesh* cpRadMesh) {
    ...
    std::map<CRasterCubeImpl*, std::set<const CRadMesh*>>::iterator iter = m_mRasterCubeMap.find(cpRadMesh->m_pClusterRC);
    cpRadMesh->m_pClusterRC = nullptr;
...
CLightScene::CLightScene()
    : m_uiCurrentPatchNumber(0), m_pCurrentPatch(nullptr), m_pISerializationManager(nullptr), m_uiStartMemoryConsumption(0), m_puiIndicator(0), m_pOcclSamples(0), m_pSubSamples(0),
      m_pfColours(0), m_uiSampleCount(0) {
...
CLightScene::~CLightScene() {
    if (m_pISerializationManager) {
        ...
        m_pISerializationManager = nullptr;
    }
    if (m_puiIndicator) {
        delete[] m_puiIndicator;
        m_puiIndicator = nullptr;
    }
    if (m_pSubSamples) {
        delete[] m_pSubSamples;
        m_pSubSamples = nullptr;
    }
    if (m_pOcclSamples) {
        delete[] m_pOcclSamples;
        m_pOcclSamples = nullptr;
    }
    if (m_pfColours) {
        delete[] m_pfColours;
        m_pfColours = nullptr;
...
inline void CLightScene::MergeTangentSpace(CRadVertex& rVertex1, CRadVertex& rVertex2) {
    ...
    Vec3d vNewNormal = (rVertex1.m_vNormal * 0.5f + rVertex2.m_vNormal * 0.5f);
...
void CLightScene::ComputeSmoothingInformation(const unsigned int uiSmoothAngle, const bool cbTSGeneratedByLM) {
    ...
    const float epsNormalSmooth =
        cosf(0.001f /*some threshold*/ + (float)uiSmoothAngle / 180.f * 3.14159f); // use a coarser eps for normals, everything which is dotted below 45 degree
    // iterate each lightmap patch
    for (radpolyit iterPatchOuter = m_lstScenePolys.begin(); iterPatchOuter != m_lstScenePolys.end(); iterPatchOuter++) { // outer lightmap mesh loop
        CRadPoly* pPatchOuter = (*iterPatchOuter);
        // with each remaining one
        for (radpolyit iterPatchInner = iterPatchOuter + 1; iterPatchInner != m_lstScenePolys.end(); iterPatchInner++) { // inner lightmap mesh loop
            ...
            {
                for (radpolyit iterPolyOuter = pPatchOuter->m_lstMerged.begin(); iterPolyOuter != pPatchOuter->m_lstMerged.end();
                     iterPolyOuter++) { // outer lightmap mesh polygon iteration
                    ...
                    for (radpolyit iterPolyInner = pPatchInner->m_lstMerged.begin(); iterPolyInner != pPatchInner->m_lstMerged.end();
                         iterPolyInner++) {                                         // inner lightmap mesh polygon iteration
                        ...
                        for (int k1 = 0; k1 < nVerts1; k1++) { // outer lightmap mesh polygon per vertex iteration
                            CRadVertex& rVert1 = rPolyOuter.m_lstOriginals[k1];
                            for (int k2 = 0; k2 < nVerts2; k2++) { // inner lightmap mesh polygon per vertex iteration
                                CRadVertex& rVert2 = rPolyInner.m_lstOriginals[k2];
                                const float fDot = rVert1.m_vNormal * rVert2.m_vNormal;
                                if (fabs(rVert1.m_vPos.x - rVert2.m_vPos.x) < epsPos && fabs(rVert1.m_vPos.y - rVert2.m_vPos.y) < epsPos &&
                                    fabs(rVert1.m_vPos.z - rVert2.m_vPos.z) < epsPos && fDot > epsNormalSmooth) {
                                    ...
                                    if (cbTSGeneratedByLM) {
                                        ...
                                        if (!bMerged[k1] &&
                                            rVert2.m_vNormal == rVert2.m_vTNormal) // if second vertex was merged, the tangent normal and vertex normal are different
                                        {
                                    ...
                                    rPolyOuter.m_SharingPolygons.push_back(
                                        std::pair<CRadPoly*, unsigned int>(&rPolyInner, uiSecondOuter)); // add this polygon to the respective list and vice versa
                            ...
                            if (bShareCondFound0) {
                                ...
                                // prepare second pair values
                                const unsigned int uiSecondOuter =
                                    ((iFirstFoundIndexOuter | (CRadPoly::scuiOneVertexShareFlag << 8)) | ((iFirstFoundIndexInner | (CRadPoly::scuiOneVertexShareFlag << 8)) << 16));
                                const unsigned int uiSecondInner =
                                    ((iFirstFoundIndexInner | (CRadPoly::scuiOneVertexShareFlag << 8)) | ((iFirstFoundIndexOuter | (CRadPoly::scuiOneVertexShareFlag << 8)) << 16));
                                rPolyOuter.m_SharingPolygons.push_back(
                                    std::pair<CRadPoly*, unsigned int>(&rPolyInner, uiSecondOuter)); // add this polygon to the respective list and vice versa
                                rPolyInner.m_SharingPolygons.push_back(std::pair<CRadPoly*, unsigned int>(&rPolyOuter, uiSecondInner));
                            }
...
bool CLightScene::ComputeRasterCube(CRasterCubeImpl* pRasterCube, const std::set<IEntityRender*>& vGeom, const Matrix33* pDirLightTransf) {
    // ---------------------------------------------------------------------------------------------
    ...
    Vec3d vTri[3];
    const CObjFace* pFace = nullptr;
    CIndoorArea* pArea = nullptr;
    INT iCurFace = 0;
    bool bFirstPass;
    RasterCubeUserT sUserTri;
    Vec3d vEdge1, vEdge2;
    CMatInfo* pMat = nullptr;
    IShader* pEff = nullptr;
    IShader* pShTempl = nullptr;
    UINT iNumNonOpaqueSkipped = 0;
    UINT iNumDecalsSkipped = 0;
    UINT iCurGeom = 0;
    IStatObj* pIGeom = nullptr;
    Matrix44 matEtyMtx;
    ...
    for (std::set<IEntityRender*>::const_iterator itGeom = vGeom.begin(); itGeom != vGeom.end(); itGeom++) {
        ...
        iCurGeom = 0;
        while (pIGeom = (*itGeom)->GetEntityStatObj(iCurGeom++, nullptr)) {
    ...
    while (true) {
                ...
                for (iCurMat = 0; iCurMat < pMaterials->Count(); iCurMat++) {
                    ...
                    if (pEff) {
                        ...
                        if (pShTempl) {
                            ...
                            if ((pShTempl->GetFlags2() & EF2_NOCASTSHADOWS) || (pShTempl->GetFlags2() & EF2_OPAQUE) == 0 || (pShTempl->GetFlags3() & EF3_NODRAW)) {
                    ...
                    for (iCurTri = 0; iCurTri < cCurMat.nNumIndices / 3; iCurTri++) {
                        ...
                        vWorldSpaceTri[0] = matEtyMtx.TransformPointOLD(*reinterpret_cast<Vec3d*>(&(reinterpret_cast<byte*>(pVertices)[pIndices[iBaseIdx + 0] * iVtxStride])));
                        vWorldSpaceTri[1] = matEtyMtx.TransformPointOLD(*reinterpret_cast<Vec3d*>(&(reinterpret_cast<byte*>(pVertices)[pIndices[iBaseIdx + 1] * iVtxStride])));
                        vWorldSpaceTri[2] = matEtyMtx.TransformPointOLD(*reinterpret_cast<Vec3d*>(&(reinterpret_cast<byte*>(pVertices)[pIndices[iBaseIdx + 2] * iVtxStride])));
        ...
        if (bFirstPass) {
            ...
            if (!pRasterCube->PreProcess(szDebugName))
                return false;
#else
            if (!pRasterCube->PreProcess(nullptr))
                return false;
#endif
        } else
            break;
...
struct LightCluster {
    ...
    Vec3d vMin;
    Vec3d vMax;
    LightCluster() : vMin(FLT_MAX, FLT_MAX, FLT_MAX), vMax(FLT_MIN, FLT_MIN, FLT_MIN), pPointLtRC(nullptr) {}
};

// returns true if there is a conservative 2D sun space overlap
static const bool CheckOverlapInSunSpace(const Vec3d& rClusterMin, const Vec3d& rClusterMax, const Vec3d& rGLMMin, const Vec3d& rGLMMax, const Matrix33& rSunSpace,
                                         const bool cbUpdateMinMax) {
    ...
    if (cbUpdateMinMax) {
        fMinXCluster = FLT_MAX;
        fMinYCluster = FLT_MAX;
        fMaxXCluster = -FLT_MAX;
        fMaxYCluster = -FLT_MAX;
...
void CLightScene::SelectObjectsFromChangedLMShadowCaster(std::vector<std::pair<IEntityRender*, CBrushObject*>>& vNodes, const std::vector<AABB>& vAABBs,
                                                         const Matrix33& rMatSunBasis, const std::vector<unsigned int>& rvNodeRadMeshIndices) {
    ...
    for (std::list<CRadMesh*>::iterator itMesh = m_lstRadMeshes.begin(); itMesh != m_lstRadMeshes.end(); itMesh++) {
        ...
        if (pMesh == nullptr || !(pMesh->m_uiFlags & REBUILD_USED) || !(pMesh->m_uiFlags & HASH_CHANGED))
        ...
        for (std::list<CRadMesh*>::iterator itMeshSV = m_lstRadMeshes.begin(); itMeshSV != m_lstRadMeshes.end(); itMeshSV++) {
            ...
            if (pShadowMesh == nullptr || pShadowMesh == pMesh)
...
void CLightScene::SelectObjectLMReceiverAndShadowCasterForChanges(std::vector<std::pair<IEntityRender*, CBrushObject*>>& vNodes, const std::vector<AABB>& vAABBs,
                                                                  const Matrix33& rMatSunBasis, const std::vector<unsigned int>& rvNodeRadMeshIndices, const ELMMode Mode) {
    ...
    for (std::list<CRadMesh*>::iterator itMesh = m_lstRadMeshes.begin(); itMesh != m_lstRadMeshes.end(); itMesh++) {
        iMeshCounter++;
        CRadMesh* pMesh = *itMesh;
        if (pMesh == nullptr || pMesh->m_bReceiveLightmap == false)
            continue; // was not valid or does not receive lightmaps
        // get bounding box for this glm
        const AABB& rAABBReceiver = vAABBs[iMeshCounter];
        // create shadow volumes for all lights
    ...
    for (std::list<CRadMesh*>::iterator itMesh = m_lstRadMeshes.begin(); itMesh != m_lstRadMeshes.end(); itMesh++) {
        ...
        if (*itMesh && !((*itMesh)->m_uiFlags & REBUILD_USED)) {
            delete *itMesh;
            *itMesh = nullptr; // will get tested every time it gets accessed
            // remove from node list as well
            vNodes[rvNodeRadMeshIndices[iCounter]].first = nullptr; // will get tested everytime it gets iterated
        }
    }
}
...
bool CLightScene::CreateFromEntity(const IndoorBaseInterface& pInterface, LMGenParam sParam, std::vector<std::pair<IEntityRender*, CBrushObject*>>& vNodes,
                                   CLMLightCollection& cLights, ICompilerProgress* pIProgress, const ELMMode Mode, volatile SSharedLMEditorData* pSharedData,
                                   const std::set<const CBrushObject*>& vSelectionIndices, bool& rErrorsOccured) {
    ...
    CRadMesh* pMesh = nullptr;
    IEntityRender* pIEtyRend = nullptr;
    Matrix44 matEtyMtx;
    IStatObj* pIGeom = nullptr;
    CRadPoly* pCurPoly = nullptr;
    UINT iCurGeom;
    ...
    assert(m_pISerializationManager != nullptr);
    ...
    if (Mode != ELMMode_ALL)
        m_pISerializationManager->Load(m_IndInterface.m_p3dEngine->GetFilePath(LM_EXPORT_FILE_NAME), false /*load no textures*/);
    ...
    if (!m_pISerializationManager->ExportDLights(strLMExportFile.c_str(), (const CDLight**)&cLights.GetSrcLights()[0], (UINT)cLights.GetSrcLights().size(), bNewZip)) {
        assert(false);
    ...
    std::vector<unsigned int> vNodeRadMeshIndices;
    vNodeRadMeshIndices.reserve(vNodes.size()); // saves per added mesh the node index
    int iNodeIndex = -1;
    for (itEty = vNodes.begin(); itEty != vNodes.end(); itEty++) {
        iNodeIndex++;
        pIEtyRend = itEty->first;
        if (pIEtyRend == nullptr)
            continue;

        iCurGeom = 0;
        if (pSharedData != nullptr)
            DisplayMemStats(pSharedData);
        if (pSharedData != nullptr && pSharedData->bCancelled == true) {
            break;
        }
        ...
        for (itLight = cLights.GetLights().begin(); itLight != cLights.GetLights().end(); itLight++) {
            ...
            if (pIEtyRend->GetEntityVisArea() != nullptr && cCurLight.eType == eDirectional)
            ...
            if (Overlap::Sphere_AABB(Sphere(cCurLight.vWorldSpaceLightPos, cCurLight.fRadius), MakeSafeAABB(vNewMin, vNewMax))) {
                if (cCurLight.uiFlags & DLF_THIS_AREA_ONLY) {
                    if (cCurLight.pVisArea != 0 && pIEtyRend->GetEntityVisArea() != 0) {
                        if (pIEtyRend->GetEntityVisArea() != cCurLight.pVisArea) {
                            ...
                            const bool bNearFound = pIEtyRend->GetEntityVisArea()->FindVisArea(
                                (IVisArea*)(cCurLight.pVisArea), 1 + /*int((cLight.uiFlags & DLF_THIS_AREA_ONLY)==0)+*/ int(((IVisArea*)(cCurLight.pVisArea))->IsPortal()), false);
                            if (!bNearFound)
                                continue; // areas do not much
                        }
                    } else if (cCurLight.pVisArea != 0 || pIEtyRend->GetEntityVisArea() != 0) {
                        continue;
                    }
                }
                pMesh->m_uiFlags &= ~ONLY_SUNLIGHT;
        ...
        if (!pMesh->FillInValues(pIEtyRend, pBrushObject, matEtyMtx) || pMesh->m_lstOldPolys.size() <= 0) {
            ...
            itEty->first = nullptr; // will get tested everytime it gets iterated
        } else {                 // if at least one polygon has been added
    ...
    if (m_sParam.m_bComputeShadows && (pSharedData == nullptr || (pSharedData != nullptr && pSharedData->bCancelled == false))) {
        if (pSharedData != nullptr)
            DisplayMemStats(pSharedData);
        ...
        _TRACE(m_LogInfo, true, "\r\nComputing clusters...");
        {
            ...
            for (itCluster = m_lstClusters.begin(); itCluster != m_lstClusters.end(); ++itCluster) {
                ...
                for (itClusterSearch = itClusterStart; itClusterSearch != m_lstClusters.end();) {
                    ...
                    if (vOldDimensions.x > fMaxClusterSize && vNewDimensions.x > vOldDimensions.x) {
                        ++itClusterSearch;
                        continue;
                    }
                    if (vOldDimensions.y > fMaxClusterSize && vNewDimensions.y > vOldDimensions.y) {
                        ++itClusterSearch;
                        continue;
                    }
                    if (vOldDimensions.z > fMaxClusterSize && vNewDimensions.z > vOldDimensions.z) {
                        ++itClusterSearch;
                        continue;
                    }
                    ...
                    if (Overlap::AABB_AABB(Vec3(0, 0, 0), AABB((*itCluster).vMin - vIntersectEpsilon, (*itCluster).vMax + vIntersectEpsilon), Vec3(0, 0, 0),
                                           AABB((*itClusterSearch).vMin - vIntersectEpsilon, (*itClusterSearch).vMax + vIntersectEpsilon))) {
                        // Merge AABB
                        (*itCluster).vMin = vMin;
                        (*itCluster).vMax = vMax;

                        // Merge GLM lists
                        for (std::set<IEntityRender*>::iterator it = (*itClusterSearch).vGLMsAffectingCluster.begin(); it != (*itClusterSearch).vGLMsAffectingCluster.end(); ++it) {
            ...
            for (itCluster = m_lstClusters.begin(); itCluster != m_lstClusters.end(); ++itCluster) {
                ...
                for (std::vector<LMCompLight>::const_iterator itLight = cLights.GetLights().begin(); itLight != cLights.GetLights().end(); itLight++) {
                    ...
                    if (Overlap::Sphere_AABB(Sphere((*itLight).vWorldSpaceLightPos, (*itLight).fRadius), MakeSafeAABB((*itCluster).vMin, (*itCluster).vMax))) {
            ...
            for (itCluster = m_lstClusters.begin(); itCluster != m_lstClusters.end(); itCluster++) {
                ...
                for (itMesh = m_lstRadMeshes.begin(); itMesh != m_lstRadMeshes.end(); itMesh++) {
                    ...
                    if (vGLMMin.x < cWorldSpaceAABB.min.x || vGLMMin.y < cWorldSpaceAABB.min.y || vGLMMin.z < cWorldSpaceAABB.min.z) {
                        continue;
                    }

                    if (vGLMMax.x > cWorldSpaceAABB.max.x || vGLMMax.y > cWorldSpaceAABB.max.y || vGLMMax.z > cWorldSpaceAABB.max.z) {
                        continue;
                    }
...
#ifdef DISPLAY_MORE_INFO
                _TRACE(m_LogInfo, true,
                       "Computing raster cube for light cluster %i of %i with %i GLMs contained in it / %i GLMs affecting it located"
                       " at (%f, %f, %f) - (%f, %f, %f)\r\n",
                       ++iCurCluster, iNumClusters, iNumGLMsContained, (*itCluster).vGLMsAffectingCluster.size(), (*itCluster).vMin.x, (*itCluster).vMin.y, (*itCluster).vMin.z,
                       (*itCluster).vMax.x, (*itCluster).vMax.y, (*itCluster).vMax.z);
#else
                _TRACE(m_LogInfo, false,
                       "Computing raster cube for light cluster %i of %i with %i GLMs contained in it / %i GLMs affecting it located"
                       " at (%f, %f, %f) - (%f, %f, %f)\r\n",
                       ++iCurCluster, iNumClusters, iNumGLMsContained, (*itCluster).vGLMsAffectingCluster.size(), (*itCluster).vMin.x, (*itCluster).vMin.y, (*itCluster).vMin.z,
                       (*itCluster).vMax.x, (*itCluster).vMax.y, (*itCluster).vMax.z);
    ...
    if (pSharedData == nullptr || (pSharedData != nullptr && pSharedData->bCancelled == false)) {
        if (pSharedData != nullptr)
            DisplayMemStats(pSharedData);
        ...
        if (pSharedData != nullptr && pSharedData->bCancelled) {
            // free memory
            ...
            delete m_pCurrentPatch;
            m_pCurrentPatch = nullptr;
        }
    }
...
void CLightScene::Shadow(LMCompLight& cLight, UINT& iNumHit, CRadMesh* pMesh, CRadPoly* pSource, const CRadVertex& Vert, const Vec3d& vSamplePos) {
    ...
    cAnyHit.SetupRay(vRayDirNormalized, vRayOrigin, fRayLen, m_sParam.m_fTexelSize, pSource);
    ...
    if (!bSkip) {
        if (cLight.eType == eDirectional) {
            ...
            if (pMesh->m_pClusterRC != nullptr) {
                // Check occlusion
                pMesh->m_pClusterRC->GatherRayHitsDirection(CVector3D(vRayOrigin.x, vRayOrigin.y, vRayOrigin.z),
                                                            CVector3D(vRayDirNormalized.x, vRayDirNormalized.y, vRayDirNormalized.z), cAnyHit);
...
static inline const bool SumDot3Values(const LMCompLight& cLight, float& fIntens, float& fLambLumSum, float& fLumSum, float& fColorRLamb, float& fColorGLamb, float& fColorBLamb,
                                       const LMCompLight& rLight, Vec3d& rvSumLightDir, const Vec3d& rvLightDir, const CRadVertex& rVertex) {
...
static void GetDot3Sample(float& fLM_DP3LerpFactor, Vec3d& rvLightDir, float fLumSum, float fLambLumSum, const CRadVertex& rVert, float& fColorRLamb, float& fColorGLamb,
                          float& fColorBLamb, const bool cbApplyBetterLighting = false) {
    // ---------------------------------------------------------------------------------------------
...
void CLightScene::DoLightCalculation(unsigned int& ruiMaxColourComp, const std::vector<LMCompLight*>& vLights, const std::vector<LMCompLight*>& vOcclLights,
                                     SComplexOSDot3Texel& dot3Texel, CRadMesh* pMesh, CRadPoly* pSource, const CRadVertex& Vert, const bool cbFirstPass,
    ...
    for (std::vector<LMCompLight*>::const_iterator ligIt = vLights.begin(); ligIt < vLights.end(); ligIt++) {
        ...
        if (cLight.eType != eDirectional) { // get vertex light direction if not directional
    ...
    if (cbFirstPass)
#ifdef APPLY_COLOUR_FIX
        ruiMaxColourComp = __max(ruiMaxColourComp,
...
bool CLightScene::Create(const IndoorBaseInterface& pInterface, const Vec3d& vMinBB, const Vec3d& vMaxBB, volatile SSharedLMEditorData* pSharedData, const ELMMode Mode,
                         const unsigned int cuiMeshesToProcessCount) {
    // for logging stuff
    ...
    if (cuiMeshesToProcessCount > 0 && m_lstRadMeshes.size() > 0) {
        ...
        if (pMesh && (pMesh->m_bReceiveLightmap == true)) {
            pIGeom = pMesh->m_pESource->GetEntityStatObj(0, nullptr);
            int iNumIndices = 0;
            ...
            _TRACE(sTraceString);
            sprintf(sTraceString, "Processing GLM No.1 ('%s' with brush type '%s') with %i Tri and %i Light%c...\r\n", pMesh->m_sGLMName, pIGeom ? pIGeom->GetFileName() : "",
                    iNumIndices / 3, pMesh->m_LocalLights.size(), pMesh->m_LocalLights.size() == 1 ? ' ' : 's');
    ...
    for (; itRadMesh != m_lstRadMeshes.end(); itRadMesh++) {
        ...
        if ((retValue = CheckNormals(fVariance, pMesh)) > 0) {
            if (retValue & WRONG_NORMALS_FLAG) {
            ...
            } else if (retValue & NOT_NORMALIZED_NORMALS_FLAG) {
#ifdef DISPLAY_MORE_INFO
                ...
                sprintf(text, "GLM: %s (%s) has denormalized normals, max variance to 1.0: %f, automatic renormalization\r\n", (const char*)pMesh->m_sGLMName,
                        pIGeom ? pIGeom->GetFileName() : "", fVariance);
        ...
        // free patch
        delete pMesh;
        *itRadMesh = pMesh = nullptr;
        bStarted = true;
    } // itRadMesh
    m_lstRadMeshes.clear();
    // signal completion
    if (pSharedData != nullptr) {
        const unsigned char cuiProgress =
            (bSerialize == true) ? static_cast<unsigned int>(static_cast<float>(cuiMeshesToProcessCount) / static_cast<float>(cuiMeshesToProcessCount + 1) * 100.0f) : 0;
...
void CLightScene::GenerateTexCoords(const CRadMesh& rRadMesh, LMAssignQueueItem& rNewGLMQueueItm) {
    ...
    CLeafBuffer* pLB = (rRadMesh.m_pESource->GetEntityStatObj(0, nullptr))->GetLeafBuffer();
    assert(!pLB->IsEmpty());
    int iIdxCnt = 0;
    unsigned short* pIndices = pLB->GetIndices(&iIdxCnt);
    UINT iCurMat;
    ...
    for (iCurMat = 0; iCurMat < pMaterials->Count(); iCurMat++) {
        ...
        for (iCurTri = 0; iCurTri < cCurMat.nNumIndices / 3; iCurTri++) {
            ...
            rNewGLMQueueItm.vSortedTexCoords[pIndices[iBaseIdx]] = (TexCoord2Comp(pPoly->m_lstOriginals[2].m_fpX, pPoly->m_lstOriginals[2].m_fpY));
            rNewGLMQueueItm.vSortedTexCoords[pIndices[iBaseIdx + 1]] = (TexCoord2Comp(pPoly->m_lstOriginals[1].m_fpX, pPoly->m_lstOriginals[1].m_fpY));
            rNewGLMQueueItm.vSortedTexCoords[pIndices[iBaseIdx + 2]] = (TexCoord2Comp(pPoly->m_lstOriginals[0].m_fpX, pPoly->m_lstOriginals[0].m_fpY));
            itPoly++;
        }
    }
}
...
void CLightScene::FlushAssignQueue() {
    ...
    m_pISerializationManager->AddRawLMData(m_sParam.m_iTextureResolution, m_sParam.m_iTextureResolution, vGLM_IDs_Using, m_pCurrentPatch->m_pLightmapImage,
                                           m_pCurrentPatch->m_pHDRLightmapImage, m_pCurrentPatch->m_pDominantDirImage,
                                           m_pCurrentPatch->m_pOcclMapImage ? reinterpret_cast<BYTE*>(&(m_pCurrentPatch->m_pOcclMapImage[0].colour)) : 0);

    // Create a new lightmap
    RenderLMData_AutoPtr pNewLM = m_pISerializationManager->CreateLightmap(
        nullptr, 0, m_sParam.m_iTextureResolution, m_sParam.m_iTextureResolution, m_pCurrentPatch->m_pLightmapImage, m_pCurrentPatch->m_pHDRLightmapImage,
        m_pCurrentPatch->m_pDominantDirImage, m_pCurrentPatch->m_pOcclMapImage ? reinterpret_cast<BYTE*>(&(m_pCurrentPatch->m_pOcclMapImage[0].colour)) : 0);
...
void CLightScene::GatherSubSampleTexel(const CRadPoly* pSource, const int ciX, const int ciY, std::set<std::pair<unsigned int, unsigned int>>& rvSubTexels) {
    ...
    const unsigned char* pHDRTexelColour = nullptr;
    ...
    for (int u = max(ciX - 1, 0); u <= min(ciX + 1, cuiPatchWidth - 1); u++)
        for (int v = max(ciY - 1, 0); v <= min(ciY + 1, cuiPatchHeight - 1); v++) {
            ...
            if (rDot3NeighbourTexel.pSourcePatch == nullptr && rDot3Texel.pSourcePatch == nullptr)
                continue;
...
void CLightScene::SetSubSampleDot3LightmapTexel(const unsigned int cuiSubSampleIndex, const float fColorRLamb, const float fColorGLamb, const float fColorBLamb, Vec3d& inLightDir,
                                                const float cfLM_DP3LerpFactor, SComplexOSDot3Texel& rDot3Texel, const SOcclusionMapTexel& rOcclTexel, bool bHDR) {
    ...
    */
    if (m_uiSampleCount > 1) // perform subsampling only if requested
    {
        ...
        for (std::set<std::pair<unsigned int, unsigned int>>::const_iterator iter = vSubTexels.begin(); iter != vSubTexels.end(); ++iter) {
            ...
            const unsigned int cuiX = iter->first;
            const unsigned int cuiY = iter->second;
    ...
    assert(pSource->m_pDominantDirData == nullptr); // shouldnt yet be allocated

#ifdef USE_DOT3_ALPHA
    pSource->m_pDominantDirData = new unsigned char[pSource->m_nW * pSource->m_nH * 4];
    assert(pSource->m_pDominantDirData);
...
#else
    pSource->m_pDominantDirData = new unsigned char[pSource->m_nW * pSource->m_nH * 3];
    assert(pSource->m_pDominantDirData);
    ...
    delete[] pSource->m_pWSDominantDirData;
    pSource->m_pWSDominantDirData = nullptr;
