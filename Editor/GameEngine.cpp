...
struct SSystemUserCallback : public ISystemUserCallback {
    bool OnError(const char* szErrorString) {
        ...
        int res = MessageBox(nullptr, str, "Engine Error", MB_YESNOCANCEL | MB_ICONERROR | MB_TOPMOST | MB_APPLMODAL | MB_DEFAULT_DESKTOP_ONLY);
...
CGameEngine::~CGameEngine() {
    ...
    m_ISystem->GetIMovieSystem()->SetCallback(nullptr);
...
#ifndef _ISNOTFARCRY
    if (m_IGame)
        GetIXGame(m_IGame)->CheckSoundVisAreas();
#endif
...
#ifndef _ISNOTFARCRY
        if (m_IGame)
            GetIXGame(m_IGame)->RetriggerAreas();
        if (m_IGame)
            GetIXGame(m_IGame)->HideLocalPlayer(true, true);
#endif
    }
