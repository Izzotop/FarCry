...
ON_WM_PAINT()
ON_WM_LBUTTONDOWN()
ON_WM_RBUTTONDOWN() ON_WM_MOUSEMOVE() ON_WM_LBUTTONUP() ON_WM_RBUTTONUP() ON_WM_SETCURSOR() ON_WM_LBUTTONDBLCLK() ON_WM_RBUTTONDOWN() ON_WM_KEYDOWN()
    //}}AFX_MSG_MAP
    END_MESSAGE_MAP()
...
void CCurveWnd::SetActiveKnot(UINT nIndex) {
    ...
    if (m_nActiveKnot != -1) {
        ...
        CKnot* pKnotOld = m_pCurve->GetKnot(m_nActiveKnot);
    ...
    if (nIndex != -1) {
        ...
        CKnot* pKnotNew = m_pCurve->GetKnot(nIndex);
...
UINT CCurveWnd::GetActiveKnot() {
    return m_nActiveKnot;
}
...
CCurveObject* CCurveWnd::GetCurveObject() {
    return m_pCurve;
}
