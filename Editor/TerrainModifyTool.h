...
enum BrushType { eBrushFlatten = 1, eBrushSmooth, eBrushRiseLower, eBrushTypeLast };
...
public:
    ...
    BrushType GetActiveBrushType() const {
        return m_currentBrushType;
    }
    void SetBrush(const CTerrainBrush& brush) {
        m_brush[m_currentBrushType] = brush;
    };
    void GetBrush(CTerrainBrush& brush) const {
        brush = m_brush[m_currentBrushType];
    };
    void EnableBrushNoise(bool enable) {
        m_brush[m_currentBrushType].bNoise = enable;
    };
