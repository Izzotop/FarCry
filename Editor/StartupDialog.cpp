...
CStartupDialog::CStartupDialog(CWnd* pParent /*=nullptr*/) : CDialog(CStartupDialog::IDD, pParent) {
...
void CStartupDialog::OnOK() {
    ...
    case 0:
        ...
        if (GetIEditor()->IsModified()) {
            ...
            nResult = MessageBox("Save changes to document ?", "CryEngine Sandbox", MB_ICONQUESTION | MB_YESNOCANCEL | MB_APPLMODAL);
        ...
        sprintf(szLevelRoot, "%sLevels\\%s", GetIEditor()->GetMasterCDFolder(), m_strDirName.GetBuffer(1));
            ...
            if (nResult == IDYES) {
                ...
                */
                ...
                if (PathFileExists(strFilePath.GetBuffer(0))) {
                    ...
                    // Load the bitmap
                    bReturn = bmpLoad.Attach(::LoadImage(AfxGetInstanceHandle(), strFilePath.GetBuffer(0), IMAGE_BITMAP, 0, 0, LR_DEFAULTSIZE | LR_LOADFROMFILE));
                    ASSERT(bReturn);
                ...
                if (PathFileExists(strFilePath.GetBuffer(0))) {
                    ...
                    VERIFY(CopyFile(strFilePath.GetBuffer(0), (CString(szLevelRoot) + "\\Skybox\\12.jpg").GetBuffer(0), FALSE));
                }
                strFilePath = GetIEditor()->GetDocument()->GetMasterCDFolder() + "EditorTemplate\\Skybox\\34.jpg";
                if (PathFileExists(strFilePath.GetBuffer(0))) {
                    VERIFY(CopyFile(strFilePath.GetBuffer(0), (CString(szLevelRoot) + "\\Skybox\\34.jpg").GetBuffer(0), FALSE));
                }
                strFilePath = GetIEditor()->GetDocument()->GetMasterCDFolder() + "EditorTemplate\\Skybox\\5.jpg";
                if (PathFileExists(strFilePath.GetBuffer(0))) {
                    VERIFY(CopyFile(strFilePath.GetBuffer(0), (CString(szLevelRoot) + "\\Skybox\\5.jpg").GetBuffer(0), FALSE));
                }
            }
        }
        ...
        if (nResult == IDYES) {
            ...
            nResult = MessageBox("Do you want to make a full export before starting to edit the level ? This"
                                 ...
                                 "Do export first ?", MB_YESNO | MB_ICONQUESTION | MB_APPLMODAL);
        }
...
void CStartupDialog::OnBrowseForMap() {
    ...
    if (cDialog.SelectFileName(&strFileName, nullptr, CString("*.cry"), CString("Levels"))) {
...
void CStartupDialog::OnChangeLevDir() {
    ...
    for (i = 0; i < (unsigned int)cDialog.m_strString.GetLength(); i++) {
        ...
        // Is the char invalid ?
        if (strpbrk(szCompBuf, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_ ") == nullptr) {
            bPathIsValid = false;
            break;
        }
    }
    ...
    // Construct the directory name
    sprintf(szLevelRoot, "%sLevels\\%s", GetIEditor()->GetDocument()->GetMasterCDFolder().GetBuffer(1), cDialog.m_strString.GetBuffer(0));
