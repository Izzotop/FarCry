...
bool CMaterial::LoadShader(const CString& shaderName) {
    ...
    if (!m_shaderItem.m_pShader) {
        ...
        GetIEditor()->GetErrorReport()->SetCurrentValidatorMaterial(nullptr);
        return false;
    }
...
namespace {
inline Vec3 ToVec3(const CFColor& col) {
...
void CMaterial::RemoveAllSubMaterials() {
    ...
    for (int i = 0; i < m_subMaterials.size(); i++) {
        m_subMaterials[i]->m_pParentMtl = nullptr;
    }
