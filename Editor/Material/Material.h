...
public:
    ...
    CString GetShaderName() const {
        return m_shaderName;
    };

    //! Sets one or more material flags from EMtlFlags enum.
    void SetMaterialFlags(int flags) {
        m_mtlFlags = flags;
    };
    ...
    SInputShaderResources& GetShaderResources() {
    ...
    unsigned int GetShaderGenMask() {
        return m_nShaderGenMask;
    }
    ...
    bool IsUsed() const {
        return m_bInUse || (m_mtlFlags & MF_ALWAYS_USED);
    };
