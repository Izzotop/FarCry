...
bool CPluginManager::LoadAllPlugins(CString strPath) {
    ...
    pfnCreatePluginInstance pfnFactory = nullptr;
    PLUGIN_INIT_PARAM sInitParam = {GetIEditor(), nullptr /*TODO: CEngineSingleton::GetGameInterface()*/};
    IPlugin* pIPlugin = nullptr;
    bool bReturn;
    uint8 iUIID = 0;
    ...
    if (cDLLFiles.StartEnumeration(strPath.GetBuffer(1), "*.DLL", &sFile)) {
        do {
            ...
            CLogFile::FormatLine("Successfully loaded plugin '%s' version '%i' (GUID: %s)", pIPlugin->GetPluginName(), pIPlugin->GetPluginVersion(), pIPlugin->GetPluginGUID());
