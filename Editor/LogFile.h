...
public:
    ...
    static void AttachListBox(HWND hWndListBox) {
        m_hWndListBox = hWndListBox;
    };
    static void AttachEditBox(HWND hWndEditBox) {
        m_hWndEditBox = hWndEditBox;
    };

    static void SetConsole(IConsole* c) {
        m_console = c;
        if (c) {
            m_pLogVerbosity = c->CreateVariable("log_Verbosity", "5", VF_DUMPTODISK);
        };
    };
