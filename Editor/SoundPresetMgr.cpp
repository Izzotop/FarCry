...
void CSoundPresetMgr::MakeTagUnique(XmlNodeRef pParent, XmlNodeRef pNode) {
    ...
    do {
        ...
        if ((pCheckNode != nullptr) && (pCheckNode != pNode)) {
...
bool CSoundPresetMgr::DumpTableRecursive(FILE* pFile, XmlNodeRef pNode, int nTabs) {
    ...
    if (pNode->getAttr("value", sValue)) {
        ...
        if (fprintf(pFile, "%s = \"%s\"", pNode->getTag(), (const char*)sValue) <= 0)
            return false;
    } else {
        if (fprintf(pFile, "%s = {\r\n", pNode->getTag()) <= 0)
            return false;
        for (int i = 0; i < pNode->getChildCount(); i++) {
            ...
            if (!DumpTableRecursive(pFile, pChild, nTabs + 1))
                return false;
        }
        for (int i = 0; i < nTabs; i++)
            if (fprintf(pFile, "\t") <= 0)
...
bool CSoundPresetMgr::Load(CString sFilename) {
    ...
    if (!pScriptSystem)
        return false;
    pScriptSystem->SetGlobalTonullptr("SoundPresetDB");
...
public:
    CSoundPresetUpdateSODump(IScriptSystem* pScriptSystem, _SmartScriptObject& pObj, CString& sPresetName, CString& sSoundName, CString& sPropName, CString& sPropValue,
                             bool bCorrectPreset = false, bool bCorrectSound = false)
        : m_pObj(pScriptSystem, pObj) {
        m_pScriptSystem = pScriptSystem;
