...
SBrushPoly* SBrush::CreateFacePoly(SBrushFace* face) {
    ...
    for (int i = 0; i < numFaces; i++) {
        ...
        if ((face->m_Plane.normal.Dot(clip->m_Plane.normal) > 0.999) && (fabs(face->m_Plane.dist - clip->m_Plane.dist) < DIST_EPSILON)) {
            if (past) {
                delete p;
                return nullptr;
            }
...
bool SBrush::BuildRenderElements() {
    ...
    if (m_indoorGeom && m_prefabGeom != nullptr) {
    ...
    CLeafBuffer* pLeafBuffer =
        renderer->CreateLeafBufferInitialized(&verts[0], verts.size(), VERTEX_FORMAT_P3F_TEX2F, &indices[0], indices.Count(), R_PRIMV_TRIANGLES, "Brush", eBT_Dynamic);
    assert(pLeafBuffer);
    ...
    mt.shaderItem = renderer->EF_LoadShaderItem("Default", eSH_Indoor, true, nullptr, 0, &rc);
  ...
  do
  {
...

    {
      ...
      if (!f->m_TexInfo.value)
        f->m_TexInfo.value = gcMapEf.mfGetVal(f->m_Shader, nullptr);
...
void SBrush::Select ()
{
  ...
  gCurMap->m_SelectedFace = nullptr;
...
glColor4f(1,1,1,1);
bool bRes = m->mfDraw(nullptr, true);
...
if (f->m_Prefabs)
{
  for (TPrefabItem *pi=f->m_Prefabs; pi; pi=pi->Next)
  {
    ...
    if (pi->m_Model)
      pi->m_Model->mfDraw(f->m_Shader, nullptr);
    else
      pi->m_Geom->mfDraw(f->m_Shader, nullptr);
    glPopMatrix();
  }
  continue;
}
...
if (obj)
  gRenDev->EF_AddEf(0, f->m_RE, f->m_Shader, obj, g_Globals.m_TemplateId, nullptr);
else
  gRenDev->EF_AddEf(0, f->m_RE, f->m_Shader, g_Globals.m_TemplateId, nullptr);
}
    */
}
        ...
        */
        ...
        if (f->m_RE) {
            ...
            */
            ...
            renderer->EF_AddEf(0, f->m_RE, pShader, nullptr, nullptr, -1);
        }
        // else
        // gRenDev->EF_AddEf(0, f->m_RE, f->m_Shader, g_Globals.m_TemplateId, nullptr);
    }
