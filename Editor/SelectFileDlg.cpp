...
CSelectFileDlg::CSelectFileDlg(CWnd* pParent /*=nullptr*/) : CDialog(CSelectFileDlg::IDD, pParent) {
...
void CSelectFileDlg::UpdatePreview(CString strFileToPreview) {
    ...
    if (strFileToPreview.IsEmpty()) {
        ...
        PathCompactPathEx(szCompactPath, strFileToPreview.GetBuffer(0), 33, nullptr);

        // Obtain file informations
        VERIFY(_stat(strFileToPreview.GetBuffer(0), &sStat) == 0);

        // Format the information string
        sprintf(szInfoText, "%s\nDim: %.2f X %.2f X %.2f\nFileSize: %i KB\n\nCreated: %sModified: %sAcessed: %s", szCompactPath, size.x, size.y, size.z, sStat.st_size / 1024,
                ctime(&sStat.st_ctime), ctime(&sStat.st_mtime), ctime(&sStat.st_atime));
...
BOOL CSelectFileDlg::OnInitDialog() {
    ...
    m_cFileTree.Create(nullptr, rc, this, nullptr);
    ...
    // folder
    if (pszFolder[0] != '\0' && m_strLastSearchFolder == m_cFileTree.GetSearchFolder() && m_cFileTree.ItemHasChildren(hItem)) {
        // Get the first child
        ...
        while (true) {
            // Is the current char a path separator or the nullptr character ?
...
bool CSelectFileDlg::SelectFileName(CString* pFileNameOut, CString* pRelativeFileNameOut, CString strFileSpec, CString strSearchFolder) {
    ////////////////////////////////////////////////////////////////////////
    ...
    m_previewModelEnabled = false;
    if (_stricmp(PathFindExtension(strFileSpec.GetBuffer(0)), ".cid") == 0 || _stricmp(PathFindExtension(strFileSpec.GetBuffer(0)), ".cgf") == 0) {
    ...
    if (pRelativeFileNameOut) {
        ...
        PathRelativePathTo(szRelativePath, GetIEditor()->GetMasterCDFolder(), FILE_ATTRIBUTE_DIRECTORY, m_strSelectedFile, nullptr);
...
void CSelectFileDlg::OnOK() {
    ...
    m_strLastPath = &m_strSelectedFile.GetBuffer(0)[strlen(GetIEditor()->GetMasterCDFolder()) + m_cFileTree.GetSearchFolder().GetLength() + 1];
