...
enum { CC_BLINK_NOCHANGE = 0, CC_BLINK_FAST = 500, CC_BLINK_NORMAL = 1000, CC_BLINK_SLOW = 2000 };

enum { CC_BLINK_TEXT = 1, CC_BLINK_BK = 2, CC_BLINK_BOTH = CC_BLINK_TEXT | CC_BLINK_BK };

#define CC_SYSCOLOR(ind) (0x80000000 | ((ind) & ~CLR_DEFAULT))

template <class BASE_TYPE = CWnd> class CColorCtrl : public BASE_TYPE {
public:
    CColorCtrl();
    virtual ~CColorCtrl() {}
...
public:
    ...
    void SetBlinkTimer() {
        ...
        if (m_nTimerID <= 0 && (m_fBlinkText || m_fBlinkBk))
            m_nTimerID = SetTimer(1, m_nDelay, nullptr);
    }
    void KillBlinkTimer() {
        // reset timer
...
template <class BASE_TYPE> CColorCtrl<BASE_TYPE>::CColorCtrl() {
...
template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::SetTextColor(COLORREF rgbText) {
    m_rgbText = rgbText;
    if (::IsWindow(m_hWnd))
        Invalidate();
}

template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::SetTextBlinkColors(COLORREF rgbBlinkText1, COLORREF rgbBlinkText2) {
    m_rgbBlinkText[0] = rgbBlinkText1;
    m_rgbBlinkText[1] = rgbBlinkText2;
}

template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::SetBkColor(COLORREF rgbBk) {
    m_rgbBk = rgbBk;
...
template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::SetBkBlinkColors(COLORREF rgbBlinkBk1, COLORREF rgbBlinkBk2) {
    m_rgbBlinkBk[0] = rgbBlinkBk1;
    m_rgbBlinkBk[1] = rgbBlinkBk2;
}

template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::SetBkPattern(UINT nBkID) {
...
template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::SetBkPattern(HBITMAP hbmpBk) {
    m_bmpBk.DeleteObject();
    if (hbmpBk != 0)
...
template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::SetBkBlinkPattern(UINT nBkID1, UINT nBkID2) {
...
template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::SetBkBlinkPattern(HBITMAP hbmpBk1, HBITMAP hbmpBk2) {
    m_bmpBlinkBk[0].DeleteObject();
    m_bmpBlinkBk[1].DeleteObject();
...
template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::UseSolidColors(BOOL fSolid) {
    m_fSolid = fSolid;
...
template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::StartBlink(int iWho, UINT nDelay) {
...
template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::StopBlink(int iWho) {
    if (iWho & CC_BLINK_TEXT)
        m_fBlinkText = FALSE;
    if (iWho & CC_BLINK_BK)
        m_fBlinkBk = FALSE;
...
template <class BASE_TYPE> HBRUSH CColorCtrl<BASE_TYPE>::CtlColor(CDC* pDC, UINT nCtlColor) {
    // Get foreground color
    COLORREF rgbText = GetColor(m_fBlinkText ? m_rgbBlinkText[m_iBlinkPhase] : m_rgbText);
    // Get background color
    COLORREF rgbBk = GetColor(m_fBlinkBk ? m_rgbBlinkBk[m_iBlinkPhase] : m_rgbBk);
...
template <class BASE_TYPE> HBRUSH CColorCtrl<BASE_TYPE>::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) {
...
template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::OnDestroy() {
    // kill timer
    KillBlinkTimer();
    BASE_TYPE::OnDestroy();
}

template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::OnTimer(UINT_PTR nIDEvent) {
...
template <class BASE_TYPE> void CColorCtrl<BASE_TYPE>::PreSubclassWindow() {
    // set timer if blinking mode
    SetBlinkTimer();
    BASE_TYPE::PreSubclassWindow();
}

template <class BASE_TYPE> int CColorCtrl<BASE_TYPE>::OnCreate(LPCREATESTRUCT lpCreateStruct) {
...
template <class BASE_TYPE, int InitialTextColor = CLR_DEFAULT, int InitialBkColor = CLR_DEFAULT> class CColorCtrlEx : public CColorCtrl<BASE_TYPE> {
public:
    CColorCtrlEx() {
...
template <class BASE_TYPE, int InitialTextColor0 = CLR_DEFAULT, int InitialTextColor1 = CLR_DEFAULT, int InitialBkColor0 = CLR_DEFAULT, int InitialBkColor1 = CLR_DEFAULT,
...
template <class BASE_TYPE> class CColorPushButton : public BASE_TYPE {
public:
    CColorPushButton();
    ~CColorPushButton();
...
public:
    void SetNoFocus(bool fNoFocus = true) {
        m_fNoFocus = fNoFocus;
    }
    void SetNoDisabledState(bool fNoDisabledState = true) {
        m_fNoDisabledState = fNoDisabledState;
    }
    void SetPushedBkColor(COLORREF rgbBk = CLR_DEFAULT);
    COLORREF GetPushedBkColor() {
        return GetColor(m_rgbBk);
    }
...
template <class BASE_TYPE> CColorPushButton<BASE_TYPE>::CColorPushButton() {
    m_fNoFocus = true;
...
template <class BASE_TYPE> void CColorPushButton<BASE_TYPE>::SetToolTip(const char* tooltipText) {
    if (!m_tooltip) {
...
template <class BASE_TYPE> void CColorPushButton<BASE_TYPE>::SetIcon(LPCSTR lpszIconResource) {
    if (m_hIcon)
        DestroyIcon(m_hIcon);
    SetIcon((HICON)::LoadImage(AfxGetInstanceHandle(), lpszIconResource, IMAGE_ICON, 0, 0, 0));
}

template <class BASE_TYPE> void CColorPushButton<BASE_TYPE>::SetIcon(HICON hIcon) {
    m_hIcon = hIcon;
...
template <class BASE_TYPE> void CColorPushButton<BASE_TYPE>::SetPushedBkColor(COLORREF rgbBk) {
    m_rgbPushedBk = rgbBk;
...
template <class BASE_TYPE> BOOL CColorPushButton<BASE_TYPE>::PreTranslateMessage(MSG* pMsg) {
...
template <class BASE_TYPE> void CColorPushButton<BASE_TYPE>::PreSubclassWindow() {
    ChangeStyle();
    BASE_TYPE::PreSubclassWindow();
}

template <class BASE_TYPE> int CColorPushButton<BASE_TYPE>::OnCreate(LPCREATESTRUCT lpCreateStruct) {
    if (BASE_TYPE::OnCreate(lpCreateStruct) == -1)
        return -1;
    ChangeStyle();
    return 0;
...
template <class BASE_TYPE> void CColorPushButton<BASE_TYPE>::ChangeStyle() {
    CString sClassName;
    ...
    if ((style & 0xf) == BS_PUSHBUTTON || (style & 0xf) == BS_DEFPUSHBUTTON) {
        ModifyStyle(0xf, BS_OWNERDRAW);
        Invalidate(FALSE);
    }
}

template <class BASE_TYPE> void CColorPushButton<BASE_TYPE>::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) {
