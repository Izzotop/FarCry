...
CTreeCtrlEx::CTreeCtrlEx() {
    ...
    m_hitemDrag = nullptr;
    m_hitemDrop = nullptr;
    m_dropCursor = LoadCursor(nullptr, IDC_ARROW);
    m_noDropCursor = LoadCursor(nullptr, IDC_NO);
}
...
HTREEITEM CTreeCtrlEx::CopyItem(HTREEITEM hItem, HTREEITEM htiNewParent, HTREEITEM htiAfter /*= TVI_LAST*/) {
    ...
    SetItemState(hNewItem, GetItemState(hItem, TVIS_STATEIMAGEMASK | TVIS_EXPANDED), TVIS_STATEIMAGEMASK | TVIS_EXPANDED);
...
void CTreeCtrlEx::OnBeginDrag(NMHDR* pNMHDR, LRESULT* pResult) {
    ...
    *pResult = 0;
    ...
    // CreateDragImage() returns nullptr if no image list
    ...
    */
    ...
    ClientToScreen(&pt);
    m_pDragImage->DragEnter(nullptr, pt);
    SetCapture();
}
...
BOOL CTreeCtrlEx::PreTranslateMessage(MSG* pMsg) {
    if (pMsg->message == WM_KEYDOWN) {
        ...
        case VK_RETURN:
            ...
            // if(SetItemText(m_CurItem.p_Item, m_Test) == nullptr)
...
HTREEITEM CTreeCtrlEx::GetDropTarget(HTREEITEM hItem) {
    if (hItem != m_hitemDrag && hItem != GetParentItem(m_hitemDrag)) {
        ...
        while ((htiParent = GetParentItem(htiParent)) != nullptr) {
...
HTREEITEM CTreeCtrlEx::FindNextItem(TV_ITEM* pItem, HTREEITEM hItem) {
    ...
    // TVIF_TEXT bit but passing in a nullptr pointer.
...
BOOL CTreeCtrlEx::CompareItems(TV_ITEM* pItem, TV_ITEM& tvTempItem) {
    ...
    if ((pItem->mask & TVIF_TEXT) && pItem->pszText && tvTempItem.pszText && // Don't compare if either is nullptr
