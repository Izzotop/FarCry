...
BOOL CMultiTree::SetItemState(HTREEITEM hItem, UINT nState, UINT nStateMask) {
    ...
    if (nStateMask & TVIS_FOCUSED) {
        ...
        if (nState & TVIS_FOCUSED) {
            ...
            if (!bWasFocus && bFocusWasSel) {
                ...
                CMultiTree_BASE::SelectItem(nullptr); // will cause notify, but can be taken as focus change
            ...
            if (bWasFocus) {
                // it had the focus
                CMultiTree_BASE::SelectItem(nullptr); // clear focus
                if (!(nStateMask & TVIS_SELECTED)) {
                    // didn't want to affect select state
                    if (bWasSel) {
    ...
    if (nSM & TVIS_SELECTED) {
        ...
        nmtv.itemOld.hItem = nmtv.itemNew.hItem = nullptr;
...
void CMultiTree::DoPreSelection(HTREEITEM hItem, BOOL bLeft, UINT nFlags) {
    if (bLeft) {
        ...
        if (_bShift) {
            ...
            m_hSelect = nullptr; // reset when a non-shift operation occurs
        }
        return;
    }
...
void CMultiTree::DoAction(HTREEITEM hItem, BOOL bLeft, UINT nFlags, CPoint point) {
    ...
    while (!nDone && ::GetMessage(&msg, nullptr, 0, 0)) {
...
void CMultiTree::DoBanding(BOOL bLeft, UINT nFlags, CPoint point) {
    ...
    UINT nTimer = SetTimer(1, MST_TIMER_PERIOD, nullptr); // for scroll
    ...
    while (!bDone && ::GetMessage(&msg, nullptr, 0, 0)) {
        ...
        case WM_MOUSEMOVE:
            ...
            if (!bDrag) {
                ...
                dc.DrawDragRect(rect, sizeEdge, nullptr, sizeEdge);
            }

            dc.DrawDragRect(rect, sizeEdge, nullptr, sizeEdge); // delete
            ...
            dc.DrawDragRect(rect, sizeEdge, nullptr, sizeEdge); // draw
            break;
    ...
    *pResult = 0;
    if ((pNMTreeView->action == TVE_COLLAPSE) || (pNMTreeView->action == TVE_COLLAPSERESET)) {
        ...
        if (bWasFocus && !bWasSel)
            CMultiTree_BASE::SelectItem(nullptr); // stop parent from gaining selection
    }
