...
HBITMAP LoadColorBitmap(LPCTSTR lpszResourceName, HMODULE hInst, int* pNumcol = nullptr);
...
public:
    ...
    virtual BOOL LoadBitmap(int nWidth, int nHeight, LPCTSTR lpszResourceName, HMODULE hInst = nullptr);
...
public:
    ...
    BOOL LoadToolBar(LPCTSTR lpszResourceName, HMODULE hInst = nullptr);
    BOOL LoadToolBar(UINT nToolBar, HMODULE hInst = nullptr);
    BOOL LoadToolBars(const UINT* arID, int n, HMODULE hInst = nullptr);
    ...
    BOOL AppendMenu(UINT nFlags, UINT nIDNewItem = 0, LPCTSTR lpszNewItem = nullptr, int nIconNormal = -1);
    ...
    BOOL InsertMenu(UINT nPosition, UINT nFlags, UINT nIDNewItem = 0, LPCTSTR lpszNewItem = nullptr, int nIconNormal = -1);
...
protected:
    ...
    CNewMenuIcons* GetToolbarIcons(UINT nToolBar, HMODULE hInst = nullptr);

    DWORD SetMenuIcons(CNewMenuIcons* pMenuIcons);
    BOOL Replace(UINT nID, UINT nNewID);

    static BOOL IsNewShell();
    BOOL IsMenuBar(HMENU hMenu = nullptr);
...
template <class baseClass> class CNewFrame : public baseClass {
...
protected:
    ...
    static const AFX_MSGMAP_ENTRY* GetMessageEntries() {
        static const AFX_MSGMAP_ENTRY Entries[] = {ON_WM_MEASUREITEM() ON_WM_MENUCHAR() ON_WM_INITMENUPOPUP() ON_WM_ENTERMENULOOP() ON_WM_EXITMENULOOP() ON_WM_TIMER()
                                                       ON_WM_CREATE() ON_WM_NCHITTEST() ON_WM_DESTROY() ON_WM_SYSCOLORCHANGE(){0, 0, 0, 0, AfxSig_end, (AFX_PMSG)0}};
        return Entries;
    }
    ...
    afx_msg void OnEnterMenuLoop(BOOL bIsTrackPopupMenu) {
        ...
        if (m_menubarItemIndex != UINT(-1)) {
            DrawMenubarItem(this, GetMenu(), m_menubarItemIndex, nullptr);
            m_menubarItemIndex = UINT(-1);
        }
                ...
                if (pNewMenu) {
                    ...
                    if (nItemIndex != (UINT)-1) {
                        if (m_menubarItemIndex != nItemIndex) {
                            ...
                            if (CNewMenu::m_dwLastActiveItem == nullptr && DrawMenubarItem(this, pNewMenu, nItemIndex, ODS_HOTLIGHT | nStatus)) {
                                // Set a new Timer
                                if (m_TimerID == nullptr) {
                                    m_TimerID = GetSafeTimerID(m_hWnd, 100);
                                }
...
#ifdef _AFXDLL
#if _MFC_VER < 0x0700
template <class baseClass> const AFX_MSGMAP CNewFrame<baseClass>::messageMap = {&CNewFrame<baseClass>::_GetBaseMessageMap, GetMessageEntries()};
#else
template <class baseClass> const AFX_MSGMAP CNewFrame<baseClass>::messageMap = {&baseClass::GetThisMessageMap, GetMessageEntries()};
#endif
#else
template <class baseClass> const AFX_MSGMAP CNewFrame<baseClass>::messageMap = {&baseClass::messageMap, GetMessageEntries()};
#endif
...
// control bar docking
template <class baseClass> void CNewFrame<baseClass>::EnableDocking(DWORD dwDockStyle) {
...
public:
    CNewDialog();
    CNewDialog(LPCTSTR lpszTemplateName, CWnd* pParentWnd = nullptr);
    CNewDialog(UINT nIDTemplate, CWnd* pParentWnd = nullptr);
...
public:
    ...
    // loadframe
    BOOL Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle = WS_OVERLAPPEDWINDOW, const RECT& rect = rectDefault,
                CWnd* pParentWnd = nullptr, // != nullptr for popups
                LPCTSTR lpszMenuName = nullptr, DWORD dwExStyle = 0, CCreateContext* pContext = nullptr);
};
