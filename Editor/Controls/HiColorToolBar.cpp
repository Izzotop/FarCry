...
void CHiColorToolBar::MakeDisabled(CBitmap& ioBM) {
    ...
    for (UINT i = 0; i < numPixels; ++i) {
        if (pixels[i].rgbtBlue == userBackgroundColor.rgbtBlue && pixels[i].rgbtGreen == userBackgroundColor.rgbtGreen && pixels[i].rgbtRed == userBackgroundColor.rgbtRed) {
...
void CHiColorToolBar::MakeToolbarImageList(UINT inBitmapID, CImageList& outImageList, LoadType lt) {
    ...
    VERIFY(bm.Attach(::LoadImage(::AfxFindResourceHandle(MAKEINTRESOURCE(inBitmapID), RT_BITMAP), MAKEINTRESOURCE(inBitmapID), IMAGE_BITMAP, 0, 0,
...
void CHiColorToolBar::AddTextStrings() {
    ...
    for (nIndex = bar.GetButtonCount() - 1; nIndex >= 0; nIndex--) {
        ...
        CString strText;
        LPCTSTR lpszButtonText = nullptr;
        ...
        if (!strText.IsEmpty()) {
            ...
            while (lpszButtonText) {
                ...
                lpszButtonText = _tcstok(nullptr, seps);
            }
        }
...
bool CHiColorToolBar::CreateAll(UINT nIDRessource, UINT inNormalImageID, CWnd* pwndParent) {
    ...
    if (!CreateEx(pwndParent, TBSTYLE_FLAT | TBSTYLE_WRAPABLE, WS_CHILD | CBRS_TOP | CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC)) {
        return false;
    }
    ...
    GetToolBarCtrl().SetRows(99, TRUE, nullptr);
