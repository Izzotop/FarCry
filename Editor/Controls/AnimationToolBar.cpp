...
void CAnimationToolBar::OnSize(UINT nType, int cx, int cy) {
    ...
    if (m_cKeyframes.m_hWnd) {
        m_cKeyframes.SetWindowPos(nullptr, 0, 0, (rcToolbar.right - rcCtrl.left) / 2, 18, SWP_NOMOVE | SWP_NOOWNERZORDER);
    }
    ...
    if (m_cAnimSequences.m_hWnd) {
        m_cAnimSequences.SetWindowPos(nullptr, 0, 0, (rcToolbar.right - rcCtrl.left) / 2, 18, SWP_NOMOVE | SWP_NOOWNERZORDER);
        ...
        ::SetWindowPos(m_cAnimSequences.m_hWnd, nullptr, rcCtrl.left, rcCtrl.top, rcCtrl.right, rcCtrl.bottom, SWP_NOZORDER);
    }
}
...
int CAnimationToolBar::OnCreate(LPCREATESTRUCT lpCreateStruct) {
    ...
    m_cKeyframes.Create(WS_CHILD | WS_VISIBLE | TBS_HORZ | TBS_BOTTOM | TBS_AUTOTICKS, rect, this, nullptr);
    ...
    // TODO: Remove WS_DISABLED when animation is implemented
    m_cAnimSequences.Create(WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST, rect, this, nullptr);
