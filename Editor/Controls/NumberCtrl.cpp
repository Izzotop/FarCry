...
void CNumberCtrl::Create(CWnd* parentWnd, CRect& rc, UINT nID, int nFlags) {
    ...
    CreateEx(0, AfxRegisterWndClass(nullptr, arrowCursor, nullptr /*(HBRUSH)GetStockObject(LTGRAY_BRUSH)*/), nullptr, WS_CHILD | WS_VISIBLE | WS_TABSTOP, rc, parentWnd, nID);
}
...
void CNumberCtrl::Create(CWnd* parentWnd, UINT ctrlID, int flags) {
    ...
    HCURSOR arrowCursor = AfxGetApp()->LoadStandardCursor(IDC_ARROW);
    CreateEx(0, AfxRegisterWndClass(nullptr, arrowCursor, nullptr /*(HBRUSH)GetStockObject(LTGRAY_BRUSH)*/), nullptr, WS_CHILD | WS_VISIBLE | WS_TABSTOP, rc, parentWnd, ctrlID);
}
...
int CNumberCtrl::CalculateDecimalPlaces(float infNumber, int iniMaxPlaces) {
    ...
    if (*_str != 0) // comma?
    {
        ...
        if (ret > iniMaxPlaces)
            ret = iniMaxPlaces; // bound to maximum
    }
