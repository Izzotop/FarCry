...
void CPropertyCtrl::RegisterWindowClass() {
    ...
    memset(&wndcls, 0, sizeof(WNDCLASS)); // start with nullptr
    // defaults
...
void CPropertyCtrl::OnRButtonUp(UINT nFlags, CPoint point) {
    ...
    CPoint p;
    ::GetCursorPos(&p);
    int res = ::TrackPopupMenuEx(menu.GetSafeHmenu(), TPM_LEFTBUTTON | TPM_RETURNCMD, p.x, p.y, GetSafeHwnd(), nullptr);
    switch (res) {
    case 1:
        OnCopy(false);
        break;
...
void CPropertyCtrl::DrawItem(CPropertyItem* item, CDC& dc, CRect& itemRect) {
    ...
    if (!m_pBoldFont) {
        ...
        m_pBoldFont->CreateFont(height, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH,
                                "MS Sans Serif");
    }
...
void CPropertyCtrl::SelectItem(CPropertyItem* item) {
    ...
    if (m_selected) {
        ...
        if (m_selChangeFunc != nullptr && m_bEnableSelChangeCallback) {
...
void CPropertyCtrl::Init() {
    ...
    if (!m_nTimer) {
        m_nTimer = SetTimer(KILLFOCUS_TIMER, 500, nullptr);
    }
}
...
void CPropertyCtrl::OnPaste() {
    ...
    if (rootNode != nullptr && rootNode->isTag("PropertyCtrl")) {
