...
#define IS_INTRESOURCE(hh) (HIWORD(hh) == nullptr)
#endif
...
static void ShowLastError() {
    ...
    if (error) {
        ...
        if (lpMsgBuf) {
            ...
            // Display the string.
            MessageBox(nullptr, temp, _T("Error"), MB_OK | MB_ICONINFORMATION);
        }
    }
}

UINT GetSafeTimerID(HWND hWnd, const UINT uiElapse) {
    UINT nNewTimerId = nullptr;
    if (IsWindow(hWnd)) {                                                             // Try to start with TimerID 31, normaly is used 1, 10, or 100
        for (UINT nTimerId = 31; nTimerId < 100 && nNewTimerId == nullptr; nTimerId++) { // Try to set a timer each uiElapse ms
            nNewTimerId = (UINT)::SetTimer(hWnd, nTimerId, uiElapse, nullptr);
        }
    ...
    if (nItemIndex != UINT(-1) && GetMenuItemRect(pWnd->m_hWnd, pMenu->m_hMenu, nItemIndex, &itemRect)) {
        ...
        if (pMenu->GetMenuItemInfo(nItemIndex, &menuInfo, TRUE)) {
            ...
            if (menuInfo.fType & MF_OWNERDRAW) {
                ...
                SendMessage(pWnd->GetSafeHwnd(), WM_DRAWITEM, nullptr, (LPARAM)&drwItem);
...
COLORREF DarkenColorXP(COLORREF ColA) {
    return RGB(MulDiv(GetRValue(ColA), 7, 10), MulDiv(GetGValue(ColA), 7, 10), MulDiv(GetBValue(ColA) + 55, 7, 10));
}
...
FktGradientFill pGradientFill = nullptr;

void DrawGradient(CDC* pDC, CRect& Rect, COLORREF StartColor, COLORREF EndColor, BOOL bHorizontal) {
...
public:
    ...
    CMenuTheme(DWORD dwThemeId, pItemMeasureFkt pMeasureItem, pItemDrawFkt pDrawItem, pItemDrawFkt pDrawTitle, DWORD dwFlags = 0);
...
public:
    CMenuThemeXP(DWORD dwThemeId, pItemMeasureFkt pMeasureItem, pItemDrawFkt pDrawItem, pItemDrawFkt pDrawTitle, DWORD dwFlags = 0);
...
public:
    class CMenuHookData {
    public:
        CMenuHookData(HWND hWnd, BOOL bSpecialWnd) : m_dwData(bSpecialWnd), m_bDrawBorder(TRUE), m_Point(0, 0) {
            // Safe actual menu
            m_hMenu = CNewMenuHook::m_hLastMenu;
            // Reset for the next menu
            CNewMenuHook::m_hLastMenu = nullptr;
...
CNewMenuIcons::CNewMenuIcons() : m_lpszResourceName(nullptr), m_hInst(nullptr), m_nColors(0), m_crTransparent(CLR_NONE), m_dwRefCount(0) {}
...
BOOL CNewMenuIcons::DoMatch(WORD* pIconInfo, COLORREF crTransparent) {
    if (m_crTransparent == crTransparent && pIconInfo != nullptr) {
        CNewMenuIconInfo* pInfo = (CNewMenuIconInfo*)pIconInfo;

        // Check for the same resource ID
        if (pInfo->wBitmapID && IS_INTRESOURCE(m_lpszResourceName) && ((UINT)(UINT_PTR)m_lpszResourceName) == pInfo->wBitmapID) {
...
BOOL CNewMenuIcons::LoadToolBar(LPCTSTR lpszResourceName, HMODULE hInst) {
    ...
    if (hRsrc == nullptr) { // Special purpose when you try to load it from a dll 30.05.2002
...
int CNewMenuBitmaps::Add(UINT nID, COLORREF crTransparent) {
    ...
    if (hBitmap != nullptr) {
        ...
        if (m_IconsList.GetSafeHandle() == nullptr) {
...
CNewMenuItemData::CNewMenuItemData() : m_nTitleFlags(0), m_nFlags(0), m_nID(0), m_nSyncFlag(0), m_pData(nullptr), m_pMenuIcon(nullptr), m_nMenuIconOffset(-1) {}

CNewMenuItemData::~CNewMenuItemData() {
    // it's a safe release. Do not care for nullptr pointers.
    m_pMenuIcon->Release();
}
...
CMenuTheme* CNewMenu::m_pActMenuDrawing = nullptr;
CTypedPtrList<CPtrList, CNewMenuIcons*>* CNewMenu::m_pSharedMenuIcons = nullptr;

// Gloabal logfont for all menutitles
LOGFONT CNewMenu::m_MenuTitleFont = {16, 0, 0, 0, FW_BOLD, 0, 0, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, _T("Arial")};
...
DWORD CNewMenu::m_dwLastActiveItem = nullptr;
...
CNewMenu::CNewMenu(HMENU hParent) : m_hParentMenu(hParent), m_bIsPopupMenu(TRUE), m_dwOpenMenu(nullptr), m_LastActiveMenuRect(0, 0, 0, 0), m_pData(nullptr) {
...
void CNewMenu::DrawItem_WinXP(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar) {
    ...
    if (state & MF_SEPARATOR) {
        ...
        if (!strText.IsEmpty()) {
            ...
            if ((lpDIS->itemState & ODS_GRAYED) || (bIsMenuBar && lpDIS->itemState & ODS_INACTIVE)) {
                ...
                if (checkflag && m_checkmaps) {
                    ...
                    if (state & ODS_DISABLED) {
                        if (m_bEnableXpBlendig) {
                            ...
                            pDC->DrawState(ptImage, size, hDrawIcon2, DSS_NORMAL, (HBRUSH)nullptr);
            ...
            if (pMenuData->m_nMenuIconOffset < 0 && state & ODS_CHECKED && !checkflag) {
                ...
                if (state & ODS_CHECKED || info.hbmpUnchecked) {
                    ...
                    if (state & ODS_CHECKED) {
                        ...
                        if (state & ODS_DISABLED)
                            pDC->DrawState(ptImage, size, info.hbmpUnchecked, DSS_DISABLED, (HBRUSH)nullptr);
                        else
                            pDC->DrawState(ptImage, size, info.hbmpUnchecked, DSS_NORMAL, (HBRUSH)nullptr);
                    }
                }
...
void CNewMenu::MeasureItem_OldStyle(LPMEASUREITEMSTRUCT lpMIS, BOOL bIsMenuBar) {
    ...
    if (state & MF_SEPARATOR) {
        ...
        if (_tcsstr(lpstrText, _T("\t")) != nullptr)
            lpMIS->itemWidth += 15;
...
BOOL CNewMenu::AppendODMenu(LPCTSTR lpstrText, UINT nFlags, UINT nID, int nIconNormal) {
    return CNewMenu::AppendODMenu(lpstrText, nFlags, nID, (CImageList*)nullptr, nIconNormal);
}

BOOL CNewMenu::AppendODMenu(LPCTSTR lpstrText, UINT nFlags, UINT nID, CBitmap* pBmp) {
    int nIndex = -1;
    CNewMenuIconLock iconLock(GetMenuIcon(nIndex, pBmp));
    return AppendODMenu(lpstrText, nFlags, nID, iconLock, nIndex);
}

BOOL CNewMenu::AppendODMenu(LPCTSTR lpstrText, UINT nFlags, UINT nID, CImageList* pil, int xoffset) {
    int nIndex = 0;
...
BOOL CNewMenu::AppendODMenu(LPCTSTR lpstrText, UINT nFlags, UINT nID, CNewMenuIcons* pIcons, int nIndex) {
...
BOOL CNewMenu::InsertODMenu(UINT nPosition, LPCTSTR lpstrText, UINT nFlags, UINT nID, CBitmap* pBmp) {
    int nIndex = -1;
    CNewMenuIconLock iconLock(GetMenuIcon(nIndex, pBmp));
    return InsertODMenu(nPosition, lpstrText, nFlags, nID, iconLock, nIndex);
}

BOOL CNewMenu::InsertODMenu(UINT nPosition, LPCTSTR lpstrText, UINT nFlags, UINT nID, CImageList* pil, int xoffset) {
    int nIndex = -1;
    CNewMenuIconLock iconLock(GetMenuIcon(nIndex, nID, pil, xoffset));
    return InsertODMenu(nPosition, lpstrText, nFlags, nID, iconLock, nIndex);
}
...
BOOL CNewMenu::ModifyODMenu(LPCTSTR lpstrText, UINT nID, CBitmap* bmp) {
    ...
    return ModifyODMenu(lpstrText, nID, (CImageList*)nullptr, 0);
}
...
BOOL CNewMenu::ModifyODMenu(LPCTSTR lpstrText, LPCTSTR OptionText, int nIconNormal) {
    ...
    if (pOptionMenu != nullptr && nIndex >= 0) {
...
CNewMenuItemData* CNewMenu::FindMenuItem(UINT nID) {
    CNewMenuItemData* pData = nullptr;
    int i;
...
CNewMenu* CNewMenu::FindAnotherMenuOption(int nId, int& nLoc, CArray<CNewMenu*, CNewMenu*>& newSubs, CArray<int, int&>& newLocs) {
    int i, numsubs, j;
...
CNewMenu* CNewMenu::FindMenuOption(LPCTSTR lpstrText, int& nLoc) {
    ...
    for (i = 0; i <= (int)m_MenuItemList.GetUpperBound(); ++i) {
        if (m_MenuItemList[i]->m_szMenuText.Compare(lpstrText) == nullptr) {
            nLoc = i;
            return this;
        }
    }
...
void CNewMenu::SynchronizeMenu() {
    ...
    for (j = 0; j < GetMenuItemCount(); ++j) {
        ...
        if (state & MF_POPUP) {
            ...
            if (pSubMenu && pSubMenu->m_hParentMenu != m_hMenu) { // Sets again the parent to this one
                pSubMenu->m_hParentMenu = m_hMenu;
            }
...
BOOL CNewMenu::OnUnInitPopupMenu(HWND hWnd) {
    ...
    if (pParent) {
        pParent->m_dwOpenMenu -= 1;
        if (pParent->m_dwOpenMenu >= nullptr && !pParent->m_bIsPopupMenu) {
    ...
    if (MF_BYPOSITION & nFlags) {
        ...
        if ((nItemState & MF_SEPARATOR) && !(nItemState & MF_POPUP)) {
            ...
            if (nullptr == pSubMenu) {
                ...
                if (bCanDelete) { // Oh yes so we can destroy it
                    delete pSubMenu;
                }
            }
        }
...
//{
//   if (il == nullptr||newlist==nullptr||xoffset<0)
//     return FALSE;
//
...
int CNewMenu::GetMenuPosition(LPCTSTR pText) {
    for (int i = 0; i < (int)(GetMenuItemCount()); ++i) {
        if (!GetSubMenu(i)) {
            for (int j = 0; j <= m_MenuItemList.GetUpperBound(); ++j) {
                if (m_MenuItemList[j]->m_szMenuText.Compare(pText) == nullptr)
                    return j;
            }
        }
    }
...
void CNewMenu::ColorBitmap(CDC* pDC, CBitmap& bmp, CSize size, COLORREF fill, COLORREF border, int hatchstyle) {
    ...
    if (nullptr != pOldBrush) {
        MemDC.SelectObject(pOldBrush);
    }
    if (nullptr != pOldPen) {
        MemDC.SelectObject(pOldPen);
    }
    if (nullptr != pOldBitmap) {
...
CMenuTheme::CMenuTheme(DWORD dwThemeId, pItemMeasureFkt pMeasureItem, pItemDrawFkt pDrawItem, pItemDrawFkt pDrawTitle, DWORD dwFlags)
    : m_dwThemeId(dwThemeId), m_dwFlags(dwFlags), m_pMeasureItem(pMeasureItem), m_pDrawItem(pDrawItem), m_pDrawTitle(pDrawTitle) {
...
CMenuThemeXP::CMenuThemeXP(DWORD dwThemeId, pItemMeasureFkt pMeasureItem, pItemDrawFkt pDrawItem, pItemDrawFkt pDrawTitle, DWORD dwFlags)
...
HMODULE CNewMenuHook::m_hLibrary = nullptr;
HHOOK CNewMenuHook::HookOldMenuCbtFilter = nullptr;
HMENU CNewMenuHook::m_hLastMenu = nullptr;
DWORD CNewMenuHook::m_dwMsgPos = 0;

CTypedPtrList<CPtrList, CMenuTheme*>* CNewMenuHook::m_pRegisteredThemesList = nullptr;
...
CNewMenuHook::CNewMenuHook() {
    ...
    AddTheme(new CMenuTheme(CNewMenu::STYLE_ORIGINAL, CNewMenu::MeasureItem_OldStyle, CNewMenu::DrawItem_OldStyle, CNewMenu::DrawMenuTitle, TRUE));

    AddTheme(new CMenuTheme(CNewMenu::STYLE_ORIGINAL_NOBORDER, CNewMenu::MeasureItem_OldStyle, CNewMenu::DrawItem_OldStyle, CNewMenu::DrawMenuTitle));

    AddTheme(new CMenuThemeXP(CNewMenu::STYLE_XP, CNewMenu::MeasureItem_WinXP, CNewMenu::DrawItem_WinXP, CNewMenu::DrawMenuTitle, TRUE));

    AddTheme(new CMenuThemeXP(CNewMenu::STYLE_XP_NOBORDER, CNewMenu::MeasureItem_WinXP, CNewMenu::DrawItem_WinXP, CNewMenu::DrawMenuTitle));

    AddTheme(new CMenuTheme(CNewMenu::STYLE_SPECIAL, CNewMenu::MeasureItem_OldStyle, CNewMenu::DrawItem_SpecialStyle, CNewMenu::DrawMenuTitle, TRUE));

    AddTheme(new CMenuTheme(CNewMenu::STYLE_SPECIAL_NOBORDER, CNewMenu::MeasureItem_OldStyle, CNewMenu::DrawItem_SpecialStyle, CNewMenu::DrawMenuTitle));
    ...
    if (HookOldMenuCbtFilter == nullptr) {
        HookOldMenuCbtFilter = ::SetWindowsHookEx(WH_CALLWNDPROC, NewMenuHook, nullptr, ::GetCurrentThreadId());
        if (HookOldMenuCbtFilter == nullptr) {
            ShowLastError();
...
CNewMenuHook::~CNewMenuHook() {
    ...
    if (CNewMenu::m_pSharedMenuIcons) {
        ...
        CNewMenu::m_pSharedMenuIcons = nullptr;
    }
}
...
CMenuTheme* CNewMenuHook::RemoveTheme(DWORD dwThemeId) {
    ...
    if (pos) {
        ...
        if (m_pRegisteredThemesList->GetCount() == nullptr) {
    ...
    case WM_PRINT: {
        ...
        if (pData && CNewMenu::m_pActMenuDrawing->DoDrawBorder()) {
            ...
            if (CNewMenu::m_pActMenuDrawing->OnCalcFrameRect(hWnd, rectClient)) {
                ...
                SelectClipRgn(dc.m_hDC, nullptr);
                dc.IntersectClipRect(clipRect);
...
BOOL CNewMenuHook::CheckSubclassing(HWND hWnd, BOOL bSpecialWnd) {
    ...
    if (pData == nullptr) {
        ...
        if (oldWndProc != nullptr && GetProp(hWnd, _OldMenuProc) == nullptr) {
    ...
    if (!bSysMenu) {
        ...
        for (state.m_nIndex = 0; state.m_nIndex < state.m_nIndexMax; state.m_nIndex++) {
            ...
            if (state.m_nID == (UINT)-1) {
                ...
                if (state.m_pSubMenu == nullptr || (state.m_nID = state.m_pSubMenu->GetMenuItemID(0)) == 0 || state.m_nID == (UINT)-1) {
...
BOOL CNewFrameWnd::LoadFrame(UINT nIDResource, DWORD dwDefaultStyle, CWnd* pParentWnd, CCreateContext* pContext) {
    // only do this once
...
BOOL CNewFrameWnd::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, LPCTSTR lpszMenuName, DWORD dwExStyle,
                          CCreateContext* pContext) {
    if (lpszMenuName != nullptr) {
        m_DefaultNewMenu.LoadMenu(lpszMenuName);
        // load in a menu that will get destroyed when window gets destroyed
        if (m_DefaultNewMenu.m_hMenu == nullptr) {
#if _MFC_VER < 0x0700
    ...
    if (!CreateEx(dwExStyle, lpszClassName, lpszWindowName, dwStyle, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, pParentWnd->GetSafeHwnd(),
                  m_DefaultNewMenu.m_hMenu, (LPVOID)pContext)) {
#if _MFC_VER < 0x0700
...
BOOL CNewMDIFrameWnd::LoadFrame(UINT nIDResource, DWORD dwDefaultStyle, CWnd* pParentWnd, CCreateContext* pContext) {
    // only do this once
    ASSERT_VALID_IDR(nIDResource);
    ASSERT(m_nIDHelp == 0 || m_nIDHelp == nIDResource);
    ...
    ASSERT(m_hWnd != nullptr);
    m_hMenuDefault = ::GetMenu(m_hWnd);
    if (m_hMenuDefault == nullptr)
        TRACE0("Warning: CMDIFrameWnd without a default menu.\n");
    return TRUE;
}
#endif

BOOL CNewMDIFrameWnd::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, LPCTSTR lpszMenuName, DWORD dwExStyle,
                             CCreateContext* pContext) {
    if (lpszMenuName != nullptr) {
        m_DefaultNewMenu.LoadMenu(lpszMenuName);
        // load in a menu that will get destroyed when window gets destroyed
...
CNewMultiDocTemplate::CNewMultiDocTemplate(UINT nIDResource, CRuntimeClass* pDocClass, CRuntimeClass* pFrameClass, CRuntimeClass* pViewClass)
