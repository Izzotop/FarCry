...
CRollupCtrl::CRollupCtrl() {
    ...
    m_strMyClass = AfxRegisterWndClass(CS_VREDRAW | CS_HREDRAW, (HCURSOR)::LoadCursor(nullptr, IDC_ARROW), (HBRUSH)brush.GetSafeHandle(),
                                       //
...
int CRollupCtrl::_InsertPage(LPCTSTR caption, CDialog* pwndTemplate, int idx, BOOL bAutoDestroyTpl) {
    ASSERT(pwndTemplate != nullptr);
    ASSERT(pwndTemplate->m_hWnd != nullptr);
    ...
    if (idx < 0) {
        ...
        m_PageList.insert(m_PageList.begin() + idx, pi);
        newidx = idx;
    }
...
int CRollupCtrl::MovePageAt(int id, int newidx) {
    ...
    if (newidx > 0 && newidx >= m_PageList.size())
        newidx = -1;
    ...
    if (newidx < 0) {
        ...
        m_PageList.insert(m_PageList.begin() + newidx, pi);
        retidx = newidx;
    }
...
void CRollupCtrl::RecalLayout() {
    ...
    for (int i = 0; i < m_PageList.size(); i++) {
        ...
        if (pi->bExpanded && pi->bEnable) {
            ...
            DeferWindowPos(hdwp, pi->pwndGroupBox->m_hWnd, 0, 2, posy, r.Width() - 3 - RC_SCROLLBARWIDTH, tr.Height() + RC_PGBUTTONHEIGHT + RC_GRPBOXINDENT - 4,
                           SWP_NOZORDER | SWP_SHOWWINDOW);

            // Update Template position and size
            DeferWindowPos(hdwp, pi->pwndTemplate->m_hWnd, 0, RC_GRPBOXINDENT, posy + RC_PGBUTTONHEIGHT, r.Width() - RC_SCROLLBARWIDTH - (RC_GRPBOXINDENT * 2), tr.Height(),
                           SWP_NOZORDER | SWP_SHOWWINDOW);

            // Update Button's position and size
            DeferWindowPos(hdwp, pi->pwndButton->m_hWnd, 0, RC_GRPBOXINDENT, posy, r.Width() - RC_SCROLLBARWIDTH - (RC_GRPBOXINDENT * 2), RC_PGBUTTONHEIGHT,
                           SWP_NOZORDER | SWP_SHOWWINDOW);
...
LRESULT CALLBACK CRollupCtrl::DlgWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (_this->m_nPageHeight > r.Height()) // Can Scroll ?
    {
        ...
        case WM_SETCURSOR:
            if ((HWND)wParam == hWnd) {
                ::SetCursor(::LoadCursor(nullptr, RC_ROLLCURSOR));
                return TRUE;
            }
            break;
