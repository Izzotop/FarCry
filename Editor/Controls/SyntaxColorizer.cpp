...
void CSyntaxColorizer::createTables() {
    ...
    *(m_pTableZero + '"') = DQSTART;
    *(m_pTableZero + '\'') = SQSTART;
    *(m_pTableZero + '/') = CMSTART;
    *(m_pTableOne + '"') = DQEND;
    *(m_pTableTwo + '\'') = SQEND;
    *(m_pTableThree + '\n') = SLEND;
    *(m_pTableThree + '\r') = SLEND;
    *(m_pTableFour + '*') = MLEND;
    ...
    *(m_pTableZero + pskNewKey->keyword[0]) = KEYWORD;
    ...
    if (m_pskKeyword == nullptr)
        ...
        // check to see if new keyword already exists at the first node
            ...
            while (curr != nullptr && strcmp(curr->keyword, Keyword) < 0) {
                prev = curr;
                curr = curr->pNext;
            }
            if (curr != nullptr && strcmp(curr->keyword, Keyword) == 0) {
                // the keyword exists, so replace the existing with the new
