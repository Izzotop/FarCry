...
BOOL CPreviewModelCtrl::Create(CWnd* pWndParent, const CRect& rc, DWORD dwStyle) {
    BOOL bReturn = CreateEx(nullptr, AfxRegisterWndClass(CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW | CS_OWNDC, AfxGetApp()->LoadStandardCursor(IDC_ARROW), nullptr, nullptr), nullptr, dwStyle, rc,
                            pWndParent, nullptr);
...
void CPreviewModelCtrl::LoadFile(const CString& modelFile, bool changeCamera) {
    ...
    if (stricmp(Path::GetExt(modelFile), "cga") == 0) {
        ...
        // Load object.
        m_object = m_engine->MakeObject(modelFile, nullptr, evs_ShareAndSortForCache);
        if (!m_object) {
            Warning("Loading of geometry object %s failed.", (const char*)modelFile);
            if (m_nTimer != 0)
                KillTimer(m_nTimer);
    ...
    /*
    if (m_nTimer == 0)
            m_nTimer = SetTimer(1,200,nullptr);
    */
