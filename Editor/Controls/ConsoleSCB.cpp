...
static CString popup_helper(HWND hwnd, int x, int y) {
    ...
    int res = TrackPopupMenuEx( hm,TPM_LEFTALIGN|TPM_TOPALIGN|TPM_RETURNCMD,x,y,hwnd,nullptr );
    if (res > 0) {
            return cmds[res-1];
    }
    */
...
BOOL CConsoleEdit::PreTranslateMessage(MSG* msg) {
    if (msg->message == WM_LBUTTONDBLCLK || msg->message == WM_RBUTTONDOWN) {
        ...
        case VK_DOWN: {
            ...
            CPoint pnt;
            GetCursorPos(&pnt);
            int res = ::TrackPopupMenuEx(menu.GetSafeHmenu(), TPM_LEFTALIGN | TPM_TOPALIGN | TPM_RETURNCMD, pnt.x, pnt.y, GetSafeHwnd(), nullptr);
            if (res > 0) {
                str = m_history[res - 1];
            }
            if (!str.IsEmpty()) {
...
int CConsoleSCB::OnCreate(LPCREATESTRUCT lpCreateStruct) {
    ...
    VERIFY(m_cListBox.Create(WS_CHILD | WS_VISIBLE | WS_VSCROLL |
            LBS_NOSEL, rcEdit, this, nullptr));
    m_cListBox.ShowWindow( SW_HIDE );
    */
    m_dialog.Create(CConsoleDialog::IDD, this);

    m_edit.Create(WS_CHILD | WS_VISIBLE | ES_MULTILINE | WS_VSCROLL | ES_AUTOVSCROLL | ES_AUTOHSCROLL | ES_READONLY, rcEdit, &m_dialog, nullptr);
...
void CConsoleSCB::OnSize(UINT nType, int cx, int cy) {
    ...
    /*
            ...
            m_cListBox.SetWindowPos(nullptr, rcEdit.left + 3, rcEdit.top + 3, rcEdit.right - 6,
    ...
    m_edit.SetWindowPos(nullptr, rcEdit.left, rcEdit.top + 2, rcEdit.right, rcEdit.bottom - 1 - inputH, SWP_NOZORDER);

    m_input.SetWindowPos(nullptr, rcEdit.left, rcEdit.bottom - inputH, rcEdit.right, rcEdit.bottom - 2, SWP_NOZORDER);
    m_input.SetFocus();
}
