        ...
        */
        ...
        if (layerInfo.pLayer->IsExternal()) {
            if (!m_externalFont.GetSafeHandle()) {
                VERIFY(m_externalFont.CreateFont(-::MulDiv(8, GetDeviceCaps(dc.m_hDC, LOGPIXELSY), 72), 0, 0, 0, FW_DONTCARE, FALSE, TRUE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
...
void CLayersListBox::Init() {
    ...
    m_hHandCursor = LoadCursor(nullptr, MAKEINTRESOURCE(IDC_HAND));
