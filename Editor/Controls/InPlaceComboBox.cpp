...
int CInPlaceComboBox::OnCreate(LPCREATESTRUCT lpCreateStruct) {
    ...
    CString myClassName = AfxRegisterWndClass(
            CS_VREDRAW | CS_HREDRAW,::LoadCursor(nullptr, IDC_ARROW),(HBRUSH)::GetStockObject(WHITE_BRUSH),nullptr );
...
void CInPlaceComboBox::OnSize(UINT nType, int cx, int cy) {
    ...
    m_wndEdit.SetWindowPos(nullptr, 0, 0, cx - m_nButtonDx, cy, SWP_NOZORDER | SWP_NOMOVE);
}
...
#if 1
    dc.DrawFrameControl(rect, DFC_SCROLL, m_wndDropDown.IsWindowVisible() ? DFCS_SCROLLDOWN | DFCS_PUSHED : DFCS_SCROLLDOWN);
#else
    dc.DrawFrameControl(rect, DFC_SCROLL, m_wndDropDown.IsWindowVisible() ? DFCS_SCROLLDOWN | DFCS_PUSHED | DFCS_FLAT : DFCS_SCROLLDOWN | DFCS_FLAT);
#endif
}
