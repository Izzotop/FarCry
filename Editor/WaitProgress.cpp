...
CWaitProgress::CWaitProgress(UINT nIDText, bool bStart) : m_bStarted(false), m_hwndProgress(nullptr) {
    char szText[1024];
...
CWaitProgress::CWaitProgress(LPCTSTR lpszText, bool bStart) : m_strText(lpszText), m_bStarted(false), m_hwndProgress(nullptr) {
    m_bCancel = false;
    m_bIgnore = false;
    if (bStart)
        Start();
...
void CWaitProgress::CreateProgressControl() {
    ASSERT(m_hwndProgress == nullptr);

    // find status bar
    CWnd* pMainWnd = ::AfxGetMainWnd();
    if (pMainWnd == nullptr)
        return;
    CStatusBar* pStatusBar = DYNAMIC_DOWNCAST(CStatusBar, pMainWnd->GetDescendantWindow(AFX_IDW_STATUS_BAR, TRUE));
    if (pStatusBar == nullptr || pStatusBar->m_hWnd == nullptr)
        return;
    ...
    */
    ...
    m_hwndProgress = ::CreateWindow(PROGRESS_CLASS, "", WS_CHILD | WS_VISIBLE, rc.left + btnWidth, rc.top, rc.Width() - btnWidth, rc.Height(), pStatusBar->m_hWnd, (HMENU)1,
