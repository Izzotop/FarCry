...
inline void SetThumbFileTime(const char* fileName, FILETIME& time) {
    HANDLE hFile = CreateFile(fileName, GENERIC_WRITE, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (INVALID_HANDLE_VALUE != hFile) {
        SetFileTime(hFile, nullptr, nullptr, &time);
        CloseHandle(hFile);
    }
}
...
void CThumbnailGenerator::GenerateForFile(const CString& fileName) {
    ...
    IStatObj* obj = engine->MakeObject(fileName, nullptr, evs_ShareAndSortForCache);
    if (obj) {
        obj->MakeObjectPicture((unsigned char*)image.GetData(), thumbSize);
