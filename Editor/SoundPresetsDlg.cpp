...
CSoundPresetsDlg::CSoundPresetsDlg(CWnd* pParent /*=nullptr*/) : CToolbarDialog(CSoundPresetsDlg::IDD, pParent) {
    m_pSoundPresetMgr = nullptr;
}
...
void CSoundPresetsDlg::OnAddPreset() {
    ...
    if (dlgName.DoModal() == IDOK) {
        if (!m_pSoundPresetMgr->AddPreset(dlgName.GetString()))
            AfxMessageBox("Cannot add preset. Check if a preset with that name exist already and if the name is valid (A-Z, 0-9 (not first character)).",
