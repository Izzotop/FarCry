...
public:
    ...
    int GetObjectHideMask() const {
        return m_objectHideMask;
    }
    ...
    bool IsDisplayLabels() const {
        return (m_flags & SETTINGS_NOLABELS) == 0;
    };

    void DisplayTracks(bool bEnable);
    bool IsDisplayTracks() const {
        return (m_flags & SETTINGS_HIDE_TRACKS) == 0;
    };

    void DisplayLinks(bool bEnable);
    bool IsDisplayLinks() const {
        return (m_flags & SETTINGS_HIDE_LINKS) == 0;
    };

    void DisplayHelpers(bool bEnable);
    bool IsDisplayHelpers() const {
        return (m_flags & SETTINGS_HIDE_HELPERS) == 0;
    };

    void SetLabelsDistance(float dist) {
        m_labelsDistance = dist;
    };
    float GetLabelsDistance() const {
        return m_labelsDistance;
    };
