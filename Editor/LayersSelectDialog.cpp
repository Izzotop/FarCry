...
CLayersSelectDialog::CLayersSelectDialog(CPoint origin, CWnd* pParent /*=nullptr*/) : CDialog(CLayersSelectDialog::IDD, pParent) {
    m_origin = origin;
}
...
void CLayersSelectDialog::ReloadLayers() {
    ...
    m_layers.SetWindowPos(nullptr, 0, 0, layersRc.right - layersRc.left, height, SWP_NOMOVE);

    m_layers.GetWindowRect(layersRc);

    CRect rc;
    GetWindowRect(rc);
    SetWindowPos(nullptr, 0, 0, rc.Width(), layersRc.Height() + 8, SWP_NOMOVE);
...
BOOL CLayersSelectDialog::OnInitDialog() {
    ...
    SetWindowPos(nullptr, m_origin.x, m_origin.y, 0, 0, SWP_NOSIZE);
