...
void CModelViewport::LoadStaticObject(const CString& file) {
    // Load Static object.
    m_object = m_engine->MakeObject(file, nullptr, evs_ShareAndSortForCache);
    if (!m_object) {
...
void CModelViewport::DrawSkyBox() {
    ...
    if (m_pSkyboxName) {
        m_renderer->EF_AddEf(0, m_pRESky, m_pSkyBoxShader, nullptr, pObj, 0, nullptr, EFSLIST_PREPROCESS);
    }
}
...
void CModelViewport::AttachObject(const CString& model, const CString& bone) {
    ...
    */
    ...
    IBindable* pBindable = nullptr;
    m_attachedCharacter = m_pAnimationSystem->MakeCharacter(model);
    if (m_attachedCharacter)
        pBindable = m_attachedCharacter;
    if (!m_attachedCharacter) {
        m_weaponModel = m_engine->MakeObject(model, nullptr, evs_ShareAndSortForCache);
        if (m_weaponModel)
