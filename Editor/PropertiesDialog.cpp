...
CPropertiesDialog::CPropertiesDialog(const CString& title, XmlNodeRef& node, CWnd* pParent /*=nullptr*/) : CXTResizeDialog(CPropertiesDialog::IDD, pParent) {
...
void CPropertiesDialog::OnSize(UINT nType, int cx, int cy) {
    ...
    CRect rc;
    GetClientRect(rc);
    // GetDlgItem(IDOK)->SetWindowPos( nullptr,
    // rc.bottom -= 8;
