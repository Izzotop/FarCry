...
CPreferencesDialog::CPreferencesDialog(CWnd* pParent /*=nullptr*/) : CXTResizeDialog(CPreferencesDialog::IDD, pParent) {
    m_pSelected = nullptr;
...
void CPreferencesDialog::OnGetdispinfoListOptions(NMHDR* pNMHDR, LRESULT* pResult) {
    ...
    if (pTVDispInfo->item.mask & (TVIF_SELECTEDIMAGE | TVIF_IMAGE)) {
        if (m_wndTree.GetChildItem(pTVDispInfo->item.hItem) != nullptr) {
            ...
            pTVDispInfo->item.iSelectedImage = pTVDispInfo->item.iImage = (m_pSelected == (PageInfo*)pTVDispInfo->item.lParam ? 0 : 1);
        }
    }
...
void CPreferencesDialog::OnSelchangedListOptions(NMHDR* pNMHDR, LRESULT* pResult) {
    ...
    if (pNMTreeView->itemNew.hItem) {
        ...
        if (m_pSelected != nullptr && hItemParent) {
