...
public:
    ...
    //////////////////////////////////////////////////////////////////////////
    CTerrainGrid* GetTerrainGrid() const {
        return m_terrainGrid;
    };

    //////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////
    void SetXY(uint x, uint y, t_hmap iVal) {
        m_pHeightmap[x + y * m_iWidth] = iVal;
    };
    t_hmap& GetXY(uint x, uint y) {
        return m_pHeightmap[x + y * m_iWidth];
    };
    t_hmap GetXY(uint x, uint y) const {
        return m_pHeightmap[x + y * m_iWidth];
    };
    ...
    unsigned char& InfoAt(int x, int y) {
        return m_info.ValueAt(x, y);
    };
    const unsigned char& InfoAt(int x, int y) const {
        return m_info.ValueAt(x, y);
    };
    unsigned char* GetInfoData() {
    ...
    //! @param rect if Rectangle is nullptr surface type calculated for whole heightmap.
    void CalcSurfaceTypes(const CRect* rect = nullptr);
    void SetLightingBit(CBitArray& lightbits, int w, int h);
    void ClearLightingBit();

    // Drawing
    void DrawSpot(unsigned long iX, unsigned long iY, uint8 iWidth, float fAddVal, float fSetToHeight = -1.0f, bool bAddNoise = false);
...
inline float CHeightmap::GetSlope(int x, int y) {
    ...
    float h = *p;
    int w = m_iWidth;
    float fs = (fabs(*(p + 1) - h) + fabs(*(p - 1) - h) + fabs(*(p + w) - h) + fabs(*(p - w) - h) + fabs(*(p + w + 1) - h) + fabs(*(p - w - 1) - h) + fabs(*(p + w - 1) - h) +
                fabs(*(p - w + 1) - h));
    fs = fs * 8;
    if (fs > 255.0f)
        fs = 255.0f;
