...
static DWORD StatObjColorArray[] = {0x00FF0000, 0x0000FF00, 0x000000FF, 0x00FFFFFF, 0x00FF00FF, 0x00FFFF00, 0x0000FFFF,
                                    0x007F00FF, 0x007FFF7F, 0x00FF7F00, 0x0000FF7F, 0x007F7F7F, 0x00000000};
...
CTerrainViewport::CTerrainViewport() {
    ...
    VERIFY(m_dcView.CreateCompatibleDC(nullptr));

    // Create a DC and a bitmap
    VERIFY(m_dcTerrain.CreateCompatibleDC(nullptr));
    VERIFY(m_bmpTerrain.CreateBitmap(m_heightmapSize.cx, m_heightmapSize.cy, 1, 32, nullptr));
...
void CTerrainViewport::ResetContent() {
    ...
    DWORD* pSurfaceTextureData = nullptr;
    ...
    for (int j = 0; j < m_heightmapSize.cy; j++)
        for (int i = 0; i < m_heightmapSize.cx; i++) {
            pSurfaceTextureData[i + j * m_heightmapSize.cx] = pWaterTexData[(i % 128) + (j % 128) * 128];
        }
...
void CTerrainViewport::OnPaint() {
    ...
    */
    ...
    if (m_bmpView.GetSafeHandle()) {
        // m_dcView.SelectObject(nullptr);
        ::SelectObject(m_dcView.m_hDC, nullptr);
    ...
    HBITMAP bmpView = ::CreateDIBSection(m_dcView.m_hDC, &BmpInfo, DIB_RGB_COLORS, (void**)&pBits, nullptr, 0);
    ASSERT(bmpView);
    ...
    if (nFlags & MK_LBUTTON) {
        ...
        */
        /*
                ...
                RedrawWindow(&rcRedraw, nullptr, RDW_NOERASE | RDW_INVALIDATE);
                */
        RedrawWindow();
    }
