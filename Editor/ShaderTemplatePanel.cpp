...
CShaderTemplatePanel::CShaderTemplatePanel(CShaderTemplateTool* obj, XmlNodeRef& node, CWnd* pParent /*=nullptr*/)
    : CDialog(CShaderTemplatePanel::IDD, pParent), m_sParameterSetName(_T("")) {
...
void CShaderTemplatePanel::SetProperties(XmlNodeRef& props) {
    ...
    if (GetSafeHwnd()) {
        if (m_node) {
            ...
            // calles OnSize()
            //            SetWindowPos( nullptr,0,0,rc.right,MIN((rc.bottom+yOffset*2),listSize.cy+yOffset*2)+30,SWP_NOMOVE );
        }
    }
}
...
void CShaderTemplatePanel::OnCbnSelchangeTemplatecombo() {
    ...
    if (sshader) {
        ...
        for (int e = 0; e < nParamCount; e++) {
            ...
            case eType_BYTE: {
                CString val;
                val.Format("%d", (int)(ptr->m_Value.m_Byte));
                str += "<" + CString(ptr->m_Name) + " type=\"Int\" value=\"" + val + "\"/>";
            } break;
            case eType_SHORT: {
                CString val;
                val.Format("%d", (int)(ptr->m_Value.m_Short));
                str += "<" + CString(ptr->m_Name) + " type=\"Int\" value=\"" + val + "\"/>";
            } break;
            case eType_INT: {
                CString val;
                val.Format("%d", (int)(ptr->m_Value.m_Int));
                str += "<" + CString(ptr->m_Name) + " type=\"Int\" value=\"" + val + "\"/>";
            } break;
            case eType_FLOAT: {
                CString val;
                val.Format("%f", (float)(ptr->m_Value.m_Float));
                str += "<" + CString(ptr->m_Name) + " type=\"Float\" value=\"" + val + "\" Precision=\"5\"/>";
            } break;
            case eType_STRING: {
                CString val;
                val.Format("%s", (char*)(ptr->m_Value.m_String));
