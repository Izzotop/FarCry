...
CEnvironmentPanel::CEnvironmentPanel(CWnd* pParent /*=nullptr*/) : CDialog(CEnvironmentPanel::IDD, pParent) {
...
BOOL CEnvironmentPanel::OnInitDialog() {
    ...
    if (m_node) {
        ...
        SetWindowPos(nullptr, 0, 0, rc.right, h + HEIGHT_OFFSET * 2 + 34, SWP_NOMOVE);

        GetClientRect(rc);
        CRect rcb;
        m_applyBtn.GetWindowRect(rcb);
        ScreenToClient(rcb);
        m_applyBtn.SetWindowPos(nullptr, rcb.left, rc.bottom - 28, 0, 0, SWP_NOSIZE);
    }
