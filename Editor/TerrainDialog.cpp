...
CTerrainDialog::CTerrainDialog(CWnd* pParent /*=nullptr*/) : CToolbarDialog(CTerrainDialog::IDD, pParent) {
...
BOOL CTerrainDialog::OnInitDialog() {
    ...
    // Create the toolbar
    if (!m_cDlgToolBar.CreateEx(this, TBSTYLE_FLAT | TBSTYLE_WRAPABLE, WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC) ||
    ...
    m_cDlgToolBar.SetWindowPos(nullptr, 0, 0, rcClient.right, 70, SWP_NOZORDER);

    // TODO: Remove this if you don't want tool tips or a resizeable toolbar
    m_cDlgToolBar.SetBarStyle(m_cDlgToolBar.GetBarStyle() | CBRS_TOOLTIPS | CBRS_FLYBY);

    // Create the brush toolbar
    if (!m_cDlgBrushToolBar.CreateEx(this, TBSTYLE_FLAT | TBSTYLE_WRAPABLE, WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC) ||
    ...
    // Resize the toolbar
    GetClientRect(&rcClient);
    m_cDlgBrushToolBar.SetWindowPos(nullptr, 0, 0, rcClient.right, 70, SWP_NOZORDER);

    // TODO: Remove this if you don't want tool tips or a resizeable toolbar
    m_cDlgBrushToolBar.SetBarStyle(m_cDlgBrushToolBar.GetBarStyle() | CBRS_TOOLTIPS | CBRS_FLYBY);
    ...
    m_cDrawHeightmap.SetWindowPos(nullptr, 20, 20, 512, 512, SWP_NOZORDER);
...
void CTerrainDialog::OnTerrainLoad() {
    ...
    CFileDialog dlg(TRUE, nullptr, nullptr, OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR, szFilters);

    if (dlg.DoModal() == IDOK) {
        char ext[_MAX_EXT];
        _splitpath(dlg.GetPathName(), nullptr, nullptr, nullptr, ext);
...
void CTerrainDialog::UpdateBorderCaption() {
    ...
    sprintf(szCaption, "Heightmap (%ix%i viewed in 512x512)", m_heightmap->GetWidth(), m_heightmap->GetHeight());
...
void CTerrainDialog::OnTerrainErase() {
    ...
    // Ask first
    if (AfxMessageBox("Really erase the heightmap ?", MB_ICONQUESTION | MB_YESNO, nullptr) != IDYES)
        return;
...
void CTerrainDialog::OnExportHeightmap() {
    ...
    CFileDialog dlg(FALSE, "bmp", nullptr, OFN_OVERWRITEPROMPT | OFN_NOCHANGEDIR, szFilters);
...
void CTerrainDialog::OnHeightmapShowLargePreview() {
    ...
    if (bOk) {
        ...
        // Show the heightmap
        ::ShellExecute(::GetActiveWindow(), "open", "HeightmapPreview.bmp", "", dir, SW_SHOWMAXIMIZED);
    }
}
...
void CTerrainDialog::OnFetch() {
    ...
    if (GetIEditor()->IsModified()) {
        // Ask first
        iResult = MessageBox("Do you really want to restore the previous heightmap state ?", "Fetch", MB_YESNO | MB_ICONQUESTION);
...
void CTerrainDialog::OnOptionsShowMapObjects() {
    ...
    // Toggle it
    GetMenu()->GetSubMenu(4)->CheckMenuItem(ID_OPTIONS_SHOWMAPOBJECTS, (iState & MF_CHECKED) ? MF_UNCHECKED : MF_CHECKED);
}
...
void CTerrainDialog::OnOptionsShowWater() {
    ...
    // Toggle it
    GetMenu()->GetSubMenu(4)->CheckMenuItem(ID_OPTIONS_SHOWWATER, (iState & MF_CHECKED) ? MF_UNCHECKED : MF_CHECKED);
}
...
void CTerrainDialog::OnExportTerrainAsGeometrie() {
    ...
    if (dlg.DoModal() == IDOK) {
        ...
        ::SetRect(&rcExport, ptMarker.x - (iAreaSize / 2), ptMarker.y - (iAreaSize / 2), ptMarker.x + (iAreaSize / 2), ptMarker.y + (iAreaSize / 2));
