...
void FillSolidRect(CDC* pDC, LPCRECT lpRect, COLORREF clr) {
    ASSERT_VALID(pDC);
    ASSERT(pDC->m_hDC != nullptr);

    pDC->SetBkColor(clr);
    pDC->ExtTextOut(0, 0, ETO_OPAQUE, lpRect, nullptr, 0, nullptr);
}
...
CTipDlg::CTipDlg(CWnd* pParent /*=nullptr*/) : CDialog(IDD_TIP, pParent) {
    ...
    CString strStoredTime = pApp->GetProfileString(szSection, szTimeStamp, nullptr);
...
void CTipDlg::DoPaint(CDC* pdc, BOOL updatetext) {
    ...
    CWnd* pStatic;
    CRect tiprect, bigrect, seprect, didyourect, bulbrect;
...
#if 1
        pdc->BitBlt(bulbrect.left, bulbrect.top, bulbrect.Width(), bulbrect.Height(), &dcTmp, 0, 0, SRCCOPY);
#endif
    ...
    CFont font, fonttext;
    BOOL fok = font.CreateFont(22, 0, 0, 0, FW_BOLD, 0, 0, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, "Times");

    fok = fonttext.CreateFont(16, 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, "Arial");
